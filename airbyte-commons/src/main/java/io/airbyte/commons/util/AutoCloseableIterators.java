...
public class AutoCloseableIterators {
  ...
  /**
   * Coerces a vanilla {@link Iterator} into a {@link AutoCloseableIterator}. The provided {@param
  ...
  public static <T> AutoCloseableIterator<T> fromIterator(
      final Iterator<T> iterator, final VoidCallable onClose) {
    return new DefaultAutoCloseableIterator<>(iterator, onClose::call);
  }

  /**
   * Wraps a {@link Stream} in a {@link AutoCloseableIterator}. The first time {@link
   * AutoCloseableIterator#close()} is called, {@link Stream#close()} will be called. It will not be
  ...
  public static <F, T> AutoCloseableIterator<T> transform(
    ...
    return new DefaultAutoCloseableIterator<>(
        Iterators.transform(fromIterator, function::apply), fromIterator::close);
  }

  /**
   * Map over a {@link AutoCloseableIterator} using a vanilla {@link Iterator} while retaining all
   * of the Resource behavior of the input {@link AutoCloseableIterator}.
   *
  ...
  public static <T> AutoCloseableIterator<T> transform(
    ...
    return new DefaultAutoCloseableIterator<>(
        iteratorCreator.apply(autoCloseableIterator), autoCloseableIterator::close);
  }
