...
public class SentryExceptionHelper {
  ...
  public static Map<ErrorMapKeys, String> getUsefulErrorMessageAndTypeFromDbtError(
    ...
    // TODO: this whole code block is quite ugh, commented to try and make each part clear but could
    // be
    // much more readable.
    mainLoop:
      ...
      try {
        ...
        if (stacktraceLines[i].contains("Database Error in model")) {
          ...
          if (stacktraceLines[i + 1].contains("SQL compilation error")) {
            errorMessageAndType.put(
                ErrorMapKeys.ERROR_MAP_MESSAGE_KEY,
                String.format(
                    "%s %s", stacktraceLines[i + 1].trim(), stacktraceLines[i + 2].trim()));
            errorMessageAndType.put(
                ErrorMapKeys.ERROR_MAP_TYPE_KEY, "DbtDatabaseSQLCompilationError");
            break;
            // Database Error: Invalid input
          } else if (stacktraceLines[i + 1].contains("Invalid input")) {
            for (final String followingLine :
                Arrays.copyOfRange(stacktraceLines, i + 1, stacktraceLines.length)) {
              if (followingLine.trim().startsWith("context:")) {
                errorMessageAndType.put(
                    ErrorMapKeys.ERROR_MAP_MESSAGE_KEY,
                    String.format("%s\n%s", stacktraceLines[i + 1].trim(), followingLine.trim()));
                errorMessageAndType.put(
                    ErrorMapKeys.ERROR_MAP_TYPE_KEY, "DbtDatabaseInvalidInputError");
                break mainLoop;
              }
            }
          ...
          if (stacktraceLines[i + 1].contains(
            ...
            errorMessageAndType.put(
                ...
                String.format(
                    "%s %s", stacktraceLines[i + 1].trim(), stacktraceLines[i + 2].trim()));
            errorMessageAndType.put(
                ErrorMapKeys.ERROR_MAP_TYPE_KEY, "DbtCompilationAmbiguousRelationError");
            break;
            ...
            if ("Database Error".equals(followingLine.trim())) {
              errorMessageAndType.put(
                  ...
                  String.format(
                      "%s",
                      stacktraceLines[
                          Arrays.stream(stacktraceLines).toList().indexOf(followingLine) + 1]
                          .trim()));
          ...
          defaultNextLine = true;
          // Database Error: formatted differently, catch last to avoid counting other types of
          // errors as
          // Database Error
        ...
        if (defaultNextLine) {
          errorMessageAndType.put(
              ErrorMapKeys.ERROR_MAP_MESSAGE_KEY, stacktraceLines[i + 1].trim());
          break;
        }
