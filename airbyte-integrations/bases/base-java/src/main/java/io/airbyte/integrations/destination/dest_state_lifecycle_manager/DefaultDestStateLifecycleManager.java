...
/**
 * Detects the type of the state being received by anchoring on the first state type it sees. Fail if receives states of multiple types--each instance
 * of this class can only support state messages of one type. The protocol specifies that a source should emit state messages of a single type during
 * a sync, so a single instance of this manager is sufficient for a destination to track state during a sync.
 *
...
public class DefaultDestStateLifecycleManager implements DestStateLifecycleManager {
  ...
  /**
   * Given the type of previously recorded state by the state manager, determines if a newly added state message's type is compatible. Based on the
   * previously set state type, determines if a new one is compatible. If the previous state is null, any new state is compatible. If new state type
   * is null, it should be treated as LEGACY. Thus, previousStateType == LEGACY and newStateType == null IS compatible. All other state types are
   * compatible based on equality.
   *
  ...
  /**
   * If the state type for the manager is not set, sets it using the state type from the message. If the type on the message is null, we assume it is
   * LEGACY. After the first, state message is added to the manager, the state type is set and is immutable.
   *
