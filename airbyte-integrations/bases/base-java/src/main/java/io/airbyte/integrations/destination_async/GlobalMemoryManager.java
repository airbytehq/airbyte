...
public class GlobalMemoryManager {
  ...
  // In cases where a queue is rapidly expanding, a larger block size allows less allocation calls.
  // On
  ...
  public synchronized long requestMemory() {
    ...
    log.debug(
        "Memory Requested: max: {}, allocated: {}, allocated in this request: {}",
  ...
  public void free(final long bytes) {
    ...
    if (currentMemoryBytes.get() < 0) {
      log.warn(
          "Freed more memory than allocated. This should never happen. Please report this bug.");
    }
  }
}
