...
echo '{"type": "LOG","log":{"level":"INFO","message":"Destination process terminated (exit code '"$destination_exit_code"')"}}'
...
then
  ...
  echo '{"type": "LOG","log":{"level":"INFO","message":"In-connector normalization terminated (exit code '"$normalization_exit_code"')"}}'
else
