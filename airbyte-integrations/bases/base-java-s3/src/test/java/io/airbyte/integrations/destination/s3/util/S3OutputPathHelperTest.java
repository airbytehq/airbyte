...
class S3OutputPathHelperTest {
  ...
  public void testGetOutputPrefix() {
    ...
    assertEquals("bucket_path/stream_name", S3OutputPathHelper
        .getOutputPrefix("bucket_path", new AirbyteStream().withName("stream_name").withSupportedSyncModes(Lists.newArrayList(SyncMode.FULL_REFRESH))));

    // With namespace
    assertEquals("bucket_path/namespace/stream_name", S3OutputPathHelper
        .getOutputPrefix("bucket_path",
            new AirbyteStream().withNamespace("namespace").withName("stream_name").withSupportedSyncModes(Lists.newArrayList(SyncMode.FULL_REFRESH))));
    ...
    assertEquals("bucket_path/namespace/subfolder/stream_name", S3OutputPathHelper
        .getOutputPrefix("bucket_path",
            new AirbyteStream().withNamespace("namespace/subfolder/").withName("stream_name").withSupportedSyncModes(Lists.newArrayList(SyncMode.FULL_REFRESH))));

    // With an AWS Glue crawler
    assertEquals("bucket_path/namespace/date=2022-03-15", S3OutputPathHelper
        .getOutputPrefix("bucket_path",
            new AirbyteStream().withNamespace("namespace").withName("date=2022-03-15").withSupportedSyncModes(Lists.newArrayList(SyncMode.FULL_REFRESH))));
  }
