...
public class JsonToAvroSchemaConverter {
  ...
  public Schema getAvroSchema(
      final JsonNode jsonSchema, final String streamName, @Nullable final String namespace) {
    ...
    for (final String subfieldName : subfieldNames) {
      ...
      if (!stdFieldName.equals(subfieldName)) {
        ...
        LOGGER.warn(
            ...
            subfieldName,
            stdFieldName);
        fieldBuilder.doc(String.format(
            "%s%s%s",
      ...
      final String subfieldNamespace = isRootNode
          // Omit the namespace for root level fields, because it is directly assigned in the
          // builder above.
  ...
  Schema parseSingleType(
    ...
    switch (fieldType) {
      case INTEGER_V1,
          ...
          BINARY_DATA_V1 -> fieldSchema = Schema.create(fieldType.getAvroType());
      case DATE_V1 -> fieldSchema = LogicalTypes.date().addToSchema(Schema.create(Schema.Type.INT));
      case TIMESTAMP_WITH_TIMEZONE_V1, TIMESTAMP_WITHOUT_TIMEZONE_V1 -> fieldSchema =
          LogicalTypes.timestampMicros().addToSchema(Schema.create(Schema.Type.LONG));
      case TIME_WITHOUT_TIMEZONE_V1 -> fieldSchema =
      ...
      case COMBINED -> {
        final Optional<JsonNode> combinedRestriction = getCombinedRestriction(fieldDefinition);
        final List<Schema> unionTypes = parseJsonTypeUnion(
            fieldName,
            fieldNamespace,
            (ArrayNode) combinedRestriction.get(),
            appendExtraProps,
            addStringToLogicalTypes);
      ...
      case ARRAY -> {
        ...
        if (items == null) {
          ...
          if ((items.has("type") && !items.get("type").isNull())
            ...
            fieldSchema = Schema.createArray(parseJsonField(
                elementFieldName,
                fieldNamespace,
                items,
                appendExtraProps,
          ...
          final List<Schema> arrayElementTypes = parseJsonTypeUnion(
              fieldName,
              fieldNamespace,
              (ArrayNode) items,
              appendExtraProps,
      ...
      case OBJECT -> fieldSchema = getAvroSchema(
          fieldDefinition,
          fieldName,
          fieldNamespace,
          false,
   ...
   * </pre>
   */
  List<Schema> parseJsonTypeUnion(
      final String fieldName,
      @Nullable final String fieldNamespace,
      final ArrayNode types,
      final boolean appendExtraProps,
      final boolean addStringToLogicalTypes) {
    final List<Schema> schemas = MoreIterators.toList(types.elements()).stream()
        .flatMap(definition -> getNonNullTypes(fieldName, definition).stream().flatMap(type -> {
          ...
          final Schema singleFieldSchema = parseSingleType(
              fieldName, namespace, type, definition, appendExtraProps, addStringToLogicalTypes);
   ...
   * </pre>
   */
  List<Schema> mergeRecordSchemas(
      final String fieldName,
      @Nullable final String fieldNamespace,
    ...
    if (!recordFieldSchemas.isEmpty()) {
      ...
      for (final Map.Entry<String, List<Schema>> entry : recordFieldSchemas.entrySet()) {
        ...
        final List<String> subfieldDocs =
            recordFieldDocs.getOrDefault(subfieldName, Collections.emptyList());
        ...
        final List<Schema> subfieldSchemas = entry.getValue().stream()
            ...
            .distinct()
            .collect(Collectors.toList());
        final String subfieldNamespace =
            fieldNamespace == null ? fieldName : (fieldNamespace + "." + fieldName);
        // recursively merge schemas of a subfield because they may include multiple record schemas
        // as well
        final List<Schema> mergedSubfieldSchemas =
            mergeRecordSchemas(subfieldName, subfieldNamespace, subfieldSchemas, appendExtraProps);
  ...
  Schema parseJsonField(
    ...
    final List<Schema> nonNullFieldTypes = getNonNullTypes(fieldName, fieldDefinition).stream()
        .flatMap(fieldType -> {
          final Schema singleFieldSchema = parseSingleType(
              fieldName,
              fieldNamespace,
    ...
    if (nonNullFieldTypes.isEmpty()) {
      ...
      if ((nonNullFieldTypes.stream().anyMatch(schema -> schema.getLogicalType() != null))
  ...
  private Schema createUnionAndCheckLongTypesDuplications(List<Schema> unionTypes) {
    ...
    Predicate<Schema> isTimestampMicrosLong =
        isALong.and(type -> Objects.nonNull(type.getLogicalType())
            && "timestamp-micros".equals(type.getLogicalType().getName()));
    ...
    Predicate<Schema> removeTimestampType =
        type -> !(hasPlainLong && hasTimestampMicrosLong && isTimestampMicrosLong.test(type));
    return Schema.createUnion(
        unionTypes.stream().filter(removeTimestampType).collect(Collectors.toList()));
  }
}
