...
def get_test_failure_logs(definition):
    ...
    if config.MODULE_NAME == "fail_on_extra_columns":
        ...
        try:
            with open(f"{utils.MIGRATIONS_FOLDER}/{config.MODULE_NAME}/test_failure_logs/{connector_technical_name}") as f:
                for line in f:
...
def get_issue_content(source_definition) -> Optional[Dict[str, Any]]:
