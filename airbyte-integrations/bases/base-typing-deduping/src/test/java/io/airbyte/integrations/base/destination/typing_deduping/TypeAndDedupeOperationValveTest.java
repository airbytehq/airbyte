...
public class TypeAndDedupeOperationValveTest {
   ...
   @BeforeEach
   public void setup() { AtomicLong start = new AtomicLong(0); minuteUpdates = () -> start.getAndUpdate(l -> l + (60 * 1000)); }

   private void elapseTime(Supplier<Long> timing, int iterations) { IntStream.range(0, iterations).forEach(__ -> { timing.get(); }); }
    @Test
   ...
   @Test public void testReadyToTypeAndDedupe() { final var valve = new
       TypeAndDedupeOperationValve(minuteUpdates);
   ...
   @Test public void testIncrementInterval() {
       final var valve = new TypeAndDedupeOperationValve(ALWAYS_ZERO); valve.addStream(STREAM_A);
       IntStream.rangeClosed(1, 3).forEach(i -> { final var index = valve.incrementInterval(STREAM_A);
   ...
   @Test public void testUpdateTimeAndIncreaseInterval() {
       final var valve = new TypeAndDedupeOperationValve(minuteUpdates); valve.addStream(STREAM_A);
       // 2 minutes
   IntStream.range(0, 98).forEach(__ -> Assertions.assertFalse(valve.readyToTypeAndDedupeWithAdditionalRecord(STREAM_A)));
   elapseTime(minuteUpdates, 2);
   Assertions.assertTrue(valve.readyToTypeAndDedupeWithAdditionalRecord(STREAM_A));
   valve.updateTimeAndIncreaseInterval(STREAM_A);
   // 5 minutes
   IntStream.range(0, 99).forEach(__ -> Assertions.assertFalse(valve.readyToTypeAndDedupeWithAdditionalRecord(STREAM_A)));
