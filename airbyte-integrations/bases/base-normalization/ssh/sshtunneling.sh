...
# Requires one argument, path to config file ($1)
function openssh() {
  ...
  tunnel_username=$(cat $1 | jq -r '.tunnel_method.tunnel_username')
  tunnel_localport=$(cat $1 | jq -r '.tunnel_method.tunnel_local_port')
  tunnel_db_remote_host=$(cat $1 | jq -r '.tunnel_method.tunnel_db_remote_host')
  tunnel_db_remote_port=$(cat $1 | jq -r '.tunnel_method.tunnel_db_remote_port')
  ...
  if [[ ${tunnel_method} = "SSH_KEY_AUTH" ]] ; then
    ...
    echo "$(cat $1 | jq -r '.tunnel_method.tunnel_user_ssh_key')" > $tmpkeyfile
    # -f=background  -N=no remote command  -M=master mode  StrictHostKeyChecking=no auto-adds host
    echo "Running: ssh -f -N -M -o StrictHostKeyChecking=no -S {control socket} -i {key file} -l ${tunnel_username} -L ${tunnel_localport}:${tunnel_db_remote_host}:${tunnel_db_remote_port} ${tunnel_host}"
    ssh -f -N -M -o StrictHostKeyChecking=no -S $tmpcontrolsocket -i $tmpkeyfile -l ${tunnel_username} -L ${tunnel_localport}:${tunnel_db_remote_host}:${tunnel_db_remote_port} ${tunnel_host} &&
    ...
    export SSHPASS=$(cat $1 | jq -r '.tunnel_method.tunnel_userpass')
    echo "Running: sshpass -e ssh -f -N -M -o StrictHostKeyChecking=no -S {control socket} -l ${tunnel_username} -L ${tunnel_localport}:${tunnel_db_remote_host}:${tunnel_db_remote_port} ${tunnel_host}"
    sshpass -e ssh -f -N -M -o StrictHostKeyChecking=no -S $tmpcontrolsocket -l ${tunnel_username} -L ${tunnel_localport}:${tunnel_db_remote_host}:${tunnel_db_remote_port} ${tunnel_host} &&
    echo "ssh tunnel opened"
    sshopen="yes"
  fi
}
