...
union all
    select 'some_stream_that_was_empty_scd' as label, count(*) as row_count, 3 as expected_count
    from {{ ref('some_stream_that_was_empty_scd') }}
union all
    select 'some_stream_that_was_empty' as label, count(*) as row_count, 3 as expected_count
