...
union all
    ...
    select distinct '_airbyte_raw_dedup_exchange_rate' as label, count(*) as row_count, 12 as expected_count
...
union all
    ...
    select distinct '_airbyte_raw_dedup_cdc_excluded' as label, count(*) as row_count, 11 as expected_count
...
union all
    select distinct 'dedup_cdc_excluded' as label, count(*) as row_count, 5 as expected_count
    from {{ ref('dedup_cdc_excluded') }}

union all

    select distinct '_airbyte_raw_pos_dedup_cdcx' as label, count(*) as row_count, 8 as expected_count
