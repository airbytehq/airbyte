...
union all
    select distinct 'exchange_rate' as label, count(*) as row_count, 16 as expected_count
    from {{ ref('exchange_rate') }}

union all

    select distinct '_airbyte_raw_dedup_exchange_rate' as label, count(*) as row_count, 8 as expected_count
...
union all
    select distinct 'dedup_exchange_rate' as label, count(*) as row_count, 10 as expected_count
...
union all
    select distinct 'renamed_dedup_cdc_excluded_scd' as label, count(*) as row_count, 3 as expected_count
    from {{ ref('renamed_dedup_cdc_excluded_scd') }}
union all
    select distinct 'renamed_dedup_cdc_excluded' as label, count(*) as row_count, 1 as expected_count
    from {{ ref('renamed_dedup_cdc_excluded') }}
)
select *
