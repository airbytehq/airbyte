{{ config(
    ...
    schema = "_airbyte_test_normalization_lfocr",
    tags = [ "top-level-intermediate" ]
) }}
-- SQL model to parse JSON blob stored in a single column and extract into separated field columns as described by the JSON Schema
-- depends_on: {{ source('test_normalization_lfocr', '_airbyte_raw_dedup_exchange_rate') }}
select
...
from {{ source('test_normalization_lfocr', '_airbyte_raw_dedup_exchange_rate') }} as table_alias
-- dedup_exchange_rate
where 1 = 1
