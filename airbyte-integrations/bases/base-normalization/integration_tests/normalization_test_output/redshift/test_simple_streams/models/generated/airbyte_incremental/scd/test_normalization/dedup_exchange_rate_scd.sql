{{ config(
    ...
    schema = "test_normalization_lfocr",
    post_hook = ["
                    {%
                    ...
                    {% endif %}
                    ","drop view _airbyte_test_normalization_lfocr.dedup_exchange_rate_stg"],
    tags = [ "top-level" ]
) }}
...
new_data as (
    ...
    -- dedup_exchange_rate from {{ source('test_normalization_lfocr', '_airbyte_raw_dedup_exchange_rate') }}
    where 1 = 1
