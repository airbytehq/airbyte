...
select
    case when json_extract_path_text(_airbyte_data, 'id', true) != '' then json_extract_path_text(_airbyte_data, 'id', true) end as id,
        ...
        case when json_extract_path_text(table_alias._airbyte_data, 'conflict_stream_name', true) != '' then json_extract_path_text(table_alias._airbyte_data, 'conflict_stream_name', true) end
...
select
    md5(cast(coalesce(cast(id as varchar), '') || '-' || coalesce(cast(conflict_stream_name as varchar), '') as varchar)) as _airbyte_conflict_stream_name_hashid,
    tmp.*
