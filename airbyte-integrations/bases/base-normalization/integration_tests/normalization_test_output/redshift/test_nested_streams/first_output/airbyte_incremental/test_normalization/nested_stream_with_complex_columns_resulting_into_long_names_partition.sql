...
select
    md5(cast(coalesce(cast(_airbyte_nested_stream_with_complex_columns_resulting_into_long_names_hashid as varchar), '') || '-' || coalesce(cast(json_serialize(double_array_data) as varchar), '') || '-' || coalesce(cast(json_serialize(data) as varchar), '') as varchar)) as _airbyte_partition_hashid,
    tmp.*
