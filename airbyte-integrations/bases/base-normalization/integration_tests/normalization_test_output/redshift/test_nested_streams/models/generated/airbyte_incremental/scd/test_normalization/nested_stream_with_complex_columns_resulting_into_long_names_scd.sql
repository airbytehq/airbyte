{{ config(
    ...
    schema = "test_normalization_lpgvp",
    post_hook = ["
                    {%
                    ...
                    {% endif %}
                    ","drop view _airbyte_test_normalization_lpgvp.nested_stream_with_complex_columns_resulting_into_long_names_stg"],
    tags = [ "top-level" ]
) }}
...
new_data as (
    ...
    -- nested_stream_with_complex_columns_resulting_into_long_names from {{ source('test_normalization_lpgvp', '_airbyte_raw_nested_stream_with_complex_columns_resulting_into_long_names') }}
    where 1 = 1
