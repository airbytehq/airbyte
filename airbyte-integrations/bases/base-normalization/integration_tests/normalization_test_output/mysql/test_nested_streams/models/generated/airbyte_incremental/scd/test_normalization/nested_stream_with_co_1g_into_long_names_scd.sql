...
--                              inner join modified_ids on scd_table._airbyte_unique_key = modified_ids._airbyte_unique_key
                                ...
                                right outer join (
                                    ...
                                    where 1=1
                                        {{ incremental_clause('_airbyte_emitted_at', this.schema + '.' + adapter.quote('nested_stream_with_co__lting_into_long_names')) }}
                                ) modified_ids
                        ...
                        {% endif %}
                        ","drop view {{ ref('nested_stream_with_co_1ng_names_scd_new_data') }}","drop view _airbyte_test_normalization.nested_stream_with_co_1g_into_long_names_stg"],
    tags = [ "top-level" ]
) }}
-- depends on: {{ ref('nested_stream_with_co_1ng_names_scd_new_data') }}
with
{% if is_incremental() %}
new_data_ids as (
...
empty_new_data as (
    ...
    select * from {{ ref('nested_stream_with_co_1ng_names_scd_new_data') }} where 1 = 0
),
...
input_data as (
    select {{ dbt_utils.star(ref('nested_stream_with_co_1g_into_long_names_stg')) }} from {{ ref('nested_stream_with_co_1ng_names_scd_new_data') }}
    union all
