...
new_data as (
    ...
    select
        *
    from {{ ref('nested_stream_with_co_1g_into_long_names_tmp')  }}
...
previous_active_scd_data as (
    ...
    select
        {{ star_intersect(ref('nested_stream_with_co_1g_into_long_names_tmp'), this, from_alias='inc_data', intersect_alias='this_data') }}
...
input_data as (
    select {{ dbt_utils.star(ref('nested_stream_with_co_1g_into_long_names_tmp')) }} from new_data
    union all
    select {{ dbt_utils.star(ref('nested_stream_with_co_1g_into_long_names_tmp')) }} from previous_active_scd_data
),
{% else %}
input_data as (
    select *
