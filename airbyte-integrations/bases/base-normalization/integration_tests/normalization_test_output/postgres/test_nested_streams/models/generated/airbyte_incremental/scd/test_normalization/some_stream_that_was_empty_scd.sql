    ...
    post_hook = ["
                    ...
                    delete from {{ final_table_relation }} where {{ final_table_relation }}._airbyte_unique_key in (
                        ...
                        from (
                            ...
                            from {{ this }}
                            where _airbyte_active_row = 0 {{ incremental_clause('_airbyte_normalized_at', this.schema + '.' + adapter.quote('some_stream_that_was_empty')) }}
                    ...
                    {% endif %}
                    ","delete from _airbyte_test_normalization.some_stream_that_was_empty_stg where _airbyte_emitted_at != (select max(_airbyte_emitted_at) from _airbyte_test_normalization.some_stream_that_was_empty_stg)"],
    tags = [ "top-level" ]
) }}
...
input_data as (
    select {{ dbt_utils.star(ref('some_stream_that_was_empty_stg')) }} from new_data
    union all
