{{ config(
    ...
    post_hook = ["
                        ...
                        set final_table_relation = adapter.get_relation(
                                ...
                                schema=this.schema,
                                identifier='some_stream_that_was_empty'
                            )
                        %}
                        {#
...
--                              inner join modified_ids on scd_table._airbyte_unique_key = modified_ids._airbyte_unique_key
                                ...
                                right outer join (
                                    ...
                                    where 1=1
                                        {{ incremental_clause('_airbyte_emitted_at', this.schema + '.' + adapter.quote('some_stream_that_was_empty')) }}
                                ) modified_ids
...
empty_new_data as (
    ...
    select * from {{ ref('some_stream_that_was_empty_scd_new_data') }} where 1 = 0
),
previous_active_scd_data as (
    -- retrieve "incomplete old" data that needs to be updated with an end date because of new changes
    select
        {{ star_intersect(ref('some_stream_that_was_empty_stg'), this, from_alias='inc_data', intersect_alias='this_data') }}
...
input_data as (
    select {{ dbt_utils.star(ref('some_stream_that_was_empty_stg')) }} from {{ ref('some_stream_that_was_empty_scd_new_data') }}
    union all
    select {{ dbt_utils.star(ref('some_stream_that_was_empty_stg')) }} from previous_active_scd_data
),
{% else %}
input_data as (
    select *
