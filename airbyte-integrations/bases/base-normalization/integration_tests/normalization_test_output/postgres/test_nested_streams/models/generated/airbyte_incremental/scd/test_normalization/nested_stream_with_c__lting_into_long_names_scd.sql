{{ config(
    ...
    post_hook = ["
                        ...
                        set final_table_relation = adapter.get_relation(
                                ...
                                schema=this.schema,
                                identifier='nested_stream_with_c__lting_into_long_names'
                            )
                        %}
                        {#
...
--                              inner join modified_ids on scd_table._airbyte_unique_key = modified_ids._airbyte_unique_key
                                ...
                                right outer join (
                                    ...
                                    from {{ ref('nested_stream_with_c__lting_into_long_names_scd_new_data') }}
                                    where 1=1
                                        {{ incremental_clause('_airbyte_emitted_at', this.schema + '.' + adapter.quote('nested_stream_with_c__lting_into_long_names')) }}
                                ) modified_ids
                        ...
                        {% endif %}
                        ","delete from {{ ref('nested_stream_with_c__lting_into_long_names_scd_new_data') }} where _airbyte_emitted_at != (select max(_airbyte_emitted_at) from {{ ref('nested_stream_with_c__lting_into_long_names_scd_new_data') }})","delete from _airbyte_test_normalization.nested_stream_with_c__lting_into_long_names_stg where _airbyte_emitted_at != (select max(_airbyte_emitted_at) from _airbyte_test_normalization.nested_stream_with_c__lting_into_long_names_stg)"],
    tags = [ "top-level" ]
) }}
-- depends on: {{ ref('nested_stream_with_c__lting_into_long_names_scd_new_data') }}
with
{% if is_incremental() %}
new_data_ids as (
...
empty_new_data as (
    ...
    select * from {{ ref('nested_stream_with_c__lting_into_long_names_scd_new_data') }} where 1 = 0
),
previous_active_scd_data as (
    -- retrieve "incomplete old" data that needs to be updated with an end date because of new changes
    select
        {{ star_intersect(ref('nested_stream_with_c__lting_into_long_names_stg'), this, from_alias='inc_data', intersect_alias='this_data') }}
...
input_data as (
    select {{ dbt_utils.star(ref('nested_stream_with_c__lting_into_long_names_stg')) }} from {{ ref('nested_stream_with_c__lting_into_long_names_scd_new_data') }}
    union all
    select {{ dbt_utils.star(ref('nested_stream_with_c__lting_into_long_names_stg')) }} from previous_active_scd_data
),
{% else %}
input_data as (
    select *
