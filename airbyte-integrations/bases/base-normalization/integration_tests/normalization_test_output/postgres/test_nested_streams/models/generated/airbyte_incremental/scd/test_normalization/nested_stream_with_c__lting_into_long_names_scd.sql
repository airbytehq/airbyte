{{ config(
    ...
    post_hook = ["
                    ...
                    {% endif %}
                    ","delete from _airbyte_test_normalization.nested_stream_with_c__lting_into_long_names_stg where _airbyte_emitted_at != (select max(_airbyte_emitted_at) from _airbyte_test_normalization.nested_stream_with_c__lting_into_long_names_stg)"],
    tags = [ "top-level" ]
) }}
...
input_data as (
    select {{ dbt_utils.star(ref('nested_stream_with_c__lting_into_long_names_stg')) }} from new_data
    union all
