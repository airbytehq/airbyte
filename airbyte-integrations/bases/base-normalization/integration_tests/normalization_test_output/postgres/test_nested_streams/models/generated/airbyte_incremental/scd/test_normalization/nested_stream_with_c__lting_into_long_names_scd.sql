...
new_data as (
    ...
    select
        *
    from {{ ref('nested_stream_with_c__lting_into_long_names_ab3')  }}
...
previous_active_scd_data as (
    ...
    select
        {{ star_intersect(ref('nested_stream_with_c__lting_into_long_names_ab3'), this, from_alias='inc_data', intersect_alias='this_data') }}
    ...
    left join {{ ref('nested_stream_with_c__lting_into_long_names_ab3')  }} as inc_data on 1 = 0
    where _airbyte_active_row = 1
),
input_data as (
    select {{ dbt_utils.star(ref('nested_stream_with_c__lting_into_long_names_ab3')) }} from new_data
    union all
    select {{ dbt_utils.star(ref('nested_stream_with_c__lting_into_long_names_ab3')) }} from previous_active_scd_data
),
{% else %}
input_data as (
    select *
