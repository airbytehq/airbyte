...
--                              inner join modified_ids on scd_table._airbyte_unique_key = modified_ids._airbyte_unique_key
                                ...
                                right outer join (
                                    ...
                                    from {{ ref('renamed_dedup_cdc_excluded_scd_new_data') }}
                                    where 1=1
                                        {{ incremental_clause('_airbyte_emitted_at', this.schema + '.' + adapter.quote('renamed_dedup_cdc_excluded')) }}
                                ) modified_ids
                        ...
                        {% endif %}
                        ","delete from {{ ref('renamed_dedup_cdc_excluded_scd_new_data') }} where _airbyte_emitted_at != (select max(_airbyte_emitted_at) from {{ ref('renamed_dedup_cdc_excluded_scd_new_data') }})","delete from _airbyte_test_normalization.renamed_dedup_cdc_excluded_stg where _airbyte_emitted_at != (select max(_airbyte_emitted_at) from _airbyte_test_normalization.renamed_dedup_cdc_excluded_stg)"],
    tags = [ "top-level" ]
) }}
-- depends on: {{ ref('renamed_dedup_cdc_excluded_scd_new_data') }}
with
{% if is_incremental() %}
new_data_ids as (
...
empty_new_data as (
    ...
    select * from {{ ref('renamed_dedup_cdc_excluded_scd_new_data') }} where 1 = 0
),
...
input_data as (
    select {{ dbt_utils.star(ref('renamed_dedup_cdc_excluded_stg')) }} from {{ ref('renamed_dedup_cdc_excluded_scd_new_data') }}
    union all
