    ...
    post_hook = ["
                        ...
                        delete from {{ final_table_relation }} where {{ final_table_relation }}._airbyte_unique_key in (
                            with modified_ids as (
                                ...
                                from {{ ref('multiple_column_names_conflicts_scd_new_data') }}
                                where 1=1
                                    {{ incremental_clause('_airbyte_emitted_at', this.schema + '.' + adapter.quote('multiple_column_names_conflicts')) }}
                            ),
                            scd_active_rows as (
                        ...
                        {% endif %}
                        ","delete from {{ ref('multiple_column_names_conflicts_scd_new_data') }} where _airbyte_emitted_at != (select max(_airbyte_emitted_at) from {{ ref('multiple_column_names_conflicts_scd_new_data') }})","delete from _airbyte_test_normalization.multiple_column_names_conflicts_stg where _airbyte_emitted_at != (select max(_airbyte_emitted_at) from _airbyte_test_normalization.multiple_column_names_conflicts_stg)"],
    tags = [ "top-level" ]
) }}
-- depends on: {{ ref('multiple_column_names_conflicts_scd_new_data') }}
with
{% if is_incremental() %}
new_data_ids as (
...
empty_new_data as (
    ...
    select * from {{ ref('multiple_column_names_conflicts_scd_new_data') }} where 1 = 0
),
previous_active_scd_data as (
    -- retrieve "incomplete old" data that needs to be updated with an end date because of new changes
    select
        {{ star_intersect(ref('multiple_column_names_conflicts_stg'), this, from_alias='inc_data', intersect_alias='this_data') }}
...
input_data as (
    select {{ dbt_utils.star(ref('multiple_column_names_conflicts_stg')) }} from {{ ref('multiple_column_names_conflicts_scd_new_data') }}
    union all
    select {{ dbt_utils.star(ref('multiple_column_names_conflicts_stg')) }} from previous_active_scd_data
),
{% else %}
input_data as (
    select *
