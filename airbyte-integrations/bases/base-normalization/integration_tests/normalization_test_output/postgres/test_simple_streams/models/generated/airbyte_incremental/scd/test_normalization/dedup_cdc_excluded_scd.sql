...
new_data as (
    ...
    select
        *
    from {{ ref('dedup_cdc_excluded_tmp')  }}
...
previous_active_scd_data as (
    ...
    select
        {{ star_intersect(ref('dedup_cdc_excluded_tmp'), this, from_alias='inc_data', intersect_alias='this_data') }}
...
input_data as (
    select {{ dbt_utils.star(ref('dedup_cdc_excluded_tmp')) }} from new_data
    union all
    select {{ dbt_utils.star(ref('dedup_cdc_excluded_tmp')) }} from previous_active_scd_data
),
{% else %}
input_data as (
    select *
