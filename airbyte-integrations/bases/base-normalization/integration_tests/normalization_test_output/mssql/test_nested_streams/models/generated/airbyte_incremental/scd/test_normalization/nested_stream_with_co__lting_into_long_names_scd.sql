{{ config(
    ...
    post_hook = ["
                    ...
                    {% endif %}
                    ","drop view _airbyte_test_normalization.nested_stream_with_co__lting_into_long_names_stg"],
    tags = [ "top-level" ]
) }}
...
new_data as (
    ...
    select
        *
    from {{ ref('nested_stream_with_co__lting_into_long_names_stg')  }}
...
previous_active_scd_data as (
    ...
    select
        {{ star_intersect(ref('nested_stream_with_co__lting_into_long_names_stg'), this, from_alias='inc_data', intersect_alias='this_data') }}
...
input_data as (
    select {{ dbt_utils.star(ref('nested_stream_with_co__lting_into_long_names_stg')) }} from new_data
    union all
    select {{ dbt_utils.star(ref('nested_stream_with_co__lting_into_long_names_stg')) }} from previous_active_scd_data
),
{% else %}
input_data as (
    select *
