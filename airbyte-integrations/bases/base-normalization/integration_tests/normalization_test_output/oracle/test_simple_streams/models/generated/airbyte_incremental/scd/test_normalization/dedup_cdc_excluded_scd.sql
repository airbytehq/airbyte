    ...
    select
      ...
      lag({{ quote('_AIRBYTE_EMITTED_AT') }}) over (
        ...
        order by
            ...
            {{ quote('_AIRBYTE_EMITTED_AT') }} desc, {{ quote('_AB_CDC_UPDATED_AT') }} desc
...
dedup_data as (
    select
        ...
        row_number() over (
            partition by {{ quote('_AIRBYTE_UNIQUE_KEY') }}, {{ quote('_AIRBYTE_START_AT') }}, {{ quote('_AIRBYTE_EMITTED_AT') }}, cast({{ quote('_AB_CDC_DELETED_AT') }} as {{ dbt_utils.type_string() }}), cast({{ quote('_AB_CDC_UPDATED_AT') }} as {{ dbt_utils.type_string() }})
            order by {{ quote('_AIRBYTE_AB_ID') }}
        ) as {{ quote('_AIRBYTE_ROW_NUM') }},
        {{ dbt_utils.surrogate_key([
          quote('_AIRBYTE_UNIQUE_KEY'),
          quote('_AIRBYTE_START_AT'),
          quote('_AIRBYTE_EMITTED_AT'), quote('_AB_CDC_DELETED_AT'), quote('_AB_CDC_UPDATED_AT')
