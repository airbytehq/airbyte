...
--                              inner join modified_ids on scd_table._airbyte_unique_key = modified_ids._airbyte_unique_key
                                ...
                                right outer join (
                                    ...
                                    from {{ ref('dedup_cdc_excluded_scd_new_data') }}
                                    where 1=1
                                        {{ incremental_clause('_airbyte_emitted_at', this.schema + '.' + quote('dedup_cdc_excluded')) }}
                                ) modified_ids
                        ...
                        {% endif %}
                        ","drop view {{ ref('dedup_cdc_excluded_scd_new_data') }}","drop view _airbyte_test_normalization.dedup_cdc_excluded_stg"],
    tags = [ "top-level" ]
) }}
-- depends on: {{ ref('dedup_cdc_excluded_scd_new_data') }}
with
{% if is_incremental() %}
new_data_ids as (
...
empty_new_data as (
    ...
    select * from {{ ref('dedup_cdc_excluded_scd_new_data') }} where 1 = 0
),
...
input_data as (
    select {{ dbt_utils.star(ref('dedup_cdc_excluded_stg')) }} from {{ ref('dedup_cdc_excluded_scd_new_data') }}
    union all
    ...
    select
        ...
        row_number() over (
            partition by
                _airbyte_unique_key,
                _airbyte_start_at,
                _airbyte_emitted_at, accurateCastOrNull(_ab_cdc_deleted_at, '{{ dbt_utils.type_string() }}'), accurateCastOrNull(_ab_cdc_updated_at, '{{ dbt_utils.type_string() }}')
