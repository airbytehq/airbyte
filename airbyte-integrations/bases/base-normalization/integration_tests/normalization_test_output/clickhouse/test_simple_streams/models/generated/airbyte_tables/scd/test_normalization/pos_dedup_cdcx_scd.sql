...
dedup_data as (
    select
        ...
        row_number() over (
            partition by _airbyte_unique_key, _airbyte_start_at, _airbyte_emitted_at, accurateCastOrNull(_ab_cdc_deleted_at, '{{ dbt_utils.type_string() }}'), accurateCastOrNull(_ab_cdc_updated_at, '{{ dbt_utils.type_string() }}'), accurateCastOrNull(_ab_cdc_log_pos, '{{ dbt_utils.type_string() }}')
