    ...
    post_hook = ["
                    ...
                    alter table {{ final_table_relation }} delete where _airbyte_unique_key in (
                        ...
                        from (
                                ...
                                from {{ this }}
                                where 1=1 {{ incremental_clause('_airbyte_normalized_at', quote(this.schema) + '.' + quote('dedup_exchange_rate')) }}
                            ) recent_records
                            left join (
                                select _airbyte_unique_key as unique_key, count(_airbyte_unique_key) as active_count
                                from {{ this }}
                                where _airbyte_active_row = 1 {{ incremental_clause('_airbyte_normalized_at', quote(this.schema) + '.' + quote('dedup_exchange_rate')) }}
