...
def is_combining_node(properties: dict) -> Set[str]:
    ...
    if data_type.ONE_OF_VAR_NAME in properties and any(
        data_type.REF_TYPE_VAR_NAME in option and data_type.WELL_KNOWN_TYPE_VAR_NAME in option[data_type.REF_TYPE_VAR_NAME] for option in properties[data_type.ONE_OF_VAR_NAME]
    ):
        return set()
    else:
