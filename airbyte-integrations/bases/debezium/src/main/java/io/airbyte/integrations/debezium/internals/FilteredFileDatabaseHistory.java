 ...
 * save the contents of the file. Debezium by default uses {@link
 * io.debezium.relational.history.FileDatabaseHistory} class to write the schema information in the
 * file. The problem is that the Debezium tracks the schema evolution of all the tables in all the
 * databases, because of that the file content can grow. In order to make sure that debezium tracks
 * only the schema of the tables that are present in the database that Airbyte is syncing, we
 ...
 * Ideally {@link FilteredFileDatabaseHistory} should have extended {@link
 * io.debezium.relational.history.FileDatabaseHistory} and overridden the {@link
 * #storeRecord(HistoryRecord)} method but {@link FilteredFileDatabaseHistory} is a final class and
...
public class FilteredFileDatabaseHistory extends AbstractDatabaseHistory {
  ...
  /**
   * Ideally the databaseName should have been initialized in the constructor of the class. But
   * since we supply the class name to debezium and it uses reflection to construct the object of
   * the class, we can't pass in the databaseName as a parameter to the constructor. That's why we
   * had to take the static approach.
   *
  ...
  public static void setDatabaseName(final String databaseName) {
    if (FilteredFileDatabaseHistory.databaseName == null) {
      ...
      throw new RuntimeException(
          "Database name has already been set : "
              + FilteredFileDatabaseHistory.databaseName
              + " can't set to : "
              + databaseName);
    }
  }
  ...
  public void storeRecord(final HistoryRecord record) throws DatabaseHistoryException {
    ...
    try {
      ...
      /**
       * We are using reflection because the method {@link
       * io.debezium.relational.history.FileDatabaseHistory#storeRecord(HistoryRecord)} is protected
       * and can not be accessed from here
       */
      final Method storeRecordMethod =
          fileDatabaseHistory.getClass().getDeclaredMethod("storeRecord", record.getClass());
  ...
  protected void recoverRecords(final Consumer<HistoryRecord> records) {
    try {
      /**
       * We are using reflection because the method {@link
       * io.debezium.relational.history.FileDatabaseHistory#recoverRecords(Consumer)} is protected
       * and can not be accessed from here
       */
      final Method recoverRecords =
          fileDatabaseHistory.getClass().getDeclaredMethod("recoverRecords", Consumer.class);
