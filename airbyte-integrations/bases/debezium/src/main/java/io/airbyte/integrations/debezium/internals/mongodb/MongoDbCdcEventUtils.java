...
public class MongoDbCdcEventUtils {
   ...
   *         key.
   */
  public static ObjectNode generateObjectIdDocument(final JsonNode debeziumEventKey) {
    final String objectId = Jsons.deserialize(debeziumEventKey.get(ID_FIELD).asText()).get(OBJECT_ID_FIELD).asText();
    return (ObjectNode) Jsons.jsonNode(Map.of(DOCUMENT_OBJECT_ID_FIELD, objectId));
  }
   ...
   *
   * @param document The Debezium event data as JSON.
   * @return The transformed Debezium event data as JSON.
   */
  public static ObjectNode transformDataTypes(final ObjectNode document) {
    final ObjectNode objectNode = (ObjectNode) Jsons.jsonNode(Collections.emptyMap());
    formatDocument(Document.parse(Jsons.serialize(document)), objectNode, ImmutableList.copyOf(document.fieldNames()));
