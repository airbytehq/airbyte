 ...
 * when a sync has reached the target position of the CDC log for MongoDB.  In this case, the
 * target position is a resume token value from the MongoDB oplog.  This implementation compares the
 * timestamp present in the Debezium change event against the timestamp of the resume token recorded
 * at the start of a sync.  When the event timestamp exceeds the resume token timestamp, the sync should
 * stop to prevent it from running forever.
 */
public class MongoDbCdcTargetPosition implements CdcTargetPosition<BsonTimestamp> {
    ...
    /**
     * Constructs a new {@link MongoDbCdcTargetPosition} by fetching the most recent resume token
     * from the MongoDB database.
     *
     * @param mongoClient A {@link MongoClient} used to retrieve the resume token.
     * @return The {@link MongoDbCdcTargetPosition} set to the most recent resume token present in the database.
     */
