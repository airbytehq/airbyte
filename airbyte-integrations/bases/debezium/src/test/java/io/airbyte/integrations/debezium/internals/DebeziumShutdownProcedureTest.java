...
public class DebeziumShutdownProcedureTest {
  ...
  public void test() throws InterruptedException {
    ...
    final DebeziumShutdownProcedure<Integer> debeziumShutdownProcedure =
        new DebeziumShutdownProcedure<>(
            sourceQueue, executorService::shutdown, () -> recordsInserted.get() >= 99);
    ...
    Assertions.assertEquals(
        100, debeziumShutdownProcedure.getRecordsRemainingAfterShutdown().size());

    for (int i = 0; i < 100; i++) {
      Assertions.assertEquals(
          i, debeziumShutdownProcedure.getRecordsRemainingAfterShutdown().poll());
    }
  }
}
