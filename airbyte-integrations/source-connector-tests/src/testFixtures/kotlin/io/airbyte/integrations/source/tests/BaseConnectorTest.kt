...
abstract class BaseConnectorTest {
    protected abstract val testDbInstance: TestDbInstance
    ...
    // Default to NoOp implementation, but initialize properly in initTestAssetCleaner()
    protected open val testAssetCleaner: TestAssetCleaner by lazy { initTestAssetCleaner() }

    protected open fun initTestAssetCleaner(): TestAssetCleaner {
        return when (testDbInstance) {
            is RemoteJdbcTestInstance ->
                RemoteJdbcTestAssetCleaner(testDbInstance as RemoteJdbcTestInstance, sqlDialect)
            else -> NoOpTestAssetCleaner()
        }
    }

    companion object {
        const val TABLE_PREFIX = "TEST_TABLE_"
        const val NAMESPACE_PREFIX = "TEST_NAMESPACE_"
    }

    @BeforeAll
    fun initializeAndConnectDatabase() {
        testDbInstance.connect()
    }

    @AfterAll
    fun disconnectDatabase() {
        try {
            testAssetCleaner.cleanupOldTestAssets()
        } catch (e: Exception) {
            println("Error during cleanup test assets: ${e.message}")
        }
        testDbInstance.disconnect()
    }
    ...
    protected open fun setupNamespaces(num: Int = 1) {
        for (i in 1..num) {
            val namespace = generateNamespace()
            namespaces.add(namespace)
            testDbInstance.executeUpdate(sqlDialect.buildCreateNamespaceQuery(namespace))
        }
    }

    protected fun dropCurrentNamespaces() {
        for (namespace in namespaces) {
            testDbInstance.executeUpdate(sqlDialect.buildDropNamespaceQuery(namespace))
        }
    }
    ...
    protected open fun setupTablesForAllNamespaces(num: Int = 1) {
        for (namespace in namespaces) {
            for (i in 1..num) {
                ...
                testDbInstance.executeUpdate(sqlDialect.buildCreateTableQuery(table))
            }
        }
    }
    ...
    fun generateNamespace(): String = NAMESPACE_PREFIX + System.currentTimeMillis()

    fun generateTableName(): String = TABLE_PREFIX + System.currentTimeMillis()
