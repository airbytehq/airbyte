...
import io.airbyte.integrations.source.mssql.MssqlCdcHelper.ReplicationMethod;
...
public class MssqlSource extends AbstractJdbcSource<JDBCType> implements Source {
  ...
  public static final String NULL_CURSOR_VALUE_WITH_SCHEMA_QUERY =
      ...
      """;
  public static final String DRIVER_CLASS = DatabaseDriver.MSSQLSERVERLEGACY.getDriverClassName();
  ...
  public JsonNode toDatabaseConfig(final JsonNode mssqlConfig) {
    ...
    final StringBuilder jdbcUrl = new StringBuilder(
        String.format("jdbc:jtds:sqlserver://%s:%s/%s;",
    ...
    // Add performance optimizations for jTDS
    additionalParameters.add("sendStringParametersAsUnicode=false");
    additionalParameters.add("loginTimeout=30");
    additionalParameters.add("socketTimeout=0");
    additionalParameters.add("packetSize=8192");
    additionalParameters.add("responseBuffering=adaptive");
    
    // Handle TDS version
    if (mssqlConfig.has("tds_version")) {
        additionalParameters.add("TDS=" + mssqlConfig.get("tds_version").asText());
    } else {
        additionalParameters.add("TDS=8.0"); // Default for SQL Server 2000+
    }
  ...
  // Add method to handle legacy SQL Server versions
  private void addLegacySqlServerSupport(final JsonNode mssqlConfig, final List<String> additionalParameters) {
      if (mssqlConfig.has("sql_server_version")) {
          final String version = mssqlConfig.get("sql_server_version").asText();
          switch (version) {
              case "2000":
                  additionalParameters.add("TDS=8.0");
                  break;
              case "7.0":
                  additionalParameters.add("TDS=7.0");
                  break;
              case "6.5":
                  additionalParameters.add("TDS=4.2");
                  break;
              default:
                  additionalParameters.add("TDS=8.0");
          }
  ...
  protected void assertSqlServerAgentRunning(final JdbcDatabase database) throws SQLException {
    try {
      ...
      if (e.getCause() != null && e.getCause().getClass().equals(java.sql.SQLException.class)) {
