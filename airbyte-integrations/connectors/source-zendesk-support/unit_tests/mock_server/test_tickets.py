...
class TestTicketsStreamIncremental(TestCase):
    ...
    @HttpMocker()
    def test_given_system_update_then_cursor_advances_on_generated_timestamp(self, http_mocker):
        """Regression test for oncall #13073.

        A ticket updated by a Zendesk automation/macro/system action bumps
        `generated_timestamp` but leaves `updated_at` unchanged. The tickets stream must
        track changes on `generated_timestamp` (the Incremental Ticket Export cursor) so
        that such updates are not silently dropped. This test simulates a ticket whose
        `updated_at` predates the incremental cursor while its `generated_timestamp`
        advanced past it, and asserts the record is emitted and the state advances on
        `generated_timestamp`.
        """
        api_token_authenticator = self._get_authenticator(self._config)
        state_cursor_value = _START_DATE.add(timedelta(days=30))
        # generated_timestamp advanced well past the state cursor (system update).
        advanced_generated_timestamp = int(state_cursor_value.add(timedelta(days=5)).timestamp())
        # updated_at stayed frozen at creation time, before the state cursor.
        stale_updated_at = _START_DATE.strftime("%Y-%m-%dT%H:%M:%SZ")
        ...
        http_mocker.get(
            ...
            TicketsResponseBuilder.tickets_response()
            .with_record(
                TicketsRecordBuilder.tickets_record()
                .with_field(FieldPath("generated_timestamp"), advanced_generated_timestamp)
                .with_field(FieldPath("updated_at"), stale_updated_at)
            )
            .build(),
        )
        ...
        assert output.most_recent_state is not None
        assert "generated_timestamp" in output.most_recent_state.stream_state.__dict__
        assert int(output.most_recent_state.stream_state.generated_timestamp) == advanced_generated_timestamp
