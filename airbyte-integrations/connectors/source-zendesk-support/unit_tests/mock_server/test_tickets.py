...
class TestTicketsStreamFullRefresh(TestCase):
    ...
    def test_given_one_page_when_read_tickets_then_return_records(self, http_mocker):
        ...
        http_mocker.get(
            ZendeskSupportRequestBuilder.tickets_endpoint(api_token_authenticator).with_start_time(self._config["start_date"]).with_any_query_params().build(),
...
class TestTicketsStreamIncremental(TestCase):
    ...
    def test_given_state_when_read_tickets_then_use_state_cursor(self, http_mocker):
        ...
        http_mocker.get(
            ZendeskSupportRequestBuilder.tickets_endpoint(api_token_authenticator).with_start_time(state_cursor_value).with_any_query_params().build(),
            TicketsResponseBuilder.tickets_response()
