...
class TestTicketsStreamFullRefresh(TestCase):
    ...
    @HttpMocker()
    def test_given_403_permission_denied_when_read_tickets_then_fail_with_error_log(self, http_mocker):
        api_token_authenticator = self._get_authenticator(self._config)
        error_message = "Forbidden - You do not have access to tickets"
        http_mocker.get(
            ZendeskSupportRequestBuilder.tickets_endpoint(api_token_authenticator).with_start_time(self._config["start_date"]).build(),
