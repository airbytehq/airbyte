...
class TestSectionsStreamFullRefresh(TestCase):
    ...
    def test_given_one_page_when_read_sections_then_return_records(self, http_mocker):
        ...
        http_mocker.get(
            ...
            SectionsResponseBuilder.sections_response()
    ...
    def test_given_two_pages_when_read_sections_then_return_all_records(self, http_mocker):
        ...
        http_mocker.get(
            self._base_sections_request(api_token_authenticator).build(),
            SectionsResponseBuilder.sections_response(self._base_sections_request(api_token_authenticator).build())
            .with_record(SectionsRecordBuilder.sections_record())
            .with_pagination()
            .build(),
        )
        ...
        http_mocker.get(
            self._base_sections_request(api_token_authenticator).with_after_cursor("after-cursor").build(),
            SectionsResponseBuilder.sections_response()
            .with_record(SectionsRecordBuilder.sections_record().with_id(67890))
            .build(),
        )
