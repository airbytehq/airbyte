...
from source_zendesk_support.components import ZendeskSupportAuditLogsIncrementalSync, ZendeskSupportExtractorEvents, ZendeskSupportAttributeDefinitionsExtractor
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
                {"ticket_events": [
                    {"via_reference_id": 123, "ticket_id": 456, "timestamp": "2022-01-01T00:00:00Z",
                     "child_events": [
                         {"event_type": "Comment", "via_reference_id": "unused", "via": {"some": "data"}}
                     ]}
                ]},
                [{"event_type": "Comment", "via_reference_id": 123, "ticket_id": 456, "timestamp": "2022-01-01T00:00:00Z",
                  "via": {"some": "data"}}]
        ),
        ...
        (
                {"ticket_events": [
                    {"via_reference_id": 123, "ticket_id": 456, "timestamp": "2022-01-01T00:00:00Z",
                     "child_events": [
                         {"event_type": "Comment", "via_reference_id": "unused", "via": "incorrect_format"}
                     ]}
                ]},
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"definitions": {"conditions_any": [{"id": 2}], "conditions_all": []}},
            [{"id": 2, "condition": "any"}]
        ),
        # Test case where conditions_any is empty
        (
            {"definitions": {"conditions_all": [{"id": 1}], "conditions_any": []}},
            [{"id": 1, "condition": "all"}]
        ),
