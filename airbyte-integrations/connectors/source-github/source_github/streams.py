...
class Security(GithubStream):
    ...
    def request_body_json(
        ...
        body = "{ repository(name: \""+ stream_slice['repository'].split("/")[1] + "\", owner: \""+ stream_slice['repository'].split("/")[0] + "\") { vulnerabilityAlerts(first: 100) { nodes { createdAt dismissedAt securityVulnerability {package { name } advisory { description }}}}}}"
        payload = {'query':body}
        return payload
