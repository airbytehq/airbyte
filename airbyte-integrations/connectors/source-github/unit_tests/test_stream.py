...
def test_error_handler_410_unknown_body_returns_retry():
    stream = RepositoryStats(repositories=["test_repo"], page_size_for_large_streams=30)
    response_mock = MagicMock(spec=requests.Response)
    response_mock.status_code = requests.codes.GONE
    response_mock.headers = {}
...
def test_read_records_404_message_for_repository_stream(caplog, requests_mock):
...
def test_read_records_410_projects_disabled_message(caplog, requests_mock):
