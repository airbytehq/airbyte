...
def test_read_records_410_projects_disabled_message(time_mock, caplog, requests_mock):
    ...
    list(read_full_refresh(stream))
    assert any("Skipping stream slice for" in msg and "Projects are disabled for this repository" in msg for msg in caplog.messages)
...
def test_stream_releases_409_empty_repository(time_mock, caplog, requests_mock):
    ...
    assert records == []
    assert any(
        "Skipping `releases` for this repository" in msg and "409 Conflict" in msg and "empty (no commits)" in msg
        for msg in caplog.messages
    )
