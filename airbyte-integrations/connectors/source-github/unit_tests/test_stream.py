...
def test_streams_read_full_refresh(requests_mock):
    ...
    # `issue_events`, `issue_milestones`, `commit_comments` and `deployments` moved to the
    # manifest in Steps 5 and 6; `releases`, `projects_v2` and `pull_request_stats` moved in
    # Step 9. Their read behavior is covered by the manifest tests.
