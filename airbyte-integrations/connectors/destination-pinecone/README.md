...
[documentation](https://docs.airbyte.com/integrations/destinations/pinecone) to generate the
necessary credentials. Then create a file `secrets/config.json` conforming to the
`destination_pinecone/spec.json` file. Note that the `secrets` directory is gitignored by default,
...
`destination pinecone test creds` and place them into `secrets/config.json`.
...
`airbyte/destination-pinecone:dev`.
