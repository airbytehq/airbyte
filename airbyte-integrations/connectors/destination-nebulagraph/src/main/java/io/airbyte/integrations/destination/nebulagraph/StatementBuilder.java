...
public final class StatementBuilder {
  ...
  public String q(String ident) {
    if (ident == null) throw new IllegalArgumentException("ident is null");
  ...
  public String buildInsertVertexValues(String tag, List<String> cols, List<String> vidAndValueTuples) {
    if (tag == null || tag.isEmpty()) throw new IllegalArgumentException("tag empty");
    if (cols == null || cols.isEmpty()) throw new IllegalArgumentException("cols empty");
    if (vidAndValueTuples == null || vidAndValueTuples.isEmpty()) throw new IllegalArgumentException("rows empty");
    StringBuilder sql = new StringBuilder();
    // Add a space between tag identifier and column list to align with Nebula examples: INSERT VERTEX tag (col,...)
    sql.append("INSERT VERTEX ").append(q(tag)).append(" (").append(joinCols(cols)).append(")\nVALUES\n");
    sql.append(String.join(",\n  ", vidAndValueTuples)).append(";");
    return sql.toString();
  }

  // Generic raw+typed batch INSERT EDGE (provided by caller with column headers and aligned value strings)
  public String buildInsertEdgeValues(String edgeType, List<String> cols, List<String> endpointsAndValueTuples) {
    ...
    if (cols == null || cols.isEmpty()) throw new IllegalArgumentException("cols empty");
    if (endpointsAndValueTuples == null || endpointsAndValueTuples.isEmpty()) throw new IllegalArgumentException("rows empty");
    StringBuilder sql = new StringBuilder();
  ...
  public String buildUpsertEdge(String edgeType, String src, String dst, long rank, List<String> cols, List<Object> values) {
    if (edgeType == null || edgeType.isEmpty()) throw new IllegalArgumentException("edgeType empty");
    if (src == null || dst == null) throw new IllegalArgumentException("src/dst is null");
    if (cols == null || values == null || cols.size() != values.size()) {
      throw new IllegalArgumentException("cols/values size mismatch");
    }
    StringBuilder set = new StringBuilder();
  ...
  // Format a row object value as a values list (in column order), and return the vid:"(v1,v2,...)" fragment
