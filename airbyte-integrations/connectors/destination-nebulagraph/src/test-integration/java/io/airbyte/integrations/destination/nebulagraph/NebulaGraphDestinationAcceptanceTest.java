...
public class NebulaGraphDestinationAcceptanceTest extends DestinationAcceptanceTest {
  ...
  @Override
  protected boolean supportBasicDataTypeTest() { return true; }

  @Override
  protected boolean supportArrayDataTypeTest() { return true; }

  @Override
  protected boolean supportObjectDataTypeTest() { return true; }

  @Override
  protected String getDefaultSchema(final JsonNode config) { return ""; }
    ...
    try {
      if (nebulaStarted) {
        ...
        try (NebulaGraphClient client = NebulaGraphClient.connect(cfg)) {
          try { client.execute("DROP SPACE " + sb.q(cfg.space)); } catch (Exception dropEx) {
  ...
  protected java.util.List<com.fasterxml.jackson.databind.JsonNode> retrieveRecords(final TestDestinationEnv testEnv,
    ...
    try (NebulaGraphClient client = NebulaGraphClient.connect(cfg)) {
      ...
      if (!isEdge) {
        final String tag = (namespace == null || namespace.isEmpty()) ? streamName.toLowerCase() : (namespace.toLowerCase() + "__" + streamName.toLowerCase());
        // Quote the tag when qualifying properties. Without quoting, tags containing hyphens, colons, diacritics,
        // or substrings that look like keywords (e.g. "with") cause parser errors (observed: "syntax error near `with'").
        // Example needed form: v.`stream-with:spécial:character_names`.`_airbyte_data`
        sql = "MATCH (v:" + sb.q(tag) + ") RETURN v." + sb.q(tag) + ".`_airbyte_data` AS d, v." + sb.q(tag) + ".`_airbyte_emitted_at` AS e, v." + sb.q(tag) + ".`_airbyte_ab_id` AS a ORDER BY e ASC, a ASC";
      } else {
        final String edgeType = (sc.edgeType != null && !sc.edgeType.isEmpty()) ? sc.edgeType.toLowerCase() : ((namespace == null || namespace.isEmpty()) ? streamName.toLowerCase() : (namespace.toLowerCase() + "__" + streamName.toLowerCase()));
        sql = "MATCH ()-[e:" + sb.q(edgeType) + "]->() RETURN e." + sb.q(edgeType) + ".`_airbyte_data` AS d, e." + sb.q(edgeType) + ".`_airbyte_emitted_at` AS e, e." + sb.q(edgeType) + ".`_airbyte_ab_id` AS a ORDER BY e ASC, a ASC";
      }
      ...
      if (jsons != null) {
        for (int i=0;i<jsons.size();i++) {
          if (i<5) System.out.println("[retrieveRecords] sample json["+i+"]=" + jsons.get(i));
        }
      }
      // Fallback: if all values null, attempt alternate MATCH syntax aliasing vertex directly
      boolean allNull = true;
      if (jsons != null) {
        for (String s : jsons) { if (s != null && !s.isEmpty() && !"NULL".equalsIgnoreCase(s)) { allNull = false; break; } }
      }
      if (allNull) {
        final String altSql;
        if (!isEdge) {
        ...
        try {
          ...
          if (alt != null) {
            jsons = alt; // overwrite
            for (int i=0;i<alt.size();i++) { if (i<5) System.out.println("[retrieveRecords] fallback sample json["+i+"]=" + alt.get(i)); }
          }
        } catch (Exception fe) {
