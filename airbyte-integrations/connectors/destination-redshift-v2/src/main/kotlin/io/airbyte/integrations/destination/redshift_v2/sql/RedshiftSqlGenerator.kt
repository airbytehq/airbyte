...
class RedshiftSqlGenerator(
    ...
    fun createTable(
        ...
        val columnDeclarations = columnUtils
            .columnsAndTypes(stream.schema.asColumns(), columnNameMapping)
            .joinToString(",\n")

        // Redshift doesn't support CREATE OR REPLACE TABLE, so we need to drop first if replace=true
        val createStatement = if (replace) {
            """
    ...
    /**
     * Move data from source table to target table using ALTER TABLE APPEND.
     * This is used for cross-schema table moves since Redshift doesn't support
     * ALTER TABLE SET SCHEMA.
     *
     * ALTER TABLE APPEND moves rows from source to target table (source becomes empty).
     * It's much faster than INSERT...SELECT for large tables.
     */
    ...
    /**
     * Generate SQL statements for upsert (merge) operation.
     * Redshift doesn't have native MERGE, so we use DELETE + INSERT pattern:
    ...
    fun upsertTable(
        ...
        val pkCondition = if (importType.primaryKey.isNotEmpty()) {
        ...
        val cursorComparison: String = if (importType.cursor.isNotEmpty()) {
        ...
        val insertFilter = if (hasCdcColumn && hardDeleteEnabled) {
        ...
        val dedupedSubquery = selectDedupedRecordsSubquery(stream, sourceTableName, columnNameMapping)
     ...
     * source table. Uses ROW_NUMBER() window function to select the most recent record per primary key.
     *
    ...
    private fun selectDedupedRecords(
        ...
        val pkList = if (importType.primaryKey.isNotEmpty()) {
        ...
        val extractedAtColumn = COLUMN_NAME_AB_EXTRACTED_AT.toRedshiftCompatibleName().quote()
        val cursorOrderClause = if (importType.cursor.isNotEmpty()) {
            val columnName = (columnNameMapping[importType.cursor.first()] ?: importType.cursor.first()).quote()
    ...
    /**
     * Generate SQL statements to alter a table's schema.
     * Returns a set of SQL statements to be executed in order.
     *
     * For type changes, we use the add-copy-rename-drop pattern since Redshift
     * doesn't support direct ALTER COLUMN TYPE for all type combinations.
     */
    fun alterTable(
        tableName: TableName,
        ...
        deletedColumns.forEach { (name, _) ->
            statements.add(
                "ALTER TABLE $prettyTableName DROP COLUMN ${name.quote()}".andLog()
            )
        }
        ...
        modifiedColumns.forEach { (name, typeChange) ->
            ...
            // Note: We don't change from nullable to NOT NULL because existing records may have nulls
        }
