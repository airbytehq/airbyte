...
data class S3StagingConfiguration(
    ...
    @get:JsonSchemaInject(json = """{"order": 1, "examples":["airbyte-staging-bucket"]}""")
    ...
    @get:JsonSchemaInject(json = """{"order": 2, "examples":["data_sync/redshift"]}""")
    ...
    @get:JsonSchemaInject(json = """{"order": 3, "examples":["us-east-1"]}""")
    ...
    @get:JsonPropertyDescription(
        "(Optional) The pattern for S3 staging file names. Supported placeholders: {date}, {date:yyyy_MM}, {timestamp}, {timestamp:millis}, {timestamp:micros}, {part_number}, {sync_id}, {format_extension}."
    )
