...
/**
 * Generates Redshift-specific SQL strings for Direct Load table operations.
 * - No index creation (Redshift uses sort keys / distribution keys instead)
 * - 5-step rename pattern for ALTER COLUMN TYPE (Redshift doesn't support `USING` clause)
 * - `JSON_SERIALIZE()` / `JSON_PARSE()` for SUPER <-> VARCHAR conversions
 *
...
class RedshiftSqlGenerator(
    ...
    fun dropTable(tableName: TableName): String =
        // TODO claude: move getFullyQualifiedName to Util class
     ...
     *
     * Builds the DDL from meta columns (via [RedshiftColumnManager]) + the user columns provided in
     * the [tableSchema]'s `finalSchema`.
     */
    // TODO claude: remove this fun
    ...
    // ================================================================
    // DML: Table data operations
    // ================================================================

    /** Generates `SELECT COUNT(*)` SQL with a fixed alias. */
    fun countTable(tableName: TableName): String =
        "SELECT COUNT(*) AS \"$COUNT_TOTAL_ALIAS\" FROM ${getFullyQualifiedName(tableName)};".andLog()

    /**
     * Generates `SELECT _airbyte_generation_id FROM table LIMIT 1` to read the current generation
     * ID from the table.
     */
    fun getGenerationId(tableName: TableName): String =
        "SELECT ${quoteIdentifier(COLUMN_NAME_AB_GENERATION_ID)} FROM ${getFullyQualifiedName(tableName)} LIMIT 1;".andLog()
    ...
    // ================================================================
    // Upsert (CTE-based DELETE + UPDATE + INSERT)
    // ================================================================

    /**
     * Generates a CTE-based upsert statement that merges data from a source (temp) table into the
     * target (final) table.
     *
     * The generated SQL performs:
     ...
     *
     * This is the same CTE-based pattern used by the Postgres connector. Redshift supports DML
     * (DELETE, UPDATE) inside CTEs.
     *
     * @throws IllegalArgumentException if the stream has no primary key.
     */
    fun upsertTable(
        stream: DestinationStream,
        sourceTableName: TableName,
     ...
     * than target data.
     *
     * When a cursor column exists, compares with extracted_at as a tiebreaker:
     * ```
     * target.cursor < source.cursor
     * OR (target.cursor = source.cursor AND target.extracted_at < source.extracted_at)
     * OR (target.cursor IS NULL AND source.cursor IS NOT NULL)
     * OR (target.cursor IS NULL AND source.cursor IS NULL AND target.extracted_at <
     *     source.extracted_at)
     * ```
     *
     * When no cursor exists, compares only extracted_at timestamps.
     */
    ...
    /**
     * Generates SQL to evolve a table's schema: add columns, remove columns, and change column
     * types.
     *
     * **Key Redshift difference from Postgres:** Redshift does NOT support `ALTER COLUMN ... TYPE
     * ... USING` for inline type conversions. Instead, type changes use a 4-step rename pattern
     ...
     *
     * New columns are intentionally added without NOT NULL constraints, since there is no
     * reasonable default for preexisting rows.
     */
    fun matchSchemas(
        tableName: TableName,
     ...
     *
     * Returns `column_name`, `data_type`, and `is_nullable` for all columns of the given table,
