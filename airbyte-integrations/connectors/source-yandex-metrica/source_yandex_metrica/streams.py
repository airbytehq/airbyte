...
from .enums import YMSource, YMPrimaryKey, YMCursor, YMStatus
...
class YandexMetricaStream(HttpStream, ABC):
    ...
    def request_headers(
        ...
        return {
            'Content-Type': 'application/x-ymetrika+json'
        }

    def next_page_token(self, response: requests.Response) -> Optional[Mapping[str, Any]]:
        return None

    def clean_logrequest(self, logrequest_id) -> Tuple[bool, any]:
        url = f"{self.url_base}{self.counter_id}/logrequest/{logrequest_id}/clean"
    ...
    def fetch_records(
        ...
        try:
            ...
            # 5. Clean logrequest
            self.clean_logrequest(logrequest_id)
...
class IncrementalYandexMetricaStream(YandexMetricaStream, IncrementalMixin):
    ...
    def state(self) -> MutableMapping[str, Any]:
        return {self.cursor_field: self._cursor_value, 'start_date': self._start_date, 'end_date': self._end_date} if self._cursor_value else {}
