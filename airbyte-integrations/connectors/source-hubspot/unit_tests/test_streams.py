...
def test_leads_incremental(stream_class, endpoint, cursor_value, requests_mock, fake_properties_list, config):
    mock_dynamic_schema_requests_with_skip(requests_mock, [])

    stream_state = AirbyteStateMessage(
        type=AirbyteStateType.STREAM,
        stream=AirbyteStreamState(
            stream_descriptor=StreamDescriptor(name="leads"), stream_state=AirbyteStateBlob(updatedAt="2021-01-01T00:00:00.000000Z")
        ),
    )

    stream = find_stream(stream_class, config, state=[stream_state])
    data_field = (
        stream._stream_partition_generator._partition_factory._retriever.record_selector.extractor.field_path[0]
        if len(stream._stream_partition_generator._partition_factory._retriever.record_selector.extractor.field_path) > 0
        else None
    ...
    # requests_mock.register_uri("GET", stream_url, responses)
    requests_mock.register_uri("POST", stream_url, responses)
    requests_mock.register_uri("GET", "/crm/v3/objects/contact", contact_response)
    requests_mock.register_uri("GET", "/contacts/v1/lists/all/contacts/all", contact_lists_v1_response)
    requests_mock.register_uri("GET", "/marketing/v3/forms", responses if not is_form_submission else forms_response)
    requests_mock.register_uri("GET", "/email/public/v1/campaigns/test_id", responses)
