...
@pytest.mark.parametrize(
    ...
    ids=[
        "basic_single_association",
        "multiple_associations_and_types",
        "empty_response",
        "no_associations_for_object"
    ]
)
...
def test_batching_record_filter_basic(mock_config, components_module):
    ...
    batch_records = list(record_filter.filter_records(
        records=iter(input_records),
        stream_state={},
        stream_slice=None,
    ...
    assert batch_records[0]["record_ids"] == [
        {"id": "1"},
        {"id": "2"},
        {"id": "3"}
    ]
