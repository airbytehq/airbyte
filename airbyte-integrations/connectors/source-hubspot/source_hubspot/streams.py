...
class API:
    ...
    def _parse_and_handle_errors(response) -> Union[MutableMapping[str, Any], List[MutableMapping[str, Any]]]:
        ...
        if response.status_code == HTTPStatus.BAD_REQUEST:
            logger.warning(
                f"Request to {response.url} didn't succeed. Please verify your credentials and try again.\nError message from Hubspot API: {message}"
            )
        elif response.status_code == HTTPStatus.FORBIDDEN:
            logger.warning(
                f"The authenticated user does not have permissions to access the URL {response.url}. Verify your permissions to access this endpoint."
            )
