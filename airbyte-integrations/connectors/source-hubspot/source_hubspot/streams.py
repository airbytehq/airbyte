...
from typing import TYPE_CHECKING, Any, Dict, Iterable, List, Mapping, MutableMapping, Optional, Set, Tuple, Union
...
class API:
    ...
    def __init__(self, credentials: Mapping[str, Any]):
        ...
        if self.is_oauth2() or self.is_private_app():
            ...
            msg = "No supported `credentials_title` specified. See spec.yaml for references"
            raise Exception(msg)
    ...
    def get(
        self, url: str, params: Optional[MutableMapping[str, Any]] = None,
    ...
    def post(
        self, url: str, data: Mapping[str, Any], params: Optional[MutableMapping[str, Any]] = None,
    ...
    def _field_to_property_schema(self, field: Dict[str, Any]) -> Mapping[str, Any]:
        ...
        if field_type == "enumeration" or field_type == "string":
            ...
            self.logger.warning(f"Field {field['name']} has unrecognized type: {field['type']} casting to string.")
            property_schema = {"type": ["null", "string"]}
...
class Stream(HttpStream, ABC):
    ...
    def __init__(self, api: API, start_date: Union[str, pendulum.datetime], credentials: Optional[Mapping[str, Any]] = None, **kwargs):
    ...
    def read_records(
        ...
        try:
            ...
            response = e.response
            if response.status_code == HTTPStatus.UNAUTHORIZED:
                msg = "The authentication to HubSpot has expired. Re-authenticate to restore access to HubSpot."
                raise AirbyteTracedException(msg)
            else:
                raise e
    ...
    @staticmethod
    def _convert_datetime_to_string(dt: pendulum.datetime, declared_format: Optional[str] = None) -> str:
    ...
    @classmethod
    def _cast_datetime(cls, field_name: str, field_value: Any, declared_format: Optional[str] = None) -> Any:
    ...
    @classmethod
    def _cast_value(cls, declared_field_types: List, field_name: str, field_value: Any, declared_format: Optional[str] = None) -> Any:
        ...
        if target_type_name == "boolean" and actual_field_type_name == "string":
            ...
            if str(field_value).lower() in ["true", "false"]:
                return str(field_value).lower() == "true"
    ...
    def _cast_record_fields_if_needed(self, record: Mapping, properties: Optional[Mapping[str, Any]] = None) -> Mapping:
        ...
        *,
        ...
        if isinstance(response, Mapping):
            ...
            if response.get(self.data_field) is None:
                ...
                """
                msg = f"Unexpected API response: {self.data_field} not in {response.keys()}"
...
class AssociationsStream(Stream):
    ...
    def stream_slices(self, sync_mode: SyncMode, cursor_field: Optional[List[str]] = None, stream_state: Optional[Mapping[str, Any]] = None) -> Iterable[str]:
...
class CRMSearchStream(IncrementalStream, ABC):
    ...
    def _read_associations(self, records: Iterable) -> Iterable[Mapping[str, Any]]:
        ...
        identifiers = [x[self.primary_key] for x in records]
    ...
    def set_sync(self, sync_mode: SyncMode, stream_state):
        self._sync_mode = sync_mode
        if self._sync_mode == SyncMode.incremental and stream_state:
            if not self._state:
                self._state = self._start_date
            else:
                self._state = self._start_date = max(self._state, self._start_date)
...
class CRMObjectStream(Stream):
    ...
    def __init__(self, include_archived_only: bool = False, **kwargs):
        ...
        if not self.entity:
            msg = "Entity must be set either on class or instance level"
            raise ValueError(msg)
...
class EngagementsRecent(EngagementsABC):
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        if self._start_date < pendulum.now() - timedelta(days=self.last_days_limit):
            msg = f'"Recent engagements" API returns records updated in the last 30 days only. Start date {self._start_date} is older so "All engagements" API should be used'
        ...
        *,
        ...
        if response_info:
            ...
            if total > self.total_records_limit:
                yield from []
                msg = '"Recent engagements" API returns only 10k most recently updated records. API response indicates that there are more records so "All engagements" API should be used'
                raise EngagementsRecentError(
                    msg,
                )
...
class ContactsMergedAudit(Stream):
    ...
    def stream_slices(
        self, sync_mode: SyncMode, cursor_field: Optional[List[str]] = None, stream_state: Optional[Mapping[str, Any]] = None, **kwargs,
    ) -> Iterable[Mapping[str, Any]]:
        slices = []
