...
The connector uses the OAuth 2.0 password grant against `{base_url}/api/oauth2/token/`: `client_id`/`client_secret` identify the OAuth application and `username`/`password` identify the Uptick user. There is no Cloud OAuth button because Uptick offers only the password grant and every customer runs on their own tenant host (`<tenant>.onuptick.com`), so a single shared Airbyte Cloud OAuth app is not possible.
...
`api_budget` is a global `HTTPAPIBudget` of 60 requests/minute (`MovingWindowCallRatePolicy`). This is a conservative placeholder — Uptick's published limit is not yet confirmed with the vendor, so treat it as a floor, not a measured value. The error handler honors `Retry-After` capped at 1800s per wait, retries up to 5 times, and `maxSecondsBetweenMessages` is 3600.
