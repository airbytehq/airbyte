...
def get_source(config: dict, state: dict | None = None) -> YamlDeclarativeSource:
    catalog = CatalogBuilder().build()
    source_state = state or StateBuilder().build()
    return YamlDeclarativeSource(path_to_yaml=str(_YAML_FILE_PATH), catalog=catalog, config=config, state=source_state)
