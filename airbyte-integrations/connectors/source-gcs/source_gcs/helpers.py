...
def get_gcs_blobs(config):
    ...
    return [blob for blob in blobs if "csv" in blob.name.lower()]
...
def read_csv_file(blob: Blob, read_header_only=False):
    ...
    file_obj.seek(0)
    df = pd.read_csv(file_obj, nrows=0) if read_header_only else pd.read_csv(file_obj)
    file_obj.close()
    return df
...
def get_stream_name(blob):
    ...
    return blob_name_without_path.replace(".csv", "")
