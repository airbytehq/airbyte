CREATE
    USER ORACLE_FULL_NN IDENTIFIED BY TEST DEFAULT TABLESPACE USERS QUOTA UNLIMITED ON
    USERS;

CREATE
    TABLE
        ORACLE_FULL_NN.TEST_DATASET(
...
<=>? @ [] ^_ `~] ', chr(33) || chr(34) || chr(35) || chr(36) || chr(37) || chr(38) || chr(39) || chr(40) || chr(41), N' テスト', power(10, -130), 123.89, 126.45, 126, 3.40282347E+038f);
INSERT INTO ORACLE_FULL_NN.TEST_DATASET VALUES (4, ' abc', BINARY_DOUBLE_INFINITY, to_date(' 9999 / 12 / 31 23:59:59 ',' yyyy / mm / dd hh24:mi:ss'), to_timestamp(' 2020 - 06 - 10 06:14:00.742123 ', ' YYYY - MM - DD HH24:MI:SS.FF'), to_timestamp_tz(' 21 - FEB - 2009 18:00:00.123456 - 5:00 ', ' DD - MON - YYYY HH24:MI:SS.FF TZH:TZM'), to_timestamp_tz(' 21 - FEB - 2009 18:00:00.000456 ', ' DD - MON - YYYY HH24:MI:SS.FF'), INTERVAL ' 9 ' MONTH, utl_raw.cast_to_raw(' SOME content here'), utl_raw.cast_to_raw(' SOME content here'), utl_raw.cast_to_raw(' SOME content here'), xmltype(' <? xml version = "1.0" encoding = "UTF-8" ?><list_configuration><config> 1 </ config ><config> 2 </ config ></ list_configuration > '), q' [ { | } ! "#$%&'()*+,-./:;<=>?@[]^_`~]', chr(33) || chr(34) || chr(35) || chr(36) || chr(37) || chr(38) || chr(39) || chr(40) || chr(41), N'テスト', 9.99999999999999999999 * power(10, 125), 123.89, 126.45, 126, BINARY_FLOAT_INFINITY);
