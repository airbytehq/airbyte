...
def test_s3_delete_tables(connection, s3_writer):
    ...
    assert connection.cursor.return_value.execute.mock_calls == [
        call("DROP TABLE IF EXISTS _airbyte_raw_dummy"),
        call("DROP TABLE IF EXISTS ext_airbyte_raw_dummy"),
    ]
...
def test_s3_data_auto_flush_one_table(mock_write, s3_writer):
    ...
    s3_writer.queue_write_data("dummy", "id1", 20200101, '{"key": "value"}')
    mock_write.assert_called_once_with(table=ANY, root_path="dummy_bucket/airbyte_output/dummy", filesystem=s3_writer.fs)
    assert len(s3_writer._buffer.keys()) == 0
    assert s3_writer._values == 0
...
def test_s3_data_auto_flush_multi_tables(mock_write, s3_writer):
    ...
    assert mock_write.mock_calls == [
        call(table=ANY, root_path="dummy_bucket/airbyte_output/dummy", filesystem=s3_writer.fs),
        call(table=ANY, root_path="dummy_bucket/airbyte_output/dummy2", filesystem=s3_writer.fs),
    ]
    assert len(s3_writer._buffer.keys()) == 0
    assert s3_writer._values == 0
...
def test_s3_final_flush(connection, s3_writer):
    ...
    s3_writer.flush()
    assert len(connection.cursor.return_value.execute.mock_calls) == 6
    self, ext_table_call1, _ = connection.cursor.return_value.execute.mock_calls[1]
    self, ext_table_call2, _ = connection.cursor.return_value.execute.mock_calls[4]
    expected_url1 = "URL = 's3://dummy_bucket/airbyte_output/dummy'"
    expected_url2 = "URL = 's3://dummy_bucket/airbyte_output/dummy2'"
