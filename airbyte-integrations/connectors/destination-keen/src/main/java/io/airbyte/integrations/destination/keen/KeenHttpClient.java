...
public class KeenHttpClient {
  ...
  public void eraseStream(
    ...
    final URI deleteUri =
        URI.create(
            String.format(
                KEEN_BASE_API_PATH + "/projects/%s/events/%s", projectId, streamToDelete));
    ...
    if (response.statusCode() != 204) {
      if (response.statusCode() == 429 && !retried) {
        ...
        throw new IllegalStateException(
            String.format(
                "Could not erase data from stream designed for overriding: "
                    + "%s. Error message: %s",
  ...
  public ArrayNode extract(final String streamName, final String projectId, final String apiKey)
      throws IOException, InterruptedException {
    final URI extractionUri =
        URI.create(
            String.format(
                keenBaseApiPath
                    + "/projects/%s/queries/extraction"
