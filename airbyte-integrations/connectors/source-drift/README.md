...
```
Once the command is done, you will find your connector image in your local docker registry: `airbyte/source-drift:dev`.
...
```Dockerfile
FROM airbyte/source-drift:latest
...
docker run airbyte/source-drift:dev spec
```
#### Run
...
4. Make the connector documentation and its changelog is up to date (`docs/integrations/sources/drift.md`).
