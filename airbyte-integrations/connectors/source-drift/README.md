...
This is the repository for the Drift configuration based source connector. For information about how
to use this connector within Airbyte, see
[the documentation](https://docs.airbyte.com/integrations/sources/drift).
...
[documentation](https://docs.airbyte.com/integrations/sources/drift) to generate the necessary
credentials. Then create a file `secrets/config.json` conforming to the `source_drift/spec.yaml`
...
`source drift test creds` and place them into `secrets/config.json`.
...
An image will be built with the tag `airbyte/source-drift:dev`.
