...
"""
Tests for the product_shipping_classes stream.
...
_STREAM_NAME = "product_shipping_classes"


def _get_response_template() -> list:
    template_path = Path(__file__).parent.parent / "resource" / "http" / "response" / "product_shipping_classes.json"
    return json.loads(template_path.read_text())


class TestProductShippingClassesFullRefresh(TestCase):
    """
    Tests for the product_shipping_classes stream in full refresh mode.

    The product_shipping_classes stream is a full refresh stream with OffsetIncrement pagination.
    """
    ...
    def test_read_records_single_page(self, http_mocker: HttpMocker) -> None:
        """Test reading a single page of product shipping classes."""
        http_mocker.get(
            WooCommerceRequestBuilder.product_shipping_classes_endpoint().with_default_params().build(),
        ...
        assert output.records[0].record.data["name"] == "Standard"

    @HttpMocker()
    def test_read_records_empty_response(self, http_mocker: HttpMocker) -> None:
        """Test reading when there are no product shipping classes."""
        http_mocker.get(
            WooCommerceRequestBuilder.product_shipping_classes_endpoint().with_default_params().build(),
    ...
    def test_read_records_pagination(self, http_mocker: HttpMocker) -> None:
        ...
        http_mocker.get(
            WooCommerceRequestBuilder.product_shipping_classes_endpoint().with_default_params().with_offset(100).build(),
