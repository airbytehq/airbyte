...
from .request_builder import get_orders_request, get_refunds_request
...
class TestFullRefresh(TestCase):
    ...
    def test_read_records(self, http_mocker: HttpMocker) -> None:
        ...
        for order_id in ["727", "723"]:
            http_mocker.get(
                get_refunds_request(order_id)
                .with_param("orderby", "id")
                .with_param("order", "asc")
                .with_param("dates_are_gmt", "true")
                .with_param("per_page", "100")
                .build(),
                get_json_http_response("refunds.json", 200),
                )

        # Read records
        output = self._read(config())

        # Check record count 2 orders, 2 refunds per order.
