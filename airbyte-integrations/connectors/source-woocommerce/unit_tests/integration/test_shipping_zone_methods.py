...
from .request_builder import get_shipping_zones_request, get_shipping_zone_methods_request
...
class TestFullRefresh(TestCase):
    ...
    def test_read_records(self, http_mocker: HttpMocker) -> None:
        ...
        for zone_id in ["0", "5"]:
            http_mocker.get(
                get_shipping_zone_methods_request(zone_id)
                .with_param("orderby", "id")
                .with_param("order", "asc")
                .with_param("dates_are_gmt", "true")
                .with_param("per_page", "100")
                .build(),
                get_json_http_response("shipping_zone_methods.json", 200),
                )

        # Read records
        output = self._read(config())

        # Check record count: 4 methods, 2 per zone.
