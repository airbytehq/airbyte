...
class TestProductsIncremental(TestCase):
    ...
    The products stream uses DatetimeBasedCursor with:
    - step: P30D (30 days)
    ...
    def test_read_records_single_slice(self, http_mocker: HttpMocker) -> None:
        """
        Test reading products with a single date slice.
        ...
        assert (
            latest_state.__dict__["date_modified_gmt"] == "2024-03-10T12:30:00"
        ), "State should be updated to the date_modified_gmt timestamp of the latest record"

    @HttpMocker()
    @freezegun.freeze_time("2024-01-15T12:00:00Z")
    def test_read_records_empty_response(self, http_mocker: HttpMocker) -> None:
        """Test reading when there are no products in the date range."""
        http_mocker.get(
            WooCommerceRequestBuilder.products_endpoint()
            .with_default_params()
            .with_modified_after("2024-01-01T00:00:00")
    ...
    def test_read_records_with_pagination(self, http_mocker: HttpMocker) -> None:
        """
        Test reading products with pagination using OffsetIncrement.
        ...
        Testing pagination here and in test_orders.py is sufficient to verify the
