...
def test_consume_one(components_module):
    """Multiple consumptions of 1 cred will reach limit"""
    credit = components_module.CallCredit(balance=3, reload_period=1)
    ts_1 = time.time()
    for i in range(4):
        credit.consume(1)
    ts_2 = time.time()
...
def test_consume_many(components_module):
...
def test_sends_request_with_default_parameters_and_receives_response(components_module, requests_per_minute, expected_call_credit_cost):
    ...
    with patch.object(components_module, "CallCredit") as MockCallCredit:
        mock_call_credit_instance = MagicMock()
        MockCallCredit.return_value = mock_call_credit_instance

        requester = components_module.FreshdeskRequester(
        ...
        with patch.object(components_module.HttpRequester, "send_request", return_value=MagicMock()):
...
def test_freshdesk_tickets_incremental_requester_send_request(
    ...
    with patch.object(components_module, "CallCredit") as mock_call_credit:
        mock_call_credit_instance = MagicMock()
