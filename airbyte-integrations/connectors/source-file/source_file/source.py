...
class SourceFile(Source):
    ...
    def _validate_and_transform(config: Mapping[str, Any]):
        if "reader_options" in config:
            try:
                ...
                if not isinstance(config["reader_options"], dict):
                    msg = "Field 'reader_options' is not a valid JSON object. Please provide key-value pairs, See field description for examples."
                    raise ConfigurationError(
                        msg,
                    )
            except ValueError:
                msg = "Field 'reader_options' is not valid JSON object. https://www.json.org/"
        ...
        if parse_result.netloc == "docs.google.com" and parse_result.path.lower().startswith("/spreadsheets/"):
            msg = f"Failed to load {config['url']}: please use the Official Google Sheets Source connector"
            raise ConfigurationError(msg)
        return config
