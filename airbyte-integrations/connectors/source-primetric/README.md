...
to generate the necessary credentials. Then create a file `secrets/config.json` conforming to the `src/source_primetric/spec.yaml` file.
...
poetry run source-primetric read --config secrets/config.json --catalog sample_files/configured_catalog.json
```
