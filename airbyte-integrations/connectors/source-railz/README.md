...
This is the repository for the Railz source connector, written in Python.
...
```
poetry run source-railz spec
poetry run source-railz check --config secrets/config.json
poetry run source-railz discover --config secrets/config.json
poetry run source-railz read --config secrets/config.json --catalog sample_files/configured_catalog.json
```
...
1. Make sure your changes are passing our test suite: `airbyte-ci connectors --name=source-railz test`
