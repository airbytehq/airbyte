...
class TeradataJdbcSourceAcceptanceTest extends JdbcSourceAcceptanceTest {
  ...
  private static final Logger LOGGER =
      LoggerFactory.getLogger(TeradataJdbcSourceAcceptanceTest.class);

  private JsonNode staticConfig;

  static {
    COLUMN_CLAUSE_WITH_PK =
        "id INTEGER NOT NULL, name VARCHAR(200) NOT NULL, updated_at DATE NOT NULL";
    ...
    INSERT_TABLE_WITHOUT_CURSOR_TYPE_QUERY =
        "INSERT INTO %s VALUES('POLYGON((1 1, 1 3, 6 3, 6 0, 1 1))');";
  ...
  public void initEnvironment() throws ExecutionException, InterruptedException {
    ...
    TeradataHttpClient teradataHttpClient =
        new TeradataHttpClient(staticConfig.get("env_host").asText());
    ...
    var response = teradataHttpClient
        .createEnvironment(request, staticConfig.get("env_token").asText())
        .get();
  ...
  public void cleanupEnvironment() throws ExecutionException, InterruptedException {
    ...
    teradataHttpClient
        .deleteEnvironment(request, staticConfig.get("env_token").asText())
        .get();
  }
  ...
  public void setup() throws Exception {
    executeStatements(
        List.of(statement -> statement.executeUpdate(
            "CREATE DATABASE \"database_name\" AS PERMANENT = 120e6, SPOOL = 120e6;")),
        staticConfig.get("host").asText(),
        staticConfig.get("username").asText(),
        staticConfig.get("password").asText());
    super.setup();
  }
