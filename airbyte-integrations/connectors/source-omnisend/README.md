...
to generate the necessary credentials. Then create a file `secrets/config.json` conforming to the `src/source_omnisend/spec.yaml` file.
...
```
poetry run source-omnisend spec
poetry run source-omnisend check --config secrets/config.json
poetry run source-omnisend discover --config secrets/config.json
poetry run source-omnisend read --config secrets/config.json --catalog sample_files/configured_catalog.json
```
