...
to generate the necessary credentials. Then create a file `secrets/config.json` conforming to the `source_chartmogul/spec.yaml` file.
...
poetry run source-chartmogul read --config secrets/config.json --catalog sample_files/configured_catalog.json
```
