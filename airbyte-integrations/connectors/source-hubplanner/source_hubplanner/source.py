...
class HubplannerStream(HttpStream, ABC):
    ...
    `class HubplannerStream(HttpStream, ABC)` which is the current class
    `class Customers(HubplannerStream)` contains behavior to pull data for customers using v1/customers
    `class Employees(HubplannerStream)` contains behavior to pull data for employees using v1/employees

    If some streams implement incremental sync, it is typical to create another class
    `class IncrementalHubplannerStream((HubplannerStream), ABC)` then have concrete stream implementations extend it. An example
    is provided below.
...
class Customers(HubplannerStream):
    """
...
class Employees(IncrementalHubplannerStream):
    """
    TODO: Change class name to match the table/data source this stream corresponds to.
    """
...
class SourceHubplanner(AbstractSource):
    ...
    def streams(self, config: Mapping[str, Any]) -> List[Stream]:
        ...
        """
        authenticator = HubplannerAuthenticator(token=config["api_key"])
        return [Projects(authenticator=authenticator)]
