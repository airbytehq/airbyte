...
from airbyte_cdk.models import AirbyteMessage, AirbyteStream, Type
...
class Client:
    ...
    def batch_buffered_write(self, input_messages:Iterable[AirbyteMessage]) -> Iterable[AirbyteMessage]:
    ...
    def delete_stream_entries(self, stream:AirbyteStream):
        ...
        try:
            ...
            if e.message.startswith("Delete class from schema! Unexpected status code: 400"):
                logging.info(f"Class {class_name} did not exist.")
            else:
                raise e
    ...
    def map_airbyte_stream_to_weaviate_class(self, stream:AirbyteStream) -> dict:
        ...
        for k, v in stream.json_schema.get("properties").items():
            ...
            # We dataType must be a list of types, and cant have null as a datatype in our dataType list entry
        ...
        # add specific config options 
        shouldIndexNull = self.config["indexNull"]
        if shouldIndexNull:
