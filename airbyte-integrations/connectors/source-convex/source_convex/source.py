...
class ConvexStream(HttpStream, ABC):
    """
    ...
    `class ConvexStream(HttpStream, ABC)` which is the current class
    `class Customers(ConvexStream)` contains behavior to pull data for customers using v1/customers
    `class Employees(ConvexStream)` contains behavior to pull data for employees using v1/employees

    If some streams implement incremental sync, it is typical to create another class
    `class IncrementalConvexStream((ConvexStream), ABC)` then have concrete stream implementations extend it. An example
    is provided below.
...
class IncrementalConvexStream(ConvexStream, ABC):
    """
...
class Employees(IncrementalConvexStream):
    """
    TODO: Change class name to match the table/data source this stream corresponds to.
    """
...
def convex_shape_variant_to_json(shape_variant):
    ...
    if variant == 'Object':
        return {"type": "object", "properties": {
            field["fieldName"]: convex_shape_to_json(field["shape"])
            for field in shape_variant["fields"]
        }}
    if variant == "Float64":
        return {"type": "number"}
    if variant == "Id":
...
class ConvexStream(HttpStream):
    ...
    def get_json_schema(self) -> Mapping[str, Any]:
        return convex_shape_to_json(self.shape)
    ...
    def request_params(
        ...
        if self.USE_DELTAS:
            ...
            args = [self.table_name, 1000]
            return {
                "path": "_system/frontend/listTableScan",
