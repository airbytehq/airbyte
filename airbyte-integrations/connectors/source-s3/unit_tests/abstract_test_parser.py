...
class AbstractTestParser(ABC):
    ...
    def _get_readmode(self, test_name: str, test_file: Mapping[str, Any]) -> str:
    ...
    def test_get_inferred_schema(self, test_file: Mapping[str, Any]) -> None:
    ...
    def test_stream_records(self, test_file: Mapping[str, Any]) -> NoReturn:  # type: ignore
        with smart_open(test_file["filepath"], self._get_readmode("stream_records", test_file)) as f:
            if "test_stream_records" in test_file["fails"]:
                with pytest.raises(Exception) as e_info:
                    [print(r) for r in test_file["AbstractFileParser"].stream_records(f)]  # type: ignore
                    self.logger.debug(str(e_info))
            else:
