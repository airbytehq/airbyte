...
class Cursor(DefaultFileBasedCursor):
    ...
    def get_state(self) -> StreamState:
        state = {"history": self._file_to_datetime_history, self.CURSOR_FIELD: self._get_cursor()}
        if self._v3_migration_start_datetime:
            state[Cursor._V3_MIN_SYNC_DATE_FIELD] = self._v3_migration_start_datetime.strftime(DefaultFileBasedCursor.DATE_TIME_FORMAT)
        return state
    ...
    def _is_legacy_state(value: StreamState) -> bool:
        if not value:
            return False
        try:
            history_keys = value.get("history", {}).keys()
            if history_keys:
                datetime.strptime(next(iter(history_keys)), Cursor._DATE_FORMAT)
