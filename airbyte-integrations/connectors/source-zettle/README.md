...
This is the repository for the Zettle source connector, written in Python.
For information about how to use this connector within Airbyte,
see [the documentation](https://docs.airbyte.io/integrations/sources/zettle).
...
```
./gradlew :airbyte-integrations:connectors:source-zettle:build
```

#### Create credentials

**If you are a community contributor**, follow the instructions in
the [documentation](https://docs.airbyte.io/integrations/sources/zettle)
to generate the necessary credentials. Then create a file `secrets/config.json` conforming to
the `source_zettle/spec.yaml` file.
...
**If you are an Airbyte core member**, copy the credentials in Lastpass under the secret name `source zettle test creds`
...
```
docker build . -t airbyte/source-zettle:$IMAGE_TAG  --platform linux/amd64

docker tag airbyte/source-zettle:$IMAGE_TAG askeladden/source-zettle:$IMAGE_TAG
docker push askeladden/source-zettle:$IMAGE_TAG
```

You can also build the connector image via Gradle:

```
./gradlew :airbyte-integrations:connectors:source-zettle:airbyteDocker
```
...
```
docker run --rm airbyte/source-zettle:dev spec
docker run --rm -v $(pwd)/secrets:/secrets airbyte/source-zettle:dev check --config /secrets/config.json
docker run --rm -v $(pwd)/secrets:/secrets airbyte/source-zettle:dev discover --config /secrets/config.json
docker run --rm -v $(pwd)/secrets:/secrets -v $(pwd)/integration_tests:/integration_tests airbyte/source-zettle:dev read --config /secrets/config.json --catalog /integration_tests/configured_catalog.json
```
...
```
./gradlew :airbyte-integrations:connectors:source-zettle:unitTest
```

To run acceptance and custom integration tests:

```
./gradlew :airbyte-integrations:connectors:source-zettle:integrationTest
```
