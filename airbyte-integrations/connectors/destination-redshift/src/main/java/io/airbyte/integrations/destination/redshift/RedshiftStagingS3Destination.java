...
public class RedshiftStagingS3Destination extends AbstractJdbcDestination<RedshiftState> implements Destination {
  ...
  public SerializedAirbyteMessageConsumer getSerializedMessageConsumer(final JsonNode config,
    ...
    final List<Migration<RedshiftState>> migrations = getMigrations(database, databaseName, sqlGenerator, redshiftDestinationHandler);
    if (disableTypeDedupe) {
      typerDeduper =
          new NoOpTyperDeduperWithV1V2Migrations<>(sqlGenerator, redshiftDestinationHandler, parsedCatalog, migrator, v2TableMigrator, migrations);
    } else {
      typerDeduper =
          new DefaultTyperDeduper<>(sqlGenerator, redshiftDestinationHandler, parsedCatalog, migrator, v2TableMigrator, migrations);
    }
