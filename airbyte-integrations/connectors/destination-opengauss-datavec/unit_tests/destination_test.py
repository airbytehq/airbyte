...
from destination_opengauss_datavec.destination import BATCH_SIZE, DestinationOpenGaussDataVec
...
class TestDestinationOpenGaussDataVec(unittest.TestCase):
    ...
    def test_check_with_init_indexer_errors(self):
        destination = DestinationOpenGaussDataVec()
        with patch("destination_opengauss_datavec.destination.OpenGaussDataVecIndexer", side_effect=Exception("Indexer Error")):
            result = destination.check(self.logger, self.config)
        self.assertEqual(result.status, Status.FAILED)
    ...
    def test_write(self, MockedEmbedder, MockedOpenGaussDataVecIndexer, MockedWriter):
        ...
        MockedOpenGaussDataVecIndexer.assert_called_once_with(
            self.config_model.indexing,
            mock_embedder.embedding_dimensions,
            self.config_model.processing,
            self.config_model.omit_raw_text,
        )
        MockedWriter.assert_called_once_with(
            self.config_model.processing, mock_indexer, mock_embedder, batch_size=BATCH_SIZE, omit_raw_text=False
        )
