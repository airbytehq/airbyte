...
class DestinationOpenGaussDataVec(Destination):
    ...
    def _init_indexer(self, config: ConfigModel):
        try:
            ...
            self.indexer = OpenGaussDataVecIndexer(
                config.indexing, self.embedder.embedding_dimensions, config.processing, config.omit_raw_text
            )
    ...
    def write(
        ...
        yield self._log(Level.INFO, "Starting openGauss DataVec destination write...")
        try:
            ...
            yield AirbyteMessage(
                ...
                trace=AirbyteTraceMessage(
                    ...
                    error=AirbyteErrorTraceMessage(
                        message=f"openGauss DataVec destination write failed: {e}",
    ...
    def check(self, logger: logging.Logger, config: Mapping[str, Any]) -> AirbyteConnectionStatus:
        logger.info("Starting openGauss DataVec destination check...")
        try:
