...
def test_get_partition_value_from_config(config, expected):
    router = MultipleAdvertiserIdsPerPartition(
        ...
        config=config,
        parameters={
            "path_in_config": [["credentials", "advertiser_id"], ["environment", "advertiser_id"]], "partition_field": "advertiser_id"
        }
    )
...
@pytest.mark.parametrize(
    ...
    [
        ({"credentials": {"auth_type": "oauth2.0", "advertiser_id": "11111111111"}},
         [{"advertiser_ids": '["11111111111"]', "parent_slice": {}}], None),
        ...
        (
                ...
                {"code": 0, "message": "ok", "data":
                    {"list": [{"advertiser_id": "11111111", "advertiser_name": "name"},
...
def test_stream_slices_multiple(config, expected, requests_mock, json_data):
    ...
    router = MultipleAdvertiserIdsPerPartition(
        parent_stream_configs=[ParentStreamConfig(
            ...
            stream=advertiser_ids_stream,
            parameters={}
        )],
        config=config,
        parameters={
            "path_in_config": [["credentials", "advertiser_id"], ["environment", "advertiser_id"]], "partition_field": "advertiser_ids"
        }
    )
    if json_data:
        requests_mock.get(
...
@pytest.mark.parametrize(
    "config, expected, json_data",
    [
        ({"credentials": {"auth_type": "oauth2.0", "advertiser_id": "11111111111"}}, [{"advertiser_id": "11111111111", "parent_slice": {}}],
         None),
        ({"environment": {"advertiser_id": "2222222222"}}, [{"advertiser_id": "2222222222", "parent_slice": {}}], None),
        (
                {"credentials": {"auth_type": "oauth2.0", "access_token": "access_token"}},
                [{"advertiser_id": "11111111", "parent_slice": {}},
                 {"advertiser_id": "22222222", "parent_slice": {}}],
                {"code": 0, "message": "ok",
                 "data": {"list": [
