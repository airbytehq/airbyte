...
(`response.get('code') == 40100`) to detect rate limiting, and separate predicates for codes `60001`,
`50000`, `51041`, and `51004` retry transient server-side errors, explicitly classified as
`transient_error`, while (`response.get('code') != 0`) detects general API errors.
