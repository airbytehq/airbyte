...
```
Once the command is done, you will find your connector image in your local docker registry: `airbyte/source-zapier-supported-storage:dev`.
...
```Dockerfile
FROM airbyte/source-zapier-supported-storage:latest
...
docker run airbyte/source-zapier-supported-storage:dev spec
```
#### Run
...
4. Make the connector documentation and its changelog is up to date (`docs/integrations/sources/zapier-supported-storage.md`).
