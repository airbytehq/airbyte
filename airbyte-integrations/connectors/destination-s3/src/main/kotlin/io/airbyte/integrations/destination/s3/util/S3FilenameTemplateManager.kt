...
class S3FilenameTemplateManager {
    ...
    fun applyPatternToFilename(parameterObject: S3FilenameTemplateParameterObject): String {
        // sanitize fileFormat
        val sanitizedFileFormat = parameterObject
            .fileNamePattern
            .trim { it <= ' ' }
            .replace(" ".toRegex(), "_")
        ...
        return Optional.ofNullable(parameterObject.objectPath).orElse(StringUtils.EMPTY) + stringSubstitutor.replace(
            sanitizedFileFormat
        )
    }
    ...
    private fun fillTheMapWithDefaultPlaceHolders(
        ...
        val valuesMap = processExtendedPlaceholder(currentTimeMillis, stringToReplaceWithPlaceholder)

        val defaultDateFormat: DateFormat = SimpleDateFormat(S3DestinationConstants.YYYY_MM_DD_FORMAT_STRING)
        defaultDateFormat.timeZone = TimeZone.getTimeZone(UTC)

        // here we set default values for supported placeholders.
        valuesMap["date"] = Optional.ofNullable(defaultDateFormat.format(currentTimeMillis)).orElse(StringUtils.EMPTY)
        valuesMap["timestamp"] = Optional.ofNullable(currentTimeMillis.toString()).orElse(StringUtils.EMPTY)
        valuesMap["sync_id"] = Optional.ofNullable(System.getenv("WORKER_JOB_ID")).orElse(StringUtils.EMPTY)
        valuesMap["format_extension"] = Optional.ofNullable(parameterObject.fileExtension).orElse(StringUtils.EMPTY)
        valuesMap["part_number"] = Optional.ofNullable(parameterObject.partId).orElse(StringUtils.EMPTY)
    ...
    private fun processExtendedPlaceholder(
        ...
        val matcher = pattern.matcher(stringToReplaceWithPlaceholder)

        while (matcher.find()) {
            val splitByColon = matcher.group(1).split(":".toRegex()).dropLastWhile { it.isEmpty() }
                .toTypedArray()
