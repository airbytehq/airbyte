...
class S3V2WriteOpOverride(
    ...
    override suspend fun execute() = coroutineScope {
        ...
        val reservationSize = (availableMemory * config.maxMemoryRatioReservedForParts).toLong()
        val memoryManager = ReservationManager(reservationSize)
        log.info {
            "Reserving ${config.maxMemoryRatioReservedForParts} * ${availableMemory}b = ${reservationSize}b for parts"
        }
        try {
                    ...
                    launch {
                        reservingDeserializingInputFlow.collect { (_, reservation) ->
                            when (val message = reservation.value) {
                                ...
                                is DestinationFile -> {
                                    if (config.devNull) {
                                        log.info { "Dev-nulling file ${message.fileMessage.fileUrl}" }
                                        File(message.fileMessage.fileUrl!!).delete()
                                        return@collect
                                    }

                                    syncManager.getStreamManager(message.stream.descriptor)
                                        .incrementReadCount()
                                    ...
                                    val stream = catalog.getStream(message.stream.descriptor)
