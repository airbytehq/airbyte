# Whisky-Hunter source connector


This is the repository for the Whisky-Hunter source connector, written in Python.
...
```
poetry run source-whisky-hunter spec
poetry run source-whisky-hunter check --config secrets/config.json
poetry run source-whisky-hunter discover --config secrets/config.json
poetry run source-whisky-hunter read --config secrets/config.json --catalog sample_files/configured_catalog.json
```
...
An image will be available on your host with the tag `airbyte/source-whisky-hunter:dev`.
...
1. Make sure your changes are passing our test suite: `airbyte-ci connectors --name=source-whisky-hunter test`
2. Bump the connector version (please follow [semantic versioning for connectors](https://docs.airbyte.com/contributing-to-airbyte/resources/pull-requests-handbook/#semantic-versioning-for-connectors)): 
    - bump the `dockerImageTag` value in in `metadata.yaml`
    - bump the `version` value in `pyproject.toml`
3. Make sure the `metadata.yaml` content is up to date.
4. Make sure the connector documentation and its changelog is up to date (`docs/integrations/sources/whisky-hunter.md`).
