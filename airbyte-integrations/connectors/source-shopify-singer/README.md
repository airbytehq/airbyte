...
```
docker build . -t airbyte/source-shopify-singer:dev
```
...
docker run --rm -v $(pwd)/secrets:/secrets airbyte/source-shopify-singer:dev check --config /secrets/config.json
docker run --rm -v $(pwd)/secrets:/secrets airbyte/source-shopify-singer:dev discover --config /secrets/config.json
docker run --rm -v $(pwd)/secrets:/secrets -v $(pwd)/sample_files:/sample_files airbyte/source-shopify-singer:dev read --config /secrets/config.json --catalog /sample_files/configured_catalog.json
```

### Integration Tests
1. From the airbyte project root, run `./gradlew :airbyte-integrations:connectors:source-shopify-singer:integrationTest` to run the standard integration test suite.
