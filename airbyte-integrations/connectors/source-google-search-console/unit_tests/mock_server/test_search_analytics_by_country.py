...
"""
Mock server tests for the search_analytics_by_country stream.

The search_analytics_by_country stream is an incremental stream that:
...
_STREAM_NAME = "search_analytics_by_country"
...
def _build_search_analytics_row(
    date: str, country: str, clicks: int = 100, impressions: int = 1000, ctr: float = 0.1, position: float = 5.0
) -> dict:
    """Build a single search analytics row with date and country dimensions."""
    return {
        "keys": [date, country],
        "clicks": clicks,
...
class TestSearchAnalyticsByCountryStream(TestCase):
    """Tests for the search_analytics_by_country stream.

    This stream partitions by site_urls AND search_types (6 types).
    Uses dimensions: ["date", "country"] for the API request.
    """
    ...
    def test_full_refresh_single_site(self, http_mocker: HttpMocker) -> None:
            ...
            if body.get("type") == "web":
                return json.dumps(
                    _build_search_analytics_response(
                        [
                            _build_search_analytics_row("2024-01-01", "usa", clicks=100, impressions=1000),
                            _build_search_analytics_row("2024-01-01", "gbr", clicks=50, impressions=500),
                            _build_search_analytics_row("2024-01-02", "usa", clicks=150, impressions=1500),
                        ]
                    )
                )
        ...
        for body in captured_bodies:
            assert body.get("dimensions") == ["date", "country"], f"Expected dimensions ['date', 'country'], got {body.get('dimensions')}"

        # Should have 3 records (from web search type)
        assert len(records) == 3

        # Verify record fields include country
        record_countries = {r.record.data["country"] for r in records}
        assert "usa" in record_countries
        assert "gbr" in record_countries
    ...
    def test_error_handler_ignore_permission_error(self, http_mocker: HttpMocker) -> None:
        """Test IGNORE error handler for permission errors (403).
            ...
            if body.get("type") == "web":
                return json.dumps(
                    _build_search_analytics_response(
                        [
                            _build_search_analytics_row("2024-01-01", "usa", clicks=100, impressions=1000),
                            _build_search_analytics_row("2024-01-02", "gbr", clicks=150, impressions=1500),
                        ]
                    )
                )
            return json.dumps(_build_search_analytics_response([]))
        ...
        def search_analytics_callback(request: rm.request._RequestObjectProxy, context: Any) -> str:
            ...
            if body.get("type") == "web":
                return json.dumps(
                    _build_search_analytics_response(
                        [
                            _build_search_analytics_row("2024-01-03", "usa", clicks=200, impressions=2000),
                            _build_search_analytics_row("2024-01-04", "deu", clicks=250, impressions=2500),
                        ]
                    )
                )
            return json.dumps(_build_search_analytics_response([]))
