...
class TestBudgetStream(TestBulkStream):
    ...
    def test_incremental_read_with_state(self, http_mocker: HttpMocker):
        ...
        assert len(output.records) == 8
        assert output.most_recent_state.get(self.stream_name, {}).get(self.account_id, {}) == {"Modified Time": "2024-01-30T12:54:12.028+00:00"}

        previous_state = state[0].stream.stream_state.dict()
        # gets DownloadParams object
        assert service_call_mock.call_args.args[0].last_sync_time_in_utc == pendulum.parse(previous_state[self.account_id]["Modified Time"])
