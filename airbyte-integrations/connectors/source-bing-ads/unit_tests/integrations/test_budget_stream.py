...
class TestBudgetStream(TestBulkStream):
    ...
    def test_return_records_from_given_csv_file(self, http_mocker: HttpMocker):
        ...
        output = self.read_stream("budget", SyncMode.full_refresh, self._config, [], "budget")
        assert len(output.records) == 1

    @HttpMocker()
    def test_return_logged_info_for_empty_csv_file(self, http_mocker: HttpMocker):
        self._get_client(http_mocker, self._config)
        output = self.read_stream("budget", SyncMode.full_refresh, self._config, [], "budget_empty")
    ...
    def test_return_logged_fatal_for_ioerror(self, http_mocker: HttpMocker):
        self._get_client(http_mocker, self._config)
        output = self.read_stream("budget", SyncMode.full_refresh, self._config, [], stream_data_file=None)
    ...
    def test_reads_with_chunks_from_given_csv_file_with_big_amount_of_data(self, http_mocker: HttpMocker):
        self._get_client(http_mocker, self._config)
        output = self.read_stream("budget", SyncMode.full_refresh, self._config, [], "budget_big_data")
    ...
    def test_incremental_read_cursor_value_matches_value_from_most_recent_record(self, http_mocker: HttpMocker):
        self._get_client(http_mocker, self._config)
        output = self.read_stream("budget", SyncMode.incremental, self._config, [], "budget_with_cursor_value")
        assert len(output.records) == 8
        assert output.most_recent_state.get("budget", {}).get("180535609", {}) == {"Modified Time": "2024-01-01T12:54:12.028+00:00"}

    @HttpMocker()
    def test_incremental_read_with_state(self, http_mocker: HttpMocker):
        state = self._state("budget_state")
        self._get_client(http_mocker, self._config)
        output = self.read_stream("budget", SyncMode.incremental, self._config, [], "budget_with_state", state)
        assert len(output.records) == 8
        assert output.most_recent_state.get("budget", {}).get("180535609", {}) == {"Modified Time": "2024-01-02T12:54:12.028+00:00"}
