...
to generate the necessary credentials. Then create a file `secrets/config.json` conforming to the `src/source_freshcaller/spec.yaml` file.
...
```
poetry run source-freshcaller spec
poetry run source-freshcaller check --config secrets/config.json
poetry run source-freshcaller discover --config secrets/config.json
poetry run source-freshcaller read --config secrets/config.json --catalog sample_files/configured_catalog.json
```
