...
/**
 * The object which is mapped to the Databricks source configuration JSON.
 *
 * Use [DatabricksSourceConfiguration] instead wherever possible. This object also allows injecting
 * values through Micronaut properties, this is made possible by the classes named
 * `MicronautPropertiesFriendly.*`.
 */
@JsonSchemaTitle("Databricks Source Spec")
@JsonPropertyOrder(
    value =
        [
            "credentials",
...
class DatabricksSourceConfigurationSpecification : ConfigurationSpecification() {
    ...
    @JsonPropertyDescription(
        "The host domain of the databricks instance (must include the account, region, cloud environment, and end with databrickscomputing.com)."
    )
    lateinit var host: String

    @JsonProperty("http_path")
    @JsonSchemaTitle("HTTP Path")
    @JsonPropertyDescription("The warehouse you created for Airbyte to access data.")
    @JsonSchemaInject(json = """{"order":3}""")
    lateinit var http_path: String
    ...
    @JsonPropertyDescription(
        "The source Databricks schema tables. Leave empty to access tables from multiple schemas."
    )
...
@JsonSubTypes(
    JsonSubTypes.Type(
        value = PersonalAccessTokenCredentialsSpecification::class,
...
@JsonSchemaTitle("Personal Access Token Authentication")
@JsonSchemaInject(json = """{"order":1}""")
data class PersonalAccessTokenCredentialsSpecification(
    @JsonProperty("auth_type")
    @JsonSchemaInject(json = """{"order":0}""")
    val authType: String = "Personal Access Token",
    @JsonProperty("token")
    @JsonSchemaTitle("Token")
    @JsonPropertyDescription("The token you created to allow Airbyte to access the database.")
    @JsonSchemaInject(json = """{"order":1}""")
    val token: String,
