...
from .response_builder import configuration_incompatible_response, plan_response
...
class TestPlanStream(TestCase):
    ...
    def test_incremental_emits_state(self, http_mocker: HttpMocker) -> None:
        ...
        assert latest_cursor_value == 1705312800  # From plan.json
    ...
    def test_error_configuration_incompatible_ignored(self, http_mocker: HttpMocker) -> None:
        """Test configuration_incompatible error is ignored for plan stream as configured in manifest."""
        http_mocker.get(
            RequestBuilder.plans_endpoint().with_any_query_params().build(),
