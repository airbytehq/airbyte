...
def _a_request() -> ChargebeeRequestBuilder:
    return ChargebeeRequestBuilder.plan_endpoint(_SITE, _SITE_API_KEY)
...
class IncrementalTest(TestCase):
    ...
    def test_given_no_initial_state_when_read_then_return_state_based_on_cursor_field(self, http_mocker: HttpMocker) -> None:
        ...
        http_mocker.get(
            _a_request().with_sort_by_asc(_CURSOR_FIELD).with_include_deleted(True).with_updated_at_btw([self._start_date_in_seconds, int((self._start_date + timedelta(days=2)).timestamp())]).build(),
        ...
        assert output.most_recent_state == { _STREAM_NAME: {_CURSOR_FIELD: str(int((self._start_date + timedelta(seconds=2678399)).timestamp()))}}
