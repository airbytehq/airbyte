...
from airbyte_protocol.models import ConfiguredAirbyteCatalog, FailureType, SyncMode, StreamDescriptor, AirbyteStateBlob
...
def _a_request() -> ChargebeeRequestBuilder:
    return ChargebeeRequestBuilder.coupon_endpoint(_SITE, _SITE_API_KEY)
...
class FullRefreshTest(TestCase):
    ...
    def test_given_multiple_pages_of_records_read_and_returned(self, http_mocker: HttpMocker) -> None:
        # Tests pagination
        http_mocker.get(
            _a_request().with_updated_at_btw([self._start_date_in_seconds, self._now_in_seconds]).build(),
            _a_response().with_record(_a_record()).with_pagination().build()
        )
        http_mocker.get(
            _a_request().with_updated_at_btw([self._start_date_in_seconds, self._now_in_seconds]).with_offset("[1707076198000,57873868]").build(),
