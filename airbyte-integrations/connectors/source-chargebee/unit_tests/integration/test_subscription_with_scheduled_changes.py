...
def _a_request() -> ChargebeeRequestBuilder:
    return ChargebeeRequestBuilder.subscription_with_scheduled_changes_endpoint(_SITE, _SITE_API_KEY)
...
class FullRefreshTest(TestCase):
    ...
    def test_given_records_returned_with_custom_field_transformation(self, http_mocker: HttpMocker) -> None:
        ...
        http_mocker.get(
            _a_request().with_any_query_params().build(),
            _a_response().with_record(_a_record().with_field(NestedPath(["subscription", "cf_my_custom_field"]), "my_custom_value")).build()
        )
        output = self._read(_config().with_start_date(self._start_date))
    ...
    def test_given_http_status_400_when_read_then_stream_is_ignored(self, http_mocker: HttpMocker) -> None:
        ...
        assert len(output.get_stream_statuses(f"{_STREAM_NAME}")) == 0
