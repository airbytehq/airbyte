...
class FullRefreshTest(TestCase):
    ...
    def test_given_multiple_pages_of_records_read_and_returned(self, http_mocker: HttpMocker) -> None:
        # Tests pagination
        http_mocker.get(_a_request().build(), _a_response().with_record(_a_record()).with_pagination().build())
        http_mocker.get(_a_request().with_offset("[1707076198000,57873868]").build(), _a_response().with_record(_a_record()).build())
...
class IncrementalTest(TestCase):
    ...
    def test_given_initial_state_value_when_read_then_state_is_updated_to_most_recent_cursor_value(self, http_mocker: HttpMocker) -> None:
        ...
        assert most_recent_state.stream_state == AirbyteStateBlob(
            migrated_at=record_cursor_value, prior_state={_CURSOR_FIELD: state_cursor_value}
        )

    @HttpMocker()
    def test_given_record_returned_with_cursor_value_before_state_record_is_not_read_and_state_not_updated(
        ...
        assert most_recent_state.stream_descriptor == StreamDescriptor(name=_STREAM_NAME)
        assert most_recent_state.stream_state == AirbyteStateBlob(
            migrated_at=state_cursor_value, prior_state={_CURSOR_FIELD: state_cursor_value}
        )
