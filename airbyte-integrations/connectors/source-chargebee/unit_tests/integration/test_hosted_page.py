...
from .response_builder import configuration_incompatible_response, hosted_page_response
...
class TestHostedPageStream(TestCase):
    """Tests for the hosted_page stream."""

    @HttpMocker()
    def test_read_records(self, http_mocker: HttpMocker) -> None:
        """Basic read test for hosted_page stream."""
        http_mocker.get(
            RequestBuilder.hosted_pages_endpoint().with_any_query_params().build(),
            hosted_page_response(),
        )

        output = read_output(config_builder=config(), stream_name=_STREAM_NAME)
        assert len(output.records) == 1
        assert output.records[0].record.data["id"] == "hosted_page_001"
    ...
    def test_incremental_emits_state(self, http_mocker: HttpMocker) -> None:
        ...
        assert latest_cursor_value == 1705312800  # From hosted_page.json
    ...
    def test_incremental_sync_with_state_and_params(self, http_mocker: HttpMocker) -> None:
        ...
        http_mocker.get(
            RequestBuilder.hosted_pages_endpoint()
        ...
        assert record["id"] == "hosted_page_001"
    ...
    def test_error_configuration_incompatible_ignored(self, http_mocker: HttpMocker) -> None:
        """Test configuration_incompatible error is ignored for hosted_page stream as configured in manifest."""
        http_mocker.get(
            RequestBuilder.hosted_pages_endpoint().with_any_query_params().build(),
