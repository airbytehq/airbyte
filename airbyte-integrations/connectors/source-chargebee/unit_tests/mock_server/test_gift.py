...
from .response_builder import configuration_incompatible_response, gift_response
...
class TestGiftStream(TestCase):
    ...
    def test_incremental_emits_state(self, http_mocker: HttpMocker) -> None:
        ...
        assert latest_cursor_value == 1705312800  # From gift.json
    ...
    def test_incremental_sync_with_state_and_params(self, http_mocker: HttpMocker) -> None:
        ...
        2. Correct request parameters are sent (only limit - NO sort_by, include_deleted, or updated_at[between])
        3. State advances to latest record's cursor value

        Note: gift stream uses updated_at cursor but has NO request_parameters at all (only limit).
        """
        ...
        # Note: Gift stream does NOT use sort_by, include_deleted, or updated_at[between]
        http_mocker.get(
            RequestBuilder.gifts_endpoint().with_limit(100).build(),
            gift_response(),
        )
    ...
    def test_error_configuration_incompatible_ignored(self, http_mocker: HttpMocker) -> None:
        """Test configuration_incompatible error is ignored for gift stream as configured in manifest."""
        http_mocker.get(
            RequestBuilder.gifts_endpoint().with_any_query_params().build(),
