...
from .response_builder import configuration_incompatible_response, promotional_credit_response
from .utils import config, read_output


_STREAM_NAME = "promotional_credit"


@freezegun.freeze_time("2024-01-15T12:00:00Z")
class TestPromotionalCreditStream(TestCase):
    """Tests for the promotional_credit stream."""

    @HttpMocker()
    def test_read_records(self, http_mocker: HttpMocker) -> None:
        """Basic read test for promotional_credit stream."""
        http_mocker.get(
            RequestBuilder.promotional_credits_endpoint().with_any_query_params().build(),
            promotional_credit_response(),
        )

        output = read_output(config_builder=config(), stream_name=_STREAM_NAME)
        assert len(output.records) == 1
        assert output.records[0].record.data["id"] == "pc_001"
    ...
    def test_incremental_emits_state(self, http_mocker: HttpMocker) -> None:
        ...
        # Verify state contains correct cursor value (promotional_credit uses created_at)
        ...
        assert latest_cursor_value == 1705312800  # From promotional_credit.json
    ...
    def test_incremental_sync_with_state_and_params(self, http_mocker: HttpMocker) -> None:
        ...
        Note: promotional_credit stream uses created_at cursor (not updated_at) and has NO include_deleted.
        """
        ...
        http_mocker.get(
            RequestBuilder.promotional_credits_endpoint()
        ...
        assert record["id"] == "pc_001"
        assert record["created_at"] >= previous_state_timestamp, "Record should be from after the state timestamp"
    ...
    def test_error_configuration_incompatible_ignored(self, http_mocker: HttpMocker) -> None:
        """Test configuration_incompatible error is ignored for promotional_credit stream as configured in manifest."""
        http_mocker.get(
            RequestBuilder.promotional_credits_endpoint().with_any_query_params().build(),
