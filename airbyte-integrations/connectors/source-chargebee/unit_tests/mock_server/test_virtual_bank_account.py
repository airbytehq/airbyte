...
from .response_builder import configuration_incompatible_response, virtual_bank_account_response
...
class TestVirtualBankAccountStream(TestCase):
    ...
    def test_incremental_emits_state(self, http_mocker: HttpMocker) -> None:
        ...
        assert latest_cursor_value == 1705312800  # From virtual_bank_account.json
    ...
    def test_error_configuration_incompatible_ignored(self, http_mocker: HttpMocker) -> None:
        """Test configuration_incompatible error is ignored for virtual_bank_account stream as configured in manifest."""
        http_mocker.get(
            RequestBuilder.virtual_bank_accounts_endpoint().with_any_query_params().build(),
