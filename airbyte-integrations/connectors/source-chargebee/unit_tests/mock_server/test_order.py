...
from .response_builder import configuration_incompatible_response, order_response
...
class TestOrderStream(TestCase):
    """Tests for the order stream."""

    @HttpMocker()
    def test_read_records(self, http_mocker: HttpMocker) -> None:
        """Basic read test for order stream."""
        http_mocker.get(
            RequestBuilder.orders_endpoint().with_any_query_params().build(),
            order_response(),
        )

        output = read_output(config_builder=config(), stream_name=_STREAM_NAME)
        assert len(output.records) == 1
        assert output.records[0].record.data["id"] == "order_001"
    ...
    def test_incremental_emits_state(self, http_mocker: HttpMocker) -> None:
        ...
        assert latest_cursor_value == 1705312800  # From order.json
    ...
    def test_incremental_sync_with_state_and_params(self, http_mocker: HttpMocker) -> None:
        ...
        http_mocker.get(
            RequestBuilder.orders_endpoint()
    ...
    def test_error_configuration_incompatible_ignored(self, http_mocker: HttpMocker) -> None:
        """Test configuration_incompatible error is ignored for order stream as configured in manifest."""
        http_mocker.get(
            RequestBuilder.orders_endpoint().with_any_query_params().build(),
