...
from .response_builder import configuration_incompatible_response, item_price_response
from .utils import config, read_output


_STREAM_NAME = "item_price"


@freezegun.freeze_time("2024-01-15T12:00:00Z")
class TestItemPriceStream(TestCase):
    """Tests for the item_price stream."""

    @HttpMocker()
    def test_read_records(self, http_mocker: HttpMocker) -> None:
        """Basic read test for item_price stream."""
        http_mocker.get(
            RequestBuilder.item_prices_endpoint().with_any_query_params().build(),
            item_price_response(),
        )

        output = read_output(config_builder=config(), stream_name=_STREAM_NAME)
        assert len(output.records) == 1
        assert output.records[0].record.data["id"] == "item_price_001"
    ...
    def test_incremental_sync_with_state_and_params(self, http_mocker: HttpMocker) -> None:
        ...
        http_mocker.get(
            RequestBuilder.item_prices_endpoint()
        ...
        assert record["id"] == "item_price_001"
    ...
    def test_error_configuration_incompatible_ignored(self, http_mocker: HttpMocker) -> None:
        """Test configuration_incompatible error is ignored for item_price stream as configured in manifest."""
        http_mocker.get(
            RequestBuilder.item_prices_endpoint().with_any_query_params().build(),
