...
This is the repository for the Chargebee source connector, written in Python. For information about
how to use this connector within Airbyte, see
[the documentation](https://docs.airbyte.com/integrations/sources/chargebee).
...
[documentation](https://docs.airbyte.com/integrations/sources/chargebee) to generate the necessary
credentials. Then create a file `secrets/config.json` conforming to the `source_chargebee/spec.yaml`
...
```
poetry run source-chargebee spec
...
poetry run source-chargebee read --config secrets/config.json --catalog integration_tests/configured_catalog.json
```
...
An image will be available on your host with the tag `airbyte/source-chargebee:dev`.
