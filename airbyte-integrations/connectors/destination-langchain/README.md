...
```
Once the command is done, you will find your connector image in your local docker registry: `airbyte/destination-langchain:dev`.
...
```Dockerfile
FROM airbyte/destination-langchain:latest
...
docker run airbyte/destination-langchain:dev spec
```
#### Run
