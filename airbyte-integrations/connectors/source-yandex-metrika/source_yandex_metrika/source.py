...
from source_yandex_metrika.aggregated_data_streams.supported_fields import (
    _RAW_GROUP_FIELDS,
    _RAW_METRICS_FIELDS,
)
...
class SourceYandexMetrika(AbstractSource):
    ...
    def _read_full_refresh(
        ...
        if stream_instance.__class__.__name__ == "YandexMetrikaRawDataStream":
            ...
            logger.debug(f"Processing raw stream slices for {configured_stream.stream.name}", extra={"stream_slices": raw_slices})
            ...
            with create_timer(self.name) as timer:
                for raw_slice in raw_slices:
                    ...
                    stream_instance_kwargs = getattr(self, self.raw_data_stream_to_field_map[stream_instance.name]["kwargs_field_name"])
    ...
    def check_connection(self, logger, config) -> Tuple[bool, any]:
        ...
        if raw_hits_config.get("is_enabled") == "enabled":
            ...
            for r_f in HITS_AVAILABLE_FIELDS.get_required_fields_names_list():
                if r_f not in raw_hits_config.get("fields", []):
                    return False, 'Сырые отчёты - источник "Просмотры" (hits) должен содержать поля "ym:pv:watchID" и "ym:pv:dateTime"'
        ...
        if raw_visits_config.get("is_enabled") == "enabled":
            ...
            for r_f in VISITS_AVAILABLE_FIELDS.get_required_fields_names_list():
                if r_f not in raw_visits_config.get("fields", []):
                    return False, 'Сырые отчёты - источник "Просмотры" (hits) должен содержать поля "ym:s:visitID" и "ym:s:dateTime"'
        ...
        for stream_n, stream in enumerate(
            filter(lambda stream: stream.__class__.__name__ == "AggregateDataYandexMetrikaReport", self.streams(config))
        ):
        ...
        for stream_n, stream in enumerate(
            filter(lambda stream: stream.__class__.__name__ == "YandexMetrikaRawDataStream", self.streams(config))
        ):
    ...
    def streams(self, config: Mapping[str, Any], init_for_test: bool = False) -> List[Stream]:
        ...
        agg_data_streams = [
            AggregateDataYandexMetrikaReport(authenticator=auth, global_config=config, report_config=stream_config)
