...
from .source import YandexMetrikaStream
...
class PreprocessedSlicePartProcessorThread(Thread, LogMessagesPoolConsumer):
    ...
    def process_log_request(self):
        try:
            filename = next(self.stream_instance.read_records(
            ...
            logger.info(
                f'Remove file {filename} for slice {self.stream_slice}')
            os.remove(filename)
