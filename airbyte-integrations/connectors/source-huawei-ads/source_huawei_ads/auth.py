...
class CredentialsCraftAuthenticator(TokenAuthenticator):
    ...
    @property
    def _url(self) -> str:
        return f"{self._cc_host}/api/v1/token/huawei_ads/{self._cc_token_id}/"

    @property
    def _service_access_token(self) -> Mapping[str, Any]:
        resp: Dict = requests.get(self._url, headers={"Authorization": f"Bearer {self._cc_token}"}).json()
    ...
    def check_connection(self):
        ...
        try:
            ...
            return False, f"CredentialsCraft token error: {e}; {token_resp}; Body: {token_resp.text}"
        return True, None
