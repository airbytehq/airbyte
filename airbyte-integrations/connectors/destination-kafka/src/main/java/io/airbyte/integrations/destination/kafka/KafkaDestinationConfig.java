...
public class KafkaDestinationConfig {
  ...
  private KafkaDestinationConfig(
      final String topicPattern, final boolean sync, final JsonNode config) {
  ...
  private KafkaProducer<String, JsonNode> buildKafkaProducer(final JsonNode config) {
    final Map<String, Object> props = ImmutableMap.<String, Object>builder()
        .put(
            ...
            config.get("bootstrap_servers").asText())
        ...
        .put(
            ...
            config.get("enable_idempotence").asBoolean())
        .put(
            ProducerConfig.COMPRESSION_TYPE_CONFIG,
            config.get("compression_type").asText())
    ...
    final Map<String, Object> filteredProps = props.entrySet().stream()
        .filter(
            entry -> entry.getValue() != null && !entry.getValue().toString().isBlank())
  ...
  private Map<String, Object> propertiesByProtocol(final JsonNode config) {
    ...
    switch (protocol) {
      ...
      case SASL_SSL, SASL_PLAINTEXT -> {
        builder.put(
            SaslConfigs.SASL_JAAS_CONFIG, protocolConfig.get("sasl_jaas_config").asText());
        builder.put(
            SaslConfigs.SASL_MECHANISM, protocolConfig.get("sasl_mechanism").asText());
      }
      default -> throw new RuntimeException(
          "Unexpected Kafka protocol: " + Jsons.serialize(protocol));
    }
