...
class StreamConfigProvider(private val config: BigqueryConfiguration) {
    /**
     * Get the stream-level configuration for a given stream descriptor. Returns null if no
     * stream-specific config exists.
     */
    fun getStreamConfig(descriptor: DestinationStream.Descriptor): StreamLevelConfig? {
        ...
        if (descriptor.namespace != null) {
            ...
            config.streamConfigMap[fullKey]?.let {
                return it
            }
        }
    ...
    /**
     * Get the effective clustering field for a stream. Priority: stream config > default config >
     * null (use PK-based)
     */
    fun getClusteringField(descriptor: DestinationStream.Descriptor): String? {
        return getStreamConfig(descriptor)?.clusteringField ?: config.defaultClusteringField
    }
    ...
    /**
     * Get the effective partitioning granularity for a stream. Priority: stream config > default
     * config > DAY
     */
    fun getPartitioningGranularity(
        ...
        return getStreamConfig(descriptor)?.partitioningGranularity
            ?: config.defaultPartitioningGranularity ?: PartitioningGranularity.DAY
    }

    /**
     * Get the effective table suffix for a stream. Priority: stream config > default config > ""
     * (no suffix)
     */
    fun getTableSuffix(descriptor: DestinationStream.Descriptor): String {
        return getStreamConfig(descriptor)?.tableSuffix ?: config.defaultTableSuffix ?: ""
    }

    /**
     * Get the effective dataset for a stream. Priority: stream config > stream namespace > default
     * dataset
     */
    fun getDataset(descriptor: DestinationStream.Descriptor): String {
        return getStreamConfig(descriptor)?.dataset ?: descriptor.namespace ?: config.datasetId
    }
}
