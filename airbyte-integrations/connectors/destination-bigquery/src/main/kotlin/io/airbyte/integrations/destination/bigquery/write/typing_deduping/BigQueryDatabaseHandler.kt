...
class BigQueryDatabaseHandler(private val bq: BigQuery, private val datasetLocation: String) :
    ...
    override fun execute(sql: Sql) {
        ...
        try {
            ...
            if (statistics.numChildJobs != null) {
                // There isn't (afaict) anything resembling job.getChildJobs(), so we have to ask bq
                // for them
            ...
            throw asInterruptedBigQueryOperation(e)
    ...
    private fun asInterruptedBigQueryOperation(e: Exception): TransientErrorException {
        Thread.currentThread().interrupt()
        return TransientErrorException(INTERRUPTED_FINAL_TABLE_UPDATE_MESSAGE, e)
    }
    ...
    companion object {
        ...
        const val INTERRUPTED_FINAL_TABLE_UPDATE_MESSAGE =
            "BigQuery final-table update was interrupted."
    }
}
