...
class BigQuerySqlGenerator(private val projectId: String?, private val datasetLocation: String?) :
    ...
    override fun createTable(
        ...
        // TODO we should switch this to using stream.schema.withAirbyteMeta(flatten = true)
        //   instead of hardcoding the airbyte column names
        return Sql.of(
            """
    ...
    override fun overwriteTable(sourceTableName: TableName, targetTableName: TableName): Sql {
        val finalTableId = targetTableName.toPrettyString(QUOTE)
        val tempFinalTableId = sourceTableName.toPrettyString(QUOTE)
        return Sql.separately(
            "DROP TABLE IF EXISTS `$projectId`.$finalTableId;",
            "ALTER TABLE `$projectId`.$tempFinalTableId RENAME TO `${targetTableName.name}`;"
        )
    }
    ...
    override fun copyTable(
        ...
        return Sql.of(
            ...
            SELECT * FROM `${sourceTableName.namespace}`.`${sourceTableName.name}`
            """.trimIndent()
        )
    }
