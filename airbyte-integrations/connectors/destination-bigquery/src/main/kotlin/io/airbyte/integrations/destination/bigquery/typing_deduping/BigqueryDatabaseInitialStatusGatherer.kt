...
// TODO move the AlterTableReport stuff to DirectLoadNativeOps
class BigqueryDatabaseInitialStatusGatherer(private val bq: BigQuery) :
    ...
    override suspend fun gatherInitialStatus(
        ...
        return streams.mapValues { (_, names) ->
            ...
            DirectLoadInitialStatus(
                realTable = getTableStatus(realTableName),
                tempTable = getTableStatus(tempTableName),
            )
        }
    }

    private fun getTableStatus(tableName: TableName): DirectLoadTableStatus? {
        return bq.getTable(tableName.namespace, tableName.name)?.let {
            DirectLoadTableStatus(isEmpty = it.numRows == BigInteger.ZERO)
        }
    }
