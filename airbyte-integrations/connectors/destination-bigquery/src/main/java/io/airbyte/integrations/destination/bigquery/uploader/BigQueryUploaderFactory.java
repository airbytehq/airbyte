...
public class BigQueryUploaderFactory {
    ...
    public static AbstractBigQueryUploader<?> getUploader(JsonNode config, ConfiguredAirbyteStream configStream, String targetTableName, String tmpTableName, BigQuery bigQuery, Schema schema) throws IOException {
        ...
        final boolean isGcsUploadingMode = UploadingMethod.GCS.equals(BigQueryUtils.getLoadingMethod(config));
        final TableId targetTable = TableId.of(schemaName, targetTableName);
        final TableId tmpTable = TableId.of(schemaName, tmpTableName);
        final JobInfo.WriteDisposition syncMode = BigQueryUtils.getWriteDisposition(configStream.getDestinationSyncMode());

        return (isGcsUploadingMode ?
                getGcsBigQueryUploader(config, configStream, targetTable, tmpTable, bigQuery, schema, syncMode) :
                getBigQueryDirectUploader(config, targetTable, tmpTable, bigQuery, schema, syncMode, datasetLocation)
        );
    }

    private static AbstractGscBigQueryUploader<?> getGcsBigQueryUploader(JsonNode config, ConfiguredAirbyteStream configStream, TableId targetTable, TableId tmpTable, BigQuery bigQuery, Schema schema, JobInfo.WriteDisposition syncMode) throws IOException {
        ...
        return new GcsAvroBigQueryUploader(targetTable, tmpTable,
                gcsCsvWriter, syncMode, schema, gcsDestinationConfig, bigQuery, BigQueryUtils.isKeepFilesInGcs(config));
    }
...
//    return new GcsCsvWriter(gcsDestinationConfig, s3Client, configuredStream, uploadTimestamp);
    ...
    private static BigQueryDirectUploader getBigQueryDirectUploader(JsonNode config, TableId targetTable, TableId tmpTable, BigQuery bigQuery, Schema schema, JobInfo.WriteDisposition syncMode, String datasetLocation) {
        ...
        final Integer bigQueryClientChunkSizeFomConfig = BigQueryUtils.getBigQueryClientChunkSize(config);
        if (bigQueryClientChunkSizeFomConfig != null) {
            writer.setChunkSize(bigQueryClientChunkSizeFomConfig);
        }

        return new BigQueryDirectUploader(targetTable, tmpTable,
                new BigQueryTableWriter(writer), syncMode, schema, bigQuery);
    }
