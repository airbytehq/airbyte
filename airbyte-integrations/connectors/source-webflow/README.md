...
This is the repository for the Webflow source connector, written in Python. For information about
how to use this connector within Airbyte, see
[Webflow source documentation](https://docs.airbyte.io/integrations/sources/webflow).

## Related tutorial

A detailed tutorial has been written about this implementation. See:
...
[documentation](https://docs.airbyte.io/integrations/sources/webflow) to generate the necessary
credentials. Then create a file `secrets/config.json` conforming to the `source_webflow/spec.yaml`
file. Note that any directory named `secrets` is git-ignored across the entire Airbyte repo, so
...
For more information about creating Webflow credentials, see
[the documentation](https://docs.airbyte.io/integrations/sources/webflow).

**If you are an Airbyte core member**, copy the credentials in Lastpass under the secret name
`source webflow test creds` and place them into `secrets/config.json`.
