...
public class MsSQLTestDatabase extends TestDatabase<MSSQLServerContainer<?>, MsSQLTestDatabase, MsSQLTestDatabase.MsSQLConfigBuilder> {
  ...
  public MsSQLTestDatabase withCdcForTable(String schemaName, String tableName, String roleName, String instanceName) {
    ...
    throw new RuntimeException("failed to enable CDC for table %s.%s within %d seconds".formatted(schemaName, tableName, MAX_RETRIES));
  }
  ...
  public MsSQLTestDatabase withCdcDisabledForTable(String schemaName, String tableName, String instanceName) {
    ...
    if (!CDC_INSTANCE_NAMES.remove(instanceName)) {
      throw new RuntimeException("CDC was disabled for instance " + instanceName);
    }
    synchronized (this) {
  ...
  protected static final String MAX_LSN_QUERY = "SELECT sys.fn_cdc_get_max_lsn() AS max_lsn;";

  public MsSQLTestDatabase waitForMaxLsn() throws SQLException {
    int tryCount = 0;
    do {
      LOGGER.info(formatLogLine("fetching max LSN, try {}/{}"), tryCount, MAX_RETRIES);
      Object maxLsn = query(ctx -> ctx.fetch(MAX_LSN_QUERY)).get(0).getValue(0);
      if (maxLsn != null) {
        LOGGER.info(formatLogLine("found non-null max LSN. Exiting"));
      }
    } while (tryCount++ < MAX_RETRIES);
    return self();
  }
