...
class MsSqlServerCdcLsnValidationIntegrationTest {
    ...
    /**
     * Validates getAccessibleCaptureInstances(): as the admin (db_owner) user, every
     * CDC-enabled capture instance should be returned.
     */
    @Test
     ...
     * db_datareader but is not a member of the cdc_restricted gating role, so dbo_restricted
     * is filtered out while the two ungated instances remain.
     */
    @Test
    ...
    companion object {
        ...
        fun startAndProvisionTestContainer() {
            // exclusive (not shared): this test mutates security state as it contains logins, roles and grants.
            dbContainer =
            ...
            val adminConfig = ops.adminConfig(dbContainer)
                ...
                ops.createLimitedUser(adminConfig)   // creates the limited user: login + db_datareader, no gating role
            }
...
object MsSqlServerCdcLsnValidationTestOperations {
    ...
    val captureInstances =
        listOf(
            ...
            // Gated by cdc_restricted (users & orders have no gating role). The limited user is never added to this role,
            // so getAccessibleCaptureInstances should hide this instance from it.
            CaptureInstanceFixture("dbo", "restricted", "dbo_restricted", gatingRole = "cdc_restricted"),
        )

    fun globalConfigSpec(container: MSSQLServerContainer<*>): MsSqlServerSourceConfigurationSpecification =
        MsSqlServerContainerFactory.config(container).also { it.setIncrementalValue(Cdc()) }

    fun limitedGlobalConfigSpec(container: MSSQLServerContainer<*>): MsSqlServerSourceConfigurationSpecification =
    ...
    fun createStreams(config: MsSqlServerSourceConfiguration) {
        JdbcConnectionFactory(config).get().use { connection: Connection ->
            ...
            try {
                ...
                log.warn { "Failed to enable CDC on database (may already be enabled): ${e.message}" }
            }
...
/**
 * One CDC-enabled table in the test.
 *  schema / table   -> source table (dbo.users).
 *  captureInstance  -> CDC capture-instance name (dbo_users).
 *  gatingRole       -> the CDC role that gates access to this capture instance.
 */
