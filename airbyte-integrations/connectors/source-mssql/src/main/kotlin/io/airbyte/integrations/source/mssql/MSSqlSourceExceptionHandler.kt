    ...
    override fun initializeErrorDictionary() {
        ...
        add(
            ConnectorErrorProfile(
                ...
                externalMessage = "Encountered an error while reading from the database, will retry",
        ...
        add(
            // Error 1205 https://learn.microsoft.com/en-us/sql/relational-databases/errors-events/mssqlserver-1205-database-engine-error
            ConnectorErrorProfile(
                errorClass = "MSSQL Exception",
                regexMatchingPattern =
                    "(?i).*was deadlocked on lock resources with another process and has been chosen as the deadlock victim. Rerun the transaction.*",
                failureType = FailureType.TRANSIENT,
                externalMessage = "Transaction conflicted with another process and was terminated, will retry.",
                sampleInternalMessage =
        ...
        // We classify it as TRANSIENT since it may be resolved through automatic retries but can also require
        // investigation and manual intervention.
        add(
            ConnectorErrorProfile(
                errorClass = "Connect Exception",
                regexMatchingPattern =
                    "(?i).*exception occurred in the change event producer.*",
                failureType = FailureType.TRANSIENT,
                externalMessage = "The sync encountered an unexpected error in the change event producer and has stopped. Please check the logs for details and troubleshoot accordingly.",
                sampleInternalMessage =
                ...
                referenceLinks = listOf("https://docs.oracle.com/javase/9/docs/api/java/lang/RuntimeException.html")
            ),
        )
    }
}
