...
/**
 * Base for all MSSQL partitions, implements the CDK's [JdbcPartition] interface.
 * Provides every partition with two TABLESAMPLE-free defaults:
 *     1. [nonResumableQuery] (a plain `SELECT <fields> FROM <table>`)
 *     2. [samplingQuery] (a plain `SELECT TOP <n>`)
 * Cursor handling, bounds, splitting, and TABLESAMPLE-based sampling are handled by [MsSqlServerJdbcResumablePartition]
 * and [MsSqlServerJdbcCursorPartition].
  */
...
/**
 * For views (no TABLESAMPLE support) or streams without an ordered column, use non-resumable cursor incremental.
 * Extends:
 *  - [MsSqlServerJdbcPartition] directly and implements [JdbcCursorPartition].
 *  - Hence, unlike [MsSqlServerJdbcCursorIncrementalPartition] it has no bounds/where, no sampling and no splitting.
 */
 ...
 *
 * On top of [MsSqlServerJdbcPartition], it adds everything needed to read a table range
 * resumably and concurrently:
 ...
 * 3. [resumableQuery] which reads the range in [checkpointColumns] order with a limit so progress can be
 ...
 *
 * It inherits the base class's functionalities and adds the cursor: a query to read the max
 * cursor value and bound the sync, [cursorCutoffTime] to exclude today's data, and an overridden
...
class MsSqlServerJdbcCursorIncrementalPartition(
    ...
    // We leave it open-ended because the cursor is truncated to 6 digits: a `<= MAX(cursor)` ceiling
