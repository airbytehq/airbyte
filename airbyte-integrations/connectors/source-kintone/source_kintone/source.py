...
class SourceKintone(Source):
    ...
    def read(
        self, logger: AirbyteLogger, config: json, catalog: ConfiguredAirbyteCatalog, state: List[Dict[str, any]]
        ...
        for app_id in config.get("app_ids"):
            ...
            app_name = client.rename_app_name(app_id, app_name_original)
            apps[app_name] = app

        cursor_field, cursor_value = None, None
        for configured_stream in catalog.streams:
            stream_name = configured_stream.stream.name
            stream_state = self._extract_stream_state(state, stream_name)
            ...
            if configured_stream.sync_mode == SyncMode.full_refresh:
                ...
                cursor_value = stream_state.get(cursor_field, None)
    ...
    def _extract_stream_state(self, state: List[Dict[str, any]], stream_name: str) -> Dict[str, any]:
        for state_message in state:
            if state_message.stream.stream_descriptor.name == stream_name:
                return state_message.stream.stream_state.dict()
        return {}
