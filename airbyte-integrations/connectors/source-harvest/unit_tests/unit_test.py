...
def _a_response(stream_name: str, data_path: str) -> HttpResponseBuilder:
    return create_response_builder(
        find_template(stream_name, __file__), records_path=FieldPath(data_path), pagination_strategy=HarvestPaginationStrategy()
    )
...
class UnitTest(TestCase):
    ...
    def test_next_page_token(self, http_mocker: HttpMocker):
        ...
        http_mocker.get(
            ...
            _a_response(stream_name="invoices", data_path="invoices")
            .with_record(_a_record(stream_name=stream_name, data_path=stream_name, primary_key=stream_pk))
            .with_pagination()
            .build(),
        )

        http_mocker.get(
            HarvestRequestBuilder.invoices_endpoint("account_id")
            .with_page(2)
            .with_per_page(50)
    ...
    def test_child_stream_partitions(self, http_mocker: HttpMocker):
        ...
        http_mocker.get(
            ...
            [
                _a_response(stream_name=stream_name, data_path=stream_name)
                .with_record(
                    _a_record(stream_name=stream_name, data_path=stream_name, primary_key=stream_pk).with_field(FieldPath(stream_pk), 1)
                )
                .with_record(
                    _a_record(stream_name=stream_name, data_path=stream_name, primary_key=stream_pk).with_field(FieldPath(stream_pk), 2)
                )
                .build()
            ],
        )
        ...
        output_1 = read(
            SourceHarvest(), config=self._config, catalog=CatalogBuilder().with_stream("invoices", SyncMode.full_refresh).build()
        )
    ...
    def test_report_based_stream(self, http_mocker: HttpMocker):
        ...
        output = read(
            SourceHarvest(), config=self._config, catalog=CatalogBuilder().with_stream(stream_name, SyncMode.full_refresh).build()
        )
    ...
    def test_report_based_stream_slices(self, http_mocker: HttpMocker):
        ...
        while replication_start_datetime < datetime.now():
            ...
            end_datetime = (
                replication_start_datetime + timedelta(days=364)
                if replication_start_datetime + timedelta(days=364) < datetime.now()
                else datetime.now()
            )

            http_mocker.get(
                HarvestRequestBuilder.expenses_clients_endpoint("account_id")
                .with_per_page(50)
                .with_from(replication_start_datetime)
                .with_to(end_datetime)
                .build(),
                _a_response(stream_name=stream_name, data_path=data_path).with_record(_a_record(stream_name, data_path, stream_pk)).build(),
            )
