...
class TestInvoicesStream(TestCase):
    """Tests for the Harvest 'invoices' stream."""

    @HttpMocker()
    def test_full_refresh(self, http_mocker: HttpMocker):
        """Test that connector correctly fetches invoices."""
    ...
    def test_incremental_sync_with_state(self, http_mocker: HttpMocker) -> None:
        ...
        http_mocker.get(
            ...
            HttpResponse(
                body=json.dumps(
                    {
                        "invoices": [{"id": 9001, "created_at": "2024-01-02T10:00:00Z", "updated_at": "2024-01-02T10:00:00Z"}],
                        "per_page": 50,
                        "total_pages": 1,
                        "page": 1,
                        "links": {},
