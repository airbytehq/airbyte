...
class TestUninvoicedStream(TestCase):
    ...
    def test_full_refresh(self, http_mocker: HttpMocker) -> None:
        ...
        with open(get_resource_path("http/response/uninvoiced.json")) as f:
            response_data = json.load(f)
        http_mocker.get(
            HarvestRequestBuilder.uninvoiced_endpoint(_ACCOUNT_ID, _API_TOKEN)
            .with_per_page(50)
    ...
    def test_unauthorized_error_handling(self, http_mocker: HttpMocker) -> None:
        ...
        http_mocker.get(
            HarvestRequestBuilder.uninvoiced_endpoint(_ACCOUNT_ID, "invalid_token")
            .with_per_page(50)
            .with_from_date("20240101")
            .with_to_date("20241230")
            .build(),
