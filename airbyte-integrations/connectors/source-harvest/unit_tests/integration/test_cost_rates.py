...
class TestCostRatesStream(TestCase):
    ...
    def test_full_refresh(self, http_mocker: HttpMocker) -> None:
        """
        Test full_refresh sync for cost_rates stream.

        This is a substream of users, so we need to:
        1. Mock the parent users stream response
        2. Mock the cost_rates response for that user
        """
        ...
        # ASSERT: Should retrieve all cost_rates records for the user
        ...
        # ASSERT: Should have expected cost rate data structure
        cost_rate = output.records[0].record.data
        assert cost_rate["amount"] == 75.0
        assert "start_date" in cost_rate
