...
class TestExpenseCategoriesStream(TestCase):
    @HttpMocker()
    def test_full_refresh(self, http_mocker: HttpMocker) -> None:
        """
        Test full_refresh sync for expense_categories stream.
        """
        config = ConfigBuilder().with_account_id(_ACCOUNT_ID).with_api_token(_API_TOKEN).build()

        with open(get_resource_path("http/response/expense_categories.json")) as f:
            response_data = json.load(f)

        http_mocker.get(
            HarvestRequestBuilder.expense_categories_endpoint(_ACCOUNT_ID, _API_TOKEN)
            .with_per_page(50)
        ...
        # ASSERT: Should retrieve all expense_categories records
    ...
    def test_empty_results(self, http_mocker: HttpMocker) -> None:
        """
        Test handling of empty results when no expense categories exist.
        """
        config = ConfigBuilder().with_account_id(_ACCOUNT_ID).with_api_token(_API_TOKEN).build()
        ...
        http_mocker.get(
            ...
            .build(),
            HttpResponse(
                body=json.dumps({"expense_categories": [], "per_page": 50, "total_pages": 0, "total_entries": 0, "page": 1, "links": {}}),
                status_code=200,
            ),
        )
    ...
    def test_incremental_sync_with_state(self, http_mocker: HttpMocker) -> None:
        ...
        http_mocker.get(
            ...
            HttpResponse(
                body=json.dumps(
                    {
                        "expense_categories": [{"id": 9001, "created_at": "2024-01-02T10:00:00Z", "updated_at": "2024-01-02T10:00:00Z"}],
                        "per_page": 50,
                        "total_pages": 1,
                        "page": 1,
                        "links": {},
    ...
    def test_unauthorized_error_handling(self, http_mocker: HttpMocker) -> None:
        ...
        http_mocker.get(
            HarvestRequestBuilder.expense_categories_endpoint(_ACCOUNT_ID, "invalid_token")
            .with_per_page(50)
            .with_updated_since("2021-01-01T00:00:00Z")
            .build(),
