...
_STREAM_NAME = "estimates"
_ACCOUNT_ID = "123456"
_API_TOKEN = "test_token_abc123"


@freezegun.freeze_time(_NOW.isoformat())
class TestEstimatesStream(TestCase):
    """Tests for the Harvest 'estimates' stream."""

    @HttpMocker()
    def test_full_refresh(self, http_mocker: HttpMocker):
        """Test that connector correctly fetches estimates."""
        config = ConfigBuilder().with_account_id(_ACCOUNT_ID).with_api_token(_API_TOKEN).build()

        with open(get_resource_path("http/response/estimates.json")) as f:
    ...
    def test_empty_results(self, http_mocker: HttpMocker) -> None:
        ...
        http_mocker.get(
            ...
            .build(),
            HttpResponse(
                body=json.dumps({"estimates": [], "per_page": 50, "total_pages": 0, "total_entries": 0, "page": 1, "links": {}}),
                status_code=200,
            ),
        )
