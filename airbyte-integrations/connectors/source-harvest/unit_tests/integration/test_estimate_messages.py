...
class TestEstimateMessagesStream(TestCase):
    ...
    def test_incremental_sync_with_state(self, http_mocker: HttpMocker) -> None:
        ...
        # Mock parent estimates stream
        parent_estimate = {"id": 1, "client_id": 1, "number": "EST-001", "amount": 5000.0, "state": "draft", "created_at": "2024-01-01T00:00:00Z", "updated_at": "2024-01-01T00:00:00Z"}
        http_mocker.get(
            HarvestRequestBuilder.estimates_endpoint(_ACCOUNT_ID, _API_TOKEN)
            .with_per_page(50)
            ...
            .build(),
            HttpResponse(
                body=json.dumps({"estimates": [parent_estimate], "per_page": 50, "total_pages": 1, "total_entries": 1, "page": 1, "links": {}}),
                status_code=200
            )
        )
        ...
        http_mocker.get(
            ...
            HttpResponse(
                body=json.dumps({"estimate_messages": [{"id": 9001, "created_at": "2024-01-02T10:00:00Z", "updated_at": "2024-01-02T10:00:00Z"}], "per_page": 50, "total_pages": 1, "page": 1, "links": {}}),
                status_code=200
            ),
        )
