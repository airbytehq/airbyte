   ...
   private Map<String, Object> propertiesByProtocol(final JsonNode config) {
     ...
     switch (protocol) {
       ...
       case SSL -> {
         ...
         builder.put(SslConfigs.SSL_KEYSTORE_LOCATION_CONFIG, protocolConfig.get("ssl_keystore_location").asText());
         builder.put(SslConfigs.SSL_TRUSTSTORE_TYPE_CONFIG, protocolConfig.get("ssl_truststore_type").asText());
         builder.put(SslConfigs.SSL_TRUSTSTORE_LOCATION_CONFIG, protocolConfig.get("ssl_truststore_location").asText());
       }
