...
public class ClickhouseTestDataComparator extends AdvancedTestDataComparator {
  ...
  private final ZonedDateTime minSupportedDateTime =
      ZonedDateTime.parse("1925-01-01T00:00:00.000Z");
  private final ZonedDateTime maxSupportedDateTime =
      ZonedDateTime.parse("2283-11-10T20:23:45.000Z");
  ...
  protected boolean compareDateValues(
    ...
    if (expectedDate.isBefore(minSupportedDate) || expectedDate.isAfter(maxSupportedDate)) {
      // inserting any dates that are out of supported range causes registers overflow in
      // clickhouseDB,
  ...
  protected ZonedDateTime parseDestinationDateWithTz(final String destinationValue) {
    return ZonedDateTime.parse(
            destinationValue, DateTimeFormatter.ofPattern(CLICKHOUSE_DATETIME_WITH_TZ_FORMAT))
        .withZoneSameInstant(ZoneOffset.UTC);
  }
  ...
  protected boolean compareDateTimeValues(
    ...
    try {
      actualDateTime = LocalDateTime.parse(
          destinationValue, DateTimeFormatter.ofPattern(CLICKHOUSE_DATETIME_WITH_TZ_FORMAT));
