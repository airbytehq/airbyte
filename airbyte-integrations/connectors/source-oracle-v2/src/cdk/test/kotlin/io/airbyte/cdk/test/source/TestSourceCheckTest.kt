...
import io.airbyte.cdk.operation.SourceCheckOperation
...
class TestSourceCheckTest {

    @Inject lateinit var checkOperation: SourceCheckOperation<TestSourceConfigurationJsonObject>
    @Inject lateinit var outputConsumer: BufferingOutputConsumer

    @Test
    fun testConfigMissingDatabase() {
        assertFailed("required property 'database' not found".toRegex())
    }
    ...
    fun testBadTables() {
        assertFailed("Unable to query any of the discovered table".toRegex())
    }
