...
class StateManager(
    ...
    private sealed class BaseStateManager<K : Key>(
        initialState: State<K>,
        private val isCheckpointUnique: Boolean = true
    ) {

        val key: K = initialState.key

        private var current: State<K> = initialState
        private var pending: State<K> = initialState
        private var pendingNumRecords: Long = 0L

        fun state(): State<K> = synchronized(this) { current }

        fun set(state: State<K>, numRecords: Long) {
            synchronized(this) {
                pending = state
        ...
        protected fun swap(): Pair<SerializableState<K>, Long>? {
            synchronized(this) {
                if (isCheckpointUnique && pendingNumRecords == 0L && pending == current) {
                    return null
                }
                return when (val pendingState: State<K> = pending) {
                    is SerializableState<K> ->
