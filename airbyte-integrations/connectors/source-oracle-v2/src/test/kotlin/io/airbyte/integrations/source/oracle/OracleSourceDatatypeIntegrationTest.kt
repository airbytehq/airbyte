...
class OracleSourceDatatypeIntegrationTest {
    ...
    private fun discover(streamName: String) {
        ...
        log.info { "test case $streamName: discovered stream ${Jsons.jsonNode(actualStream)}" }
    ...
    private fun records(streamName: String) {
        ...
        fun sortedRecordData(data: List<JsonNode>): JsonNode =
            Jsons.arrayNode().apply { addAll(data.sortedBy { it.toString() }) }
    ...
    companion object {
        ...
        val dateValues =
            mapOf(
                "'2024-03-01'" to """"2024-03-01"""",
                "'2024-02-29'" to """"2024-02-29"""",
                "'2024-02-28'" to """"2024-02-28"""",
            )
        val tsValues =
            mapOf(
        ...
        val tsTzValues =
            mapOf(
                "TIMESTAMP '2024-03-01 06:07:08.9' AT TIME ZONE 'CET'" to
                    """"2024-03-01T07:07:08.900000+01:00"""",
            )
        val tsLocalTzValues =
            mapOf(
                "TIMESTAMP '2024-03-01 06:07:08.9'" to """"2024-03-01T06:07:08.900000"""",
            ...
            get() {
                val recordData: List<JsonNode> =
                    sqlToAirbyte.values.map {
                        Jsons.deserialize("""{"${columnName.uppercase()}":${it}}""")
                    }
                if (noVarray) {
                ...
                val varrayRecordData: List<JsonNode> =
                    sqlToAirbyte.values.map {
                        Jsons.deserialize("""{"${varrayColumnName.uppercase()}":[${it},${it}]}""")
                    }
                return mapOf(
