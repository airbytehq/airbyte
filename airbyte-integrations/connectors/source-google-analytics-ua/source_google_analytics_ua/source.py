...
class GoogleAnalyticsUATypesList(HttpStream):
    """
...
class GoogleAnalyticsUAStream(HttpStream, ABC):
    primary_key = None
    http_method = "POST"
    ...
    def __init__(self, config: MutableMapping):
        ...
        self._config = config
        self.dimensions_ref, self.metrics_ref = GoogleAnalyticsUATypesList().read_records(sync_mode=None)
    ...
    def get_json_schema(self) -> Mapping[str, Any]:
        """
        Override get_json_schema CDK method to retrieve the schema information for GoogleAnalyticsUA Object dynamically.
        """
...
class GoogleAnalyticsUAIncrementalObjectsBase(GoogleAnalyticsUAStream):
...
class TestStreamConnection(GoogleAnalyticsUAStream):
    """
...
class SourceGoogleAnalyticsUA(AbstractSource):
    ...
    def streams(self, config: MutableMapping[str, Any]) -> List[Stream]:
        streams: List[GoogleAnalyticsUAStream] = []

        authenticator = self.get_authenticator(config)

        config["authenticator"] = authenticator

        reports = json.loads(pkgutil.get_data("source_google_analytics_ua", "defaults/default_reports.json"))
        ...
        for stream in config["ga_streams"]:
            ...
            stream_bases = (GoogleAnalyticsUAStream,)

            if "ga:date" in stream["dimensions"]:
                stream_bases = (GoogleAnalyticsUAIncrementalObjectsBase,)
