...
from source_google_analytics_ua.source import (
    DATA_IS_NOT_GOLDEN_MSG,
    RESULT_IS_SAMPLED_MSG,
    GoogleAnalyticsUAIncrementalObjectsBase,
    GoogleAnalyticsUAStream,
    GoogleAnalyticsUATypesList,
    SourceGoogleAnalyticsUA,
)
...
def test_metrics_dimensions_type_list(mock_metrics_dimensions_type_list_link):
    test_metrics, test_dimensions = GoogleAnalyticsUATypesList().read_records(sync_mode=None)
...
def test_lookup_metrics_dimensions_data_type(test_config, metrics_dimensions_mapping, mock_metrics_dimensions_type_list_link):
    ...
    g = GoogleAnalyticsUAStream(config=test_config)
...
@patch("source_google_analytics_ua.source.jwt")
...
def test_unknown_metrics_or_dimensions_error_validation(
    ...
    records = GoogleAnalyticsUAStream(test_config).read_records(sync_mode=None)
    assert list(records) == []
...
def test_stream_slice_limits(test_config, mock_metrics_dimensions_type_list_link):
    ...
    g = GoogleAnalyticsUAIncrementalObjectsBase(config=test_config)
...
def test_next_page_token(test_config, configured_response, expected_token):
    ...
    token = GoogleAnalyticsUAStream(test_config).next_page_token(response)
