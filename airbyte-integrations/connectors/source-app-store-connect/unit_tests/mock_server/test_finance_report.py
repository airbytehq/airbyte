...
_STREAM_NAME = "finance_report"


class TestFinanceReportStream(TestCase):
    @HttpMocker()
    def test_read_records_and_emit_monthly_state(self, http_mocker: HttpMocker) -> None:
        ...
        assert output.records[0].record.data["report_date"] == f"{report_period(_STREAM_NAME)}-01"
