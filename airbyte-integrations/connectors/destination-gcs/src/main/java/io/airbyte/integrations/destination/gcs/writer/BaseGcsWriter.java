...
public abstract class BaseGcsWriter implements S3Writer {
  ...
  /**
   *
   * <li>1. Create bucket if necessary.
   * <li>2. Under OVERWRITE mode, delete all objects with the output prefix.
   */
  @Override
   ...
   * method does not work for GCS. So we use {@link AmazonS3#headBucket} instead, which will throw
   * an exception if the bucket does not exist, or there is no permission to access it.
   */
