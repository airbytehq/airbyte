...
class BoxFileTextStream(Stream, ABC):
    """
    ...
    `class BoxFileTextStream(HttpStream, ABC)` which is the current class
    `class Customers(BoxFileTextStream)` contains behavior to pull data for customers using v1/customers
    `class Employees(BoxFileTextStream)` contains behavior to pull data for employees using v1/employees

    If some streams implement incremental sync, it is typical to create another class
    `class IncrementalBoxFileTextStream((BoxFileTextStream), ABC)` then have concrete stream implementations extend it. An example
    is provided below.
...
class Customers(BoxFileTextStream):
    """
...
class IncrementalBoxFileTextStream(BoxFileTextStream, ABC):
    """
...
class Employees(IncrementalBoxFileTextStream):
    """
    TODO: Change class name to match the table/data source this stream corresponds to.
    """
...
class BoxFile(BoxFileTextStream):
    ...
    def read_records(self) -> File:
        ...
        try:
            ...
            logger.error(f"Unable to read file {self.file_id} - {e}")
