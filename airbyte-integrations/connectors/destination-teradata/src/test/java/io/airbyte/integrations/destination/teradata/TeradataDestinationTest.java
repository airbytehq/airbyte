...
public class TeradataDestinationTest {
    ...
    switch (sslMethod) {
      case "verify-ca", "verify-full" -> {
        additionalParameters = ImmutableMap.of(
            TeradataDestination.PARAM_SSL_MODE,
            Jsons.jsonNode(ImmutableMap.of(
                TeradataDestination.PARAM_MODE,
                sslMethod,
                TeradataDestination.CA_CERT_KEY,
                "dummycertificatecontent")));
      }
      default -> {
        additionalParameters = ImmutableMap.of(
            TeradataDestination.PARAM_SSL_MODE,
            Jsons.jsonNode(ImmutableMap.of(TeradataDestination.PARAM_MODE, sslMethod)));
      }
    }
  ...
  void testJdbcUrlExtraParams() {
    ...
    assertEquals("db", jdbcConfig.get(JdbcUtils.SCHEMA_KEY).asText());
    assertEquals(
        EXTRA_JDBC_PARAMS, jdbcConfig.get(JdbcUtils.JDBC_URL_PARAMS_KEY).asText());
  }
  ...
  void testDefaultSchemaName() {
    ...
    assertEquals(EXPECTED_JDBC_URL, jdbcConfig.get(JdbcUtils.JDBC_URL_KEY).asText());
    assertEquals(
        TeradataDestination.DEFAULT_SCHEMA_NAME,
  ...
  void testSSLDefaultMode() {
    ...
    final Map<String, String> properties = destination.getDefaultConnectionProperties(jdbcConfig);
    assertEquals(
        TeradataDestination.REQUIRE,
        properties.get(TeradataDestination.PARAM_SSLMODE).toString());
  }
  ...
  void testSSLVerfifyFullMode() {
    ...
    final Map<String, String> properties = destination.getDefaultConnectionProperties(jdbcConfig);
    assertEquals(
        TeradataDestination.VERIFY_FULL,
        properties.get(TeradataDestination.PARAM_SSLMODE).toString());
    assertNotNull(properties.get(TeradataDestination.PARAM_SSLCA).toString());
  }
}
