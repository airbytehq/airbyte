...
public class TeradataHttpClient {
  ...
  public CompletableFuture<Void> startEnvironment(
      EnvironmentRequest environmentRequest, String token) {
    var requestBody =
        handleCheckedException(() -> objectMapper.writeValueAsString(environmentRequest.request()));
    return getVoidCompletableFuture(environmentRequest.name(), token, requestBody);
  }

  public CompletableFuture<Void> stopEnvironment(
      EnvironmentRequest environmentRequest, String token) {
    var requestBody =
        handleCheckedException(() -> objectMapper.writeValueAsString(environmentRequest.request()));
    return getVoidCompletableFuture(environmentRequest.name(), token, requestBody);
  ...
  private CompletableFuture<Void> getVoidCompletableFuture(
      String name, String token, String jsonPayLoadString) {
    HttpRequest.BodyPublisher publisher = HttpRequest.BodyPublishers.ofString(jsonPayLoadString);
    var httpRequest = HttpRequest.newBuilder(
            URI.create(baseUrl.concat("/environments/").concat(name)))
        .headers(AUTHORIZATION, BEARER + token, CONTENT_TYPE, APPLICATION_JSON)
