...
public class TeradataDestinationAcceptanceTest extends JdbcDestinationAcceptanceTest {
  ...
  private static final Logger LOGGER =
      LoggerFactory.getLogger(TeradataDestinationAcceptanceTest.class);
  ...
  void initEnvironment() throws Exception {
    ...
    TeradataHttpClient teradataHttpClient =
        new TeradataHttpClient(configJson.get("env_url").asText());
  ...
  void cleanupEnvironment() throws ExecutionException, InterruptedException {
    try {
      TeradataHttpClient teradataHttpClient =
          new TeradataHttpClient(configJson.get("env_url").asText());
      var request =
          new EnvironmentRequest(configJson.get("env_name").asText(), new OperationRequest("stop"));
      teradataHttpClient.stopEnvironment(request, configJson.get("env_token").asText());
    } catch (BaseException be) {
      LOGGER.error("Environemnt " + configJson.get("env_name").asText() + " is not available. "
          + be.getMessage());
    }
  }
  ...
  protected void setup(TestDestinationEnv testEnv, HashSet<String> TEST_SCHEMAS) {
    ...
    try {
      ...
      AirbyteTraceMessageUtility.emitSystemErrorTrace(
          e, "Database " + SCHEMA_NAME + " creation got failed.");
    }
  }
  ...
  protected void tearDown(TestDestinationEnv testEnv) throws Exception {
    ...
    try {
      ...
      AirbyteTraceMessageUtility.emitSystemErrorTrace(
          e, "Database " + SCHEMA_NAME + " delete got failed.");
    } finally {
