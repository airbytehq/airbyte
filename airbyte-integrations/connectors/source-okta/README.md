...
to generate the necessary credentials. Then create a file `secrets/config.json` conforming to the `source_okta/spec.yaml` file.
...
poetry run source-okta read --config secrets/config.json --catalog sample_files/configured_catalog.json
```
