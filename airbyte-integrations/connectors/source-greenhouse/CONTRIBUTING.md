...
This connector uses Greenhouse Harvest v3 with two OAuth authentication branches selected by `credentials.auth_type` through `SelectiveAuthenticator`: `Client` uses Authorization Code credentials and rotating refresh tokens, while `ClientCredentials` uses a self-managed custom integration without a refresh token. The `advanced_auth` predicate must remain `[credentials, auth_type]` with `predicate_value: Client` so only the Cloud/partner branch drives the OAuth consent flow. The branches intentionally differ: `Client` uses the `expires_at` timestamp and `refresh_token_updater`, while `ClientCredentials` uses integer `expires_in` and must not gain `token_expiry_date_format` or `refresh_token_updater`. Cursor follow-up requests use the opaque URL from the `Link` header and must not repeat first-request-only parameters such as `per_page`, date filters, parent filters, or static filters. The `refresh_token_updater` persists each rotated refresh token on the Authorization Code branch. Connections using that branch idle longer than the approximately 24-hour refresh-token lifetime require manual reauthentication. The legacy `applied_at` watermark is discarded during the 1.0.0 upgrade so `applications` backfills once on the new `updated_at` cursor. Child streams preserve their minimum recoverable `updated_at` cursor while flattening partition state and resume without a backfill.
