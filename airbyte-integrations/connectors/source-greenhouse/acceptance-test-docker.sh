...
if [ -n "$USE_LOCAL_CDK" ]; then
  ...
  docker build -f $CDK_DIR/Dockerfile.connectors $CDK_DIR --target $TARGET -t airbyte/airbyte-cdk-connectors
  docker build -f $CDK_DIR/Dockerfile.sats $CDK_DIR --target $TARGET -t airbyte/airbyte-cdk-sats

  # Build the SAT Docker image
  docker build ../../bases/source-acceptance-test --build-arg $USE_LOCAL_CDK -t airbyte/source-acceptance-test
else
