...
class BuilderCalls(CianStream):
    ...
    def __init__(
        self, authenticator: TokenAuthenticator, date_from: datetime.date, date_to: datetime.date, building_id: Optional[int] = None
    ):
        super().__init__(authenticator)
    ...
    def stream_slices(
        ...
        for t_delta in range((self.date_to - self.date_from).days + 1):
...
class SourceCian(AbstractSource):
    def check_connection(self, logger, config) -> Tuple[bool, any]:
        ...
        try:
            test_stream = BuilderCalls(authenticator=self.get_auth(config), date_from=today_date, date_to=today_date)
