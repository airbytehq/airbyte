...
class ChanneledStream(SlackStream, ABC):
    ...
    def __init__(self, channel_filter: Optional[List[str]] = None, **kwargs):
...
class JoinChannelsStream(HttpStream):
    ...
    def request_body_json(self, stream_slice: Optional[Mapping] = None, **kwargs) -> Optional[Mapping]:
...
class SourceSlack(AbstractSource):
    def _get_authenticator(self, config: Mapping[str, Any]):
        ...
        if credentials_title == "Default OAuth2.0 authorization":
            ...
            msg = f"No supported option_title: {credentials_title} specified. See spec.json for references"
            raise Exception(msg)
