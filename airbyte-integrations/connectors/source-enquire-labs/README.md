...
```
./gradlew :airbyte-integrations:connectors:source-enquire-labs:build
```

#### Create credentials
**If you are a community contributor**, follow the instructions in the [documentation](https://docs.airbyte.io/integrations/sources/enquire-labs)
to generate the necessary credentials. Then create a file `secrets/config.json` conforming to the `source_enquire_labs/spec.json` file.
Note that any directory named `secrets` is gitignored across the entire Airbyte repo, so there is no danger of accidentally checking in sensitive information.
See `integration_tests/sample_config.json` for a sample config file.

**If you are an Airbyte core member**, copy the credentials in Lastpass under the secret name `source enquire-labs test creds`
...
```
./gradlew :airbyte-integrations:connectors:source-enquire-labs:airbyteDocker
```
...
To run unit tests:
```
./gradlew :airbyte-integrations:connectors:source-enquire-labs:unitTest
```
To run acceptance and custom integration tests:
```
./gradlew :airbyte-integrations:connectors:source-enquire-labs:integrationTest
```
