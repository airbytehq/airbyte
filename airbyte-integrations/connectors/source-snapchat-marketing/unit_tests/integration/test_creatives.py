...
class TestCreatives(TestCase):
    ...
    def test_read_records_with_pagination(self, http_mocker: HttpMocker) -> None:
        next_link = f"https://adsapi.snapchat.com/v1/adaccounts/{AD_ACCOUNT_ID}/creatives?cursor=page2"
        http_mocker.post(
            OAuthRequestBuilder.oauth_endpoint().build(),
            oauth_response(),
        )
        ...
        http_mocker.get(
            RequestBuilder.creatives_endpoint(AD_ACCOUNT_ID).build(),
            creatives_response(creative_id="creative_1", has_next=True, next_link=next_link),
        )
        http_mocker.get(
            HttpRequest(url=next_link),
            creatives_response(creative_id="creative_2", has_next=False),
        )
