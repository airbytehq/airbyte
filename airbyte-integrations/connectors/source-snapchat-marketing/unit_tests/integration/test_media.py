...
class TestMedia(TestCase):
    ...
    def test_read_records_with_pagination(self, http_mocker: HttpMocker) -> None:
        next_link = f"https://adsapi.snapchat.com/v1/adaccounts/{AD_ACCOUNT_ID}/media?cursor=page2"
        http_mocker.post(
            OAuthRequestBuilder.oauth_endpoint().build(),
            oauth_response(),
        )
        ...
        http_mocker.get(
            RequestBuilder.media_endpoint(AD_ACCOUNT_ID).build(),
            media_response(media_id="media_1", has_next=True, next_link=next_link),
        )
        http_mocker.get(
            HttpRequest(url=next_link),
            media_response(media_id="media_2", has_next=False),
        )
