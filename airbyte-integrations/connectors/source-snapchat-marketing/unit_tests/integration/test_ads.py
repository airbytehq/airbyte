...
class TestAds(TestCase):
    ...
    def test_read_records_with_pagination(self, http_mocker: HttpMocker) -> None:
        next_link = f"https://adsapi.snapchat.com/v1/adaccounts/{AD_ACCOUNT_ID}/ads?cursor=page2"
        http_mocker.post(
            OAuthRequestBuilder.oauth_endpoint().build(),
            oauth_response(),
        )
        ...
        http_mocker.get(
            RequestBuilder.ads_endpoint(AD_ACCOUNT_ID).build(),
            ads_response(ad_id="ad_1", has_next=True, next_link=next_link),
        )
        http_mocker.get(
            HttpRequest(url=next_link),
            ads_response(ad_id="ad_2", has_next=False),
        )
