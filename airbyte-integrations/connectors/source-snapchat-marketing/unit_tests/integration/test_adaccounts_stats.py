...
class TestAdaccountsStatsIncremental(TestCase):
    ...
    @HttpMocker()
    def test_daily_stream_accepts_legacy_second_precision_state(self, http_mocker: HttpMocker) -> None:
        previous_state_date = "2024-01-15T23:59:57"
        state = StateBuilder().with_stream_state("adaccounts_stats_daily", {"start_time": previous_state_date}).build()
        ...
        output = _read(config_builder=config(), stream_name="adaccounts_stats_daily", sync_mode=SyncMode.incremental, state=state)
