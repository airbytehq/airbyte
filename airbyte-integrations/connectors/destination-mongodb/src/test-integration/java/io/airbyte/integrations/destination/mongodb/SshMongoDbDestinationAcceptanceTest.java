...
public abstract class SshMongoDbDestinationAcceptanceTest extends MongodbDestinationAcceptanceTest {
  ...
  protected void setup(final TestDestinationEnv testEnv, final HashSet<String> TEST_SCHEMAS) {
    container =
        new MongoDBContainer(DOCKER_IMAGE_NAME).withNetwork(network).withExposedPorts(DEFAULT_PORT);
    container.start();
  ...
  protected List<JsonNode> retrieveRecords(
    ...
    final MongoDatabase database = getMongoDatabase(
        HostPortResolver.resolveIpAddress(container),
