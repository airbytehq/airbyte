...
This is the repository for the Datadog configuration based source connector.
...
```
Once the command is done, you will find your connector image in your local docker registry: `airbyte/source-datadog:dev`.
...
```Dockerfile
FROM airbyte/source-datadog:latest
...
docker run airbyte/source-datadog:dev spec
```
#### Run
...
4. Make the connector documentation and its changelog is up to date (`docs/integrations/sources/datadog.md`).
