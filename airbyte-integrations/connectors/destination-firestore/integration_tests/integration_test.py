...
def test_check_invalid_config():
    outcome = DestinationFirestore().check(
        AirbyteLogger(), {"project_id": "not_a_real_id"})
...
def retrieve_all_records(client):
    return [
        AirbyteMessage(
            type=Type.RECORD, record=AirbyteRecordMessage(
                stream=collection.id, data=doc.to_dict(), emitted_at=0)
        )
...
def test_write_append(config: Mapping, configured_catalog: ConfiguredAirbyteCatalog, writer: FirestoreWriter):
    ...
    expected_records = [_record(stream, str(i), i)
                        for i in range(1, 3)]
...
def test_write_overwrite(config: Mapping, configured_catalog: ConfiguredAirbyteCatalog, writer: FirestoreWriter):
    ...
    list(destination.write(config, configured_catalog, [
        *[_record(stream, str(i), i) for i in range(1, 3)],
        state_message
    ]))
