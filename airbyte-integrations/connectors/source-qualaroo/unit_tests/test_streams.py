...
from source_qualaroo.source import Surveys, Responses, QualarooStream
...
def test_request_params(patch_base_class, config):
    ...
    inputs = {"stream_slice": None, "stream_state": None,
              "next_page_token": {"before": "id"}}
    expected_params = {
        "limit": 500,
...
def test_surveys_stream(requests_mock):
    mock_surveys_request = requests_mock.get(
        ...
        json=[{"id": "b11111111111111111111111", "name": "survey_1"},
              {"id": "b22222222222222222222222", "name": "survey_2"}],
    )
    ...
    assert records == [{"id": "b11111111111111111111111", "name": "survey_1"}, {
        "id": "b22222222222222222222222", "name": "survey_2"}]

    stream2 = Surveys(
        config={**config, "survey_ids": ["b22222222222222222222222"]})
...
def test_responses_stream(requests_mock):
    ...
    mock_responses_request_1 = requests_mock.get(
        ...
        headers=NO_SLEEP_HEADERS,
        json=[{"id": "c11111111111111111111111", "name": "response_1"},
    ...
    mock_responses_request_2 = requests_mock.get(
        ...
        headers=NO_SLEEP_HEADERS,
        json=[{"id": "c33333333333333333333333", "name": "response_3"},
    ...
    stream2 = Responses(
        config={**config, "survey_ids": ["b22222222222222222222222"]})
    records = read_all_records(stream2)
    assert records == [{"id": "c33333333333333333333333", "name": "response_3"}, {
        "id": "c44444444444444444444444", "name": "response_4"}]
