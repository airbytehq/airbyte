...
class ClickhouseDirectLoaderTest {
    ...
    @Test
    fun `complete case 1 - #accept munges and accumulates record then flushes buffer when accumulated bytes hits threshold`() = runTest {
        val bytes = ClickhouseDirectLoader.Constants.MAX_BATCH_SIZE_BYTES
        val input = mockk<DestinationRecordRaw> {
            every { serializedSizeBytes } returns bytes
        }
        val result = loader.accept(input)
    ...
    @Test
    fun `complete case 2 - #accept munges and accumulates record then flushes buffer when accumulated record count hits threshold`() = runTest {
        val input = mockk<DestinationRecordRaw>(relaxed = true)
    ...
    object Fixtures {
        val mungedRecord = mapOf(
            "key" to StringValue("text")
        )
    }
}
