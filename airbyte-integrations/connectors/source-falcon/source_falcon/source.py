...
class SourceFalcon(YamlDeclarativeSource):
    ...
    def _raas_streams(self, config: Mapping[str, Any]) -> List[Stream]:
        ...
        reports_stream_template = [
            x for x in stream_configs if x["name"] == REPORTS_STREAM_NAME
        ][0]
        raas_streams = self._get_custom_stream_config_reports_from_template(
            config, reports_stream_template
        )
        source_streams = [
            ...
            for stream_config in self._initialize_cache_for_parent_streams(
                deepcopy(raas_streams)
            )
        ]
    ...
    def _get_custom_stream_config_reports_from_template(
        ...
        for report_id in config["credentials"]["report_ids"]:
            ...
            new_transformations = (
                new_stream["transformations"][0]["fields"]
                + ReportXMLSchemaHelper(
                    config, report_id
                ).fields_transform_string_array()
            )
