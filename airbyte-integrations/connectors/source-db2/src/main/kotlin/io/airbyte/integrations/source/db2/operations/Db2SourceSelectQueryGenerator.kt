...
@Singleton
@Primary
class Db2SourceSelectQueryGenerator : SelectQueryGenerator {
    ...
    private fun SelectQuerySpec.sql(): String {
        ...
        val fetchClause: String =
            when (limit) {
                NoLimit -> return sqlWithoutLimit
                Limit(0) -> "FETCH FIRST 0 ROWS ONLY"
                is Limit -> "FETCH FIRST ? ROWS ONLY"
            }
        return if (where == NoWhere && orderBy == NoOrderBy) {
            "$sqlWithoutLimit $fetchClause"
        } else {
            "${select.sql()} FROM ($sqlWithoutLimit) $fetchClause"
        }
    }
    ...
    private fun FromNode.sql(): String =
        when (this) {
            ...
            is FromSample -> {
                ...
                "FROM (SELECT * FROM ($inner) FETCH FIRST $sampleSize ROWS ONLY)"
            }
        }
