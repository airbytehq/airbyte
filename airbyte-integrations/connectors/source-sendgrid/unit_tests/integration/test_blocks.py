...
from .response_builder import blocks_response, empty_response, error_response
...
class TestBlocksStream(TestCase):
    @HttpMocker()
    def test_read_records(self, http_mocker: HttpMocker) -> None:
        """Basic read test for blocks stream"""
        http_mocker.get(
            RequestBuilder.blocks_endpoint().with_any_query_params().build(),
            blocks_response(),
        )

        output = _read(config_builder=config())
        assert len(output.records) == 1
        assert output.records[0].record.data["email"] == "blocked@example.com"
        assert output.records[0].record.data["reason"] == "Connection timed out"
...
@freezegun.freeze_time("2024-01-31T00:00:00Z")
class TestBlocksPagination(TestCase):
    @HttpMocker()
    def test_pagination_two_pages(self, http_mocker: HttpMocker) -> None:
        """Test pagination with 2 pages using offset-based pagination"""
        page1_records = [
            {"created": 1704067200, "email": f"block{i}@example.com", "reason": "Blocked", "status": "4.0.0"} for i in range(500)
        ]
        page2_records = [{"created": 1704067200, "email": "block500@example.com", "reason": "Blocked", "status": "4.0.0"}]

        http_mocker.get(
            RequestBuilder.blocks_endpoint().with_any_query_params().build(),
            [
                blocks_response(records=page1_records),
                blocks_response(records=page2_records),
            ],
        )
...
@freezegun.freeze_time("2024-01-31T00:00:00Z")
class TestBlocksIncremental(TestCase):
    @HttpMocker()
...
@freezegun.freeze_time("2024-01-31T00:00:00Z")
class TestBlocksErrors(TestCase):
    @HttpMocker()
