...
class TestGlobalSuppressionsStream:
    """
    Tests for the global_suppressions stream with offset pagination and incremental sync.
    This stream uses the same paginator as bounces, blocks, spam_reports, and invalid_emails.
    """
    ...
    def test_read_full_refresh_single_page(self):
        ...
        with HttpMocker() as http_mocker:
            http_mocker.get(
                HttpRequest(
                    url=f"{_BASE_URL}/v3/suppression/unsubscribes",
                    query_params={
                        "limit": "500",
                        "offset": "0",
                ...
                HttpResponse(body=_get_response("global_suppressions.json"), status_code=200),
            )
            ...
            assert actual_messages.records[0].record.data["email"] == "unsubscribed@example.com"
    ...
    def test_read_full_refresh_with_pagination(self):
        ...
        with HttpMocker() as http_mocker:
            first_page_records = json.loads(_get_response("global_suppressions.json")) * 500
            http_mocker.get(
                HttpRequest(
                    url=f"{_BASE_URL}/v3/suppression/unsubscribes",
                    query_params={
