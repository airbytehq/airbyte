...
from .response_builder import empty_response, error_response, global_suppressions_response
from .utils import config, read_output


_STREAM_NAME = "global_suppressions"
...
class TestGlobalSuppressionsStream(TestCase):
    @HttpMocker()
    def test_read_records(self, http_mocker: HttpMocker) -> None:
        """Basic read test for global_suppressions stream"""
        http_mocker.get(
            RequestBuilder.global_suppressions_endpoint().with_any_query_params().build(),
            global_suppressions_response(),
        )

        output = _read(config_builder=config())
        assert len(output.records) == 1
        assert output.records[0].record.data["email"] == "unsubscribed@example.com"
...
@freezegun.freeze_time("2024-01-31T00:00:00Z")
class TestGlobalSuppressionsPagination(TestCase):
    @HttpMocker()
    def test_pagination_two_pages(self, http_mocker: HttpMocker) -> None:
        """Test pagination with 2 pages using offset-based pagination"""
        page1_records = [{"created": 1704067200, "email": f"unsub{i}@example.com"} for i in range(500)]
        page2_records = [{"created": 1704067200, "email": "unsub500@example.com"}]

        http_mocker.get(
            RequestBuilder.global_suppressions_endpoint().with_any_query_params().build(),
            [
                global_suppressions_response(records=page1_records),
                global_suppressions_response(records=page2_records),
            ],
        )
...
@freezegun.freeze_time("2024-01-31T00:00:00Z")
class TestGlobalSuppressionsIncremental(TestCase):
    @HttpMocker()
...
@freezegun.freeze_time("2024-01-31T00:00:00Z")
class TestGlobalSuppressionsErrors(TestCase):
    @HttpMocker()
