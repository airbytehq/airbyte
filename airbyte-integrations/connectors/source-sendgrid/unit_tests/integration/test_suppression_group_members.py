...
class TestSuppressionGroupMembersStream:
    """
    Tests for the suppression_group_members stream with offset pagination and incremental sync.
    This stream uses cursor field 'created_at' (different from other suppression streams that use 'created').
    """
    ...
    def test_read_full_refresh_single_page(self):
        ...
        with HttpMocker() as http_mocker:
            http_mocker.get(
                HttpRequest(
                    url=f"{_BASE_URL}/v3/asm/suppressions",
                    query_params={
                        "limit": "500",
                ...
                HttpResponse(body=_get_response("suppression_group_members.json"), status_code=200),
            )
            ...
            assert actual_messages.records[0].record.data["email"] == "member@example.com"
            assert actual_messages.records[0].record.data["group_id"] == 123
            assert actual_messages.records[0].record.data["group_name"] == "Weekly Newsletter"
    ...
    def test_read_full_refresh_with_pagination(self):
        ...
        with HttpMocker() as http_mocker:
            first_page_records = json.loads(_get_response("suppression_group_members.json")) * 500
            http_mocker.get(
                HttpRequest(
                    url=f"{_BASE_URL}/v3/asm/suppressions",
                    query_params={
    ...
    def test_read_incremental_with_prior_state(self):
        ...
        config = ConfigBuilder().build()
        state = StateBuilder().with_stream_state(_STREAM_NAME, {"created_at": 1704844800}).build()
