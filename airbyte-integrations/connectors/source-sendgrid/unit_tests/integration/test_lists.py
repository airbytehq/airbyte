...
class TestListsStream:
    """Tests for the lists stream with cursor-based pagination."""
    ...
    def test_read_full_refresh_single_page(self):
        ...
        with HttpMocker() as http_mocker:
            http_mocker.get(
                HttpRequest(
                    url=f"{_BASE_URL}/v3/marketing/lists",
                    query_params={"page_size": "1000"},
                ),
                HttpResponse(body=_get_response("lists.json"), status_code=200),
            )
            ...
            assert actual_messages.records[0].record.data["id"] == "abc123-def456"
            assert actual_messages.records[0].record.data["name"] == "Marketing List"
    ...
    def test_read_full_refresh_with_pagination(self):
        ...
        with HttpMocker() as http_mocker:
            ...
            http_mocker.get(
                HttpRequest(
                    url=f"{_BASE_URL}/v3/marketing/lists?page_token=token123&page_size=1000",
                ),
                HttpResponse(body=_get_response("lists.json"), status_code=200),
            )
    ...
    def test_read_returns_expected_fields(self):
        ...
        with HttpMocker() as http_mocker:
            ...
            assert "contact_count" in record_data
