...
from .response_builder import empty_response, error_response, spam_reports_response
...
class TestSpamReportsStream(TestCase):
    @HttpMocker()
    def test_read_records(self, http_mocker: HttpMocker) -> None:
        """Basic read test for spam_reports stream"""
        http_mocker.get(
            RequestBuilder.spam_reports_endpoint().with_any_query_params().build(),
            spam_reports_response(),
        )

        output = _read(config_builder=config())
        assert len(output.records) == 1
        assert output.records[0].record.data["email"] == "spam@example.com"
        assert output.records[0].record.data["ip"] == "192.168.1.1"
...
@freezegun.freeze_time("2024-01-31T00:00:00Z")
class TestSpamReportsPagination(TestCase):
    @HttpMocker()
    def test_pagination_two_pages(self, http_mocker: HttpMocker) -> None:
        """Test pagination with 2 pages using offset-based pagination"""
        page1_records = [{"created": 1704067200, "email": f"spam{i}@example.com", "ip": "192.168.1.1"} for i in range(500)]
        page2_records = [{"created": 1704067200, "email": "spam500@example.com", "ip": "192.168.1.1"}]

        http_mocker.get(
            RequestBuilder.spam_reports_endpoint().with_any_query_params().build(),
            [
                spam_reports_response(records=page1_records),
                spam_reports_response(records=page2_records),
            ],
        )
...
@freezegun.freeze_time("2024-01-31T00:00:00Z")
class TestSpamReportsIncremental(TestCase):
    @HttpMocker()
...
@freezegun.freeze_time("2024-01-31T00:00:00Z")
class TestSpamReportsErrors(TestCase):
    @HttpMocker()
