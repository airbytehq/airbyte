...
def writer_no_existing_luzmo_columns(
    config: Mapping[str, Any],
    configured_catalog: ConfiguredAirbyteCatalog,
    logger: MagicMock,
) -> LuzmoWriter:
    """Returns a LuzmoWriter using MagicMock, and mocking the return_value of all used LuzmoClient methods."""
    with patch("destination_luzmo.writer.LuzmoClient", MagicMock()) as luzmo_client_mock:
        # Mock get_ordered_columns to return no existing Luzmo columns (dataset hasn't been created yet --> first sync)
        luzmo_client_mock.return_value.get_ordered_columns.return_value = []
        # luzmo_client_mock.return_value.batch_write.return_value = None
        # luzmo_client_mock.return_value.set_replace_tag_on_dataset.return_value = None
        return LuzmoWriter(config, configured_catalog, logger)


def test_small_enough_data_point_limit(writer: LuzmoWriter):
    """Tests whether the FLUSH_INTERVAL variable is smaller than the maximum amount of data points Luzmo supports."""
    assert writer.FLUSH_INTERVAL <= 10000


def test_init(writer: LuzmoWriter):
    """Tests whether LuzmoWriter is correctly initialized for streams with no known Luzmo dataset (i.e. first sync for each stream)."""
    ...
    # Assert each stream is correctly initializing update_metadata (due to no columns from Luzmo in this writer, both are True)
    assert writer.writers["orders"]["update_metadata"] is True
    assert writer.writers["products"]["update_metadata"] is True


def test_transform_data(writer: LuzmoWriter):
...
def test_transform_data_missing_data(writer: LuzmoWriter):
    case = unittest.TestCase()
...
def test_transform_data_additional_data(writer: LuzmoWriter):
    case = unittest.TestCase()
...
def test_transform_data_bool_data(writer: LuzmoWriter):
    case = unittest.TestCase()
...
def test_transform_data_dict_data(writer: LuzmoWriter):
    case = unittest.TestCase()
...
def test_transform_data_arr_data(writer: LuzmoWriter):
    case = unittest.TestCase()
...
def test_queue_write_operation(writer: LuzmoWriter):
...
def test_queue_write_operation_two_streams(writer: LuzmoWriter):
    # Set flush interval to max value to avoid flushing data
    writer.FLUSH_INTERVAL = 10000
...
def test_queue_write_operation_non_existing_stream(writer: LuzmoWriter):
    # Set flush interval to max value to avoid flushing data
    writer.FLUSH_INTERVAL = 10000
...
def test_flush(writer: LuzmoWriter):
    writer.client.batch_write = MagicMock()  # type: ignore
...
def test_queue_write_flush_operation(writer: LuzmoWriter):
...
def test_flush_all(writer: LuzmoWriter):
    writer.client.batch_write = MagicMock()  # type: ignore
...
def test_delete_stream_entries(writer: LuzmoWriter):
...
def _get_luzmo_and_merged_columns(writer: LuzmoWriter) -> Mapping[str, Any]:
    ...
    for stream_name in writer.writers:
        luzmo_columns = writer.client.get_ordered_columns(stream_name)
        merged_columns = writer.writers[stream_name]["column_headers"]
        result[stream_name] = {
            "luzmo_columns": luzmo_columns,
...
@pytest.fixture
def writer_existing_luzmo_columns(
    config: Mapping[str, Any],
    configured_catalog: ConfiguredAirbyteCatalog,
    logger: MagicMock,
) -> LuzmoWriter:
    """This will return a LuzmoWriter that mocks airbyte stream catalogs that contains the same columns as those existing in Luzmo."""
    existing_luzmo_columns = {}
    for configured_stream in configured_catalog.streams:
        existing_luzmo_columns[configured_stream.stream.name] = [
            column_name for column_name in configured_stream.stream.json_schema["properties"]
        ]

    def get_existing_luzmo_columns(stream_name):
        return existing_luzmo_columns[stream_name]

    with patch("destination_luzmo.writer.LuzmoClient", MagicMock()) as luzmo_client_mock:
        # Mock get_ordered_columns to return existing_luzmo_columns
        luzmo_client_mock.return_value.get_ordered_columns = MagicMock(side_effect=get_existing_luzmo_columns)
        return LuzmoWriter(config, configured_catalog, logger)


def test_init_existing_luzmo_columns(writer_existing_luzmo_columns: LuzmoWriter):
    """Tests whether each stream is correctly initializing update_metadata.
    Due to identical columns in Luzmo for this writer, both are False.
    """
    assert writer_existing_luzmo_columns.writers["orders"]["update_metadata"] is False
    assert writer_existing_luzmo_columns.writers["products"]["update_metadata"] is False


def test_equal_luzmo_and_merged_columns(
    writer_existing_luzmo_columns: LuzmoWriter,
):
    result = _get_luzmo_and_merged_columns(writer_existing_luzmo_columns)

    for stream_name in result:
        for index, column in enumerate(result[stream_name]["merged_columns"]):
            # Assert that merged_columns are in same order as columns defined on Luzmo's side.
            assert result[stream_name]["luzmo_columns"][index] == column["name"]


def test_queue_write_operation_with_correct_data_order(
    writer_existing_luzmo_columns: LuzmoWriter,
):
    writer_existing_luzmo_columns.client.batch_write = MagicMock()  # type: ignore

    result = _get_luzmo_and_merged_columns(writer_existing_luzmo_columns)
    # Set flush interval to max value to avoid flushing data
    writer_existing_luzmo_columns.FLUSH_INTERVAL = 10000

    order_data = {"order_id": 1, "amount": 100.0, "customer_id": "cust_1"}
    writer_existing_luzmo_columns.queue_write_operation("orders", order_data)
    expected_data = []
    for column in result["orders"]["merged_columns"]:
        expected_data.append(order_data[column["name"]])
    assert writer_existing_luzmo_columns.writers["orders"]["write_buffer"][0] == expected_data
...
def writer_new_airbyte_column(
    ...
    """This will return a LuzmoWriter that mocks airbyte stream catalogs that contains one column that does not exist in Luzmo."""
    existing_luzmo_columns = {}
    for configured_stream in configured_catalog_with_new_column.streams:
        ...
        del columns[1]
        existing_luzmo_columns[configured_stream.stream.name] = columns

    def get_existing_luzmo_columns(stream_name):
        return existing_luzmo_columns[stream_name]

    with patch("destination_luzmo.writer.LuzmoClient", MagicMock()) as luzmo_client_mock:
        # Mock get_ordered_columns to return existing_luzmo_columns (which does not include one column defined in configured stream)
        luzmo_client_mock.return_value.get_ordered_columns = MagicMock(side_effect=get_existing_luzmo_columns)
        luzmo_client_mock.return_value.batch_writer.return_value = None
        luzmo_client_mock.return_value.set_replace_tag_on_dataset.return_value = None
        return LuzmoWriter(config, configured_catalog_with_new_column, logger)


def test_init_new_airbyte_column(writer_new_airbyte_column: LuzmoWriter):
...
def test_new_column_update_metadata(writer_new_airbyte_column: LuzmoWriter):
    """Tests whether Airbyte streams with at least one new column defined results in update_metadata,
    to inform Luzmo about new column data being pushed."""
...
def test_new_column_appended(writer_new_airbyte_column: LuzmoWriter):
    """Tests whether the Airbyte streams with one new column appends it at the end of the column list"""
    result = _get_luzmo_and_merged_columns(writer_new_airbyte_column)
    for stream_name in result:
        assert len(result[stream_name]["merged_columns"]) == len(result[stream_name]["luzmo_columns"]) + 1
        for index, column in enumerate(result[stream_name]["luzmo_columns"]):
            # Assert that merged_columns are in same order as columns defined on Luzmo's side.
            assert result[stream_name]["merged_columns"][index]["name"] == column
        with pytest.raises(Exception):
            # Test whether last element of merged_columns is the column that is not defined on Luzmo's end.
            result[stream_name]["luzmo_columns"].index(result[stream_name]["merged_columns"][-1]["name"])
...
def writer_deleted_airbyte_column(
    ...
    """This will return a LuzmoWriter that mocks airbyte stream catalogs that doesn't contain one column that does exist in Luzmo."""
    existing_luzmo_columns = {}
    ...
    with patch("destination_luzmo.writer.LuzmoClient", MagicMock()) as luzmo_client_mock:
        ...
        luzmo_client_mock.return_value.batch_writer.return_value = None
        luzmo_client_mock.return_value.set_replace_tag_on_dataset.return_value = None
        return LuzmoWriter(config, configured_catalog_with_deleted_column, logger)


def test_init_deleted_airbyte_column(writer_deleted_airbyte_column: LuzmoWriter):
...
def test_deleted_column_update_metadata(writer_deleted_airbyte_column: LuzmoWriter):
    """Tests whether Airbyte streams that do not contain a column defined on Luzmo's side results in update_metadata for only
    overwrite streams (to inform Luzmo about new column data being pushed)"""
    assert writer_deleted_airbyte_column.writers["orders_append"]["update_metadata"] is False
    assert writer_deleted_airbyte_column.writers["orders_overwrite"]["update_metadata"] is True


def test_merged_columns_order_for_deleted_column(
    writer_deleted_airbyte_column: LuzmoWriter,
):
    """Tests whether Airbyte streams that do not contain a column defined on Luzmo's side still correctly puts the other columns in
    the right order"""
    result = _get_luzmo_and_merged_columns(writer_deleted_airbyte_column)
    for stream_name in result:
        # Test whether merged_columns contains one less element
        assert len(result[stream_name]["merged_columns"]) == len(result[stream_name]["luzmo_columns"]) - 1

        luzmo_columns_without_deleted = [
            column_name for column_name in result[stream_name]["luzmo_columns"] if column_name != "customer_name"
        ]
        # Test whether elements, without deleted column, are equal and in the same position
        assert luzmo_columns_without_deleted == [column["name"] for column in result[stream_name]["merged_columns"]]
