...
def _convert_airbyte_configured_stream_into_headers_dict(
    ...
    Note that the Airbyte types are currently not used due to Luzmo's Data API Service not supporting specifying column types.
    """
    column_headers = {}
...
class LuzmoWriter:
    # Luzmo's Data API service has a limit of pushing 10 000 data points (i.e. rows) in a single request.
    # (see note here: https://developer.luzmo.com/?shell#data_create)
    ...
    def __init__(
        ...
        """Create a single Luzmo Client and a dict of writers.
        The Luzmo Client will be used to send API requests to Luzmo's API.
        ...
        """
        self.logger = logger
        self.client = LuzmoClient(config, logger)
    ...
    def queue_write_operation(self, stream_name: str, data: Mapping):
        ...
        """
        luzmo_data = self.transform_data(stream_name, data)
        self.writers[stream_name]["write_buffer"].append(luzmo_data)
    ...
    def flush(self, stream_name: str):
        """Write a batch of data from the write buffer using the Luzmo client."""
    ...
    def transform_data(self, stream_name: str, airbyte_data: Mapping) -> list[Any]:
        """Transform Airbyte data (one row) into Luzmo's expected data format (a list in the appropriate order).
        ...
        for column in airbyte_data:
            ...
            for column_header in self.writers[stream_name]["column_headers"]:
                if column_header["name"] == column:
                    unknown_data = False
                    # Luzmo doesn't support storing or querying nested (list, dict) or boolean data.
    ...
    def _create_writers(self, configured_catalog: ConfiguredAirbyteCatalog):
        ...
        This method will also merge the Luzmo columns for the stream's dataset, if existing."""
        writers = {}
        for configured_stream in configured_catalog.streams:
            result = self._merge_luzmo_and_airbyte_column_headers(configured_stream)
    ...
    def _merge_luzmo_and_airbyte_column_headers(self, configured_stream: ConfiguredAirbyteStream):
        """Merge columns known by Airbyte and Luzmo.
        - If the dataset does not yet exist in Luzmo (i.e. the first sync), the columns order will be based on "for el in dict" order.
        - Upon next synchronizations, the dataset exists in Luzmo. Its column order will be used to send data in the corresponding order.
        - If a new column is added to the source table (i.e. this column doesn't exist yet in Luzmo),
        ...
        - If an existing column is removed from the source:
          1. If the next synchronization for this stream runs in "overwrite" mode (or a "replace" tag is set), the Luzmo dataset will
             no longer contain the original column.
          2. If the next synchronization for this stream runs in "append" mode, the Luzmo dataset will
        ...
        """
        luzmo_column_headers = self.client.get_ordered_columns(configured_stream.stream.name)
        ...
        for airbyte_column_header in airbyte_column_headers:
            ...
            try:
                # Add an order based on the order of the column in the Luzmo dataset
                merged_column_header["order"] = luzmo_column_headers.index(airbyte_column_header)
            except ValueError:
                # Add an appropriate order to ensure the column appears at the end of the data
                new_column_count += 1
                merged_column_header["order"] = len(luzmo_column_headers) + new_column_count
        ...
        if new_column_count > 0:
            ...
            if len(luzmo_column_headers) > 0:
                self.logger.info(
                    f"One or more columns defined in stream {configured_stream.stream.name} are not yet present in Luzmo, "
                    f"and will added upon next successful synchronization."
                )
            else:
                self.logger.info(
                    f"The dataset for stream {configured_stream.stream.name} doesn't seem to exist in Luzmo. "
                    f"The next sync for this stream will create it."
                )
        elif not update_metadata:
            # Validate whether all columns in Luzmo are still part of the configured airbyte catalog definition.
            for luzmo_column_header in luzmo_column_headers:
                try:
                    # Try to find the Luzmo column header in the Airbyte columns
                    airbyte_column_headers[luzmo_column_header]
                except KeyError:
                    # Luzmo's column hasn't been found, so we'll need to update the dataset's metadata upon next sync.
                    if configured_stream.destination_sync_mode == DestinationSyncMode.overwrite:
                        self.logger.info(
                            f"The source column {luzmo_column_header} in Luzmo is no longer present in the configured "
                            f"stream {configured_stream.stream.name} (i.e. in the source). As the stream synchronization is "
                            f"in overwrite mode, the existing column in Luzmo will be deleted upon next sync. Check "
                            f"carefully whether this column is used in any existing Luzmo dashboards!"
                        )
                        update_metadata = True
