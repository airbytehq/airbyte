...
to generate the necessary credentials. Then create a file `secrets/config.json` conforming to the `source_my_hours/spec.yaml` file.
...
```
Once the command is done, you will find your connector image in your local docker registry: `airbyte/source-my-hours:dev`.
...
```Dockerfile
FROM airbyte/source-my-hours:latest
...
docker run airbyte/source-my-hours:dev spec
