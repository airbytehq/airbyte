...
class MyHoursAuthenticator(Oauth2Authenticator):
    ...
    def retrieve_refresh_token(self, email: str, password: str):
        ...
        self._access_token = json_response[self.get_access_token_name()]
        self._token_expiry_date = t0.add(seconds=json_response[self.get_expires_in_name()])
    ...
    def refresh_access_token(self) -> Tuple[str, int]:
        try:
            response = requests.request(method="POST", url=self.get_token_refresh_endpoint(), data=self.get_refresh_request_body())
            response.raise_for_status()
