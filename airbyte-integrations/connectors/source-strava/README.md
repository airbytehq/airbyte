...
```
poetry run source-strava spec
poetry run source-strava check --config secrets/config.json
poetry run source-strava discover --config secrets/config.json
poetry run source-strava read --config secrets/config.json --catalog sample_files/configured_catalog.json
```
...
1. Make sure your changes are passing our test suite: `airbyte-ci connectors --name=source-strava test`
