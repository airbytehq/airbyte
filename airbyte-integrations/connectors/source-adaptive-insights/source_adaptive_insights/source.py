...
class AdaptiveInsightsStream(HttpStream, ABC):
    ...
    `class AdaptiveInsightsStream(HttpStream, ABC)` which is the current class
    `class Customers(AdaptiveInsightsStream)` contains behavior to pull data for customers using v1/customers
    `class Employees(AdaptiveInsightsStream)` contains behavior to pull data for employees using v1/employees

    If some streams implement incremental sync, it is typical to create another class
    `class IncrementalAdaptiveInsightsStream((AdaptiveInsightsStream), ABC)` then have concrete stream implementations extend it. An example
    is provided below.
