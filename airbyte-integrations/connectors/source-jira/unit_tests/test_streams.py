...
@responses.activate
def test_field_schemes_stream(config, field_schemes_response):
    responses.add(
        responses.GET,
        f"https://{config['domain']}/rest/api/3/config/fieldschemes?maxResults=50",
        json=field_schemes_response,
    )

    stream = find_stream("field_schemes", config)
    records = list(read_full_refresh(stream))
