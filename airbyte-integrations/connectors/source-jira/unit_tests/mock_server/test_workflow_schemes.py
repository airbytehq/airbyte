...
_STREAM_NAME = "workflow_schemes"
_DOMAIN = "airbyteio.atlassian.net"


@freezegun.freeze_time(_NOW.isoformat())
class TestWorkflowSchemesStream(TestCase):
    """
    Tests for the Jira 'workflow_schemes' stream.
    ...
    def test_full_refresh_single_page(self, http_mocker: HttpMocker):
        ...
        workflow_scheme_records = [
            {
                "id": 10000,
                "name": "Default Workflow Scheme",
                "description": "Default workflow scheme for the project.",
                "defaultWorkflow": "jira",
            },
            {
                "id": 10001,
                "name": "Bug Workflow Scheme",
                "description": "Workflow scheme for bug issues.",
                "defaultWorkflow": "bug-workflow",
            },
        ]

        http_mocker.get(
            JiraRequestBuilder.workflow_schemes_endpoint(_DOMAIN).with_any_query_params().build(),
            JiraPaginatedResponseBuilder("values")
            .with_records(workflow_scheme_records)
