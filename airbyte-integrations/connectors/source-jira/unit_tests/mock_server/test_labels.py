...
class TestLabelsStream(TestCase):
    """
    Tests for the Jira 'labels' stream.

    This is a full refresh stream with pagination.
    Endpoint: /rest/api/3/label
    Uses custom LabelsRecordExtractor to transform string labels into objects
    Primary key: label
    """
    ...
    def test_full_refresh_single_page(self, http_mocker: HttpMocker):
        ...
        The LabelsRecordExtractor transforms string labels into {"label": "value"} objects.
        """
        config = ConfigBuilder().with_domain(_DOMAIN).build()

        # Labels are returned as strings in the API response
        label_values = ["bug", "enhancement", "documentation"]

        http_mocker.get(
            JiraRequestBuilder.labels_endpoint(_DOMAIN).with_any_query_params().build(),
            JiraPaginatedResponseBuilder("values")
            .with_records(label_values)
        ...
        # LabelsRecordExtractor transforms strings into {"label": "value"} objects
    ...
    def test_pagination_multiple_pages(self, http_mocker: HttpMocker):
        ...
        # Page 1
        page1_labels = ["bug", "enhancement"]

        # Page 2
        page2_labels = ["documentation"]

        http_mocker.get(
            JiraRequestBuilder.labels_endpoint(_DOMAIN).with_any_query_params().build(),
            [
                JiraPaginatedResponseBuilder("values")
                .with_records(page1_labels)
                .with_pagination(start_at=0, max_results=2, total=3, is_last=False)
                .build(),
                JiraPaginatedResponseBuilder("values")
                .with_records(page2_labels)
