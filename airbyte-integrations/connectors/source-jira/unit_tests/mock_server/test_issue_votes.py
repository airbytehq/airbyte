...
_STREAM_NAME = "issue_votes"
_DOMAIN = "airbyteio.atlassian.net"


@freezegun.freeze_time(_NOW.isoformat())
class TestIssueVotesStream(TestCase):
    """
    Tests for the Jira 'issue_votes' stream.
    ...
    def test_full_refresh_with_multiple_issues(self, http_mocker: HttpMocker):
        """
        Test full refresh sync with votes from multiple issues.
        """
        ...
        # Mock votes for issue 1
        issue1_votes = {
            "self": "https://airbyteio.atlassian.net/rest/api/3/issue/PROJ-1/votes",
            "votes": 5,
            "hasVoted": False,
            "voters": [{"accountId": "user1"}, {"accountId": "user2"}],
        }

        # Mock votes for issue 2
        issue2_votes = {
            "self": "https://airbyteio.atlassian.net/rest/api/3/issue/PROJ-2/votes",
            "votes": 3,
            "hasVoted": True,
            "voters": [{"accountId": "user3"}],
        }

        http_mocker.get(
            JiraRequestBuilder.issue_votes_endpoint(_DOMAIN, "PROJ-1").build(),
            HttpResponse(body=json.dumps(issue1_votes), status_code=200),
        )
        http_mocker.get(
            JiraRequestBuilder.issue_votes_endpoint(_DOMAIN, "PROJ-2").build(),
            HttpResponse(body=json.dumps(issue2_votes), status_code=200),
        )
        ...
        votes_counts = [r.record.data["votes"] for r in output.records]
        assert 5 in votes_counts
        assert 3 in votes_counts
    ...
    def test_issue_id_transformation(self, http_mocker: HttpMocker):
        ...
        votes = {"self": "https://airbyteio.atlassian.net/rest/api/3/issue/PROJ-1/votes", "votes": 5, "hasVoted": False, "voters": []}

        http_mocker.get(
            JiraRequestBuilder.issue_votes_endpoint(_DOMAIN, "PROJ-1").build(),
            HttpResponse(body=json.dumps(votes), status_code=200),
        )
    ...
    @HttpMocker()
    def test_issue_with_zero_votes(self, http_mocker: HttpMocker):
        """
        Test that connector handles issues with zero votes gracefully.
        """
        config = ConfigBuilder().with_domain(_DOMAIN).build()
        ...
        http_mocker.get(
            ...
            HttpResponse(
                body=json.dumps(
                    {"self": "https://airbyteio.atlassian.net/rest/api/3/issue/PROJ-1/votes", "votes": 0, "hasVoted": False, "voters": []}
                ),
                status_code=200,
            ),
        )
        ...
        # Should still return 1 record even with 0 votes
        assert len(output.records) == 1
        assert output.records[0].record.data["votes"] == 0
        assert not any(log.log.level == "ERROR" for log in output.logs)
