...
1. **Incremental Streams Read From the Recents Endpoint, Not the Entity Endpoints** -- ten of the
   eleven incremental streams poll `/v1/recents` for anything modified since the start date, so they
   ...
   backfilled. `deal_flow` reads each deal's change log and filters on `log_time` client-side.
   The other twenty-three streams are full refresh and ignore the start date, except
   `deal_products` and `deal_installments`, which only expand the deals returned by `deals`.
