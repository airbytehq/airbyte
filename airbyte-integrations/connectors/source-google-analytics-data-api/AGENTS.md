...
The property dimension is gated by `single_stream_per_report`: when disabled, the resolver creates one stream per report and property; when enabled, it creates one stream per report, partitions requests with a `ListPartitionRouter` over `property_ids`, and adds `property_id` from the partition. The gate produces `(0, True)` for `source_config_1`, so its `[1]` is a boolean and must never be concatenated into a stream name.

The `Consolidated` suffix in enabled mode is load-bearing, not cosmetic. Destination tables and incremental state are both keyed by stream name, so a consolidated stream reusing the plain report name would look to the platform like a continuation of the single-property stream: it would inherit that stream's cursor, which only ever tracked the first property ID, and every other property would resume from it instead of backfilling — silently, with no error. `test_enabling_the_flag_never_reuses_a_per_property_stream_name` pins this; the only intentional name shared between modes is the static `property_metadata`.

The same mechanism has an unavoidable consequence that naming cannot fix: adding a property ID to a connection that already has the flag enabled makes it a new **partition** on an existing stream, and `ConcurrentPerPartitionCursor._generate_slices_from_partition` seeds an unseen partition from `self._global_cursor` rather than from `start_datetime`. The new property is therefore never backfilled and the user must clear the stream. There is no declarative knob for this; it is documented as a caution in `docs/integrations/sources/google-analytics-data-api.md`.
