...
import pytest
from source_instagram.streams import InstagramStream, Users, UserInsights, UserLifetimeInsights, Media, MediaInsights, Stories, \
    StoryInsights
...
def test_media_read(api, user_stories_data, requests_mock):
    ...
    assert records == [{'business_account_id': 'test_id',
                        'id': 'test_id',
...
def test_user_read(api, user_data, requests_mock):
    ...
    records = read_full_refresh(stream)
    assert records == [{'biography': 'Dino data crunching app',
...
def test_user_lifetime_insights_read(api, config, user_insight_data, requests_mock):
    ...
    records = read_full_refresh(stream)
    assert records == [
        {'page_id': 'act_unknown_account', 'business_account_id': 'test_id', 'metric': 'impressions', 'date': '2020-05-04T07:00:00+0000',
         'value': 4}]
...
@pytest.mark.parametrize(
    ...
    ids=["oauth_error", "rate_limit_error", "too_many_request_error", "viewers_insights_error",
         "4028_issue_error", "transient_error", "user_media_creation_time_error"]
)
