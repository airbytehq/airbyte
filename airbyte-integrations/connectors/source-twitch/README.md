# Twitch source connector

This directory contains the manifest-only connector for `source-twitch`.
...
For information about how to configure and use this connector within Airbyte, see [the connector's full documentation](https://docs.airbyte.com/integrations/sources/twitch).
...
```bash
airbyte-ci connectors --name=source-twitch build
```

An image will be available on your host with the tag `airbyte/source-twitch:dev`.

### Creating credentials

**If you are a community contributor**, follow the instructions in the [documentation](https://docs.airbyte.com/integrations/sources/twitch)
...
```bash
docker run --rm airbyte/source-twitch:dev spec
docker run --rm -v $(pwd)/secrets:/secrets airbyte/source-twitch:dev check --config /secrets/config.json
docker run --rm -v $(pwd)/secrets:/secrets airbyte/source-twitch:dev discover --config /secrets/config.json
docker run --rm -v $(pwd)/secrets:/secrets -v $(pwd)/integration_tests:/integration_tests airbyte/source-twitch:dev read --config /secrets/config.json --catalog /integration_tests/configured_catalog.json
```
...
```bash
airbyte-ci connectors --name=source-twitch test
```

## Publishing a new version of the connector

If you want to contribute changes to `source-twitch`, here's how you can do that:
