...
actual migration.
*/
class ExtractedAtUtcTimezoneMigration(private val database: JdbcDatabase) :
    Migration<SnowflakeState> {
    private val logger: Logger =
        LoggerFactory.getLogger(ExtractedAtUtcTimezoneMigration::class.java)
    ...
    override fun migrateIfNecessary(
        ...
        if (!state.initialRawTableStatus.rawTableExists) {
            ...
            logger.info(
                "Skipping ExtractedAtUtcTimezoneMigration for ${stream.id.originalNamespace}.${stream.id.originalName} because the raw table doesn't exist"
            )
            return Migration.MigrationResult(
                state.destinationState.copy(extractedAtInUtc = true),
                false
            )
        }
        if (stream.destinationSyncMode == DestinationSyncMode.OVERWRITE) {
            ...
            logger.info(
                "Skipping ExtractedAtUtcTimezoneMigration for ${stream.id.originalNamespace}.${stream.id.originalName} because the sync mode is OVERWRITE."
            )
            return Migration.MigrationResult(
                state.destinationState.copy(extractedAtInUtc = true),
                false
        ...
        if (state.destinationState.extractedAtInUtc) {
            logger.info(
                "Skipping ExtractedAtUtcTimezoneMigration for ${stream.id.originalNamespace}.${stream.id.originalName}, already done."
            )
            return Migration.MigrationResult(state.destinationState, false)
        }
        val rawRecordTimezone: JsonNode? =
            database
                .queryJsons(
                    { connection ->
                        connection.prepareStatement(
                            select(
                                ...
                                .from(
                                    table(quotedName(stream.id().rawNamespace, stream.id().rawName))
                                )
                                .limit(1)
                ...
                .first()
        if (
            rawRecordTimezone == null ||
                (rawRecordTimezone.get("tzh").intValue() == 0 &&
                    rawRecordTimezone.get("tzm").intValue() == 0)
        ) {
            // There are no raw records, or the raw records are already in UTC. No migration
            // necessary. Update the state.
            logger.info(
                "Skipping ExtractedAtUtcTimezoneMigration for ${stream.id.originalNamespace}.${stream.id.originalName} because the raw table doesn't contain records needing migration."
            )
            return Migration.MigrationResult(
                state.destinationState.copy(extractedAtInUtc = true),
                false
        ...
        logger.info(
            "Executing ExtractedAtUtcTimezoneMigration for ${stream.id.originalNamespace}.${stream.id.originalName} for real."
        )
            ...
            Sql.of(
                update(table(quotedName(stream.id().rawNamespace, stream.id().rawName)))
                    .set(
                        ...
                        // We _could_ go through convert_timezone and manually add a negative offset
                        // number of hours
        ...
        // Invalidate the initial state - we've modified all the extracted_at timestamps, so need to
        // refetch them.
        return Migration.MigrationResult(
            state.destinationState.copy(needsSoftReset = true, extractedAtInUtc = true),
            true
        )
    }
}
