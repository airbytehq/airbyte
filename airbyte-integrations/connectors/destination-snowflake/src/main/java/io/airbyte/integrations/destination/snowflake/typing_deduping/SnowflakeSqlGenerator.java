...
public class SnowflakeSqlGenerator implements SqlGenerator<SnowflakeTableDefinition> {
  ...
  public StreamId buildStreamId(
    ...
    return new StreamId(
        ...
        StreamId.concatenateRawTableName(namespace, name),
        namespace,
        name);
  }
  ...
  public boolean existingSchemaMatchesStreamConfig(
      final StreamConfig stream, final SnowflakeTableDefinition existingTable)
      throws TableNotMigratedException {
    if (!existingTable
        .columns()
        .keySet()
        .containsAll(JavaBaseConstants.V2_FINAL_TABLE_METADATA_COLUMNS)) {
    ...
    final LinkedHashMap<String, String> actualColumns = existingTable.columns().entrySet().stream()
        .filter(
            column -> !JavaBaseConstants.V2_FINAL_TABLE_METADATA_COLUMNS.contains(column.getKey()))
        .collect(
            LinkedHashMap::new,
  ...
  private String extractAndCast(final ColumnId column, final AirbyteType airbyteType) {
    if (airbyteType instanceof final Union u) {
      ...
      return switch (dialectType) {
        case "TIMESTAMP_TZ" -> new StringSubstitutor(Map.of("column_name", column.originalName()))
            .replace(
        ...
        case "VARIANT" -> "\"_airbyte_data\":\"" + column.originalName() + "\"";
          ...
          // NULL (_not_ a
          // JSON null).
        case "OBJECT" -> new StringSubstitutor(Map.of("column_name", column.originalName()))
            .replace(
                """
            CASE
        ...
        case "ARRAY" -> new StringSubstitutor(Map.of("column_name", column.originalName()))
            .replace(
                """
            CASE
            ...
            END
            """);
        default -> "TRY_CAST(\"_airbyte_data\":\"" + column.originalName() + "\"::text as "
            + dialectType + ")";
      };
    }
  }
  ...
  String insertNewRecords(
    ...
    final String columnCasts = streamColumns.entrySet().stream()
        ...
        .collect(joining("\n"));
    final String columnErrors = streamColumns.entrySet().stream()
        .map(
            col -> new StringSubstitutor(Map.of(
                ...
                CASE
                  ...
                  ELSE []
                END"""))
        .reduce("ARRAY_CONSTRUCT()", (acc, col) -> "ARRAY_CAT(" + acc + ", " + col + ")");
