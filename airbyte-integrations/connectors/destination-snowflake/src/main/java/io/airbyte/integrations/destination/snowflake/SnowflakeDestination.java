...
public class SnowflakeDestination extends SwitchingDestination<SnowflakeDestination.DestinationType> {
    ...
    private static boolean isInternalStaging(JsonNode config) {
        return config.has("loading_method") && config.get("loading_method").isObject() && config.get("loading_method").get("method").asText().equals("Internal Staging");
    }
