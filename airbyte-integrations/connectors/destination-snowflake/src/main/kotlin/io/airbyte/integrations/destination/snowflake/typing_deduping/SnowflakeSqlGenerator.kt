...
class SnowflakeSqlGenerator(private val retentionPeriodDays: Int) : SqlGenerator {
    ...
    // TODO: Delete this or keep it behind a flag to fallback if MERGE doesn't work out.
    ...
    private fun upsertRecords(
        ...
        val targetAbExtractedAt =
            airbyteExtractedAtUtcForced(
                "TARGET_TABLE.${JavaBaseConstants.COLUMN_NAME_AB_EXTRACTED_AT.uppercase().quoted()}"
            )
        val pkEqualityMatch =
            stream.primaryKey
                .map { it.name.quoted() }
    ...
    private fun extractAndCast(
        ...
        return cast(
            """"${JavaBaseConstants.COLUMN_NAME_DATA}":"${escapeJsonIdentifier(column.originalName)}"""",
            airbyteType,
            useTryCast
        )
    }
