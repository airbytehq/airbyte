...
class SnowflakeSuperLimitationTransformerTest {
    private var transformer: SnowflakeSuperLimitationTransformer? = null
    @BeforeEach
    fun setup() {
        val column1 = snowflakeSqlGenerator.buildColumnId("column1")
        val column2 = snowflakeSqlGenerator.buildColumnId("column2")
        ...
        IntStream.range(3, 1025).forEach { i: Int ->
            columns[snowflakeSqlGenerator.buildColumnId("column$i")] = AirbyteProtocolType.STRING
        }
        ...
        transformer = SnowflakeSuperLimitationTransformer(parsedCatalog, "test_schema")
    }
    ...
    @Test
    @Throws(IOException::class)
    fun testSnowflakeSuperLimit_ShouldRemovePartialRecord() {
        ...
        Assertions.assertTrue(
            serialize(transformed.first!!).toByteArray(StandardCharsets.UTF_8).size <
                SnowflakeSuperLimitationTransformer.SNOWFLAKE_SUPER_MAX_BYTE_SIZE
        )
    ...
    @Test
    fun testSnowflakeSuperLimit_ShouldRemoveWholeRecord() {
        val testData: MutableMap<String, String> = HashMap()
        ...
        Assertions.assertTrue(
            serialize<AirbyteRecordMessageMeta>(transformed.second!!)
                ...
                .size < SnowflakeSuperLimitationTransformer.SNOWFLAKE_SUPER_MAX_BYTE_SIZE
        )
        Assertions.assertEquals(2, transformed.second!!.changes.size)
        // Assert that transformation added the change
    ...
    @Test
    fun testSnowflakeSuperLimit_ShouldFailOnPKMissing() {
        val testData: MutableMap<String, String> = HashMap()
    ...
    companion object {
        //private val snowflakeSqlGenerator = SnowflakeSqlGenerator(SnowflakeSQLNameTransformer(), false)
        private val snowflakeSqlGenerator = SnowflakeSqlGenerator(0)
    }
}
