...
public class SnowflakeTestUtils {
  ...
  public static List<JsonNode> dumpRawTable(JdbcDatabase database, String tableIdentifier)
      throws SQLException {
    return dumpTable(
        List.of(
  ...
  public static List<JsonNode> dumpFinalTable(
      JdbcDatabase database, String databaseName, String schema, String table) throws SQLException {
    // We have to discover the column names, because if we just SELECT * then snowflake will upcase
    // all column names.
    List<String> columns = database
        .queryJsons(
            """
        ...
        .map(column -> {
          ...
          return switch (type) {
            ...
            case "TIMESTAMP_NTZ", "TIMESTAMP_LTZ" -> "TO_VARCHAR(" + quotedName
                + ", 'YYYY-MM-DD\"T\"HH24:MI:SS.FF') as " + quotedName;
   ...
   */
  public static List<JsonNode> dumpTable(
      List<String> columns, JdbcDatabase database, String tableIdentifier) throws SQLException {
