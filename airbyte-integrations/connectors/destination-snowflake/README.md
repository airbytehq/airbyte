...
CREATE STORAGE INTEGRATION INTEGRATION_TEST_DESTINATION_GCS_INTEGRATION
  TYPE = EXTERNAL_STAGE
  STORAGE_PROVIDER = GCS
  ENABLED = TRUE
  STORAGE_ALLOWED_LOCATIONS = ('gcs://bucketname');

CREATE STAGE INTEGRATION_TEST_DESTINATION_GCS_STAGE
  url = 'gcs://bucketname'
  storage_integration = INTEGRATION_TEST_DESTINATION_GCS_INTEGRATION;
...
GRANT USAGE ON INTEGRATION INTEGRATION_TEST_DESTINATION_GCS_INTEGRATION TO ROLE INTEGRATION_TESTER_DESTINATION;
GRANT USAGE ON STAGE INTEGRATION_TEST_DESTINATION_GCS_STAGE TO ROLE INTEGRATION_TESTER_DESTINATION;
...
DESC STORAGE INTEGRATION INTEGRATION_TEST_DESTINATION_GCS_INTEGRATION;
```
...
DROP STAGE IF EXISTS INTEGRATION_TEST_DESTINATION_GCS_STAGE;
DROP INTEGRATION IF EXISTS INTEGRATION_TEST_DESTINATION_GCS_INTEGRATION;
```