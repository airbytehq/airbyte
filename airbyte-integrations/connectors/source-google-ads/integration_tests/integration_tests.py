    ...
    for key, value in data.items():
        ...
        if len(parts) > 1:
            ...
            if not hasattr(row, parent_field):
                setattr(row, parent_field,
                        row.__class__())  # Assuming the nested message type is the same as the parent. Adjust if different.
...
@pytest.mark.parametrize(
    ...
    [
        (
                ...
                [
                    {
                        "customer_label.resource_name": "123", "customer_label.customer": "customer",
                        "customer.id": 123, "customer_label.label": "customer_label"
                    },
                    {
                        "customer_label.resource_name": "1234", "customer_label.customer": "customer",
                        "customer.id": 123, "customer_label.label": "customer_label1"
                    }
                ]
        ),
        (
