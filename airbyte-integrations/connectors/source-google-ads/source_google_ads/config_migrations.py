...
class MigrateAuthType:
    """
    Backfills `credentials.auth_type` for configs that pre-date the OAuth /
    Service Account `oneOf` split.

    Existing configs only have OAuth fields (`client_id`, `client_secret`,
    `refresh_token`). The new spec requires the `auth_type` discriminator so the
    declarative `SelectiveAuthenticator` knows which branch to use. This
    migration sets `auth_type: "Client"` on legacy configs in place.
    """
    ...
    def update_config(cls, config: Mapping[str, Any]) -> Mapping[str, Any]:
        # mutate the config dict in place so the rest of the connector picks up the discriminator
