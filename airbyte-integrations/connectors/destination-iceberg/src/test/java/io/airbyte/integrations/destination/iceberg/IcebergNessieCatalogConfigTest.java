...
import io.airbyte.integrations.destination.iceberg.config.catalog.NessieCatalogConfig;
...
@Slf4j
class IcebergNessieCatalogConfigTest {
  ...
  private static final String FAKE_NESSIE_URI = "http://fake-nessie-uri";
  private static final String NESSIE_REF = "main";
  ...
  private AmazonS3 s3;
  private NessieCatalogConfig config;
  private Catalog catalog;
  private IcebergCatalogConfigFactory factory;

  @BeforeAll
  static void staticSetup() {
    IcebergNessieCatalogConfigTest.mockedIcebergGenerics = mockStatic(IcebergGenerics.class);
  }

  @AfterAll
  static void staticStop() {
    IcebergNessieCatalogConfigTest.mockedIcebergGenerics.close();
  }
  ...
  void setup() {
    ...
    JsonNode jsonNode = Jsons.jsonNode(ofEntries(entry(IcebergConstants.NESSIE_CATALOG_URI_CONFIG_KEY, FAKE_NESSIE_URI),
        entry(IcebergConstants.NESSIE_CATALOG_REFERENCE_KEY, NESSIE_REF),
        entry(IcebergConstants.NESSIE_CATALOG_TOKEN_CONFIG_KEY, FAKE_TOKEN)));

    config = new NessieCatalogConfig(jsonNode);
  ...
  @Test
  public void checksNessieServerUri() {
    ...
    assertThat(status.getMessage()).contains("nessie_uri is required");
  }

  @Test
  public void nessieCatalogSparkConfigTest() {
    Map<String, String> sparkConfig = config.sparkConfigMap();
    log.info("Spark Config for Nessie catalog: {}", sparkConfig);

    // Catalog config
    assertThat(sparkConfig.get("spark.sql.catalog.iceberg.catalog-impl")).isEqualTo(NessieCatalog.class.getName());
    assertThat(sparkConfig.get("spark.sql.catalog.iceberg.uri")).isEqualTo(FAKE_NESSIE_URI);
    assertThat(sparkConfig.get("spark.sql.catalog.iceberg.authentication.type")).isEqualTo("BEARER");
    assertThat(sparkConfig.get("spark.sql.catalog.iceberg.authentication.token")).isEqualTo(FAKE_TOKEN);
    assertThat(sparkConfig.get("spark.sql.catalog.iceberg.ref")).isEqualTo(NESSIE_REF);
  ...
  public void s3ConfigForCatalogInitializeTest() {
    ...
    log.info("S3 Config for NessieCatalog Initialize: {}", properties);
