...
import io.airbyte.integrations.destination.iceberg.container.NessieServerWithMinioCompose;
...
public abstract class BaseIcebergNessieCatalogS3IntegrationTest extends DestinationAcceptanceTest {

  private static final Logger LOGGER = LoggerFactory.getLogger(BaseIcebergNessieCatalogS3IntegrationTest.class);

  private static NessieServerWithMinioCompose composeContainer;
  private static JsonNode config;

  static void startCompose(final DataFileFormat fileFormat) {
    composeContainer = new NessieServerWithMinioCompose();
    composeContainer.start();
    config = composeContainer.getComposeConfig(fileFormat);
    IcebergIntegrationTestUtil.createS3WarehouseBucket(config);
    LOGGER.info("==> Started Nessie Server with Minio - Docker Compose...");
  }

  @AfterAll
  public static void stopCompose() {
    IcebergIntegrationTestUtil.stopAndCloseContainer(composeContainer, "Nessie Server with Minio - Docker Compose");
  }
