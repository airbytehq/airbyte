...
class SourceDentclinic(AbstractSource):
    ...
    def streams(self, config: Mapping[str, Any]) -> List[Stream]:
        ...
        return [BookingsFr(config=config, enable_days_back_limit=True), Bookings(config=config,enable_days_back_limit=False), Clinics(config=config),
                Services(config=config), Resources(config=config), PatientsReport(config=config), UtilizationReport(config=config, enable_days_back_limit=False), UtilizationReportFr(config=config, enable_days_back_limit=True)] 
