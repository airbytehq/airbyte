...
/** Base class for Db2 JDBC implementations of [PartitionReader]. */
@SuppressFBWarnings(value = ["NP_NONNULL_RETURN_VIOLATION"], justification = "lateinit usage")
sealed class Db2PartitionReader<P : JdbcPartition<*>>(
    val partition: P,
) : PartitionReader {
    ...
    fun out(row: SelectQuerier.ResultRow) {
        val db2StreamState = streamState as Db2JdbcStreamState
...
/** DB2 implementation of [PartitionReader] which reads the [partition] in its entirety. */
class Db2NonResumablePartitionReader<P : JdbcPartition<*>>(
    partition: P,
    val deleteQuerier: DeleteQuerier
) : Db2PartitionReader<P>(partition) {
    ...
    override suspend fun run() {
        ...
        if (
            partition is Db2JdbcCursorIncrementalPartition &&
