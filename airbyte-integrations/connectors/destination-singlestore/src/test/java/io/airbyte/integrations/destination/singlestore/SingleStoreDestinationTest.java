...
public class SingleStoreDestinationTest {
  ...
  private static final String EXPECTED_JDBC_URL = "jdbc:singlestore://localhost:3306/db?_connector_name=Airbyte Destination Connector";
  static final Map<String, String> SSL_JDBC_PARAMETERS = ImmutableMap.of("allowLocalInfile", "true", "sslMode", "trust");
  ...
  private static final ConfiguredAirbyteCatalog CATALOG =
      new ConfiguredAirbyteCatalog()
          .withStreams(List.of(CatalogHelpers.createConfiguredAirbyteStream(STREAM_NAME, NAMESPACE, Field.of("id", JsonSchemaType.NUMBER),
              Field.of("name", JsonSchemaType.BOOLEAN), Field.of("_ab_cdc_deleted_at", JsonSchemaType.STRING_TIMESTAMP_WITHOUT_TIMEZONE))));
  ...
  void testJdbcUrlAndConfigNoExtraParams() {
    final JsonNode jdbcConfig = new SingleStoreDestination().toJdbcConfig(buildConfigNoJdbcParameters());
    assertEquals(EXPECTED_JDBC_URL, jdbcConfig.get(JdbcUtils.JDBC_URL_KEY).asText());
  }

  @Test
  void testJdbcUrlEmptyExtraParams() {
    final JsonNode jdbcConfig = new SingleStoreDestination().toJdbcConfig(buildConfigWithExtraJdbcParameters(""));
    assertEquals(EXPECTED_JDBC_URL, jdbcConfig.get(JdbcUtils.JDBC_URL_KEY).asText());
  }
  ...
  void testJdbcUrlExtraParams() {
    ...
    final JsonNode jdbcConfig = new SingleStoreDestination().toJdbcConfig(buildConfigWithExtraJdbcParameters(extraParam));
    assertEquals(EXPECTED_JDBC_URL, jdbcConfig.get(JdbcUtils.JDBC_URL_KEY).asText());
  }

  @Test
  void testDefaultParamsNoSSL() {
    final Map<String, String> defaultProperties = new SingleStoreDestination().getDefaultConnectionProperties(
        buildConfigNoExtraJdbcParametersWithoutSsl());
    assertEquals(Map.of("allowLocalInfile", "true"), defaultProperties);
  }
