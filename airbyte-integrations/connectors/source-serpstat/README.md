...
to generate the necessary credentials. Then create a file `secrets/config.json` conforming to the `spec` inside `source_serpstat/manifest.yaml` file.
...
```
poetry run source-serpstat spec
poetry run source-serpstat check --config secrets/config.json
poetry run source-serpstat discover --config secrets/config.json
poetry run source-serpstat read --config secrets/config.json --catalog integration_tests/configured_catalog.json
```
