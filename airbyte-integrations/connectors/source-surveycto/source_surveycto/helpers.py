...
class Helpers(object):
    ...
    def call_survey_cto(config, form_id):
        ...
        url = (
            f"https://{server_name}.surveycto.com/" +
            f"api/v2/forms/data/wide/json/{form_id}?date={start_date}"
        )
        ...
        for data in response_json:
            try:
                ...
                data["starttime"] = datetime.strptime(
                    data["starttime"],
                    dateformat_in
                ).strftime(dateformat_out)
                data["endtime"] = datetime.strptime(
                    data["endtime"],
                    dateformat_in
                ).strftime(dateformat_out)
                data["CompletionDate"] = datetime.strptime(
                    data["CompletionDate"],
                    dateformat_in
                ).strftime(dateformat_out)
                data["SubmissionDate"] = datetime.strptime(
                    data["SubmissionDate"],
                    dateformat_in
                ).strftime(dateformat_out)
                yield data
