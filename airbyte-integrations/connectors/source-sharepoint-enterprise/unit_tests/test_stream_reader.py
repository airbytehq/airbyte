...
from source_sharepoint_enterprise.sharepoint_client import (
    SourceMicrosoftSharePointClient,
)
from source_sharepoint_enterprise.spec import SourceMicrosoftSharePointSpec
from source_sharepoint_enterprise.stream_reader import (
    FileReadMode,
...
def setup_client_class():
    ...
    with patch("source_sharepoint_enterprise.sharepoint_client.ConfidentialClientApplication") as mock_client_class:
...
@patch("source_sharepoint_enterprise.stream_reader.execute_query_with_retry")
@patch("source_sharepoint_enterprise.stream_reader.SourceMicrosoftSharePointStreamReader.filter_files_by_globs_and_start_date")
def test_get_matching_files(
    mock_filter_files,
    mock_execute_query,
    setup_reader_class,
    mock_drive_files,
    mock_shared_drive_files,
):
...
def test_open_file(mock_smart_open, file_extension, expected_compression):
    ...
    mock_file = Mock(
        download_url=f"https://example.com/file.{file_extension}",
...
@patch("source_sharepoint_enterprise.stream_reader.requests.get")
@patch("source_sharepoint_enterprise.stream_reader.requests.head")
def test_get_file(
    mock_requests_head,
    mock_requests_get,
...
@patch("source_sharepoint_enterprise.stream_reader.SourceMicrosoftSharePointStreamReader._list_directories_and_files")
...
@patch(
    "source_sharepoint_enterprise.stream_reader.execute_query_with_retry",
    side_effect=lambda x: x,
)
def test_get_shared_files_from_all_drives(
    mock_execute_query_with_retry,
    drive_ids,
    shared_drive_item_dicts,
    expected_result,
    expected_calls,
):
    stream_reader = SourceMicrosoftSharePointStreamReader()
    ...
    with patch.object(
        ...
        shared_drive_items = [
            MagicMock(
                remote_item=MagicMock(parentReference={"driveId": item["drive_id"]}),
                id=item["id"],
...
empty_subfolder_response = {
    "value": [],
    "name": "subfolder1",
}  # No files or folders inside subfolder1
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            MagicMock(
                status_code=400,
                json=MagicMock(return_value={"error": {"message": "Bad Request"}}),
            ),
            [],
            [],
            True,
...
def test_drives_property(auth_type, user_principal_name, has_refresh_token):
    with (
        patch("source_sharepoint_enterprise.stream_reader.execute_query_with_retry") as mock_execute_query,
        patch("source_sharepoint_enterprise.stream_reader.SourceMicrosoftSharePointStreamReader.one_drive_client") as mock_one_drive_client,
    ):
...
def test_get_site_drive_default_site():
    ...
    with patch("source_sharepoint_enterprise.stream_reader.execute_query_with_retry") as mock_execute_query:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            "Client",
            "ACCESSIBLE_DRIVES",
            ["_get_files_by_drive_name"],
        ),
        (
            "dummy_refresh_token",
            "Client",
            "ALL",
            ["_get_files_by_drive_name", "_get_shared_files_from_all_drives"],
        ),
...
def test_get_all_sites_returns_sites_successfully():
    ...
    with (
        patch("source_sharepoint_enterprise.sharepoint_base_reader.get_site_prefix") as mock_get_site_prefix,
        patch.object(reader, "_get_client_context") as mock_get_client_context,
        patch("source_sharepoint_enterprise.stream_reader.SearchService") as mock_search_service,
        patch("source_sharepoint_enterprise.stream_reader.execute_query_with_retry") as mock_execute_query,
    ):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            True,
            None,
        ),  # Case: search_job.value exists but PrimaryQueryResult is None
    ],
)
def test_get_all_sites_with_no_results(test_case, search_job_value, primary_query_result):
    ...
    with (
        ...
        mock_get_site_prefix.return_value = (
            "https://test-tenant.sharepoint.com",
            "test-tenant",
        )
