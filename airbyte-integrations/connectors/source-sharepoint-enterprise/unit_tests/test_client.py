...
import pytest
from source_sharepoint_enterprise.sharepoint_client import SourceMicrosoftSharePointClient
...
def test_client_property_initializes_graph_client_when_not_exists():
    ...
    with (
        patch("source_sharepoint_enterprise.sharepoint_client.ConfidentialClientApplication") as mock_confidential_client,
        patch("source_sharepoint_enterprise.sharepoint_client.GraphClient") as mock_graph_client,
    ):
...
def test_access_token_property():
    ...
    with (
        ...
        patch("source_sharepoint_enterprise.sharepoint_client.ConfidentialClientApplication"),
    ):
        mock_config = create_mock_config()
        client = SourceMicrosoftSharePointClient(mock_config)
...
def test_get_token_response_object_wrapper():
    ...
    with (
        patch.object(SourceMicrosoftSharePointClient, "_get_access_token") as mock_get_access_token,
        patch("source_sharepoint_enterprise.sharepoint_client.TokenResponse") as mock_token_response,
        patch("source_sharepoint_enterprise.sharepoint_client.ConfidentialClientApplication"),
    ):
        mock_config = create_mock_config()
        client = SourceMicrosoftSharePointClient(mock_config)
...
def test_client_property_raises_error_when_config_missing():
    ...
    with patch("source_sharepoint_enterprise.sharepoint_client.ConfidentialClientApplication"):
