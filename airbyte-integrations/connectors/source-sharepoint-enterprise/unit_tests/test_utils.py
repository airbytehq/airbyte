...
from source_sharepoint_enterprise.utils import (
...
def test_execute_query_with_retry(status_code, retry_after_header, expected_retries, error_message):
    ...
    with (
        patch("source_sharepoint_enterprise.utils.time.sleep") as mock_sleep,
        patch("source_sharepoint_enterprise.utils.datetime") as mock_datetime,
    ):
...
@patch("source_sharepoint_enterprise.utils.execute_query_with_retry")
