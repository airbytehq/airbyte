...
```
./gradlew :airbyte-integrations:connectors:source-opsgenie:airbyteDocker
```
...
To run unit tests:
```
./gradlew :airbyte-integrations:connectors:source-opsgenie:unitTest
```
To run acceptance and custom integration tests:
```
./gradlew :airbyte-integrations:connectors:source-opsgenie:integrationTest
```
...
Before committing code and raising PRs, make sure you run the linting process from the repo root to make sure that everything's ok:

```bash
./gradlew :airbyte-integrations:connectors:source-opsgenie:airbytePythonFormat
```
