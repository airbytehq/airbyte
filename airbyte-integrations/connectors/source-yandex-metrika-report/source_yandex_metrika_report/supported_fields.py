...
GROUP_VALUES = ["day", "week", "month", "quarter", "year"]
ATTRIBUTION_VALUES = [
    "first",
    "last",
    "lastsign",
...
def _format_attribution_field(field_name: str) -> list[str]:
    formatted_fields = []
    for attribution in ATTRIBUTION_VALUES:
        formatted_fields.append(field_name.replace("<attribution>", attribution))
    return formatted_fields


def _format_goal_id_field(field_name: str) -> list[str]:
    return [field_name.replace("<goal_id>", "\d+")]


def _format_group_field(field_name: str) -> list[str]:
    formatted_fields = []
    for group in GROUP_VALUES:
        formatted_fields.append(field_name.replace("<group>", group))
    return formatted_fields


def _format_experiment_ab_field(field_name: str) -> list[str]:
    return [field_name.replace("<experiment_ab>", "\d+")]


def _format_currency_field(field_name: str) -> list[str]:
    formatted_fields = []
    for currency in CURRENCY_VALUES:
        formatted_fields.append(field_name.replace("<currency>", currency))
    return formatted_fields


format_funcs = {
    "<attribution>": _format_attribution_field,
    "<goal_id>": _format_goal_id_field,
    "<group>": _format_group_field,
    "<experiment_ab>": _format_experiment_ab_field,
    "<currency>": _format_currency_field,
}


def _prepare_fields_list(fields_list):
    ...
    for field in fields_list:
        ...
        original_fields_list.append(field)
        for replace_key in format_funcs:
            format_func = format_funcs[replace_key]
            if replace_key in field_name:
                replaced_n += 1
                if isinstance(field, tuple):
    ...
    if replaced_n > 0:
        prepared_fields_list = _prepare_fields_list(prepared_fields_list)
    return (prepared_fields_list, original_fields_list)


def field_lookup(field_name: str, list_of_fields: list):
    if isinstance(list_of_fields[0], tuple):
        for pattern, pattern_field_type in list_of_fields:
            if re.match(pattern, field_name):
                return True, ("^" + pattern + "$", pattern_field_type)
    else:
        for pattern in list_of_fields:
            if re.match("^" + pattern + "$", field_name):
                return True, pattern


_GROUP_FIELDS = _prepare_fields_list(_RAW_GROUP_FIELDS)
_METRICS_FIELDS = _prepare_fields_list(_RAW_METRICS_FIELDS)

GROUP_FIELDS = _GROUP_FIELDS[0][1] + _GROUP_FIELDS[1]
