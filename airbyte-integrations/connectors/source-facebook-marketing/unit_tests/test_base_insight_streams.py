...
class TestBaseInsightsStream:
    def test_init(self, api, accounts):
        stream = AdsInsights(api=api, accounts=accounts, start_date=datetime(2010, 1, 1), end_date=datetime(2011, 1, 1),
    ...
    def test_init_override(self, api, accounts):
        stream = AdsInsights(
            api=api,
            accounts=accounts,
    ...
    def test_read_records_all(self, mocker, api, accounts):
    ...
    def test_read_records_random_order(self, mocker, api, accounts):
        """1. yield all from mock
        2. if read slice 2, 3 state not changed
            if read slice 2, 3, 1 state changed to 3
        """
        ...
        job.get_result.return_value = [mocker.Mock(), mocker.Mock(), mocker.Mock()]
        job.interval = pendulum.Period(pendulum.date(2010, 1, 1), pendulum.date(2010, 1, 1))
        stream = AdsInsights(api=api, accounts=accounts, start_date=datetime(2010, 1, 1), end_date=datetime(2011, 1, 1), insights_lookback_window=28)
    ...
    def test_state(self, api, accounts, state):
    ...
    def test_stream_slices_no_state(self, api, accounts, async_manager_mock, start_date):
        """Stream will use start_date when there is not state"""
        end_date = start_date + duration(weeks=2)
        stream = AdsInsights(api=api, accounts=accounts, start_date=start_date, end_date=end_date, insights_lookback_window=28)
    ...
    def test_stream_slices_no_state_close_to_now(self, api, accounts, async_manager_mock, recent_start_date):
    ...
    def test_stream_slices_with_state(self, api, accounts, async_manager_mock, start_date):
    ...
    def test_stream_slices_with_state_close_to_now(self, api, accounts, async_manager_mock, recent_start_date):
    ...
    def test_stream_slices_with_state_and_slices(self, api, accounts, async_manager_mock, start_date):
    ...
    def test_get_json_schema(self, api, accounts):
        stream = AdsInsights(api=api, accounts=accounts, start_date=datetime(2010, 1, 1), end_date=datetime(2011, 1, 1), insights_lookback_window=28)
    ...
    def test_get_json_schema_custom(self, api, accounts):
        stream = AdsInsights(
            api=api,
            accounts=accounts,
            start_date=datetime(2010, 1, 1),
    ...
    def test_fields(self, api, accounts):
        stream = AdsInsights(
            api=api,
            accounts=accounts,
            start_date=datetime(2010, 1, 1),
    ...
    def test_fields_custom(self, api, accounts):
        stream = AdsInsights(
            api=api,
            accounts=accounts,
            start_date=datetime(2010, 1, 1),
    ...
    def test_level_custom(self, api, accounts):
        stream = AdsInsights(
            api=api,
            accounts=accounts,
            start_date=datetime(2010, 1, 1),
