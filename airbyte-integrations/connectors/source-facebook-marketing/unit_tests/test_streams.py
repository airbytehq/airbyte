...
def test_filter_all_statuses(source, api, mocker):
    ...
    assert FBMarketingStream(source=source, api=api)._filter_all_statuses() == expected
...
def test_ads_insights_breakdowns(api, class_name, breakdowns, action_breakdowns):
    kwargs = {"source": None, "api": api, "start_date": pendulum.now(
