...
class TestMetricsStream(TestCase):
    ...
    def test_pagination_multiple_pages(self, http_mocker: HttpMocker):
        ...
        http_mocker.get(
            KlaviyoRequestBuilder.from_url("https://a.klaviyo.com/api/metrics?page[cursor]=abc123", _API_KEY).build(),
            KlaviyoPaginatedResponseBuilder()
            .with_records(
                [
                    {
    ...
    def test_unauthorized_401_error_fails(self, http_mocker: HttpMocker):
        ...
        http_mocker.get(
            KlaviyoRequestBuilder.metrics_endpoint("invalid_key").with_any_query_params().build(),
            HttpResponse(
