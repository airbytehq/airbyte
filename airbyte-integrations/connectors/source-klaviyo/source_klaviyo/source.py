...
from source_klaviyo.streams import BadFlows, BadLists, Campaigns, EmailTemplates, Events, Flows, GlobalExclusions, Lists, Metrics, Profiles
...
class SourceKlaviyo(AbstractSource):
    ...
    def streams(self, config: Mapping[str, Any]) -> List[Stream]:
        ...
        return [
            ...
            BadLists(api_key=api_key, start_date=start_date),
            Metrics(api_key=api_key, start_date=start_date),
            Flows(api_key=api_key, start_date=start_date),
            BadFlows(api_key=api_key, start_date=start_date),
