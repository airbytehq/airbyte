...
Klaviyo's Flow Series Reports and Campaign Values Reports endpoints each allow 225 requests per day on top of the XS burst and steady limits. The manifest paces each endpoint independently at 1 request per `PT6M24S` (about 225 requests per day per endpoint, matching the documented quota), which is why large reporting backfills run slowly. A raw daily (`P1D`) bucket is deliberately not used because the limiter blocks with `time.sleep()`, which would allow a burst followed by hours of sleep. Pacing targets the full documented quota rather than leaving headroom, so a request can land just before a quota refresh boundary; this was a deliberate trade-off for throughput. Reporting requests send `Connection: close` because Klaviyo's edge drops idle keep-alive connections long before the 384s pacing gap (verified by `RemoteDisconnected` after about 400s idle). The reporting error handler allows `max_time: 1800` so a retry is not guillotined by a single pacing wait.
