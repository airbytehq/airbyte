...
from typing import Any, Final, Iterable, List, Mapping, MutableMapping, Optional, Union
...
class NetsuiteODBCStream(Stream):
    def get_primary_key_from_stream(self, stream):
        ...
        if not key:
            ...
            raise Exception(
                "Assumption Broken: Primary Key should always be a top level field.  Instead, we're receiving a sub-field as a primary key."
            )
        else:
            return key[0]
    ...
    def read_records(
        ...
        while True:
            ...
            # if number of rows >= page limit, we need to fetch another page
                ...
                self.logger.info(
                    f"Fetching another page for the stream {self.table_name}.  Last seen ID is {self.cursor_value_last_id_seen}"
                )
                number_records = 0
                self.cursor.execute(self.generate_ordered_query(stream_slice))
                continue
    ...
    def get_updated_state(self, current_stream_state: MutableMapping[str, Any], latest_record: Mapping[str, Any]) -> Mapping[str, Any]:
        ...
        return {"last_id_seen": last_id_seen, "last_date_updated": last_date_updated}
