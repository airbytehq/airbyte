...
class NetsuiteODBCTableDiscoverer:
    ...
    def get_table_stream(self, table: json) -> AirbyteStream:
        ...
        if table_incremental_column is not None:
            return AirbyteStream(
                ...
                default_cursor_field=[table_incremental_column],
                primary_key=primary_key_column,
            )
        else:
            return AirbyteStream(
                name=table_name, json_schema=json_schema, supported_sync_modes=["full_refresh"], primary_key=primary_key_column
            )
    ...
    def is_column_incremental(self, column: json) -> bool:
        ...
        if (column_meta_data is not None and "M-" in column_meta_data) or column_name in [
            "datelastmodified",
            "lastmodified",
            "lastmodifieddate",
        ]:
