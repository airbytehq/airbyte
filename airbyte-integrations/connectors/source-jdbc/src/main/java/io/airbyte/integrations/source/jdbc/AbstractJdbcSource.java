 ...
 */
public abstract class AbstractJdbcSource<Datatype> extends AbstractDbSource<Datatype, JdbcDatabase>
    implements Source {
  ...
  @Override
  protected AutoCloseableIterator<JsonNode> queryTableFullRefresh(
    ...
    if (syncMode.equals(SyncMode.INCREMENTAL) && getStateEmissionFrequency() > 0) {
      ...
      // If we are in FULL_REFRESH mode, state messages are never emitted, so we don't care about
      // ordering
      // of the records.
      return queryTable(
          database,
  ...
  protected List<CheckedConsumer<JdbcDatabase, Exception>> getCheckOperations(final JsonNode config)
    ...
    return ImmutableList.of(database -> {
      ...
      database.bufferedResultSetQuery(
          connection -> connection.getMetaData().getCatalogs(), sourceOperations::rowToJson);
    });
  }
  ...
  private String getCatalog(final SqlDatabase database) {
    return (database.getSourceConfig().has(JdbcUtils.DATABASE_KEY)
        ? database.getSourceConfig().get(JdbcUtils.DATABASE_KEY).asText()
        : null);
  }

  @Override
  protected List<TableInfo<CommonField<Datatype>>> discoverInternal(
      final JdbcDatabase database, final String schema) throws Exception {
    ...
    final Set<JdbcPrivilegeDto> tablesWithSelectGrantPrivilege =
        getPrivilegesTableForCurrentUser(database, schema);
    return database
        .bufferedResultSetQuery(
            // retrieve column metadata from the database
            connection ->
                connection.getMetaData().getColumns(getCatalog(database), schema, null, null),
            // store essential column metadata to a Json object from the result set about each
            // column
            this::getColumnMetadata)
        .stream()
        .filter(excludeNotAccessibleTables(internalSchemas, tablesWithSelectGrantPrivilege))
        // group by schema and table name to handle the case where a table with the same name exists
        // in
        // multiple schemas.
        .collect(Collectors.groupingBy(t -> ImmutablePair.of(
            t.get(INTERNAL_SCHEMA_NAME).asText(), t.get(INTERNAL_TABLE_NAME).asText())))
        .values()
        .stream()
            ...
            .fields(fields.stream()
                ...
                .map(f -> {
                  ...
                  LOGGER.debug(
                      ...
                      jsonType);
                  return new CommonField<Datatype>(
                      f.get(INTERNAL_COLUMN_NAME).asText(), datatype) {};
                })
  ...
  protected Predicate<JsonNode> excludeNotAccessibleTables(
    ...
    return jsonNode -> {
      ...
      return tablesWithSelectGrantPrivilege.stream().anyMatch(e -> e.getSchemaName()
              .equals(jsonNode.get(INTERNAL_SCHEMA_NAME).asText()))
          && tablesWithSelectGrantPrivilege.stream().anyMatch(e -> e.getTableName()
              .equals(jsonNode.get(INTERNAL_TABLE_NAME).asText()))
  ...
  private JsonNode getColumnMetadata(final ResultSet resultSet) throws SQLException {
    final var fieldMap = ImmutableMap.<String, Object>builder()
        ...
        .put(
            ...
            resultSet.getObject(JDBC_COLUMN_SCHEMA_NAME) != null
  ...
  @VisibleForTesting
  public record PrimaryKeyAttributesFromDb(String streamName, String primaryKey, int keySequence) {}

  @Override
  protected Map<String, List<String>> discoverPrimaryKeys(
      final JdbcDatabase database, final List<TableInfo<CommonField<Datatype>>> tableInfos) {
    LOGGER.info("Discover primary keys for tables: "
        + tableInfos.stream().map(TableInfo::getName).collect(Collectors.toSet()));
    try {
      // Get all primary keys without specifying a table name
      final Map<String, List<String>> tablePrimaryKeys =
          aggregatePrimateKeys(database.bufferedResultSetQuery(
              connection ->
                  connection.getMetaData().getPrimaryKeys(getCatalog(database), null, null),
              r -> {
                final String schemaName = r.getObject(JDBC_COLUMN_SCHEMA_NAME) != null
                ...
                final String streamName = JdbcUtils.getFullyQualifiedTableName(
                    schemaName, r.getString(JDBC_COLUMN_TABLE_NAME));
    ...
    return tableInfos.stream()
        .collect(Collectors.toMap(
            tableInfo ->
                JdbcUtils.getFullyQualifiedTableName(tableInfo.getNameSpace(), tableInfo.getName()),
            tableInfo -> {
              final String streamName = JdbcUtils.getFullyQualifiedTableName(
                ...
                final Map<String, List<String>> primaryKeys =
                    aggregatePrimateKeys(database.bufferedResultSetQuery(
                        connection -> connection
                            ...
                            .getPrimaryKeys(
                                getCatalog(database),
                                tableInfo.getNameSpace(),
                                tableInfo.getName()),
                        r -> new PrimaryKeyAttributesFromDb(
                            streamName, r.getString(JDBC_COLUMN_COLUMN_NAME), r.getInt(KEY_SEQ))));
          ...
          try {
            final Stream<JsonNode> stream = database.unsafeQuery(
                connection -> {
                  ...
                  final String fullTableName = getFullyQualifiedTableNameWithQuoting(
                      schemaName, tableName, getQuoteString());
                  final String quotedCursorField =
                      enquoteIdentifier(cursorInfo.getCursorField(), getQuoteString());
                  ...
                  if (cursorInfo.getCursorRecordCount() <= 0L) {
                    ...
                    LOGGER.info(
                        ...
                        tableName,
                        cursorInfo.getCursorRecordCount(),
                        actualRecordCount);
                  ...
                  final String wrappedColumnNames = getWrappedColumnNames(
                      database, connection, columnNames, schemaName, tableName);
                  final StringBuilder sql = new StringBuilder(String.format(
                      "SELECT %s FROM %s WHERE %s %s ?",
                      wrappedColumnNames, fullTableName, quotedCursorField, operator));
                  // if the connector emits intermediate states, the incremental query must be
                  // sorted by the cursor
                  // field
                  ...
                  sourceOperations.setCursorField(
                      preparedStatement, 1, cursorFieldType, cursorInfo.getCursor());
   ...
   */
  protected String getWrappedColumnNames(
      final JdbcDatabase database,
  ...
  protected long getActualCursorRecordCount(
      final Connection connection,
    ...
    if (cursor == null) {
      final String cursorRecordQuery = String.format(
          ...
          columnName, fullTableName, quotedCursorField);
   ...
   */
  @Override
  protected void logPreSyncDebugData(
      final JdbcDatabase database, final ConfiguredAirbyteCatalog catalog) throws SQLException {
    LOGGER.info(
  ...
  protected List<ConfiguredAirbyteStream> identifyStreamsToSnapshot(
    ...
    final Set<AirbyteStreamNameNamespacePair> alreadySyncedStreams =
        stateManager.getCdcStateManager().getInitialStreamsSynced();
    if (alreadySyncedStreams.isEmpty()
