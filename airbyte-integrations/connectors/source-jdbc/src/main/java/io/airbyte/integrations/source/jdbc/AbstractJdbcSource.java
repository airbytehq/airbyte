 ...
 */
public abstract class AbstractJdbcSource extends AbstractRelationalDbSource<JDBCType, JdbcDatabase>
    implements Source {
    ...
    return ImmutableList.of(
        database -> {
          ...
          database.bufferedResultSetQuery(
              conn -> conn.getMetaData().getCatalogs(), sourceOperations::rowToJson);
        });
  }
   ...
   */
  private static Map<String, List<String>> aggregatePrimateKeys(
      final List<SimpleImmutableEntry<String, String>> entries) {
  ...
  private String getCatalog(final SqlDatabase database) {
    return (database.getSourceConfig().has("database")
        ? database.getSourceConfig().get("database").asText()
        : null);
  }

  @Override
  protected List<TableInfo<CommonField<JDBCType>>> discoverInternal(
    ...
    return database
        ...
        .map(
            fields ->
                TableInfo.<CommonField<JDBCType>>builder()
                        ...
                        fields.stream()
                            .map(
                                f -> {
                                  ...
                                  try {
                                    jdbcType =
                                        JDBCType.valueOf(f.get(INTERNAL_COLUMN_TYPE).asInt());
                                  ...
                                  return new CommonField<JDBCType>(
                                      f.get(INTERNAL_COLUMN_NAME).asText(), jdbcType) {};
                                })
  ...
  protected Map<String, List<String>> discoverPrimaryKeys(
      final JdbcDatabase database, final List<TableInfo<CommonField<JDBCType>>> tableInfos) {
    LOGGER.info(
        "Discover primary keys for tables: "
            + tableInfos.stream().map(tab -> tab.getName()).collect(Collectors.toSet()));
    try {
              ...
              database.bufferedResultSetQuery(
                  ...
                  r -> {
                    final String schemaName =
                        r.getObject(JDBC_COLUMN_SCHEMA_NAME) != null
                            ? r.getString(JDBC_COLUMN_SCHEMA_NAME)
                            : r.getString(JDBC_COLUMN_DATABASE_NAME);
                    final String streamName =
                        sourceOperations.getFullyQualifiedTableName(
                            ...
                            database.bufferedResultSetQuery(
                                ...
                                r ->
                                    new SimpleImmutableEntry<>(
                                        streamName, r.getString(JDBC_COLUMN_COLUMN_NAME))));
                    ...
                    connection -> {
                      ...
                      final String sql =
                          String.format(
                              ...
                              sourceOperations.getFullyQualifiedTableNameWithQuoting(
                                  connection, schemaName, tableName),
                              sourceOperations.enquoteIdentifier(connection, cursorField));

                      final PreparedStatement preparedStatement = connection.prepareStatement(sql);
                      sourceOperations.setStatementField(
                          preparedStatement, 1, cursorFieldType, cursor);
  ...
  public JdbcDatabase createDatabase(final JsonNode config) throws SQLException {
    ...
    final JdbcDatabase database =
        Databases.createStreamingJdbcDatabase(
            jdbcConfig.get("username").asText(),
            jdbcConfig.has("password") ? jdbcConfig.get("password").asText() : null,
            jdbcConfig.get("jdbc_url").asText(),
            driverClass,
            jdbcStreamingQueryConfiguration,
            jdbcConfig.has("connection_properties")
                ? jdbcConfig.get("connection_properties").asText()
                : null,
            getSourceOperations());
