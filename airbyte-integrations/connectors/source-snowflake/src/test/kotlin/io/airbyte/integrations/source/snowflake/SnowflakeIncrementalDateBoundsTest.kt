...
class SnowflakeIncrementalDateBoundsTest : SnowflakeOperationsBaseTest() {
    ...
    fun testBothDatesAddedToRegularQueryWithExistingCursorBounds() {
        ...
        val existingWhere =
            Where(
                And(
                    ...
                    LesserOrEqual(cursorField, LocalDateCodec.encode(LocalDate.parse("2024-09-01"))),
                )
            )
        ...
        assert(query.sql.contains(""""created_at" > ?""")) { "Expected existing cursor lower bound" }
        assert(query.sql.contains(""""created_at" >= ?""")) { "Expected startDate condition" }
        assertEquals(2, query.sql.split(""""created_at" <= ?""").size - 1, "Expected two <= conditions")
    }
    ...
    fun testBothDatesAddedToSamplingQuery() {
        ...
        val innerWhere =
            Where(
                And(
                    ...
                    LesserOrEqual(cursorField, LocalDateCodec.encode(LocalDate.parse("2024-06-01"))),
                )
            )
        ...
        assertEquals(2, query.sql.split(""""event_date" >= ?""").size - 1, "startDate in both inner and outer WHERE")
        assertEquals(4, query.sql.split(""""event_date" <= ?""").size - 1, "endDate (<=) in cursor bounds + date bounds, inner and outer")
    }

    // ---- guard: full_refresh_temporal_column must not affect incremental queries ----

    /**
     * Configuring full_refresh_temporal_column must have zero effect on incremental queries.
     * The cursor is always detected via ORDER BY or the SELECT MAX cache — the temporal column
     * lookup (step 3 in applyDateBounds) must never be reached for incremental streams.
     *
    ...
    /**
     * Same guard for PK-ordered incremental reads (cursor cached via SELECT MAX).
     * The cache (step 2) must take priority over the temporal column check (step 3).
     */
    @Test
