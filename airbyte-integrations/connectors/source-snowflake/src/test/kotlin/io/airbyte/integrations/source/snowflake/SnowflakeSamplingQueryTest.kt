...
class SnowflakeSamplingQueryTest {
    ...
    fun testIncrementalPartitionSamplingQuery() {
            ...
            SelectQuerySpec(
                ...
                from =
                    FromSample(
                        ...
                        namespace = "POLARIS",
                        sampleRateInvPow2 = 16, // 1/65536 sample rate as in the issue
                        sampleSize = 1024,
                        where = whereClause
                    ),
                where =
                    whereClause, // This would also be in the outer query for the actual data read
        ...
        assertEquals(expectedSql, query.sql)
        assertEquals(2, query.bindings.size) // Only outer WHERE contributes bindings
    }
