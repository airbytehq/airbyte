...
class SnowflakeSourceConfigurationTest {
    ...
    private fun baseSpec() =
        SnowflakeSourceConfigurationSpecification().apply {
            ...
            database = "TEST_DATABASE"
            credentials =
                UsernamePasswordCredentialsSpecification(username = "testuser", password = "testpass")
        }

    @Test
    fun testValidStartDate() {
        val config = factory.makeWithoutExceptionHandling(baseSpec().apply { startDate = "2024-01-01" })
        assertEquals("2024-01-01", config.startDate)
    }

    @Test
    fun testValidEndDate() {
        val config = factory.makeWithoutExceptionHandling(baseSpec().apply { endDate = "2024-12-31" })
