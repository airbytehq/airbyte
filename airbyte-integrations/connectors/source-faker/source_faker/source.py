...
class SourceFaker(ConcurrentSourceAdapter):
    ...
    def streams(self, config: Mapping[str, Any]) -> List[Stream]:
        ...
        users_stream = self._wrap_for_concurrency(
            Users(self.message_repository, count, seed, parallelism, records_per_slice, always_updated), seed
        )
        contacts_stream = create_canonical_stream_facade(
            users_stream._abstract_stream,
            Contact,
            UserToContact(),
            CanonicalFakerCursor(stream_name=users_stream.name, stream_namespace=users_stream.namespace, message_repository=self.message_repository, seed=seed),
            logger,
            self._slice_logger,
        )
        streams: List[Stream] = [
...
class CanonicalFakerCursor(FakerCursor):
    def __init__(self, stream_name: str, stream_namespace: Optional[str], message_repository: MessageRepository, seed: int | None) -> None:
        super().__init__(stream_name, stream_namespace, message_repository, seed)
