...
def test_read_big_random_data():
    source = SourceFaker()
    config = {"count": 1000, "records_per_slice": 100, "records_per_sync": 1000, "threads": 1}
    catalog = ConfiguredAirbyteCatalog(
        streams=[
            {
                "stream": {"name": "users", "json_schema": {}, "supported_sync_modes": ["incremental"]},
...
def test_with_purchases():
    source = SourceFaker()
    config = {"count": 1000, "records_per_sync": 1000, "threads": 1}
    catalog = ConfiguredAirbyteCatalog(
        streams=[
            {
                "stream": {"name": "users", "json_schema": {}, "supported_sync_modes": ["incremental"]},
...
def test_sync_ends_with_limit():
    source = SourceFaker()
    config = {"count": 100, "records_per_sync": 5, "threads": 1}
    catalog = ConfiguredAirbyteCatalog(
        streams=[
            {
                "stream": {"name": "users", "json_schema": {}, "supported_sync_modes": ["incremental"]},
...
def test_read_with_seed():
    ...
    source = SourceFaker()
    config = {"count": 1, "seed": 100, "threads": 1}
    catalog = ConfiguredAirbyteCatalog(
        streams=[
            {
                "stream": {"name": "users", "json_schema": {}, "supported_sync_modes": ["incremental"]},
    ...
    assert records[0].record.data["occupation"] == "Illustrator"
    assert records[0].record.data["email"] == "upcoming1928@protonmail.com"


def test_ensure_no_purchases_without_users():
    with pytest.raises(ValueError):
        source = SourceFaker()
        config = {"count": 100, "threads": 1}
        catalog = ConfiguredAirbyteCatalog(
            streams=[
