...
class SourceFtp(AbstractSource):
    ...
    def _infer_json_schema(
        ...
        if len(files) == 0:
            logger.warning(
                f"No files found in folder {config['folder_path']} with pattern {file_pattern}"
            )
        ...
        df = connection.fetch_file(
            ...
            autogenerate_column_names=config.get("autogenerate_column_names"),
            autogenerated_column_names_prefix=config.get(
                "autogenerated_column_names_prefix"
            ),
            column_names_separator=config.get("column_names_separator"),
        )
        ...
        if len(df) < 1:
            logger.warning(
                f"No records found in file {files[0]}, can't infer json schema"
            )
            return self._default_json_schema
    ...
    ) -> Tuple[bool, AirbyteConnectionStatus]:
        try:
            ...
            return (
                False,
                AirbyteConnectionStatus(
                    status=Status.FAILED, message=f"An exception occurred: {str(ex)}"
                ),
            )
