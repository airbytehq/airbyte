...
def encrypt_test_file_fixture(gpg_keyring, config):
    ...
    test_content = """string_col,int_col
        test1,1
        test2,2
        """
    ...
    with open(temp_file, "rb") as f:
        gpg_keyring["gpg"].encrypt_file(f, recipients=[gpg_keyring["fingerprint"]], output=encrypted_file)
...
def config_gpg_fixture(config, gpg_keyring):
    ...
    gpg_config["streams"] = [{"name": "gpg_test_stream", "globs": ["**/*.gpg"], "format": {"filetype": "csv"}}]
...
def test_gpg_get_file_with_gpg_decryption(configured_catalog, config_gpg, encrypt_test_file):
    ...
    config_file_transfer["delivery_method"] = {"delivery_type": "use_file_transfer", "preserve_directory_structure": True}
