...
class TrustpilotStream(HttpStream, ABC):
    ...
    def __init__(self, api_key: Optional[str] = None, authenticator: AuthBase = None, business_unit_names: Optional[List[str]] = None):
...
class TrustpilotIncrementalStream(TrustpilotPaginagedStream, ABC):
    ...
    def __init__(self, start_date: Optional[datetime] = None, **kargs):
...
class PrivateReviews(TrustpilotIncrementalStream):
    ...
    def _clean_row(self, row: Mapping[str, Any]):
        ...
        # remove nested 'links'
        if "consumer" in row and "links" in row["consumer"]:
            del row["consumer"]["links"]
        if "businessUnit" in row and "links" in row["businessUnit"]:
