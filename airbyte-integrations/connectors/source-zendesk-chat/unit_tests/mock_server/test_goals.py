...
def _goal_record(goal_id: int, name: str):
    return {
        "id": goal_id,
        "name": name,
...
class TestGoalsStream(TestCase):
    @HttpMocker()
    def test_full_refresh_returns_records(self, http_mocker: HttpMocker):
        """Test that goals stream returns array of goal records."""
        records = [
            _goal_record(1, "Conversion Goal"),
            _goal_record(2, "Engagement Goal"),
        ]
        http_mocker.get(
            HttpRequest(f"https://{_SUBDOMAIN}.zendesk.com/api/v2/chat/goals"),
