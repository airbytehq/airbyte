...
def _role_record(role_id: int, name: str):
    return {
        "id": role_id,
        "name": name,
...
class TestRolesStream(TestCase):
    ...
    def test_full_refresh_returns_records(self, http_mocker: HttpMocker):
        """Test that roles stream returns array of role records."""
        records = [
            _role_record(1, "Administrator"),
            _role_record(2, "Agent"),
        ]
        http_mocker.get(
            HttpRequest(f"https://{_SUBDOMAIN}.zendesk.com/api/v2/chat/roles"),
