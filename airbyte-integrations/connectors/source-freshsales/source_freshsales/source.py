...
class FreshsalesStream(HttpStream, ABC):
    ...
    `class FreshsalesStream(HttpStream, ABC)` which is the current class
    `class Customers(FreshsalesStream)` contains behavior to pull data for customers using v1/customers
    `class Employees(FreshsalesStream)` contains behavior to pull data for employees using v1/employees

    If some streams implement incremental sync, it is typical to create another class
    `class IncrementalFreshsalesStream((FreshsalesStream), ABC)` then have concrete stream implementations extend it. An example
    is provided below.
