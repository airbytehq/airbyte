...
```
./gradlew :airbyte-integrations:connectors:source-freshsales:airbyteDocker
```
...
```
docker build . --no-cache -t airbyte/source-freshsales:dev \
...
To run unit tests:
```
./gradlew :airbyte-integrations:connectors:source-freshsales:unitTest
```
