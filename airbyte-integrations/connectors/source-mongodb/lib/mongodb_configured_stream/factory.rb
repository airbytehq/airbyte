...
class MongodbConfiguredStream::Factory
  def self.build(configured_stream:, state:)
    case configured_stream['sync_mode']
    when SyncMode::FullRefresh
      MongodbConfiguredStream::FullRefresh.new(configured_stream: configured_stream, state: state)
    when SyncMode::Incremental
      MongodbConfiguredStream::Incremental.new(configured_stream: configured_stream, state: state)
    else
