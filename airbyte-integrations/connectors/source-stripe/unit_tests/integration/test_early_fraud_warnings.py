...
def _early_fraud_warnings_request() -> StripeRequestBuilder:
    return StripeRequestBuilder.radar_early_fraud_warnings_endpoint(
        _ACCOUNT_ID, _CLIENT_SECRET
    )
...
def _given_early_fraud_warnings_availability_check(http_mocker: HttpMocker) -> None:
    http_mocker.get(
        StripeRequestBuilder.radar_early_fraud_warnings_endpoint(
            _ACCOUNT_ID, _CLIENT_SECRET
        )
...
class FullRefreshTest(TestCase):
    ...
    def test_given_one_page_when_read_then_return_records(
        ...
        http_mocker.get(
            ...
            _early_fraud_warnings_response()
            .with_record(_an_early_fraud_warning())
            .with_record(_an_early_fraud_warning())
            .build(),
        )
    ...
    def test_given_many_pages_when_read_then_return_records(
        ...
        http_mocker.get(
            ...
            .with_pagination()
            .with_record(
                _an_early_fraud_warning().with_id("last_record_id_from_first_page")
            )
            .build(),
        )
        http_mocker.get(
            _early_fraud_warnings_request()
...
class IncrementalTest(TestCase):
    ...
    @HttpMocker()
    def test_given_no_state_when_read_then_use_early_fraud_warnings_endpoint(
        self, http_mocker: HttpMocker
    ) -> None:
        _given_events_availability_check(http_mocker)
        ...
        http_mocker.get(
            _early_fraud_warnings_request().with_limit(100).build(),
            _early_fraud_warnings_response()
            .with_record(_an_early_fraud_warning().with_cursor(cursor_value))
            .build(),
        )
    ...
    def test_given_state_when_read_then_query_events_using_types_and_state_value_plus_1(
        ...
        http_mocker.get(
            ...
            .with_record(
                ...
                .with_field(_DATA_FIELD, _an_early_fraud_warning().build())
            )
            .build(),
        )
    ...
    def test_given_state_and_pagination_when_read_then_return_records(
        ...
        http_mocker.get(
            ...
            .build(),
            _events_response()
            .with_record(self._an_early_fraud_warning_event())
            .build(),
        )
