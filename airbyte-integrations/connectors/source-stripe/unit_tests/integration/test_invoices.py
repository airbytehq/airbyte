...
def _invoices_request() -> StripeRequestBuilder:
    return StripeRequestBuilder.invoices_endpoint(_ACCOUNT_ID, _CLIENT_SECRET)
...
class FullRefreshTest(TestCase):
    ...
    def test_given_one_page_when_read_then_return_records(self, http_mocker: HttpMocker) -> None:
        http_mocker.get(
            _invoices_request().with_created_gte(_A_START_DATE).with_created_lte(_NOW).with_limit(100).build(),
            _invoices_response().with_record(_an_invoice()).with_record(_an_invoice()).build(),
        )
