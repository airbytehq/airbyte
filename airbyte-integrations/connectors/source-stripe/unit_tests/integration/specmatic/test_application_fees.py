...
class ApplicationFeesSpecmaticTest(SpecmaticIntegrationTestCase):
    """
    Specmatic-backed contract integration tests for GET /v1/application_fees.
    """
    ...
    def test_specmatic_contract_read(self) -> None:
        """Zero-Hardcoding contract test for application_fees stream against Specmatic mock server."""
    ...
    def test_given_one_page_when_read_then_return_records(self) -> None:
        """Zero-Hardcoding spec-driven read test for application_fees."""
        now = datetime.now(timezone.utc)
        self.source = get_source(_CONFIG, _NO_STATE)
        output = self._read(_config(now))
        self.assert_contract_read_success(output)
