...
class ExternalAccountBankAccountsSpecmaticTest(SpecmaticIntegrationTestCase):
    """
    Specmatic-backed contract integration tests for external_account_bank_accounts.
    """
    ...
    def test_specmatic_contract_read(self) -> None:
        """Zero-Hardcoding contract test for external_account_bank_accounts stream against Specmatic mock server."""
    ...
    def test_given_one_page_when_read_then_return_records(self) -> None:
        """Zero-Hardcoding spec-driven read test for external_account_bank_accounts."""
        now = datetime.now(timezone.utc)
        self.source = get_source(_CONFIG, _NO_STATE)
        output = self._read(_config(now))
        self.assert_contract_read_success(output)
