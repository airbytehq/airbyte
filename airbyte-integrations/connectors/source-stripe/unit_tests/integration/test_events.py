...
from airbyte_cdk.test.mock_http.response_builder import (
    FieldPath, HttpResponseBuilder, RecordBuilder, create_record_builder,
    create_response_builder, find_template)
...
def _config() -> ConfigBuilder:
    return (
        ConfigBuilder()
        .with_start_date(_NOW - timedelta(days=73))
...
class FullRefreshTest(TestCase):
    ...
    def test_given_many_pages_when_read_then_return_records(
        ...
        http_mocker.get(
            ...
            _a_response()
            .with_pagination()
            .with_record(_a_record().with_id("last_record_id_from_first_page"))
            .build(),
        )
        http_mocker.get(
            _a_request()
    ...
    def test_given_start_date_before_30_days_stripe_limit_and_slice_range_when_read_then_perform_request_before_30_days(
        ...
        http_mocker.get(
            _a_request()
            .with_created_gte(slice_datetime + slice_range + _THIRD_REQUEST)
            .with_created_lte(_NOW)
            .with_limit(100)
            .build(),
            _a_response().build(),
    ...
    @HttpMocker()
    def test_given_lookback_window_when_read_then_request_before_start_date(
        self, http_mocker: HttpMocker
    ) -> None:
        ...
        http_mocker.get(
            _a_request()
            .with_created_gte(start_date - lookback_window)
            .with_created_lte(_NOW)
            .with_limit(100)
            .build(),
            _a_response().build(),
        ...
        self._read(
            _config()
            .with_start_date(start_date)
            .with_lookback_window_in_days(lookback_window.days)
        )
    ...
    @HttpMocker()
    def test_given_http_status_401_when_read_then_stream_is_incomplete(
        self, http_mocker: HttpMocker
    ) -> None:
        http_mocker.get(
            _a_request().with_any_query_params().build(),
        ...
        output = self._read(
            _config().with_start_date(_A_START_DATE), expecting_exception=True
        )
    ...
    @HttpMocker()
    def test_when_read_then_validate_availability_for_full_refresh_and_incremental(
        self, http_mocker: HttpMocker
    ) -> None:
...
class IncrementalTest(TestCase):
    ...
    @HttpMocker()
    def test_given_no_initial_state_when_read_then_return_state_based_on_cursor_field(
        self, http_mocker: HttpMocker
    ) -> None:
    ...
    @HttpMocker()
    def test_given_state_when_read_then_use_state_for_query_params(
        self, http_mocker: HttpMocker
    ) -> None:
        ...
        http_mocker.get(
            _a_request()
            .with_created_gte(state_value + _AVOIDING_INCLUSIVE_BOUNDARIES)
            .with_created_lte(_NOW)
            .with_limit(100)
            .build(),
            _a_response().with_record(_a_record()).build(),
        ...
        self._read(
            ...
            StateBuilder()
            .with_stream_state("events", {"created": int(state_value.timestamp())})
            .build(),
        )
    ...
    @HttpMocker()
    def test_given_state_more_recent_than_cursor_when_read_then_return_state_based_on_cursor_field(
        self, http_mocker: HttpMocker
    ) -> None:
        """
        ...
        output = self._read(
            _config().with_start_date(_A_START_DATE),
            StateBuilder()
            .with_stream_state("events", {"created": more_recent_than_record_cursor})
            .build(),
        )
        ...
        assert most_recent_state.stream_state == AirbyteStateBlob(
            created=more_recent_than_record_cursor
        )
