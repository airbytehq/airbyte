...
_ENDPOINT_TEMPLATE_NAME = "issuing_transactions"
...
def _transactions_request() -> StripeRequestBuilder:
...
def _given_transactions_availability_check(http_mocker: HttpMocker) -> None:
    http_mocker.get(
        StripeRequestBuilder.issuing_transactions_endpoint(_ACCOUNT_ID, _CLIENT_SECRET)
...
class FullRefreshTest(TestCase):
    ...
    def test_given_many_pages_when_read_then_return_records(
        ...
        http_mocker.get(
            ...
            _transactions_response()
            .with_pagination()
            .with_record(_a_transaction().with_id("last_record_id_from_first_page"))
            .build(),
        )
        http_mocker.get(
            _transactions_request()
...
class IncrementalTest(TestCase):
    ...
    @HttpMocker()
    def test_given_no_state_when_read_then_use_transactions_endpoint(
        self, http_mocker: HttpMocker
    ) -> None:
        _given_events_availability_check(http_mocker)
        ...
        http_mocker.get(
            ...
            .build(),
            _transactions_response()
            .with_record(_a_transaction().with_cursor(cursor_value))
            .build(),
        )
    ...
    def test_given_state_when_read_then_query_events_using_types_and_state_value_plus_1(
        ...
        http_mocker.get(
            ...
            .with_record(
                ...
                .with_field(_DATA_FIELD, _a_transaction().build())
            )
            .build(),
        )
    ...
    def test_given_state_and_small_slice_range_when_read_then_perform_multiple_queries(
        ...
        http_mocker.get(
            ...
            .build(),
            _events_response()
            .with_record(self._a_transaction_event())
            .with_record(self._a_transaction_event())
            .build(),
        )
