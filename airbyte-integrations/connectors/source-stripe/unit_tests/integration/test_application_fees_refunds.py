...
def _application_fees_refunds_request(application_fee_id: str) -> StripeRequestBuilder:
    return StripeRequestBuilder.application_fees_refunds_endpoint(
        application_fee_id, _ACCOUNT_ID, _CLIENT_SECRET
    )
...
def _given_application_fees_availability_check(http_mocker: HttpMocker) -> None:
    http_mocker.get(
        StripeRequestBuilder.application_fees_endpoint(_ACCOUNT_ID, _CLIENT_SECRET)
...
class FullRefreshTest(TestCase):
    ...
    def test_given_one_page_when_read_then_return_records(
        ...
        http_mocker.get(
            ...
            .with_record(
                _an_application_fee().with_field(
                    _REFUNDS_FIELD,
                    _as_dict(_refunds_response().with_record(_a_refund())),
                )
            )
            .build(),
        )
    ...
    @HttpMocker()
    def test_given_multiple_refunds_pages_when_read_then_query_pagination_on_child(
        self, http_mocker: HttpMocker
    ) -> None:
        _given_events_availability_check(http_mocker)
        http_mocker.get(
        ...
        http_mocker.get(
            ...
            _application_fees_refunds_request("parent_id")
            .with_limit(100)
            .with_starting_after("latest_refund_id")
            .build(),
    ...
    @HttpMocker()
    def test_given_multiple_application_fees_pages_when_read_then_query_pagination_on_parent(
        self, http_mocker: HttpMocker
    ) -> None:
        _given_events_availability_check(http_mocker)
        http_mocker.get(
    ...
    @HttpMocker()
    def test_given_parent_stream_without_refund_when_read_then_stream_is_unavailable(
        self, http_mocker: HttpMocker
    ) -> None:
    ...
    @HttpMocker()
    def test_given_slice_range_and_refunds_pagination_when_read_then_do_not_slice_child(
        self, http_mocker: HttpMocker
    ) -> None:
        """
...
class IncrementalTest(TestCase):
    ...
    @HttpMocker()
    def test_given_no_state_when_read_then_use_application_fees_endpoint(
        self, http_mocker: HttpMocker
    ) -> None:
        _given_events_availability_check(http_mocker)
        ...
        http_mocker.get(
            ...
            .with_record(
                _an_application_fee().with_field(
                    _REFUNDS_FIELD,
                    _as_dict(
                        _refunds_response().with_record(
                            _a_refund().with_cursor(cursor_value)
                        )
                    ),
                )
            )
    ...
    def test_given_state_when_read_then_query_events_using_types_and_state_value_plus_1(
        ...
        http_mocker.get(
            ...
            .with_record(
                ...
                .with_field(_DATA_FIELD, _a_refund().build())
            )
            .build(),
        )
    ...
    def test_given_state_and_small_slice_range_when_read_then_perform_multiple_queries(
        ...
        http_mocker.get(
            ...
            .build(),
            _events_response()
            .with_record(self._a_refund_event())
            .with_record(self._a_refund_event())
            .build(),
        )
