...
class FullRefreshTest(TestCase):
    ...
    def test_given_one_page_when_read_then_return_records(self, http_mocker: HttpMocker) -> None:
        http_mocker.get(
            ...
            .with_record(
                _an_application_fee().with_id("1").with_field(
                    _REFUNDS_FIELD, _as_dict(_refunds_response().with_record(_a_refund()).with_record(_a_refund()))
                )
            )
            .with_record(_an_application_fee().with_id("2").with_field(_REFUNDS_FIELD, _as_dict(_refunds_response().with_record(_a_refund()))))
            .build(),
        )
    ...
    def test_given_slice_range_when_read_then_perform_multiple_requests(self, http_mocker: HttpMocker) -> None:
        ...
        http_mocker.get(
            ...
            .build(),
            _application_fees_response()
            .with_record(_an_application_fee().with_id("1").with_field(_REFUNDS_FIELD, _as_dict(_refunds_response().with_record(_a_refund()))))
            .build(),
        )
        http_mocker.get(
