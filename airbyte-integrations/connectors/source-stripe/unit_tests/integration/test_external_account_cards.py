...
def _given_external_accounts_availability_check(http_mocker: HttpMocker) -> None:
    http_mocker.get(
        StripeRequestBuilder.external_accounts_endpoint(_ACCOUNT_ID, _CLIENT_SECRET)
...
class FullRefreshTest(TestCase):
    ...
    def test_given_one_page_when_read_then_return_records(
        ...
        http_mocker.get(
            ...
            .with_record(_an_external_account_card())
            .with_record(_an_external_account_card())
            .build(),
        )
    ...
    def test_given_many_pages_when_read_then_return_records(
        ...
        http_mocker.get(
            ...
            .with_pagination()
            .with_record(
                _an_external_account_card().with_id("last_record_id_from_first_page")
            )
            .build(),
        )
        http_mocker.get(
            _external_accounts_request()
...
class IncrementalTest(TestCase):
    ...
    @HttpMocker()
    def test_given_no_state_when_read_then_use_external_accounts_endpoint(
        self, http_mocker: HttpMocker
    ) -> None:
        _given_events_availability_check(http_mocker)
        http_mocker.get(
    ...
    @HttpMocker()
    def test_given_object_is_not_back_account_when_read_then_filter_out(
        self, http_mocker: HttpMocker
    ) -> None:
        start_date = _NOW - timedelta(days=40)
        state_datetime = _NOW - timedelta(days=5)
        ...
        http_mocker.get(
            ...
            .build(),
            _events_response()
            .with_record(_an_event().with_field(_DATA_FIELD, {"object": "not a card"}))
            .build(),
        )
    ...
    def test_given_state_and_small_slice_range_when_read_then_perform_multiple_queries(
        ...
        http_mocker.get(
            ...
            .build(),
            _events_response()
            .with_record(self._an_external_account_event())
            .with_record(self._an_external_account_event())
            .build(),
        )
