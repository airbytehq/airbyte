...
_STREAM_NAME = "reviews"
_ENDPOINT_TEMPLATE_NAME = "reviews"
...
def _reviews_request() -> StripeRequestBuilder:
    return StripeRequestBuilder.reviews_endpoint(_ACCOUNT_ID, _CLIENT_SECRET)
...
def _source(
    catalog: ConfiguredAirbyteCatalog, config: Dict[str, Any], state: Optional[TState]
) -> SourceStripe:
...
def _given_reviews_availability_check(http_mocker: HttpMocker) -> None:
    http_mocker.get(
        StripeRequestBuilder.reviews_endpoint(_ACCOUNT_ID, _CLIENT_SECRET)
...
def _given_events_availability_check(http_mocker: HttpMocker) -> None:
    http_mocker.get(
        StripeRequestBuilder.events_endpoint(_ACCOUNT_ID, _CLIENT_SECRET)
        .with_any_query_params()
        .build(),
...
class FullRefreshTest(TestCase):
    ...
    @HttpMocker()
    def test_given_one_page_when_read_then_return_records(
    ...
    @HttpMocker()
    def test_given_many_pages_when_read_then_return_records(
        self, http_mocker: HttpMocker
    ) -> None:
        _given_events_availability_check(http_mocker)
        http_mocker.get(
            ...
            _reviews_response()
            .with_pagination()
            .with_record(_a_review().with_id("last_record_id_from_first_page"))
            .build(),
        )
        http_mocker.get(
            _reviews_request()
    ...
    @HttpMocker()
    def test_given_no_state_when_read_then_return_ignore_lookback(
        self, http_mocker: HttpMocker
    ) -> None:
        _given_events_availability_check(http_mocker)
        http_mocker.get(
        ...
        self._read(
            _config().with_start_date(_A_START_DATE).with_lookback_window_in_days(10)
        )
    ...
    def test_when_read_then_add_cursor_field(self, http_mocker: HttpMocker) -> None:
        ...
        assert (
            output.records[0].record.data["updated"]
            == output.records[0].record.data["created"]
        )

    @HttpMocker()
    def test_given_slice_range_when_read_then_perform_multiple_requests(
        self, http_mocker: HttpMocker
    ) -> None:
        ...
        self._read(
            ...
            .with_slice_range_in_days(slice_range.days)
        )

        # request matched http_mocker

    @HttpMocker()
    def test_given_http_status_400_when_read_then_stream_is_ignored(
        self, http_mocker: HttpMocker
    ) -> None:
        http_mocker.get(
    ...
    @HttpMocker()
    def test_given_http_status_401_when_read_then_system_error(
        self, http_mocker: HttpMocker
    ) -> None:
        http_mocker.get(
            _reviews_request().with_any_query_params().build(),
    ...
    @HttpMocker()
    def test_given_rate_limited_when_read_then_retry_and_return_records(
        self, http_mocker: HttpMocker
    ) -> None:
        _given_events_availability_check(http_mocker)
        http_mocker.get(
    ...
    @HttpMocker()
    def test_given_small_slice_range_when_read_then_availability_check_performs_too_many_queries(
        self, http_mocker: HttpMocker
    ) -> None:
        ...
        self._read(
            ...
            .with_slice_range_in_days(1)
        )

        http_mocker.assert_number_of_calls(events_requests, 30)

    def _read(
        self, config: ConfigBuilder, expecting_exception: bool = False
    ) -> EntrypointOutput:
        return _read(
            config, SyncMode.full_refresh, expecting_exception=expecting_exception
        )
...
class IncrementalTest(TestCase):
    ...
    @HttpMocker()
    def test_given_no_state_when_read_then_use_reviews_endpoint(
        self, http_mocker: HttpMocker
    ) -> None:
        _given_events_availability_check(http_mocker)
        ...
        http_mocker.get(
            ...
            .build(),
            _reviews_response()
            .with_record(_a_review().with_cursor(cursor_value))
            .build(),
        )
    ...
    @HttpMocker()
    def test_given_state_when_read_then_query_events_using_types_and_state_value_plus_1(
        self, http_mocker: HttpMocker
    ) -> None:
        ...
        http_mocker.get(
            ...
            .with_record(
                ...
                .with_field(_DATA_FIELD, _a_review().build())
            )
            .build(),
        )
    ...
    @HttpMocker()
    def test_given_state_and_pagination_when_read_then_return_records(
        self, http_mocker: HttpMocker
    ) -> None:
        _given_reviews_availability_check(http_mocker)
        _given_events_availability_check(http_mocker)
        ...
        http_mocker.get(
            ...
            .with_record(
                _an_event()
                .with_id("last_record_id_from_first_page")
                .with_field(_DATA_FIELD, _a_review().build())
            )
            .build(),
        )
    ...
    @HttpMocker()
    def test_given_state_and_small_slice_range_when_read_then_perform_multiple_queries(
        self, http_mocker: HttpMocker
    ) -> None:
        state_datetime = _NOW - timedelta(days=5)
        ...
        http_mocker.get(
            ...
            .build(),
            _events_response()
            .with_record(self._a_review_event())
            .with_record(self._a_review_event())
            .build(),
        )
        ...
        output = self._read(
            _config()
            .with_start_date(_NOW - timedelta(days=30))
            .with_slice_range_in_days(slice_range.days),
            StateBuilder()
            .with_stream_state(
                _STREAM_NAME, {"updated": int(state_datetime.timestamp())}
    ...
    def test_given_state_earlier_than_30_days_when_read_then_query_events_using_types_and_event_lower_boundary(
        ...
        self._read(
            _config().with_start_date(start_date),
            StateBuilder()
            .with_stream_state(_STREAM_NAME, {"updated": int(state_value.timestamp())})
            .build(),
        )
