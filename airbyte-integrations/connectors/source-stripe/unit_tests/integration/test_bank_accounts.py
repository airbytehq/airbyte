...
class FullRefreshTest(TestCase):
    ...
    def test_given_one_page_when_read_then_return_records(self, http_mocker: HttpMocker) -> None:
        http_mocker.get(
            ...
            _customers_response()
            .with_record(
                _a_customer().with_id("1").with_field(
                    _SOURCES_FIELD, _as_dict(_bank_accounts_response().with_record(_a_bank_account()).with_record(_a_bank_account()))
                )
            )
            .with_record(_a_customer().with_id("2").with_field(_SOURCES_FIELD, _as_dict(_bank_accounts_response().with_record(_a_bank_account()))))
            .build(),
        )
    ...
    def test_given_slice_range_when_read_then_perform_multiple_requests(self, http_mocker: HttpMocker) -> None:
        ...
        http_mocker.get(
            ...
            .build(),
            _customers_response()
            .with_record(_a_customer().with_id("1").with_field(_SOURCES_FIELD, _as_dict(_bank_accounts_response().with_record(_a_bank_account()))))
            .build(),
        )
        http_mocker.get(
