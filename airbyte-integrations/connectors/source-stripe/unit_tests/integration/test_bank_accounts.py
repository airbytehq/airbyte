...
def _customers_bank_accounts_request(customer_id: str) -> StripeRequestBuilder:
    return StripeRequestBuilder.customers_bank_accounts_endpoint(
        customer_id, _ACCOUNT_ID, _CLIENT_SECRET
    )
...
def _given_customers_availability_check(http_mocker: HttpMocker) -> None:
    http_mocker.get(
        StripeRequestBuilder.customers_endpoint(_ACCOUNT_ID, _CLIENT_SECRET)
...
class FullRefreshTest(TestCase):
    ...
    def test_given_one_page_when_read_then_return_records(
        ...
        http_mocker.get(
            ...
            .with_record(
                _a_customer().with_field(
                    _SOURCES_FIELD,
                    _as_dict(_bank_accounts_response().with_record(_a_bank_account())),
                )
            )
            .build(),
        )
    ...
    @HttpMocker()
    def test_given_source_is_not_bank_account_when_read_then_filter_record(
        self, http_mocker: HttpMocker
    ) -> None:
        _given_events_availability_check(http_mocker)
        http_mocker.get(
            ...
            .with_record(
                _a_customer().with_field(
                    _SOURCES_FIELD,
                    _as_dict(
                        _bank_accounts_response().with_record(_NOT_A_BANK_ACCOUNT)
                    ),
                )
            )
            .build(),
    ...
    @HttpMocker()
    def test_given_multiple_bank_accounts_pages_when_read_then_query_pagination_on_child(
        self, http_mocker: HttpMocker
    ) -> None:
        _given_events_availability_check(http_mocker)
        http_mocker.get(
            ...
            .with_record(
                ...
                .with_field(
                    ...
                    _as_dict(
                        ...
                        .with_pagination()
                        .with_record(
                            _a_bank_account().with_id("latest_bank_account_id")
                        )
                    ),
                )
            )
        ...
        http_mocker.get(
            ...
            _customers_bank_accounts_request("parent_id")
            .with_limit(100)
            .with_starting_after("latest_bank_account_id")
            .build(),
    ...
    @HttpMocker()
    def test_given_multiple_customers_pages_when_read_then_query_pagination_on_parent(
        self, http_mocker: HttpMocker
    ) -> None:
        _given_events_availability_check(http_mocker)
        http_mocker.get(
    ...
    @HttpMocker()
    def test_given_parent_stream_without_bank_accounts_when_read_then_stream_is_unavailable(
        self, http_mocker: HttpMocker
    ) -> None:
    ...
    @HttpMocker()
    def test_given_slice_range_and_bank_accounts_pagination_when_read_then_do_not_slice_child(
        self, http_mocker: HttpMocker
    ) -> None:
        """
    ...
    @HttpMocker()
    def test_given_one_page_when_read_then_cursor_field_is_set(
        self, http_mocker: HttpMocker
    ) -> None:
        _given_events_availability_check(http_mocker)
        http_mocker.get(
            ...
            [
                ...
                .with_record(
                    _a_customer().with_field(
                        _SOURCES_FIELD,
                        _as_dict(
                            _bank_accounts_response().with_record(_a_bank_account())
                        ),
                    )
                )
                .build(),
...
class IncrementalTest(TestCase):
    ...
    @HttpMocker()
    def test_given_no_state_and_successful_sync_when_read_then_set_state_to_now(
        self, http_mocker: HttpMocker
    ) -> None:
    ...
    def test_given_state_when_read_then_query_events_using_types_and_state_value_plus_1(
        ...
        http_mocker.get(
            ...
            .with_record(
                ...
                .with_field(_DATA_FIELD, _a_bank_account().build())
            )
            .build(),
        )
    ...
    def test_given_state_and_small_slice_range_when_read_then_perform_multiple_queries(
        ...
        http_mocker.get(
            ...
            .build(),
            _events_response()
            .with_record(self._a_bank_account_event())
            .with_record(self._a_bank_account_event())
            .build(),
        )
