...
def _charges_request() -> StripeRequestBuilder:
    return StripeRequestBuilder.charges_endpoint(_ACCOUNT_ID, _CLIENT_SECRET)
...
def _charges_response() -> HttpResponseBuilder:
...
class FullRefreshTest(TestCase):
    ...
    def test_given_one_page_when_read_then_return_records(self, http_mocker: HttpMocker) -> None:
        http_mocker.get(
            _charges_request().with_created_gte(_A_START_DATE).with_created_lte(_NOW).with_limit(100).build(),
            _charges_response().with_record(_a_charge()).with_record(_a_charge()).build(),
        )
