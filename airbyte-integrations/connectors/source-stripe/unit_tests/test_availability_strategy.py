...
def stream_mock(mocker):
    def _mocker():
        return mocker.Mock(
            stream_slices=mocker.Mock(return_value=[{}]),
            read_records=mocker.Mock(return_value=[{}]),
        )
...
def test_traverse_over_substreams(stream_mock, mocker):
    ...
    mocker.patch(
        "source_stripe.availability_strategy.StripeAvailabilityStrategy.check_availability",
    ...
    # Start traverse
    is_available, reason = child_1_1_1.availability_strategy.check_availability(
        child_1_1_1, mocker.Mock(), mocker.Mock()
    )
...
def test_traverse_over_substreams_failure(stream_mock, mocker):
    ...
    check_availability_mock = mocker.MagicMock(
        side_effect=[(True, None), (False, "child_1")]
    )
    mocker.patch(
        "source_stripe.availability_strategy.StripeAvailabilityStrategy.check_availability",
        check_availability_mock,
...
def test_substream_availability(mocker, stream_by_name):
    ...
    is_available, reason = stream.availability_strategy.check_availability(
        stream, mocker.Mock(), mocker.Mock()
    )
    assert is_available and reason is None

    assert check_availability_mock.call_count == 2
    assert isinstance(
        check_availability_mock.call_args_list[0].args[0], IncrementalStripeStream
    )
    assert isinstance(
        check_availability_mock.call_args_list[1].args[0], StripeLazySubStream
    )
...
def test_substream_availability_no_parent(mocker, stream_by_name):
    ...
    assert isinstance(
        check_availability_mock.call_args_list[0].args[0], StripeLazySubStream
    )
...
@pytest.mark.parametrize(
    ...
    (
        ...
        (
            "subscription_items",
            {"/v1/subscriptions": {"data": [{"id": 1}]}, "/v1/events": {"data": []}},
            3,
        ),
        (
            "bank_accounts",
            {"/v1/customers": {"data": [{"id": 1}]}, "/v1/events": {"data": []}},
            2,
        ),
        (
...
def test_availability_strategy_visits_endpoints(
    stream_by_name,
    stream_name,
    endpoints,
    expected_calls,
