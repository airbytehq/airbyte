...
import static io.airbyte.integrations.source.postgres.PostgresSpecConstants.INVALID_CDC_CURSOR_POSITION;
  ...
  public static List<AutoCloseableIterator<AirbyteMessage>> cdcCtidIteratorsCombined(final JdbcDatabase database,
    ...
    try {
      ...
      if (!savedOffsetAfterReplicationSlotLSN) {
        ...
        if (!sourceConfig.get("replication_method").has(INVALID_CDC_CURSOR_POSITION) || sourceConfig.get("replication_method").get(INVALID_CDC_CURSOR_POSITION).equals(FAIL_SYNC_OPTION)) {
