...
public class PostgresCdcProperties {
  ...
  public static Properties getDebeziumDefaultProperties(final JdbcDatabase database) {
    ...
    props.setProperty(
        "plugin.name", PostgresUtils.getPluginValue(sourceConfig.get("replication_method")));
    if (sourceConfig.has("snapshot_mode")) {
      // The parameter `snapshot_mode` is passed in test to simulate reading the WAL Logs directly
      // and
  ...
  private static Properties commonProperties(final JdbcDatabase database) {
    ...
    props.setProperty(
        ...
        Long.toString(Duration.ofSeconds(HEARTBEAT_FREQUENCY_SEC).toMillis()));
    ...
    if (!sourceConfig.has(JdbcUtils.SSL_KEY)
      ...
      if (sourceConfig.has(JdbcUtils.SSL_MODE_KEY)
          && sourceConfig.get(JdbcUtils.SSL_MODE_KEY).has(JdbcUtils.MODE_KEY)) {

        if (dbConfig.has(SSL_MODE) && !dbConfig.get(SSL_MODE).asText().isEmpty()) {
          LOGGER.debug("sslMode: {}", dbConfig.get(SSL_MODE).asText());
          props.setProperty(
              "database.sslmode",
              PostgresSource.toSslJdbcParamInternal(
                  SslMode.valueOf(dbConfig.get(SSL_MODE).asText())));
        }

        if (dbConfig.has(PostgresSource.CA_CERTIFICATE_PATH)
            && !dbConfig.get(PostgresSource.CA_CERTIFICATE_PATH).asText().isEmpty()) {
          props.setProperty(
        ...
        if (dbConfig.has(CLIENT_KEY_STORE_PASS)
          ...
          props.setProperty(
              "database.sslpassword", dbConfig.get(CLIENT_KEY_STORE_PASS).asText());
        }
      } else {
