...
import static io.airbyte.integrations.source.postgres.cdc.DateTimeConverter.convertToDate;
import static io.airbyte.integrations.source.postgres.cdc.DateTimeConverter.convertToTime;
import static io.airbyte.integrations.source.postgres.cdc.DateTimeConverter.convertToTimestamp;
import static io.airbyte.integrations.source.postgres.cdc.DateTimeConverter.convertToTimestampWithTimezone;
...
public class PostgresDebeziumDatetimeConverter implements CustomConverter<SchemaBuilder, RelationalColumn> {
  ...
  private void registerNumber(final RelationalColumn field, final ConverterRegistration<SchemaBuilder> registration) {
    registration.register(SchemaBuilder.string().optional(), x -> {
      if (x == null) {
        Object defaultValue = convertDefaultValue(field);
  ...
  private void registerBytea(final RelationalColumn field, final ConverterRegistration<SchemaBuilder> registration) {
    registration.register(SchemaBuilder.string().optional(), x -> {
      if (x == null) {
        Object defaultValue = convertDefaultValue(field);
        return defaultValue == null ? null : "\\x" + encodeHexString((byte[]) defaultValue);
      }
      return "\\x" + encodeHexString((byte[]) x);
    });
  }
  ...
  private static Object convertDefaultValue(final RelationalColumn field) {
    if (field.isOptional()) {
      return null;
