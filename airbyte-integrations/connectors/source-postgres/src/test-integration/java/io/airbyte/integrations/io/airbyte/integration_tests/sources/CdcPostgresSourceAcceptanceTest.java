...
public class CdcPostgresSourceAcceptanceTest extends SourceAcceptanceTest {
  ...
  protected void setupEnvironment(final TestDestinationEnv environment) throws Exception {
    ...
    final Database database =
        Databases.createDatabase(
            ...
            SQLDialect.POSTGRES);
    /**
     * cdc expects the INCREMENTAL tables to contain primary key checkout {@link
     * io.airbyte.integrations.source.postgres.PostgresSource#removeIncrementalWithoutPk(AirbyteStream)}
     */
    database.query(
        ctx -> {
          ctx.execute(
  ...
  protected ConfiguredAirbyteCatalog getConfiguredCatalog() {
     ...
     * specify cursor field for INCREMENTAL tables Take a look at {@link
     * io.airbyte.integrations.source.postgres.PostgresSource#setIncrementalToSourceDefined(AirbyteStream)}
     * We should also specify the primary keys for INCREMENTAL tables checkout {@link
     * io.airbyte.integrations.source.postgres.PostgresSource#removeIncrementalWithoutPk(AirbyteStream)}
     */
