...
class PostgresSourceDatatypeTest : AbstractPostgresSourceDatatypeTest() {
    ...
    protected override fun setupDatabase(): Database {
        testdb = PostgresTestDatabase.`in`(PostgresTestDatabase.BaseImage.POSTGRES_17, PostgresTestDatabase.ContainerModifier.CONF)
            ...
            .with("CREATE TYPE inventory_item AS (name text, supplier_id integer, price numeric);") // In one of the test case, we have some money values with currency symbol. Postgres can only
            ...
            .with("SET lc_monetary TO 'en_US.utf8';") // Set up a fixed timezone here so that timetz and timestamptz always have the same time zone
