...
open class PostgresSourceAcceptanceTest : AbstractPostgresSourceAcceptanceTest() {
    ...
    private fun getConfig(username: String, password: String?, vararg schemas: String?): JsonNode {
        return testdb.configBuilder()
            ...
            .let { cb -> password?.let { p -> cb.with(JdbcUtils.PASSWORD_KEY, p) }; cb }
            .withSchemas(*schemas)
    ...
    fun testFullRefreshWithRevokingSchemaPermissions() {
        ...
        val configuredCatalog: ConfiguredAirbyteCatalog? =
            this.limitPermissionConfiguredCatalog
    ...
    private fun revokeSchemaPermissions(database: Database) {
        database.query<Any?>(
            ContextQueryFunction { ctx: DSLContext ->
                ctx.fetch(
                    String.format(
                        "REVOKE USAGE ON schema %s FROM %s;", LIMIT_PERMISSION_SCHEMA,
                        this.limitPermissionRoleName,
                    ),
                )
                null
            ...
            ContextQueryFunction { ctx: DSLContext ->
                ctx.fetch(
                    String.format(
                        ...
                        this.limitPermissionRoleName, LIMIT_PERMISSION_ROLE_PASSWORD,
                    ),
                )
                ctx.fetch(String.format("CREATE SCHEMA %s;", LIMIT_PERMISSION_SCHEMA))
                ctx.fetch(
                    java.lang.String.format(
                        "GRANT CONNECT ON DATABASE %s TO %s;", testdb.databaseName,
                        this.limitPermissionRoleName,
                    ),
                )
                ctx.fetch(
                    String.format(
                        "GRANT USAGE ON schema %s TO %s;", LIMIT_PERMISSION_SCHEMA,
                        this.limitPermissionRoleName,
                    ),
                )
                ctx.fetch(
                ...
                ctx.fetch(
                    String.format(
                        "GRANT SELECT ON table %s.id_and_name TO %s;", LIMIT_PERMISSION_SCHEMA,
                        this.limitPermissionRoleName,
                    ),
                )
                null
