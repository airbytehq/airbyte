...
class PostgresTestDatabase(container: PostgreSQLContainer<*>) :
    TestDatabase<PostgreSQLContainer<*>,
        PostgresTestDatabase,
        PostgresTestDatabase.PostgresConfigBuilder>(container) {
    enum class BaseImage(val reference: String, val majorVersion: Int) {
    ...
    protected override fun inContainerBootstrapCmd(): Stream<Stream<String>> {
        return Stream.of<Stream<String>>(
            psqlCmd(
                Stream.of(
                    java.lang.String.format("CREATE DATABASE %s", databaseName),
                    java.lang.String.format("CREATE USER %s PASSWORD '%s'", userName, password),
                    java.lang.String.format(
                        "GRANT ALL PRIVILEGES ON DATABASE %s TO %s",
                        databaseName,
                        userName
                    ),
                    java.lang.String.format("ALTER USER %s WITH SUPERUSER", userName)
                )
            )
        )
    }
     ...
     */
    protected override fun inContainerUndoBootstrapCmd(): Stream<String> {
    ...
    fun dropDatabaseAndUser() {
        execInContainer(
            psqlCmd(
                Stream.of(
                    java.lang.String.format("DROP DATABASE %s", databaseName),
                    java.lang.String.format("DROP OWNED BY %s", userName),
                    java.lang.String.format("DROP USER %s", userName)
                )
            )
        )
    }
    ...
    @JvmRecord
    data class Certificates(val caCertificate: String?, val clientCertificate: String?, val clientKey: String?)

    public override fun configBuilder(): PostgresConfigBuilder {
    ...
    fun withReplicationSlot(): PostgresTestDatabase {
        return this
            .with("SELECT pg_create_logical_replication_slot('%s', 'pgoutput');", this.replicationSlotName)
    ...
    class PostgresConfigBuilder(testdb: PostgresTestDatabase) :
        ...
        fun withStandardReplication(): PostgresConfigBuilder {
            return with("replication_method", ImmutableMap.builder<Any?, Any?>().put("method", "Standard").build())
        }
        ...
        fun withCdcReplication(
            ...
            return this
                .with("is_test", true)
                .with(
                    "replication_method", Jsons.jsonNode(
        ...
        fun withXminReplication(): PostgresConfigBuilder {
            return this.with(
                "replication_method",
                Jsons.jsonNode(ImmutableMap.builder<Any?, Any?>().put("method", "Xmin").build())
            )
        }
    }

    public override fun close() {
//        PostgresDebeziumStateUtil.disposeInitialState() // TEMP
        super.close()
    }
    ...
    companion object {
        ...
        fun `in`(baseImage: BaseImage, vararg modifiers: ContainerModifier): PostgresTestDatabase {
            val methodNames =
                Stream.of<ContainerModifier>(*modifiers).map<String> { im: ContainerModifier -> im.methodName }
