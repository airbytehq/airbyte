...
def test_get_updated_state(patch_incremental_base_class):
    stream = make_stream()
    inputs = {
        "current_stream_state": {"lastModifiedDate": "2022-01-01T00:00:00.000Z"},
        "latest_record": {"lastModifiedDate": "2021-12-31T23:59:59.999Z"},
    }
...
def test_request_params(patch_incremental_base_class):
    ...
    inpt = {
        "stream_state": {},
        "stream_slice": {"q": 'lastModifiedDate AFTER "2022-01-01 12:00:00 AM" AND lastModifiedDate BEFORE 2022-02-01'},
    }
...
def test_stream_slices_stream_state(patch_incremental_base_class):
    ...
    with patch("source_netsuite.source.date", new=date_mock):
        ...
        assert expected == stream.stream_slices(
            **{"sync_mode": SyncMode.incremental, "stream_state": {"lastModifiedDate": "2022-01-01T00:00:00Z"}}
        )
