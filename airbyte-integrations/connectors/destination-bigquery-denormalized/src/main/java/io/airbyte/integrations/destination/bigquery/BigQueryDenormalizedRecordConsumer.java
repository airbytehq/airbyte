...
public class BigQueryDenormalizedRecordConsumer extends BigQueryRecordConsumer {
  ...
  private static final Logger LOGGER =
      LoggerFactory.getLogger(BigQueryDenormalizedRecordConsumer.class);

  private final StandardNameTransformer namingResolver;
  private final Set<String> invalidKeys;

  public BigQueryDenormalizedRecordConsumer(
  ...
  protected JsonNode formatData(final FieldList fields, final JsonNode root) {
    ...
    if (root.isObject()) {
          ...
          Jsons.keys(root).stream()
              ...
              .collect(
                  Collectors.toMap(
                      ...
                      key ->
                          formatData(
                              fields.get(namingResolver.getIdentifier(key)).getSubFields(),
                              root.get(key)))));
      ...
      final FieldList subFields =
          (arrayField.getSubFields() == null || arrayField.getSubFields().isEmpty()
              ? fields
      ...
      // "Array of Array of" (nested arrays) are not permitted by BigQuery ("Array of Record of
      // Array of"
      // is)
      // Turn all "Array of" into "Array of Record of" instead
      return Jsons.jsonNode(
          ImmutableMap.of(BigQueryDenormalizedDestination.NESTED_ARRAY_FIELD, items));
    } else {
      return root;
    }
  }
