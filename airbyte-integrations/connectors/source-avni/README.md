# Avni source connector

This directory contains the manifest-only connector for `source-avni`.
...
For information about how to configure and use this connector within Airbyte, see [the connector's full documentation](https://docs.airbyte.com/integrations/sources/avni).
...
```bash
airbyte-ci connectors --name=source-avni build
```

An image will be available on your host with the tag `airbyte/source-avni:dev`.
...
```bash
airbyte-ci connectors --name=source-avni test
```

## Publishing a new version of the connector

If you want to contribute changes to `source-avni`, here's how you can do that:
1. Make your changes locally, or load the connector's manifest into Connector Builder and make changes there.
2. Make sure your changes are passing our test suite with `airbyte-ci connectors --name=source-avni test`
...
4. Make sure the connector documentation and its changelog is up to date (`docs/integrations/sources/avni.md`).
