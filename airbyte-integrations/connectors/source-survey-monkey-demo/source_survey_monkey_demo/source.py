...
class SurveyMonkeyBaseStream(HttpStream, ABC):
        ...
        **kwargs: Any,
        ...
        policies = [
            MovingWindowCallRatePolicy(
                rates=[Rate(limit=_call_limit, interval=datetime.timedelta(minutes=1))],
                matchers=[],
            ),
        ]
    ...
    def request_params(
        ...
        if next_page_token:
            ...
            return {
                ...
                "sort_order": "ASC",
                "start_modified_at": datetime.datetime.fromtimestamp(stream_slice["start_date"]).strftime(_DATETIME_FORMAT),
                "end_modified_at": datetime.datetime.fromtimestamp(stream_slice["end_date"]).strftime(_DATETIME_FORMAT),
            }
    ...
    def stream_slices(self, stream_state: Mapping[str, Any] = None, **kwargs) -> Iterable[Optional[Mapping[str, any]]]:
        start_ts = stream_state.get("date_modified", self._start_date) if stream_state else self._start_date
...
class SourceSurveyMonkeyDemo(ConcurrentSourceAdapter):
    ...
    def streams(self, config: Mapping[str, Any]) -> List[Stream]:
        ...
        start_date = datetime.datetime.strptime(config["start_date"], _DATETIME_FORMAT).timestamp()

        survey_stream = SurveyMonkeyBaseStream(
            name="surveys", path="/v3/surveys", primary_key="id", authenticator=auth, cursor_field="date_modified", start_date=start_date
        )
        synchronous_streams = [
            survey_stream,
            SurveyMonkeySubstream(
                name="survey_responses",
                path="/v3/surveys/{stream_slice[id]}/responses/",
                primary_key="id",
                authenticator=auth,
                parent_stream=survey_stream,
            ),
        ]
        ...
        for stream in synchronous_streams:
            ...
            configured_streams.append(StreamFacade.create_from_stream(stream, self, _logger, legacy_state, cursor))
