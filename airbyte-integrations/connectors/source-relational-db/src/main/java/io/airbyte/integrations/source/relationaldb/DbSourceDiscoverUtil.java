...
public class DbSourceDiscoverUtil {
  ...
  private static final List<String> AIRBYTE_METADATA =
      Arrays.asList("_ab_cdc_lsn", "_ab_cdc_updated_at", "_ab_cdc_deleted_at");
   ...
   */
  public static <DataType> void logSourceSchemaChange(
    ...
    for (final ConfiguredAirbyteStream airbyteStream : catalog.getStreams()) {
      ...
      final TableInfo<CommonField<DataType>> table =
          fullyQualifiedTableNameToInfo.get(fullyQualifiedTableName);
      final List<Field> fields = table.getFields().stream()
      ...
      catalogProperties.fieldNames().forEachRemaining(fieldName -> {
        // Ignoring metadata fields since those are automatically added onto the catalog schema by
        // Airbyte
  ...
  public static <DataType> AirbyteCatalog convertTableInfosToAirbyteCatalog(
    ...
    final List<TableInfo<Field>> tableInfoFieldList = tableInfos.stream()
        .map(t -> {
          // some databases return multiple copies of the same record for a column (e.g. redshift)
          // because
          // they have at least once delivery guarantees. we want to dedupe these, but first we
          // check that the
          ...
          final List<Field> fields = t.getFields().stream()
              .map(commonField -> toField(commonField, airbyteTypeConverter))
              .distinct()
              .collect(toList());
          final String fullyQualifiedTableName =
              getFullyQualifiedTableName(t.getNameSpace(), t.getName());
          final List<String> primaryKeys = fullyQualifiedTableNameToPrimaryKeys.getOrDefault(
              fullyQualifiedTableName, Collections.emptyList());
          return TableInfo.<Field>builder()
    ...
    final List<AirbyteStream> streams = tableInfoFieldList.stream()
        .map(tableInfo -> {
          ...
          return CatalogHelpers.createAirbyteStream(
                  tableInfo.getName(), tableInfo.getNameSpace(), tableInfo.getFields())
              .withSupportedSyncModes(
                  tableInfo.getCursorFields() != null
                          && tableInfo.getCursorFields().isEmpty()
  ...
  private static <DataType> Field toField(
      final CommonField<DataType> commonField,
      final Function<DataType, JsonSchemaType> airbyteTypeConverter) {
    if (airbyteTypeConverter.apply(commonField.getType()) == JsonSchemaType.OBJECT
        && commonField.getProperties() != null
        && !commonField.getProperties().isEmpty()) {
      final var properties = commonField.getProperties().stream()
          .map(commField -> toField(commField, airbyteTypeConverter))
          .toList();
      return Field.of(
          commonField.getName(), airbyteTypeConverter.apply(commonField.getType()), properties);
    } else {
      return Field.of(commonField.getName(), airbyteTypeConverter.apply(commonField.getType()));
    }
  }

  private static <DataType> void assertColumnsWithSameNameAreSame(
      final String nameSpace, final String tableName, final List<CommonField<DataType>> columns) {
    columns.stream()
