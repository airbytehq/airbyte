...
public abstract class AbstractDbSource<DataType, Database extends AbstractDatabase>
    extends BaseConnector implements Source {
  ...
  public AutoCloseableIterator<AirbyteMessage> read(
    ...
    final Map<String, TableInfo<CommonField<DataType>>> fullyQualifiedTableNameToInfo =
        discoverWithoutSystemTables(database).stream()
            .collect(
                Collectors.toMap(
                    t -> String.format("%s.%s", t.getNameSpace(), t.getName()),
                    Function.identity()));
    ...
    final List<AutoCloseableIterator<AirbyteMessage>> fullRefreshIterators =
        getFullRefreshIterators(
            database, catalog, fullyQualifiedTableNameToInfo, stateManager, emittedAt);
    final List<AutoCloseableIterator<AirbyteMessage>> iteratorList =
        Stream.of(incrementalIterators, fullRefreshIterators)
            .flatMap(Collection::stream)
            .collect(Collectors.toList());

    return AutoCloseableIterators.appendOnClose(
        AutoCloseableIterators.concatWithEagerClose(iteratorList),
        () -> {
    ...
    for (final ConfiguredAirbyteStream airbyteStream : catalog.getStreams()) {
      if (selector.test(airbyteStream)) {
        ...
        final String fullyQualifiedTableName =
            getFullyQualifiedTableName(stream.getNamespace(), stream.getName());
  ...
  protected AutoCloseableIterator<AirbyteMessage> createReadIterator(
      final Database database,
      ...
      if (cursorOptional.isPresent()) {
        ...
        airbyteMessageIterator =
            getFullRefreshStream(
                database, streamName, namespace, selectedDatabaseFields, table, emittedAt);
      }
  ...
  protected AutoCloseableIterator<AirbyteMessage> getIncrementalStream(
      final Database database,
      final ConfiguredAirbyteStream airbyteStream,
      final List<String> selectedDatabaseFields,
      final TableInfo<CommonField<DataType>> table,
    ...
    final AutoCloseableIterator<JsonNode> queryIterator =
  ...
  protected AutoCloseableIterator<AirbyteMessage> getFullRefreshStream(
      final Database database,
    ...
    final AutoCloseableIterator<JsonNode> queryStream =
        queryTableFullRefresh(
            database, selectedDatabaseFields, table.getNameSpace(), table.getName());
  ...
  protected void assertColumnsWithSameNameAreSame(
   ...
   */
  public abstract AutoCloseableIterator<JsonNode> queryTableFullRefresh(
      final Database database,
   ...
   * @return iterator with read data
   */
  public abstract AutoCloseableIterator<JsonNode> queryTableIncremental(
      Database database,
