# Contributing to source-bigquery

`source-bigquery` 1.0.0 is a Bulk CDK (`airbyte-cdk/bulk`, `extract` core + `extract-jdbc` toolkit,
`cdkVersion=1.1.11`) rewrite of the connector. Versions up to 0.4.5, called the legacy connector
below, were built on the old Java CDK with the native `google-cloud-bigquery` client and lived in
the same directory; their code was removed in 1.0.0. The connector connects through Google's Apache-2.0 JDBC driver
...
Stages 1 (`spec` + `check`), 2 (`discover`), 3 (first `read`, stream statuses), 4 (`read`:
full refresh, cursor-based incremental with resume, legacy state translation, speed mode over Unix
domain sockets in JSONL or protobuf) and 5 (validation at scale, which led to the Storage Read API
read path) are implemented; see "Stage status and next steps" at the end.

## Build and test

```bash
export JAVA_HOME=/opt/homebrew/opt/openjdk@21/libexec/openjdk.jdk/Contents/Home   # macOS/Homebrew; JDK 21
./gradlew :airbyte-integrations:connectors:source-bigquery:compileKotlin
./gradlew :airbyte-integrations:connectors:source-bigquery:test        # Testcontainers, needs Docker
./gradlew :airbyte-integrations:connectors:source-bigquery:assemble    # builds airbyte/source-bigquery:dev
```
...
find airbyte-integrations/connectors/source-bigquery/src -name '*.kt' | xargs java -jar /tmp/ktfmt-0.39.jar --kotlinlang-style
```
...
docker run --rm -e DATA_CHANNEL_MEDIUM=SOCKET -e DATA_CHANNEL_FORMAT=PROTOBUF \
  ...
  -v $PWD/secrets:/secrets -v bq-sockets:/tmp/sockets airbyte/source-bigquery:dev \
...
Value fidelity evidence (2026-09-22, `/tmp/bq-perf/diff-records.py`, byte-exact JSON text of
...
## Parity with source-bigquery 0.4.5 (the legacy connector)
...
getProjectTables`/`getDatasetTables`; 1.0.0 runs `tables.get` on 32 threads per dataset).
...
- `check`: legacy ran `select 1` and, with a dataset, `select 1 from `<dataset>`.`INFORMATION_SCHEMA.TABLES` where 1=0`;
  1.0.0 runs discovery (`datasets.list`/`tables.list`/`tables.get`) plus
...
- `check`: both images succeed with and without `dataset_id`. Failure messages differ in wording
  ...
  ` was not found in location US`); unknown project -> 1.0.0 `Project <id> is not found. Make sure it
references valid GCP project that hasn't been deleted.`, legacy `ProjectId must be non-empty`;
  broken private key -> 1.0.0 `'credentials_json' has an invalid 'private_key'...` (validated in the
  config factory because the driver only says `No valid credentials provided.`), legacy `Unexpected
exception reading PKCS#8 data`; `authorized_user` JSON -> 1.0.0 factory message, legacy `'type' value
...
- Incremental: with an `INT64`/`NUMERIC` cursor the state is `{"primary_key":{},"cursors":{"id":3}}`
  ...
  `"cursor":"2026-08-27T17:23:58Z"`). Resuming 1.0.0 from the **legacy** state file emits no records
...
- Views: BigQuery rejects the sampling query on a view with "TABLESAMPLE SYSTEM can only be
  applied directly to base tables."; with `BigQueryTableTypes` 1.0.0 samples the view with plain
  `LIMIT` queries and reads it (1017 rows, same as legacy).
- `check` on a dataset without tables: 1.0.0 fails with "Discovered zero tables.", legacy succeeds
...
- **Known gap**: the Bulk CDK drops every column whose name starts with `_ab_` at READ time
  ...
  missing from 1.0.0 records while legacy emitted them. Tables written by Airbyte destinations
...
- Seeded dataset `source_bigquery_v2_parity` (tables `all_types`, `with_pk`, `no_rows`, view
  ...
  `PRIMARY KEY ... NOT ENFORCED` reach `tableConstraints` and 1.0.0 advertises them
  ...
  legacy after the text-based reads above; `TIMESTAMP '0001-01-01'` is emitted correctly by 1.0.0
...
- `BigQueryTableTypes` remembers the `TableDefinition.Type` of every table fetched by the
  ...
  directly to base tables.", and a 1.0.0 `read` of a real view succeeds through the fallback.
...
Next: a CDK fix or workaround for `_ab_*` columns. The deliberate deviations from the legacy
connector are released as the 1.0.0 breaking change (`releases.breakingChanges` in `metadata.yaml`,
migration guide `docs/integrations/sources/bigquery-migrations.md`). The user-facing docs page is
