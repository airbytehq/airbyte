...
def phase1():
    ...
    try:
        ...
        result = airbyte("POST", "sources/check_connection", {
            "sourceId": source_id,
        }) if source_id else {"status": "skip"}
...
def phase2():
    ...
    try:
        ...
        # Create source
        src = airbyte("POST", "sources/create", {
        ...
        dst = airbyte("POST", "destinations/create", {
            "workspaceId": workspace,
        ...
        # Create connection
        conn_resp = airbyte("POST", "connections/create", {
            "sourceId": source_id,
    ...
    try:
        ...
        record("TEST-2.6", found, "snapshot.locking.mode=none in logs" if found else "Not found in logs (may require docker log inspection)")
    except Exception as e:
...
def phase3():
    ...
    try:
        ...
        record("TEST-3.6", passed,
               f"Updated rows: {updated_count}/3, active total: {active_count}/55")
    except Exception as e:
    ...
    try:
        ...
        deleted_rows = sf_query(
            f'SELECT COUNT(*) FROM "{SNOWFLAKE_DATABASE}"."{SNOWFLAKE_SCHEMA}"."{TEST_TABLE}"'
            f' WHERE "_AB_CDC_DELETED_AT" IS NOT NULL'
        )
        deleted_count = deleted_rows[0][0]
        passed = active_count == 53 and deleted_count >= 2
        record("TEST-3.9", passed,
               f"Active rows: {active_count}/53, soft-deleted: {deleted_count} (>= 2)")
    except Exception as e:
    ...
    try:
        ...
        bulk_count = rows[0][0]
        record("TEST-3.12", bulk_count == 20,
               f"Bulk-updated rows in Snowflake: {bulk_count}/20")
    except Exception as e:
...
def phase4():
    ...
    try:
        ...
        record("TEST-4.2", sf_count == mysql_count,
               f"MySQL active: {mysql_count}, Snowflake active: {sf_count}")
    except Exception as e:
    ...
    try:
        ...
        record("TEST-4.3", len(dup_check) == 0,
               f"Duplicate IDs found: {len(dup_check)}" if dup_check else "Zero duplicates")
    except Exception as e:
...
def phase5():
    ...
    try:
        sf_query(
            f'DROP TABLE IF EXISTS "{SNOWFLAKE_DATABASE}"."{SNOWFLAKE_SCHEMA}"."{TEST_TABLE}"'
        )
