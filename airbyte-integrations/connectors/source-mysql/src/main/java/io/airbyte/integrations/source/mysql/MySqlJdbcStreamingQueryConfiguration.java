...
public class MySqlJdbcStreamingQueryConfiguration implements JdbcStreamingQueryConfiguration {
  ...
  public void accept(final Connection connection, final PreparedStatement preparedStatement)
    ...
    // performance.
    // e.g. conn = DriverManager.getConnection("jdbc:mysql://localhost/?useCursorFetch=true",
    // "user",
    // "s3cr3t");
    ...
    // implementation below. It fetches records one at a time, which while inefficient, at least
    // does
    // not risk OOM.
    // connection.setAutoCommit(false);
