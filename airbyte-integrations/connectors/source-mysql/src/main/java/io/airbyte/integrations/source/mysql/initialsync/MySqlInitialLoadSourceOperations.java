...
public class MySqlInitialLoadSourceOperations extends MySqlSourceOperations {
  ...
  public JsonNode rowToJson(final ResultSet queryContext) throws SQLException {
    if (metadataInjector.isPresent()) {
      ...
      for (int i = 1; i <= columnCount; i++) {
        ...
        // type-specific
        // parsing. if it is null, we can move on. while awkward, this seems to be the agreed upon
        // way of
  ...
  public static class CdcMetadataInjector {
    ...
    private void inject(final ObjectNode record) {
      metadataInjector.addMetaDataToRowsFetchedOutsideDebezium(
          record, transactionTimestamp, stateAttributes);
    }
  }
}
