 ...
 * generated by treating them as numbers in a Unicode-based numeric system, with the base determined by
 * the highest code point present in the boundaries.
 */
fun unicodeInterpolatedStrings(start: String, end: String, steps: Int): List<String> {
    ...
    // Determine the interpolation base from the highest code point present in the boundaries, reducing the risk of
    // producing characters unsupported by the column's character set.
    val maxCodePoint: Int = ((a + b ).maxOrNull() ?: 0).coerceAtLeast(0x7F)
    // Avoid interpolation when the code point range reaches UTF-16 surrogates. Although valid code points exist above
