...
/**
 * Tests for GTID set validation in CDC warm-start, specifically covering the
 * Debezium 3.1.x NPE in MySqlGtidSet.subtract() when GTID sets contain
 * server UUIDs absent from the other set (e.g., after MySQL failover/topology change).
 *
 * See: https://github.com/airbytehq/oncall/issues/12810
 * Upstream fix: https://github.com/debezium/debezium/pull/6894 (DBZ-9682)
 */
class MySqlSourceGtidValidationTest {

    /**
     * Demonstrates the Debezium 3.1.x NPE bug in MySqlGtidSet.subtract().
     * When the source set contains a server UUID not present in the other set,
     * forServerWithId() returns null and the subsequent getUUID() call NPEs.
     */
    @Test
    ...
    /**
     * When the available GTID set contains UUIDs absent from the saved state
     * (typical post-failover scenario), the validate() method should catch the
     * Debezium NPE and return AbortDebeziumWarmStartState when FAIL_SYNC is configured.
     */
    @Test
    fun testValidateHandlesGtidNpeWithFailSync() {
        val ops = createOperations(
        ...
        val stateValue = buildOpaqueState(
    ...
    /**
     * Same post-failover scenario but with RESET_SYNC — should return
     * ResetDebeziumWarmStartState so the platform can reset and re-snapshot.
     */
    @Test
    ...
    /**
     * When GTID sets have matching UUIDs and saved is contained within available,
     * validation should succeed normally (our defensive catches don't interfere).
     */
    @Test
    ...
    private fun createOperations(
        ...
        every { jdbcConnectionFactory.get() } answers { createMockConnection(serverGtids, purgedGtids) }
        ...
        val configuration = MySqlSourceConfiguration(
            ...
            incrementalConfiguration = CdcIncrementalConfiguration(
    ...
    /**
     * Builds an OpaqueStateValue matching the format expected by
     * deserializeStateUnvalidated(): the offset map has a single entry where
     * both key and value are serialized as JSON strings.
     */
    private fun buildOpaqueState(
        ...
        val keyNode = Jsons.arrayNode().apply {
            add("test")
            add(Jsons.objectNode().apply { put("server", topicPrefix) })
        }
        val valueNode = Jsons.objectNode().apply {
            put("ts_sec", 1000L)
            put("file", file)
            put("pos", pos)
            put("gtids", gtids)
        ...
        return Jsons.objectNode().apply {
            set<ObjectNode>(
                ...
                Jsons.objectNode().apply {
                    set<ObjectNode>("mysql_cdc_offset", offsetNode)
                },
            )
        }
    }
