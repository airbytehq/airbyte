...
public abstract class AbstractMySqlSourceDatatypeTest extends AbstractSourceDatabaseTypeTest {
  ...
  protected static final Logger LOGGER =
      LoggerFactory.getLogger(AbstractMySqlSourceDatatypeTest.class);
  ...
  protected void initTests() {
    ...
    addDataTypeTestData(TestDataHolder.builder()
        ...
        .addExpectedValues(
            null, String.valueOf(Math.pow(10, 308)), String.valueOf(1 / Math.pow(10, 45)), "10.5")
        .build());
    ...
    for (final String fullSourceType : Set.of("datetime", "datetime not null default now()")) {
      addDataTypeTestData(TestDataHolder.builder()
          ...
          .addInsertValues(
              "'2005-10-10 23:22:21'", "'2013-09-05T10:10:02'", "'2013-09-06T10:10:02'")
    ...
    final Set<String> charTypes =
        Stream.of(MysqlType.CHAR, MysqlType.VARCHAR).map(Enum::name).collect(Collectors.toSet());
    ...
    final Set<String> blobTypes = Stream.of(
    ...
    addDataTypeTestData(TestDataHolder.builder()
        ...
        .airbyteType(JsonSchemaType.STRING_BASE_64)
        .fullSourceDataType(
            MysqlType.VARBINARY.name() + "(20000)") // size should be enough to save test.png
        .addInsertValues(
            "null", "'test'", "'тест'", String.format("FROM_BASE64('%s')", getFileDataInBase64()))
        .addExpectedValues(null, "dGVzdA==", "0YLQtdGB0YI=", getFileDataInBase64())
        .build());

    final Set<String> textTypes = Stream.of(
  ...
  private String getFileDataInBase64() {
    final File file =
        new File(getClass().getClassLoader().getResource("test.png").getFile());
    try {
      return Base64.encodeBase64String(FileUtils.readFileToByteArray(file));
    } catch (final IOException e) {
      LOGGER.error(String.format(
          "Fail to read the file: %s. Error: %s", file.getAbsoluteFile(), e.getMessage()));
    }
    return null;
  }
}
