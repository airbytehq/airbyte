...
from source_apple_search_ads.with_campaign_streams import Adgroups
...
def test_adgroups_request_params(patch_base_class):
    stream = Adgroups(org_id=DEFAULT_CONFIG["org_id"], authenticator=NoAuth)
...
def test_adgroups_next_page_token(patch_base_class):
    stream = Adgroups(org_id=DEFAULT_CONFIG["org_id"], authenticator=NoAuth)
...
def test_adgroups_parse_response(patch_base_class):
    stream = Adgroups(org_id=DEFAULT_CONFIG["org_id"], authenticator=NoAuth)
...
def test_adgroups_request_headers(patch_base_class):
    stream = Adgroups(org_id=DEFAULT_CONFIG["org_id"], authenticator=NoAuth)
    inputs = {"stream_slice": None, "stream_state": None, "next_page_token": None}
...
def test_adgroups_http_method(patch_base_class):
    stream = Adgroups(org_id=DEFAULT_CONFIG["org_id"], authenticator=NoAuth)
    expected_method = "GET"
...
def test_stream_slices(patch_base_class):
    ...
    stream = Adgroups(org_id=DEFAULT_CONFIG["org_id"], authenticator=auth)
...
def test_adgroups_should_retry(patch_base_class, http_status, should_retry):
...
def test_adgroups_backoff_time(patch_base_class):
    response_mock = MagicMock()
    stream = Adgroups(org_id=DEFAULT_CONFIG["org_id"], authenticator=NoAuth)
