...
from source_apple_search_ads.with_campaign_report_streams import ReportCampaigns
...
def test_report_campaigns_request_params(patch_base_class):
    stream = ReportCampaigns(org_id=DEFAULT_CONFIG["org_id"], authenticator=NoAuth)
...
def test_report_campaigns_next_page_token(patch_base_class):
    stream = ReportCampaigns(org_id=DEFAULT_CONFIG["org_id"], authenticator=NoAuth)
...
def test_report_campaigns_parse_response(patch_base_class):
    stream = ReportCampaigns(org_id=DEFAULT_CONFIG["org_id"], authenticator=NoAuth)

    records = [
        {"key": "value", "metadata": {"campaignId": 1001}},
        {"key": "value2", "metadata": {"campaignId": 1002}}
    ]
    response = MagicMock()
    response.json.return_value = {
        "data": {
...
def test_report_campaigns_request_headers(patch_base_class):
    stream = ReportCampaigns(org_id=DEFAULT_CONFIG["org_id"], authenticator=NoAuth)
    inputs = {"stream_slice": None, "stream_state": None, "next_page_token": None}
...
def test_report_campaigns_http_method(patch_base_class):
    stream = ReportCampaigns(org_id=DEFAULT_CONFIG["org_id"], authenticator=NoAuth)
    expected_method = "POST"
    assert stream.http_method == expected_method

@pytest.mark.freeze_time
def test_report_campaigns_request_body_json(patch_base_class, freezer):
...
def test_stream_slices(patch_base_class):
    ...
    stream = ReportCampaigns(org_id=DEFAULT_CONFIG["org_id"], authenticator=auth)
...
def test_report_campaigns_should_retry(patch_base_class, http_status, should_retry):
...
def test_report_campaigns_backoff_time(patch_base_class):
    response_mock = MagicMock()
    stream = ReportCampaigns(org_id=DEFAULT_CONFIG["org_id"], authenticator=NoAuth)
