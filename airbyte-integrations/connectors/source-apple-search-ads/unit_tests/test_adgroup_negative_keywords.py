...
from source_apple_search_ads.with_campaign_adgroup_streams import AdgroupNegativeKeywords
...
def setup_responses():
    ...
    responses.add(
        responses.GET,
        "https://api.searchads.apple.com/api/v4/campaigns/1001/adgroups",
        json={
            "data": [{
                "id": 4001
            }],
...
def test_adgroup_negative_keywords_request_params(patch_base_class):
    stream = AdgroupNegativeKeywords(org_id=DEFAULT_CONFIG["org_id"], authenticator=NoAuth)
...
def test_adgroup_negative_keywords_next_page_token(patch_base_class):
    stream = AdgroupNegativeKeywords(org_id=DEFAULT_CONFIG["org_id"], authenticator=NoAuth)
...
def test_adgroup_negative_keywords_parse_response(patch_base_class):
    stream = AdgroupNegativeKeywords(org_id=DEFAULT_CONFIG["org_id"], authenticator=NoAuth)
...
def test_adgroup_negative_keywords_request_headers(patch_base_class):
    stream = AdgroupNegativeKeywords(org_id=DEFAULT_CONFIG["org_id"], authenticator=NoAuth)
    inputs = {"stream_slice": None, "stream_state": None, "next_page_token": None}
...
def test_adgroup_negative_keywords_http_method(patch_base_class):
    stream = AdgroupNegativeKeywords(org_id=DEFAULT_CONFIG["org_id"], authenticator=NoAuth)
    expected_method = "GET"
...
def test_stream_slices(patch_base_class):
    ...
    stream = AdgroupNegativeKeywords(org_id=DEFAULT_CONFIG["org_id"], authenticator=auth)
    inputs = {"sync_mode": SyncMode.incremental, "cursor_field": [], "stream_state": {}}
    expected_stream_slice = [
        {"adgroup_id": 4001, "campaign_id": 1001}
    ]
...
def test_adgroup_negative_keywords_should_retry(patch_base_class, http_status, should_retry):
...
def test_adgroup_negative_keywords_backoff_time(patch_base_class):
    response_mock = MagicMock()
    stream = AdgroupNegativeKeywords(org_id=DEFAULT_CONFIG["org_id"], authenticator=NoAuth)
