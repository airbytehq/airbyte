...
from source_apple_search_ads.with_campaign_streams import CreativeSets
...
def test_creative_sets_request_params(patch_base_class):
    stream = CreativeSets(org_id=DEFAULT_CONFIG["org_id"], authenticator=NoAuth)
...
def test_creative_sets_next_page_token(patch_base_class):
    stream = CreativeSets(org_id=DEFAULT_CONFIG["org_id"], authenticator=NoAuth)
...
def test_creative_sets_parse_response(patch_base_class):
    stream = CreativeSets(org_id=DEFAULT_CONFIG["org_id"], authenticator=NoAuth)
...
def test_creative_sets_request_headers(patch_base_class):
    stream = CreativeSets(org_id=DEFAULT_CONFIG["org_id"], authenticator=NoAuth)
    inputs = {"stream_slice": None, "stream_state": None, "next_page_token": None}
...
def test_creative_sets_http_method(patch_base_class):
    stream = CreativeSets(org_id=DEFAULT_CONFIG["org_id"], authenticator=NoAuth)
    expected_method = "POST"
    assert stream.http_method == expected_method

def test_creative_sets_request_body_json(patch_base_class):
    stream = CreativeSets(org_id=DEFAULT_CONFIG["org_id"], authenticator=NoAuth)

    inputs = {"stream_slice": {"adam_id": 1001}, "stream_state": None, "next_page_token": None}
    expected_params = {
        "selector": {
            "conditions": [{
                "field": "adamId",
...
def test_creative_sets_should_retry(patch_base_class, http_status, should_retry):
...
def test_creative_sets_backoff_time(patch_base_class):
    response_mock = MagicMock()
    stream = CreativeSets(org_id=DEFAULT_CONFIG["org_id"], authenticator=NoAuth)
