 ...
 *
 * PostgreSQL's LSN is a 64-bit unsigned integer representing a position in the Write-Ahead Log.
 * It's typically displayed as two 32-bit hexadecimal values separated by a slash (e.g.,
 * "0/16B3748").
 */
data class PostgresV2SourceCdcPosition(
    ...
    /**
     * Returns the LSN as a formatted string in PostgreSQL's standard LSN format. For example:
     * "0/16B3748"
     */
    ...
    /**
     * The LSN value can be used directly as a cursor value for the purposes of deduping records in
     * incremental syncs.
     */
    val cursorValue: Long
        get() = lsn

    override fun compareTo(other: PostgresV2SourceCdcPosition): Int = lsn.compareTo(other.lsn)
    ...
    companion object {
        ...
        fun fromLsn(lsn: Long): PostgresV2SourceCdcPosition = PostgresV2SourceCdcPosition(lsn)
    }
}
