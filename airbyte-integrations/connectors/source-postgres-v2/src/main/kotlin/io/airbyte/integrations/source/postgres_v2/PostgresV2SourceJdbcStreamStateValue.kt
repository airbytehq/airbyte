...
/**
 * State value for PostgreSQL streams.
 * Tracks checkpoint position for resumability.
 */
data class PostgresV2SourceJdbcStreamStateValue(
    @JsonProperty("cursor")
    ...
    val stateType: String = StateType.CURSOR_BASED.serialized,

    @JsonProperty("stream_name")
    val streamName: String = "",

    @JsonProperty("cursor_field")
    val cursorField: List<String> = listOf(),

    @JsonProperty("stream_namespace")
    val streamNamespace: String = "",

    @JsonProperty("cursor_record_count")
    val cursorRecordCount: Int = 0,
    ...
    val incrementalState: JsonNode? = null,
) {
    companion object {
        /** Value representing the completion of a FULL_REFRESH snapshot. */
        val snapshotCompleted: OpaqueStateValue
            get() = Jsons.valueToTree(PostgresV2SourceJdbcStreamStateValue(stateType = "primary_key"))
        ...
        fun cursorIncrementalCheckpoint(
            ...
            return when (cursorCheckpoint.isNull) {
                ...
                false ->
                    Jsons.valueToTree(
                        PostgresV2SourceJdbcStreamStateValue(
