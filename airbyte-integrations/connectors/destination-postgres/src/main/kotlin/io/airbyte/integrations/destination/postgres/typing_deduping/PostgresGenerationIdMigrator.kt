...
class PostgresGenerationIdMigration(
    ...
    override fun migrateIfNecessary(
        ...
        if (!state.initialRawTableStatus.rawTableExists) {
            ...
            logger.info(
                "Skipping migration for ${stream.id.originalNamespace}.${stream.id.originalName} because the raw table doesn't exist"
            )
        ...
        if (existingRawTable.columns[JavaBaseConstants.COLUMN_NAME_AB_GENERATION_ID] != null) {
            ...
            // the state.
            logger.info(
                "Skipping migration for ${stream.id.rawNamespace}.${stream.id.rawName} because the generation_id column is already present"
            )
        } else {
            logger.info(
                "Executing migration for ${stream.id.rawNamespace}.${stream.id.rawName} for real"
            )
        ...
        if (maybeExistingFinalTable.isEmpty) {
            logger.info(
                "Stopping migration for ${stream.id.originalNamespace}.${stream.id.originalName} because the public table doesn't exist"
            )
            return Migration.MigrationResult(
                state.destinationState.copy(isAirbyteGenerationIdPresent = true),
                false
