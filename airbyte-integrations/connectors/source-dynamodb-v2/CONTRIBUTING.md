...
docker run --rm -v "$PWD/secrets:/secrets:ro" -v ddb-sock:/sockets \
  ...
  airbyte/source-dynamodb-v2:dev read --config /secrets/config.json --catalog /secrets/catalog.json
```
...
binary (`bin/source-dynamodb-v2 --read ...`, no image needed) with stdout streamed through a
