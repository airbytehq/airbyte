        ...
        try:
            ...
            for config_name in configs_to_load:
                try:
                    ...
                    all_splits = datasets.get_dataset_split_names(
                        dataset_name, 
                    ...
                    logger.warning(
                        f"Failed to discover config {config_name}: {str(inner_err)}"
                    )
        ...
        try:
            for configured_stream in catalog.streams:
                ...
                yield stream_status_as_airbyte_message(
                    airbyte_stream, AirbyteStreamStatus.STARTED
                )
                ...
                try:
                    ...
                    logger.info(
                        f"Loading dataset {dataset_name} config={config_name} split={split_name} (streaming={streaming})"
                    )
                    ...
                    logger.error(
                        f"Failed to load dataset {dataset_name} config={config_name} split={split_name}: {load_err}"
                    )
                    yield stream_status_as_airbyte_message(
                        airbyte_stream, AirbyteStreamStatus.INCOMPLETE
                    )
                    continue
                ...
                try:
                    for record in dataset:
                        ...
                        if record_count == 1:
                            ...
                            yield stream_status_as_airbyte_message(
                                airbyte_stream, AirbyteStreamStatus.RUNNING
                            )

                    logger.info(
                        f"Successfully read {record_count} records from {stream_name}"
                    )

                except Exception as read_err:
                    logger.error(
                        f"Failed to read data from {stream_name}: {str(read_err)}\n{traceback.format_exc()}"
                    )
                    yield stream_status_as_airbyte_message(
                        airbyte_stream, AirbyteStreamStatus.INCOMPLETE
                    )
                ...
                yield stream_status_as_airbyte_message(
                    airbyte_stream, AirbyteStreamStatus.COMPLETE
                )
