...
def test_real_write_to_mockapi():
        ...
        streams=[
            ConfiguredAirbyteStream(
                stream=AirbyteStream(
                    name="users", json_schema={"type": "object", "properties": {}}, supported_sync_modes=[SyncMode.full_refresh]
                ),
            ...
            ConfiguredAirbyteStream(
                stream=AirbyteStream(
                    name="deals", json_schema={"type": "object", "properties": {}}, supported_sync_modes=[SyncMode.full_refresh]
                ),
                sync_mode=SyncMode.full_refresh,
                destination_sync_mode=DestinationSyncMode.append,
            ),
    ...
    test_messages = [
        ...
        AirbyteMessage(
            ...
            record=AirbyteRecordMessage(
                stream="deals",
                data={"title": f"Test Deal Gamma {timestamp}", "amount": 99999, "avatar": "https://logo.clearbit.com/testcompany.com"},
                emitted_at=int(datetime.now().timestamp() * 1000),
            ),
        ),
        AirbyteMessage(
            ...
            record=AirbyteRecordMessage(
                stream="deals",
                data={"deal_name": f"Test Deal Delta {timestamp}", "value": 88888, "image": "https://logo.clearbit.com/testdelta.com"},
                emitted_at=int(datetime.now().timestamp() * 1000),
            ),
        ),
    ]
