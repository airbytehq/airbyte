...
class BulkSalesforceStream(SalesforceStream):
    ...
    def filter_null_byte(self, s: str):
        """
        https://github.com/airbytehq/airbyte/issues/8300
        """
        res = s.replace("\x00", "")
        if len(res) < len(s):
            self.logger.warning("String size reduced %d -> %d, removed null byte(s)", len(s), len(res))
        return res

    def download_data(self, url: str) -> Tuple[int, dict]:
        job_data = self._send_http_request("GET", f"{url}/results")
        decoded_content = self.filter_null_byte(job_data.content.decode("utf-8"))
