...
class ExactStream(HttpStream, ABC):
    ...
    `class ExactStream(HttpStream, ABC)` which is the current class
    `class Customers(ExactStream)` contains behavior to pull data for customers using v1/customers
    `class Employees(ExactStream)` contains behavior to pull data for employees using v1/employees

    If some streams implement incremental sync, it is typical to create another class
    `class IncrementalExactStream((ExactStream), ABC)` then have concrete stream implementations extend it. An example
    is provided below.
