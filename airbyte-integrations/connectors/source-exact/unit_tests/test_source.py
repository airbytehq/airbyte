...
def test_check_connection_failure_forbidden(requests_mock):
    ...
    assert source.check_connection(logger_mock, config_mock) == (False, "Exception happened during connection check. Validate that the access_token is still valid at this point. Details\n401 Client Error: None for url: https://start.exactonline.nl/api/v1/current/Me")
