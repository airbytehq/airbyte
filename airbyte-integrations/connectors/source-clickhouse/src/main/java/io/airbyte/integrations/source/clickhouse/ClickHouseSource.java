...
public class ClickHouseSource extends AbstractJdbcSource implements Source {
   ...
   * get it but the ClickHouse JDBC driver returns an empty result set from the method {@link
   * ru.yandex.clickhouse.ClickHouseDatabaseMetadata#getPrimaryKeys}. That's why we have to query
   * the system table mentioned here
   ...
   * setting fetch size to 1000) for ClickHouse is cause method {@link
   * ru.yandex.clickhouse.ClickHouseConnectionImpl#setAutoCommit} is empty and method {@link
   * ru.yandex.clickhouse.ClickHouseStatementImpl#setFetchSize} is empty
   */
