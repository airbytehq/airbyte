...
```
./gradlew :airbyte-integrations:connectors:destination-milvus:build
```
...
to generate the necessary credentials. Then create a file `secrets/config.json` conforming to the `destination_milvus/spec.json` file.
...
```
./gradlew :airbyte-integrations:connectors:destination-milvus:airbyteDocker
```
...
```
docker run --rm airbyte/destination-milvus:dev spec
docker run --rm -v $(pwd)/secrets:/secrets airbyte/destination-milvus:dev check --config /secrets/config.json
# messages.jsonl is a file containing line-separated JSON representing AirbyteMessages
cat messages.jsonl | docker run --rm -v $(pwd)/secrets:/secrets -v $(pwd)/integration_tests:/integration_tests airbyte/destination-milvus:dev write --config /secrets/config.json --catalog /integration_tests/configured_catalog.json
```
## Testing
