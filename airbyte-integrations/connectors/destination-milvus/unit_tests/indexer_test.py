...
from airbyte_cdk.models.airbyte_protocol import DestinationSyncMode, AirbyteStream, SyncMode
...
class TestMilvusIndexer(unittest.TestCase):
    ...
    def test_check_handles_failure_conditions(self, MockCollection, MockConnections):
        ...
        mock_collection.describe.return_value = {"auto_id": True, "fields": [{"name": self.mock_config.vector_field, "type": DataType.INT32}]}
        result = self.milvus_indexer.check()
        ...
        mock_collection.describe.return_value = {"auto_id": True, "fields": [{"name": self.mock_config.vector_field, "type": DataType.FLOAT_VECTOR, "params": {"dim": 64}}]}
        result = self.milvus_indexer.check()
        self.assertEqual(result, f"Vector field {self.mock_config.vector_field} is not a {self.mock_embedder.embedding_dimensions}-dimensional vector")
    ...
    def test_index_calls_insert(self):
        ...
        mock_collection.insert.assert_called_with([{"key": "value", "vector": self.mock_embedder.embed_texts.return_value[0], "text": "some content"}])
