...
public abstract class StarburstGalaxyDestinationAcceptanceTest extends DestinationAcceptanceTest {
  ...
  private static final Logger LOGGER =
      LoggerFactory.getLogger(StarburstGalaxyDestinationAcceptanceTest.class);
  private static final String INPUT_FILES_BASE_LOCATION = "testdata/";
  private static final StandardNameTransformer nameTransformer =
      new StarburstGalaxyNameTransformer();
  ...
  protected List<JsonNode> retrieveRecords(
    ...
    final String schemaName = StreamCopierFactory.getSchema(
        namespace, galaxyDestinationConfig.galaxyCatalogSchema(), nameTransformer);
    final JsonFieldNameUpdater nameUpdater =
        AvroRecordHelper.getFieldNameUpdater(streamName, namespace, streamSchema);
    return executeQuery(ctx -> ctx
        .select(asterisk())
  ...
  protected void tearDown(final TestDestinationEnv testEnv) throws SQLException {
    // clean up database
    final List<JsonNode> schemas = executeQuery(format(
        "SHOW SCHEMAS LIKE '%s'",
        galaxyDestinationConfig.galaxyCatalogSchema().toLowerCase(ENGLISH)));
    schemas.stream().map(node -> node.get("Schema").asText()).forEach(schema -> {
      try {
        final List<JsonNode> tables = executeQuery(
            format("SHOW TABLES FROM %s", galaxyDestinationConfig.galaxyCatalogSchema()));
  ...
  private List<JsonNode> executeQuery(final String query) throws SQLException {
    return executeQuery(ctx -> ctx.resultQuery(query).stream()
  ...
  public void testPromoteSourceSchemaChanges() throws Exception {
    ...
    testStreamSync(
        OVERWRITE,
        sampleStream,
        "schema-overwrite.json",
        "data-overwrite.json",
        "expected-schema-overwrite.json");
    testStreamSync(
        APPEND,
        sampleStream,
        "schema-append.json",
        "data-append.json",
        "expected-schema-append.json");
    assertEquals(
        2,
        executeQuery(format(
                "SELECT COUNT(*) FROM %s.%s",
                galaxyDestinationConfig.galaxyCatalogSchema(), sampleStream))
            .get(0)
            .get("_col0")
            .asInt());
  }

  private void testStreamSync(
    ...
    final AirbyteMessage overwriteMessage =
        createRecordMessage(streamName, getTestDataFromResourceJson(dataFileName));
    runDestinationWrite(
        getCommonCatalog(streamName, overwriteSchema, syncMode), configJson, overwriteMessage);
  ...
  private void validateTableSchema(final String streamName, final String expectedSchemaFileName)
      throws SQLException {
    final List<JsonNode> describeRecords = executeQuery(
        format("DESCRIBE %s.%s", galaxyDestinationConfig.galaxyCatalogSchema(), streamName));
    final Map<String, String> actualDataTypes = describeRecords.stream()
        .collect(Collectors.toMap(
            column -> column.get("Column").asText(),
            column -> column.get("Type").asText()));
    ...
    expectedDataTypes
        .fields()
        .forEachRemaining(
            field -> assertEquals(field.getValue().asText(), actualDataTypes.get(field.getKey())));
  }
  ...
  public void testJsonV0Types() throws Exception {
    testDifferentTypes(
        ...
        "datatypeV0.json",
        "dataV0.json",
        "expected-datatypeV0.json",
        "expected-dataV0.json");
  }
  ...
  public void testJsonV1Types() throws Exception {
    testDifferentTypes(
        ...
        "datatypeV1.json",
        "dataV1.json",
        "expected-datatypeV1.json",
        "expected-dataV1.json");
  }

  private void testDifferentTypes(
      final String streamName,
    ...
    final AirbyteMessage datatypeMessage =
        createRecordMessage(streamName, getTestDataFromResourceJson(dataFile));
    runDestinationWrite(
        getCommonCatalog(streamName, datatypeSchema, OVERWRITE), configJson, datatypeMessage);
    final JsonFieldNameUpdater nameUpdater = AvroRecordHelper.getFieldNameUpdater(
        streamName, galaxyDestinationConfig.galaxyCatalogSchema(), datatypeSchema);
    validateTableSchema(streamName, expectedDataTypeFile);

    final List<JsonNode> records = executeQuery(ctx -> ctx
        .select(asterisk())
    ...
    expectedData
        .fields()
        .forEachRemaining(field -> assertEquals(field.getValue(), actualData.get(field.getKey())));
  }
  ...
  public static ConfiguredAirbyteCatalog getCommonCatalog(
      final String stream, final JsonNode schema, final DestinationSyncMode destinationSyncMode) {
    return new ConfiguredAirbyteCatalog()
        .withStreams(Lists.newArrayList(new ConfiguredAirbyteStream()
            ...
            .withSyncMode(FULL_REFRESH)
            .withDestinationSyncMode(destinationSyncMode)));
  }

  private static void runDestinationWrite(
      final ConfiguredAirbyteCatalog catalog,
      final JsonNode config,
      final AirbyteMessage... messages)
      throws Exception {
  ...
  private static JsonNode getTestDataFromResourceJson(final String fileName) {
    try {
      final String fileContent =
          readString(Path.of(Objects.requireNonNull(StarburstGalaxyDestinationAcceptanceTest.class
                  .getClassLoader()
