 ...
 * </ul>
 */
public class StarburstGalaxyS3StreamCopier extends StarburstGalaxyStreamCopier {
  ...
  public StarburstGalaxyS3StreamCopier(
    ...
    Schema avroSchema = getAvroSchema(
        ...
        configuredStream.getStream().getNamespace(),
        configuredStream.getStream().getJsonSchema());
  ...
  static Schema getAvroSchema(String streamName, String namespace, JsonNode jsonSchema) {
    ...
    return schemaConverter.getAvroSchema(
        jsonSchema, streamName, namespace, true, true, false, true);
  }
  ...
  public void write(final UUID id, final AirbyteRecordMessage recordMessage, final String fileName)
      throws Exception {
    recordMessage.setEmittedAt(
        recordMessage.getEmittedAt() * 1000); // Corresponding Galaxy type expects micro precision.
  ...
  @Override
  protected String getTmpTableMetadataFileName() throws IOException {
    ...
    GetObjectRequest getObjectRequest = new GetObjectRequest(
        s3Config.getBucketName(), tmpTableBasePath + "/metadata/" + VERSION_HINT_FILENAME);
    S3Object object = s3Client.getObject(getObjectRequest);
    String currentMetadataFileVersion =
        new String(object.getObjectContent().readAllBytes(), UTF_8).strip();
    LOGGER.info(
        "[Stream {}] Current metadata file version {}", streamName, currentMetadataFileVersion);
    String metadataJsonFile = "v" + currentMetadataFileVersion + ".metadata.json";
    String newMetadataJsonFileName = "0".repeat(5 - currentMetadataFileVersion.length())
        + currentMetadataFileVersion + "-" + randomUUID() + ".metadata.json";
    ...
    LOGGER.info(
        "New metadata file: {}/{}/{}", tmpTableBasePath, "metadata", newMetadataJsonFileName);
  ...
  public String generateMergeStatement(final String destTableName) {
    String fields = String.join(
        ", ", galaxySchema.columns().stream().map(ColumnMetadata::name).collect(joining(", ")));
    String insertData = format(
        "INSERT INTO %s.%s(%s) SELECT %s FROM %s.%s",
        quotedSchemaName, destTableName, fields, fields, quotedSchemaName, tmpTableName);
