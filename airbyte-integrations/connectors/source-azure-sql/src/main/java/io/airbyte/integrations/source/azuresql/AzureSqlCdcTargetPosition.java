...
public class AzureSqlCdcTargetPosition implements CdcTargetPosition {

  private static final Logger LOGGER = LoggerFactory.getLogger(AzureSqlCdcTargetPosition.class);
  public final Lsn targetLsn;

  public AzureSqlCdcTargetPosition(final Lsn targetLsn) {
  ...
  public boolean equals(final Object o) {
    ...
    final AzureSqlCdcTargetPosition that = (AzureSqlCdcTargetPosition) o;
  ...
  public static AzureSqlCdcTargetPosition getTargetPosition(final JdbcDatabase database, final String dbName/*not used in azure sql*/) {
    try {

      final List<JsonNode> jsonNodes = database
          .bufferedResultSetQuery(conn -> conn.createStatement().executeQuery(
              "SELECT sys.fn_cdc_get_max_lsn() AS max_lsn;"), JdbcUtils.getDefaultSourceOperations()::rowToJson);
      ...
      if (jsonNodes.get(0).get("max_lsn") != null) {
        ...
        return new AzureSqlCdcTargetPosition(maxLsn);
      } else {
