...
| task | medium | child | date_updated | `date_updated_gt`/`date_updated_lt` query params (Unix ms) | incremental | Added by session #2 |
...
- **Child streams (10 streams):** `folder`, `list`, `list_comments`, `list_custom_fields`, `space`, `space_tags`, `team_custom_fields`, `team_goals`, `time_tracking`, `time_tracking_tags` — partitioned via `SubstreamPartitionRouter`. A follow-up session should evaluate whether these can be made incremental independently or via `incremental_dependency`.
- **Now incremental:** `task` — uses `date_updated_gt`/`date_updated_lt` query params with `DatetimeBasedCursor` on `date_updated` field (Unix ms).
