# Outbrain-Amplify source connector
...
```
poetry run source-outbrain-amplify spec
poetry run source-outbrain-amplify check --config secrets/config.json
poetry run source-outbrain-amplify discover --config secrets/config.json
poetry run source-outbrain-amplify read --config secrets/config.json --catalog sample_files/configured_catalog.json
```
...
An image will be available on your host with the tag `airbyte/source-outbrain-amplify:dev`.
...
4. Make sure the connector documentation and its changelog is up to date (`docs/integrations/sources/outbrain-amplify.md`).
