...
sealed class SapHanaPartitionReader<P : JdbcPartition<*>>(
    val partition: P,
) : PartitionReader {
    ...
    val streamRecordConsumer: StreamRecordConsumer =
        streamState.streamFeedBootstrap
            .streamRecordConsumers()[streamState.streamFeedBootstrap.feed.id]!!
    ...
    fun out(row: SelectQuerier.ResultRow) {
        val sapHanaStreamState = streamState as SapHanaJdbcStreamState
    ...
    private fun decorateTriggerBasedCdcRecord(row: SelectQuerier.ResultRow): NativeRecordPayload {
        ...
        val transactionTimestamp = row.data[TriggerTableConfig.CURSOR_FIELD.id]
        ...
        validatedRecord[TriggerTableConfig.CURSOR_FIELD.id] = transactionTimestamp
...
/** SAP HANA implementation of [PartitionReader] which reads the [partition] in its entirety. */
class SapHanaNonResumablePartitionReader<P : JdbcPartition<*>>(
    partition: P,
    val deleteQuerier: DeleteQuerier
) : SapHanaPartitionReader<P>(partition) {
    ...
    override suspend fun run() {
        ...
        if (
            ...
            val whereClause = "\"${TriggerTableConfig.CURSOR_FIELD.id}\" < '${latestCheckpoint}'"
