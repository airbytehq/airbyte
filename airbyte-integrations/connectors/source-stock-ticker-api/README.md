# Stock Ticker Api Source

This is the repository for the Stock Ticker Api source connector, written in Python.
...
to generate the necessary credentials. Then create a file `secrets/config.json` conforming to the `source_stock_ticker_api/spec.yaml` file.
Note that any directory named `secrets` is gitignored across the entire Airbyte repo, so there is no danger of accidentally checking in sensitive information.
See `integration_tests/sample_config.json` for a sample config file.

**If you are an Airbyte core member**, copy the credentials in Lastpass under the secret name `source stock-ticker-api test creds`
...
```
./gradlew :airbyte-integrations:connectors:source-stock-ticker-api:airbyteDocker
```
...
docker run --rm -v $(pwd)/secrets:/secrets -v $(pwd)/integration_tests:/integration_tests airbyte/source-stock-ticker-api:dev read --config /secrets/config.json --catalog /integration_tests/configured_catalog.json
```
## Testing
...
To run unit tests:
```
./gradlew :airbyte-integrations:connectors:source-stock-ticker-api:unitTest
```
