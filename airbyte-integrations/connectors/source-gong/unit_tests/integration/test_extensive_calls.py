...
class TestExtensiveCalls(TestCase):
    @HttpMocker()
    def test_read_records(self, http_mocker: HttpMocker) -> None:
        http_mocker.post(
            RequestBuilder.extensive_calls_endpoint().with_any_query_params().build(),
            extensive_calls_response(call_id=CALL_ID),
        )

        output = _read(config_builder=config())
        assert len(output.records) == 1
        assert output.records[0].record.data["metaData"]["id"] == CALL_ID
