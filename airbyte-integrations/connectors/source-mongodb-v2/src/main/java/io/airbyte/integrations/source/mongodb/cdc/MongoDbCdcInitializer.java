...
public class MongoDbCdcInitializer {
  ...
  public List<AutoCloseableIterator<AirbyteMessage>> createCdcIterators(
    ...
    try {
      ...
      if (MongoUtil.isUnauthorizedMongoCommandException(e)) {
        LOGGER.error("MongoDB user lacks permissions required for CDC change streams.", e);
        throw new ConfigErrorException(MongoConstants.CDC_CHANGE_STREAM_PERMISSION_ERROR_MESSAGE, e);
      }
      throw e;
    }
