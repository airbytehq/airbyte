...
public class MongoDbSource extends BaseConnector implements Source {
  ...
  private RuntimeException handlePotentialCdcError(final Exception e) {
    if (e instanceof MongoCommandException mongoCommandException
        && mongoCommandException.getErrorCode() == MongoConstants.MONGODB_UNAUTHORIZED_ERROR_CODE) {
      LOGGER.error("MongoDB unauthorized error detected during CDC sync. Original error: {}", e.getMessage(), e);
