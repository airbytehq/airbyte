...
public class MongoDbSource extends BaseConnector implements Source {
  ...
  /**
   * Wraps an iterator to catch known CDC errors and provide helpful, actionable error messages to
   * users.
   */
  ...
  private static RuntimeException handleKnownCdcException(final Exception e) {
    if (MongoUtil.isUnauthorizedChangeStreamException(e)) {
      LOGGER.error("Unauthorized MongoDB change stream error detected during CDC sync. Original error: {}", e.getMessage(), e);
