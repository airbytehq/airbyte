...
public class MongoDbSource extends BaseConnector implements Source {
  ...
  /**
   * Wraps an iterator to convert known CDC exceptions into configuration errors.
   *
  ...
  private AutoCloseableIterator<AirbyteMessage> wrapIteratorWithCdcErrorHandling(
    ...
    return new AutoCloseableIterator<>() {
      ...
      private RuntimeException handlePotentialCdcError(final Exception e) {
        if (MongoUtil.isUnauthorizedException(e)) {
          LOGGER.error("Unauthorized MongoDB change stream access detected during CDC sync. Original error: {}", e.getMessage(), e);
          throw new ConfigErrorException(MongoConstants.CHANGE_STREAM_UNAUTHORIZED_ERROR_MESSAGE, e, e.getMessage());
        }
