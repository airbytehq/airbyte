...
public class MongoUtil {
   ...
   * error occurs when the configured MongoDB user lacks the privileges required for the requested
   * operation, e.g. the {@code changeStream} action needed to open a change stream.
   *
   * @param exception The exception to check.
   * @return true if the exception chain contains a {@link MongoCommandException} with error code 13,
   *         false otherwise.
   */
  public static boolean isMongoCommandUnauthorizedException(final Throwable exception) {
    Throwable current = exception;
    while (current != null) {
      if (current instanceof MongoCommandException mongoException
