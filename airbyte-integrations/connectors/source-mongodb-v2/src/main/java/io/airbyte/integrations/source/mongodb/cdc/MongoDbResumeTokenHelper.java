...
public class MongoDbResumeTokenHelper {
  ...
  public static BsonDocument getMostRecentResumeTokenForDatabases(final MongoClient mongoClient,
    ...
    try (final MongoChangeStreamCursor<ChangeStreamDocument<BsonDocument>> eventStreamCursor = eventStream.cursor()) {
      ...
      if (isUnauthorizedException(e)) {
        LOGGER.error("MongoDB authorization error detected during CDC change-stream initialization. Original error: {}", e.getMessage(), e);
  ...
  private static boolean isUnauthorizedException(final MongoCommandException e) {
    return e.getErrorCode() == MongoConstants.UNAUTHORIZED_ERROR_CODE ||
        MongoConstants.UNAUTHORIZED_ERROR_CODE_NAME.equals(e.getErrorCodeName());
  }
