...
public class MongoDbResumeTokenHelper {
  ...
  private static final String CHANGE_STREAM_UNAUTHORIZED_MESSAGE = "MongoDB user lacks change stream permissions.";
  ...
  public static BsonDocument getMostRecentResumeTokenForDatabases(final MongoClient mongoClient,
    ...
    try (final MongoChangeStreamCursor<ChangeStreamDocument<BsonDocument>> eventStreamCursor = eventStream.cursor()) {
      ...
      if (e.getErrorCode() == UNAUTHORIZED_ERROR_CODE) {
        throw new ConfigErrorException(CHANGE_STREAM_UNAUTHORIZED_MESSAGE, e, e.getMessage());
      }
      throw e;
    }
  }
