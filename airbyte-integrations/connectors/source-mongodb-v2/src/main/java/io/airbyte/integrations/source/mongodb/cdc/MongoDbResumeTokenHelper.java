  ...
  public static BsonDocument getMostRecentResumeTokenForDatabases(final MongoClient mongoClient,
    ...
    try (final MongoChangeStreamCursor<ChangeStreamDocument<BsonDocument>> eventStreamCursor = eventStream.cursor()) {
      ...
      if (MongoUtil.isUnauthorizedException(e)) {
        LOGGER.error("MongoDB change stream authorization failed.", e);
        throw new ConfigErrorException(MongoConstants.CDC_CHANGE_STREAM_UNAUTHORIZED_ERROR_MESSAGE, e,
            MongoConstants.CDC_CHANGE_STREAM_UNAUTHORIZED_INTERNAL_MESSAGE + " Raw error: " + e.getMessage());
      }
      throw e;
    }
  }
