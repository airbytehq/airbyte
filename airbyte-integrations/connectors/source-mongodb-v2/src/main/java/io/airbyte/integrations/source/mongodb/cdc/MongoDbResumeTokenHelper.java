  ...
  public static BsonDocument getMostRecentResumeTokenForDatabases(final MongoClient mongoClient,
    ...
    try (final MongoChangeStreamCursor<ChangeStreamDocument<BsonDocument>> eventStreamCursor = eventStream.cursor()) {
      ...
      if (MongoUtil.isMongoUnauthorizedException(e)) {
        LOGGER.error("MongoDB Unauthorized error detected while opening change stream. Original error: {}", e.getMessage(), e);
        throw new ConfigErrorException(MongoConstants.MONGO_UNAUTHORIZED_ERROR_MESSAGE, e);
      }
      throw e;
    }
  }
