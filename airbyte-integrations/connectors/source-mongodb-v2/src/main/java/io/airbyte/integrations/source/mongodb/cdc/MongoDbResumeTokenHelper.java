  ...
  public static BsonDocument getMostRecentResumeTokenForDatabases(final MongoClient mongoClient,
    ...
    try (final MongoChangeStreamCursor<ChangeStreamDocument<BsonDocument>> eventStreamCursor = eventStream.cursor()) {
      ...
      if (MongoUtil.isUnauthorizedException(e)) {
        LOGGER.error("MongoDB authorization error detected during CDC resume token initialization.", e);
        throw new ConfigErrorException(
            MongoConstants.CDC_CHANGE_STREAM_UNAUTHORIZED_ERROR_MESSAGE,
            e,
            "MongoDB authorization failed during CDC resume token initialization: " + e.getMessage());
      }
      throw e;
    }
  }
