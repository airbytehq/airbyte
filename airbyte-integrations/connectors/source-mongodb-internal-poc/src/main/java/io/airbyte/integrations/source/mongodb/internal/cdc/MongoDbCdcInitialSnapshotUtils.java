...
public class MongoDbCdcInitialSnapshotUtils {
    ...
    /**
     * Returns the list of configured Airbyte streams that need to perform the initial snapshot
     * portion of a CDC sync.  This includes streams that:
     * <ol>
     ...
     * </ol>
     *
     * In addition, if the saved offset is no longer present in the server, all streams are used
     * in the initial snapshot in order to restore the offset to an existing value.
     *
     * @param stateManager The {@link MongoDbStateManager} that contains information about each stream's progress.
     * @param fullCatalog The fully configured Airbyte catalog.
     * @param savedOffsetAfterResumeToken Boolean value that indicates whether the offset exists on the server.
    ...
    public static List<ConfiguredAirbyteStream> getStreamsForInitialSnapshot(final MongoDbStateManager stateManager,
        ...
        if (!savedOffsetAfterResumeToken) {
            /*
             * If the saved offset is not after the most recent resume token retrieved from the server,
             * re-sync everything via initial snapshot as we have lost track of the changes.  This occurs
             * when the oplog cycles faster than a sync interval, resulting in the stored offset in our state
             * being removed from the oplog.
             */
