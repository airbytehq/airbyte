...
public class MongoDbSource extends BaseConnector implements Source {
  ...
  public AirbyteConnectionStatus check(final JsonNode config) {
    try (final MongoClient mongoClient = MongoConnectionUtils.createMongoClient(config)) {
      ...
      /*
       * Perform the authorized collections check before the cluster type check. The MongoDB
       * Java driver needs to actually execute a command in order to fetch the cluster description.
       * Querying for the authorized collections guarantees that the cluster description
