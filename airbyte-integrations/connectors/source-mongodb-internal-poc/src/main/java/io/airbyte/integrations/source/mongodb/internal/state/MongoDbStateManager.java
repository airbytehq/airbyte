...
public class MongoDbStateManager {
    ...
    /**
     * Map of streams (name/namespace tuple) to the current stream state information stored in the state.
     */
     ...
     * @return The {@link MongoDbStreamState} associated with the stream or an empty {@link Optional} if the stream
     *  is not currently tracked by the manager.
     */
    ...
    /**
     * Generates an {@link AirbyteStateMessage} from the state tracked by this manager.  The resulting state message
     * contains a global state object with the CDC state as the "shared state" and the individual stream states as the
     * "stream states".
     *
