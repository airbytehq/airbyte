...
from source_wildberries_seller.streams.statistics import (
    IncomeStream,
    StockStream,
    OrderStream,
    SaleStream,
...
class SourceWildberriesSeller(AbstractSource):
    def streams(self, config: Mapping[str, Any]) -> List[Stream]:
        ...
        statistics_date_from, statistics_date_to = self._prepare_dates(statistics_config)
        statistics_strict_date_from = statistics_config.get("strict_date_from", False)
    ...
    def check_connection(self, logger: logging.Logger, config: Mapping[str, Any]) -> Tuple[IsSuccess, Message | None]:
        ...
        is_success, message = self._check_statistics_config(config=statistics_config, credentials=credentials)
        if not is_success:
