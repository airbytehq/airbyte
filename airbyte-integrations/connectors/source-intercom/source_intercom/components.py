...
class IncrementalSingleSlice(StreamSlicer, JsonSchemaMixin):
    ...
    def _get_max_state_value(self, current_state_value: Optional[Union[int,str]], last_record_value: Optional[Union[int,str]]) -> Optional[Union[int,str]]:
...
class IncrementalSubstreamSlicer(StreamSlicer, JsonSchemaMixin):
    ...
    def update_cursor(self, stream_slice: StreamSlice, last_record: Optional[Record] = None):
        ...
        self._cursor['prior_state'] = {
            ...
            self.parent_stream_name: {self.parent_cursor_field: self.initial_state.get(self.parent_stream_name, {}).get(self.parent_cursor_field)}
            }
        self._update_substream_cursor(stream_slice, last_record)
    
    def read_parent_stream(self, sync_mode: SyncMode, cursor_field: Optional[str], stream_state: Mapping[str, Any]) -> Iterable[Mapping[str, Any]]:
            ...
            for parent_record in self.parent_stream.read_records(
                ...
                try:
                    ...
                    slice = {
                        ...
                        self.parent_stream_name: {self.parent_cursor_field: self._cursor.get(self.parent_stream_name, {}).get(self.parent_cursor_field)},
                    }
