...
This is the repository for the Xkcd configuration based source connector.
...
to generate the necessary credentials. Then create a file `secrets/config.json` conforming to the `src/source_xkcd/spec.yaml` file.
...
poetry run source-xkcd read --config secrets/config.json --catalog integration_tests/configured_catalog.json
```
