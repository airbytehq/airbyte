...
from source_amazon_ads.streams.report_streams.report_streams_v3 import ReportGenerationFailure, ReportGenerationInProgress, TooManyRequests
...
def test_display_report_stream(config):
    ...
    stream = AirbyteCampSp(config, profiles, authenticator=mock.MagicMock())
    stream_slice = {"profile": profiles[0], "reportDate": "20210725",'start_date':"20210725",'end_date':"20210725"}
...
def test_stream_slices_different_timezones(config):
    ...
    stream = AirbyteCampSp(config, [profile1, profile2], authenticator=mock.MagicMock())
    slices = list(stream.stream_slices(SyncMode.incremental, cursor_field=stream.cursor_field, stream_state={}))
...
def test_get_date_range_lazy_evaluation():
    get_date_range = partial(AirbyteCampSp.get_date_range, AirbyteCampSp)
...
def test_display_report_stream_with_custom_record_types(config_gen, custom_record_types, flag_match_error):
    ...
    stream = AirbyteCampSp(config_gen(report_record_types=custom_record_types), profiles, authenticator=mock.MagicMock())
    stream_slice = {"profile": profiles[0], "reportDate": "20210725"}
