...
class LifetimeInsights(XingStream, HttpSubStream):
    parent = Customers
    primary_key = "ad_id"
    ...
    def request_params(
        ...
        params = {}
        if self.config.get("lifetime_insights"):
            lifetime_insights = self.config.get("lifetime_insights")
            if lifetime_insights.get("fields"):
                params["fields"] = ",".join(lifetime_insights.get("fields"))
            else:
    ...
    def path(
        ...
        return f"customers/{stream_slice['customer_id']}/lifetime_insights"
