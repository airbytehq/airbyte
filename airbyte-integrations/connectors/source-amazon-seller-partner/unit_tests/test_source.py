...
def test_flat_file_all_orders_streams_have_composite_primary_key(connector_config_without_start_date, stream_name):
    """
    These reports are one row per order item, so `amazon-order-id` alone is not unique and deduplicating on it
    collapses multi-item orders into a single row. The primary key must include `order-item-id`.
    """
    streams = get_source(connector_config_without_start_date).streams(connector_config_without_start_date)
    stream = next(stream for stream in streams if stream.name == stream_name)
    assert stream.as_airbyte_stream().source_defined_primary_key == [["amazon-order-id"], ["order-item-id"]]
