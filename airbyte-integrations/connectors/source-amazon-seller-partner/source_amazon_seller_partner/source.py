        ...
        try:
            ...
            if stream_name not in stream_name_to_stream:
                ...
                return AirbyteConnectionStatus(
                    status=Status.FAILED, message=f"Stream {stream_name} not found. Available streams: {available_streams}"
                )
            ...
            try:
                ...
                if hasattr(stream, "check_availability"):
                    ...
                    if not is_available:
                        return AirbyteConnectionStatus(status=Status.FAILED, message=f"Stream {stream_name} is not available: {reason}")
                else:
                ...
                return AirbyteConnectionStatus(status=Status.FAILED, message=f"Stream {stream_name} is not available: {str(e)}")

            return AirbyteConnectionStatus(status=Status.SUCCEEDED)

        except Exception as e:
            return AirbyteConnectionStatus(status=Status.FAILED, message=f"Error checking connection: {str(e)}")
