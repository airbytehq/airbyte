...
class AWSSignature(AuthBase):
    ...
    def _get_authorization_header(self, prepared_request: requests.PreparedRequest) -> str:
        ...
        if len(url_parsed.query) > 0:
            split_query_parameters = [param.split("=") for param in url_parsed.query.split("&")]
            ordered_query_parameters = sorted(split_query_parameters, key=lambda param: (param[0], param[1]))
        else:
            ordered_query_parameters = []

        canonical_querystring = "&".join("=".join(param) for param in ordered_query_parameters)
        ...
        canonical_headers = "".join(":".join(h) + "\n" for h in ordered_headers.items())
        ...
        return "AWS4-HMAC-SHA256 Credential={}/{}, SignedHeaders={}, Signature={}".format(
