...
class IncrementalAmazonSPStream(AmazonSPStream, ABC):
    ...
    def parse_response(
        self, response: requests.Response, stream_state: Optional[Mapping[str, Any]] = None, stream_slice: Optional[Mapping[str, Any]] = None, **kwargss,
    ) -> Iterable[Mapping]:
...
class ReportsAmazonSPStream(Stream, ABC):
    ...
    def _create_prepared_request(
        self, path: str, http_method: str = "GET", headers: Optional[Mapping] = None, params: Optional[Mapping] = None, json: Any = None, data: Any = None,
    ...
    def parse_response(
        self, response: requests.Response, stream_state: Optional[Mapping[str, Any]] = None, stream_slice: Optional[Mapping[str, Any]] = None, **kwargs,
    ) -> Iterable[Mapping]:
    ...
    def read_records(
        ...
        if is_done:
            ...
            msg = f"The report for stream '{self.name}' was aborted due to a fatal error"
            raise Exception(msg)
        elif is_cancelled:
            logger.warn(f"The report for stream '{self.name}' was cancelled or there is no data to return")
        else:
            msg = f"Unknown response for stream `{self.name}`. Response body {report_payload}"
            raise Exception(msg)
...
class SellerFeedbackReports(IncrementalReportsAmazonSPStream):
    ...
    def get_transform_function(self):
        def transform_function(original_value: Any, field_schema: Dict[str, Any]) -> Any:
            if original_value and "format" in field_schema and field_schema["format"] == "date":
                ...
                if not date_format:
                    msg = f"Date format not found for Markeplace ID: {self.marketplace_id}"
                    raise KeyError(msg)
                return pendulum.from_format(original_value, date_format).to_date_string()
    ...
    def parse_document(document):
        ...
        if len(original_fieldnames) != len(SellerFeedbackReports.NORMALIZED_FIELD_NAMES):
            msg = "Original and normalized header field count does not match"
    ...
    def get_transform_function(self):
        def transform_function(original_value: Any, field_schema: Dict[str, Any]) -> Any:
            if original_value and field_schema.get("format") == "date":
                return pendulum.from_format(original_value, "MM/DD/YYYY").to_date_string()
            return original_value
...
class IncrementalAnalyticsStream(AnalyticsStream):
    ...
    def stream_slices(
        ...
        while start_date < end_date:
            ...
            slice_range = self.fixed_period_in_days if self.fixed_period_in_days != 0 else self.period_in_days
...
class SellerAnalyticsSalesAndTrafficReports(IncrementalAnalyticsStream):
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        if self.name in self._advanced_stream_options:
