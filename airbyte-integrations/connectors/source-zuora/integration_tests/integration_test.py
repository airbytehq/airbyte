...
class TestZuora:
    ...
    def test_excluded_streams_are_not_in_the_list(self, config):
        ...
        for excluded_stream in ZUORA_EXCLUDED_STREAMS:
            assert excluded_stream not in generated_stream_class_names
    ...
    def test_check_job_status(self, config):
        ...
        job_data_url = ZuoraJobStatusCheck(next(iter(job_id)), self.config).read_records(sync_mode=None)

        # Return True if there is a URL leading to a file
        assert "https://" in next(iter(job_data_url))
    ...
    def test_get_job_result(self, config):
        ...
        job_result = ZuoraGetJobResult(next(iter(job_data_url))).read_records(sync_mode=None)
