...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            {"use_global_cursor": False, "states": [], "state": {"end_date": "2024-06-10"}},
            {"end_date": "2024-06-10"},
            id="empty_partitions_use_existing_global_cursor",
        ),
        pytest.param(
            {"use_global_cursor": False, "states": []},
            {},
            id="empty_partitions_without_global_cursor",
        ),
        pytest.param(
            {"end_date": "2024-06-10"},
            {"end_date": "2024-06-10"},
...
def test_linkedin_ads_per_partition_to_global_state_migration(components_module, stream_state, expected_state):
    declarative_stream = MagicMock()
    declarative_stream.incremental_sync.cursor_field = "end_date"
    declarative_stream.parameters = {}
    migration = components_module.LinkedInAdsPerPartitionToGlobalStateMigration(
        declarative_stream=declarative_stream,
        config={},
    )

    assert migration.migrate(stream_state) == expected_state
