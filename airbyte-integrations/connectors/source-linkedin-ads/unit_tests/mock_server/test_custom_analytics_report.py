...
_STREAM_NAME = "custom_analytics_report"
...
def _create_analytics_record(
    ...
    """
    Create an analytics record for custom_analytics_report.
...
class TestCustomAnalyticsReportStream(TestCase):
    """
    Tests for the LinkedIn Ads 'custom_analytics_report' stream.
    ...
    @HttpMocker()
    def test_multiple_parent_campaigns(self, http_mocker: HttpMocker):
        """
        ...
        http_mocker.get(
            ...
            .with_records(
                [
                    _create_analytics_record(1001, "2024-06-01", 1000, 50),
                    _create_analytics_record(1002, "2024-06-01", 2000, 100),
                ]
            )
            .build(),
        )
        ...
        # We expect at least 2 records (one per campaign)
        # The exact count may vary based on how the analytics endpoint is called
        assert len(output.records) >= 2
        campaign_ids = [r.record.data["sponsoredCampaign"] for r in output.records]
        assert "1001" in campaign_ids or "1002" in campaign_ids
