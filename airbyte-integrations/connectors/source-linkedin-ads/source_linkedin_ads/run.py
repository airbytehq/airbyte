...
def _get_source(args: List[str]):
    ...
    try:
        return SourceLinkedinAds(
            SourceLinkedinAds.read_catalog(catalog_path) if catalog_path else None,
            SourceLinkedinAds.read_config(config_path) if config_path else None,
            SourceLinkedinAds.read_state(state_path) if state_path else None,
        )
...
def run():
    ...
    MigrateCredentials.migrate(_args, source)
    if source:
        launch(source, _args)
