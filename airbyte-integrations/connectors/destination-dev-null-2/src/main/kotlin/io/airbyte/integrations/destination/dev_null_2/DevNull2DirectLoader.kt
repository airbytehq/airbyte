...
abstract class DevNull2DirectLoader(
...
@Singleton
class DevNull2DirectLoaderFactory(private val config: DevNull2Configuration) :
    DirectLoaderFactory<DevNull2DirectLoader> {
    private val log = KotlinLogging.logger {}

    override fun create(
        streamDescriptor: DestinationStream.Descriptor,
        part: Int
    ): DevNull2DirectLoader {
...
class SilentDirectLoader(config: DevNull2Configuration) : DevNull2DirectLoader(config) {
    override fun acceptInner(record: DestinationRecordRaw) {
        /* Do nothing */
    }
}

class ThrottledDirectLoader(config: DevNull2Configuration, private val millisPerRecord: Long) :
    DevNull2DirectLoader(config) {
