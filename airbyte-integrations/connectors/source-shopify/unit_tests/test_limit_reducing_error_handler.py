...
class TestOrdersLimitReducingErrorHandler:
    def test_orders_stream_500_error_handling(self, requests_mock):
        ...
        requests_mock.get(
            "https://test-shop.myshopify.com/admin/api/2026-07/events.json?filter=Order&verb=destroy",
            [{"status_code": 200, "json": {"events": []}}],
        )
        # Simulate initial URL with 500 errors, then success with pagination
        requests_mock.get(
            "https://test-shop.myshopify.com/admin/api/2026-07/orders.json?limit=250&status=any",
            [
                ...
                {"status_code": 500},  # Retry with 250 fails again
                {
                    ...
                    "json": ORDERS_PAGE_1,
                    "headers": {
                        "Link": '<https://test-shop.myshopify.com/admin/api/2026-07/orders.json?limit=250&page_info=page1>; rel="next"'
                    },
                },
            ],
        )
        ...
        requests_mock.get(
            "https://test-shop.myshopify.com/admin/api/2026-07/orders.json?limit=125&status=any",
            [
                {
                    "status_code": 200,
                    "json": ORDERS_PAGE_1,
                    "headers": {
                        "Link": '<https://test-shop.myshopify.com/admin/api/2026-07/orders.json?limit=125&page_info=page1>; rel="next"'
                    },
                }
            ],
        )
        # Paginated responses
        requests_mock.get(
            "https://test-shop.myshopify.com/admin/api/2026-07/orders.json?limit=250&page_info=page1",
            [
                {
                    "status_code": 200,
                    "json": ORDERS_PAGE_2,
                    "headers": {
                        "Link": '<https://test-shop.myshopify.com/admin/api/2026-07/orders.json?limit=250&page_info=page2>; rel="next"'
                    },
                }
            ],
        )
        requests_mock.get(
            "https://test-shop.myshopify.com/admin/api/2026-07/orders.json?limit=250&page_info=page2",
