...
class ShopifyAuthenticator(TokenAuthenticator):
    ...
    def _exchange_credentials_for_token(self) -> tuple:
        ...
        try:
            ...
            error_message = f"Failed to exchange client credentials for access token: {e}"
            ...
            raise ClientCredentialsTokenError(error_message) from e
        except requests.exceptions.RequestException as e:
            error_message = f"Network error while exchanging client credentials: {e}"
            logger.error(error_message)
            raise ClientCredentialsTokenError(error_message) from e
        ...
        try:
            ...
            error_message = f"Invalid response from Shopify token endpoint: {e}"
            logger.error(error_message)
            raise ClientCredentialsTokenError(error_message) from e
