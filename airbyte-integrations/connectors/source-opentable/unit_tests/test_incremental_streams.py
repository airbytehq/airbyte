...
from source_opentable.source import OpentableStream
...
def test_get_updated_state(patch_incremental_base_class, config):
    ...
    inputs = {"current_stream_state": {"9999":'2021-10-10 10:10:10'}, "latest_record": {"rid":"9999", "updated_at_utc":"2021-12-10T10:10:10Z"}}
...
def test_supports_incremental(patch_incremental_base_class, mocker, config):
    mocker.patch.object(OpentableStream, "cursor_field", "dummy_field")
    stream = OpentableStream(config, start_date=config["start_date"], rid_list=config["rid_list"])
