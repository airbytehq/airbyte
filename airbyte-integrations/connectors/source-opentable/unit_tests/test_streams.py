...
import pytest
from source_opentable.source import OpentableStream, OpentableAuthenticator
...
def patch_base_class(mocker, config):
    ...
    mocker.patch.object(OpentableStream, "path", "v0/example_endpoint")
    mocker.patch.object(OpentableStream, "primary_key", "test_primary_key")
    mocker.patch.object(OpentableStream, "__abstractmethods__", set())


def test_request_params(patch_base_class, config):
    stream = OpentableStream(config, start_date=config["start_date"], rid_list=config["rid_list"])
