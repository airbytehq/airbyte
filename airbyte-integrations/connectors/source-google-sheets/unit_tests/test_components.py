...
_MANIFEST = resolve_manifest(source=SourceGoogleSheets(catalog=catalog_helper(SyncMode.full_refresh, "a_stream"),config=_CONFIG, state={})).record.data["manifest"]
_FIELD_PATH = list(dpath.get(obj=_MANIFEST, glob=["dynamic_streams", 0, 'stream_template', 'schema_loader', 'retriever', 'record_selector', 'extractor', 'field_path'], default=[]))
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
                {'sheets': [
                    {'data': [{'rowData': [{'values': [{'formattedValue': ''}, {'formattedValue': ''}, {'formattedValue': ''}]}]}]}]},
                [{'values': []}]
        ),
    ],
    ids=[
...
def test_dpath_schema_extractor(body, expected_records: List):
    extractor = DpathSchemaExtractor(
        field_path=_FIELD_PATH, config=config, decoder=decoder_json, parameters=parameters
    )
...
def test_parse_raw_schema_value(raw_schema_data, expected_data):
    ...
    parsed_data =  extractor.parse_raw_schema_values(raw_schema_data, schema_pointer=_SCHEMA_TYPE_IDENTIFIERS['schema_pointer'], key_pointer=_SCHEMA_TYPE_IDENTIFIERS['key_pointer'], names_conversion=False)
