...
class TestSource(GoogleSheetsBaseTest):
    ...
    def test_given_spreadsheet_when_check_then_status_is_succeeded(self, http_mocker: HttpMocker) -> None:
        ...
        TestSource.get_sheet_first_row(http_mocker, "check_succeeded_range")
    ...
    def test_check_expected_to_read_data_from_1_sheet(self, http_mocker: HttpMocker) -> None:
        TestSource.get_spreadsheet_info_and_sheets(http_mocker, "check_succeeded_meta", 200)
        TestSource.get_sheet_first_row(http_mocker, "check_wrong_range", 200)
        error_message = (
    ...
    def test_check_duplicated_headers(self, http_mocker: HttpMocker) -> None:
        ...
        TestSource.get_spreadsheet_info_and_sheets(http_mocker, "check_succeeded_meta", 200)
        TestSource.get_sheet_first_row(http_mocker, "check_duplicate_headers", 200)
    ...
    def test_given_grid_sheet_type_with_at_least_one_row_when_discover_then_return_stream(self, http_mocker: HttpMocker) -> None:
        TestSource.get_spreadsheet_info_and_sheets(http_mocker, "only_headers_meta", 200)
        TestSource.get_sheet_first_row(http_mocker, "only_headers_range", 200)
    ...
    def test_discover_return_expected_schema(self, http_mocker: HttpMocker) -> None:
        ...
        TestSource.get_spreadsheet_info_and_sheets(http_mocker, "multiple_streams_schemas_meta", 200)
        TestSource.get_sheet_first_row(http_mocker, f"multiple_streams_schemas_{_STREAM_NAME}_range", 200)
        TestSource.get_sheet_first_row(http_mocker, f"multiple_streams_schemas_{_B_STREAM_NAME}_range", 200, _B_STREAM_NAME)
        TestSource.get_sheet_first_row(http_mocker, f"multiple_streams_schemas_{_C_STREAM_NAME}_range", 200, _C_STREAM_NAME)
    ...
    def test_discover_empty_column_return_expected_schema(self, http_mocker: HttpMocker) -> None:
        ...
        TestSource.get_spreadsheet_info_and_sheets(http_mocker, "discover_with_empty_column_spreadsheet_info_and_sheets", 200)
        TestSource.get_sheet_first_row(http_mocker, f"discover_with_empty_column_get_sheet_first_row", 200)
    ...
    def test_discover_with_duplicated_return_expected_schema(self, http_mocker: HttpMocker):
        ...
        TestSource.get_spreadsheet_info_and_sheets(http_mocker, f"{test_file_base_name}_{GET_SPREADSHEET_INFO}")
        TestSource.get_sheet_first_row(http_mocker, f"{test_file_base_name}_{GET_SHEETS_FIRST_ROW}")
        ...
        expected_log_message = AirbyteMessage(
            type=Type.LOG, log=AirbyteLogMessage(level=Level.INFO, message="Duplicate headers found in sheet. Ignoring them: ['header_2']")
        )
    ...
    def test_discover_with_names_conversion(self, http_mocker: HttpMocker) -> None:
        ...
        TestSource.get_spreadsheet_info_and_sheets(http_mocker, "only_headers_meta", 200)
        TestSource.get_sheet_first_row(http_mocker, "names_conversion_range", 200)
        expected_schema = {
            "$schema": "http://json-schema.org/draft-07/schema#",
    ...
    def test_given_grid_sheet_type_without_rows_when_discover_then_ignore_stream(self, http_mocker: HttpMocker) -> None:
        TestSource.get_spreadsheet_info_and_sheets(http_mocker, "no_rows_meta", 200)
        output = self._discover(self._config, expecting_exception=False)
    ...
    def test_given_not_grid_sheet_type_when_discover_then_ignore_stream(self, http_mocker: HttpMocker) -> None:
        TestSource.get_spreadsheet_info_and_sheets(http_mocker, "non_grid_sheet_meta", 200)
        output = self._discover(self._config, expecting_exception=False)
        assert len(output.catalog.catalog.streams) == 0

    @HttpMocker()
    def test_when_read_then_return_records(self, http_mocker: HttpMocker) -> None:
        TestSource.get_spreadsheet_info_and_sheets(http_mocker, "read_records_meta")
        TestSource.get_sheet_first_row(http_mocker, "read_records_range")
        TestSource.get_stream_data(http_mocker, "read_records_range_with_dimensions")
    ...
    def test_when_read_empty_column_then_return_records(self, http_mocker: HttpMocker) -> None:
        ...
        TestSource.get_spreadsheet_info_and_sheets(http_mocker, f"{test_file_base_name}_{GET_SPREADSHEET_INFO}")
        TestSource.get_sheet_first_row(http_mocker, f"{test_file_base_name}_{GET_SHEETS_FIRST_ROW}")
        TestSource.get_stream_data(http_mocker, f"{test_file_base_name}_{GET_STREAM_DATA}")
        first_property = "header_1"
        second_property = "header_2"
        configured_catalog = (
            CatalogBuilder()
    ...
    def test_when_read_then_return_records_with_name_conversion(self, http_mocker: HttpMocker) -> None:
        # will convert '1 тест' to '_1_test and 'header2' to 'header_2'
        TestSource.get_spreadsheet_info_and_sheets(http_mocker, "read_records_meta")
        TestSource.get_sheet_first_row(http_mocker, "names_conversion_range")
        TestSource.get_stream_data(http_mocker, "read_records_range_with_dimensions")
    ...
    def test_when_read_multiple_streams_return_records(self, http_mocker: HttpMocker) -> None:
        ...
        TestSource.get_stream_data(http_mocker, f"multiple_streams_schemas_{_STREAM_NAME}_range_2")
        TestSource.get_stream_data(http_mocker, f"multiple_streams_schemas_{_B_STREAM_NAME}_range_2", stream_name=_B_STREAM_NAME)
        TestSource.get_stream_data(http_mocker, f"multiple_streams_schemas_{_C_STREAM_NAME}_range_2", stream_name=_C_STREAM_NAME)
    ...
    def test_when_read_then_status_and_state_messages_emitted(self, http_mocker: HttpMocker) -> None:
        TestSource.get_spreadsheet_info_and_sheets(http_mocker, "read_records_meta_2", 200)
        TestSource.get_sheet_first_row(http_mocker, "read_records_range_2", 200)
        TestSource.get_stream_data(http_mocker, "read_records_range_with_dimensions_2")
    ...
    def test_read_empty_sheet(self, http_mocker: HttpMocker) -> None:
        TestSource.get_spreadsheet_info_and_sheets(http_mocker, "read_records_meta", 200)
        TestSource.get_sheet_first_row(http_mocker, "read_records_range_empty", 200)
    ...
    def test_read_expected_data_on_1_sheet(self, http_mocker: HttpMocker) -> None:
        TestSource.get_spreadsheet_info_and_sheets(http_mocker, "read_records_meta", 200)
        TestSource.get_sheet_first_row(http_mocker, "read_records_range_with_unexpected_extra_sheet", 200)
