...
class GoogleSheetSourceTest(TestCase):
    ...
    def test_check_access_expired(self, http_mocker: HttpMocker) -> None:
        ...
        expected_message = (
            exception_description_by_status_code(status_codes.FORBIDDEN, _SPREADSHEET_ID) + ". The caller does not have right permissions."
        )
    ...
    def test_when_read_by_batches_make_expected_requests(self, http_mocker: HttpMocker):
        ...
        for range_file_postfix in ("first_batch", "second_batch", "third_batch", "fourth_batch", "fifth_batch"):
            ...
            GoogleSheetSourceTest.get_stream_data(
    ...
    def test_read_429_error(self, http_mocker: HttpMocker) -> None:
        ...
        expected_message = f"Exception while syncing stream {_STREAM_NAME}: " + exception_description_by_status_code(
            status_codes.TOO_MANY_REQUESTS, _STREAM_NAME
        )
    ...
    def test_read_500_error(self, http_mocker: HttpMocker) -> None:
        ...
        expected_message = "Exception while syncing stream a_stream_name: There was an issue with the Google Sheets API. This is usually a temporary issue from Google's side. Please try again. If this issue persists, contact support"
        assert output.errors[0].trace.error.internal_message == expected_message
