 ...
 * </ul>
 */
public class IometeStreamCopier implements StreamCopier {

    private static final Logger LOGGER = LoggerFactory.getLogger(IometeStreamCopier.class);
    ...
    private final IometeSqlOperations sqlOperations;
    ...
    public IometeStreamCopier(final String stagingFolder,
                              final String schema,
        ...
        this.purgeStagingData = iometeConfig.isPurgeStagingData();
        this.database = database;
        this.sqlOperations = (IometeSqlOperations) sqlOperations;
        ...
        this.tmpTableLocation = String.format("s3a://%s/%s",
    ...
    public void write(final UUID id, final AirbyteRecordMessage recordMessage, final String fileName) throws Exception {
        LOGGER.info("Writing message: {}", recordMessage);
    ...
    public String createDestinationTable() throws Exception {
        ...
        final String createTable = String.format(
                "%s %s.%s " +
                        ...
                        "AS SELECT * FROM parquet.`%s` LIMIT 0",
                createStatement, schemaName, destTableName,
                streamName,
                destinationSyncMode.value(), String.join(", ", IometeConstants.DEFAULT_TBL_PROPERTIES),
                tmpTableLocation);
    ...
    public String generateMergeStatement(final String destTableName) {
        final String copyData = String.format(
                "INSERT INTO %s.%s SELECT * FROM parquet.`%s`",
