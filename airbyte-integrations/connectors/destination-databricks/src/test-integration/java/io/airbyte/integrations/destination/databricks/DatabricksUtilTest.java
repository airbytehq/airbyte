...
public class DatabricksUtilTest {
  ...
  /**
   * Magic Databricks JDBC param. This param exists because of issue with fetching data via SELECT. It produces next error: Error occured while
   * deserializing arrow data: sun.misc.Unsafe or java.nio.DirectByteBuffer.<init>(long, int) not available databricks. This solution was accidentally
   * found here (But it is not related to the error above) https://community.databricks.com/s/question/0D58Y00009AHCDSSA5/jdbc-driver-support-for-openjdk-17.
   * But no idea what it does, but it fixed the issue.
   * NOTE: Only for integration tests
   */
