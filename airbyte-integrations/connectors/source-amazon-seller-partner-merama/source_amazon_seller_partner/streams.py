...
class ReportsAmazonSPStream(Stream, ABC):
    ...
    def _report_data(
        ...
        reports_without_date_parameter = ['GET_VENDOR_FORECASTING_REPORT']
        ...
        if self.name not in reports_without_date_parameter: params["dataStartTime"] = replication_start_date.strftime(DATE_TIME_FORMAT)

        if self._replication_end_date and sync_mode == SyncMode.full_refresh and self.name not in reports_without_date_parameter:
    ...
    def read_records(
        ...
        while not is_processed and seconds_waited < self.max_wait_seconds:
            ...
            logger.info("processing status: %s", report_payload.get("processingStatus"))
            logger.info("Sleep seconds: %s", self.sleep_seconds)
        ...
        if is_done:
            ...
            request_headers = self.request_headers()
            logger.info("request_headers - report done: %s", request_headers)
            ...
            logger.info("REQUEST - report done: %s", request)
...
class VendorForecastingReport(AnalyticsStream):
    name = "GET_VENDOR_FORECASTING_REPORT"
    result_key = "forecastByAsin"
    cursor_field = "endDate"
    availability_sla_days = 3  # Data is only available after 3 days
