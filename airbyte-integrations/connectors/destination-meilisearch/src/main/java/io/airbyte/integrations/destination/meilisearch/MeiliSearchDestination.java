 ...
 *
 * <p>The strategy is to inject an extra airbyte primary key field in each record. The value of that
 ...
 *
 * <p>Index names can only contain alphanumeric values, so we normalize stream names to meet these
 * constraints. This is why streamName and indexName are treated separately in this connector.
 *
 * <p>This destination can support full refresh and incremental. It does NOT support normalization.
 * It breaks from the paradigm of having a "raw" and "normalized" table. There is no DBT for
...
public class MeiliSearchDestination extends BaseConnector implements Destination {
  ...
  public AirbyteConnectionStatus check(final JsonNode config) {
    try {
      ...
      // create a fake index and add a record to it to make sure we can connect and have write
      // access.
      ...
      return new AirbyteConnectionStatus()
          ...
          .withMessage("Check connection failed: " + e.getMessage());
    }
  }
  ...
  public AirbyteMessageConsumer getConsumer(
    ...
    return new BufferedStreamConsumer(
        ...
        (hasFailed) ->
            LOGGER.info(
                "Completed writing to MeiliSearch. Status: {}", hasFailed ? "FAILED" : "SUCCEEDED"),
        catalog,
        (data) -> true,
        MAX_BATCH_SIZE);
  }

  private static Map<String, Index> createIndices(
      final ConfiguredAirbyteCatalog catalog, final Client client) throws Exception {
  ...
  private static RecordWriter recordWriterFunction(
      final Map<String, Index> indexNameToWriteConfig) {
    return (namePair, records) -> {
      ...
      // Only writes the data, not the full AirbyteRecordMessage. This is different from how
      // database
      // destinations work. There is not really a viable way to "transform" data after it is
      // MeiliSearch.
      // Tools like DBT do not apply. Therefore, we need to try to write data in the most usable
      // format
                  ...
                  .peek(
                      o ->
                          ((ObjectNode) o)
                              .put(
                                  AB_PK_COLUMN,
                                  Names.toAlphanumericAndUnderscore(UUID.randomUUID().toString())))
                  .peek(
                      o ->
                          ((ObjectNode) o)
                              .put(AB_EMITTED_AT_COLUMN, LocalDateTime.now().format(FORMATTER)))
  ...
  static Client getClient(final JsonNode config) {
    return new Client(
        new Config(
            ...
            config.has("api_key") ? config.get("api_key").asText() : null));
  }
