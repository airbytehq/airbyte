...
For information about how to use this connector within Airbyte, see [the User Documentation](https://docs.airbyte.com/integrations/sources/{{dashCase name}}).
...
```
./gradlew :airbyte-integrations:connectors:source-{{dashCase name}}:buildConnectorImage
```

Once built, the docker image name and tag will be `airbyte/source-{{dashCase name}}:dev`.
...
4. Make the connector documentation and its changelog is up to date (`docs/integrations/sources/{{dashCase name}}.md`).
