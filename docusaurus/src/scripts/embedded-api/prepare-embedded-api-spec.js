...
const { SPEC_CACHE_PATH, API_SIDEBAR_PATH, EMBEDDED_API_SPEC_URL } = require("./constants");
...
async function main() {
  ...
  try {
    ...
    if (previousSpec) {
      ...
      log(`📋 Previous spec info: ${previousSpec.info?.title} v${previousSpec.info?.version}`);
      ...
      log("");
      const duration = Date.now() - startTime;
      log(`[prepare-embedded-api] ⚠️  Completed in ${duration}ms (used cached spec)`);
      process.exit(0);
    } else {
      ...
      logError("💡 Tip: Run this script successfully once to create an initial cache");
      ...
      log("");
      const duration = Date.now() - startTime;
      log(`[prepare-embedded-api] ⚠️  Completed in ${duration}ms (fallback spec)`);
      process.exit(0);
    }
  }
}
