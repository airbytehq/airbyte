...
export const SpecSchema = ({ specJSON }) => {
    ...
    <>
      <ul className={styles.contents}>
        {props.schema.oneOf.map((schema, i) => {
          return (
            <li key={i} className={styles.contents}>
...
function showCollapsible(schema) {
  return (
    (schema.type === "object" && schema.properties) || showDescription(schema)
  );
}
          ...
          <>
            ...
            <Disclosure.Panel className={styles.contents}>
              {showDescription(schema) && (
                <Description
                  schema={schema}
                  style={getIndentStyle(newDepth + 1)}
                />
              )}
              {schema.type === "object" && schema.oneOf && (
                <JSONSchemaOneOf schema={schema} depth={newDepth} />
              )}
              {schema.type === "object" && schema.properties && (
                <JSONSchemaObject schema={schema} depth={newDepth} />
              )}
              {schema.type === "array" && (
...
function Description({ schema, style }) {
  return (
    <div className={styles.descriptionItem} style={style}>
      {typeof schema.default !== "undefined" && !schema.const && (
        <div>
          Default: <pre>{JSON.stringify(schema.default, null, 2)}</pre>
        </div>
      )}
      {schema.const && (
        <div>
          Constant value: <pre>{JSON.stringify(schema.const, null, 2)}</pre>
        </div>
      )}
      {schema.pattern && (
        <div>
          Pattern
          {schema.pattern_descriptor && <> ({schema.pattern_descriptor})</>}:{" "}
          <pre>{schema.pattern}</pre>
        </div>
      )}
      ...
      {schema.examples && schema.examples.length === 1 && (
        <div>
          Example: <pre>{JSON.stringify(schema.examples[0])}</pre>
        </div>
      )}
      {schema.description && (
        <div>
          <TextWithHTML text={schema.description} />
        </div>
      )}
      {isOneOf(schema) && (
        <Heading as="h5" className={styles.oneOfHeader}>
          One of:
        </Heading>
      )}
      {isObjectArray(schema) && (
        <Heading as="h5" className={styles.oneOfHeader}>
          Item properties:
        </Heading>
      )}
