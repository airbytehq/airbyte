...
public class MongoUtils {
   ...
   */
  public static List<TreeNode<CommonField<BsonType>>> getUniqueFields(final MongoCollection<Document> collection, final int Depth_Level) {
    ...
    return allkeys.stream().map(key -> {
      ...
      if (type.equals(DOCUMENT)) {
        setSubFields(collection, fieldNode, key, Depth_Level);
      }
      return fieldNode;
    }).toList();
  }
