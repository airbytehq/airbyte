...
/**
 * This database allows a developer to specify a {@link JdbcStreamingQueryConfig}. This allows the developer to specify the correct configuration in
 * order for a {@link PreparedStatement} to execute as in a streaming / chunked manner.
 */
public class StreamingJdbcDatabase extends DefaultJdbcDatabase {
  ...
  /**
   * Assuming that the {@link JdbcStreamingQueryConfig} is configured correctly for the JDBC driver being used, this method will return data in
   * streaming / chunked fashion. Review the provided {@link JdbcStreamingQueryConfig} to understand the size of these chunks. If the entire stream is
   * consumed the database connection will be closed automatically and the caller need not call close on the returned stream. This query (and the
   * first chunk) are fetched immediately. Subsequent chunks will not be pulled until the first chunk is consumed.
   *
  ...
  /**
   * This method differs from {@link DefaultJdbcDatabase#toUnsafeStream} in that it takes a streaming config that adjusts the fetch size dynamically
   * according to sampled row size.
   */
