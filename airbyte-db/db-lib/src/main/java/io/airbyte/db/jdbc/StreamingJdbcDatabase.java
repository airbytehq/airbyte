...
public class StreamingJdbcDatabase extends DefaultJdbcDatabase {
  ...
  protected <T> Stream<T> toUnsafeStream(final ResultSet resultSet,
     ...
     *
     * @Override public boolean hasNext() { try { return (!resultSet.isLast()
     *//* && ((resultSet.getRow() != 0) || resultSet.isBeforeFirst()) *//*
                                                                         * ); } catch (final SQLException e) { streamException = e; isStreamFailed =
                                                                         * true; return false; } }
                                                                         *
                                                                         * @Override public T next() { try { if (!resultSet.next()) {
                                                                         * resultSet.close(); return null; } final T dataRow =
                                                                         * mapper.apply(resultSet); streamingConfig.accept(resultSet, dataRow); return
                                                                         * dataRow; } catch (final SQLException e) {
                                                                         * LOGGER.error("SQLState: {}, Message: {}", e.getSQLState(), e.getMessage());
                                                                         * streamException = e; isStreamFailed = true; // throwing an exception in
                                                                         * tryAdvance() method lead to the endless loop in Spliterator and stream //
                                                                         * will never close return null; } } });
                                                                         */
