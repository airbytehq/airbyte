...
public class IncrementalUtils {
  ...
  public static JsonSchemaPrimitive getCursorType(
    ...
    if (stream.getStream().getJsonSchema().get(PROPERTIES).get(cursorField).get("type") == null
        && stream.getStream().getJsonSchema().get(PROPERTIES).get(cursorField).get("$ref")
            == null) {
      throw new IllegalStateException(String.format(
    ...
    if (stream.getStream().getJsonSchema().get(PROPERTIES).get(cursorField).get("type") == null) {
      return JsonSchemaPrimitiveUtil.PRIMITIVE_TO_REFERENCE_BIMAP
          .inverse()
          .get(stream
              .getStream()
              .getJsonSchema()
      ...
      return JsonSchemaPrimitive.valueOf(stream
          .getStream()
          .getJsonSchema()
          .get(PROPERTIES)
          .get(cursorField)
