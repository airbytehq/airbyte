 ...
 */
public abstract class AbstractJdbcCompatibleSourceOperations<Datatype>
    implements JdbcCompatibleSourceOperations<Datatype> {
  ...
  protected void putTime(
      ...
      throws SQLException {
    node.put(
        columnName, DateTimeConverter.convertToTime(getObject(resultSet, index, LocalTime.class)));
  }
  ...
  protected void putTimestamp(
    ...
    try {
      node.put(
          columnName,
          DateTimeConverter.convertToTimestamp(getObject(resultSet, index, LocalDateTime.class)));
    } catch (Exception e) {
  ...
  private void setDateAsTimestamp(
      final PreparedStatement preparedStatement, final int parameterIndex, final String value)
      throws SQLException {
    try {
  ...
  protected void setBigInteger(
    ...
    preparedStatement.setLong(
        parameterIndex, new BigDecimal(value).toBigInteger().longValue());
  }
  ...
  protected <ObjectType> ObjectType getObject(
      final ResultSet resultSet, final int index, final Class<ObjectType> clazz)
      throws SQLException {
    return resultSet.getObject(index, clazz);
  }

  protected void putTimeWithTimezone(
      final ObjectNode node, final String columnName, final ResultSet resultSet, final int index)
      throws SQLException {
