...
public class JdbcSourceOperations implements SourceOperations<ResultSet, JDBCType> {
  ...
  public JsonNode rowToJson(final ResultSet queryContext) throws SQLException {
    ...
    for (int i = 1; i <= columnCount; i++) {
      ...
      // type-specific
      // parsing. if it is null, we can move on. while awkward, this seems to be the agreed upon way
      // of
  ...
  protected void setJsonField(final ResultSet r, final int i, final ObjectNode o)
      throws SQLException {
  ...
  protected void putDouble(
    ...
    node.put(
        columnName,
        DataTypeUtils.returnNullIfInvalid(() -> resultSet.getDouble(index), Double::isFinite));
  }

  protected void putReal(
      final ObjectNode node, final String columnName, final ResultSet resultSet, final int index)
      throws SQLException {
    node.put(
        columnName,
        DataTypeUtils.returnNullIfInvalid(() -> resultSet.getFloat(index), Float::isFinite));
  }
  ...
  public void setStatementField(
    ...
    switch (cursorFieldType) {
      ...
      default -> throw new IllegalArgumentException(
          String.format("%s is not supported.", cursorFieldType));
    }
  }
  ...
  protected void setTimestamp(
    ...
    // Parsing TIME as a TIMESTAMP might potentially break for ClickHouse cause it doesn't expect
    // TIME
    // value in the following format
    try {
      preparedStatement.setTimestamp(
          parameterIndex, Timestamp.from(DataTypeUtils.DATE_FORMAT.parse(value).toInstant()));
  ...
  public String enquoteIdentifierList(final Connection connection, final List<String> identifiers)
      throws SQLException {
  ...
  public String enquoteIdentifier(final Connection connection, final String identifier)
      throws SQLException {
  ...
  public String getFullyQualifiedTableNameWithQuoting(
      final Connection connection, final String schemaName, final String tableName)
      throws SQLException {
    final String quotedTableName = enquoteIdentifier(connection, tableName);
    return schemaName != null
        ? enquoteIdentifier(connection, schemaName) + "." + quotedTableName
        : quotedTableName;
  }

  @Override
  public JsonSchemaPrimitive getType(final JDBCType bigQueryType) {
