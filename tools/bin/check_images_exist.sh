...
checkNormalizationImages() {
  ...
  local image_version;
  definition_file_path=airbyte-config-oss/init/src/main/resources/seed/destination_definitions.yaml
...
checkConnectorImages() {
  ...
  CONNECTOR_DEFINITIONS=$(grep "dockerRepository" -h -A1 airbyte-config-oss/init/src/main/resources/seed/*.yaml | grep -v -- "^--$" | tr -d ' ')
