...
def get_source_connectors():
    ...
    with open(os.path.join(toplevel, SOURCE_DEFINITIONS)) as fp:
        source_definitions = yaml.safe_load(fp)
    res = {}
...
def generate_all_specs():
    ...
    source_connectors = get_source_connectors()
    for connector_name, docker_image in source_connectors.items():
