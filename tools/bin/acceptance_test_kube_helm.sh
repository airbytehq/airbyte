...
helm install --values tools/bin/fluent_values.yaml --set env[0].name="AWS_ACCESS_KEY_ID" --set env[0].value=\"${AWS_ACCESS_KEY_ID}\" \
 --set env[1].name="AWS_SECRET_ACCESS_KEY" --set env[1].value=\"${AWS_SECRET_ACCESS_KEY}\" \
 --set env[2].name="AWS_SESSION_TOKEN" --set env[2].value=\"${AWS_SESSION_TOKEN}\" \
 --set env[4].name="WORKFLOW_RUN_ID" --set env[4].value=\"${WORKFLOW_RUN_ID}\" \
