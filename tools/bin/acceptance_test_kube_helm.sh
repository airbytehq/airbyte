...
helm template --values tools/bin/fluent_values.yaml --set env[0].name="AWS_ACCESS_KEY_ID" --set env[0].value=$(echo "$AWS_S3_INTEGRATION_TEST_CREDS" | jq -r .aws_access_key_id) \
