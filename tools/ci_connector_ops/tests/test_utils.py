...
class TestConnector:
    ...
    @pytest.mark.parametrize(
        "connector, exists, expected_language",
        [
            (utils.Connector("source-faker"), True, utils.ConnectorLanguage.PYTHON),
            (utils.Connector("source-greenhouse"), True, utils.ConnectorLanguage.LOW_CODE),
            (utils.Connector("source-bigquery"), True, utils.ConnectorLanguage.JAVA),
            (utils.Connector("destination-bigquery"), True, utils.ConnectorLanguage.JAVA),
            (utils.Connector("source-notpublished"), False, None),
        ],
    )
    def test_init(self, connector, exists, expected_language, mocker, tmp_path):
        ...
        if exists:
            ...
            assert connector.language == expected_language
        else:
