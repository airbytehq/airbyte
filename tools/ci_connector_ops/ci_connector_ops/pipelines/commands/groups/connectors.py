...
from ci_connector_ops.pipelines.format import run_connector_format_pipeline
...
def format(ctx: click.Context) -> bool:
    click.secho(f"Will format the following connectors: {', '.join(ctx.obj['selected_connectors_names'])}.", fg="green")
    connectors_contexts = [
        ConnectorContext(
            pipeline_name="Format connector {connector.technical_name}",
            connector=connector,
            is_local=ctx.obj["is_local"],
            git_branch=ctx.obj["git_branch"],
            git_revision=ctx.obj["git_revision"],
