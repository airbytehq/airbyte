...
async def run_connector_test_pipelines(dagger_client: Client, connector: Connector, gsm_credentials: str):
...
def connectors_ci(ctx: click.Context, diffed_branch: str, gcp_gsm_credentials: str):
    ...
    if not gcp_gsm_credentials:
        raise click.ClickException("You need to set GCP_GSM_CREDENTIALS env var or pass it as an option.")
...
@connectors_ci.command()
@click.pass_context
def test_all_modified_connector(ctx: click.Context):
