...
public class JobSubmitter implements Runnable {
  ...
  /**
   * Since job submission and job execution happen in two separate thread pools, and job execution
   * is what removes a job from the submission queue, it is possible for a job to be submitted
   * multiple times.
   *
   * <p>This synchronised block guarantees only a single thread can utilise the concurrent set to
   * decide whether a job should be submitted. This job id is added here, and removed in the finish
   * block of {@link #submitJob(Job)}.
   *
   * <p>Since {@link JobPersistence#getNextJob()} returns the next queued job, this solution cause
   ...
   *
   * <p>See https://github.com/airbytehq/airbyte/issues/4378 for more info.
   */
  private synchronized Consumer<Job> attemptJobSubmit() {
    return job -> {
      if (!runningJobs.contains(job.getId())) {
        ...
        LOGGER.info(
            "Attempting to submit already running job {}. There are probably too many queued jobs.",
            job.getId());
  ...
  void submitJob(final Job job) {
    ...
    // attempt number should be. if it is not, that the lifecycle will fail. this should not happen
    // as
    // long as job submission for a single job is single threaded. this is a compromise to allow the
    // job
    // persistence to control what the attempt number should be while still allowing us to declare
    // it
    // before the lifecycle begins.
    final int attemptNumber = job.getAttempts().size();
    threadPool.submit(
        new LifecycledCallable.Builder<>(workerRun)
            .setOnStart(
                () -> {
                  // TODO(Issue-4204): This should save the fully qualified job path.
                  final Path logFilePath =
                      workerRun.getJobRoot().resolve(LogClientSingleton.LOG_FILENAME);
                  final long persistedAttemptId =
                      persistence.createAttempt(job.getId(), logFilePath);
