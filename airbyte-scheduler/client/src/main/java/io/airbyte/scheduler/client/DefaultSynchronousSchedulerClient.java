...
public class DefaultSynchronousSchedulerClient implements SynchronousSchedulerClient {
  ...
  public DefaultSynchronousSchedulerClient(
  ...
  public SynchronousResponse<StandardCheckConnectionOutput> createSourceCheckConnectionJob(
    ...
    final JsonNode sourceConfiguration =
        oAuthConfigSupplier.injectSourceOAuthParameters(
            source.getSourceDefinitionId(), source.getWorkspaceId(), source.getConfiguration());
  ...
  public SynchronousResponse<AirbyteCatalog> createDiscoverSchemaJob(
    ...
    return execute(
        ...
        source.getSourceDefinitionId(),
        jobId ->
            temporalClient.submitDiscoverSchema(UUID.randomUUID(), 0, jobDiscoverCatalogConfig),
        source.getWorkspaceId());
  }
  ...
  @VisibleForTesting
  <T> SynchronousResponse<T> execute(
    ...
    try {
      ...
      final JobState outputState =
          operationOutput.getMetadata().isSucceeded() ? JobState.SUCCEEDED : JobState.FAILED;
      track(
          jobId,
          configType,
      ...
      return SynchronousResponse.fromTemporalResponse(
          operationOutput, jobId, configType, connectorDefinitionId, createdAt, endedAt);
  ...
  private <T> void track(
    ...
    switch (configType) {
      ...
      case CHECK_CONNECTION_DESTINATION -> jobTracker.trackCheckConnectionDestination(
          ...
          jobState,
          (StandardCheckConnectionOutput) value);
      case DISCOVER_SCHEMA -> jobTracker.trackDiscover(
          jobId, connectorDefinitionId, workspaceId, jobState);
      case GET_SPEC -> {
        // skip tracking for get spec to avoid noise.
      }
      default -> throw new IllegalArgumentException(
          String.format(
              "Jobs of type %s cannot be processed here. They should be consumed in the JobSubmitter.",
              configType));
    }
  }
}
