...
public class BucketSpecCacheSchedulerClient implements SynchronousSchedulerClient {
  ...
  private static final Logger LOGGER =
      LoggerFactory.getLogger(BucketSpecCacheSchedulerClient.class);
  ...
  public BucketSpecCacheSchedulerClient(
      final SynchronousSchedulerClient client, final String bucketName) {
    this(
        client,
        dockerImage ->
            attemptToFetchSpecFromBucket(
                StorageOptions.getDefaultInstance().getService(), bucketName, dockerImage));
  }

  @VisibleForTesting
  BucketSpecCacheSchedulerClient(
      final SynchronousSchedulerClient client,
  ...
  public SynchronousResponse<ConnectorSpecification> createGetSpecJob(final String dockerImage)
    ...
    if (cachedSpecOptional.isPresent()) {
      ...
      final SynchronousJobMetadata mockMetadata =
          new SynchronousJobMetadata(
              UUID.randomUUID(), ConfigType.GET_SPEC, null, now, now, true, null);
  ...
  private static void validateConfig(final JsonNode json) throws JsonValidationException {
    ...
    final JsonNode specJsonSchema =
        JsonSchemaValidator.getSchema(
            AirbyteProtocolSchema.PROTOCOL.getFile(), "ConnectorSpecification");
    jsonSchemaValidator.ensure(specJsonSchema, json);
  }

  public static Optional<ConnectorSpecification> attemptToFetchSpecFromBucket(
      final Storage storage, final String bucketName, final String dockerImage) {
    final String[] dockerImageComponents = dockerImage.split(":");
    Preconditions.checkArgument(
        dockerImageComponents.length == 2, "Invalidate docker image: " + dockerImage);
    final String dockerImageName = dockerImageComponents[0];
    final String dockerImageTag = dockerImageComponents[1];

    final Path specPath =
        Path.of("specs").resolve(dockerImageName).resolve(dockerImageTag).resolve("spec.json");
