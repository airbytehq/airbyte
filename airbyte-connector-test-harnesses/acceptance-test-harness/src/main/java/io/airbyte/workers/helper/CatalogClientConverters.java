...
public class CatalogClientConverters {
  ...
  public static io.airbyte.protocol.models.AirbyteCatalog toAirbyteProtocol(
    ...
    var airbyteStream = catalog.getStreams().stream()
        .map(stream -> {
          try {
  ...
  private static io.airbyte.protocol.models.AirbyteStream toConfiguredProtocol(
    ...
    if (config.getFieldSelectionEnabled() != null && config.getFieldSelectionEnabled()) {
      ...
      for (final var selectedFieldInfo : config.getSelectedFields()) {
        if (selectedFieldInfo.getFieldPath() == null
            || selectedFieldInfo.getFieldPath().isEmpty()) {
      ...
      // NOTE: we verified above that each selected field has at least one element in the field
      // path.
      final Set<String> selectedFieldNames = config.getSelectedFields().stream()
          .map((field) -> field.getFieldPath().get(0))
          .collect(Collectors.toSet());
      // TODO(mfsiega-airbyte): we only check the top level of the cursor/primary key fields because
      // we
      ...
      if (config.getSyncMode().equals(SyncMode.INCREMENTAL) // INCREMENTAL sync mode, AND
          ...
          && !selectedFieldNames.contains(
              config.getCursorField().get(0))) { // The cursor isn't in the selected fields.
        throw new JsonValidationException(
      ...
      for (final String selectedFieldName : selectedFieldNames) {
        if (!properties.has(selectedFieldName)) {
          throw new JsonValidationException(String.format(
              "Requested selected field %s not found in JSON schema", selectedFieldName));
        }
      }
    ...
    return new io.airbyte.protocol.models.AirbyteStream()
        ...
        .withSupportedSyncModes(Enums.convertListTo(
        ...
        .withSourceDefinedPrimaryKey(Optional.ofNullable(stream.getSourceDefinedPrimaryKey())
  ...
  public static io.airbyte.api.client.model.generated.AirbyteCatalog toAirbyteCatalogClientApi(
    ...
    return new io.airbyte.api.client.model.generated.AirbyteCatalog()
        .streams(catalog.getStreams().stream()
  ...
  private static io.airbyte.api.client.model.generated.AirbyteStreamConfiguration
    ...
    if (stream.getSupportedSyncModes().size() > 0) {
      result.setSyncMode(Enums.convertTo(
          stream.getSupportedSyncModes().get(0),
  ...
  private static io.airbyte.api.client.model.generated.AirbyteStream toAirbyteStreamClientApi(
    ...
    return new io.airbyte.api.client.model.generated.AirbyteStream()
        ...
        .supportedSyncModes(Enums.convertListTo(
            stream.getSupportedSyncModes(), io.airbyte.api.client.model.generated.SyncMode.class))
