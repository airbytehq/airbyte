...
public class DefaultDiscoverCatalogTestHarness implements DiscoverCatalogTestHarness {
  ...
  private static final Logger LOGGER =
      LoggerFactory.getLogger(DefaultDiscoverCatalogTestHarness.class);
  private static final String WRITE_DISCOVER_CATALOG_LOGS_TAG =
  ...
  public DefaultDiscoverCatalogTestHarness(
  ...
  public ConnectorJobOutput run(
    ...
    try {
      ...
      final ConnectorJobOutput jobOutput =
          new ConnectorJobOutput().withOutputType(OutputType.DISCOVER_CATALOG_ID);
      ...
      final Optional<AirbyteCatalog> catalog =
          messagesByType.getOrDefault(Type.CATALOG, new ArrayList<>()).stream()
      ...
      final Optional<FailureReason> failureReason =
          TestHarnessUtils.getJobFailureReasonFromMessages(
              OutputType.DISCOVER_CATALOG_ID, messagesByType);
      ...
      if (catalog.isPresent()) {
        final DiscoverCatalogResult result = AirbyteApiClient.retryWithJitter(
            () -> airbyteApiClient
                .getSourceApi()
                .writeDiscoverCatalogResult(
                    buildSourceDiscoverSchemaWriteRequestBody(discoverSchemaInput, catalog.get())),
  ...
  private SourceDiscoverSchemaWriteRequestBody buildSourceDiscoverSchemaWriteRequestBody(
      final StandardDiscoverCatalogInput discoverSchemaInput, final AirbyteCatalog catalog) {
    return new SourceDiscoverSchemaWriteRequestBody()
        .catalog(CatalogClientConverters.toAirbyteCatalogClientApi(catalog))
        .sourceId(
            // NOTE: sourceId is marked required in the OpenAPI config but the code generator
            // doesn't enforce
            // it, so we check again here.
            discoverSchemaInput.getSourceId() == null
                ? null
                : UUID.fromString(discoverSchemaInput.getSourceId()))
        .connectorVersion(discoverSchemaInput.getConnectorVersion())
        .configurationHash(discoverSchemaInput.getConfigHash());
  }
