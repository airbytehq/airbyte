...
**Flow:**
```
Source emits STATE after N records
  ↓
Destination buffers those N records
  ↓
InsertBuffer.flush() writes to database
  ↓
Framework marks checkpoint complete
  ↓
Destination emits STATE to stdout
  ↓
Platform persists state
```

**Guarantee:** STATE only emitted after corresponding records are persisted

**Recovery:** On failure, retry from last persisted STATE
