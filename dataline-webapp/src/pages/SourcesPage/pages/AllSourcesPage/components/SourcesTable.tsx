...
const SourcesTable: React.FC = () => {
        ...
        () => [
            {
                ...
                Cell: ({ cell, row }: CellProps<ITableDataItem>) => (
                    <NameCell value={cell.value} error={row.original.error} enabled={row.original.enabled} />
                )
            },
            {
                Header: <FormattedMessage id="sources.connector" />,
                accessor: "sourceName",
                Cell: ({ cell, row }: CellProps<ITableDataItem>) => <ConnectorCell value={cell.value} enabled={row.original.enabled} />
            },
            {
                Header: <FormattedMessage id="sources.frequency" />,
                accessor: "schedule",
                Cell: ({ cell, row }: CellProps<ITableDataItem>) => <FrequencyCell value={cell.value} enabled={row.original.enabled} />
            },
            {
                Header: <FormattedMessage id="sources.lastSync" />,
                accessor: "lastSync",
                Cell: ({ cell, row }: CellProps<ITableDataItem>) => <LastSyncCell valueInSecond={cell.value} enabled={row.original.enabled} />
            },
            {
    ...
    const clickRow = (connection: any) => push(`${Routes.Source}/${connection.connectionId}`);
