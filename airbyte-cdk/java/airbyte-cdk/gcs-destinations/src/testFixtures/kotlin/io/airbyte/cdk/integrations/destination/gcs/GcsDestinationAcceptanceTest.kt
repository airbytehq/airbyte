...
abstract class GcsDestinationAcceptanceTest(protected val outputFormat: S3Format) : DestinationAcceptanceTest() {
    ...
    fun testCheckConnectionInsufficientRoles() {
        val baseConfigJson = Jsons.deserialize(IOs.readFile(Path.of(
        ...
        Assertions.assertEquals(StandardCheckConnectionOutput.Status.FAILED, runCheck(configJson).status)
    }

    @Test
    fun testCheckIncorrectHmacKeyAccessIdCredential() {
        val baseJson = baseConfigJson
        val credential = Jsons.jsonNode(ImmutableMap.builder<Any, Any>()
    ...
    fun testCheckIncorrectHmacKeySecretCredential() {
        ...
        val credential = Jsons.jsonNode(ImmutableMap.builder<Any, Any>()
                .put("credential_type", "HMAC_KEY")
                .put("hmac_key_access_id", baseJson["credential"]["hmac_key_access_id"].asText())
    ...
    companion object {
        ...
        protected const val SECRET_FILE_PATH_INSUFFICIENT_ROLES: String = "secrets/insufficient_roles_config.json"
    }
}
