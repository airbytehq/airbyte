...
internal class TestHarnessUtilsTest {
    ...
    internal inner class GentleCloseWithHeartbeat {
        ...
        private var forceShutdown: BiConsumer<Process?, Duration?>? = null

        @BeforeEach
        fun setup() {
            process = Mockito.mock(Process::class.java)
            heartbeatMonitor = Mockito.mock(HeartbeatMonitor::class.java)
            forceShutdown = Mockito.mock(BiConsumer::class.java)
        }
        ...
        fun testStartsWait() {
            ...
            Mockito.doAnswer { ignored: InvocationOnMock? ->
                ...
                true
            }.`when`<HeartbeatMonitor?>(heartbeatMonitor).isBeating
    ...
    fun testMapStreamNamesToSchemasWithNullNamespace() {
        ...
        Assertions.assertNotNull(mapOutput[AirbyteStreamNameNamespacePair("user_preferences", null)])
    }
    ...
    fun testMapStreamNamesToSchemasWithMultipleNamespaces() {
        ...
        val syncInput = syncPair.value
        val mapOutput = TestHarnessUtils.mapStreamNamesToSchemas(syncInput)
        Assertions.assertNotNull(mapOutput[AirbyteStreamNameNamespacePair("user_preferences", "namespace")])
        Assertions.assertNotNull(mapOutput[AirbyteStreamNameNamespacePair("user_preferences", "namespace2")])
    }
    ...
    companion object {
         ...
         * forcibly.
         */
        fun gentleCloseWithHeartbeat(process: Process?,
