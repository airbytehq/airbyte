...
internal class JsonsTest {
    ...
    fun testSerialize() {
        Assertions.assertEquals(
                SERIALIZED_JSON,
                Jsons.serialize(ToClass(ABC, 999, 888L)))
    ...
    fun testSerializeJsonNode() {
        ...
        Assertions.assertEquals(
                "{\"test\":\"dGVzdA==\"}",
                Jsons.serialize(Jsons.jsonNode(ImmutableMap.of(
                        TEST, BinaryNode("test".toByteArray(StandardCharsets.UTF_8))))))
    }
    ...
    fun testTryDeserialize() {
        Assertions.assertEquals(
                ...
                Jsons.tryDeserialize("{\"str\":\"abc\", \"num\": 999, \"numLong\": 888}", ToClass::class.java))

        Assertions.assertEquals(
                Optional.of(ToClass(ABC, 999, 0L)),
                Jsons.tryDeserialize("{\"str\":\"abc\", \"num\": 999, \"test\": 888}", ToClass::class.java))
    }
    ...
    fun testToJsonNode() {
        Assertions.assertEquals(
                SERIALIZED_JSON,
                Jsons.jsonNode(ToClass(ABC, 999, 888L)).toString())
    ...
    fun testToObject() {
        ...
        Assertions.assertEquals(
                ...
                Jsons.`object`<List<ToClass>>(Jsons.jsonNode(Lists.newArrayList(expected)), object : TypeReference<List<ToClass?>?>() {}))
    ...
    fun testToBytes() {
        ...
        Assertions.assertArrayEquals(jsonString.toByteArray(Charsets.UTF_8), Jsons.toBytes(Jsons.deserialize(jsonString)))
    }
    ...
    fun testKeys() {
        ...
        val nestedJsonNode = Jsons.jsonNode(ImmutableMap.of(TEST, ABC, TEST2, ImmutableMap.of("test3", "def")))
...
"""
    ...
    fun testGetOptional() {
        ...
        Assertions.assertEquals(Optional.of(Jsons.jsonNode<Any?>(null)), Jsons.getOptional(json, STU))
    ...
    fun testGetStringOrNull() {
        val json = Jsons.deserialize("{ \"abc\": { \"def\": \"ghi\" }, \"jkl\": \"mno\", \"pqr\": 1 }")
    ...
    fun testFlatten__noArrays() {
        ...
        val expected = Stream.of(*arrayOf(
                arrayOf<Any>("abc.def", GHI),
                arrayOf<Any>(JKL, true),
                arrayOf<Any>(PQR, 1),
        )).collect(Collectors.toMap(Function { data: Array<Any> -> data[0] as String }, Function { data: Array<Any> -> data[1] }))
