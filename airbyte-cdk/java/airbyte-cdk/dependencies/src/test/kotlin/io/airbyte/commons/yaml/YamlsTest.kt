...
internal class YamlsTest {
    ...
    fun testListWriter() {
        ...
        val deserialize: List<*> = Yamls.deserialize<List<*>>(writer.toString(), MutableList::class.java)
    ...
    fun testStreamRead() {
        val classes: List<ToClass> = Lists.newArrayList(
                ToClass("1", 1, 1),
                ToClass("2", 2, 2),
                ToClass("3", 3, 3))
        val input = Mockito.spy(ByteArrayInputStream(Yamls.serialize(classes).toByteArray(StandardCharsets.UTF_8)))
