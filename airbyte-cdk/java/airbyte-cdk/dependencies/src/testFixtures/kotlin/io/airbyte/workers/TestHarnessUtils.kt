...
object TestHarnessUtils {
    ...
    fun gentleClose(process: Process?, timeout: Long, timeUnit: TimeUnit?) {
        ...
        if (process.info() != null) {
            process.info().commandLine().ifPresent { commandLine: String? -> LOGGER.debug("Gently closing process {}", commandLine) }
        }
    ...
    private fun getConnectorCommandFromOutputType(outputType: ConnectorJobOutput.OutputType): FailureHelper.ConnectorCommand {
        return when (outputType) {
            ...
            ConnectorJobOutput.OutputType.DISCOVER_CATALOG_ID -> FailureHelper.ConnectorCommand.DISCOVER
        }
    }

    fun getMostRecentConfigControlMessage(messagesByType: Map<AirbyteMessage.Type?, List<AirbyteMessage?>?>?): Optional<AirbyteControlConnectorConfigMessage> {
        return messagesByType!!.getOrDefault(AirbyteMessage.Type.CONTROL, ArrayList())!!.stream()
    ...
    private fun getTraceMessageFromMessagesByType(messagesByType: Map<AirbyteMessage.Type?, List<AirbyteMessage?>?>?): Optional<AirbyteTraceMessage> {
        return messagesByType!!.getOrDefault(AirbyteMessage.Type.TRACE, ArrayList())!!.stream()
    ...
    fun getJobFailureReasonFromMessages(outputType: ConnectorJobOutput.OutputType,
                                        messagesByType: Map<AirbyteMessage.Type?, List<AirbyteMessage?>?>?): Optional<FailureReason?> {
    ...
    fun mapStreamNamesToSchemas(syncInput: StandardSyncInput): Map<AirbyteStreamNameNamespacePair, JsonNode> {
        return syncInput.catalog.streams.stream().collect(
                Collectors.toMap(
                        Function { k: ConfiguredAirbyteStream -> AirbyteStreamNameNamespacePair.fromAirbyteStream(k.stream) },
