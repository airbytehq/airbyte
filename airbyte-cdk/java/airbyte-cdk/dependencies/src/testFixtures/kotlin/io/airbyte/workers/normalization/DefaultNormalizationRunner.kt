...
class DefaultNormalizationRunner(private val processFactory: ProcessFactory,
                                 private val normalizationImageName: String,
                                 private val normalizationIntegrationType: String) : NormalizationRunner {
    private val streamFactory = NormalizationAirbyteStreamFactory(CONTAINER_LOG_MDC_BUILDER)
    private var airbyteMessagesByType: MutableMap<AirbyteMessage.Type, List<AirbyteMessage?>?>? = null
    private var dbtErrorStack: String? = null

    private var process: Process? = null

    @Throws(Exception::class)
    override fun configureDbt(jobId: String,
                              attempt: Int,
                              jobRoot: Path,
                              config: JsonNode?,
        ...
        return if (Strings.isNullOrEmpty(gitRepoBranch)) {
            runProcess(jobId, attempt, jobRoot, files, resourceRequirements, "configure-dbt",
                    "--integration-type", normalizationIntegrationType.lowercase(Locale.getDefault()),
    ...
    @Throws(Exception::class)
    override fun normalize(jobId: String,
                           attempt: Int,
                           jobRoot: Path,
                           config: JsonNode,
                           catalog: ConfiguredAirbyteCatalog,
                           resourceRequirements: ResourceRequirements?): Boolean {
        val files: Map<String?, String?> = ImmutableMap.of(
                WorkerConstants.DESTINATION_CONFIG_JSON_FILENAME, Jsons.serialize(config),
    ...
    @Throws(Exception::class)
    private fun runProcess(jobId: String,
                           attempt: Int,
                           jobRoot: Path,
                           files: Map<String?, String?>,
                           resourceRequirements: ResourceRequirements?,
                           vararg args: String): Boolean {
        try {
            ...
            process = processFactory.create(
                    ...
                    null,
                    java.util.Map.of(Metadata.JOB_TYPE_KEY, Metadata.SYNC_JOB, Metadata.SYNC_STEP_KEY, Metadata.NORMALIZE_STEP),
                    emptyMap(),
                    emptyMap(),
                    emptyMap(),
                    *args)
            ...
            process!!.inputStream.use { stdout ->
                ...
                if ("" != dbtErrorStack) {
                    val dbtTraceMessage = AirbyteMessage()
                            ...
                            .withTrace(AirbyteTraceMessage()
                                    ...
                                    .withError(AirbyteErrorTraceMessage()
                                            .withFailureType(AirbyteErrorTraceMessage.FailureType.SYSTEM_ERROR) // TODO: decide on best FailureType for this
                                            .withMessage("Normalization failed during the dbt run. This may indicate a problem with the data itself.") // due to the lack of consistent defining features in dbt errors we're injecting a breadcrumb to the
                    ...
                    airbyteMessagesByType.putIfAbsent(AirbyteMessage.Type.TRACE, java.util.List.of(dbtTraceMessage))
                }
            }
     ...
     */
        if (process!!.isAlive) {
            ...
            throw TestHarnessException("Normalization process did not terminate normally (exit code: " + process!!.exitValue() + ")")
        } else {
    ...
    override val traceMessages: Stream<AirbyteTraceMessage>
        get() {
            if (airbyteMessagesByType != null && airbyteMessagesByType!![AirbyteMessage.Type.TRACE] != null) {
                return airbyteMessagesByType!![AirbyteMessage.Type.TRACE]!!.stream().map { obj: AirbyteMessage? -> obj!!.trace }
            }
            return Stream.empty()
        }
