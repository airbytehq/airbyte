...
object CatalogClientConverters {
    ...
    fun toAirbyteProtocol(catalog: AirbyteCatalog): io.airbyte.protocol.models.AirbyteCatalog {
        val protoCatalog =
                io.airbyte.protocol.models.AirbyteCatalog()
        val airbyteStream = catalog.streams.stream().map { stream: AirbyteStreamAndConfiguration ->
            try {
    ...
    private fun toConfiguredProtocol(stream: AirbyteStream?,
                                     config: AirbyteStreamConfiguration?): io.airbyte.protocol.models.AirbyteStream {
        if (config!!.fieldSelectionEnabled != null && config.fieldSelectionEnabled!!) {
            ...
            val selectedFieldNames =
                    config.selectedFields!!.stream().map { field: SelectedFieldInfo -> field.fieldPath!![0] }.collect(Collectors.toSet())
            ...
            if (config.syncMode == io.airbyte.api.client.model.generated.SyncMode.INCREMENTAL && !config.cursorField!!.isEmpty() // There is a cursor configured, AND
                    && !selectedFieldNames.contains(config.cursorField!![0])) { // The cursor isn't in the selected fields.
            ...
            for (selectedFieldName in selectedFieldNames) {
                if (!properties.has(selectedFieldName)) {
                    throw JsonValidationException(String.format("Requested selected field %s not found in JSON schema", selectedFieldName))
                }
            }
            (properties as ObjectNode).retain(selectedFieldNames)
        }
        return io.airbyte.protocol.models.AirbyteStream()
                .withName(stream!!.name)
                .withJsonSchema(stream.jsonSchema)
                .withSupportedSyncModes(Enums.convertListTo(stream.supportedSyncModes, SyncMode::class.java))
    ...
    fun toAirbyteCatalogClientApi(
            catalog: io.airbyte.protocol.models.AirbyteCatalog): AirbyteCatalog {
        return AirbyteCatalog()
                .streams(catalog.streams
                        .stream()
                        .map { stream: io.airbyte.protocol.models.AirbyteStream -> toAirbyteStreamClientApi(stream) }
                        .map { s: AirbyteStream ->
    ...
    private fun generateDefaultConfiguration(
            stream: AirbyteStream): AirbyteStreamConfiguration {
        val result =
        ...
        if (stream.supportedSyncModes!!.size > 0) {
            result.syncMode = Enums.convertTo(stream.supportedSyncModes!![0],
    ...
    private fun toAirbyteStreamClientApi(
            stream: io.airbyte.protocol.models.AirbyteStream): AirbyteStream {
        return AirbyteStream()
                .name(stream.name)
                .jsonSchema(stream.jsonSchema)
                .supportedSyncModes(Enums.convertListTo(stream.supportedSyncModes,
                        io.airbyte.api.client.model.generated.SyncMode::class.java))
