...
import io.airbyte.integrations.base.destination.typing_deduping.StreamId.finalTableId
...
internal class MockSqlGenerator : SqlGenerator {
    ...
    override fun updateTable(stream: StreamConfig?,
                             finalSuffix: String?,
                             minRawTimestamp: Optional<Instant?>?,
                             useExpensiveSaferCasting: Boolean): Sql? {
        val timestampFilter = minRawTimestamp
        ...
        return of(("UPDATE TABLE " + stream!!.id().finalTableId("", finalSuffix)).toString() + casting + timestampFilter)
    }

    override fun overwriteFinalTable(stream: StreamId?, finalSuffix: String?): Sql? {
        return of("OVERWRITE TABLE " + stream!!.finalTableId("") + " FROM " + stream.finalTableId("", finalSuffix!!))
    }

    override fun migrateFromV1toV2(streamId: StreamId?, namespace: String?, tableName: String?): Sql? {
        return of("MIGRATE TABLE " + java.lang.String.join(".", namespace, tableName) + " TO " + streamId!!.rawTableId(""))
    }

    override fun prepareTablesForSoftReset(stream: StreamConfig): Sql? {
        return of("PREPARE " + java.lang.String.join(".", stream.id().originalNamespace(), stream.id().originalName()) + " FOR SOFT RESET")
    }
