...
import io.airbyte.integrations.base.destination.typing_deduping.StreamId.rawTableId
...
class DefaultTyperDeduperTest {
    ...
    private data class MockState(val needsSoftReset: Boolean,
                                 ...
                                 val nonSoftResetMigrationCompleted: Boolean) : MinimumDestinationState {
        override fun withSoftReset(needsSoftReset: Boolean): MockState {
            return MockState(needsSoftReset, this.softResetMigrationCompleted, this.nonSoftResetMigrationCompleted)
        }
    }

    private var sqlGenerator: MockSqlGenerator? = null
    private var destinationHandler: DestinationHandler<MockState>? = null

    private var initialStates: List<DestinationInitialStatus<MockState>?>? = null
    private var updatedStates: MutableMap<StreamId?, MockState>? = null

    private var migrator: DestinationV1V2Migrator? = null
    private var typerDeduper: TyperDeduper? = null

    private val MIGRATION_REQUIRING_SOFT_RESET: Migration<MockState> = object : Migration<MockState?> {
        @SneakyThrows
        override fun migrateIfNecessary(destinationHandler: DestinationHandler<MockState>,
                                        stream: StreamConfig,
                                        state: DestinationInitialStatus<MockState>): Migration.MigrationResult<MockState> {
            destinationHandler.execute(of("MIGRATE " + stream.id().rawTableId("")))
            return Migration.MigrationResult(MockState(true, true, state.destinationState.nonSoftResetMigrationCompleted), false)
        }
    }

    private val MIGRATION_NOT_REQUIRING_SOFT_RESET: Migration<MockState> = object : Migration<MockState?> {
        override fun migrateIfNecessary(destinationHandler: DestinationHandler<MockState>,
                                        stream: StreamConfig,
                                        status: DestinationInitialStatus<MockState>): Migration.MigrationResult<MockState> {
    ...
    private val MIGRATION_NOOP: Migration<MockState> = object : Migration<MockState?> {
        override fun migrateIfNecessary(destinationHandler: DestinationHandler<MockState>,
                                        stream: StreamConfig,
                                        status: DestinationInitialStatus<MockState>): Migration.MigrationResult<MockState> {
            return Migration.MigrationResult(
    ...
    fun setup() {
        ...
        val overwriteNsState: DestinationInitialStatus<MockState> = Mockito.mock(DestinationInitialStatus::class.java)
        ...
        val appendNsState: DestinationInitialStatus<MockState> = Mockito.mock(DestinationInitialStatus::class.java)
        ...
        val dedupeNsState: DestinationInitialStatus<MockState> = Mockito.mock(DestinationInitialStatus::class.java)
        ...
        initialStates
                .forEach(Consumer { initialState: DestinationInitialStatus<MockState>? -> Mockito.`when`(initialState!!.initialRawTableStatus).thenReturn(InitialRawTableStatus(true, true, Optional.empty())) })
        ...
        parsedCatalog = ParsedCatalog(java.util.List.of(
                OVERWRITE_STREAM_CONFIG,
                APPEND_STREAM_CONFIG,
                DEDUPE_STREAM_CONFIG))

        typerDeduper = DefaultTyperDeduper(sqlGenerator, destinationHandler, parsedCatalog!!, migrator, emptyList())
    }
    ...
    fun emptyDestination() {
        initialStates!!.forEach(Consumer { initialState: DestinationInitialStatus<MockState>? -> Mockito.`when`(initialState!!.isFinalTablePresent).thenReturn(false) })
        ...
        Mockito.verify(destinationHandler).execute(of("UPDATE TABLE overwrite_ns.overwrite_stream WITHOUT SAFER CASTING"))
        typerDeduper!!.typeAndDedupe("append_ns", "append_stream", false)
        Mockito.verify(destinationHandler).execute(of("UPDATE TABLE append_ns.append_stream WITHOUT SAFER CASTING"))
        typerDeduper!!.typeAndDedupe("dedup_ns", "dedup_stream", false)
        Mockito.verify(destinationHandler).execute(of("UPDATE TABLE dedup_ns.dedup_stream WITHOUT SAFER CASTING"))
        Mockito.verifyNoMoreInteractions(*Mockito.ignoreStubs(destinationHandler))
        Mockito.clearInvocations(destinationHandler)
    ...
    fun existingEmptyTable() {
        initialStates!!.forEach(Consumer { initialState: DestinationInitialStatus<MockState>? ->
        ...
        typerDeduper!!.prepareFinalTables()
        Mockito.verify(destinationHandler).execute(of("CREATE TABLE overwrite_ns.overwrite_stream_airbyte_tmp"))
        Mockito.verify(destinationHandler).execute(of("PREPARE append_ns.append_stream FOR SOFT RESET"))
        Mockito.verify(destinationHandler).execute(of("UPDATE TABLE append_ns.append_stream_ab_soft_reset WITHOUT SAFER CASTING"))
        Mockito.verify(destinationHandler).execute(of("OVERWRITE TABLE append_ns.append_stream FROM append_ns.append_stream_ab_soft_reset"))
        Mockito.verify(destinationHandler).execute(of("PREPARE dedup_ns.dedup_stream FOR SOFT RESET"))
        Mockito.verify(destinationHandler).execute(of("UPDATE TABLE dedup_ns.dedup_stream_ab_soft_reset WITHOUT SAFER CASTING"))
        Mockito.verify(destinationHandler).execute(of("OVERWRITE TABLE dedup_ns.dedup_stream FROM dedup_ns.dedup_stream_ab_soft_reset"))
        Mockito.verify(destinationHandler).commitDestinationStates(updatedStates)
        Mockito.verifyNoMoreInteractions(*Mockito.ignoreStubs(destinationHandler))
        Mockito.clearInvocations(destinationHandler)

        typerDeduper!!.typeAndDedupe("overwrite_ns", "overwrite_stream", false)
        Mockito.verify(destinationHandler).execute(of("UPDATE TABLE overwrite_ns.overwrite_stream_airbyte_tmp WITHOUT SAFER CASTING"))
        typerDeduper!!.typeAndDedupe("append_ns", "append_stream", false)
        Mockito.verify(destinationHandler).execute(of("UPDATE TABLE append_ns.append_stream WITHOUT SAFER CASTING"))
        typerDeduper!!.typeAndDedupe("dedup_ns", "dedup_stream", false)
        Mockito.verify(destinationHandler).execute(of("UPDATE TABLE dedup_ns.dedup_stream WITHOUT SAFER CASTING"))
        ...
        typerDeduper!!.commitFinalTables()
        Mockito.verify(destinationHandler).execute(of("OVERWRITE TABLE overwrite_ns.overwrite_stream FROM overwrite_ns.overwrite_stream_airbyte_tmp"))
        Mockito.verifyNoMoreInteractions(*Mockito.ignoreStubs(destinationHandler))
    }
    ...
    fun existingNonemptyTable() {
        initialStates!!.forEach(Consumer { initialState: DestinationInitialStatus<MockState>? ->
            ...
            Mockito.`when`(initialState.initialRawTableStatus)
                    .thenReturn(InitialRawTableStatus(true, true, Optional.of(Instant.parse("2023-01-01T12:34:56Z"))))
        })
        ...
        Mockito.verify(destinationHandler)
                .execute(of("UPDATE TABLE overwrite_ns.overwrite_stream_airbyte_tmp WITHOUT SAFER CASTING WHERE extracted_at > 2023-01-01T12:34:56Z"))
        typerDeduper!!.typeAndDedupe("append_ns", "append_stream", false)
        Mockito.verify(destinationHandler)
                .execute(of("UPDATE TABLE append_ns.append_stream WITHOUT SAFER CASTING WHERE extracted_at > 2023-01-01T12:34:56Z"))
        typerDeduper!!.typeAndDedupe("dedup_ns", "dedup_stream", false)
        Mockito.verify(destinationHandler).execute(of("UPDATE TABLE dedup_ns.dedup_stream WITHOUT SAFER CASTING WHERE extracted_at > 2023-01-01T12:34:56Z"))
        Mockito.verifyNoMoreInteractions(*Mockito.ignoreStubs(destinationHandler))
        Mockito.clearInvocations(destinationHandler)
    ...
    fun existingNonemptyTableMatchingSchema() {
        initialStates!!.forEach(Consumer { initialState: DestinationInitialStatus<MockState>? ->
            ...
            Mockito.`when`(initialState.isFinalTableEmpty).thenReturn(false)
            Mockito.`when`(initialState.isSchemaMismatch).thenReturn(false)
            Mockito.`when`(initialState.initialRawTableStatus).thenReturn(InitialRawTableStatus(true, true, Optional.of(Instant.now())))
        })
    ...
    @Throws(Exception::class)
    fun failedSetup() {
        Mockito.doThrow(RuntimeException("foo")).`when`(destinationHandler).execute(ArgumentMatchers.any())
    ...
    fun noUnprocessedRecords() {
        initialStates
                .forEach(Consumer { initialState: DestinationInitialStatus<MockState>? -> Mockito.`when`(initialState!!.initialRawTableStatus).thenReturn(InitialRawTableStatus(true, false, Optional.empty())) })
        ...
        typerDeduper!!.typeAndDedupe(Map.of(
                StreamDescriptor().withName("overwrite_stream").withNamespace("overwrite_ns"), StreamSyncSummary(Optional.of(0L)),
                StreamDescriptor().withName("append_stream").withNamespace("append_ns"), StreamSyncSummary(Optional.of(1L))))
    ...
    fun unprocessedRecords() {
        initialStates!!.forEach(Consumer { initialState: DestinationInitialStatus<MockState>? ->
            Mockito.`when`(initialState!!.initialRawTableStatus)
                    .thenReturn(InitialRawTableStatus(true, true, Optional.of(Instant.parse("2023-01-23T12:34:56Z"))))
        })
        ...
        Mockito.verify(destinationHandler)
                .execute(of("UPDATE TABLE overwrite_ns.overwrite_stream WITHOUT SAFER CASTING WHERE extracted_at > 2023-01-23T12:34:56Z"))
        Mockito.verify(destinationHandler)
                .execute(of("UPDATE TABLE append_ns.append_stream WITHOUT SAFER CASTING WHERE extracted_at > 2023-01-23T12:34:56Z"))
        Mockito.verify(destinationHandler).execute(of("UPDATE TABLE dedup_ns.dedup_stream WITHOUT SAFER CASTING WHERE extracted_at > 2023-01-23T12:34:56Z"))
    }
    ...
    fun multipleSoftResets() {
        typerDeduper = DefaultTyperDeduper(
                sqlGenerator!!,
                destinationHandler,
                parsedCatalog!!,
                migrator!!,
        ...
        Mockito.verify(destinationHandler).commitDestinationStates(Map.of(
                OVERWRITE_STREAM_CONFIG.id(), MockState(true, true, true),
                APPEND_STREAM_CONFIG.id(), MockState(true, true, true),
                DEDUPE_STREAM_CONFIG.id(), MockState(true, true, true)))
        ...
        Mockito.verify(destinationHandler).commitDestinationStates(Map.of(
                OVERWRITE_STREAM_CONFIG.id(), MockState(false, true, true),
                APPEND_STREAM_CONFIG.id(), MockState(false, true, true),
                DEDUPE_STREAM_CONFIG.id(), MockState(false, true, true)))
    ...
    fun migrationsMixedResults() {
        typerDeduper = DefaultTyperDeduper(
                ...
                parsedCatalog!!,
                migrator!!,
                java.util.List.of(MIGRATION_REQUIRING_SOFT_RESET, MIGRATION_NOT_REQUIRING_SOFT_RESET))
    ...
    fun previousSyncSoftReset() {
        ...
        Mockito.verify(destinationHandler).commitDestinationStates(Map.of(
                OVERWRITE_STREAM_CONFIG.id(), MockState(true, false, false),
                APPEND_STREAM_CONFIG.id(), MockState(true, false, false),
                DEDUPE_STREAM_CONFIG.id(), MockState(true, false, false)))
        Mockito.verify(destinationHandler).gatherInitialState(ArgumentMatchers.any())
        Mockito.verify(destinationHandler)
                .execute(separately("CREATE SCHEMA airbyte_internal", "CREATE SCHEMA overwrite_ns", "CREATE SCHEMA append_ns", "CREATE SCHEMA dedup_ns"))
        Mockito.verifyNoMoreInteractions(destinationHandler)
        ...
        // And we should commit the states. Note that we now set needsSoftReset=false.
        Mockito.verify(destinationHandler).commitDestinationStates(Map.of(
                OVERWRITE_STREAM_CONFIG.id(), MockState(false, false, false),
                APPEND_STREAM_CONFIG.id(), MockState(false, false, false),
                DEDUPE_STREAM_CONFIG.id(), MockState(false, false, false)))

        Mockito.verifyNoMoreInteractions(destinationHandler)
    }

    companion object {
        private val OVERWRITE_STREAM_CONFIG = StreamConfig(
                StreamId("overwrite_ns", "overwrite_stream", "airbyte_internal", "overwrite_stream", "overwrite_ns", "overwrite_stream"),
                null,
                DestinationSyncMode.OVERWRITE,
                null,
                null,
                null)
        private val APPEND_STREAM_CONFIG = StreamConfig(
                StreamId("append_ns", "append_stream", "airbyte_internal", "append_stream", "append_ns", "append_stream"),
                null,
                DestinationSyncMode.APPEND,
                null,
                null,
                null)
        private val DEDUPE_STREAM_CONFIG = StreamConfig(
                StreamId("dedup_ns", "dedup_stream", "airbyte_internal", "dedup_stream", "dedup_ns", "dedup_stream"),
                null,
