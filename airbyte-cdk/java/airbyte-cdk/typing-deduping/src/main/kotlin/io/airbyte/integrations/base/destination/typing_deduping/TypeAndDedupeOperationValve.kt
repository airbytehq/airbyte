 ...
 * stream
 */
class TypeAndDedupeOperationValve @JvmOverloads constructor(private val nowness: Supplier<Long> = SYSTEM_NOW) : ConcurrentHashMap<AirbyteStreamNameNamespacePair, Long?>() {
    ...
    fun readyToTypeAndDedupe(key: AirbyteStreamNameNamespacePair): Boolean {
        ...
        return nowness.get() - get(key)!! > typeAndDedupeIncreasingIntervals[incrementalIndex[key]!!]
    }

    /**
     * Increment the interval at which typing and deduping should occur for the stream, max out at last
     * index of [TypeAndDedupeOperationValve.typeAndDedupeIncreasingIntervals]
     *
     ...
     * [TypeAndDedupeOperationValve.readyToTypeAndDedupe] will set
    ...
    companion object {
        private val LOGGER: Logger = LoggerFactory.getLogger(TypeAndDedupeOperationValve::class.java)
        ...
        val typeAndDedupeIncreasingIntervals: List<Long> = java.util.List.of(NEGATIVE_MILLIS, SIX_HOURS_MILLIS)
