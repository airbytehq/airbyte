...
class CatalogParser @JvmOverloads constructor(private val sqlGenerator: SqlGenerator, private val rawNamespace: String = DEFAULT_AIRBYTE_INTERNAL_NAMESPACE) {
    fun parseCatalog(catalog: ConfiguredAirbyteCatalog): ParsedCatalog {
        ...
        for (stream in catalog.streams) {
            ...
            if (streamConfigs.stream().anyMatch { s: StreamConfig? -> s!!.id!!.finalTableId("") == originalStreamConfig.id!!.finalTableId("") }
                    || streamConfigs.stream().anyMatch { s: StreamConfig? -> s!!.id!!.rawTableId("") == originalStreamConfig.id!!.rawTableId("") }) {
                ...
                val hash = DigestUtils.sha1Hex(originalStreamConfig.id!!.finalNamespace + "&airbyte&" + originalName).substring(0, 3)
                val newName = originalName + "_" + hash
                actualStreamConfig = StreamConfig(
            ...
            actualStreamConfig.columns.keys.forEach(Consumer { columnId: ColumnId? ->
            ...
            actualStreamConfig.primaryKey.forEach(Consumer { pk: ColumnId? ->
    ...
    fun toStreamConfig(stream: ConfiguredAirbyteStream): StreamConfig {
        ...
        val airbyteColumns = if (schema is Struct) {
            schema.properties
        ...
        require(!stream.primaryKey.stream().anyMatch { key: List<String?> -> key.size > 1 }) { "Only top-level primary keys are supported" }
        val primaryKey = stream.primaryKey.stream().map { key: List<String?> -> sqlGenerator.buildColumnId(key[0]) }.toList()
        ...
        for ((key, value) in airbyteColumns) {
            ...
            if (columns.keys.stream().noneMatch { c: ColumnId? -> c!!.canonicalName == originalColumnId!!.canonicalName }) {
                ...
                while (true) {
                    ...
                    if (columns.keys.stream().noneMatch { c: ColumnId? -> c!!.canonicalName == canonicalName }) {
                        break
                    } else {
                        i++
                    }
