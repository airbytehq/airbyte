...
interface TyperDeduper {
     ...
     *
     *
     * This method may affect the behavior of [.prepareFinalTables]. For example, modifying a
     ...
     * run (for example, at the end of a sync), they may set `mustRun` to true.
     *
     *
     * This method relies on callers to prevent concurrent modification to the underlying raw tables.
     * This is most easily accomplished using [.getRawTableInsertLock], if the
     * caller guards all raw table writes using `getRawTableInsertLock().lock()` and
     * `getRawTableInsertLock().unlock()`. While `typeAndDedupe` is executing, that lock
     ...
     * [.typeAndDedupe] will not proceed while this lock is held.
     *
     *
