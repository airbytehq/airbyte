...
public class TypeAndDedupeTransaction {
  ...
  /**
   * It can be expensive to build the errors array in the airbyte_meta column, so we first attempt an 'unsafe' transaction which assumes everything is
   * typed correctly. If that fails, we will run a more expensive query which handles casting errors
   ...
   */
  public static void executeTypeAndDedupe(final SqlGenerator sqlGenerator, final DestinationHandler destinationHandler, StreamConfig streamConfig, Optional<Instant> minExtractedAt, String suffix) throws Exception {
    try {
      LOGGER.info("Attempting typing and deduping for {}.{} with suffix", streamConfig.id().originalNamespace(), streamConfig.id().originalName(), suffix);
  ...
  /**
   * Everything in {@link TypeAndDedupeTransaction#executeTypeAndDedupe(SqlGenerator, DestinationHandler, StreamConfig, Optional, String)} but with
   * a little extra prep work for the soft reset temp tables
   * @param sqlGenerator for generating sql for the destination
   * @param destinationHandler for executing sql created
   * @param streamConfig which stream to operate on
   * @throws Exception if the safe query fails
   */
  public static void executeSoftReset(final SqlGenerator sqlGenerator, final DestinationHandler destinationHandler, StreamConfig streamConfig) throws Exception {
