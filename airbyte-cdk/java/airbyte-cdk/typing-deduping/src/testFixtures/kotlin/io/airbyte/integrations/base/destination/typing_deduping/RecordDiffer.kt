 ...
 * output format.
 */
class RecordDiffer @SafeVarargs constructor(private val rawRecordColumnNames: Map<String, String>,
     ...
     */
    init {
        val rawTableIdentifyingColumns: Array<Pair<String, AirbyteType>> = Arrays.stream<Pair<ColumnId, AirbyteType>>(identifyingColumns)
                .map<Any>(Function<Pair<ColumnId, AirbyteType>, Any> { p: Pair<ColumnId, AirbyteType> ->
                    Pair.of( // Raw tables always retain the original column names
                            p.left.originalName(),
                            p.right)
                })
                .toArray<Pair<*, *>>(IntFunction<Array<Pair<*, *>>> { _Dummy_.__Array__() })
        ...
        this.rawRecordSortComparator = rawRecordIdentityComparator
                .thenComparing { record: JsonNode -> asString(record[getMetadataColumnName(rawRecordColumnNames, "_airbyte_raw_id")]) }
        this.rawRecordIdentityExtractor = buildIdentityExtractor(rawTableIdentifyingColumns, rawRecordColumnNames)

        val finalTableIdentifyingColumns: Array<Pair<String, AirbyteType>> = Arrays.stream<Pair<ColumnId, AirbyteType>>(identifyingColumns)
                .map<Pair<String, AirbyteType>> { p: Pair<ColumnId, AirbyteType> ->
                    Pair.of<String, AirbyteType>( // Final tables may have modified the column names, so use the final name here.
                            p.left.name(),
                            p.right)
                }
                .toArray<Pair<*, *>>(IntFunction<Array<Pair<*, *>>> { _Dummy_.__Array__() })
        ...
        this.finalRecordSortComparator = finalRecordIdentityComparator
                .thenComparing { record: JsonNode -> asString(record[getMetadataColumnName(finalRecordColumnNames, "_airbyte_raw_id")]) }
     ...
     */
    fun verifySyncResult(expectedRawRecords: List<JsonNode>,
    ...
    fun diffRawTableRecords(expectedRecords: List<JsonNode>, actualRecords: List<JsonNode>) {
        val diff = diffRecords(
                expectedRecords.stream().map { record: JsonNode -> this.copyWithLiftedData(record) }.collect(Collectors.toList()),
                actualRecords.stream().map { record: JsonNode -> this.copyWithLiftedData(record) }.collect(Collectors.toList()),
    ...
    private fun copyWithLiftedData(record: JsonNode): JsonNode {
        ...
        Streams.stream(airbyteData.fields()).forEach { field: Map.Entry<String, JsonNode?> ->
            if (!copy.has(field.key)) {
                ...
                throw RuntimeException("Cannot lift field " + field.key + " because it already exists in the record.")
            }
        }
        return copy
    }
     ...
     */
    private fun buildIdentityComparator(identifyingColumns: Array<Pair<String, AirbyteType>>, columnNames: Map<String, String>): Comparator<JsonNode> {
        // Start with a noop comparator for convenience
        var comp = Comparator.comparing { record: JsonNode? -> 0 }
        for ((key, value) in identifyingColumns) {
            comp = comp.thenComparing(Function { record: JsonNode -> extract(record, key, value) })
        }
        comp = comp.thenComparing { record: JsonNode -> asTimestampWithTimezone(record[getMetadataColumnName(columnNames, "_airbyte_extracted_at")]) }
        return comp
    }

    /**
     * See [&lt;][.buildIdentityComparator] for an explanation of
     * dataExtractor.
     */
    private fun buildIdentityExtractor(identifyingColumns: Array<Pair<String, AirbyteType>>,
                                       columnNames: Map<String, String>): Function<JsonNode, String> {
        return Function { record: JsonNode ->
            (Arrays.stream(identifyingColumns)
                    .map { column: Pair<String, AirbyteType> -> getPrintableFieldIfPresent(record, column.key) }
     ...
     */
    private fun diffRecords(originalExpectedRecords: List<JsonNode>,
                            ...
                            columnNames: Map<String, String>): String {
        ...
        while (expectedRecordIndex < expectedRecords.size) {
            message += "Row was expected but missing: " + expectedRecords[expectedRecordIndex] + "\n"
            expectedRecordIndex++
        }
        while (actualRecordIndex < actualRecords.size) {
            message += "Row was not expected but present: " + actualRecords[actualRecordIndex] + "\n"
            actualRecordIndex++
        }
    ...
    private fun diffSingleRecord(recordIdExtractor: Function<JsonNode, String>,
                                 expectedRecord: JsonNode,
                                 actualRecord: JsonNode,
                                 columnNames: Map<String, String>): String {
        var foundMismatch = false
        var mismatchedRecordMessage = "Row had incorrect data: " + recordIdExtractor.apply(expectedRecord) + "\n"
     ...
     * [BaseTypingDedupingTest.dumpFinalTableRecords] for an explanation of how
     ...
     */
    private fun checkForExtraOrNonNullFields(expectedRecord: JsonNode,
                                             actualRecord: JsonNode,
                                             columnNames: Map<String, String>): LinkedHashMap<String, JsonNode> {
    ...
    companion object {
        ...
        private fun areJsonNodesEquivalent(expectedValue: JsonNode?, actualValue: JsonNode?): Boolean {
            return if (expectedValue == null || actualValue == null) {
                ...
                IntStream.range(0, expectedValue.size())
                        .allMatch { i: Int -> areJsonNodesEquivalent(expectedValue[i], actualValue[i]) }
            } else if (expectedValue is ObjectNode && actualValue is ObjectNode) {
                // If both values are objects compare their fields and values
                expectedValue.size() == actualValue.size() && Stream.generate { expectedValue.fieldNames().next() }
                        .limit(expectedValue.size().toLong())
                        .allMatch { field: String? -> areJsonNodesEquivalent(expectedValue[field], actualValue[field]) }
            } else {
                ...
                (expectedValue == actualValue || (expectedValue.isIntegralNumber && actualValue.isIntegralNumber && expectedValue.bigIntegerValue() == actualValue.bigIntegerValue())
                        || (expectedValue.isNumber && actualValue.isNumber && expectedValue.decimalValue() == actualValue.decimalValue()))
            }
        }
        ...
        private fun extract(node: JsonNode, field: String, type: AirbyteType): Comparable<*> {
            return if (type is AirbyteProtocolType) {
                when (type) {
                    ...
                    AirbyteProtocolType.TIMESTAMP_WITH_TIMEZONE -> asTimestampWithTimezone(node[field])
                    AirbyteProtocolType.TIMESTAMP_WITHOUT_TIMEZONE -> asTimestampWithoutTimezone(node[field])
                    AirbyteProtocolType.TIME_WITH_TIMEZONE -> asTimeWithTimezone(node[field])
                    AirbyteProtocolType.TIME_WITHOUT_TIMEZONE -> asTimeWithoutTimezone(node[field])
