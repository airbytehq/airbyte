...
class DebeziumEngineSharedStateTest {
    ...
    fun testMaxRecords() {
        ...
        doTestCase(testConfig(maxRecords = 2), expected,
                Change(1),
                Change(2),
                Change(3)
        )
    ...
    fun testSnapshotting() {
        ...
        doTestCase(testConfig(), expected,
                SnapshotOngoing(1),
                SnapshotOngoing(2),
                SnapshotComplete(3),
                Change(4)
    ...
    fun testBoundChecks() {
        ...
        doTestCase(testConfig(maxLsn = 2), expected,
                Wait(),
                Change(1),
                Wait(),
                Change(2),
    ...
    fun testMemoryPressure() {
        ...
        doTestCase(testConfig(maxRecordBytes = 60), expected,
                Heartbeat(1),
                Heartbeat(2),
                Change(3),
                Change(4),
    ...
    fun testOverallLatency() {
        ...
        doTestCase(testConfig(maxTime = Duration.ofSeconds(2)), expected,
                Wait(),
                Heartbeat(1),
                Wait(),
                Heartbeat(2),
    ...
    fun testConfig(maxLsn: Long = Long.MAX_VALUE,
                ...
                .withLsnMapper(object : DebeziumComponent.Config.LsnMapper<Long> {

                    override fun get(offset: DebeziumComponent.State.Offset): Long = maxLsn

                    override fun get(record: DebeziumComponent.Record): Long? = record.debeziumEventValue["lsn"].asLong()
    ...
    fun doTestCase(config: Config, expected: EndState, vararg events: Event) {
        ...
        Assertions.assertEquals(output.data.count().toLong(), output.executionSummary.records.count())
        ...
        val actual = EndState(
                output.data.map { r -> Pair(r.debeziumEventValue["lsn"].asInt(), r.kind()) }.toList(),
