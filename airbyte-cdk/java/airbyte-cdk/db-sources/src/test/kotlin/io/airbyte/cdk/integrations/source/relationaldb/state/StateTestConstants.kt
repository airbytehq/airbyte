...
object StateTestConstants {
    ...
    val NAME_NAMESPACE_PAIR1: AirbyteStreamNameNamespacePair = AirbyteStreamNameNamespacePair(STREAM_NAME1, NAMESPACE)
    const val STREAM_NAME2: String = "bicycles"
    val NAME_NAMESPACE_PAIR2: AirbyteStreamNameNamespacePair = AirbyteStreamNameNamespacePair(STREAM_NAME2, NAMESPACE)
    ...
    fun getState(cursorField: String?, cursor: String?): Optional<DbStreamState?> {
        return Optional.of(DbStreamState()
    ...
    fun getCatalog(cursorField: String?): Optional<ConfiguredAirbyteCatalog?> {
        return Optional.of(ConfiguredAirbyteCatalog()
                .withStreams(List.of(getStream(cursorField).orElse(null))))
    }

    fun getStream(cursorField: String?): Optional<ConfiguredAirbyteStream?> {
        return Optional.of(ConfiguredAirbyteStream()
                .withStream(AirbyteStream().withName(STREAM_NAME1))
                .withCursorField(if (cursorField == null) emptyList() else Lists.newArrayList(cursorField)))
    }
}
