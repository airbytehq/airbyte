...
class DebeziumProducerBuilderTest {
    ...
    private fun checkTopicName(name: String, expected: String) {
        val actual = DebeziumProducer.Builder().withDebeziumName(name).props["topic.prefix"]
    ...
    private fun checkIncludeLists(
        ...
        val actual = DebeziumProducer.Builder().withCatalog(configuredCatalog).props
        Assertions.assertEquals(expectedTableIncludeList, actual["table.include.list"])
        Assertions.assertEquals(expectedColumnIncludeList, actual["column.include.list"])
    }
