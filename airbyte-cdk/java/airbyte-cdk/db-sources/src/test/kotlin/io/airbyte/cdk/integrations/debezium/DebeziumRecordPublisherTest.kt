...
internal class DebeziumRecordPublisherTest {
    ...
    fun testTableIncludelistCreation() {
        ...
        val actualWhitelist = RelationalDbDebeziumPropertiesManager.getTableIncludelist(catalog, getCdcStreamListFromCatalog(catalog))
    ...
    fun testColumnIncludelistFiltersFullRefresh() {
        ...
        val actualWhitelist = RelationalDbDebeziumPropertiesManager.getColumnIncludeList(catalog, getCdcStreamListFromCatalog(catalog))
    ...
    fun testTableIncludelistFiltersStreamList() {
        ...
        val expectedWhitelist = "\\Qpublic.id_and_name\\E"
        val cdcStreamList = catalog.streams.stream()
            .filter { configuredStream: ConfiguredAirbyteStream -> configuredStream.stream.name.equals("id_and_name") }
            .map { stream: ConfiguredAirbyteStream ->
                stream.stream.namespace + "." + stream.stream.name
            }.toList()
        val actualWhitelist = RelationalDbDebeziumPropertiesManager.getTableIncludelist(catalog, cdcStreamList)
    ...
    fun testColumnIncludelistFiltersStreamList() {
        ...
        val expectedWhitelist =
            "\\Qpublic.id_and_name\\E\\.(\\Qfld2\\E|\\Qfld1\\E),\\Qpublic.id_\\,something\\E,\\Qpublic.n\"aMéS\\E"
        val cdcStreamList = catalog.streams.stream()
            .filter { configuredStream: ConfiguredAirbyteStream -> !configuredStream.stream.name.equals("id_and_name2")}
            .map { stream: ConfiguredAirbyteStream ->
                stream.stream.namespace + "." + stream.stream.name
            }.toList()
        val actualWhitelist = RelationalDbDebeziumPropertiesManager.getColumnIncludeList(catalog, cdcStreamList)
    ...
    fun testColumnIncludeListEscaping() {
        ...
        val anchored =
            "^" + RelationalDbDebeziumPropertiesManager.getColumnIncludeList(catalog, getCdcStreamListFromCatalog(catalog)) + "$"
