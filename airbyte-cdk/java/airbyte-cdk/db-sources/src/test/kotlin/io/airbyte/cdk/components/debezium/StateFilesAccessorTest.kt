...
class StateFilesAccessorTest {
    ...
    fun testOffsetRoundTrip() {
        ...
        val expected = DebeziumState.Offset(mapOf(Pair(key, expectedValue)))
        ...
        StateFilesAccessor().use {
            ...
            Assertions.assertTrue(it.offsetFilePath.toFile().exists())
            val actual =
                it.readUpdatedOffset(DebeziumState.Offset(mapOf(Pair(key, Jsons.emptyObject()))))
    ...
    fun testSchemaRoundTrip() {
        ...
        StateFilesAccessor().use {
            ...
            Assertions.assertEquals(
                expected.debeziumSchemaHistory.size,
                actual.debeziumSchemaHistory.size
            )
            for (idx in 1..expected.debeziumSchemaHistory.size) {
                val expectedDocument = expected.debeziumSchemaHistory.get(idx - 1).document()
                val actualDocument = actual.debeziumSchemaHistory.get(idx - 1).document()
