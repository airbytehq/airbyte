 ...
 */
class AirbyteFileOffsetBackingStore(private val offsetFilePath: Path, private val dbName: Optional<String>) {
    fun read(): Map<String, String> {
        val raw = load()

        return raw.entries.stream().collect(Collectors.toMap(
                Function { e: Map.Entry<ByteBuffer?, ByteBuffer?> -> byteBufferToString(e.key) },
                Function { e: Map.Entry<ByteBuffer?, ByteBuffer?> -> byteBufferToString(e.value) }))
    }
    ...
    fun persist(cdcState: JsonNode?) {
        ...
        val mappedAsStrings = updatedMap.entries.stream().collect(Collectors.toMap(
    ...
    private fun updateStateForDebezium2_1(mapAsString: Map<String, String?>): Map<String, String?> {
        ...
        if (mapAsString.size > 0) {
            ...
            val newKey = if (dbName.isPresent) SQL_SERVER_STATE_MUTATION.apply(key.substring(i, i1 + 1), dbName.get()) else key.substring(i, i1 + 1)
    ...
    private fun load(): Map<ByteBuffer?, ByteBuffer?> {
        ...
        if (obj !is HashMap<*, *>) throw ConnectException("Expected HashMap but found " + obj.javaClass)
    ...
    companion object {
        private val LOGGER: Logger = LoggerFactory.getLogger(AirbyteFileOffsetBackingStore::class.java)
