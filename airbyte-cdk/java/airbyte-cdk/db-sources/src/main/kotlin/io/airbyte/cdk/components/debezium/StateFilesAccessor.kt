...
class StateFilesAccessor : AutoCloseable {
    ...
    fun readUpdatedOffset(offset: DebeziumComponent.State.Offset): DebeziumComponent.State.Offset {
        ...
        fileOffsetBackingStore.stop()
        val debeziumOffset =
            future.resultNow().map { (key, value) -> Pair(toJson(key), toJson(value)) }.toMap()
    ...
    fun writeOffset(offset: DebeziumComponent.State.Offset) {
        val bytesMap =
            offset.debeziumOffset
                .map { (key, value) -> Pair(toByteBuffer(key), toByteBuffer(value)) }
                .toMap()
        fileOffsetBackingStore.start()
    ...
    init {
        ...
        offsetProps[StandaloneConfig.OFFSET_STORAGE_FILE_FILENAME_CONFIG] =
    ...
    companion object {
        ...
        private fun keyConverter(): Converter {
            ...
            val converterConfig =
                mapOf(Pair(JsonConverterConfig.SCHEMAS_ENABLE_CONFIG, false.toString()))
