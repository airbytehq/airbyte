...
class SourceStateIterator<T>(private val messageIterator: Iterator<T>,
                             private val stream: ConfiguredAirbyteStream,
                             private val sourceStateMessageProducer: SourceStateMessageProducer<*>,
                             private val stateEmitFrequency: StateEmitFrequency) : AbstractIterator<AirbyteMessage?>(), MutableIterator<AirbyteMessage?> {
    ...
    override fun computeNext(): AirbyteMessage? {
        ...
        if (iteratorHasNextValue) {
            if (shouldEmitStateMessage() && sourceStateMessageProducer.shouldEmitStateMessage(stream)) {
                val stateMessage = sourceStateMessageProducer.generateStateMessageAtCheckpoint(stream)
            ...
            try {
                ...
                val processedMessage = sourceStateMessageProducer.processRecordMessage(stream, message)
                recordCount++
            ...
            val finalStateMessageForStream = sourceStateMessageProducer.createFinalStateMessage(stream)
            finalStateMessageForStream!!.withSourceStats(AirbyteStateStats().withRecordCount(recordCount.toDouble()))
            recordCount = 0L
            return AirbyteMessage()
                    .withType(AirbyteMessage.Type.STATE)
