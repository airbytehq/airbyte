...
class DebeziumEngineSharedState(val config: DebeziumComponent.Config) {
    ...
    private val buffer: MutableList<BufferElement> =
        Collections.synchronizedList(ArrayList(1_000_000))
    ...
    private fun addToBuffer(
        record: DebeziumComponent.Record,
        isWithinBounds: Boolean,
        timestamp: Instant
    ) {
        val e =
            BufferElement(
                if (record.isHeartbeat) null else Jsons.toBytes(record.debeziumEventValue),
                isWithinBounds,
                timestamp.toEpochMilli()
            )
        synchronized(buffer) { buffer.add(e) }
        if (!e.isHeartbeat) {
    ...
    fun build(state: DebeziumComponent.State): DebeziumComponent.Output {
        val serializedRecordsBuilder =
            ImmutableList.builderWithExpectedSize<ByteArray>(numRecords.get().toInt())
        ...
        synchronized(buffer) {
            ...
            for ((serializedRecord, isWithinBounds, epochMilli) in buffer) {
                ...
                if (serializedRecord != null) {
                    ...
                    recordStats.addValue(
                        max(0.0, (epochMilli - previousRecordEpochMilli).toDouble())
                    )
                    previousRecordEpochMilli = epochMilli
                    if (!isWithinBounds) {
                        recordOutOfBoundsStats.addValue(
                            max(0.0, (epochMilli - previousRecordOutOfBoundsEpochMilli).toDouble())
                        )
        ...
        val lazyRecordSequence = Sequence {
            object : Iterator<DebeziumComponent.Record> {
                ...
                override fun next() =
                    DebeziumComponent.Record(Jsons.fromBytes(serializedRecords.get(index++)))
            }
        }
