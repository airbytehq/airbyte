...
/**
 * The purpose of this class is : to , 1. Read the contents of the file [.path] which contains
 * the schema history at the end of the sync so that it can be saved in state for future syncs.
 * Check [.read] 2. Write the saved content back to the file [.path] at the beginning
 * of the sync so that debezium can function smoothly. Check persist(Optional&lt;JsonNode&gt;).
 */
class AirbyteSchemaHistoryStorage(private val path: Path, private val compressSchemaHistoryForState: Boolean) {
    ...
    fun read(): SchemaHistory<String> {
        ...
        if ((fileSizeMB > SIZE_LIMIT_TO_COMPRESS_MB) && compressSchemaHistoryForState) {
            ...
            if (fileSizeMB > compressedSizeMB) {
                ...
                throw RuntimeException("Compressing increased the size of the content. Size before compression " + fileSizeMB + ", after compression "
                        + compressedSizeMB)
            }
                ...
                Files.newBufferedReader(path, UTF8).use { bufferedReader ->
                    while (true) {
                        ...
                        if (!line.isEmpty()) {
                            val record = reader.read(line)
                            val recordAsString = writer.write(record)
                            gzipOutputStream.write(recordAsString.toByteArray(StandardCharsets.UTF_8))
                            gzipOutputStream.write(lineSeparator.toByteArray(StandardCharsets.UTF_8))
                        }
                    }
                }
            }
    ...
    private fun makeSureFileExists() {
        try {
            ...
            throw IllegalStateException(
                    "Unable to check or create history file at " + path + ": " + e.message, e)
        }
    }
    ...
    private fun writeToFile(fileAsString: String) {
        try {
            val split = fileAsString.split(System.lineSeparator().toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()
            for (element in split) {
                val read = reader.read(element)
                val line = writer.write(read)

                Files
                        .newBufferedWriter(path, StandardOpenOption.APPEND).use { historyWriter ->
                            try {
    ...
    private fun writeCompressedStringToFile(compressedString: String) {
        try {
            ByteArrayInputStream(Jsons.deserialize(compressedString, ByteArray::class.java)).use { inputStream ->
    ...
    companion object {
        private val LOGGER: Logger = LoggerFactory.getLogger(AirbyteSchemaHistoryStorage::class.java)
