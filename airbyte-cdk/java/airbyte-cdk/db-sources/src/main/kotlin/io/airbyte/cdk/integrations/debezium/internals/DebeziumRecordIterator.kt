 ...
 */
class DebeziumRecordIterator<T>(private val queue: LinkedBlockingQueue<ChangeEvent<String?, String?>>,
    ...
    override fun computeNext(): ChangeEventWithMetadata? {
        ...
        while (!MoreBooleans.isTruthy(publisherStatusSupplier.get()) || !queue.isEmpty()) {
            ...
            val waitTime = if (receivedFirstRecord) this.subsequentRecordWaitTime else this.firstRecordWaitTime
            try {
            ...
            if (isHeartbeatEvent(next)) {
                ...
                } else if (heartbeatPos == this.lastHeartbeatPosition && heartbeatPosNotChanging()) {
        ...
        while (!debeziumShutdownProcedure.recordsRemainingAfterShutdown.isEmpty()) {
            ...
            try {
                event = debeziumShutdownProcedure.recordsRemainingAfterShutdown.poll(100, TimeUnit.MILLISECONDS)
            } catch (e: InterruptedException) {
                throw RuntimeException(e)
            }
    ...
    private fun heartbeatPosNotChanging(): Boolean {
        ...
        val timeElapsedSinceLastHeartbeatTs = Duration.between(this.tsLastHeartbeat, LocalDateTime.now())
