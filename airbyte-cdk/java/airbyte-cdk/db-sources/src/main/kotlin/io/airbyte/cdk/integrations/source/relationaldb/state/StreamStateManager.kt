...
/**
 * Constructs a new [StreamStateManager] that is seeded with the provided
 * [AirbyteStateMessage].
 *
 * @param airbyteStateMessages The initial state represented as a list of
 * [AirbyteStateMessage]s.
 * @param catalog The [ConfiguredAirbyteCatalog] for the connector associated with this state
 * manager.
 */(private val rawAirbyteStateMessages: List<AirbyteStateMessage?>, catalog: ConfiguredAirbyteCatalog?) : AbstractStateManager<AirbyteStateMessage?, AirbyteStreamState?>(catalog,
        Supplier { rawAirbyteStateMessages.stream().map { obj: AirbyteStateMessage? -> obj!!.stream }.collect(Collectors.toList()) },
    ...
    override val cdcStateManager: CdcStateManager
        get() {
            throw UnsupportedOperationException("CDC state management not supported by stream state manager.")
        }
        ...
        if (pair.isPresent) {
            ...
            if (cursorInfo.isPresent) {
                ...
                return AirbyteStateMessage()
                        .withType(AirbyteStateMessage.AirbyteStateType.STREAM) // Temporarily include legacy state for backwards compatibility with the platform
                ...
                return AirbyteStateMessage().withType(AirbyteStateMessage.AirbyteStateType.STREAM).withStream(AirbyteStreamState())
            }
        } else {
