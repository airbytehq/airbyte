...
object DbSourceDiscoverUtil {
    ...
    private val AIRBYTE_METADATA: List<String> = mutableListOf("_ab_cdc_lsn",
            "_ab_cdc_updated_at",
            "_ab_cdc_deleted_at")
   ...
   */
    fun <DataType> logSourceSchemaChange(fullyQualifiedTableNameToInfo: Map<String?, TableInfo<CommonField<DataType>>>,
        ...
        for (airbyteStream in catalog!!.streams) {
            ...
            val fullyQualifiedTableName = getFullyQualifiedTableName(stream.namespace,
                    stream.name)
            ...
            val fields = table.fields
                    .stream()
                    .map { commonField: CommonField<DataType> -> toField(commonField, airbyteTypeConverter) }
                    .distinct()
    ...
    fun <DataType> convertTableInfosToAirbyteCatalog(tableInfos: List<TableInfo<CommonField<DataType>>>,
                                                     fullyQualifiedTableNameToPrimaryKeys: Map<String, MutableList<String>>,
                                                     airbyteTypeConverter: Function<DataType, JsonSchemaType>): AirbyteCatalog {
        val tableInfoFieldList = tableInfos.stream()
                .map { t: TableInfo<CommonField<DataType>> ->
                    ...
                    val fields = t.fields
                            ...
                            .distinct()
                            .collect(Collectors.toList())
                    val fullyQualifiedTableName = getFullyQualifiedTableName(t.nameSpace,
                            t.name)
                    val primaryKeys = fullyQualifiedTableNameToPrimaryKeys.getOrDefault(
                            fullyQualifiedTableName, emptyList())
                    TableInfo.builder<Field>().nameSpace(t.nameSpace).name(t.name)
        ...
        val streams = tableInfoFieldList.stream()
                .map { tableInfo: TableInfo<Field> ->
                    val primaryKeys = tableInfo.primaryKeys.stream()
                            .filter { obj: String? -> Objects.nonNull(obj) }
                            .map(Function<String, List<String>> { o: String? -> listOf(o) })
                            .collect(Collectors.toList())
                    CatalogHelpers
                            .createAirbyteStream(tableInfo.name, tableInfo.nameSpace,
                                    tableInfo.fields)
    ...
    private fun <DataType> toField(commonField: CommonField<DataType>, airbyteTypeConverter: Function<DataType, JsonSchemaType>): Field {
        if (airbyteTypeConverter.apply(commonField.type) === JsonSchemaType.OBJECT && commonField.properties != null && !commonField.properties.isEmpty()) {
            val properties = commonField.properties.stream().map { commField: CommonField<DataType> -> toField(commField, airbyteTypeConverter) }.toList()
            return Field.of(commonField.name, airbyteTypeConverter.apply(commonField.type), properties)
        } else {
            return Field.of(commonField.name, airbyteTypeConverter.apply(commonField.type))
        }
    }

    private fun <DataType> assertColumnsWithSameNameAreSame(nameSpace: String,
                                                            tableName: String,
                                                            columns: List<CommonField<DataType>>) {
        columns.stream()
                .collect(Collectors.groupingBy(Function { obj: CommonField<DataType> -> obj.name }))
                .values
                .forEach(Consumer { columnsWithSameName: List<CommonField<DataType>> ->
                    val comparisonColumn = columnsWithSameName[0]
                    columnsWithSameName.forEach(Consumer { column: CommonField<DataType> ->
                        if (column != comparisonColumn) {
                            throw RuntimeException(
                                    String.format(
                                            ...
                                            comparisonColumn.name, nameSpace, tableName, columns))
                        }
                    })
                })
    }
