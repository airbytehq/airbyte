...
object RelationalDbQueryUtils {
    ...
    fun getFullyQualifiedTableNameWithQuoting(nameSpace: String?, tableName: String, quoteString: String?): String {
        return (if (nameSpace == null || nameSpace.isEmpty()) getIdentifierWithQuoting(tableName, quoteString)
        else getIdentifierWithQuoting(nameSpace, quoteString) + "." + getIdentifierWithQuoting(tableName, quoteString))
    }
    ...
    fun <Database : SqlDatabase?> queryTable(database: Database,
                                             sqlQuery: String?,
                                             tableName: String?,
                                             schemaName: String?): AutoCloseableIterator<JsonNode> {
        val airbyteStreamNameNamespacePair = AirbyteStreamUtils.convertFromNameAndNamespace(tableName, schemaName)
        return AutoCloseableIterators.lazyIterator({
            try {
                LOGGER.info("Queueing query: {}", sqlQuery)
                val stream = database!!.unsafeQuery(sqlQuery)
                return@lazyIterator AutoCloseableIterators.fromStream<JsonNode>(stream, airbyteStreamNameNamespacePair)
            } catch (e: Exception) {
    ...
    fun prettyPrintConfiguredAirbyteStreamList(streamList: List<ConfiguredAirbyteStream>): String {
        return streamList.stream().map { s: ConfiguredAirbyteStream -> "%s.%s".formatted(s.stream.namespace, s.stream.name) }.collect(Collectors.joining(", "))
    }
