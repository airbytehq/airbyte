...
object RecordWaitTimeUtil {
    ...
    fun checkFirstRecordWaitTime(config: JsonNode) {
        ...
        if (firstRecordWaitSeconds.isPresent) {
            ...
            require(!(seconds < MIN_FIRST_RECORD_WAIT_TIME.seconds || seconds > MAX_FIRST_RECORD_WAIT_TIME.seconds)) {
                String.format("initial_waiting_seconds must be between %d and %d seconds",
                        MIN_FIRST_RECORD_WAIT_TIME.seconds, MAX_FIRST_RECORD_WAIT_TIME.seconds)
            }
        }
    }
