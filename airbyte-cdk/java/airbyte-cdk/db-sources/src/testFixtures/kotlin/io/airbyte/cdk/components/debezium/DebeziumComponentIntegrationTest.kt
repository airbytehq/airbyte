...
abstract class DebeziumComponentIntegrationTest {
    ...
    fun testCompletesWithEnoughRecords() {
        ...
        val input1 =
            DebeziumComponent.Input(configBuilder().withMaxRecords(maxRecords).build(), state0)
        val output1 = DebeziumEngineManager.debeziumComponent().collect(input1)
        assertProgress(input1, output1)
        ...
        Assertions.assertFalse(
            output2.completionReasons.contains(
                DebeziumComponent.CompletionReason.HAS_COLLECTED_ENOUGH_RECORDS
            )
        )
    ...
    fun testCheckpointing() {
        ...
        Assertions.assertEquals(
            numBatches * numRowsInBatch,
            outputs1.flatMap { it.data }.size.toLong()
        )
    }
    ...
    fun assertCompletionReason(output: DebeziumComponent.Output, expected: CompletionReason) {
        ...
        Assertions.assertTrue(
            output.completionReasons.contains(expected),
    ...
    fun assertDataCountWithinBounds(
        ...
        output: DebeziumComponent.Output,
        upperBoundExclusive: Long
    ) {
        Assertions.assertTrue(
            output.data.count() >= lowerBoundInclusive,
            String.format(
                "expected no less than %d records, obtained %d",
                lowerBoundInclusive,
                output.data.count()
            )
        )
        Assertions.assertTrue(
            output.data.count() < upperBoundExclusive,
            String.format(
                "expected less than %d records, obtained %d",
                upperBoundExclusive,
                output.data.count()
            )
        )
