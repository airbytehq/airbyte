...
abstract class AbstractSourceFillDbWithTestData : AbstractSourceBasePerformanceTest() {
        ...
        database.query<Any?> { ctx: DSLContext? ->
            for (currentSteamNumber in 0 until numberOfStreams) {
                ...
                for (i in 0 until numberOfBatches) {
                    val insertQueryTemplate = prepareInsertQueryTemplate(schemaName, i,
                            numberOfColumns,
    ...
    protected fun prepareCreateTableQuery(dbSchemaName: String?,
                                          numberOfColumns: Int,
                                          currentTableName: String?): String {
        val sj = StringJoiner(",")
    ...
    protected fun prepareInsertQueryTemplate(dbSchemaName: String?,
                                             batchNumber: Int,
                                             numberOfColumns: Int,
                                             recordsNumber: Int): String {
        ...
        return String.format(INSERT_INTO_DB_TABLE_QUERY_TEMPLATE, dbSchemaName, "%s", fieldsNames.toString(),
                insertGroupValuesJoiner.toString())
    }

    companion object {
        private const val CREATE_DB_TABLE_TEMPLATE = "CREATE TABLE %s.%s(id INTEGER PRIMARY KEY, %s)"
        ...
        protected val c: Logger = LoggerFactory.getLogger(AbstractSourceFillDbWithTestData::class.java)
