...
@SuppressFBWarnings(value = ["MS_SHOULD_BE_FINAL"], justification = "The static variables are updated in sub classes for convenience, and cannot be final.")
abstract class JdbcStressTest {
    ...
    open fun setup() {
        ...
        source = getSource()
        streamName = defaultSchemaName.map { `val`: String -> `val` + "." + TABLE_NAME }.orElse(TABLE_NAME)
        config = getConfig()

        val jdbcConfig = source!!.toDatabaseConfig(config)
        val database: JdbcDatabase = DefaultJdbcDatabase(
                create(
        ...
        for (i in 0 until batchCount) {
            ...
            database.execute(CheckedConsumer { connection: Connection -> connection.createStatement().execute(sql) })
        }
    }
    ...
    private fun runTest(configuredCatalog: ConfiguredAirbyteCatalog, testName: String) {
        ...
        val read: Iterator<AirbyteMessage> = source!!.read(config!!, configuredCatalog, Jsons.jsonNode(emptyMap<Any, Any>()))
        val actualCount = MoreStreams.toStream(read)
        ...
        Assertions.assertEquals(expectedRoundedRecordsCount, bitSet!!.cardinality().toLong(), "testing: $testName")
    }
    ...
    private fun assertExpectedMessage(actualMessage: AirbyteMessage) {
        ...
        val expectedRecordNumber: Number =
                if (driverClass.lowercase(Locale.getDefault()).contains("oracle")) BigDecimal(recordNumber)
                else recordNumber

        val expectedMessage = AirbyteMessage().withType(AirbyteMessage.Type.RECORD)
                .withRecord(AirbyteRecordMessage().withStream(streamName)
    ...
    companion object {
        ...
        private val configuredCatalogIncremental: ConfiguredAirbyteCatalog
            get() = ConfiguredAirbyteCatalog()
                    .withStreams(listOf(ConfiguredAirbyteStream().withStream(catalog.streams[0])
                            .withCursorField(listOf(COL_ID))
                            .withSyncMode(SyncMode.INCREMENTAL)
                            .withDestinationSyncMode(DestinationSyncMode.APPEND)))

        private val catalog: AirbyteCatalog
            get() = AirbyteCatalog().withStreams(Lists.newArrayList(CatalogHelpers.createAirbyteStream(
                    streamName,
