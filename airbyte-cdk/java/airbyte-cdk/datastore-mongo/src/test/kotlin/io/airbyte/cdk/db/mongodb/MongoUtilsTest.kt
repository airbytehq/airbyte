...
internal class MongoUtilsTest {
    ...
    fun testTransformToStringIfMarked() {
        val columnNames = listOf("_id", "createdAt", "connectedWallets", "connectedAccounts_aibyte_transform")
        val fieldName = "connectedAccounts"
        val node = Jsons.deserialize(
        ...
        Assertions.assertNotNull(node[fieldName + AIRBYTE_SUFFIX])
        Assertions.assertTrue(node[fieldName + AIRBYTE_SUFFIX].isTextual)
    }
}
