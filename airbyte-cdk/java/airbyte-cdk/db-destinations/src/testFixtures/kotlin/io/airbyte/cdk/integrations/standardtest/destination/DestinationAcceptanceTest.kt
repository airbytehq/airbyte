...
abstract class DestinationAcceptanceTest {
    protected var TEST_SCHEMAS: HashSet<String?>? = null

    private var testEnv: TestDestinationEnv? = null
    ...
    protected var testDataComparator: TestDataComparator = getTestDataComparator()
    ...
    private val imageNameWithoutTag: String
        get() = if (imageName.contains(":")) imageName.split(":".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()[0] else imageName
    ...
    @get:Throws(Exception::class)
    protected abstract val failCheckConfig: JsonNode?
        /**
         * Configuration specific to the integration. Will be passed to integration where appropriate in
    ...
    protected abstract fun retrieveRecords(testEnv: TestDestinationEnv?,
                                           streamName: String?,
                                           namespace: String?,
     ...
     *
     * @param config - integration-specific configuration returned by [.getConfig].
    ...
    protected val normalizationIntegrationType: String?
        get() {
            val metadata = readMetadata()["data"] ?: return null
            val normalizationConfig = metadata["normalizationConfig"] ?: return null
            val normalizationIntegrationType = normalizationConfig["normalizationIntegrationType"] ?: return null
    ...
    /**
     * Same idea as [.retrieveRecords]. Except this
     * method should pull records from the table that contains the normalized records and convert them
     * back into the data as it would appear in an [AirbyteRecordMessage]. Only need to override
     * this method if [.normalizationFromDefinition] returns true.
     *
     * @param testEnv - information about the test environment.
     * @param streamName - name of the stream for which we are retrieving records.
     * @param namespace - the destination namespace records are located in. Null if not applicable.
     ...
     */
    @Throws(Exception::class)
    protected fun retrieveNormalizedRecords(testEnv: TestDestinationEnv?,
                                            streamName: String?,
                                            namespace: String?): List<JsonNode> {
    ...
    fun testCheckConnection() {
        Assertions.assertEquals(StandardCheckConnectionOutput.Status.SUCCEEDED, runCheck(config).status)
    }
    ...
    fun testCheckConnectionInvalidCredentials() {
        Assertions.assertEquals(StandardCheckConnectionOutput.Status.FAILED, runCheck(failCheckConfig).status)
    }
    ...
    fun testSync(messagesFilename: String?, catalogFilename: String?) {
        val catalog = Jsons.deserialize(MoreResources.readResource(catalogFilename),
                AirbyteCatalog::class.java)
        val configuredCatalog = CatalogHelpers.toDefaultConfiguredCatalog(
                catalog)
        val messages: List<io.airbyte.protocol.models.v0.AirbyteMessage> = MoreResources.readResource(messagesFilename).lines()
                .map<io.airbyte.protocol.models.v0.AirbyteMessage>(Function { record: String? -> Jsons.deserialize(record, io.airbyte.protocol.models.v0.AirbyteMessage::class.java) })
                .collect<List<io.airbyte.protocol.models.v0.AirbyteMessage>, Any>(Collectors.toList<io.airbyte.protocol.models.v0.AirbyteMessage>())
    ...
    @ArgumentsSource(DataArgumentsProvider::class)
    @Throws(Exception::class)
    fun testSyncWithLargeRecordBatch(messagesFilename: String?,
        ...
        val largeNumberRecords = Collections
                .nCopies(400, messages)
                .stream()
                .flatMap { obj: List<io.airbyte.protocol.models.v0.AirbyteMessage> -> obj.stream() } // regroup messages per stream
                .sorted(Comparator
                        .comparing { obj: io.airbyte.protocol.models.v0.AirbyteMessage -> obj.type }
                        .thenComparing { message: io.airbyte.protocol.models.v0.AirbyteMessage ->
                            if (message.type == io.airbyte.protocol.models.v0.AirbyteMessage.Type.RECORD) message.record.stream
    ...
    fun testSecondSync() {
        ...
        val catalog =
                Jsons.deserialize<AirbyteCatalog>(
                        MoreResources.readResource(DataArgumentsProvider.Companion.EXCHANGE_RATE_CONFIG.getCatalogFileVersion(protocolVersion)),
                        AirbyteCatalog::class.java)
        val configuredCatalog = CatalogHelpers.toDefaultConfiguredCatalog(
                catalog)

        val firstSyncMessages: List<io.airbyte.protocol.models.v0.AirbyteMessage> = MoreResources.readResource(
                DataArgumentsProvider.Companion.EXCHANGE_RATE_CONFIG.getMessageFileVersion(protocolVersion)).lines()
                .map<io.airbyte.protocol.models.v0.AirbyteMessage>(Function<String, io.airbyte.protocol.models.v0.AirbyteMessage> { record: String? -> Jsons.deserialize<io.airbyte.protocol.models.v0.AirbyteMessage>(record, io.airbyte.protocol.models.v0.AirbyteMessage::class.java) })
                .collect<List<io.airbyte.protocol.models.v0.AirbyteMessage>, Any>(Collectors.toList<io.airbyte.protocol.models.v0.AirbyteMessage>())
        val config = config
        ...
        val configuredDummyCatalog = CatalogHelpers.toDefaultConfiguredCatalog(
                dummyCatalog)
        // update messages to set new dummy stream name
        firstSyncMessages.stream().filter { message: io.airbyte.protocol.models.v0.AirbyteMessage -> message.record != null }
                .forEach { message: io.airbyte.protocol.models.v0.AirbyteMessage -> message.record.stream = DUMMY_CATALOG_NAME }
        // sync dummy data
        runSyncAndVerifyStateOutput(config, firstSyncMessages, configuredDummyCatalog, false)

        // Run second sync
        val secondSyncMessages: List<io.airbyte.protocol.models.v0.AirbyteMessage> = Lists.newArrayList(
                io.airbyte.protocol.models.v0.AirbyteMessage()
                        ...
                        .withRecord(AirbyteRecordMessage()
                                ...
                                .withData(Jsons.jsonNode(ImmutableMap.builder<Any, Any>()
                                        .put("id", 1)
                                        .put("currency", "USD")
                                        .put("date", "2020-03-31T00:00:00Z") // TODO(sherifnada) hack: write decimals with sigfigs because Snowflake stores 10.1 as "10" which
                ...
                io.airbyte.protocol.models.v0.AirbyteMessage()
                        ...
                        .withState(AirbyteStateMessage().withData(
                ...
                io.airbyte.protocol.models.v0.AirbyteMessage()
                        ...
                        .withRecord(AirbyteRecordMessage()
                                ...
                                .withData(Jsons.jsonNode(ImmutableMap.builder<Any, Any>()
                                        .put("id", 1)
                                        .put("currency", "USD\u2028")
                                        .put("date", "2020-03-\n31T00:00:00Z\r") // TODO(sherifnada) hack: write decimals with sigfigs because Snowflake stores 10.1 as "10" which
                                        // fails destination tests
                                        .put("HKD", 10.1)
                                        .put("NZD", 700.1)
                                        .build()))),
    ...
    fun testIncrementalSync() {
        ...
        val configuredCatalog = CatalogHelpers.toDefaultConfiguredCatalog(
                catalog)
        configuredCatalog.streams.forEach(Consumer { s: ConfiguredAirbyteStream ->
        ...
        val expectedMessagesAfterSecondSync: MutableList<io.airbyte.protocol.models.v0.AirbyteMessage> = ArrayList()
    ...
    fun testIncrementalSyncWithNormalizationDropOneColumn() {
        ...
        val catalog = Jsons.deserialize<AirbyteCatalog>(
                MoreResources.readResource(DataArgumentsProvider.Companion.EXCHANGE_RATE_CONFIG.getCatalogFileVersion(ProtocolVersion.V0)),
                AirbyteCatalog::class.java)
        ...
        configuredCatalog.streams.forEach(Consumer { s: ConfiguredAirbyteStream ->
            ...
            s.withPrimaryKey(
                    java.util.List.of(listOf("id"), listOf("currency"), listOf("date"), listOf("NZD"), listOf("USD")))
        })

        var messages: MutableList<io.airbyte.protocol.models.v0.AirbyteMessage> = MoreResources.readResource(DataArgumentsProvider.Companion.EXCHANGE_RATE_CONFIG.getMessageFileVersion(ProtocolVersion.V0))
                .lines()
                .map<io.airbyte.protocol.models.v0.AirbyteMessage>(Function<String, io.airbyte.protocol.models.v0.AirbyteMessage> { record: String? -> Jsons.deserialize<io.airbyte.protocol.models.v0.AirbyteMessage>(record, io.airbyte.protocol.models.v0.AirbyteMessage::class.java) })
                .collect<List<io.airbyte.protocol.models.v0.AirbyteMessage>, Any>(Collectors.toList<io.airbyte.protocol.models.v0.AirbyteMessage>())
        ...
        val defaultSchema = getDefaultSchema(config)
        var actualMessages = retrieveNormalizedRecords(catalog,
                defaultSchema)
        ...
        messages = MoreResources.readResource(DataArgumentsProvider.Companion.EXCHANGE_RATE_CONFIG.getMessageFileVersion(ProtocolVersion.V0)).lines()
                .map<io.airbyte.protocol.models.v0.AirbyteMessage>(Function<String, io.airbyte.protocol.models.v0.AirbyteMessage> { record: String? -> Jsons.deserialize<io.airbyte.protocol.models.v0.AirbyteMessage>(record, io.airbyte.protocol.models.v0.AirbyteMessage::class.java) })
                .collect<List<io.airbyte.protocol.models.v0.AirbyteMessage>, Any>(Collectors.toList<io.airbyte.protocol.models.v0.AirbyteMessage>())
        ...
        val expectedMessages = messages.stream().map { message: io.airbyte.protocol.models.v0.AirbyteMessage ->
    ...
    fun testSyncWithNormalization(messagesFilename: String?, catalogFilename: String?) {
        ...
        val defaultSchema = getDefaultSchema(config)
        val actualMessages = retrieveNormalizedRecords(catalog,
                defaultSchema)
        assertSameMessages(messages, actualMessages, true)
    }
     ...
     * do so, this is not necessarily true. This explains [.implementsAppendDedup].
     */
    @Test
    @Throws(Exception::class)
    fun testIncrementalDedupeSync() {
        ...
        val latestMessagesOnly = expectedMessagesAfterSecondSync
                .stream()
                .filter { message: io.airbyte.protocol.models.v0.AirbyteMessage -> message.type == io.airbyte.protocol.models.v0.AirbyteMessage.Type.RECORD && message.record != null }
                .collect(Collectors.toMap(
                        Function { message: io.airbyte.protocol.models.v0.AirbyteMessage ->
                            message.record.data["id"].asText() +
                                    message.record.data["currency"].asText() +
                                    message.record.data["date"].asText() +
                                    message.record.data["NZD"].asText()
                        },
                        Function { message: io.airbyte.protocol.models.v0.AirbyteMessage -> message },  // keep only latest emitted record message per primary key/cursor
                        BinaryOperator { a: io.airbyte.protocol.models.v0.AirbyteMessage, b: io.airbyte.protocol.models.v0.AirbyteMessage -> if (a.record.emittedAt > b.record.emittedAt) a else b }))
        // Filter expectedMessagesAfterSecondSync and keep latest messages only (keep same message order)
        val expectedMessages = expectedMessagesAfterSecondSync
                .stream()
                .filter { message: io.airbyte.protocol.models.v0.AirbyteMessage -> message.type == io.airbyte.protocol.models.v0.AirbyteMessage.Type.RECORD && message.record != null }
                .filter { message: io.airbyte.protocol.models.v0.AirbyteMessage ->
                    val key = message.record.data["id"].asText() +
                            message.record.data["currency"].asText() +
                            message.record.data["date"].asText() +
                            message.record.data["NZD"].asText()
                    (message.record.emittedAt
                            == latestMessagesOnly[key]!!.record.emittedAt)
                }.collect(Collectors.toList())
    ...
    fun testCustomDbtTransformations() {
        ...
        val runner = DbtTransformationRunner(processFactory,
                DefaultNormalizationRunner(
                        processFactory,
                        normalizationImageName,
                        normalizationIntegrationType))
        ...
        val dbtConfig = OperatorDbt() // Forked from https://github.com/dbt-labs/jaffle_shop because they made a change that would have
    ...
    fun testCustomDbtTransformationsFailure() {
        ...
        dbtConfig.withDbtArguments("test")
        Assertions.assertFalse(runner.transform(JOB_ID, JOB_ATTEMPT, transformationRoot, config, null, dbtConfig),
    ...
    fun testSyncUsesAirbyteStreamNamespaceIfNotNull() {
        ...
        val messages: List<io.airbyte.protocol.models.v0.AirbyteMessage> = MoreResources.readResource(
                DataArgumentsProvider.Companion.EXCHANGE_RATE_CONFIG.getMessageFileVersion(protocolVersion)).lines()
                .map<io.airbyte.protocol.models.v0.AirbyteMessage>(Function<String, io.airbyte.protocol.models.v0.AirbyteMessage> { record: String? -> Jsons.deserialize<io.airbyte.protocol.models.v0.AirbyteMessage>(record, io.airbyte.protocol.models.v0.AirbyteMessage::class.java) })
                .collect<List<io.airbyte.protocol.models.v0.AirbyteMessage>, Any>(Collectors.toList<io.airbyte.protocol.models.v0.AirbyteMessage>())
        val messagesWithNewNamespace = getRecordMessagesWithNewNamespace(
                messages, namespace)
    ...
    fun testSyncWriteSameTableNameDifferentNamespace() {
        ...
        for (stream in catalog.streams) {
            val clonedStream = mapper.readValue(mapper.writeValueAsString(stream),
        ...
        val messageFile: String = DataArgumentsProvider.Companion.EXCHANGE_RATE_CONFIG.getMessageFileVersion(protocolVersion)
        val ns1Messages: List<io.airbyte.protocol.models.v0.AirbyteMessage> = MoreResources.readResource(messageFile).lines()
                .map<io.airbyte.protocol.models.v0.AirbyteMessage>(Function { record: String? -> Jsons.deserialize(record, io.airbyte.protocol.models.v0.AirbyteMessage::class.java) })
                .collect<List<io.airbyte.protocol.models.v0.AirbyteMessage>, Any>(Collectors.toList<io.airbyte.protocol.models.v0.AirbyteMessage>())
        val ns1MessagesAtNamespace1 = getRecordMessagesWithNewNamespace(ns1Messages, namespace1)
        val ns2Messages: List<io.airbyte.protocol.models.v0.AirbyteMessage> = MoreResources.readResource(messageFile).lines()
                .map<io.airbyte.protocol.models.v0.AirbyteMessage>(Function { record: String? -> Jsons.deserialize(record, io.airbyte.protocol.models.v0.AirbyteMessage::class.java) })
                .collect<List<io.airbyte.protocol.models.v0.AirbyteMessage>, Any>(Collectors.toList<io.airbyte.protocol.models.v0.AirbyteMessage>())
    ...
    @Throws(Exception::class)
    fun testNamespaces(testCaseId: String?,
                       namespaceInCatalog: String?,
                       namespaceInDst: String?) {
        val nameTransformer = nameTransformer
        nameTransformer.ifPresent { namingConventionTransformer: NamingConventionTransformer ->
            assertNamespaceNormalization(testCaseId,
                    namespaceInDst,
        ...
        val catalog = Jsons.deserialize<AirbyteCatalog>(
                MoreResources.readResource(DataArgumentsProvider.Companion.NAMESPACE_CONFIG.getCatalogFileVersion(protocolVersion)),
                AirbyteCatalog::class.java)
        catalog.streams.forEach(Consumer { stream: AirbyteStream -> stream.namespace = namespaceInCatalog })
        val configuredCatalog = CatalogHelpers.toDefaultConfiguredCatalog(
                catalog)

        val messages: List<io.airbyte.protocol.models.v0.AirbyteMessage> = MoreResources.readResource(
                DataArgumentsProvider.Companion.NAMESPACE_CONFIG.getMessageFileVersion(protocolVersion)).lines()
                .map<io.airbyte.protocol.models.v0.AirbyteMessage>(Function<String, io.airbyte.protocol.models.v0.AirbyteMessage> { record: String? -> Jsons.deserialize<io.airbyte.protocol.models.v0.AirbyteMessage>(record, io.airbyte.protocol.models.v0.AirbyteMessage::class.java) })
                .collect<List<io.airbyte.protocol.models.v0.AirbyteMessage>, Any>(Collectors.toList<io.airbyte.protocol.models.v0.AirbyteMessage>())
        val messagesWithNewNamespace = getRecordMessagesWithNewNamespace(messages, namespaceInCatalog)
    ...
    fun testSyncNotFailsWithNewFields() {
        ...
        val secondSyncMessagesWithNewFields: MutableList<io.airbyte.protocol.models.v0.AirbyteMessage> = Lists.newArrayList(
                io.airbyte.protocol.models.v0.AirbyteMessage()
                        .withType(io.airbyte.protocol.models.v0.AirbyteMessage.Type.RECORD)
                        .withRecord(AirbyteRecordMessage()
        ...
        val destinationOutput = retrieveRecords(testEnv, stream.name,
                getDefaultSchema(config), stream.jsonSchema)
        // Remove state message
        secondSyncMessagesWithNewFields.removeIf { airbyteMessage: io.airbyte.protocol.models.v0.AirbyteMessage -> airbyteMessage.type == io.airbyte.protocol.models.v0.AirbyteMessage.Type.STATE }
    ...
    protected open val nameTransformer: Optional<NamingConventionTransformer>
        /**
     ...
     */
    protected fun assertNamespaceNormalization(testCaseId: String?,
                                               expectedNormalizedNamespace: String?,
                                               actualNormalizedNamespace: String?) {
        Assertions.assertEquals(expectedNormalizedNamespace, actualNormalizedNamespace,
                String.format(
    ...
    private fun runSpec(): ConnectorSpecification {
        return convertProtocolObject(
                DefaultGetSpecTestHarness(
                        AirbyteIntegrationLauncher(JOB_ID, JOB_ATTEMPT, imageName, processFactory, null, null, false, EnvVariableFeatureFlags()))
    ...
    protected fun runCheckWithCatchedException(
            config: JsonNode?): StandardCheckConnectionOutput.Status {
        try {
            val standardCheckConnectionOutput = DefaultCheckConnectionTestHarness(
                    AirbyteIntegrationLauncher(JOB_ID, JOB_ATTEMPT, imageName, processFactory, null, null, false, EnvVariableFeatureFlags()),
                    mConnectorConfigUpdater)
                    .run(StandardCheckConnectionInput().withConnectionConfiguration(config), jobRoot)
                    .checkConnection
    ...
    protected val destination: AirbyteDestination
        get() = DefaultAirbyteDestination(
                AirbyteIntegrationLauncher(JOB_ID, JOB_ATTEMPT, imageName, processFactory, null, null, false, EnvVariableFeatureFlags()))

    @Throws(Exception::class)
    protected fun runSyncAndVerifyStateOutput(config: JsonNode,
        ...
        val destinationOutput = runSync(config, messages, catalog,
                runNormalization)

        val expectedStateMessage = reversed(messages)
                .stream()
                .filter(Predicate { m: io.airbyte.protocol.models.v0.AirbyteMessage -> m.type == io.airbyte.protocol.models.v0.AirbyteMessage.Type.STATE })
                .findFirst()
                .orElseThrow {
        ...
        val actualStateMessage = destinationOutput
                .stream()
                .filter { m: io.airbyte.protocol.models.v0.AirbyteMessage? -> m!!.type == io.airbyte.protocol.models.v0.AirbyteMessage.Type.STATE }
                .findFirst()
    ...
    private fun runSync(
            ...
            messages: List<io.airbyte.protocol.models.v0.AirbyteMessage>,
            catalog: io.airbyte.protocol.models.v0.ConfiguredAirbyteCatalog,
            runNormalization: Boolean): List<io.airbyte.protocol.models.v0.AirbyteMessage?> {
        val destinationConfig = WorkerDestinationConfig()
        ...
        messages.forEach(
                Consumer { message: io.airbyte.protocol.models.v0.AirbyteMessage -> Exceptions.toRuntime { destination.accept(convertProtocolObject(message, AirbyteMessage::class.java)) } })
        destination.notifyEndOfInput()

        val destinationOutput: MutableList<io.airbyte.protocol.models.v0.AirbyteMessage?> = ArrayList()
        while (!destination.isFinished) {
            destination.attemptRead().ifPresent { m: AirbyteMessage -> destinationOutput.add(convertProtocolObject(m, io.airbyte.protocol.models.v0.AirbyteMessage::class.java)) }
        }
        ...
        val runner: NormalizationRunner = DefaultNormalizationRunner(
                processFactory,
                normalizationImageName,
                normalizationIntegrationType)
        runner.start()
        ...
        if (!runner.normalize(JOB_ID, JOB_ATTEMPT, normalizationRoot,
                        ...
                        destinationConfig.catalog, null)) {
    ...
    @Throws(Exception::class)
    protected fun retrieveRawRecordsAndAssertSameMessages(catalog: AirbyteCatalog,
                                                          messages: List<io.airbyte.protocol.models.v0.AirbyteMessage>,
        ...
        for (stream in catalog.streams) {
            ...
            val msgList = retrieveRecords(testEnv, streamName, schema,
                    stream.jsonSchema)
                    .stream()
                    .map { data: JsonNode? ->
                        AirbyteRecordMessage().withStream(streamName).withNamespace(schema)
                                .withData(data)
                    }
                    .toList()
    ...
    // ignores emitted at.
    protected fun assertSameMessages(expected: List<io.airbyte.protocol.models.v0.AirbyteMessage>,
                                     actual: List<AirbyteRecordMessage>,
                                     pruneAirbyteInternalFields: Boolean) {
        val expectedProcessed = expected.stream()
                .filter { message: io.airbyte.protocol.models.v0.AirbyteMessage -> message.type == io.airbyte.protocol.models.v0.AirbyteMessage.Type.RECORD }
                ...
                .map { recordMessage: AirbyteRecordMessage -> if (pruneAirbyteInternalFields) safePrune(recordMessage) else recordMessage }
                .map { obj: AirbyteRecordMessage -> obj.data }
                .collect(Collectors.toList())

        val actualProcessed = actual.stream()
                .map { recordMessage: AirbyteRecordMessage -> if (pruneAirbyteInternalFields) safePrune(recordMessage) else recordMessage }
                .map { obj: AirbyteRecordMessage -> obj.data }
                .collect(Collectors.toList())
    ...
    @Throws(Exception::class)
    protected fun retrieveNormalizedRecords(catalog: AirbyteCatalog,
                                            defaultSchema: String?): List<AirbyteRecordMessage> {
        val actualMessages: MutableList<AirbyteRecordMessage> = ArrayList()

        for (stream in catalog.streams) {
            val streamName = stream.name

            val msgList = retrieveNormalizedRecords(testEnv, streamName,
                    defaultSchema)
                    .stream()
                    .map { data: JsonNode? -> AirbyteRecordMessage().withStream(streamName).withData(data) }.toList()
            actualMessages.addAll(msgList)
        }
    ...
    fun testStressPerformance() {
        ...
        val configuredTestCatalog = CatalogHelpers
                .toDefaultConfiguredCatalog(testCatalog)

        val config = config
        val destinationConfig = WorkerDestinationConfig()
                .withConnectionId(UUID.randomUUID())
                .withCatalog(convertProtocolObject(configuredTestCatalog, ConfiguredAirbyteCatalog::class.java))
                .withDestinationConnectionConfiguration(config)
        val destination = destination
        ...
        for (streamCounter in 0 until streamsSize) {
            ...
            for (msgCounter in 0 until messagesNumber) {
                val msg = io.airbyte.protocol.models.v0.AirbyteMessage()
                        .withType(io.airbyte.protocol.models.v0.AirbyteMessage.Type.RECORD)
                        .withRecord(AirbyteRecordMessage().withStream(USERS_STREAM_NAME + streamCounter)
                                .withData(
                                        Jsons.jsonNode(
                                                ImmutableMap.builder<Any, Any>().put(NAME, LOREM_IPSUM)
                                                        .put(ID, streamCounter.toString() + "_" + msgCounter)
                                                        .build()))
                                .withEmittedAt(Instant.now().toEpochMilli()))
                try {
                    destination.accept(convertProtocolObject(msg, AirbyteMessage::class.java))
                } catch (e: Exception) {
            ...
            val msgState = io.airbyte.protocol.models.v0.AirbyteMessage()
                    .withType(io.airbyte.protocol.models.v0.AirbyteMessage.Type.STATE)
                    .withState(AirbyteStateMessage()
                            .withData(
                                    Jsons.jsonNode(ImmutableMap.builder<Any, Any>().put("start_date", "2020-09-02").build())))
            try {
                destination.accept(convertProtocolObject(msgState, AirbyteMessage::class.java))
            } catch (e: Exception) {
    ...
    protected open fun getTestDataComparator(): TestDataComparator {
        return BasicTestDataComparator { identifier: String? -> this.resolveIdentifier(identifier) }
    }
    ...
    open val protocolVersion: ProtocolVersion
        /**
         * The method should be overridden if destination connector support newer protocol version otherwise
         * [io.airbyte.cdk.integrations.standardtest.destination.ProtocolVersion.V0] is used
         *
         *
         ...
         * @return
         */
        get() = ProtocolVersion.V0

    private fun checkTestCompatibility(
            testCompatibility: DataTypeTestArgumentProvider.TestCompatibility): Boolean {
    ...
    @Throws(Exception::class)
    fun testDataTypeTestWithNormalization(messagesFilename: String?,
                                          catalogFilename: String?,
    ...
    fun testSyncNumberNanDataType() {
        ...
        if (protocolVersion == ProtocolVersion.V0 || !numericTypesSupport.isSupportNumberNan()) {
            return
        }
        val catalog = readCatalogFromFile(ArgumentProviderUtil.prefixFileNameByVersion(DataTypeTestArgumentProvider.Companion.NUMBER_TYPE_CATALOG, protocolVersion))
        val configuredCatalog = CatalogHelpers.toDefaultConfiguredCatalog(catalog)
        val messages = readMessagesFromFile(ArgumentProviderUtil.prefixFileNameByVersion(DataTypeTestArgumentProvider.Companion.NAN_TYPE_MESSAGE, protocolVersion))
        val config = config
        val defaultSchema = getDefaultSchema(config)
    ...
    fun testSyncIntegerNanDataType() {
        // NaN/Infinity protocol supports started from V1 version or higher
        val numericTypesSupport = specialNumericTypesSupportTest
        if (protocolVersion == ProtocolVersion.V0 || !numericTypesSupport.isSupportIntegerNan()) {
            return
        }
        val catalog = readCatalogFromFile(ArgumentProviderUtil.prefixFileNameByVersion(DataTypeTestArgumentProvider.Companion.INTEGER_TYPE_CATALOG, protocolVersion))
        val configuredCatalog = CatalogHelpers.toDefaultConfiguredCatalog(catalog)
        val messages = readMessagesFromFile(ArgumentProviderUtil.prefixFileNameByVersion(DataTypeTestArgumentProvider.Companion.NAN_TYPE_MESSAGE, protocolVersion))
        val config = config
        val defaultSchema = getDefaultSchema(config)
    ...
    fun testSyncNumberInfinityDataType() {
        // NaN/Infinity protocol supports started from V1 version or higher
        val numericTypesSupport = specialNumericTypesSupportTest
        if (protocolVersion == ProtocolVersion.V0 || !numericTypesSupport.isSupportNumberInfinity()) {
            return
        }
        val catalog = readCatalogFromFile(ArgumentProviderUtil.prefixFileNameByVersion(DataTypeTestArgumentProvider.Companion.NUMBER_TYPE_CATALOG, protocolVersion))
        val configuredCatalog = CatalogHelpers.toDefaultConfiguredCatalog(catalog)
        val messages = readMessagesFromFile(ArgumentProviderUtil.prefixFileNameByVersion(DataTypeTestArgumentProvider.Companion.INFINITY_TYPE_MESSAGE, protocolVersion))
        val config = config
        val defaultSchema = getDefaultSchema(config)
    ...
    fun testSyncIntegerInfinityDataType() {
        // NaN/Infinity protocol supports started from V1 version or higher
        val numericTypesSupport = specialNumericTypesSupportTest
        if (protocolVersion == ProtocolVersion.V0 || !numericTypesSupport.isSupportIntegerInfinity()) {
            return
        }
        val catalog = readCatalogFromFile(ArgumentProviderUtil.prefixFileNameByVersion(DataTypeTestArgumentProvider.Companion.INTEGER_TYPE_CATALOG, protocolVersion))
        val configuredCatalog = CatalogHelpers.toDefaultConfiguredCatalog(catalog)
    ...
    @Throws(Exception::class)
    private fun runAndCheck(catalog: AirbyteCatalog, configuredCatalog: io.airbyte.protocol.models.v0.ConfiguredAirbyteCatalog, messages: List<io.airbyte.protocol.models.v0.AirbyteMessage>) {
        if (normalizationFromDefinition()) {
    ...
    @Throws(Exception::class)
    private fun runAndCheckWithNormalization(messages: List<io.airbyte.protocol.models.v0.AirbyteMessage>,
    ...
    @Throws(Exception::class)
    private fun runAndCheckWithoutNormalization(messages: List<io.airbyte.protocol.models.v0.AirbyteMessage>,
                                                configuredCatalog: io.airbyte.protocol.models.v0.ConfiguredAirbyteCatalog,
                                                catalog: AirbyteCatalog) {
        val config = config
        runSyncAndVerifyStateOutput(config, messages, configuredCatalog, false)
    ...
    /**
     * Can be used in overridden [ getSpecialNumericTypesSupportTest()][.getSpecialNumericTypesSupportTest] method to specify if connector supports Integer/Number NaN
    ...
    class SpecialNumericTypes {
        @Builder.Default
        var supportIntegerNan: Boolean = false

        @Builder.Default
        var supportNumberNan: Boolean = false

        @Builder.Default
        var supportIntegerInfinity: Boolean = false

        @Builder.Default
        var supportNumberInfinity: Boolean = false
    }
        ...
        override fun provideArguments(context: ExtensionContext): Stream<out Arguments> {
            ...
            return MoreIterators.toList(testCases.elements()).stream()
                    ...
                    .map { testCase: JsonNode ->
                        val namespaceInCatalog = TestingNamespaces.generate(testCase["namespace"].asText())
                        val namespaceInDst = TestingNamespaces.generateFromOriginal(namespaceInCatalog, testCase["namespace"].asText(), testCase["normalized"].asText())
                        Arguments.of(
                                testCase["id"].asText(),  // Add uniqueness to namespace to avoid collisions between tests.
                                namespaceInCatalog,
                                namespaceInDst)
                    }
        }
    ...
    companion object {
        ...
        /**
         * Same as [.pruneMutate], except does a defensive copy and returns a new json node
        ...
        private fun pruneMutate(json: JsonNode) {
            for (key in Jsons.keys(json)) {
                ...
                val airbyteInternalFields = Sets.newHashSet(
                        "emitted_at",
                        "ab_id",
                        "normalized_at",
                        "EMITTED_AT",
                ...
                if (airbyteInternalFields.stream()
                                .anyMatch { internalField: String -> key.lowercase(Locale.getDefault()).contains(internalField.lowercase(Locale.getDefault())) }
                        || json[key].isNull) {
        ...
        protected val specialNumericTypesSupportTest: SpecialNumericTypes
             ...
             */
            get() = SpecialNumericTypes.builder().build()
        ...
        @Throws(IOException::class)
        private fun readMessagesFromFile(messagesFilename: String?): List<io.airbyte.protocol.models.v0.AirbyteMessage> {
        ...
        private fun getRecordMessagesWithNewNamespace(
                ...
                namespace: String?): List<io.airbyte.protocol.models.v0.AirbyteMessage> {
            airbyteMessages.forEach(Consumer { message: io.airbyte.protocol.models.v0.AirbyteMessage ->
                if (message.record != null) {
