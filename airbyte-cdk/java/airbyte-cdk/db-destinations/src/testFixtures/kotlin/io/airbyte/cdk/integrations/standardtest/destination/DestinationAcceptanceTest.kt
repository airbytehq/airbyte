...
abstract class DestinationAcceptanceTest(
     ...
     */
    @Throws(Exception::class)
    protected abstract fun setup(testEnv: TestDestinationEnv, testSchemas: HashSet<String>)
    ...
    fun testSecondSync() {
        ...
        val catalog =
            Jsons.deserialize(
                MoreResources.readResource(
                    DataArgumentsProvider.EXCHANGE_RATE_CONFIG.catalogFile
                ),
                AirbyteCatalog::class.java
            )
        val configuredCatalog = CatalogHelpers.toDefaultConfiguredCatalog(catalog)
        ...
        val firstSyncMessages: List<AirbyteMessage> =
            MoreResources.readResource(
                    DataArgumentsProvider.EXCHANGE_RATE_CONFIG.messageFile
                )
                .trim()
                .lines()
    ...
    open fun testIncrementalDedupeSync() {
        ...
        configuredCatalog.streams.forEach { s ->
            ...
            // use composite primary key of various types (string, float)
            s.withPrimaryKey(
                listOf(listOf("id"), listOf("currency"), listOf("date"), listOf("NZD"))
            )
        }
        ...
        val latestMessagesOnly =
            expectedMessagesAfterSecondSync
                ...
                .mapValues { (_, v) -> v.maxBy { it.record.emittedAt } }
    ...
    fun testSyncUsesAirbyteStreamNamespaceIfNotNull() {
        ...
        val messages =
            MoreResources.readResource(DataArgumentsProvider.EXCHANGE_RATE_CONFIG.messageFile)
                .trim()
                .lines()
                .map { Jsons.deserialize(it, AirbyteMessage::class.java) }
    ...
    fun testSyncWriteSameTableNameDifferentNamespace() {
        ...
        val configuredCatalog = CatalogHelpers.toDefaultConfiguredCatalog(catalog)
        val messageFile: String = DataArgumentsProvider.EXCHANGE_RATE_CONFIG.messageFile
        val ns1Messages =
    ...
    fun testNamespaces(testCaseId: String?, namespaceInCatalog: String, namespaceInDst: String?) {
        ...
        val catalog =
            Jsons.deserialize<AirbyteCatalog>(
                MoreResources.readResource(
                    DataArgumentsProvider.NAMESPACE_CONFIG.catalogFile
                ),
                AirbyteCatalog::class.java
            )
        ...
        val messages =
            MoreResources.readResource(DataArgumentsProvider.NAMESPACE_CONFIG.messageFile)
                .lines()
                .map { Jsons.deserialize(it, AirbyteMessage::class.java) }
    ...
    fun testAirbyteFields() {
        val configuredCatalog =
            Jsons.deserialize(
                MoreResources.readResource("users_with_generation_id_configured_catalog.json"),
    ...
    fun testAirbyteTimeTypes() {
        val configuredCatalog =
            Jsons.deserialize(
                MoreResources.readResource("every_time_type_configured_catalog.json"),
                ConfiguredAirbyteCatalog::class.java
            )
        val config = getConfig()
        val messages =
            MoreResources.readResource("every_time_type_messages.txt").trim().lines().map {
                Jsons.deserialize(it, AirbyteMessage::class.java)
            }
    ...
    fun testProblematicTypes() {
        ...
        val configuredCatalog =
            Jsons.deserialize(
                MoreResources.readResource("problematic_types_configured_catalog.json"),
                ConfiguredAirbyteCatalog::class.java
            )
        val config = getConfig()
        val messagesIn =
            MoreResources.readResource("problematic_types_messages_in.txt").trim().lines().map {
                Jsons.deserialize(it, AirbyteMessage::class.java)
            }
        ...
        val destinationSchemaStr =
            if (!expectUnionsPromotedToDisjointRecords) {
                MoreResources.readResource("problematic_types_coerced_schemaless_schema.json")
            } else {
                MoreResources.readResource("problematic_types_disjoint_union_schema.json")
            }
        ...
        val expectedMessages =
            if (!expectUnionsPromotedToDisjointRecords) {
                    MoreResources.readResource(
                        "problematic_types_coerced_schemaless_messages_out.txt"
                    )
                } else { // expectSchemalessObjectsCoercedToStrings
                    MoreResources.readResource("problematic_types_disjoint_union_messages_out.txt")
                }
                .trim()
                .lines()
    ...
    protected fun assertSameMessages(
        ...
        val expectedProcessed =
            expected
                .filter { message: AirbyteMessage -> message.type == Type.RECORD }
    ...
    fun testStressPerformance() {
        ...
        val usersStreamName = "users" // stream's name prefix. Will get "user0", "user1", etc.
        val id = "id"
        val name = "name"
            ...
            for (msgCounter in 0 until messagesNumber) {
                val msg =
                    AirbyteMessage()
                        ...
                        .withRecord(
                            AirbyteRecordMessage()
                                .withStream(usersStreamName + streamCounter)
                                .withData(
                                    Jsons.jsonNode(
                                        ImmutableMap.builder<Any, Any>()
