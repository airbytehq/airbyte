...
abstract class JdbcSqlGeneratorIntegrationTest<DestinationState : MinimumDestinationState?>
    ...
    private fun insertRecords(tableName: Name, columnNames: List<String>, records: List<JsonNode>?, vararg columnsToParseJson: String) {
        var insert = dslContext.insertInto(
                DSL.table(tableName),
                columnNames.stream().map { columnName: String? -> DSL.field(DSL.quotedName(columnName)) }.toList())
            ...
            insert = insert.values(
                    columnNames.stream()
                            .map { fieldName: String ->
                                ...
                                val columnAsString = if (column == null) {
                                    null
    ...
    override fun createRawTable(streamId: StreamId?) {
        database.execute(dslContext.createTable(DSL.name(streamId!!.rawNamespace, streamId.rawName))
    ...
    override fun createV1RawTable(v1RawTable: StreamId?) {
        database.execute(dslContext.createTable(DSL.name(v1RawTable!!.rawNamespace, v1RawTable.rawName))
    ...
    @Throws(Exception::class)
    override fun insertFinalTableRecords(includeCdcDeletedAt: Boolean,
                                         streamId: StreamId?,
                                         suffix: String?,
    ...
    override fun dumpRawTableRecords(streamId: StreamId?): List<JsonNode> {
        return database.queryJsons(dslContext.selectFrom(DSL.name(streamId!!.rawNamespace, streamId.rawName)).getSQL(ParamType.INLINED))
    }

    @Throws(Exception::class)
    override fun dumpFinalTableRecords(streamId: StreamId?, suffix: String?): List<JsonNode> {
        return database
                .queryJsons(dslContext.selectFrom(DSL.name(streamId!!.finalNamespace, streamId.finalName + suffix)).getSQL(ParamType.INLINED))
    }
