...
abstract class JdbcTypingDedupingTest : BaseTypingDedupingTest() {
    ...
    /**
     * Subclasses using a config where the default schema is not in the `schema` key should
     * override this method and [.setDefaultSchema].
     */
    ...
    /**
     * Subclasses using a config where the default schema is not in the `schema` key should
     * override this method and [.getDefaultSchema].
     */
    ...
    override fun teardownStreamAndNamespace(streamNamespace: String?, streamName: String?) {
        ...
        database!!.execute(DSL.dropTableIfExists(DSL.name(rawSchema, concatenateRawTableName(streamNamespace, streamName!!))).sql)
