...
interface BaseRecordDestinationAcceptanceTest: BaseDestinationAcceptanceTest {
    var testSchemas: HashSet<String>
    val mConnectorConfigUpdater: ConnectorConfigUpdater
    ...
    var _testDataComparator: TestDataComparator
    ...
    fun getTestDataComparator(): TestDataComparator {
        return BasicTestDataComparator { @Suppress("deprecation") this.resolveIdentifier(it) }
    }

    fun inDestinationNormalizationFlags(shouldNormalize: Boolean): Map<String, String> {
     ...
     */
    @Throws(Exception::class) fun getFailCheckConfig(): JsonNode?
     ...
     * this. Destinations are free to appropriately this. The return value is used to
     * assert correctness.
     *
    ...
    /**
     * to return true if a destination implements namespaces and should be tested as such.
     */
    ...
    open fun normalizationFromDefinition(): Boolean {
    ...
    open fun dbtFromDefinition(): Boolean {
        val metadata = readMetadata()["data"] ?: return false
        val supportsDbt = metadata["supportsDbt"]
        return supportsDbt != null && supportsDbt.asBoolean(false)
    }

    open val destinationDefinitionKey: String
        get() = imageNameWithoutTag

    open fun getNormalizationIntegrationType(): String? {
        val metadata = readMetadata()["data"] ?: return null
        val normalizationConfig = metadata["normalizationConfig"] ?: return null
     ...
     * [AirbyteRecordMessage]. Only need to this method if [.normalizationFromDefinition]
     * returns true.
     *
     * @param testEnv
     * - information about the test environment.
     ...
     */
    @Throws(Exception::class)
    open fun retrieveNormalizedRecords(
        testEnv: AbstractDestinationAcceptanceTest.TestDestinationEnv?,
        streamName: String?,
        namespace: String?
    ): List<JsonNode> {
    ...
    open fun resolveIdentifier(identifier: String?): List<String?> {
    ...
    fun testSyncWithLargeRecordBatch(messagesFilename: String, catalogFilename: String) {
        ...
        val lotsOfRecordAndStateBlocks =
            Collections.nCopies(
                400,
                messages.filter { it.type == io.airbyte.protocol.models.v0.AirbyteMessage.Type.RECORD || it.type == io.airbyte.protocol.models.v0.AirbyteMessage.Type.STATE }
            )
        val traceMessages = messages.filter { it.type == io.airbyte.protocol.models.v0.AirbyteMessage.Type.TRACE }
    ...
    fun testSecondSync() {
        ...
        val firstSyncMessages: List<io.airbyte.protocol.models.v0.AirbyteMessage> =
            MoreResources.readResource(
                ...
                .trim()
                .lines()
                .map { Jsons.deserialize<io.airbyte.protocol.models.v0.AirbyteMessage>(it, io.airbyte.protocol.models.v0.AirbyteMessage::class.java) }
        ...
        firstSyncMessages
            .filter { message: io.airbyte.protocol.models.v0.AirbyteMessage -> message.record != null }
            .forEach { message: io.airbyte.protocol.models.v0.AirbyteMessage -> message.record.stream = DUMMY_CATALOG_NAME }
        firstSyncMessages
            .filter { message: io.airbyte.protocol.models.v0.AirbyteMessage -> message.type == io.airbyte.protocol.models.v0.AirbyteMessage.Type.TRACE }
        ...
        // here,
        // then this need to be fixed in connectors logic to only required streams
    ...
    fun testIncrementalSync() {
        ...
        val firstSyncMessages: List<io.airbyte.protocol.models.v0.AirbyteMessage> =
            MoreResources.readResource(
                ...
                .trim()
                .lines()
                .map { Jsons.deserialize(it, io.airbyte.protocol.models.v0.AirbyteMessage::class.java) }
    ...
    open fun testIncrementalDedupeSync() {
        ...
        val latestMessagesOnly =
            expectedMessagesAfterSecondSync
                .filter { it.type == io.airbyte.protocol.models.v0.AirbyteMessage.Type.RECORD && it.record != null }
                .groupBy {
    ...
    open fun testCustomDbtTransformations() {
        ...
        if (!runner.run(AbstractDestinationAcceptanceTest.JOB_ID, AbstractDestinationAcceptanceTest.JOB_ATTEMPT, transformationRoot, config, null, dbtConfig)) {
            throw TestHarnessException("dbt debug Failed.")
        }
        // 2. Install any dependencies packages, if any
        dbtConfig.withDbtArguments("deps")
        if (!runner.transform(
                AbstractDestinationAcceptanceTest.JOB_ID,
                AbstractDestinationAcceptanceTest.JOB_ATTEMPT, transformationRoot, config, null, dbtConfig)) {
    ...
    fun testEntrypointEnvVar() {
        val entrypoint =
            EntrypointEnvChecker.getEntrypointEnvVariable(
                processFactory,
                AbstractDestinationAcceptanceTest.JOB_ID,
                AbstractDestinationAcceptanceTest.JOB_ATTEMPT,
                jobRoot,
                imageName
            )
    ...
    open fun testSyncNotFailsWithNewFields() {
        ...
        val secondSyncMessagesWithNewFields: MutableList<io.airbyte.protocol.models.v0.AirbyteMessage> =
            Lists.newArrayList(
                io.airbyte.protocol.models.v0.AirbyteMessage()
                    .withType(io.airbyte.protocol.models.v0.AirbyteMessage.Type.RECORD)
                    .withRecord(
        ...
        secondSyncMessagesWithNewFields.removeIf { airbyteMessage: io.airbyte.protocol.models.v0.AirbyteMessage ->
            airbyteMessage.type == io.airbyte.protocol.models.v0.AirbyteMessage.Type.STATE || airbyteMessage.type == io.airbyte.protocol.models.v0.AirbyteMessage.Type.TRACE
        }
    ...
    open fun testAirbyteFields() {
        val configuredCatalog =
            Jsons.deserialize(
                ...
                io.airbyte.protocol.models.v0.ConfiguredAirbyteCatalog::class.java
            )
        val config = getConfig()
        val messages =
    ...
    open fun testAirbyteTimeTypes() {
        ...
        val expectedByStream =
            messages.filter { it.type == io.airbyte.protocol.models.v0.AirbyteMessage.Type.RECORD }.groupBy { it.record.stream }
    ...
    open fun supportNamespaceTest(): Boolean {
        return false
    }
     ...
     */
    open fun getNameTransformer(): Optional<NamingConventionTransformer> =
        Optional.empty()

    /**
     * this method if the normalized namespace is different from the default one. E.g.
    ...
    open fun assertNamespaceNormalization(
        ...
        Assertions.assertEquals(
            ...
            String.format(
                "Test case %s failed; if this is expected, please assertNamespaceNormalization",
                testCaseId
            )
        )
    }
    ...
    fun runCheckWithCatchedException(
        config: JsonNode?
    ...
    fun testStressPerformance() {
        ...
        val destinationConfig =
            WorkerDestinationConfig()
                ...
                .withCatalog(
                    AbstractDestinationAcceptanceTest.convertProtocolObject(
    ...
    open fun supportBasicDataTypeTest(): Boolean {
        return false
    }

    open fun supportArrayDataTypeTest(): Boolean {
        return false
    }

    open fun supportObjectDataTypeTest(): Boolean {
        return false
    }

    open fun supportIncrementalSchemaChanges(): Boolean {
        return false
    }
    ...
    companion object {
        ...
        val specialNumericTypesSupportTest: SpecialNumericTypes
            /**
             * NaN and Infinity test are not supported by default. Please this method to
             * specify NaN/Infinity types support example:
             *
             * <pre>
             *
             * SpecialNumericTypes getSpecialNumericTypesSupportTest() { return
