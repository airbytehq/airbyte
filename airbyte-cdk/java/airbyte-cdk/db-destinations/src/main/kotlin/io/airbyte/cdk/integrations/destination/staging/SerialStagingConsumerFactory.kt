...
open class SerialStagingConsumerFactory {
    fun create(outputRecordCollector: Consumer<AirbyteMessage?>?,
               ...
               useDestinationsV2Columns: Boolean): AirbyteMessageConsumer {
        val writeConfigs = createWriteConfigs(namingResolver, config, catalog, parsedCatalog, useDestinationsV2Columns)
    ...
    companion object {
        private val LOGGER: Logger = LoggerFactory.getLogger(SerialStagingConsumerFactory::class.java)
        ...
        private fun createWriteConfigs(namingResolver: NamingConventionTransformer,
            ...
            return catalog.streams.stream().map(toWriteConfig(namingResolver, config, parsedCatalog, useDestinationsV2Columns)).collect(Collectors.toList())
        }
