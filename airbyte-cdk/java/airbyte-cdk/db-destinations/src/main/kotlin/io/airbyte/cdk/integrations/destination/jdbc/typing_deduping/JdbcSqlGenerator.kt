...
abstract class JdbcSqlGenerator(protected val namingTransformer: NamingConventionTransformer) : SqlGenerator {
    ...
    protected fun toDialectType(type: AirbyteType): DataType<*> {
        ...
        return when (type.typeName) {
            Struct.TYPE, UnsupportedOneOf.TYPE -> structType
     ...
     */
    protected abstract fun extractRawDataFields(columns: LinkedHashMap<ColumnId?, AirbyteType?>?, useExpensiveSaferCasting: Boolean): MutableList<Field<*>>
     ...
     * @return
     */
    protected abstract fun buildAirbyteMetaColumn(columns: LinkedHashMap<ColumnId?, AirbyteType?>?): Field<*>?
     ...
     * @return
     */
    protected abstract fun getRowNumber(primaryKey: List<ColumnId?>?, cursorField: Optional<ColumnId?>?): Field<Int>
     ...
     */
    @VisibleForTesting
    fun buildFinalTableFields(columns: LinkedHashMap<ColumnId?, AirbyteType>, metaColumns: Map<String?, DataType<*>?>): List<Field<*>> {
        val fields =
                metaColumns.entries.stream().map { metaColumn: Map.Entry<String?, DataType<*>?> -> DSL.field(DSL.quotedName(metaColumn.key), metaColumn.value) }.collect(Collectors.toList())
        val dataFields =
                columns.entries.stream().map { column: Map.Entry<ColumnId?, AirbyteType> -> DSL.field(DSL.quotedName(column.key!!.name), toDialectType(column.value)) }.collect(
                        Collectors.toList())
    ...
    fun getFinalTableMetaColumns(includeMetaColumn: Boolean): LinkedHashMap<String?, DataType<*>?> {
        ...
        metaColumns[COLUMN_NAME_AB_RAW_ID] = SQLDataType.VARCHAR(36).nullable(false)
        metaColumns[COLUMN_NAME_AB_EXTRACTED_AT] = timestampWithTimeZoneType.nullable(false)
        if (includeMetaColumn) metaColumns[COLUMN_NAME_AB_META] = structType.nullable(false)
        return metaColumns
    }
     ...
     */
    @VisibleForTesting
    fun buildRawTableSelectFields(columns: LinkedHashMap<ColumnId?, AirbyteType?>?,
                                  metaColumns: Map<String?, DataType<*>?>,
                                  useExpensiveSaferCasting: Boolean): List<Field<*>> {
        val fields =
                metaColumns.entries.stream().map { metaColumn: Map.Entry<String?, DataType<*>?> -> DSL.field(DSL.quotedName(metaColumn.key), metaColumn.value) }.collect(Collectors.toList())
    ...
    @VisibleForTesting
    fun rawTableCondition(syncMode: DestinationSyncMode, isCdcDeletedAtPresent: Boolean, minRawTimestamp: Optional<Instant>): Condition {
        var condition: Condition = DSL.field(DSL.name(COLUMN_NAME_AB_LOADED_AT)).isNull()
        ...
        if (minRawTimestamp.isPresent) {
            condition = condition.and(DSL.field(DSL.name(COLUMN_NAME_AB_EXTRACTED_AT)).gt(minRawTimestamp.get().toString()))
        }
        return condition
    }
    ...
    override fun createTable(stream: StreamConfig?, suffix: String?, force: Boolean): Sql {
        ...
        return transactionally(Stream.concat(
                Stream.of(
                        DSL.dropTableIfExists(DSL.quotedName(stream.id.finalNamespace, finalTableIdentifier)).getSQL(ParamType.INLINED),
                        createTableSql(stream.id.finalNamespace, finalTableIdentifier, stream.columns)),
                createIndexSql(stream, suffix).stream()).toList())
    }

    override fun updateTable(streamConfig: StreamConfig?,
                             finalSuffix: String?,
                             minRawTimestamp: Optional<Instant>,
                             useExpensiveSaferCasting: Boolean): Sql? {
    ...
    override fun overwriteFinalTable(stream: StreamId?, finalSuffix: String?): Sql? {
        return transactionally(
                DSL.dropTableIfExists(DSL.name(stream!!.finalNamespace, stream.finalName)).getSQL(ParamType.INLINED),
    ...
    override fun migrateFromV1toV2(streamId: StreamId?, namespace: String?, tableName: String?): Sql? {
        ...
        return transactionally(
                ...
                DSL.createTable(rawTableName)
                        ...
                        .column(COLUMN_NAME_AB_LOADED_AT, timestampWithTimeZoneType.nullable(true))
                        ...
                        .`as`(DSL.select(
                                ...
                                DSL.cast(null, timestampWithTimeZoneType).`as`(COLUMN_NAME_AB_LOADED_AT),
                                DSL.field(COLUMN_NAME_DATA).`as`(COLUMN_NAME_DATA),
                                DSL.cast(null, structType).`as`(COLUMN_NAME_AB_META)).from(DSL.table(DSL.name(namespace, tableName))))
                        .getSQL(ParamType.INLINED))
    }

    override fun clearLoadedAt(streamId: StreamId?): Sql {
        return of(DSL.update<Record>(DSL.table(DSL.name(streamId!!.rawNamespace, streamId.rawName)))
                .set<Any>(DSL.field(COLUMN_NAME_AB_LOADED_AT), DSL.inline(null as String?))
                .sql)
    }

    @VisibleForTesting
    fun selectFromRawTable(schemaName: String?,
                           tableName: String?,
                           columns: LinkedHashMap<ColumnId?, AirbyteType?>?,
                           metaColumns: Map<String?, DataType<*>?>,
                           condition: Condition?,
                           useExpensiveSaferCasting: Boolean): SelectConditionStep<Record> {
        val dsl = dslContext
        return dsl
    ...
    @VisibleForTesting
    fun insertIntoFinalTable(schemaName: String?,
                             tableName: String?,
                             columns: LinkedHashMap<ColumnId?, AirbyteType>,
                             metaFields: Map<String?, DataType<*>?>): InsertValuesStepN<Record> {
        val dsl = dslContext
        return dsl
                .insertInto(DSL.table(DSL.quotedName(schemaName, tableName)))
                .columns(buildFinalTableFields(columns, metaFields))
    }

    private fun insertAndDeleteTransaction(streamConfig: StreamConfig?,
                                           finalSuffix: String?,
                                           minRawTimestamp: Optional<Instant>,
                                           useExpensiveSaferCasting: Boolean): Sql {
        val finalSchema = streamConfig!!.id.finalNamespace
        val finalTable = streamConfig.id.finalName + (finalSuffix?.lowercase(Locale.getDefault()) ?: "")
        ...
        // generate fields.
        val rawTableRowsWithCast = DSL.name(TYPING_CTE_ALIAS).`as`(
        ...
        val finalTableFields = buildFinalTableFields(streamConfig.columns, getFinalTableMetaColumns(true))
        val rowNumber = getRowNumber(streamConfig.primaryKey, streamConfig.cursor)
        val filteredRows = DSL.name(NUMBERED_ROWS_CTE_ALIAS).`as`(
                DSL.select(DSL.asterisk(), rowNumber).from(rawTableRowsWithCast))
        ...
        val insertStmtWithDedupe =
                insertIntoFinalTable(finalSchema, finalTable, streamConfig.columns, getFinalTableMetaColumns(true))
                        .select(DSL.with(rawTableRowsWithCast)
                                .with(filteredRows)
                                .select(finalTableFields)
                                .from(filteredRows)
                                .where(DSL.field(DSL.name(ROW_NUMBER_COLUMN_NAME), Int::class.java).eq(1)) // Can refer by CTE.field but no use since we don't strongly type
                                // them.
                        )
                        .getSQL(ParamType.INLINED)
        ...
        val insertStmt =
                insertIntoFinalTable(finalSchema, finalTable, streamConfig.columns, getFinalTableMetaColumns(true))
                        ...
                        .getSQL(ParamType.INLINED)
        val deleteStmt = deleteFromFinalTable(finalSchema, finalTable, streamConfig.primaryKey, streamConfig.cursor)
        val deleteCdcDeletesStmt =
                if (streamConfig.columns.containsKey(cdcDeletedAtColumn)) deleteFromFinalTableCdcDeletes(finalSchema, finalTable) else ""
    ...
    private fun mergeTransaction(streamConfig: StreamConfig,
                                 finalSuffix: String,
                                 minRawTimestamp: Optional<Instant>,
                                 useExpensiveSaferCasting: Boolean): String {
    ...
    protected fun createTableSql(namespace: String?, tableName: String?, columns: LinkedHashMap<ColumnId?, AirbyteType>): String {
        val dsl = dslContext
        val createTableSql = dsl
                .createTable(DSL.quotedName(namespace, tableName))
    ...
    private fun deleteFromFinalTable(schemaName: String?,
                                     tableName: String,
                                     primaryKeys: List<ColumnId?>,
                                     cursor: Optional<ColumnId?>): String {
        val dsl = dslContext
        // Unknown type doesn't play well with where .. in (select..)
        val airbyteRawId: Field<Any> = DSL.field(DSL.quotedName(COLUMN_NAME_AB_RAW_ID))
        ...
        return dsl.deleteFrom(DSL.table(DSL.quotedName(schemaName, tableName)))
                .where(airbyteRawId.`in`(
                        DSL.select(airbyteRawId)
                                .from(DSL.select(airbyteRawId, rowNumber)
                                        .from(DSL.table(DSL.quotedName(schemaName, tableName))).asTable("airbyte_ids"))
    ...
    private fun checkpointRawTable(schemaName: String?, tableName: String?, minRawTimestamp: Optional<Instant>): String {
        val dsl = dslContext
        var extractedAtCondition = DSL.noCondition()
        if (minRawTimestamp.isPresent) {
            extractedAtCondition = extractedAtCondition.and(DSL.field(DSL.name(COLUMN_NAME_AB_EXTRACTED_AT)).gt(minRawTimestamp.get().toString()))
        }
        return dsl.update<Record>(DSL.table(DSL.quotedName(schemaName, tableName)))
                .set<Any>(DSL.field(DSL.quotedName(COLUMN_NAME_AB_LOADED_AT)), currentTimestamp())
                .where(DSL.field(DSL.quotedName(COLUMN_NAME_AB_LOADED_AT)).isNull()).and(extractedAtCondition)
                .getSQL(ParamType.INLINED)
    }
    ...
    protected fun castedField(
            ...
            type: AirbyteType,
            alias: String?,
            useExpensiveSaferCasting: Boolean): Field<*> {
        if (type is AirbyteProtocolType) {
        ...
        return when (type.typeName) {
            Struct.TYPE, UnsupportedOneOf.TYPE -> DSL.cast(field, structType).`as`(DSL.quotedName(alias))
    ...
    protected fun castedField(field: Field<*>?, type: AirbyteProtocolType?, useExpensiveSaferCasting: Boolean): Field<*> {
