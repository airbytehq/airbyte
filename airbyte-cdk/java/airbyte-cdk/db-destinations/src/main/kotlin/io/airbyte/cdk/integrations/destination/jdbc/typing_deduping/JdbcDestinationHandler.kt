...
@Slf4j
abstract class JdbcDestinationHandler<DestinationState>(protected val databaseName: String,
                                                        protected val jdbcDatabase: JdbcDatabase,
                                                        protected val rawTableSchemaName: String,
                                                        private val dialect: SQLDialect) : DestinationHandler<DestinationState> {
    ...
    private fun getInitialRawTableState(id: StreamId): InitialRawTableStatus {
        val tableExists = jdbcDatabase.executeMetadataQuery { dbmetadata: DatabaseMetaData? ->
            LOGGER.info("Retrieving table from Db metadata: {} {} {}", databaseName, id.rawNamespace, id.rawName)
            try {
                dbmetadata!!.getTables(databaseName, id.rawNamespace, id.rawName, null).use { table ->
        ...
        jdbcDatabase.unsafeQuery(
                CheckedFunction<Connection, PreparedStatement, SQLException> { conn: Connection ->
                    conn.prepareStatement(
                            dslContext.select(DSL.field("MIN(_airbyte_extracted_at)").`as`("min_timestamp"))
                                    .from(DSL.name(id.rawNamespace, id.rawName))
                                    .where(DSL.condition("_airbyte_loaded_at IS NULL"))
                                    .sql)
                },
                CheckedFunction<ResultSet, Timestamp, SQLException> { record: ResultSet -> record.getTimestamp("min_timestamp") }).use { timestampStream ->
            // Filter for nonNull values in case the query returned NULL (i.e. no unloaded records).
            val minUnloadedTimestamp: Optional<Timestamp> = timestampStream.filter(Predicate<Timestamp> { obj: Timestamp? -> Objects.nonNull(obj) }).findFirst()
            if (minUnloadedTimestamp.isPresent) {
                // Decrement by 1 second since timestamp precision varies between databases.
                val ts = minUnloadedTimestamp
        ...
        jdbcDatabase.unsafeQuery(
                CheckedFunction<Connection, PreparedStatement, SQLException> { conn: Connection ->
                    conn.prepareStatement(
                            dslContext.select(DSL.field("MAX(_airbyte_extracted_at)").`as`("min_timestamp"))
                                    .from(DSL.name(id.rawNamespace, id.rawName))
                                    .sql)
                },
                CheckedFunction<ResultSet, Timestamp, SQLException> { record: ResultSet -> record.getTimestamp("min_timestamp") }).use { timestampStream ->
            ...
            val minUnloadedTimestamp: Optional<Timestamp> = timestampStream.filter(Predicate<Timestamp> { obj: Timestamp? -> Objects.nonNull(obj) }).findFirst()
            return InitialRawTableStatus(true, false, minUnloadedTimestamp.map { obj: Timestamp -> obj.toInstant() })
        }
    }
    ...
    @Throws(Exception::class)
    override fun gatherInitialState(streamConfigs: List<StreamConfig?>?): List<DestinationInitialStatus<DestinationState>> {
        // Use stream n/ns pair because we don't want to build the full StreamId here
        val destinationStatesFuture = CompletableFuture.supplyAsync {
            try {
        ...
        val initialStates = streamConfigs!!.stream()
                .map { streamConfig: StreamConfig? -> retrieveState(destinationStatesFuture, streamConfig) }
                .toList()
        val states =
    ...
    protected val allDestinationStates: Map<AirbyteStreamNameNamespacePair, DestinationState>
        get() {
            ...
            jdbcDatabase.execute(
                    dslContext.createTableIfNotExists(DSL.quotedName(rawTableSchemaName, DESTINATION_STATE_TABLE_NAME))
                            .column(DSL.quotedName(DESTINATION_STATE_TABLE_COLUMN_NAME), SQLDataType.VARCHAR)
                            .column(DSL.quotedName(DESTINATION_STATE_TABLE_COLUMN_NAMESPACE), SQLDataType.VARCHAR) // Just use a string type, even if the destination has a json type.
                            // We're never going to query this column in a fancy way - all our processing can happen
                            // client-side.
                            .column(DSL.quotedName(DESTINATION_STATE_TABLE_COLUMN_STATE), SQLDataType.VARCHAR) // Add an updated_at field. We don't actually need it yet, but it can't hurt!
                            .column(DSL.quotedName(DESTINATION_STATE_TABLE_COLUMN_UPDATED_AT), SQLDataType.TIMESTAMPWITHTIMEZONE)
                            .getSQL(ParamType.INLINED))
            ...
            return jdbcDatabase.queryJsons(
                    dslContext.select(
                            ...
                            DSL.field(DSL.quotedName(DESTINATION_STATE_TABLE_COLUMN_STATE))).from(DSL.quotedName(rawTableSchemaName, DESTINATION_STATE_TABLE_NAME))
                            .sql)
    ...
    private fun retrieveState(destinationStatesFuture: CompletableFuture<Map<AirbyteStreamNameNamespacePair, DestinationState>>,
                              streamConfig: StreamConfig?): CompletionStage<DestinationInitialStatus<DestinationState>> {
        return destinationStatesFuture.thenApply { destinationStates: Map<AirbyteStreamNameNamespacePair, DestinationState> ->
            try {
                ...
                val destinationState = destinationStates.getOrDefault(streamConfig.id.asPair(), toDestinationState(Jsons.emptyObject()))
                return@thenApply DestinationInitialStatus<DestinationState>(streamConfig, finalTableDefinition.isPresent, initialRawTableState,
    ...
    private fun isAirbyteRawIdColumnMatch(existingTable: TableDefinition): Boolean {
        return existingTable.columns.containsKey(COLUMN_NAME_AB_RAW_ID) && toJdbcTypeName(AirbyteProtocolType.STRING) == existingTable.columns[COLUMN_NAME_AB_RAW_ID]!!.type
    }

    private fun isAirbyteExtractedAtColumnMatch(existingTable: TableDefinition): Boolean {
        return existingTable.columns.containsKey(COLUMN_NAME_AB_EXTRACTED_AT) && toJdbcTypeName(AirbyteProtocolType.TIMESTAMP_WITH_TIMEZONE) == existingTable.columns[COLUMN_NAME_AB_EXTRACTED_AT]!!.type
    }

    private fun isAirbyteMetaColumnMatch(existingTable: TableDefinition): Boolean {
        return existingTable.columns.containsKey(COLUMN_NAME_AB_META) && toJdbcTypeName(Struct(java.util.LinkedHashMap<String, AirbyteType>())) == existingTable.columns[COLUMN_NAME_AB_META]!!.type
    }

    protected fun existingSchemaMatchesStreamConfig(stream: StreamConfig?, existingTable: TableDefinition): Boolean {
        ...
        val intendedColumns = stream!!.columns.entries.stream()
                .collect(Supplier { LinkedHashMap() },
                        BiConsumer<java.util.LinkedHashMap<String, String>, Map.Entry<ColumnId, AirbyteType?>> { map: java.util.LinkedHashMap<String, String>, column: Map.Entry<ColumnId, AirbyteType?> -> map[column.key.name] = toJdbcTypeName(column.value) },
                        BiConsumer { obj: java.util.LinkedHashMap<String, String>, m: java.util.LinkedHashMap<String, String>? -> obj.putAll(m!!) })

        // Filter out Meta columns since they don't exist in stream config.
        val actualColumns = existingTable.columns.entries.stream()
                .filter { column: Map.Entry<String?, ColumnDefinition> ->
                    V2_FINAL_TABLE_METADATA_COLUMNS.stream()
                            .noneMatch(Predicate<String> { airbyteColumnName: String -> airbyteColumnName == column.key })
                }
                .collect({ LinkedHashMap() },
                        { map: java.util.LinkedHashMap<String?, String>, column: Map.Entry<String?, ColumnDefinition> -> map[column.key] = column.value.type },
                        { obj: java.util.LinkedHashMap<String?, String>, m: java.util.LinkedHashMap<String?, String>? -> obj.putAll(m!!) })
    ...
    override fun commitDestinationStates(destinationStates: Map<StreamId?, DestinationState>?) {
        ...
        val deleteStates = dslContext.deleteFrom(DSL.table(DSL.quotedName(rawTableSchemaName, DESTINATION_STATE_TABLE_NAME)))
                .where(destinationStates.keys.stream()
                        .map { streamId: StreamId? ->
                            DSL.field(DSL.quotedName(DESTINATION_STATE_TABLE_COLUMN_NAME)).eq(streamId!!.originalName)
                                    .and(DSL.field(DSL.quotedName(DESTINATION_STATE_TABLE_COLUMN_NAMESPACE)).eq(streamId.originalNamespace))
                        }
                        .reduce(
                                DSL.falseCondition()
                        ) { obj: Condition, arg2: Condition? -> obj.or(arg2) })
                .getSQL(ParamType.INLINED)

        // Reinsert all of our states
        var insertStatesStep =
                dslContext.insertInto(DSL.table(DSL.quotedName(rawTableSchemaName, DESTINATION_STATE_TABLE_NAME)))
                        .columns(
                                DSL.field(DSL.quotedName(DESTINATION_STATE_TABLE_COLUMN_NAME), String::class.java),
                                DSL.field(DSL.quotedName(DESTINATION_STATE_TABLE_COLUMN_NAMESPACE), String::class.java),
                                DSL.field(DSL.quotedName(DESTINATION_STATE_TABLE_COLUMN_STATE), String::class.java),  // This field is a timestamptz, but it's easier to just insert a string
                                // and assume the destination can cast it appropriately.
                                // Destination-specific timestamp syntax is weird and annoying.
                                DSL.field(DSL.quotedName(DESTINATION_STATE_TABLE_COLUMN_UPDATED_AT), String::class.java))
    ...
    companion object {
        ...
        fun findExistingTable(jdbcDatabase: JdbcDatabase,
                              databaseName: String?,
                              schemaName: String?,
                              tableName: String?): Optional<TableDefinition> {
            val retrievedColumnDefns = jdbcDatabase.executeMetadataQuery { dbMetadata: DatabaseMetaData? ->
                ...
                try {
                    dbMetadata!!.getColumns(databaseName, schemaName, tableName, null).use { columns ->
                        while (columns.next()) {
                            ...
                            columnDefinitions[columnName] = ColumnDefinition(columnName, typeName, columnSize, fromIsNullableIsoString(isNullable))
                        }
                    }
                } catch (e: SQLException) {
