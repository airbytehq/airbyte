...
object CopyConsumerFactory {
    ...
    fun <T> create(outputRecordCollector: Consumer<AirbyteMessage?>?,
                   ...
                   defaultSchema: String): AirbyteMessageConsumer {
    ...
    private fun <T> createWriteConfigs(namingResolver: StandardNameTransformer,
                                       config: T,
                                       catalog: ConfiguredAirbyteCatalog,
                                       streamCopierFactory: StreamCopierFactory<T>,
                                       defaultSchema: String,
                                       database: JdbcDatabase,
                                       sqlOperations: SqlOperations): Map<AirbyteStreamNameNamespacePair?, StreamCopier?> {
        ...
        for (configuredStream in catalog.streams) {
            ...
            val copier = streamCopierFactory.create(defaultSchema, config, stagingFolder, configuredStream, namingResolver, database, sqlOperations)
    ...
    private fun recordWriterFunction(pairToCopier: Map<AirbyteStreamNameNamespacePair?, StreamCopier?>,
                                     sqlOperations: SqlOperations,
                                     pairToIgnoredRecordCount: MutableMap<AirbyteStreamNameNamespacePair, Long>): RecordWriter<AirbyteRecordMessage?> {
        return RecordWriter<AirbyteRecordMessage> { pair: AirbyteStreamNameNamespacePair, records: List<AirbyteRecordMessage> ->
            ...
            for (recordMessage in records) {
                ...
                if (sqlOperations.isValidData(recordMessage.data)) {
                    ...
                    pairToIgnoredRecordCount[pair] = pairToIgnoredRecordCount.getOrDefault(pair, 0L) + 1L
                }
            }
        }
    }
    ...
    private fun removeStagingFilePrinter(pairToCopier: Map<AirbyteStreamNameNamespacePair?, StreamCopier?>): CheckAndRemoveRecordWriter {
        return CheckAndRemoveRecordWriter { pair: AirbyteStreamNameNamespacePair?, stagingFileName: String? ->
    ...
    private fun onCloseFunction(pairToCopier: Map<AirbyteStreamNameNamespacePair?, StreamCopier?>,
                                database: JdbcDatabase,
                                sqlOperations: SqlOperations,
                                pairToIgnoredRecordCount: Map<AirbyteStreamNameNamespacePair, Long>,
                                dataSource: DataSource): OnCloseFunction {
    ...
    private fun closeAsOneTransaction(pairToCopier: Map<AirbyteStreamNameNamespacePair?, StreamCopier?>,
                                      hasFailed: Boolean,
                                      db: JdbcDatabase,
                                      sqlOperations: SqlOperations,
                                      dataSource: DataSource) {
        ...
        try {
            ...
            for (copier in streamCopiers) {
                try {
                    ...
                    val message = String.format("Failed to finalize copy to temp table due to: %s", e)
