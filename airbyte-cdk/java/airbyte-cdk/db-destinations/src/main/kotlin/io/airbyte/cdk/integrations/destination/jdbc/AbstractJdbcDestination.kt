...
(driverClass: String?,
 ...
 protected val sqlOperations: SqlOperations) : JdbcConnector(driverClass!!), Destination {
    ...
    protected fun modifyDataSourceBuilder(builder: DataSourceFactory.DataSourceBuilder): DataSourceFactory.DataSourceBuilder {
        return builder
    }
    ...
    private fun assertCustomParametersDontOverwriteDefaultParameters(customParameters: Map<String, String>,
    ...
    protected fun getDataTransformer(parsedCatalog: ParsedCatalog?,
                                     defaultNamespace: String?): StreamAwareDataTransformer {
    ...
    private fun getV2MessageConsumer(config: JsonNode,
        ...
        val parsedCatalog = rawNamespaceOverride
        ...
        typerDeduper = if (disableTypeDedupe) {
            NoOpTyperDeduperWithV1V2Migrations(sqlGenerator, destinationHandler, parsedCatalog, migrator, v2TableMigrator, migrations)
        } else {
            DefaultTyperDeduper(sqlGenerator, destinationHandler, parsedCatalog, migrator, v2TableMigrator, migrations)
        }
    ...
    companion object {
        ...
        @Deprecated("")
        @Throws(Exception::class)
        fun attemptSQLCreateAndDropTableOperations(outputSchema: String?,
                                                   database: JdbcDatabase,
         ...
         */
        @Throws(Exception::class)
        fun attemptTableOperations(outputSchema: String?,
                                   database: JdbcDatabase,
                                   namingResolver: NamingConventionTransformer,
                                   sqlOps: SqlOperations,
            ...
            try {
                ...
                database.bufferedResultSetQuery({ conn: Connection -> conn.metaData.catalogs }, { queryContext: ResultSet? -> JdbcUtils.defaultSourceOperations.rowToJson(queryContext!!) })

                // verify we have write permissions on the target schema by creating a table with a random name,
                // then dropping that table
                val outputTableName = namingResolver.getIdentifier("_airbyte_connection_test_" + UUID.randomUUID().toString().replace("-".toRegex(), ""))
                ...
                try {
                    if (attemptInsert) {
                        sqlOps.insertRecords(database, java.util.List.of(dummyRecord), outputSchema, outputTableName)
                    }
                } finally {
        ...
        private val dummyRecord: PartialAirbyteMessage
            ...
            get() {
                ...
                return PartialAirbyteMessage()
                        .withRecord(PartialAirbyteRecordMessage()
                                .withStream("stream1")
