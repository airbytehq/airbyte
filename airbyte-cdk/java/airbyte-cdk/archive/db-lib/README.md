...
Check `io.airbyte.cdk.db.instance.configs` for example.

## Database Instance
- Create a new package under `io.airbyte.cdk.db.instance` with the name of the database.
...
- Run the `newMigration` command to create a new migration file in `io.airbyte.cdk.db.instance.<db-name>.migrations`.
