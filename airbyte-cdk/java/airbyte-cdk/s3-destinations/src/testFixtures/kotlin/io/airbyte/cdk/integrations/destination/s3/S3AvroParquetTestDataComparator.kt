...
class S3AvroParquetTestDataComparator : AdvancedTestDataComparator() {
    protected fun compareDateValues(airbyteMessageValue: String?, destinationValue: String): Boolean {
        val destinationDate = LocalDate.ofEpochDay(destinationValue.toLong())
        val expectedDate = LocalDate.parse(airbyteMessageValue, DateTimeFormatter.ofPattern(AdvancedTestDataComparator.AIRBYTE_DATE_FORMAT))
    ...
    protected fun parseDestinationDateWithTz(destinationValue: String): ZonedDateTime {
        return ZonedDateTime.ofInstant(getInstantFromEpoch(destinationValue), ZoneOffset.UTC)
    }

    protected fun compareDateTimeValues(airbyteMessageValue: String?, destinationValue: String): Boolean {
