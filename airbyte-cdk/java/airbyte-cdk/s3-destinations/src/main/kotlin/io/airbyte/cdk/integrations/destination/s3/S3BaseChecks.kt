...
object S3BaseChecks {
     ...
     */
    @JvmStatic
    fun attemptS3WriteAndDelete(storageOperations: S3StorageOperations,
                                s3Config: S3DestinationConfig,
                                bucketPath: String) {
        attemptS3WriteAndDelete(storageOperations, s3Config, bucketPath, s3Config.s3Client)
    }
    ...
    fun testMultipartUpload(s3Client: AmazonS3?, bucketName: String?, bucketPath: String?) {
        ...
        try {
            manager.multiPartOutputStreams[0].use { outputStream ->
                CSVPrinter(PrintWriter(outputStream, true, StandardCharsets.UTF_8), CSVFormat.DEFAULT).use { csvPrinter ->
    ...
    fun attemptS3WriteAndDelete(storageOperations: S3StorageOperations,
                                ...
                                bucketPath: String,
                                s3: AmazonS3?) {
        val prefix = if (Strings.isNullOrEmpty(bucketPath)) {
            ""
        ...
        val outputTableName = prefix + "_airbyte_connection_test_" + UUID.randomUUID().toString().replace("-".toRegex(), "")
     ...
     */
    private fun attemptWriteAndDeleteS3Object(storageOperations: S3StorageOperations,
                                              s3Config: S3DestinationConfig,
