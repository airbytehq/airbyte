    ...
    companion object {
        ...
        fun fromJsonSchemaType(@Nonnull jsonSchemaType: String, jsonSchemaAirbyteType: String? = null): JsonSchemaType {
            ...
            if (jsonSchemaAirbyteType != null) {
                matchSchemaType = Arrays.stream(entries.toTypedArray())
                        .filter { type: JsonSchemaType -> jsonSchemaType == type.jsonSchemaType }
                        .filter { type: JsonSchemaType -> jsonSchemaAirbyteType == type.jsonSchemaAirbyteType }
                        .toList()
            }
            ...
            if (matchSchemaType == null || matchSchemaType.isEmpty()) {
                matchSchemaType =
                        Arrays.stream(entries.toTypedArray()).filter { format: JsonSchemaType -> jsonSchemaType == format.jsonSchemaType && format.jsonSchemaAirbyteType == null }.toList()
            }

            require(!matchSchemaType!!.isEmpty()) { String.format("Unexpected jsonSchemaType - %s and jsonSchemaAirbyteType - %s", jsonSchemaType, jsonSchemaAirbyteType) }
