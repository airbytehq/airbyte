 ...
 */
@JvmRecord
data class AesCbcEnvelopeEncryption(@field:Nonnull @param:Nonnull val key: ByteArray, @field:Nonnull @param:Nonnull val keyType: KeyType) : EncryptionConfig {
    enum class KeyType {
        EPHEMERAL,
        USER_PROVIDED
    }
    ...
    companion object {
        fun fromJson(encryptionNode: JsonNode): AesCbcEnvelopeEncryption {
            ...
            return if (StringUtils.isEmpty(kek)) {
                ...
                AesCbcEnvelopeEncryption(EncryptionConfig.Companion.BASE64_DECODER.decode(kek), KeyType.USER_PROVIDED)
            }
        }

        private fun encryptionWithRandomKey(): AesCbcEnvelopeEncryption {
            try {
                val kekGenerator = KeyGenerator.getInstance(AesCbcEnvelopeEncryptionBlobDecorator.KEY_ENCRYPTING_ALGO)
                kekGenerator.init(AesCbcEnvelopeEncryptionBlobDecorator.AES_KEY_SIZE_BITS)
                return AesCbcEnvelopeEncryption(kekGenerator.generateKey().encoded, KeyType.EPHEMERAL)
