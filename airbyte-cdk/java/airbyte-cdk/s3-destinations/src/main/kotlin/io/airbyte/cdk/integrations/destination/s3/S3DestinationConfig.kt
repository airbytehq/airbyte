...
/**
 * An S3 configuration. Typical usage sets at most one of `bucketPath` (necessary for more
...
open class S3DestinationConfig {
    ...
    protected open fun createS3Client(): AmazonS3? {
        ...
        val credentialsProvider = s3CredentialConfig.getS3CredentialsProvider()
        val credentialType = s3CredentialConfig.getCredentialType()
        ...
        return AmazonS3ClientBuilder.standard()
                .withEndpointConfiguration(AwsClientBuilder.EndpointConfiguration(endpoint, bucketRegion))
    ...
    override fun equals(o: Any?): Boolean {
        ...
        return endpoint == that.endpoint && bucketName == that.bucketName && bucketPath == that.bucketPath && bucketRegion == that.bucketRegion && s3CredentialConfig == that.s3CredentialConfig && formatConfig == that.formatConfig
    }

    override fun hashCode(): Int {
        return Objects.hash(endpoint, bucketName, bucketPath, bucketRegion, s3CredentialConfig, formatConfig)
    }

    class Builder(private var bucketName: String?, private var bucketPath: String, private var bucketRegion: String?) {
        private var endpoint: String? = ""
        private var pathFormat = S3DestinationConstants.DEFAULT_PATH_FORMAT

        private var credentialConfig: S3CredentialConfig? = null
    ...
    companion object {
        ...
        @JvmStatic
        fun getS3DestinationConfig(@Nonnull config: JsonNode?, @Nonnull storageProvider: StorageProvider?): S3DestinationConfig {
            var builder = create(
            ...
            if (config.has(S3Constants.FILE_NAME_PATTERN)) {
                builder = builder.withFileNamePattern(config[S3Constants.FILE_NAME_PATTERN].asText())
            }
            ...
            when (storageProvider) {
                StorageProvider.CF_R2 -> {
                    if (config.has(S3Constants.ACCOUNT_ID)) {
                        val endpoint = String.format(R2_INSTANCE_URL, getProperty(config, S3Constants.ACCOUNT_ID))
                        builder = builder.withEndpoint(endpoint)
                    }
                    builder = builder.withCheckIntegrity(false) // https://developers.cloudflare.com/r2/platform/s3-compatibility/api/#implemented-object-level-operations
                            // 3 or less
            ...
            val credentialConfig = if (config.has(S3Constants.ACCESS_KEY_ID)) {
                S3AccessKeyCredentialConfig(getProperty(config, S3Constants.ACCESS_KEY_ID), getProperty(config, S3Constants.SECRET_ACCESS_KEY))
            } else {
        ...
        private fun getProperty(@Nonnull config: JsonNode?, @Nonnull key: String): String? {
            val node = config!![key] ?: return null
            return node.asText()
        }
    }
}
