...
class S3CsvWriter private constructor(config: S3DestinationConfig,
                                      s3Client: AmazonS3,
                                      ...
                                      csvSheetGenerator: CsvSheetGenerator?) : BaseS3Writer(config, s3Client, configuredStream), DestinationFileWriter {
    ...
    init {
        ...
        LOGGER.info("Full S3 path for stream '{}': s3://{}/{}", stream.name, config.bucketName,
                outputPath)
        ...
        if (writeHeader) {
            csvSettings = csvSettings.withHeader(*csvSheetGenerator.getHeaderRow().toTypedArray<String?>())
        }
        this.csvPrinter = CSVPrinter(PrintWriter(outputStream, true, StandardCharsets.UTF_8), csvSettings)
    }

    class Builder(private val config: S3DestinationConfig,
        ...
        private var csvSheetGenerator: CsvSheetGenerator? = null
        ...
        fun build(): S3CsvWriter {
            if (csvSheetGenerator == null) {
                ...
                csvSheetGenerator = CsvSheetGenerator.Factory.create(configuredStream.stream.jsonSchema, formatConfig)
            }
            return S3CsvWriter(config,
                    s3Client,
                    configuredStream,
                    uploadTimestamp,
                    uploadThreads,
