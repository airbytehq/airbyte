...
class S3ConsumerFactory {
    fun create(
        ...
        LOGGER.info{"SGX calling create"}
    ...
    fun createAsync(
        ...
        val flushFunction =
            if (s3Config.formatConfig.format == FileUploadFormat.RAW_FILES) {
                FileTransferDestinationFlushFunction(streamDescriptorToWriteConfig, storageOps, featureFlags)
            } else {
                val createFunction =
                    getCreateFunction(
                        s3Config,
    ...
    private class FileTransferDestinationFlushFunction(
        ...
        override fun flush(
            ...
            val absolutePath = records[0].record!!.file!!.fileUrl!!
            val relativePath = records[0].record!!.file!!.fileRelativePath!!
            val fullObjectKey = writeConfig.fullOutputPath + relativePath
            val dataFile = File(absolutePath)
            LOGGER.info {"SGX calling storageOps.loadDataIntoBucket with fullObjectKey = $fullObjectKey, " +
    ...
    companion object {
        ...
        private fun toWriteConfig(
            ...
            return Function { stream: ConfiguredAirbyteStream ->
                ...
                LOGGER.info {"SGX creating write config for namespace=$namespace, streamName=$streamName, fullOutputPath=$fullOutputPath, customOutputFormat=$customOutputFormat"}
