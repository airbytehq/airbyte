...
class S3ConsumerFactory {
    fun create(outputRecordCollector: Consumer<AirbyteMessage?>?,
               storageOperations: BlobStorageOperations,
               namingResolver: NamingConventionTransformer,
               onCreateBuffer: BufferCreateFunction?,
               s3Config: S3DestinationConfig?,
               catalog: ConfiguredAirbyteCatalog?): AirbyteMessageConsumer {
    ...
    private fun flushBufferFunction(storageOperations: BlobStorageOperations,
                                    writeConfigs: List<WriteConfig>,
                                    catalog: ConfiguredAirbyteCatalog?): FlushBufferFunction {
        val pairToWriteConfig =
                writeConfigs.stream()
                        .collect(Collectors.toUnmodifiableMap(
                                Function { config: WriteConfig -> toNameNamespacePair(config) }, Function.identity()))

        return FlushBufferFunction { pair: AirbyteStreamNameNamespacePair, writer: SerializableBuffer ->
            LOGGER.info("Flushing buffer for stream {} ({}) to storage", pair.name, FileUtils.byteCountToDisplaySize(writer.byteCount))
            require(pairToWriteConfig.containsKey(pair)) { String.format("Message contained record from a stream %s that was not in the catalog. \ncatalog: %s", pair, Jsons.serialize(catalog)) }
    ...
    private fun onCloseFunction(storageOperations: BlobStorageOperations,
                                writeConfigs: List<WriteConfig>): OnCloseFunction {
        return OnCloseFunction { hasFailed: Boolean, streamSyncSummaries: Map<StreamDescriptor?, StreamSyncSummary?>? ->
            if (hasFailed) {
                LOGGER.info("Cleaning up destination started for {} streams", writeConfigs.size)
                for (writeConfig in writeConfigs) {
                    storageOperations.cleanUpBucketObject(writeConfig.fullOutputPath, writeConfig.storedFiles)
    ...
    companion object {
        ...
        private fun createWriteConfigs(storageOperations: BlobStorageOperations,
                                       namingResolver: NamingConventionTransformer,
                                       config: S3DestinationConfig?,
                                       catalog: ConfiguredAirbyteCatalog?): List<WriteConfig> {
        ...
        private fun toWriteConfig(storageOperations: BlobStorageOperations,
                                  namingResolver: NamingConventionTransformer,
                                  s3Config: S3DestinationConfig?): Function<ConfiguredAirbyteStream, WriteConfig> {
            return Function { stream: ConfiguredAirbyteStream ->
                ...
                val bucketPath = s3Config.getBucketPath()
                val customOutputFormat = java.lang.String.join("/", bucketPath, s3Config.getPathFormat())
                val fullOutputPath = storageOperations.getBucketObjectPath(namespace, streamName, SYNC_DATETIME, customOutputFormat)
