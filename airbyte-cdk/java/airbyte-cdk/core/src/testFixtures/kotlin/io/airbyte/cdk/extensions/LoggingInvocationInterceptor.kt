 ...
 * ServiceLoader](https://docs.oracle.com/javase%2F9%2Fdocs%2Fapi%2F%2F/java/util/ServiceLoader.html) The declaration can be found in
...
class LoggingInvocationInterceptor : InvocationInterceptor {
    private class LoggingInvocationInterceptorHandler : InvocationHandler {
        ...
        override fun invoke(proxy: Any?, method: Method?, args: Array<Any?>?): Any? {
            ...
            if (methodName == "interceptDynamicTest") {
                logLineSuffix = "execution of DynamicTest %s".formatted(extensionContext!!.displayName)
            } else if (methodName == "interceptTestClassConstructor") {
                logLineSuffix = "instance creation for %s".formatted(invocationContext!!.targetClass)
            ...
            try {
                ...
                if (timeout != null) {
                    ...
                    Timer("TimeoutTimer-" + currentThread.name, true).schedule(timeoutTask, timeout.toMillis())
                } else {
        ...
        companion object {
            ...
            private val PATTERN: Pattern? = Pattern.compile("([1-9]\\d*) *((?:[nμm]?s)|m|h|d)?",
            ...
            fun parseDuration(text: String?): Duration? {
                ...
                if (matcher.matches()) {
                    ...
                    val unit = if (unitAbbreviation == null) TimeUnit.SECONDS else UNITS_BY_ABBREVIATION!![unitAbbreviation.lowercase()]
                    return Duration.ofSeconds(unit!!.toSeconds(value))
                }
                throw DateTimeParseException("Timeout duration is not in the expected format (<number> [ns|μs|ms|s|m|h|d])",
                        text, 0)
            }
    ...
    private val proxy: InvocationInterceptor? = Proxy.newProxyInstance(
            javaClass.classLoader,
            arrayOf<Class<*>?>(InvocationInterceptor::class.java),
            LoggingInvocationInterceptorHandler()) as InvocationInterceptor

    @Throws(Throwable::class)
    override fun interceptAfterAllMethod(invocation: InvocationInterceptor.Invocation<Void?>?,
    ...
    @Throws(Throwable::class)
    override fun interceptAfterEachMethod(invocation: InvocationInterceptor.Invocation<Void?>?,
                                          invocationContext: ReflectiveInvocationContext<Method?>?,
                                          extensionContext: ExtensionContext?) {
    ...
    @Throws(Throwable::class)
    override fun interceptBeforeAllMethod(invocation: InvocationInterceptor.Invocation<Void?>?,
                                          invocationContext: ReflectiveInvocationContext<Method?>?,
                                          extensionContext: ExtensionContext?) {
    ...
    @Throws(Throwable::class)
    override fun interceptBeforeEachMethod(invocation: InvocationInterceptor.Invocation<Void?>?,
                                           invocationContext: ReflectiveInvocationContext<Method?>?,
                                           extensionContext: ExtensionContext?) {
    ...
    @Throws(Throwable::class)
    override fun interceptDynamicTest(invocation: InvocationInterceptor.Invocation<Void?>?,
    ...
    override fun interceptTestMethod(invocation: InvocationInterceptor.Invocation<Void?>?,
        ...
        if (!Modifier.isPublic(invocationContext!!.executable!!.modifiers)) {
            LOGGER!!.warn("Junit method {}.{} is not declared as public", invocationContext.executable!!.declaringClass.canonicalName,
                    invocationContext.executable!!.name)
        }
    ...
    @Throws(Throwable::class)
    override fun <T> interceptTestFactoryMethod(invocation: InvocationInterceptor.Invocation<T?>?,
                                                invocationContext: ReflectiveInvocationContext<Method?>?,
                                                extensionContext: ExtensionContext?): T? {
        return proxy!!.interceptTestFactoryMethod(invocation, invocationContext, extensionContext)
    }

    @Throws(Throwable::class)
    override fun <T> interceptTestClassConstructor(invocation: InvocationInterceptor.Invocation<T?>?,
    ...
    companion object {
        private val LOGGER: Logger? = LoggerFactory.getLogger(LoggingInvocationInterceptor::class.java)
        private val JUNIT_METHOD_EXECUTION_TIMEOUT_PROPERTY_NAME: String? = "JunitMethodExecutionTimeout"
    }
}
