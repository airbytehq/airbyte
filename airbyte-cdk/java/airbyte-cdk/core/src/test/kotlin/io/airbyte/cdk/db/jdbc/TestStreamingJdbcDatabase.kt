...
internal class TestStreamingJdbcDatabase {
    ...
    fun setup() {
        ...
        val connectionPool = DataSourceFactory.create(
        ...
        streamingJdbcDatabase = StreamingJdbcDatabase(connectionPool, JdbcUtils.getDefaultSourceOperations(), Supplier<JdbcStreamingQueryConfig> { AdaptiveStreamingQueryConfig() })
    }
    ...
    fun testQuery() {
        ...
        val actual = streamingJdbcDatabase!!.queryJsons({ connection: Connection ->
        ...
        val expectedRecords: List<JsonNode> = Lists.newArrayList(
     ...
     * contain more than `FetchSizeConstants.INITIAL_SAMPLE_SIZE` rows. Otherwise, all rows will be fetched in the first
