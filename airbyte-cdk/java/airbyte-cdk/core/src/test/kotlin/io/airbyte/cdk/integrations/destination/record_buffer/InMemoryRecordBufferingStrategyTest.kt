...
class InMemoryRecordBufferingStrategyTest {
    private val recordWriter: RecordWriter<AirbyteRecordMessage> = Mockito.mock(RecordWriter::class.java)

    @Test
    @Throws(Exception::class)
    fun testBuffering() {
        val buffering = InMemoryRecordBufferingStrategy(recordWriter, MAX_QUEUE_SIZE_IN_BYTES.toLong())
        ...
        Mockito.verify(recordWriter, Mockito.times(1)).accept(stream2, List.of(message3.record, message4.record))
    }
