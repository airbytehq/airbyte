...
internal class ConcurrentStreamConsumerTest {
    ...
    fun testAcceptMessage() {
        ...
        val streamConsumer: Consumer<AutoCloseableIterator<AirbyteMessage>> = Mockito.mock(Consumer::class.java)
    ...
    fun testAcceptMessageWithMultipleExceptions() {
        val stream1: AutoCloseableIterator<AirbyteMessage> = Mockito.mock(AutoCloseableIterator::class.java)
        val stream2: AutoCloseableIterator<AirbyteMessage> = Mockito.mock(AutoCloseableIterator::class.java)
        val stream3: AutoCloseableIterator<AirbyteMessage> = Mockito.mock(AutoCloseableIterator::class.java)
        val streamConsumer: Consumer<AutoCloseableIterator<AirbyteMessage>> = Mockito.mock(Consumer::class.java)
        ...
        Assertions.assertDoesNotThrow { concurrentStreamConsumer.accept(List.of(stream1, stream2, stream3)) }

        Mockito.verify(streamConsumer, Mockito.times(3)).accept(ArgumentMatchers.any(AutoCloseableIterator::class.java))
        Assertions.assertTrue(concurrentStreamConsumer.exception.isPresent)
    ...
    fun testMoreStreamsThanAvailableThreads() {
        ...
        Mockito.verify(streamConsumer, Mockito.times(streams.size)).accept(ArgumentMatchers.any(AutoCloseableIterator::class.java))
    }
