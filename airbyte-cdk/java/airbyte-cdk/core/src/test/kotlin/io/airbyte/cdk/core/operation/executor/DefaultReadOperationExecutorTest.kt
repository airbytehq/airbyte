...
class DefaultReadOperationExecutorTest {
    ...
    internal fun `test that for each message in the iterator, an airbyte message is written to the output record consumer`() {
        ...
        every { shutdownUtils.stopOrphanedThreads(ShutdownUtils.EXIT_HOOK,
            ShutdownUtils.INTERRUPT_THREAD_DELAY_MINUTES,
            TimeUnit.MINUTES,
            ShutdownUtils.EXIT_THREAD_DELAY_MINUTES,
            TimeUnit.MINUTES) } returns Unit

        val executor = DefaultReadOperationExecutor(messageIterator=messageIterator,
            outputRecordCollector=outputRecordCollector, shutdownUtils=shutdownUtils)
        ...
        verify(exactly=1) { shutdownUtils.stopOrphanedThreads(ShutdownUtils.EXIT_HOOK,
            ShutdownUtils.INTERRUPT_THREAD_DELAY_MINUTES,
            TimeUnit.MINUTES,
            ShutdownUtils.EXIT_THREAD_DELAY_MINUTES,
            TimeUnit.MINUTES) }
