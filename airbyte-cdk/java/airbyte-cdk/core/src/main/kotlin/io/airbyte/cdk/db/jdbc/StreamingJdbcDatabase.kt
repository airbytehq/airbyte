 ...
 * developer to specify the correct configuration in order for a [PreparedStatement] to
 * execute as in a streaming / chunked manner.
 */
class StreamingJdbcDatabase(dataSource: DataSource,
                            sourceOperations: JdbcCompatibleSourceOperations<*>?,
                            private val streamingQueryConfigProvider: Supplier<JdbcStreamingQueryConfig>) : DefaultJdbcDatabase(dataSource, sourceOperations) {
    /**
     ...
     */
    protected fun <T> toUnsafeStream(resultSet: ResultSet,
                                     mapper: CheckedFunction<ResultSet, T, SQLException?>,
                                     streamingConfig: JdbcStreamingQueryConfig): Stream<T> {
