 ...
 */
open class SshTunnel @JvmOverloads constructor(val originalConfig: JsonNode?,
                                               ...
                                               connectionOptions: Optional<SshConnectionOptions>? = Optional.empty()) : AutoCloseable {
    ...
    @JvmRecord
    data class SshConnectionOptions(val sessionHeartbeatInterval: Duration,
    ...
    init {
        ...
        if (tunnelMethod == TunnelMethod.NO_TUNNEL) {
            ...
            this.sshclient = connectionOptions!!.map { sshConnectionOptions: SshConnectionOptions ->
                createClient(sshConnectionOptions.sessionHeartbeatInterval,
    ...
    val configInTunnel: JsonNode?
        get() {
            if (tunnelMethod == TunnelMethod.NO_TUNNEL) {
                ...
                if (hostKey != null) {
                    Jsons.replaceNestedString(clone, hostKey, SshdSocketAddress.LOCALHOST_ADDRESS.hostName)
                }
                if (portKey != null) {
                ...
                if (endPointKey != null) {
                    val tunnelEndPointURL =
                            URL(remoteServiceProtocol, SshdSocketAddress.LOCALHOST_ADDRESS.hostName, tunnelLocalPort, remoteServicePath)
    ...
    val privateKeyPair: KeyPair
        ...
        get() {
            ...
            throw ConfigErrorException("Unable to load private key pairs, verify key pairs are properly inputted")
        }
    ...
    private fun createClient(sessionHeartbeatInterval: Duration, globalHeartbeatInterval: Duration, idleTimeout: Duration): SshClient {
    ...
    open fun openTunnel(client: SshClient?): ClientSession? {
        try {
            ...
            val address = session.startLocalPortForwarding( // entering 0 lets the OS pick a free port for us.
                    SshdSocketAddress(InetSocketAddress.createUnresolved(SshdSocketAddress.LOCALHOST_ADDRESS.hostName, 0)),
            ...
            if (e is SshException && e.message
                            .lowercase()
    ...
    companion object {
        ...
        const val SSH_TIMEOUT_DISPLAY_MESSAGE: String = "Timed out while opening a SSH Tunnel. Please double check the given SSH configurations and try again."
        ...
        fun getInstance(config: JsonNode?, hostKey: List<String>?, portKey: List<String>?): SshTunnel {
            val tunnelMethod = Jsons.getOptional(config, "tunnel_method", "tunnel_method")
                    .map { method: JsonNode -> TunnelMethod.valueOf(method.asText().trim { it <= ' ' }) }
        ...
        private fun getSshConnectionOptions(config: JsonNode?): @NotNull Optional<SshConnectionOptions>? {
            ...
            if (connectionOptionConfig.isPresent) {
                ...
                val sessionHeartbeatInterval = Jsons.getOptional(connectionOptionsNode, SESSION_HEARTBEAT_INTERVAL_KEY)
                        .map { interval: JsonNode -> Duration.ofMillis(interval.asLong()) }
                        .orElse(Duration.ofSeconds(1))
                val globalHeartbeatInterval = Jsons.getOptional(connectionOptionsNode, GLOBAL_HEARTBEAT_INTERVAL_KEY)
                        .map { interval: JsonNode -> Duration.ofMillis(interval.asLong()) }
                        .orElse(Duration.ofSeconds(2))
                val idleTimeout = Jsons.getOptional(connectionOptionsNode, IDLE_TIMEOUT_KEY)
                        .map { interval: JsonNode -> Duration.ofMillis(interval.asLong()) }
                        .orElse(Duration.ZERO)
                connectionOptions = Optional.of(
                        SshConnectionOptions(sessionHeartbeatInterval, globalHeartbeatInterval, idleTimeout))
            } else {
        ...
        @Throws(Exception::class)
        fun <T> sshWrap(config: JsonNode?,
                        hostKey: List<String>?,
                        portKey: List<String>?,
                        wrapped: CheckedFunction<JsonNode?, T, Exception?>): T {
