...
abstract class JdbcConnector protected constructor(@JvmField protected val driverClassName: String) : BaseConnector() {
    ...
    companion object {
        ...
        fun getConnectionTimeout(connectionProperties: Map<String?, String?>, driverClassName: String?): Duration {
            val parsedConnectionTimeout = when (DatabaseDriver.Companion.findByDriverClassName(driverClassName)) {
                DatabaseDriver.POSTGRESQL -> maybeParseDuration(connectionProperties[POSTGRES_CONNECT_TIMEOUT_KEY], ChronoUnit.SECONDS)
                        .or { Optional.of<Duration>(POSTGRES_CONNECT_TIMEOUT_DEFAULT_DURATION) }

                DatabaseDriver.MYSQL -> maybeParseDuration(connectionProperties["connectTimeout"], ChronoUnit.MILLIS)
                DatabaseDriver.MSSQLSERVER -> maybeParseDuration(connectionProperties["loginTimeout"], ChronoUnit.SECONDS)
                else -> maybeParseDuration(connectionProperties[CONNECT_TIMEOUT_KEY], ChronoUnit.SECONDS) // Enforce minimum timeout duration for unspecified data sources.
