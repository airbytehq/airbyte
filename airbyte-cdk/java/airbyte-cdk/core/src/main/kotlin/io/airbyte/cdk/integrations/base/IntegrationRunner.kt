 ...
 * destination.
 */
class IntegrationRunner @VisibleForTesting internal constructor(cliParser: IntegrationCliParser,
    ...
    constructor(destination: Destination?) : this(IntegrationCliParser(), Consumer<AirbyteMessage?> { message: AirbyteMessage? -> Destination.Companion.defaultOutputRecordCollector(message) }, destination, null)

    constructor(source: Source?) : this(IntegrationCliParser(), Consumer<AirbyteMessage?> { message: AirbyteMessage? -> Destination.Companion.defaultOutputRecordCollector(message) }, null, source)

    init {
        Preconditions.checkState((destination != null) xor (source != null), "can only pass in a destination or a source")
    ...
    @VisibleForTesting
    internal constructor(cliParser: IntegrationCliParser,
                         outputRecordCollector: Consumer<AirbyteMessage?>,
                         destination: Destination?,
                         source: Source?,
                         jsonSchemaValidator: JsonSchemaValidator) : this(cliParser, outputRecordCollector, destination, source) {
        validator = jsonSchemaValidator
    }
    ...
    private fun runInternal(parsed: IntegrationConfig?) {
        ...
        try {
            when (parsed.getCommand()) {
                Command.SPEC -> outputRecordCollector.accept(AirbyteMessage().withType(AirbyteMessage.Type.SPEC).withSpec(integration.spec()))
                Command.CHECK -> {
                    val config = parseConfig(parsed!!.configPath)
                    ...
                    try {
                        ...
                        outputRecordCollector.accept(AirbyteMessage().withType(AirbyteMessage.Type.CONNECTION_STATUS).withConnectionStatus(
                                AirbyteConnectionStatus().withStatus(AirbyteConnectionStatus.Status.FAILED).withMessage(e.message)))
                    }

                    outputRecordCollector.accept(AirbyteMessage().withType(AirbyteMessage.Type.CONNECTION_STATUS).withConnectionStatus(integration.check(config)))
                }

                Command.DISCOVER -> {
                    val config = parseConfig(parsed!!.configPath)
                    validateConfig(integration.spec()!!.connectionSpecification, config, "DISCOVER")
                    outputRecordCollector.accept(AirbyteMessage().withType(AirbyteMessage.Type.CATALOG).withCatalog(source!!.discover(config)))
                }

                Command.READ -> {
                    val config = parseConfig(parsed!!.configPath)
                    validateConfig(integration.spec()!!.connectionSpecification, config, "READ")
                    val catalog = parseConfig(parsed.catalogPath, ConfiguredAirbyteCatalog::class.java)
                    val stateOptional = parsed.statePath.map { path: Path? -> parseConfig(path) }
                    try {
                ...
                Command.WRITE -> {
                    ...
                    DestinationConfig.Companion.initialize(config, (integration as Destination).isV2Destination)
                    val catalog = parseConfig(parsed.catalogPath, ConfiguredAirbyteCatalog::class.java)

                    try {
                        destination!!.getSerializedMessageConsumer(config, catalog, outputRecordCollector).use { consumer ->
                ...
                else -> throw IllegalStateException("Unexpected value: " + parsed.getCommand())
            }
        } catch (e: Exception) {
            ...
            if (parsed.getCommand() == Command.CHECK) {
    ...
    private fun produceMessages(messageIterator: AutoCloseableIterator<AirbyteMessage?>?, recordCollector: Consumer<AirbyteMessage?>) {
        messageIterator!!.airbyteStream.ifPresent { s: AirbyteStreamNameNamespacePair? -> LOGGER.debug("Producing messages for stream {}...", s) }
        messageIterator.forEachRemaining(recordCollector)
        messageIterator.airbyteStream.ifPresent { s: AirbyteStreamNameNamespacePair? -> LOGGER.debug("Finished producing messages for stream {}...") }
    }

    @Throws(Exception::class)
    private fun readConcurrent(config: JsonNode, catalog: ConfiguredAirbyteCatalog, stateOptional: Optional<JsonNode?>) {
        val streams = source!!.readStreams(config, catalog, stateOptional.orElse(null))

        try {
            ConcurrentStreamConsumer({ stream: AutoCloseableIterator<AirbyteMessage?>? -> this.consumeFromStream(stream) }, streams!!.size).use { streamConsumer ->
                /*
       ...
       */
                ...
                val partitions = Lists.partition(streams.stream().toList(),
                        partitionSize!!)

                // Submit each stream partition for concurrent execution
                partitions.forEach(Consumer { partition: List<AutoCloseableIterator<AirbyteMessage?>?> ->
    ...
    @Throws(Exception::class)
    private fun readSerial(config: JsonNode, catalog: ConfiguredAirbyteCatalog, stateOptional: Optional<JsonNode?>) {
        try {
    ...
    private fun consumeFromStream(stream: AutoCloseableIterator<AirbyteMessage?>?) {
        try {
            val streamStatusTrackingRecordConsumer = StreamStatusUtils.statusTrackingRecordCollector(stream,
                    outputRecordCollector, Optional.of(Consumer { obj: AirbyteStreamStatusHolder? -> AirbyteTraceMessageUtility.emitStreamStatusTrace() }))
            produceMessages(stream, streamStatusTrackingRecordConsumer)
        } catch (e: Exception) {
            stream!!.airbyteStream.ifPresent { s: AirbyteStreamNameNamespacePair? -> LOGGER.error("Failed to consume from stream {}.", s, e) }
    ...
    companion object {
         ...
         */
        @VisibleForTesting
        fun stopOrphanedThreads(exitHook: Runnable,
            ...
            if (!runningThreads.isEmpty()) {
                LOGGER.warn("""
                  ...
                  The main thread is: {}
                  """.trimIndent(), dumpThread(currentThread))
                val scheduledExecutorService = Executors
                        .newSingleThreadScheduledExecutor(BasicThreadFactory.Builder() // this thread executor will create daemon threads, so it does not block exiting if all other active
                ...
                for (runningThread in runningThreads) {
                    ...
                    scheduledExecutorService.schedule({ runningThread.interrupt() }, interruptTimeDelay.toLong(), interruptTimeUnit)
                }
        ...
        @Throws(Exception::class)
        private fun validateConfig(schemaJson: JsonNode, objectJson: JsonNode, operationType: String) {
            val validationResult = validator.validate(schemaJson, objectJson)
            if (!validationResult.isEmpty()) {
                throw Exception(String.format("Verification error(s) occurred for %s. Errors: %s ",
        ...
        fun parseConnectorVersion(connectorImage: String?): String {
            ...
            val tokens = connectorImage.split(":".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()
