            ...
            try {
                ...
                if (stacktraceLines[i].contains("Database Error in model")) {
                    ...
                    if (stacktraceLines[i + 1].contains("SQL compilation error")) {
                        errorMessageAndType[ErrorMapKeys.ERROR_MAP_MESSAGE_KEY] = String.format("%s %s", stacktraceLines[i + 1].trim { it <= ' ' }, stacktraceLines[i + 2].trim { it <= ' ' })
                        ...
                        for (followingLine in Arrays.copyOfRange<String>(stacktraceLines, i + 1, stacktraceLines.size)) {
                            if (followingLine.trim { it <= ' ' }.startsWith("context:")) {
                                errorMessageAndType[ErrorMapKeys.ERROR_MAP_MESSAGE_KEY] = String.format("%s\n%s", stacktraceLines[i + 1].trim { it <= ' ' }, followingLine.trim { it <= ' ' })
                        ...
                        errorMessageAndType[ErrorMapKeys.ERROR_MAP_MESSAGE_KEY] = String.format("%s\n%s", stacktraceLines[i + 1].trim { it <= ' ' }, stacktraceLines[i + 2].trim { it <= ' ' })
                    ...
                    for (followingLine in Arrays.copyOfRange<String>(stacktraceLines, i + 1, stacktraceLines.size)) {
                        if ("Database Error" == followingLine.trim { it <= ' ' }) {
                            errorMessageAndType[ErrorMapKeys.ERROR_MAP_MESSAGE_KEY] = String.format("%s", stacktraceLines[Arrays.stream(stacktraceLines).toList().indexOf(followingLine) + 1].trim { it <= ' ' })
