...
object SSLCertificateUtils {
    ...
    private fun saveKeyStoreToFile(keyStore: KeyStore, keyStorePassword: String, filesystem: FileSystem?, directory: String?): URI {
        val fs = Objects.requireNonNullElse(filesystem, FileSystems.getDefault())
        val pathToStore = fs!!.getPath(Objects.toString(directory, ""))
        val pathToFile = pathToStore.resolve(KEYSTORE_FILE_NAME + RANDOM.nextInt() + KEYSTORE_FILE_TYPE)
    ...
    private fun fromPEMString(certString: String): Certificate {
        ...
        val byteArrayInputStream = ByteArrayInputStream(certString.toByteArray(StandardCharsets.UTF_8))
        val bufferedInputStream = BufferedInputStream(byteArrayInputStream)
        return cf.generateCertificate(bufferedInputStream)
    }

    @Throws(KeyStoreException::class, CertificateException::class, IOException::class, NoSuchAlgorithmException::class)
    fun keyStoreFromCertificate(cert: Certificate?,
    ...
    fun keyStoreFromCertificate(certString: String,
                                keyStorePassword: String,
                                filesystem: FileSystem?,
                                directory: String?): URI {
    ...
    fun keyStoreFromClientCertificate(
        ...
        val keyStore = KeyStore.getInstance(PKCS_12)
        keyStore.load(null)
        keyStore.setKeyEntry(KEYSTORE_ENTRY_PREFIX, key, keyStorePassword.toCharArray(), arrayOf(cert))
        return saveKeyStoreToFile(keyStore, keyStorePassword, filesystem, directory)
    }

    @Throws(IOException::class, InterruptedException::class, NoSuchAlgorithmException::class, InvalidKeySpecException::class, CertificateException::class, KeyStoreException::class)
    fun keyStoreFromClientCertificate(
            certString: String,
            keyString: String,
            keyStorePassword: String,
