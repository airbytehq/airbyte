...
class AirbyteExceptionHandler : Thread.UncaughtExceptionHandler {
    override fun uncaughtException(thread: Thread, throwable: Throwable) {
        ...
        val deinterpolatableException = ExceptionUtils.getThrowableList(throwable).stream()
                .filter { t: Throwable -> THROWABLES_TO_DEINTERPOLATE.stream().anyMatch { deinterpolatableClass: Class<out Throwable> -> deinterpolatableClass.isAssignableFrom(t.javaClass) } }
                .findFirst()
        ...
        if (deinterpolatableException.isPresent) {
            ...
            mangledMessage = STRINGS_TO_DEINTERPOLATE.stream() // Sort the strings longest to shortest, in case any target string is a substring of another
                    // e.g. "airbyte_internal" should be swapped out before "airbyte"
                    .sorted(Comparator.comparing { obj: String? -> obj!!.length }.reversed())
                    .reduce(originalMessage) { message: String?, targetString: String? -> deinterpolate(message, targetString) }
    ...
    companion object {
        ...
        const val logMessage: String = "Something went wrong in the connector. See the logs for more details."
         ...
         *
         *  1. Not contain the stacktrace at all. This causes Sentry to use its fallback grouping (using
         * exception class and message)
         *  1. Contain the original exception message as the external message, and a mangled message as the
         * internal message.
         *
         */
        @VisibleForTesting
