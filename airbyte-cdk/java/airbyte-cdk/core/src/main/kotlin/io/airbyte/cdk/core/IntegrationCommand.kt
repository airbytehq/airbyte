...
class IntegrationCommand : Runnable {
    ...
    @Inject lateinit var operations: List<Operation>
    ...
    private fun execute(command: String): Result<AirbyteMessage?> {
        return try {
            ...
            val operation: Operation? =
                operations.firstOrNull { o: Operation -> (operationType == o.type()) }
            operation?.execute()
                ?: Result.failure(
                    IntegrationCommandException(
            ...
            Result.failure(
                IntegrationCommandException(
                    "Connector does not support the '${command.lowercase()}' operation.",
                    e
                )
            )
        }
