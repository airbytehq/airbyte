...
import io.airbyte.cdk.core.destination.async.buffer.BufferDequeue
...
class DetectStreamToFlush(
    ...
    fun computeQueueThreshold(): Long {
        val isBuffer90Full: Boolean =
            EAGER_FLUSH_THRESHOLD <= bufferDequeue.getTotalGlobalQueueSizeBytes()
                .toDouble() / bufferDequeue.getMaxQueueSizeBytes()
        // when we are closing or queues are very full, flush regardless of how few items are in the queue.
        return if (flushAllStreams.get() || isBuffer90Full) 0 else destinationFlushFunction.getQueueFlushThresholdBytes()
    }
    ...
    fun getNextStreamToFlush(queueSizeThresholdBytes: Long): Optional<StreamDescriptor> {
        for (stream in orderStreamsByPriority(bufferDequeue.getBufferedStreams())) {
            ...
            val debugString = "trigger info: ${stream.namespace} - ${stream.name}, ${isTimeTriggeredResult.getRight()}, ${isSizeTriggeredResult.getRight()}"
            ...
            if (isSizeTriggeredResult.getLeft() || isTimeTriggeredResult.getLeft()) {
                ...
                latestFlushTimeMsPerStream[stream] = nowProvider.getOrDefault(Clock.systemUTC()).millis()
    ...
    fun isTimeTriggered(latestFlushTimeMs: Long): ImmutablePair<Boolean, String> {
        val timeSinceLastFlushMs = nowProvider.getOrDefault(Clock.systemUTC()).millis() - latestFlushTimeMs
    ...
    fun isSizeTriggered(
        ...
        val debugString = "size trigger: $isSizeTriggered current threshold b: ${airbyteFileUtils.byteCountToDisplaySize(
            queueSizeThresholdBytes,
        )}, queue size b: ${airbyteFileUtils.byteCountToDisplaySize(
            currentQueueSize,
        )}, penalty b: ${airbyteFileUtils.byteCountToDisplaySize(
            sizeOfRunningWorkersEstimate,
    ...
    fun estimateSizeOfRunningWorkers(
        ...
        val workersWithoutBatchesSizeEstimate =
            (
                min(
                    destinationFlushFunction.getOptimalBatchSizeBytes().toDouble(),
