...
class GlobalMemoryManager(val maxMemoryBytes: Long) {
    ...
    fun free(bytes: Long) {
        ...
        var newValue = currentMemoryBytes.addAndGet(-bytes)

        // Clamp to 0: negative values break the allocation gate in requestMemory(),
        // allowing unbounded buffering that leads to OOM.
        if (newValue < 0) {
            // CAS loop to safely clamp — another thread may have already corrected it.
            while (newValue < 0) {
                if (currentMemoryBytes.compareAndSet(newValue, 0L)) {
                    logger.warn {
                        "Freed more memory than allocated ($bytes bytes freed, but only " +
                            "${newValue + bytes} were tracked). Clamped to 0."
                    }
                    break
                }
                newValue = currentMemoryBytes.get()
            }
        }
    }
