...
/**
 * This [DestStateLifecycleManager] handles any state where the state messages are scoped by
 ...
 *
 */
class DestStreamStateLifecycleManager(private val defaultNamespace: String?) : DestStateLifecycleManager {
    ...
    override fun addState(message: AirbyteMessage) {
        Preconditions.checkArgument(message.state.type == AirbyteStateMessage.AirbyteStateType.STREAM)
        ...
        actualStreamId = if (namespace == null || namespace.isEmpty()) {
            ...
            StreamDescriptor()
                    .withName(originalStreamId.name)
    ...
    companion object {
        ...
        private fun listStatesInOrder(streamToState: Map<StreamDescriptor, AirbyteMessage>): Queue<AirbyteMessage?> {
            return streamToState
                    .entries
                    .stream() // typically, we support by namespace and then stream name, so we retain that pattern here.
                    .sorted(Comparator
                            .comparing<Map.Entry<StreamDescriptor, AirbyteMessage>, String>(
                                    { entry: Map.Entry<StreamDescriptor, AirbyteMessage> -> entry.key.namespace },
                                    Comparator.nullsFirst<String>(Comparator.naturalOrder<String>())) // namespace is allowed to be null
                            .thenComparing<String> { entry: Map.Entry<StreamDescriptor, AirbyteMessage> -> entry.key.name })
                    .map<AirbyteMessage> { obj: Map.Entry<StreamDescriptor, AirbyteMessage> -> obj.value }
                    .collect(Collectors.toCollection<AirbyteMessage, LinkedList<AirbyteMessage?>>(Supplier<LinkedList<AirbyteMessage?>> { LinkedList() }))
        }
         ...
         */
        private fun moveToNextPhase(prevPhase: MutableMap<StreamDescriptor, AirbyteMessage>, nextPhase: MutableMap<StreamDescriptor, AirbyteMessage>) {
