...
object DateTimeConverter {
    ...
    val TIME_WITH_TIMEZONE_FORMATTER: DateTimeFormatter = DateTimeFormatter.ofPattern(
    ...
    fun convertToTimeWithTimezone(time: Any): String {
        if (time is OffsetTime) {
            return if (hasZeroSecondsAndNanos(time.toLocalTime())) time.format(DataTypeUtils.TIMETZ_FORMATTER) else time.toString()
        } else {
            ...
            return if (hasZeroSecondsAndNanos(timetz.toLocalTime())) timetz.format(DataTypeUtils.TIMETZ_FORMATTER) else timetz.toString()
        }
    }
    ...
    fun convertToTimestampWithTimezone(timestamp: Any): String {
        if (timestamp is Timestamp) {
            ...
            return AbstractJdbcCompatibleSourceOperations.Companion.resolveEra(timestamp.toLocalDate(), timestamp.format(DataTypeUtils.TIMESTAMPTZ_FORMATTER))
            ...
            if (!loggedUnknownTimestampWithTimeZoneClass) {
                LOGGER.info("Unknown class for Timestamp with time zone data type" + timestamp.javaClass)
    ...
    /**
     * See [.convertToTimestampWithTimezone] for explanation of the weird things happening
     * here.
     */
    fun convertToTimestamp(timestamp: Any): String {
        if (timestamp is Timestamp) {
            ...
            return AbstractJdbcCompatibleSourceOperations.Companion.resolveEra(timestamp,
                    if (hasZeroSecondsAndNanos(localDateTime.toLocalTime())) localDateTime.format(DataTypeUtils.TIMESTAMP_FORMATTER) else localDateTime.toString())
        } else if (timestamp is Instant) {
            // Incremental mode
            return AbstractJdbcCompatibleSourceOperations.Companion.resolveEra(timestamp.atZone(ZoneOffset.UTC).toLocalDate(), timestamp.atOffset(ZoneOffset.UTC).toLocalDateTime().format(DataTypeUtils.TIMESTAMP_FORMATTER))
        } else if (timestamp is LocalDateTime) {
            val date: LocalDate = timestamp.toLocalDate()
            return AbstractJdbcCompatibleSourceOperations.Companion.resolveEra(date,
                    if (hasZeroSecondsAndNanos(timestamp.toLocalTime())) timestamp.format(DataTypeUtils.TIMESTAMP_FORMATTER) else timestamp.toString())
        } else {
    ...
    fun convertToDate(date: Any): String {
        if (date is Date) {
            ...
            return AbstractJdbcCompatibleSourceOperations.Companion.resolveEra(date, localDate.format(DataTypeUtils.DATE_FORMATTER))
        } else if (date is LocalDate) {
            // Incremental mode
            return AbstractJdbcCompatibleSourceOperations.Companion.resolveEra(date, date.format(DataTypeUtils.DATE_FORMATTER))
        } else {
            ...
            return AbstractJdbcCompatibleSourceOperations.Companion.resolveEra(localDate, localDate.format(DataTypeUtils.DATE_FORMATTER))
        }
    }
    ...
    fun convertToTime(time: Any): String {
        if (time is Time) {
            ...
            if (value >= 0 && value < TimeUnit.DAYS.toNanos(1)) {
                ...
                val updatedValue = min(abs(value.toDouble()), LocalTime.MAX.toNanoOfDay().toDouble()).toLong()
    ...
    private fun formatTime(localTime: LocalTime): String {
        return if (hasZeroSecondsAndNanos(localTime)) localTime.format(DataTypeUtils.TIME_FORMATTER) else localTime.toString()
    }
