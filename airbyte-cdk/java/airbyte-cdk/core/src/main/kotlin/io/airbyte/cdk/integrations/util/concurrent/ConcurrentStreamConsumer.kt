...
/**
 * [Consumer] implementation that consumes [AirbyteMessage] records from each provided
 * stream concurrently.
 *
 *
 *
 ...
 */
class ConcurrentStreamConsumer(streamConsumer: Consumer<AutoCloseableIterator<AirbyteMessage?>?>, requestedParallelism: Int) : Consumer<Collection<AutoCloseableIterator<AirbyteMessage?>?>>, AutoCloseable {
    ...
    private val streamStatusEmitter = Optional.of(Consumer { obj: AirbyteStreamStatusHolder? -> AirbyteTraceMessageUtility.emitStreamStatusTrace() })

    /**
     * Constructs a new [ConcurrentStreamConsumer] that will use the provided stream consumer to
     * execute each stream submitted to the [&lt;][.accept] method of
     * this consumer. Streams submitted to the [&lt;][.accept] method
     * will be converted to a [Runnable] and executed on an [ExecutorService] configured by
     * this consumer to ensure concurrent execution of each stream.
     *
     * @param streamConsumer The [Consumer] that accepts streams as an
     ...
     */
        val futures: Collection<CompletableFuture<Void>> = streams.stream()
                .map { stream: AutoCloseableIterator<AirbyteMessage?>? -> ConcurrentStreamRunnable(stream, this) }
                .map { runnable: ConcurrentStreamRunnable? -> CompletableFuture.runAsync(runnable, executorService) }
     ...
     */
        val defaultPoolSize = Optional.ofNullable(System.getenv("DEFAULT_CONCURRENT_STREAM_CONSUMER_THREADS"))
                .map { s: String -> s.toInt() }
                .orElseGet { Runtime.getRuntime().availableProcessors() }
        LOGGER.debug("Default parallelism: {}, Requested parallelism: {}", defaultPoolSize, requestedParallelism)
        val parallelism = min(defaultPoolSize.toDouble(), (if (requestedParallelism > 0) requestedParallelism else 1).toDouble()).toInt()
        LOGGER.debug("Computed concurrent stream consumer parallelism: {}", parallelism)
        return parallelism
    }

    /**
     * Creates the [ExecutorService] that will be used by the consumer to consume from the
    ...
    private fun createExecutorService(nThreads: Int): ExecutorService {
        return ThreadPoolExecutor(nThreads, nThreads, 0L, TimeUnit.MILLISECONDS, LinkedBlockingQueue(),
                ConcurrentStreamThreadFactory(), AbortPolicy())
    }
    ...
    private fun executeStream(stream: AutoCloseableIterator<AirbyteMessage?>?) {
        try {
            stream.use {
                stream!!.airbyteStream.ifPresent { s: AirbyteStreamNameNamespacePair? -> LOGGER.debug("Consuming from stream {}...", s) }
                ...
                stream.airbyteStream.ifPresent { s: AirbyteStreamNameNamespacePair? -> LOGGER.debug("Consumption from stream {} complete.", s) }
            }
        } catch (e: Exception) {
            stream!!.airbyteStream.ifPresent { s: AirbyteStreamNameNamespacePair? -> LOGGER.error("Unable to consume from stream {}.", s, e) }
        ...
        override fun newThread(r: Runnable): Thread {
            ...
            if (r is ConcurrentStreamRunnable) {
                ...
                if (stream!!.airbyteStream.isPresent) {
                    ...
                    thread.name = String.format("%s-%s-%s", CONCURRENT_STREAM_THREAD_NAME, airbyteStream.namespace, airbyteStream.name)
                } else {
     ...
     */
    private class ConcurrentStreamRunnable(stream: AutoCloseableIterator<AirbyteMessage?>?, consumer: ConcurrentStreamConsumer) : Runnable {
        override fun run() {
