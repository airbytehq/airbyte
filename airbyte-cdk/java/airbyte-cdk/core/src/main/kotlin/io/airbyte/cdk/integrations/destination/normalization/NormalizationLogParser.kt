 ...
 * [io.airbyte.workers.normalization.NormalizationAirbyteStreamFactory]. That class is not
    ...
    companion object {
        ...
        fun main(args: Array<String>) {
            ...
            val airbyteMessageStream =
                    normalizationLogParser.create(BufferedReader(InputStreamReader(System.`in`, StandardCharsets.UTF_8)))
            airbyteMessageStream.forEachOrdered { message: AirbyteMessage? -> println(Jsons.serialize(message)) }

            val errors = normalizationLogParser.getDbtErrors()
            val dbtErrorStack = java.lang.String.join("\n", errors)
            if ("" != dbtErrorStack) {
                val errorMap = SentryExceptionHelper.getUsefulErrorMessageAndTypeFromDbtError(dbtErrorStack)
                val internalMessage = errorMap!![SentryExceptionHelper.ErrorMapKeys.ERROR_MAP_MESSAGE_KEY]
                val traceMessage = AirbyteMessage()
