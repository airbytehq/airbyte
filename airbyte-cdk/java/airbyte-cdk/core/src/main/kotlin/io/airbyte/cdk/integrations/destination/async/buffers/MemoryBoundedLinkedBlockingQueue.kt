 ...
 * amount of memory it is allowed to use can be resized after it is instantiated. <p> This class
 * intentionally hides the underlying queue inside of it. For this class to work, it has to override
 * each method on a queue that adds or removes records from the queue. The Queue interface has a lot
 * of methods to override, and we don't want to spend the time overriding a lot of methods that
 * won't be used. By hiding the queue, we avoid someone accidentally using a queue method that has
 * not been modified. If you need access to another of the queue methods, pattern match adding the
 * memory tracking as seen in {@link HiddenQueue}, and then delegate to that method from this
 * top-level class.
 *
