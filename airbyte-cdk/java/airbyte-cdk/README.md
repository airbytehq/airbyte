...
|:-----------|:-----------|:------------------------------------------------------------|:---------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 0.48.19    | 2026-09-16 | [\#81456](https://github.com/airbytehq/airbyte/pull/81456)  | Allow connectors to override the Debezium `max.batch.size`/`max.queue.size`/`max.queue.size.in.bytes` defaults. Cache the parsed Debezium event in `ChangeEventWithMetadata` instead of re-deserializing it on every read. Report estimated queue bytes and heap use in the CDC queue stats log line, and rate-limit the MongoDB change-stream prefetch buffer warning. |
