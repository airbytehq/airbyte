...
Install the CDK project into a dedicated virtual environment.
...
* Run tests via `poetry run pytest -s unit_tests`
* Perform static type checks using `poetry run mypy airbyte_cdk`. `MyPy` configuration is in `mypy.ini`.
 * Run `poetry run mypy <files to check>` to only check specific files. This is useful as the CDK still contains code that is not compliant.
