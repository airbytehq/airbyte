...
def test_deriving_schema_nested_structures():
    inferrer = SchemaInferrer()
    inferrer.accumulate(AirbyteRecordMessage(stream="my_stream", data={"obj": { "data": [1.0, 2.0, 3.0]}}, emitted_at=NOW))
    inferrer.accumulate(AirbyteRecordMessage(stream="my_stream", data={"obj": { "other_key": "xyz" }}, emitted_at=NOW))
    assert json.dumps(inferrer.get_inferred_schemas()["my_stream"]) == '{"$schema": "http://json-schema.org/schema#", "type": "object", "properties": {"obj": {"type": "object", "properties": {"data": {"type": "array", "items": {"type": "number"}}, "other_key": {"type": "string"}}}}}'
