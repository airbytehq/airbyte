...
from airbyte_protocol.models import ConfiguredAirbyteCatalog, AirbyteMessage, Type, AirbyteRecordMessage, AirbyteLogMessage, Level, \
    AirbyteStateMessage, AirbyteTraceMessage, TraceType, AirbyteAnalyticsTraceMessage, AirbyteErrorTraceMessage
...
class EntrypointWrapperTest(TestCase):
    ...
    @patch("airbyte_cdk.test.entrypoint_wrapper.AirbyteEntrypoint")
    def test_given_error_message_when_read_then_output_has_trace_error(self, entrypoint):
        entrypoint.return_value.run.return_value = _to_entrypoint_output([_AN_ERROR_MESSAGE])
        output = read(self._a_source, _A_CONFIG, _A_CATALOG, _A_STATE)
        assert output.errors == [_AN_ERROR_MESSAGE]
    ...
    def test_given_trace_message_when_read_then_output_has_trace_messages(self, entrypoint):
        ...
        output = read(self._a_source, _A_CONFIG, _A_CATALOG, _A_STATE)
        assert output.analytics == [_AN_ANALYTIC_MESSAGE]
