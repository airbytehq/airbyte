...
def test_get_grouped_messages_record_limit(mock_entrypoint_read: Mock, request_record_limit: int, max_record_limit: int) -> None:
    ...
    single_slice = actual_response.slices[0]
    total_records = 0
    for _i, actual_page in enumerate(single_slice.pages):
...
def test_create_response_from_log_message(log_message: str, expected_response: HttpResponse) -> None:
    response_message = json.loads(log_message) if isinstance(log_message, str) else log_message
...
def test_given_multiple_control_messages_with_same_timestamp_then_stream_read_has_latest_based_on_message_order(mock_entrypoint_read: Mock) -> None:
    ...
    latest_config = {"latest": 1}
    mock_source = make_mock_source(mock_entrypoint_read, iter(
        [*any_request_and_response_with_a_record(), connector_configuration_control_message(emitted_at, earliest_config), connector_configuration_control_message(emitted_at, latest_config)],
    ),
                                   )
    connector_builder_handler = MessageGrouper(MAX_PAGES_PER_SLICE, MAX_SLICES)
    stream_read: StreamRead = connector_builder_handler.get_message_groups(
...
def test_given_auxiliary_requests_then_return_auxiliary_request(mock_entrypoint_read: Mock) -> None:
    mock_source = make_mock_source(mock_entrypoint_read, iter(
        [*any_request_and_response_with_a_record(), auxiliary_request_log_message()],
    ))
    connector_builder_handler = MessageGrouper(MAX_PAGES_PER_SLICE, MAX_SLICES)
    stream_read: StreamRead = connector_builder_handler.get_message_groups(
        source=mock_source, config=CONFIG, configured_catalog=create_configured_catalog("hashiras"),
