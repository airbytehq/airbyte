...
def test_read():
    ...
    with patch("airbyte_cdk.connector_builder.message_grouper.MessageGrouper.get_message_groups", return_value=stream_read) as mock:
        output_record = handle_connector_builder_request(
            source, "test_read", config, ConfiguredAirbyteCatalog.model_validate(CONFIGURED_CATALOG), _A_STATE, limits
        )
        mock.assert_called_with(source, config, ConfiguredAirbyteCatalog.model_validate(CONFIGURED_CATALOG), _A_STATE, limits.max_records)
...
def test_config_update():
    ...
    with patch(
        ...
        output = handle_connector_builder_request(
            source, "test_read", config, ConfiguredAirbyteCatalog.model_validate(CONFIGURED_CATALOG), _A_STATE, TestReadLimits()
        )
...
def test_read_returns_error_response(mock_from_exception):
    ...
    source = MockManifestDeclarativeSource()
    limits = TestReadLimits()
    response = read_stream(source, TEST_READ_CONFIG, ConfiguredAirbyteCatalog.model_validate(CONFIGURED_CATALOG), _A_STATE, limits)
