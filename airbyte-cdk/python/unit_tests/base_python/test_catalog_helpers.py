...
from airbyte_cdk.base_python import CatalogHelper


def test_coerce_catalog_as_full_refresh():
    incremental = AirbyteStream(name="1", json_schema={"k": "v"}, supported_sync_modes=[SyncMode.incremental, SyncMode.full_refresh],
                                source_defined_cursor=True, default_cursor_field=["cursor"])
    ...
    expected = AirbyteCatalog(streams=[
