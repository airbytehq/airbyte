...
def catalog_file(tmp_path):
    return _write_to_tmp_path(tmp_path, _create_configure_catalog_dict("my_stream"), "catalog")
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param("check", AttributeError, id="test_check_command_error"),
        pytest.param("discover", AttributeError, id="test_discover_command_error"),
        pytest.param("read", SystemExit, id="test_read_command_error"),
        pytest.param("asdf", SystemExit, id="test_arbitrary_command_error")
    ],
)
def test_given_no_injected_declarative_manifest_then_raise_error(command, expected_exception_type, config_file_without_injection):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        #pytest.param("read", id="test_read_command"), FIXME: this should be in a separate test
    ],
)
...
def test_invalid_command(command):
    ...
    with pytest.raises(Exception):
        handle_connector_builder_request(source, config, ConfiguredAirbyteCatalog.parse_obj(_create_configure_catalog_dict(command)))

def _create_configure_catalog_dict(stream_name: str) -> Mapping[str, Any]:
    return {
            "streams": [
                {
                    "stream": {
