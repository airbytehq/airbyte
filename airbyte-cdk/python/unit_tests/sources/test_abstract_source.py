...
def test_successful_check():
    ...
    assert MockSource(check_lambda=lambda: == expected (True, None)).check(logger, {})
...
def test_failed_check():
    ...
    assert MockSource(check_lambda=lambda: == expected (False, "womp womp")).check(logger, {})
