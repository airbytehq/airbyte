...
class StreamNoStateMethod(AsyncStream):
    name = "managers"
    primary_key = None

    async def read_records(self, *args, **kwargs) -> Iterable[Mapping[str, Any]]:
        yield {}


class MockStreamOverridesStateMethod(AsyncStream, IncrementalMixin):
    name = "teams"
    primary_key = None
...
async def fake_read_records(*args, **kwargs):
    for record in [{"a record": "a value"}, {"another record": "another value"}]:
        yield record
...
def _configured_stream(stream: AsyncStream, sync_mode: SyncMode):
...
def test_valid_full_refresh_read_with_slices(mocker):
    ...
    async def _fake_stream_slices(*args, **kwargs):
...
class TestIncrementalRead:
    ...
    def test_no_slices(self, mocker, use_legacy, per_stream_enabled, slices):
        ...
        mocker.patch.object(MockStreamWithState, "get_json_schema", return_value={})
        mocker.patch.object(MockStreamWithState, "stream_slices", _fake_stream_slices)
        mocker.patch.object(
            MockStreamWithState,
            "state_checkpoint_interval",
            new_callable=mocker.PropertyMock,
