...
@pytest.mark.parametrize("flush", [0, 0.01])
class Test:
    ...
    def test_external_request_source(capsys, flush, deployment_mode, url_base, expected_records, expected_error):
        ...
        with mock.patch.dict(os.environ, {"DEPLOYMENT_MODE": deployment_mode}, clear=False):  # clear=True clears the existing os.environ dict
            with mock.patch.object(HttpTestStream, "url_base", url_base):
                args = ["read", "--config", "config.json", "--catalog", "configured_catalog.json", "--flush", str(flush)]
                if expected_error:
    ...
    @patch.object(requests.Session, "send", fixture_mock_send)
    def test_external_oauth_request_source(capsys, flush, deployment_mode, token_refresh_url, expected_records, expected_error):
