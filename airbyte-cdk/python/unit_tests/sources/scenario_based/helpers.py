...
def verify_spec(scenario: TestScenario[AbstractSource]) -> None:
    assert spec(scenario) == scenario.expected_spec


def verify_check(tmp_path: PosixPath, scenario: TestScenario[AbstractSource]) -> None:
    expected_exc, expected_msg = scenario.expected_check_error

    if expected_exc:
        with pytest.raises(expected_exc):
            output = check(tmp_path, scenario)
            if expected_msg:
...
def spec(scenario: TestScenario[AbstractSource]) -> Mapping[str, Any]:
    output = get_source_iter(
        scenario.source,
        ["spec"],
    )
...
def check(tmp_path: PosixPath, scenario: TestScenario[AbstractSource]) -> Dict[str, Any]:
    output = get_source_iter(
        scenario.source,
        ["check", "--config", make_file(tmp_path / "config.json", scenario.config)],
    )
    return json.loads(next(output))["connectionStatus"]  # type: ignore


def discover(tmp_path: PosixPath, scenario: TestScenario[AbstractSource]) -> Dict[str, Any]:
    output = [json.loads(o) for o in get_source_iter(
        scenario.source,
