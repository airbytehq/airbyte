...
class BaseConnector(ABC, Generic[TConfig]):
    ...
    def read_config(config_path: str) -> Mapping[str, Any]:
        ...
        if isinstance(config, Mapping):
            ...
            msg = f"The content of {config_path} is not an object and therefore is not a valid config. Please ensure the file represent a config."
            raise ValueError(
                msg,
            )
    ...
    def _read_json_file(file_path: str) -> Union[None, bool, float, int, str, List[Any], Mapping[str, Any]]:
        ...
        try:
            ...
            msg = f"Could not read json file {file_path}: {error}. Please ensure that it is a valid JSON."
    ...
    def spec(self, logger: logging.Logger) -> ConnectorSpecification:
        ...
        if yaml_spec and json_spec:
            msg = "Found multiple spec files in the package. Only one of spec.yaml or spec.json should be provided."
        ...
        if yaml_spec:
            ...
            try:
                ...
                msg = f"Could not read json spec file: {error}. Please ensure that it is a valid JSON."
                raise ValueError(msg)
        else:
            msg = "Unable to find spec.yaml or spec.json in the package."
