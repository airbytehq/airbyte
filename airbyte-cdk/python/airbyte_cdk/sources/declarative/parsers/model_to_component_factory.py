...
class ModelToComponentFactory:
    ...
    def _get_class_from_fully_qualified_class_name(self, full_qualified_class_name: str) -> Any:
        ...
        try:
            ...
            # Refine exception handling to pinpoint import errors
            raise ValueError(f"Could not load class {full_qualified_class_name}. Make sure the module and class names are correct.")
        except ImportError:
            raise ValueError(f"Could not import module {module}. Verify the module path and ensure it is installed.")
    ...
    def _get_connector_module() -> Any:
        ...
        # Utilize cache to avoid repeated directory scans
        if not hasattr(ModelToComponentFactory, "_cached_connector_module"):
            for item in os.scandir(main_dir):
                if item.is_dir() and re.match(r"source_.*", item.name):
                    ModelToComponentFactory._cached_connector_module = item.name
                    return item.name
            ModelToComponentFactory._cached_connector_module = None
        return ModelToComponentFactory._cached_connector_module
