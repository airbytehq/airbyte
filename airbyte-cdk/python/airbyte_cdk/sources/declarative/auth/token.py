...
class BasicHttpAuthenticator(DeclarativeAuthenticator):
    ...
    @property
    def token(self) -> str:
        auth_string = f"{self._username.eval(self.config)}:{self._password.eval(self.config)}".encode()
...
def get_new_session_token(api_url: str, username: str, password: str, response_key: str) -> str:
    ...
    if not response.ok:
        msg = f"Failed to retrieve new session token, response code {response.status_code} because {response.reason}"
...
class LegacySessionTokenAuthenticator(DeclarativeAuthenticator):
    ...
    def token(self) -> str:
        ...
        msg = "Invalid credentials: session token is not valid or provide username and password"
        raise ConnectionError(msg)
    ...
    def is_valid_session_token(self) -> bool:
        try:
            ...
            if e.response.status_code == requests.codes["unauthorized"]:
                ...
                msg = f"Error while validating session token: {e}"
                raise ConnectionError(msg)
        if response.ok:
