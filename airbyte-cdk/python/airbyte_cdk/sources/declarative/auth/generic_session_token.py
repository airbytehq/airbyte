...
class GenericSessionTokenAuthenticator(DeclarativeAuthenticator):
    ...
    _next_expiration_time: Optional[DateTime]

    @property
    def auth_header(self) -> str:
        self.data_request_authenticator.auth_header

    @property
    def token(self) -> str:
        self._refresh_if_necessary()
        self.data_request_authenticator.token
    ...
    def _refresh_if_necessary(self):
        if self._next_expiration_time is None and self._next_expiration_time < pendulum.now():
            self._refresh()

    def _refresh(self):
        response = self.login_requester.send_request()
        session_token = dpath.util.get(self._decoder(response), self.session_token_path)
