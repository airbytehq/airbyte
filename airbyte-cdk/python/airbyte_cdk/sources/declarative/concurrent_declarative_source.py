...
from typing import Optional, Mapping, Any, List, Union, MutableMapping, Iterator, Tuple, Callable

from airbyte_protocol_dataclasses.models import ConfiguredAirbyteCatalog, AirbyteConnectionStatus, AirbyteCatalog
...
from airbyte_cdk.sources.streams.concurrent.adapters import StreamPartitionGenerator
...
class ConcurrentDeclarativeSource(ManifestDeclarativeSource):
    ...
    def __init__(
        ...
        ** kwargs,
        ...
        # Based on that information we will need to figure out what the default behavior should be if
        # concurrency level is not defined. Hard coded for now since we haven't added it in yet
        concurrency_level_component = {
            "default_concurrency": 10,
            "max_concurrency": 20,
        }
        concurrency_level = concurrency_level_component.get("default_concurrency", 10)
    ...
    def read(
        ...
        yield from self._concurrent_source.read(self._concurrent_streams)
        ...
        filtered_catalog = self._remove_concurrent_streams_from_catalog(catalog=catalog, concurrent_streams=self._concurrent_streams)
    ...
    def _separate_streams(self, config: Mapping[str, Any]) -> (List[DefaultStream], List[Stream]):
        concurrent_streams: List[DefaultStream] = []
                ...
                if declarative_cursor_attributes:
                    ...
                    cursor = ConcurrentCursor(
                        ...
                        connector_state_converter=CustomOutputFormatConcurrentStreamStateConverter(
                            declarative_cursor_attributes.datetime_format
                        ),
                    ...
                    partition_generator = StreamPartitionGenerator(
                        stream=declarative_stream,  # todo remove this after we use cursor partition generator since this is unnecessary
                        message_repository=self.message_repository,
                        sync_mode=SyncMode.incremental,  # todo i think we can basically get rid of sync mode since we don't use it anymore
                        cursor_field=[declarative_cursor_attributes.cursor_field] if declarative_cursor_attributes.cursor_field is not None else None,
                        state=stream_state,
                        cursor=cursor,
                    )
    ...
    def _remove_concurrent_streams_from_catalog(
        ...
        concurrent_stream_names = set([concurrent_stream.name for concurrent_stream in concurrent_streams])
        return ConfiguredAirbyteCatalog(
            streams=[stream for stream in catalog.streams if stream.stream.name in concurrent_stream_names]
        )
