...
class DatetimeStreamSlicer(StreamSlicer):
    ...
    def __post_init__(self, parameters: Mapping[str, Any]):
        ...
        self.stream_state_field_start = InterpolatedString.create(self.stream_state_field_start or "start_time", parameters=parameters)
        self.stream_state_field_end = InterpolatedString.create(self.stream_state_field_end or "end_time", parameters=parameters)
    ...
    def update_cursor(self, stream_slice: StreamSlice, last_record: Optional[Record] = None):
        ...
        stream_slice_value_end = stream_slice.get(self.stream_state_field_end.eval(self.config))
        ...
        if self._cursor and cursor:
            ...
            self._cursor = cursor
        if self.stream_state_field_end:
    ...
    def _partition_daterange(self, start: datetime.datetime, end: datetime.datetime, step: Union[datetime.timedelta, Duration]):
        start_field = self.stream_state_field_start.eval(self.config)
        end_field = self.stream_state_field_end.eval(self.config)
        dates = []
        while start <= end:
    ...
    def _get_request_options(self, option_type: RequestOptionType, stream_slice: StreamSlice):
        ...
        if self.start_time_option and self.start_time_option.inject_into == option_type:
            options[self.start_time_option.field_name] = stream_slice.get(self.stream_state_field_start.eval(self.config))
        if self.end_time_option and self.end_time_option.inject_into == option_type:
            options[self.end_time_option.field_name] = stream_slice.get(self.stream_state_field_end.eval(self.config))
        return options
