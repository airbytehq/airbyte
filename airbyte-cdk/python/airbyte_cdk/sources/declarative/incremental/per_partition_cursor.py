...
from airbyte_cdk.sources.declarative.types import DeclarativeStreamSlice, Record, StreamSlice, StreamState
...
class PerPartitionCursor(Cursor):
    ...
    def stream_slices(self) -> Iterable[DeclarativeStreamSlice]:
        ...
        for partition in slices:
            ...
            for cursor_slice in cursor.stream_slices():
                yield DeclarativeStreamSlice(partition, cursor_slice)
    ...
    def close_slice(self, stream_slice: DeclarativeStreamSlice, most_recent_record: Optional[Record]) -> None:
        try:
            cursor_most_recent_record = (
                Record(most_recent_record.data, DeclarativeStreamSlice({}, stream_slice.cursor_slice))
                if most_recent_record
                else most_recent_record
            )
            self._cursor_per_partition[self._to_partition_key(stream_slice.partition)].close_slice(
                DeclarativeStreamSlice({}, stream_slice.cursor_slice), cursor_most_recent_record
            )
    ...
    def _to_dict(self, partition_key: str) -> StreamSlice:
        return self._partition_serializer.to_partition(partition_key)

    def select_state(self, stream_slice: Optional[DeclarativeStreamSlice] = None) -> Optional[StreamState]:
        if not stream_slice:
        ...
        *,
        ...
        if stream_slice:
            return self._partition_router.get_request_params(  # type: ignore # this always returns a mapping
                stream_state=stream_state, stream_slice=DeclarativeStreamSlice(stream_slice.partition, {}), next_page_token=next_page_token
            ) | self._cursor_per_partition[self._to_partition_key(stream_slice.partition)].get_request_params(
                stream_state=stream_state,
                stream_slice=DeclarativeStreamSlice({}, stream_slice.cursor_slice),
    ...
    def _convert_record_to_cursor_record(record: Record) -> Record:
        return Record(record.data, DeclarativeStreamSlice({}, record.associated_slice.cursor_slice) if record.associated_slice else None)
