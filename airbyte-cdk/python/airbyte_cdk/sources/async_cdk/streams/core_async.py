...
class AsyncStream(Stream, ABC):
    ...
    async def read_incremental(  # type: ignore  # ignoring typing for ConnectorStateManager because of circular dependencies
        ...
        logger.debug(
            f"Processing stream slices for {self.name} (sync_mode: incremental)",
    ...
    async def check_availability(
        self, logger: logging.Logger, source: Optional["AsyncAbstractSource"] = None
        ...
        if self.availability_strategy:
            return await self.availability_strategy.check_availability(
                self, logger, source
            )
        return True, None
