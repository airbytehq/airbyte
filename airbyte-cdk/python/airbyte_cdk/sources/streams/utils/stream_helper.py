...
def get_first_stream_slice(
    ...
    sync_mode = SyncMode.full_refresh if not stream_state else SyncMode.incremental
    slices = iter(stream.stream_slices(cursor_field=stream.cursor_field, sync_mode=sync_mode, stream_state=stream_state))
    return next(slices)


def get_first_record_for_slice(
    stream: Stream, stream_slice: Optional[Mapping[str, Any]], stream_state: Optional[Mapping[str, Any]] = None
) -> StreamData:
    """
