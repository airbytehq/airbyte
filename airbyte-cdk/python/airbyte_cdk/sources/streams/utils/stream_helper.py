...
def get_first_record_for_slice(stream: Stream, stream_slice: Optional[Mapping[str, Any]]) -> StreamData:
    ...
    try:
        ...
        if stream_has_setter:
            stream.exit_on_rate_limit(True)  # type: ignore[attr-defined] # we check for the setter method using stream_has_setter
        ...
        if stream_has_setter:
            stream.exit_on_rate_limit(original_exit_on_rate_limit)  # type: ignore[attr-defined] # we check for the setter method using stream_has_setter
