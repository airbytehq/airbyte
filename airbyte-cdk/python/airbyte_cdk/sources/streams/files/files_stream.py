...
from .formats import AbstractFileParser, AvroParser, CsvParser, ParquetParser
...
class FilesStream(Stream, ABC):
    ...
    def file_iterator(self) -> Iterator[FileInfo]:
        """
        Provider-specific method to iterate through bucket/container/etc. and yield a FileInfo for each file.
        This supplies the 'FileInfo' to use in StorageFile(), which has all file properties (last_modified, key, size).
        All these meta options are saved during loading of files' list.

        :yield: FileInfo object to use in StorageFile()
        """

    def pattern_matched_file_iterator(self, file_infos: Iterable[FileInfo]) -> Iterator[FileInfo]:
        """
        iterates through file_infos and yields only those file_infos that match user-provided path patterns

        :param file_infos: typically the output of file_iterator()
    ...
    def get_time_ordered_file_infos(self) -> List[FileInfo]:
        ...
        """
        return sorted(self.pattern_matched_file_iterator(self.file_iterator()), key=lambda file_info: file_info.last_modified)
...
class IncrementalFilesStream(FilesStream, ABC):
