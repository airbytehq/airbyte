...
from airbyte_cdk.sources.streams.concurrent.default_stream_async import AsyncDefaultStream
...
class AsyncStreamFacade(AsyncStream):
    ...
    def create_from_stream(
        ...
        return AsyncStreamFacade(
            AsyncDefaultStream(
                partition_generator=AsyncStreamPartitionGenerator(
                    stream,
                    message_repository,
                ...
                availability_strategy=AsyncStreamAvailabilityStrategy(stream, source),
                primary_key=pk,
...
class AsyncStreamPartition(Partition):
    """
...
class AsyncStreamPartitionGenerator(PartitionGenerator):
    """
    ...
    async def generate(self) -> Iterable[Partition]:
        async for s in self._stream.stream_slices(sync_mode=self._sync_mode, cursor_field=self._cursor_field, stream_state=self._state):
            yield AsyncStreamPartition(
                self._stream, copy.deepcopy(s), self.message_repository, self._sync_mode, self._cursor_field, self._state, self._cursor
            )


@deprecated("This class is experimental. Use at your own risk.")
class AsyncAvailabilityStrategyFacade(AvailabilityStrategy):
