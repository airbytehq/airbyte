...
from airbyte_cdk.sources.streams.call_rate import CallRateLimitHit, Rate
...
class HttpRequestMatcher(RequestMatcher):
    ...
    def __call__(self, request: Any) -> bool:
        ...
        :return: True if matches the provided request object, False - otherwise
        """
        # if isinstance(request, requests.Request):  # TODO: do I need to handle a non-"prepared" request?
        #     prepared_request = request.prepare()
        if isinstance(request, aiohttp.ClientRequest):
...
class HttpAPIBudget(APIBudget):
    ...
    def get_reset_ts_from_response(self, response: aiohttp.ClientResponse) -> Optional[datetime.datetime]:
    ...
    def get_calls_left_from_response(self, response: aiohttp.ClientResponse) -> Optional[int]:
        if response.headers.get(self._ratelimit_remaining_header):
            return int(response.headers[self._ratelimit_remaining_header])

        if response.status in self._status_codes_for_ratelimit_hit:
            return 0
