...
class FileBasedDiscoverErrorHandler(AbstractDiscoverErrorHandler):
    ...
    def __init__(
        self, exceptions_to_log: List[type[Exception]], underlying_exceptions_to_log: List[Union[FileBasedSourceError, str]]
    ) -> None:
        self._exceptions_to_log = exceptions_to_log or []
        self._underlying_exceptions_to_log = [
            exc.value if isinstance(exc, FileBasedSourceError) else exc for exc in underlying_exceptions_to_log
        ] or []
