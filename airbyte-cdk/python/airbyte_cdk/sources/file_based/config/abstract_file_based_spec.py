...
class DeliverRawFiles(BaseModel):
    class Config(OneOfOptionConfig):
        ...
        description = "Copy raw files to S3 without parsing their contents. Bits are copied into the destination exactly as they appeared in the source. Recommended for use with unstructured text data, non-text and compressed files.\n\nRequires selection of a supporting destination. See docs for specific requirements and limitations."
        discriminator = "delivery_type"
