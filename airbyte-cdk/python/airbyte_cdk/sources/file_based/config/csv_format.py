...
class CsvFormat(BaseModel):
    ...
    header_definition_type: CsvHeaderDefinitionType = Field(
    ...
    user_provided_column_names: Optional[List[str]] = Field(
        title="User Provided Column Names",
        description="Given header definition type is user-provided, column names will be provided using this field.",
        default=[],
        order=101,
    )
    ...
    def csv_has_header_row(self) -> bool:
        return self.header_definition_type == CsvHeaderDefinitionType.FROM_CSV
    ...
    def validate_optional_args(cls, values: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if definition_type == CsvHeaderDefinitionType.USER_PROVIDED and not column_names:
            raise ValidationError(
                "`user_provided_column_names` should be defined if the definition 'User Provided'.", model=CsvFormat
            )
