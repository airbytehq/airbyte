...
from airbyte_cdk.sources.file_based.config.csv_format import CsvFormat, CsvHeaderDefinitionType, InferenceType
...
class _CsvReader:
    def read_data(
        ...
        with stream_reader.open_file(file, file_read_mode, config_format.encoding, logger) as fp:
            ...
            rows_to_skip = (
                ...
                + (1 if config_format.csv_has_header_row() else 0)
    ...
    def _get_headers(self, fp: IOBase, config_format: CsvFormat, dialect_name: str) -> List[str]:
        ...
        if config_format.header_definition_type == CsvHeaderDefinitionType.USER_PROVIDED:
            return config_format.user_provided_column_names

        self._skip_rows(fp, config_format.skip_rows_before_header)
        if config_format.header_definition_type == CsvHeaderDefinitionType.AUTOGENERATED:
