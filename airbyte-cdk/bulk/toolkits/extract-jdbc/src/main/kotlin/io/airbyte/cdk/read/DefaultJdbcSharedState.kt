...
class DefaultJdbcSharedState(
    ...
    @Value("\${$PROPERTY_PREFIX.jdbc.withSampling:$WITH_SAMPLING}")
    override val withSampling: Boolean,
    @Value("\${$PROPERTY_PREFIX.jdbc.tableSampleSize:$TABLE_SAMPLE_SIZE}")
    override val maxSampleSize: Int,
    /** How many bytes per second we can expect the database to send to the connector. */
    @Value("\${$PROPERTY_PREFIX.jdbc.throughputBytesPerSecond:$THROUGHPUT_BYTES_PER_SECOND}")
    val expectedThroughputBytesPerSecond: Long,
    /** Smallest possible fetchSize value. */
    @Value("\${$PROPERTY_PREFIX.jdbc.minFetchSize:$FETCH_SIZE_LOWER_BOUND}") val minFetchSize: Int,
    /** Default fetchSize value, in absence of any other estimate. */
    @Value("\${$PROPERTY_PREFIX.jdbc.defaultFetchSize:$DEFAULT_FETCH_SIZE}")
    val defaultFetchSize: Int,
    /** Largest possible fetchSize value. */
    @Value("\${$PROPERTY_PREFIX.jdbc.maxFetchSize:$FETCH_SIZE_UPPER_BOUND}") val maxFetchSize: Int,
    /** How much of the JVM heap can we fill up with [java.sql.ResultSet] data. */
    @Value("\${$PROPERTY_PREFIX.jdbc.memoryCapacityRatio:$MEM_CAPACITY_RATIO}")
    val memoryCapacityRatio: Double,
    /** Estimated bytes used as overhead for each row in a [java.sql.ResultSet]. */
    @Value("\${$PROPERTY_PREFIX.jdbc.estimatedRecordOverheadBytes:$RECORD_OVERHEAD_BYTES}")
    val estimatedRecordOverheadBytes: Long,
    /** Estimated bytes used as overhead for each column value in a [java.sql.ResultSet]. */
    @Value("\${$PROPERTY_PREFIX.jdbc.estimatedFieldOverheadBytes:$FIELD_OVERHEAD_BYTES}")
