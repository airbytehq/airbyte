...
class JdbcPartitionsCreatorTest {
     ...
     *
     * Scenario: A table was previously synced (cursor state exists) but has since been
     * truncated. SELECT MAX(cursor) returns NULL (.isNull), but the prior state causes
     * the partition factory to keep creating new partitions each round.
     *
    ...
    fun testEmptyTableWithPriorCursorState() {
        ...
        val sharedState = sharedState(
            mockedQueries = arrayOf(
                ...
                TestFixtures.MockedQuery(
                    ...
                    // when fieldValue is null, encode() returns NullCodec.encode(null) which is a NullNode
        ...
        // Should get a single CheckpointOnlyPartitionReader, identify that the table is empty and marked
        // it as done.
        Assertions.assertEquals(1, readers.size)
        // CheckpointOnlyPartitionReader signals an empty table that needs no further reading
        Assertions.assertTrue(readers.first() is JdbcPartitionsCreator<*, *, *>.CheckpointOnlyPartitionReader)
     ...
     *
     * Regression test to verify that an empty table with prior cursor state does not
     * cause an infinite loop. Simulates two rounds of the partition creation cycle:
     * Round 1: detects the empty table and emits a nullNode state
     * Round 2: receives that state and returns null (stream complete).
     *
     * If the fix is reverted, Round 2
     * would create another partition instead of completing, and the test would fail.
     */
    @Test
