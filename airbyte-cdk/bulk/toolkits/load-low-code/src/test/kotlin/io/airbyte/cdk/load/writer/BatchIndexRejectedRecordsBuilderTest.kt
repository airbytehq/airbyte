...
class BatchIndexRejectedRecordsBuilderTest {
    ...
    internal fun `test rejected records when isFull then return list`() {
        val rejectedRecordsBuilder =
            BatchIndexRejectedRecordsBuilder(
                TRUE_CONDITION,
        ...
        val rejectedRecords =
            rejectedRecordsBuilder.getRejectedRecords(
                anInterpolableResponse(
                    Jsons.readTree(
                        """{"$REJECTED_FIELD": [{"$INDEX_FIELD": 1, "$A_FIELD_TO_REPORT": "toto", "$ANOTHER_FIELD_TO_REPORT": "tata"}]}"""
                    )
                )
            )
    ...
    internal fun `test condition not met when isFull then return empty list`() {
        ...
        val rejectedRecords =
            rejectedRecordsBuilder.getRejectedRecords(anInterpolableResponse(ANY_RESPONSE))
    ...
    internal fun `test rejection field not found when isFull then throw`() {
        ...
        assertFailsWith<IllegalArgumentException> {
            rejectedRecordsBuilder.getRejectedRecords(
                anInterpolableResponse(Jsons.readTree("""{"not_rejected_field": []}"""))
            )
        }
    }
    ...
    internal fun `test index field not found when isFull then return throw`() {
        ...
        assertFailsWith<IllegalArgumentException> {
            rejectedRecordsBuilder.getRejectedRecords(
                anInterpolableResponse(
                    Jsons.readTree(
                        """{"$REJECTED_FIELD": [{"not_index_field": 0, "$A_FIELD_TO_REPORT": "toto", "$ANOTHER_FIELD_TO_REPORT": "tata"}]}"""
                    )
                )
            )
        }
    ...
    internal fun `test given index is higher than number of accumulated records when isFull then throw`() {
        ...
        assertFailsWith<IndexOutOfBoundsException> {
            rejectedRecordsBuilder.getRejectedRecords(
                anInterpolableResponse(
                    Jsons.readTree(
                        """{"$REJECTED_FIELD": [{"$INDEX_FIELD": 100, "$A_FIELD_TO_REPORT": "toto", "$ANOTHER_FIELD_TO_REPORT": "tata"}]}"""
                    )
                )
            )
        }
    ...
    fun aRecord(): DestinationRecordRaw {
        every { any<DestinationRecordRaw>().toDlqRecord(any(), any()) } returns
