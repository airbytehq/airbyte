 ...
 * of the stream or when [InsertLoaderAccumulator.accept] returns an
 * [InsertLoaderAccumulator.Request].
 *
 * This means:
 * - The first time a record is seen for a stream, [InsertLoader.createAccumulator] is called,
 * - Then [InsertLoaderAccumulator.accept] is called, for that record and all subsequent records,
 * until it returns an [InsertLoaderAccumulator.Request]
 ...
 * if at least one record has been seen for that batch, [InsertLoaderAccumulator.finish] is called,
 * which must return an input. (After this, the accumulator is discarded and will be recreated only
 * if more data is seen for that stream.)
 *
...
interface InsertLoaderAccumulator<Q : InsertLoaderRequest> : AutoCloseable {
