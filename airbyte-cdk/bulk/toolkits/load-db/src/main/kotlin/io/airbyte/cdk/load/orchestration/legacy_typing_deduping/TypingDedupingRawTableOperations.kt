...
interface TypingDedupingRawTableOperations {
    /**
     * Prepare the raw table, including any associated blob storage.
     * Similar to [createFinalTable], accepts a [suffix] parameter, which should be used in
     * conjunction with [overwriteRawTable].
     *
    ...
    /**
     * Swap the "temporary" raw table into the "real" raw table. For example, `DROP TABLE IF NOT EXISTS
    ...
    /**
     * Copy all records from the temporary raw table into the real raw table, then drop the temporary raw table.
