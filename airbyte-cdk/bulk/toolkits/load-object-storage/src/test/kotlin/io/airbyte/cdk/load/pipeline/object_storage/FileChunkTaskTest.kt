...
class FileChunkTaskTest {
    ...
    fun `enrichRecordsWithFilePath updates the schema and the corresponding data field`() {
        val message = AirbyteMessage().withRecord(
            AirbyteRecordMessage().withData(
                Jsons.deserialize("""{"something": "has to give"}""")
            )
        )
        val schema = ObjectType(
            properties = LinkedHashMap(mapOf("something" to FieldType(StringType, nullable = true)))
        )
        val record = DestinationRecordRaw(
        ...
        assertEquals(FieldType(StringType, nullable = true), schema.properties[COLUMN_NAME_AIRBYTE_FILE_PATH])
        assertContains(Jsons.serialize(record.asRawJson()), """"$COLUMN_NAME_AIRBYTE_FILE_PATH":"$myFilePath"""")
    }
}
