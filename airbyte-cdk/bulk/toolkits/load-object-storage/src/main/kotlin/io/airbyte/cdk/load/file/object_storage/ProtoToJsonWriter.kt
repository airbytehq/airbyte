...
class ProtoToJsonWriter(
    ...
    fun writeMeta(gen: JsonGenerator, record: DestinationRecordRaw) = with(gen) {
    ...
    private fun writeValue(
        ...
        is TimeTypeWithTimezone -> gen.writeStringOrNull(field, proxy.getTimeWithTimezone(field))
        is TimeTypeWithoutTimezone -> gen.writeStringOrNull(
            field,
            proxy.getTimeWithoutTimezone(field),
        )

        is TimestampTypeWithTimezone -> gen.writeStringOrNull(
            field,
            proxy.getTimestampWithTimezone(field),
        )

        is TimestampTypeWithoutTimezone -> gen.writeStringOrNull(
            field,
    ...
    private fun JsonGenerator.writeTreeOrNull(
        ...
        if (node == null) writeNullField(field.name) else {
