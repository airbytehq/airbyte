...
class ObjectStorageDestinationState(
    ...
    suspend fun getObjectsToDelete(): List<Pair<Long, RemoteObject<*>>> {
        ...
        return client
            ...
            .mapNotNull { obj ->
                val generationId =
                    client.getMetadata(obj.key)[METADATA_GENERATION_ID_KEY]?.toLongOrNull()
                if (generationId == null) {
                    log.warn { "No generation id found for object ${obj.key}; not deleting" }
                    null
                } else if (generationId < stream.minimumGenerationId) {
                    println(
                        "Deleting object ${obj.key} with generation id $generationId < ${stream.minimumGenerationId}"
                    )
