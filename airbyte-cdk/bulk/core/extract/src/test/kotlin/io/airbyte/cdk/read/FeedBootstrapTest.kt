...
class FeedBootstrapTest {
    ...
    fun testTriggerBasedCdcMetadataDecoration() {
            ...
            Stream(
                ...
                schema =
                    setOf(
                        k,
                        v,
                        // No GlobalCursor here - this is key for trigger-based CDC detection
                        CommonMetaField.CDC_UPDATED_AT,
                        CommonMetaField.CDC_DELETED_AT
                    ),
                configuredSyncMode = ConfiguredSyncMode.INCREMENTAL,
                configuredPrimaryKey = listOf(k),
                configuredCursor =
                    GlobalCursor, // For trigger based CDC the cursor is not global, we just use
                // this object for test case
                )
        ...
        // The _ab_cdc_lsn should be null since there's no global state
        Assertions.assertTrue(data.get("_ab_cdc_lsn").isNull)
