...
CDC offset types must implement `PartiallyOrdered<T>` instead of `Comparable<T>`. Its `compareTo` method returns a nullable `Int`: return `null` when two offsets cannot be ordered because one or more position fields are missing or otherwise incomparable.
...
The new interface is defined in `io.airbyte.cdk.read.cdc`:
...
Update connector implementations and generic bounds from `Comparable<T>` to `PartiallyOrdered<T>`. The CDC reader and partition creator now use `isGreater` and `isGreaterOrEqual` for offset checks. When `compareTo` returns `null`, these helpers return `false`, so the reader does not incorrectly conclude that an incomparable offset has reached the upper bound or that progress has stalled. The optional `startup(offset: DebeziumOffset)` hook was added to `CdcPartitionsCreatorDebeziumOperations`; override it when a connector needs to perform one-time CDC startup work.

##### Source fields: `Field` → `EmittedField`

Source-provided fields must use `EmittedField`. The old `Field` name is retained only as a deprecated typealias:
...
Replace `Field` imports, type annotations, and generic arguments with `EmittedField`. In particular, update `MetadataQuerier.fields`, `DiscoveredStream.columns`, CDC `DeserializedRecord.changes`, and output callback maps. `NonEmittedField` is the separate type for data fields generated by a connector during operation; use it when that distinction applies.

The `Field` compatibility alias can be seen in the release source, but connectors should migrate directly to `EmittedField`:
