 ...
 */
class ProtobufToAirbyteValue(private val fields: Array<AirbyteValueProxy.FieldAccessor>) {
    ...
    private fun convertField(
        ...
        return when (field.type) {
            is BooleanType ->
                convertPrimitiveField { proxy.getBoolean(field) }?.let(::BooleanValue) ?: NullValue
            is IntegerType ->
                convertPrimitiveField { proxy.getInteger(field) }?.let(::IntegerValue) ?: NullValue
            is NumberType ->
                convertPrimitiveField { proxy.getNumber(field) }?.let(::NumberValue) ?: NullValue
            is DateType ->
                convertPrimitiveField { proxy.getDate(field) }?.let(::DateValue) ?: NullValue
            is StringType ->
                convertPrimitiveField { proxy.getString(field) }?.let(::StringValue) ?: NullValue
            is TimeTypeWithTimezone ->
                convertPrimitiveField { proxy.getTimeWithTimezone(field) }
                    ?.let(::TimeWithTimezoneValue) ?: NullValue
            is TimeTypeWithoutTimezone ->
                convertPrimitiveField { proxy.getTimeWithoutTimezone(field) }
                    ?.let(::TimeWithoutTimezoneValue) ?: NullValue
            is TimestampTypeWithTimezone ->
                convertPrimitiveField { proxy.getTimestampWithTimezone(field) }
                    ?.let(::TimestampWithTimezoneValue) ?: NullValue
            is TimestampTypeWithoutTimezone ->
                convertPrimitiveField { proxy.getTimestampWithoutTimezone(field) }
                    ?.let(::TimestampWithoutTimezoneValue) ?: NullValue
            is ArrayType,
