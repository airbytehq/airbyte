 ...
 *
 *  These differences might diverge/converge as we tune (ie, because of lock contention with
 *  multiple sockets, or because the socket pattern ends up working for stdio as well). For now,
 *  since the main difference is what is done with the pipeline events,
 *  we'll consolidate bookkeeping into a single class that yields events from DestinationMessage(s).
 *  CheckpointIds can be inferred iff the record does not provide one.
 */
@Singleton
class PipelineEventBookkeeper(
    ...
    suspend fun handleStreamMessage(
        ...
        return when (message) {
            is DestinationRecord -> {
                ...
                log.info { "Read RECORD for stream ${stream.descriptor}" }
                PipelineMessage(
            ...
            is DestinationFileStreamIncomplete ->
                throw IllegalStateException("File stream ${stream.descriptor} failed upstream, cannot continue.")
        }
    }
    ...
    suspend fun handleCheckpoint(reservation: Reserved<CheckpointMessage>) {
        when (val checkpoint = reservation.value) {
            ...
            is GlobalCheckpoint -> {
                ...
                checkpointQueue.publish(
                    reservation.replace(
                        GlobalCheckpointWrapped(streamIndexes, messageWithCount)
                    )
                )
            }
        }
