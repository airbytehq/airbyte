...
class DefaultDestinationTaskLauncher(
    ...
    inner class TaskWrapper(override val innerTask: ScopedTask) : WrappedTask<ScopedTask> {
    ...
    private suspend fun enqueue(task: ScopedTask) {
        taskScopeProvider.launch(TaskWrapper(task))
    }
    ...
    override suspend fun handleException(e: Exception) {
        catalog.streams.forEach {
            enqueue(failStreamTaskFactory.make(this, e, it.descriptor))
        }
    }

    override suspend fun handleFailStreamComplete(stream: DestinationStream.Descriptor, e: Exception) {
