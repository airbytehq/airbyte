...
import io.airbyte.cdk.load.command.ObjectStoragePathConfiguration
...
interface S3PathSpecification {
    ...
    @get:JsonSchemaTitle("S3 Staging Infix")
    @get:JsonPropertyDescription(
        "Path to use when staging data in the bucket directory. Documentation TBD."
    )
    @get:JsonProperty("s3_staging_infix", defaultValue = "{s3BucketPath}/__airbyte_tmp")
