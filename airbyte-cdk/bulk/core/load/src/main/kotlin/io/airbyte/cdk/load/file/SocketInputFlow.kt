        ...
        if (config.runSetup) {
            if (part == 0) {
                ...
                catalog.streams.forEach {
                    log.info { "Running start for ${it.descriptor}" }
                    destinationWriter.createStreamLoader(it).start()
                }
                setupComplete.complete(Unit)
            } else {
                log.info { "Part $part waiting for setup to complete" }
                setupComplete.await()
            }
        } else {
                ...
                .use { bufferedInputStream ->
                    ...
                    when (config.inputSerializationFormat) {
                        ...
                        DestinationConfiguration.InputSerializationFormat.FLATBUFFERS -> {
                            log.info { "Socket $socketName reading flatbuffer" }
