...
abstract class ProtoMapper {
    private val fakeSortedCatalog: List<Pair<String, ProtoDataType>> = listOf(
     ...
     */
    abstract fun mapSchema(schemaIn: List<Pair<String, ProtoDataType>>): List<Pair<String, ProtoDataType>>

    /**
     * Called per record to transform that data. Changes must match the schema changes or
     * everything will explode. You may safely mutate the list in place.
     */
...
class IdentityProtoMapper: ProtoMapper() {
    override fun mapSchema(schemaIn: List<Pair<String, ProtoDataType>>): List<Pair<String, ProtoDataType>> {
        return schemaIn
    }
