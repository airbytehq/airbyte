...
class LoadPipelineStepTask<S : AutoCloseable, K1 : WithStream, T, K2 : WithStream, U : Any>(
                    ...
                    is PipelineMessage -> {
                        ...
                        val state =
                            stateStore.getOrPut(input.key) {
                                ...
                                it.copy(inputCount = it.inputCount + 1)
                            }
                        val (newState, output) =
                        ...
                        val finalOutput = output ?: if (flushStrategy?.shouldFlush(state.inputCount) == true) {
    ...
    private suspend fun handleOutput(inputKey: K1, checkpointCounts: Map<CheckpointId, Long>, output: U) {
