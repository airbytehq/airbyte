...
class DockerizedDestination(
    ...
    init {
        ...
        destinationStdin = when (dataChannelMedium) {
            ...
            DataChannelMedium.SOCKETS -> {
                ...
                val tcpSocketWriter = TCPSocketWriter("localhost", sidecarPort, awaitFirstWrite = true)
    ...
    private fun createSharedVolume() {
        val createVolumeCommand = listOf(
            "docker",
            "volume",
            "create",
            "--name",
    ...
    private fun startDestination(featureFlags: Array<out FeatureFlag>): Process {
        ...
        val cmd: MutableList<String> =
            (listOf(
                ...
                "--name",
                containerName,
                "--mount", "source=$sharedVolumeName,target=/var/airbyte",
                "-v",
    ...
    private fun startSidecar(sidecarPort: Int): Process {
        ...
        val socatCommand = listOf(
            "docker", "run",
            "--rm",
            "-d", "--name", "socat-sidecar-$randomSuffix",
            "--mount", "source=$sharedVolumeName,target=/var/airbyte",
            "-p", "$sidecarPort:$sidecarPort",
            "alpine/socat",
            "-dddd",
    ...
    private fun removeSharedVolume() {
        val removeVolumeCommand = listOf(
            "docker",
            "volume",
            "rm",
            sharedVolumeName,
                ...
                launch {
                    ...
                    while (destinationStdout.hasNextLine()) {
                        ...
                        if (message.type == AirbyteMessage.Type.LOG) {
                            ...
                            if (message.log.message.contains("Socket $socketPath connected for reading")) {
