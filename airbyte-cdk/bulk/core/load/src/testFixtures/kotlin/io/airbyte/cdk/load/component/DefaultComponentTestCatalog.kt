 ...
 *   @Singleton @Primary
 *   fun catalog() = DefaultComponentTestCatalog.make()
 */
object DefaultComponentTestCatalog {
    fun make(): ConfiguredAirbyteCatalog {
        val jsonNodeFactory = JsonNodeFactory.instance
        val schema = jsonNodeFactory.objectNode().apply {
            put("type", "object")
            set<Nothing>("properties", jsonNodeFactory.objectNode().apply {
                set<Nothing>("id", jsonNodeFactory.objectNode().apply {
                    put("type", "integer")
                })
                set<Nothing>("name", jsonNodeFactory.objectNode().apply {
                    put("type", "string")
                })
            })
        }
