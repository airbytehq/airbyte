...
function useInitSdk<Sdk extends FirebaseSdks>(
  ...
  if (React.useContext(SdkContext)) {
    throw new Error(
      `Cannot initialize SDK ${sdkName} because it already exists in Context`
    );
  }
...
export const useInitAuth: InitSdkHook<Auth> = (initializer) =>
  useInitSdk<Auth>("auth", AuthSdkContext, initializer);
