...
describe("<FrequentlyUsedDestinations />", () => {
  test("should renders with mock data without crash", () => {
    const { getAllByText } = renderFrequentlyUsedDestinationsComponent({
      destinations: mockData,
      onDestinationSelect: () => {},
      propertyPath: "serviceType",
    });

    expect(getAllByText("Google Sheets")).toHaveLength(2);
    expect(getAllByText("BigQuery")).toHaveLength(2);
    expect(getAllByText("DynamoDB")).toHaveLength(2);
    expect(getAllByText("MongoDB")).toHaveLength(2);
    expect(getAllByText("Postgres")).toHaveLength(2);
  });
});
