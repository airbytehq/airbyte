...
public class EmptyAirbyteSourceTest {
  ...
  public void testGlobal() throws Exception {
    ...
    /*
      The comparison could be what it is bellow but it makes it hard to see what is the diff. It has been break dow into multiples assertions.
      (same comment in the other tests)
      ...
      AirbyteStateMessage expectedState = new AirbyteStateMessage()
       ...
       .withGlobal(
         new AirbyteGlobalState()
           ...
           .withStreamStates(
             Lists.newArrayList(
               new AirbyteStreamState().withStreamState(null).withStreamDescriptor(new StreamDescriptor().withName("a")),
               new AirbyteStreamState().withStreamState(null).withStreamDescriptor(new StreamDescriptor().withName("b")),
               new AirbyteStreamState().withStreamState(Jsons.emptyObject()).withStreamDescriptor(new StreamDescriptor().withName("c"))
             )
           )
      );
  ...
  private List<AirbyteStateMessage> createPerStreamState(List<StreamDescriptor> streamDescriptors) {
    return streamDescriptors.stream().map(streamDescriptor ->
  ...
  private List<AirbyteStateMessage> createGlobalState(List<StreamDescriptor> streamDescriptors, JsonNode sharedState) {
    AirbyteGlobalState globalState = new AirbyteGlobalState()
        ...
        .withStreamStates(
            streamDescriptors.stream().map(streamDescriptor ->
                new AirbyteStreamState()
                    .withStreamDescriptor(streamDescriptor)
                    .withStreamState(Jsons.emptyObject())
                )
