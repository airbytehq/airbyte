...
class ResourceRequirementsUtilsTest {
  ...
  void testNoReqsSet() {
    ...
    final ResourceRequirements sourceReqs = ResourceRequirementsUtils.getResourceRequirements(input, ActorType.SOURCE, JobType.SYNC);
    final ResourceRequirements destReqs = ResourceRequirementsUtils.getResourceRequirements(input, ActorType.DESTINATION, JobType.SYNC);
  ...
  void testDefaultDefinitionReqsSet() {
    ...
    final StandardSyncInput input = new StandardSyncInput()
        .withSourceResourceRequirements(new ActorDefinitionResourceRequirements().withDefault(defaultSourceReqs))
        .withDestinationResourceRequirements(new ActorDefinitionResourceRequirements().withDefault(defaultDestReqs));
  ...
  void testJobSpecificResourceRequirementsOverrideDefault() {
    final ResourceRequirements defaultReqs = new ResourceRequirements().withCpuRequest("2").withMemoryRequest("300Mi");
  ...
  void testConnectionResourceRequirementsOverrideDefault() {
    ...
    final ResourceRequirements connectionResourceReqs = new ResourceRequirements().withCpuLimit("5").withMemoryLimit("400Mi");
    final StandardSyncInput input = new StandardSyncInput()
        .withSourceResourceRequirements(new ActorDefinitionResourceRequirements()
            .withDefault(defaultReqs)
            .withJobSpecific(List.of(jobTypeResourceLimit)))
