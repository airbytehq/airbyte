...
class EnvConfigsTest {
  ...
  void testWorkerPodTolerations() {
    ...
    when(function.apply(EnvConfigs.WORKER_POD_TOLERATIONS))
        .thenReturn("key=airbyte-server,operator=Exists,effect=NoSchedule");
    Assertions.assertEquals(
        config.getWorkerPodTolerations(),
        List.of(new WorkerPodToleration("airbyte-server", "NoSchedule", null, "Exists")));

    when(function.apply(EnvConfigs.WORKER_POD_TOLERATIONS))
        .thenReturn("key=airbyte-server,operator=Equals,value=true,effect=NoSchedule");
    Assertions.assertEquals(
        config.getWorkerPodTolerations(),
        List.of(new WorkerPodToleration("airbyte-server", "NoSchedule", "true", "Equals")));
  ...
  void testWorkerPodNodeSelectors() {
    ...
    when(function.apply(EnvConfigs.WORKER_POD_NODE_SELECTORS))
        .thenReturn("airbyte=server,something=nothing");
    Assertions.assertEquals(
        config.getWorkerNodeSelectors(), Map.of("airbyte", "server", "something", "nothing"));
  }
}
