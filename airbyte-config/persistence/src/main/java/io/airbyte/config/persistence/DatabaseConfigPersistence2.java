...
public class DatabaseConfigPersistence2 implements ConfigPersistence {
  ...
  private StandardSourceDefinition getStandardSourceDefinition(final String configId) throws IOException, ConfigNotFoundException {
    ...
    return new StandardSourceDefinition()
        ...
        .withSourceType(record.get(ACTOR_DEFINITION.SOURCE_TYPE) == null ? null
            : Enums.toEnum(record.get(ACTOR_DEFINITION.SOURCE_TYPE, String.class), StandardSourceDefinition.SourceType.class).orElseThrow())
  ...
  private StandardSyncOperation getStandardSyncOperation(final String configId) throws IOException, ConfigNotFoundException {
    ...
    return new StandardSyncOperation()
        ...
        .withOperatorType(Enums.toEnum(record.get(OPERATION.OPERATOR_TYPE, String.class), StandardSyncOperation.OperatorType.class).orElseThrow())
  ...
  private StandardSync getStandardSync(final String configId) throws IOException, ConfigNotFoundException {
    ...
    return new StandardSync()
        ...
        .withNamespaceDefinition(
            Enums.toEnum(record.get(CONNECTION.NAMESPACE_DEFINITION, String.class), io.airbyte.config.JobSyncConfig.NamespaceDefinitionType.class)
                .orElseThrow())
        ...
        .withStatus(record.get(CONNECTION.STATUS) == null ? null
            : Enums.toEnum(record.get(CONNECTION.STATUS, String.class), StandardSync.Status.class).orElseThrow())
  ...
  public <T> List<T> listConfigs(AirbyteConfig configType, Class<T> clazz) throws JsonValidationException, IOException {
    ...
    if (configType == ConfigSchema.STANDARD_WORKSPACE) {
      ...
      getStandardSourceDefinitionWithMetadata().forEach(c -> config.add((T) c.getConfig()));
    } else if (configType == ConfigSchema.STANDARD_DESTINATION_DEFINITION) {
      getStandardDestinationDefinitionWithMetadata().forEach(c -> config.add((T) c.getConfig()));
    } else if (configType == ConfigSchema.SOURCE_CONNECTION) {
      getSourceConnectionWithMetadata().forEach(c -> config.add((T) c.getConfig()));
    } else if (configType == ConfigSchema.DESTINATION_CONNECTION) {
      getDestinationConnectionWithMetadata().forEach(c -> config.add((T) c.getConfig()));
    } else if (configType == ConfigSchema.SOURCE_OAUTH_PARAM) {
      getSourceOauthParamWithMetadata().forEach(c -> config.add((T) c.getConfig()));
    } else if (configType == ConfigSchema.DESTINATION_OAUTH_PARAM) {
      getDestinationOauthParamWithMetadata().forEach(c -> config.add((T) c.getConfig()));
    } else if (configType == ConfigSchema.STANDARD_SYNC_OPERATION) {
      getStandardSyncOperationWithMetadata().forEach(c -> config.add((T) c.getConfig()));
    } else if (configType == ConfigSchema.STANDARD_SYNC) {
      getStandardSyncWithMetadata().forEach(c -> config.add((T) c.getConfig()));
    } else if (configType == ConfigSchema.STANDARD_SYNC_STATE) {
      getStandardSyncStateWithMetadata().forEach(c -> config.add((T) c.getConfig()));
    } else {
      throw new IllegalArgumentException("Unknown Config Type " + configType);
    }
  ...
  public <T> List<ConfigWithMetadata<T>> listConfigsWithMetadata(AirbyteConfig configType, Class<T> clazz) throws IOException {
    ...
    if (configType == ConfigSchema.STANDARD_WORKSPACE) {
      ...
      getStandardSourceDefinitionWithMetadata().forEach(c -> configWithMetadata.add((ConfigWithMetadata<T>) c));
    } else if (configType == ConfigSchema.STANDARD_DESTINATION_DEFINITION) {
      getStandardDestinationDefinitionWithMetadata().forEach(c -> configWithMetadata.add((ConfigWithMetadata<T>) c));
    } else if (configType == ConfigSchema.SOURCE_CONNECTION) {
      getSourceConnectionWithMetadata().forEach(c -> configWithMetadata.add((ConfigWithMetadata<T>) c));
    } else if (configType == ConfigSchema.DESTINATION_CONNECTION) {
      getDestinationConnectionWithMetadata().forEach(c -> configWithMetadata.add((ConfigWithMetadata<T>) c));
    } else if (configType == ConfigSchema.SOURCE_OAUTH_PARAM) {
      getSourceOauthParamWithMetadata().forEach(c -> configWithMetadata.add((ConfigWithMetadata<T>) c));
    } else if (configType == ConfigSchema.DESTINATION_OAUTH_PARAM) {
      getDestinationOauthParamWithMetadata().forEach(c -> configWithMetadata.add((ConfigWithMetadata<T>) c));
    } else if (configType == ConfigSchema.STANDARD_SYNC_OPERATION) {
      getStandardSyncOperationWithMetadata().forEach(c -> configWithMetadata.add((ConfigWithMetadata<T>) c));
    } else if (configType == ConfigSchema.STANDARD_SYNC) {
      getStandardSyncWithMetadata().forEach(c -> configWithMetadata.add((ConfigWithMetadata<T>) c));
    } else if (configType == ConfigSchema.STANDARD_SYNC_STATE) {
      getStandardSyncStateWithMetadata().forEach(c -> configWithMetadata.add((ConfigWithMetadata<T>) c));
    } else {
      throw new IllegalArgumentException("Unknown Config Type " + configType);
    }
  ...
  private List<ConfigWithMetadata<StandardWorkspace>> getStandardWorkspaceWithMetadata() throws IOException {
    final Result<Record> result = database.query(ctx -> ctx.select(asterisk())
        .from(WORKSPACE)
        .fetch());
  ...
  private List<ConfigWithMetadata<StandardSourceDefinition>> getStandardSourceDefinitionWithMetadata() throws IOException {
    final Result<Record> result = database.query(ctx -> ctx.select(asterisk())
        .from(ACTOR_DEFINITION)
  ...
  private List<ConfigWithMetadata<StandardDestinationDefinition>> getStandardDestinationDefinitionWithMetadata() throws IOException {
    final Result<Record> result = database.query(ctx -> ctx.select(asterisk())
        .from(ACTOR_DEFINITION)
  ...
  private List<ConfigWithMetadata<SourceConnection>> getSourceConnectionWithMetadata() throws IOException {
    final Result<Record> result = database.query(ctx -> ctx.select(asterisk())
        .from(ACTOR)
  ...
  private List<ConfigWithMetadata<DestinationConnection>> getDestinationConnectionWithMetadata() throws IOException {
    final Result<Record> result = database.query(ctx -> ctx.select(asterisk())
        .from(ACTOR)
  ...
  private List<ConfigWithMetadata<SourceOAuthParameter>> getSourceOauthParamWithMetadata() throws IOException {
    final Result<Record> result = database.query(ctx -> ctx.select(asterisk())
        .from(ACTOR_OAUTH_PARAMETER)
  ...
  private List<ConfigWithMetadata<DestinationOAuthParameter>> getDestinationOauthParamWithMetadata() throws IOException {
    final Result<Record> result = database.query(ctx -> ctx.select(asterisk())
        .from(ACTOR_OAUTH_PARAMETER)
  ...
  private List<ConfigWithMetadata<StandardSyncOperation>> getStandardSyncOperationWithMetadata() throws IOException {
    final Result<Record> result = database.query(ctx -> ctx.select(asterisk())
        .from(OPERATION)
        .fetch());
  ...
  private List<ConfigWithMetadata<StandardSync>> getStandardSyncWithMetadata() throws IOException {
    final Result<Record> result = database.query(ctx -> ctx.select(asterisk())
        .from(CONNECTION)
        .fetch());
  ...
  private List<ConfigWithMetadata<StandardSyncState>> getStandardSyncStateWithMetadata() throws IOException {
    final Result<Record> result = database.query(ctx -> ctx.select(asterisk())
        .from(STATE)
        .fetch());
  ...
  public void deleteConfig(AirbyteConfig configType, String configId) throws ConfigNotFoundException, IOException {
    if (configType == ConfigSchema.STANDARD_WORKSPACE) {
      ...
      deleteDefinition(configId);
    } else if (configType == ConfigSchema.STANDARD_DESTINATION_DEFINITION) {
      deleteDefinition(configId);
    } else if (configType == ConfigSchema.SOURCE_CONNECTION) {
      deleteConnection(configId);
    } else if (configType == ConfigSchema.DESTINATION_CONNECTION) {
      deleteConnection(configId);
    } else if (configType == ConfigSchema.SOURCE_OAUTH_PARAM) {
      deleteOauthParams(configId);
    } else if (configType == ConfigSchema.DESTINATION_OAUTH_PARAM) {
      deleteOauthParams(configId);
    } else if (configType == ConfigSchema.STANDARD_SYNC_OPERATION) {
      deleteStandardSyncOperation(configId);
    } else if (configType == ConfigSchema.STANDARD_SYNC) {
      deleteStandardSync(configId);
    } else if (configType == ConfigSchema.STANDARD_SYNC_STATE) {
      deleteStandardSyncState(configId);
    } else {
      throw new IllegalArgumentException("Unknown Config Type " + configType);
    }
  }

  private void deleteStandardWorkspace(final String configId) throws IOException {
    database.transaction(ctx -> {
      final UUID workspaceId = UUID.fromString(configId);
      final boolean isExistingConfig = ctx.fetchExists(select()
          .from(WORKSPACE)
  ...
  private void deleteDefinition(final String configId) throws IOException {
    database.transaction(ctx -> {
      final UUID definitionId = UUID.fromString(configId);
      final boolean isExistingConfig = ctx.fetchExists(select()
          .from(ACTOR_DEFINITION)
          .where(ACTOR_DEFINITION.ID.eq(definitionId)));

      if (isExistingConfig) {
        ctx.deleteFrom(ACTOR_DEFINITION)
            .where(ACTOR_DEFINITION.ID.eq(definitionId))
            .execute();
        return null;
      }
  ...
  private void deleteConnection(final String configId) throws IOException {
    database.transaction(ctx -> {
      final UUID connectorId = UUID.fromString(configId);
      final boolean isExistingConfig = ctx.fetchExists(select()
          .from(ACTOR)
          .where(ACTOR.ID.eq(connectorId)));

      if (isExistingConfig) {
        ctx.deleteFrom(ACTOR)
            .where(ACTOR.ID.eq(connectorId))
            .execute();
        return null;
      }
  ...
  private void deleteOauthParams(final String configId) throws IOException {
    database.transaction(ctx -> {
      final UUID oauthParameterId = UUID.fromString(configId);
      final boolean isExistingConfig = ctx.fetchExists(select()
          .from(ACTOR_OAUTH_PARAMETER)
          .where(ACTOR_OAUTH_PARAMETER.ID.eq(oauthParameterId)));
  ...
  private void deleteStandardSyncOperation(final String configId) throws IOException {
    database.transaction(ctx -> {
      final UUID operationId = UUID.fromString(configId);
      final boolean isExistingConfig = ctx.fetchExists(select()
          .from(OPERATION)
          .where(OPERATION.ID.eq(operationId)));

      if (isExistingConfig) {
        ctx.deleteFrom(OPERATION)
            .where(OPERATION.ID.eq(operationId))
            .execute();
        return null;
      }
  ...
  private void deleteStandardSyncState(final String configId) throws IOException {
    database.transaction(ctx -> {
      final UUID connectionId = UUID.fromString(configId);
      final boolean isExistingConfig = ctx.fetchExists(select()
          .from(STATE)
   ...
   */
  @VisibleForTesting
  <T> ConnectorCounter updateConnectorDefinitions(
