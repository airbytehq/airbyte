...
public class DatabaseConfigPersistence implements ConfigPersistence {
  ...
  public void migrateFileConfigs(final Configs serverConfigs) throws IOException {
    database.transaction(
        ctx -> {
          ...
          final boolean hasExistingFileConfigs =
              FileSystemConfigPersistence.hasExistingConfigs(serverConfigs.getConfigRoot());
          if (hasExistingFileConfigs) {
            LOGGER.info("Load existing local config directory into configs database");
            final ConfigPersistence fileSystemPersistence =
                new FileSystemConfigPersistence(serverConfigs.getConfigRoot());
  ...
  public <T> T getConfig(
        ...
        database.query(
            ctx ->
                ctx.select(asterisk())
                    ...
                    .where(
                        AIRBYTE_CONFIGS.CONFIG_TYPE.eq(configType.name()),
                        AIRBYTE_CONFIGS.CONFIG_ID.eq(configId))
                    .fetch());
    ...
    if (result.isEmpty()) {
      ...
      throw new IllegalStateException(
          String.format("Multiple %s configs found for ID %s: %s", configType, configId, result));
    }
  ...
  public <T> void writeConfig(final AirbyteConfig configType, final String configId, final T config)
    ...
    database.transaction(
        ctx -> {
          final boolean isExistingConfig =
              ctx.fetchExists(
                  select()
                      .from(AIRBYTE_CONFIGS)
                      .where(
  ...
  public Map<String, Stream<JsonNode>> dumpConfigs() throws IOException {
    ...
    final Map<String, Result<Record>> results =
        database.query(
            ctx ->
                ctx.select(asterisk())
                    .from(AIRBYTE_CONFIGS)
    ...
    return results.entrySet().stream()
        .collect(
            Collectors.toMap(
                ...
                e ->
                    e.getValue().stream()
                        .map(r -> Jsons.deserialize(r.get(AIRBYTE_CONFIGS.CONFIG_BLOB).data()))));
  }
  ...
  int insertConfigRecord(
    ...
    final String configId =
        idFieldName == null ? UUID.randomUUID().toString() : configJson.get(idFieldName).asText();
    ...
    if (insertionCount != 1) {
      LOGGER.warn(
          "{} config {} already exists (insertion record count: {})",
          configType,
          configId,
          insertionCount);
    }
  ...
  int updateConfigRecord(
    ...
    final int updateCount =
        ctx.update(AIRBYTE_CONFIGS)
            .set(AIRBYTE_CONFIGS.CONFIG_BLOB, JSONB.valueOf(Jsons.serialize(configJson)))
            .set(AIRBYTE_CONFIGS.UPDATED_AT, timestamp)
            .where(
                AIRBYTE_CONFIGS.CONFIG_TYPE.eq(configType), AIRBYTE_CONFIGS.CONFIG_ID.eq(configId))
            .execute();
    if (updateCount != 1) {
      LOGGER.warn(
          "{} config {} is not updated (updated record count: {})",
          configType,
          configId,
          updateCount);
    }
  ...
  @VisibleForTesting
  void copyConfigsFromSeed(final DSLContext ctx, final ConfigPersistence seedConfigPersistence)
      throws SQLException {
                      ...
                      .map(
                          configJson -> {
                            final String idFieldName =
                                ConfigSchemaMigrationSupport.CONFIG_SCHEMA_ID_FIELD_NAMES.get(
                                    configType);
                            return insertConfigRecord(
                                ctx, timestamp, configType, configJson, idFieldName);
                          })
                      .reduce(0, Integer::sum);
                })
            .reduce(0, Integer::sum);
  ...
  @VisibleForTesting
  void updateConfigsFromSeed(final DSLContext ctx, final ConfigPersistence seedConfigPersistence)
      throws SQLException {
    ...
    try {
      ...
      final Map<String, ConnectorInfo> connectorRepositoryToInfoMap =
          getConnectorRepositoryToInfoMap(ctx);
      ...
      final List<StandardSourceDefinition> latestSources =
          seedConfigPersistence.listConfigs(
              ConfigSchema.STANDARD_SOURCE_DEFINITION, StandardSourceDefinition.class);
      final ConnectorCounter sourceConnectorCounter =
      ...
      final ConnectorCounter destinationConnectorCounter =
          updateConnectorDefinitions(
              ctx,
              timestamp,
      ...
      LOGGER.info(
          "Connector definitions have been updated ({} new connectors, and {} updates)",
          newConnectorCount,
  ...
  <T> ConnectorCounter updateConnectorDefinitions(
    ...
    for (final T definition : latestDefinitions) {
      ...
      if (!connectorRepositoryToIdVersionMap.containsKey(repository)) {
        ...
        newCount +=
            insertConfigRecord(
                ctx, timestamp, configType.name(), latestDefinition, configType.getIdFieldName());
        continue;
      }
      ...
      if (connectorRepositoriesInUse.contains(repository)) {
        if (newFields.size() == 0) {
          ...
          final JsonNode definitionToUpdate =
              getDefinitionWithNewFields(currentDefinition, latestDefinition, newFields);
      ...
      if (hasNewVersion(connectorInfo.dockerImageTag, latestImageTag)) {
        ...
        LOGGER.info(
            "Connector {} needs update: {} vs {}",
            repository,
            connectorInfo.dockerImageTag,
            latestImageTag);
        updatedCount +=
            updateConfigRecord(
                ctx, timestamp, configType.name(), latestDefinition, connectorInfo.definitionId);
        ...
        updatedCount +=
            updateConfigRecord(
                ctx, timestamp, configType.name(), definitionToUpdate, connectorInfo.definitionId);
      } else {
        LOGGER.info(
            "Connector {} does not need update: {}", repository, connectorInfo.dockerImageTag);
      }
    }
  ...
  static boolean hasNewVersion(final String currentVersion, final String latestVersion) {
    try {
      return new AirbyteVersion(latestVersion)
              .patchVersionCompareTo(new AirbyteVersion(currentVersion))
          > 0;
  ...
  static Set<String> getNewFields(
      final JsonNode currentDefinition, final JsonNode latestDefinition) {
  ...
  static JsonNode getDefinitionWithNewFields(
      final JsonNode currentDefinition,
      final JsonNode latestDefinition,
   ...
   *     repository name instead of definition id because connectors can be added manually by users,
   *     and are not always the same as those in the seed.
   */
  @VisibleForTesting
  Map<String, ConnectorInfo> getConnectorRepositoryToInfoMap(final DSLContext ctx) {
    ...
    final Field<String> repoField =
        field("config_blob ->> 'dockerRepository'", SQLDataType.VARCHAR).as("repository");
    return ctx
        .select(AIRBYTE_CONFIGS.CONFIG_ID, repoField, configField)
        .from(AIRBYTE_CONFIGS)
        .where(
            AIRBYTE_CONFIGS.CONFIG_TYPE.in(
        ...
        .collect(
            Collectors.toMap(
                ...
                row ->
                    new ConnectorInfo(
                        row.getValue(AIRBYTE_CONFIGS.CONFIG_ID),
                        Jsons.deserialize(row.getValue(configField).data())),
                ...
                (c1, c2) -> {
                  ...
                  LOGGER.warn(
                      "Duplicated connector version found for {}: {} ({}) vs {} ({})",
                      c1.dockerRepository,
                      c1.dockerImageTag,
                      c1.definitionId,
                      c2.dockerImageTag,
   ...
   *     connectors can be added manually by users, and their config ids are not always the same as
   *     those in the seed.
   */
  private Set<String> getConnectorRepositoriesInUse(final DSLContext ctx) {
    ...
    usedConnectorDefinitionIds.addAll(
        ctx
    ...
    return ctx
        .select(field("config_blob ->> 'dockerRepository'", SQLDataType.VARCHAR))
        .from(AIRBYTE_CONFIGS)
