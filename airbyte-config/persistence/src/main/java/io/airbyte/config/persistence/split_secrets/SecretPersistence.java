...
public interface SecretPersistence extends ReadOnlySecretPersistence {
  ...
  static Optional<SecretPersistence> getLongLived(final Configs configs, final DSLContext dslContext) throws IOException {
  ...
  static SecretsHydrator getSecretsHydrator(final Configs configs, final DSLContext dslContext) throws IOException {
    final var persistence = getLongLived(configs, dslContext);
  ...
  static Optional<SecretPersistence> getEphemeral(final Configs configs, final DSLContext dslContext) throws IOException {
    switch (configs.getSecretPersistenceType()) {
      case TESTING_CONFIG_DB_TABLE -> {
