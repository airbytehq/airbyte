...
public class JsonSecretsProcessorTest {
  ...
  private static final JsonNode SCHEMA_ONE_LAYER =
      Jsons.deserialize(
          "{\n"
              + "  \"properties\": {\n"
              + "    \"secret1\": {\n"
  ...
  private static final JsonNode SCHEMA_INNER_OBJECT =
      Jsons.deserialize(
          "{\n"
              + "    \"type\": \"object\",\n"
              + "    \"properties\": {\n"
    ...
    final JsonNode expected =
        Jsons.jsonNode(
            ImmutableMap.builder()
                ...
                .put("secret2", JsonSecretsProcessor.SECRETS_MASK)
                .build());
  ...
  public void testMaskSecretsNotInObj() {
    final JsonNode obj =
        Jsons.jsonNode(ImmutableMap.builder().put("field1", "value1").put("field2", 2).build());
  ...
  public void testMaskSecretInnerObject() {
    ...
    final JsonNode base =
        Jsons.jsonNode(
            ImmutableMap.builder().put("warehouse", "house").put("loading_method", oneOf).build());

    final JsonNode actual = processor.maskSecrets(base, SCHEMA_INNER_OBJECT);

    final JsonNode expectedOneOf =
        Jsons.jsonNode(
            ImmutableMap.builder()
                .put("s3_bucket_name", "name")
  ...
  public void testMaskSecretNotInInnerObject() {
    final JsonNode base = Jsons.jsonNode(ImmutableMap.builder().put("warehouse", "house").build());

    final JsonNode actual = processor.maskSecrets(base, SCHEMA_INNER_OBJECT);

    final JsonNode expected =
        Jsons.jsonNode(ImmutableMap.builder().put("warehouse", "house").build());
  ...
  public void testCopySecretNotInSrcInnerObject() {
    final JsonNode src = Jsons.jsonNode(ImmutableMap.builder().put("warehouse", "house").build());
