...
PATH_TO_NON_FORMATTED_CODE = (
    "airbyte-ci/connectors/pipelines/tests/test_format/non_formatted_code"
)
PATH_TO_FORMATTED_CODE = (
    "airbyte-ci/connectors/pipelines/tests/test_format/formatted_code"
)
...
def now_formatted_directory(dagger_client, tmp_dir_with_non_formatted_code):
    return (
        dagger_client.host()
        .directory(tmp_dir_with_non_formatted_code)
        .with_timestamps(0)
    )
...
def already_formatted_directory(dagger_client, tmp_dir_with_formatted_code):
    return (
        dagger_client.host().directory(tmp_dir_with_formatted_code).with_timestamps(0)
    )
...
def directory_with_expected_formatted_code(dagger_client):
    ...
    return (
        dagger_client.host().directory(expected_formatted_code_path).with_timestamps(0)
    )
...
async def test_check_and_fix_all_on_non_formatted_code(
    ...
    mocker.patch.object(
        ...
        "get_dagger_client",
        mocker.AsyncMock(return_value=dagger_client),
    )
    mocker.patch.object(
        commands.FormatCommand, "LOCAL_REPO_PATH", tmp_dir_with_non_formatted_code
    )
    runner = CliRunner()
    result = await runner.invoke(
        commands.format_code, [subcommand, "all"], catch_exceptions=False
    )
    if subcommand == "fix":
        assert (
            await now_formatted_directory.diff(
                directory_with_expected_formatted_code
            ).entries()
            == []
        )
...
async def test_check_and_fix_all_on_formatted_code(
    ...
    mocker.patch.object(
        commands.FormatCommand, "LOCAL_REPO_PATH", tmp_dir_with_formatted_code
    )
    runner = CliRunner()
    result = await runner.invoke(
        commands.format_code, [subcommand, "all"], catch_exceptions=False
    ...
    if subcommand == "fix":
        assert (
            await already_formatted_directory.diff(
                directory_with_expected_formatted_code
            ).entries()
            == []
        )
