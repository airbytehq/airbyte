...
async def test_with_python_connector_installed_from_setup(context_with_setup, python_connector_base_image_address, latest_cdk_version):
    ...
    cdk_install_latest_output = (
        await container.with_env_variable("CACHEBUSTER", datetime.datetime.now().isoformat()).with_exec(["pip", "uninstall", "-y", f"airbyte-cdk=={latest_cdk_version}"]).with_exec(["pip", "install", f"airbyte-cdk=={latest_cdk_version}"])
        .stdout()
    )
