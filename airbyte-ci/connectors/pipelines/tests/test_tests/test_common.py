...
class TestAcceptanceTests:
    ...
    def get_patched_acceptance_test_step(
        self, dagger_client, mocker, test_context_ci, test_input_dir
    ):
        test_secrets = {
            "config.json": dagger_client.set_secret("config.json", "connector_secret")
        }
        test_context_ci.get_connector_dir = mocker.AsyncMock(
            return_value=test_input_dir
        )
        test_context_ci.connector_acceptance_test_image = "bash:latest"
        test_context_ci.get_connector_secrets = mocker.AsyncMock(
    ...
    async def test_cat_container_provisioning(
        ...
        acceptance_test_step = self.get_patched_acceptance_test_step(
            dagger_client, mocker, test_context_ci, test_input_dir
        )
