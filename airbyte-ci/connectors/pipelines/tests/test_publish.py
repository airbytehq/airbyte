...
STEPS_TO_PATCH = [
    ...
    (publish_pipeline, "CheckPypiPackageDoesNotExist"),
]
...
async def test_run_connector_pypi_publish_pipeline(
    mocker,
    pypi_enabled,
    ...
    mocked_publish_to_pypi.return_value.run.return_value = mocker.Mock(name="publish_to_pypi_result", status=publish_step_status)

    publish_pipeline.CheckPypiPackageDoesNotExist.return_value.run.return_value = mocker.Mock(
        name="pypi_package_does_not_exist_result", status=pypi_package_does_not_exist_status
    )
