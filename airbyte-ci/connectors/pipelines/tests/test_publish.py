...
class TestCheckConnectorImageDoesNotExists:
    ...
    async def test_run_skipped_when_already_published(self, three_random_connectors_image_names, publish_context):
        ...
        for image_name in three_random_connectors_image_names:
            ...
            step = publish.pipeline.CheckConnectorImageDoesNotExist(publish_context)
...
class TestUploadSpecToCache:
    ...
    async def test_run(self, mocker, dagger_client, valid_spec, successful_upload, random_connector, publish_context):
        ...
        if valid_spec:
            publish.pipeline.upload_to_gcs.assert_called_once_with(
            ...
            spec_file = publish.pipeline.upload_to_gcs.call_args.args[1]
        ...
        if (not valid_spec and successful_upload) or (not valid_spec and not successful_upload):
            ...
            publish.pipeline.upload_to_gcs.assert_not_called()
    ...
    def test_parse_spec_output_invalid_json(self, publish_context):
        ...
        with pytest.raises(publish.pipeline.InvalidSpecOutputError):
...
STEPS_TO_PATCH = [
    (publish.pipeline.metadata, "MetadataValidation"),
    (publish.pipeline.metadata, "MetadataUpload"),
    ...
    (publish.pipeline.steps, "run_connector_build"),
]
...
async def test_run_connector_publish_pipeline_when_failed_validation(mocker, pre_release):
    ...
    report = await publish.pipeline.run_connector_publish_pipeline(context, semaphore)
...
async def test_run_connector_publish_pipeline_when_image_exists_or_failed(mocker, check_image_exists_status, publish_context):
    ...
    run_upload_spec_to_cache = publish.pipeline.UploadSpecToCache.return_value.run
    run_upload_spec_to_cache.return_value = mocker.Mock(status=StepStatus.SUCCESS)

    run_check_connector_image_does_not_exist = publish.pipeline.CheckConnectorImageDoesNotExist.return_value.run
    run_check_connector_image_does_not_exist.return_value = mocker.Mock(status=check_image_exists_status)

    run_metadata_upload = publish.pipeline.metadata.MetadataUpload.return_value.run

    semaphore = anyio.Semaphore(1)
    report = await publish.pipeline.run_connector_publish_pipeline(publish_context, semaphore)
    run_metadata_validation.assert_called_once()
...
async def test_run_connector_publish_pipeline_when_image_does_not_exist(
    ...
    publish.pipeline.CheckConnectorImageDoesNotExist.return_value.run.return_value = mocker.Mock(
    ...
    publish.pipeline.steps.run_connector_build.return_value = mocker.Mock(
        name="build_connector_for_publish_result", status=build_step_status, output_artifact=built_connector_platform
    )

    publish.pipeline.PushConnectorImageToRegistry.return_value.run.return_value = mocker.Mock(
        name="push_connector_image_to_registry_result", status=push_step_status
    )

    publish.pipeline.PullConnectorImageFromRegistry.return_value.run.return_value = mocker.Mock(
        name="pull_connector_image_from_registry_result", status=pull_step_status
    )

    publish.pipeline.UploadSpecToCache.return_value.run.return_value = mocker.Mock(
        name="upload_spec_to_cache_result", status=upload_to_spec_cache_step_status
    )
    publish.pipeline.metadata.MetadataUpload.return_value.run.return_value = mocker.Mock(
    ...
    steps_to_run = [
        ...
        publish.pipeline.CheckConnectorImageDoesNotExist.return_value.run,
        publish.pipeline.steps.run_connector_build,
        publish.pipeline.PushConnectorImageToRegistry.return_value.run,
        publish.pipeline.PullConnectorImageFromRegistry.return_value.run,
    ]
    ...
    if build_step_status is StepStatus.SUCCESS:
        publish.pipeline.PushConnectorImageToRegistry.return_value.run.assert_called_once_with(["linux/amd64"])
    else:
        publish.pipeline.PushConnectorImageToRegistry.return_value.run.assert_not_called()
        publish.pipeline.PullConnectorImageFromRegistry.return_value.run.assert_not_called()
        publish.pipeline.UploadSpecToCache.return_value.run.assert_not_called()
        publish.pipeline.metadata.MetadataUpload.return_value.run.assert_not_called()
