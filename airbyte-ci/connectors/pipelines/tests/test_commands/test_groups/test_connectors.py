...
async def test_commands_do_not_override_connector_selection(
    ...
    mocker.patch.object(
        connectors_test_command, "ConnectorContext", mock_connector_context
    )
    mocker.patch.object(
    ...
    await runner.invoke(
        command, command_args, catch_exceptions=True, obj=click_context_obj
    )
...
def test_should_use_remote_secrets(
    ...
    mocker.patch.object(
        connectors_commands.os,
        "getenv",
        return_value="test" if gsm_env_var_set else None,
    )
    if expect_click_usage_error:
        with pytest.raises(click.UsageError):
            connectors_commands.should_use_remote_secrets(use_remote_secrets_user_input)
    else:
        final_use_remote_secrets = connectors_commands.should_use_remote_secrets(
            use_remote_secrets_user_input
        )
