...
class GradleTask(Step, ABC):
    ...
    async def _collect_test_logs(self, gradle_container: Container) -> Optional[Artifact]:
        ...
        test_logs_dir_name_in_zip = f"test-logs-{datetime.fromtimestamp(cast(float, self.context.pipeline_start_timestamp)).isoformat()}-{self.context.git_branch}-{self.gradle_task_name}".replace(
            "/", "_")
        if test_logs_dir_name_in_container not in await gradle_container.directory(f"{self.context.connector.code_directory}/build").entries():
    ...
    async def _collect_test_results(self, gradle_container: Container) -> Optional[Artifact]:
        ...
        test_results_dir_name_in_zip = f"test-results-{datetime.fromtimestamp(cast(float, self.context.pipeline_start_timestamp)).isoformat()}-{self.context.git_branch}-{self.gradle_task_name}".replace("/", "_")
        if test_results_dir_name_in_container not in await gradle_container.directory(f"{self.context.connector.code_directory}/build").entries():
