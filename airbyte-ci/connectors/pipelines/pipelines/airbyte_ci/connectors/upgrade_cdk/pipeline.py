...
class SetCDKVersion(Step):
    ...
    async def upgrade_cdk_version_for_python_connector(
        ...
        airbyte_cdk_dependency = re.search(
            r"airbyte-cdk(?P<extra>\[[a-zA-Z0-9-]*\])?(?P<version>[<>=!~]+[0-9]*(?:\.[0-9]*)?(?:\.[0-9]*)?)?",
            setup_py_content,
        )
        # If there is no airbyte-cdk dependency, add the version
        ...
        new_version_str = (
            f"airbyte-cdk{airbyte_cdk_dependency.group('extra') or ''}>={new_version}"
        )
        updated_setup_py = setup_py_content.replace(
            airbyte_cdk_dependency.group(), new_version_str
        )
