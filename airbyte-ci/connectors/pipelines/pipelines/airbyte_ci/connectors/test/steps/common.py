...
class VersionCheck(Step, ABC):
    ...
    GITHUB_URL_PREFIX_FOR_CONNECTORS = "https://raw.githubusercontent.com/airbytehq/airbyte/main/airbyte-integrations/connectors"
    ...
    async def _run(self) -> StepResult:
        ...
        if self.context.ci_context in [CIContext.main, CIContext.NIGHTLY_BUILDS]:
            return StepResult(
                self,
                status=StepStatus.SKIPPED,
                stdout="Version check are not running in main context.",
            )
        try:
...
class VersionIncrementCheck(VersionCheck):
    ...
    def failure_message(self) -> str:
        return f"The dockerImageTag in {METADATA_FILE_NAME} was not incremented. The files you modified should lead to a version bump. main version is {self.master_connector_version}, current version is {self.current_connector_version}"
...
class VersionFollowsSemverCheck(VersionCheck):
    ...
    @property
    def failure_message(self) -> str:
        return f"The dockerImageTag in {METADATA_FILE_NAME} is not following semantic versioning or was decremented. main version is {self.master_connector_version}, current version is {self.current_connector_version}"
...
class QaChecks(Step):
    ...
    async def _run(self) -> StepResult:
        ...
        https://github.com/airbytehq/airbyte/blob/main/airbyte-ci/connector_ops/connector_ops/qa_checks.py
        ...
        if (
            ...
            original_connector = Connector(
                self.context.connector.technical_name.replace(
                    "-strict-encrypt", ""
                ).replace("-secure", "")
            )
            include += [
        ...
        qa_checks = (
            ...
            .with_exec(
                ["run-qa-checks", f"connectors/{self.context.connector.technical_name}"]
            )
        )
...
class AcceptanceTests(Step):
    ...
    def __init__(
        self, context: PipelineContext, concurrent_test_run: Optional[bool] = False
    ) -> None:
        ...
        cat_container = (
            ...
            .with_(
                await secrets.mounted_connector_secrets(
                    self.context, "/test_input/secrets"
                )
            )
        )
...
class CheckBaseImageIsUsed(Step):
    ...
    async def _run(self, *args, **kwargs) -> StepResult:
        ...
        if not is_certified:
            return self.skip(
                "Connector is not certified, it does not require the use of our base image."
            )

        is_using_base_image = (
            self.context.connector.metadata.get("connectorBuildOptions", {}).get(
                "baseImage"
            )
            is not None
        )
        ...
        has_dockerfile = (
            "Dockerfile"
            in await (
                await self.context.get_connector_dir(include="Dockerfile")
            ).entries()
        )
        if has_dockerfile:
            return StepResult(
        ...
        return StepResult(
            self,
            StepStatus.SUCCESS,
            stdout="Connector is certified and uses our base image.",
        )
