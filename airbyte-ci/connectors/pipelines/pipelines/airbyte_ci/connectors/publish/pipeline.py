...
from auto_merge.consts import AUTO_MERGE_LABEL, AUTO_MERGE_PROMOTED_RC_LABEL
    ...
    async with semaphore:
        async with context:
            ...
            if initial_pr_creation_result.success:
                ...
                post_changelog_pr_update = CreateOrUpdatePullRequest(context, skip_ci=True, labels=[AUTO_MERGE_LABEL, AUTO_MERGE_PROMOTED_RC_LABEL])
                pr_creation_args, pr_creation_kwargs = get_promotion_pr_creation_arguments(
                    all_modified_files, context, results, current_version, promoted_version
                )
