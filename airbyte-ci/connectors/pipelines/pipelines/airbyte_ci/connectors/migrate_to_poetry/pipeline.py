...
PACKAGE_NAME_PATTERN = r"^([a-zA-Z0-9_.\-]+)(?:\[(.*?)\])?([=~><!]=?[0-9\.]+)?$"
...
class PoetryInit(Step):
    ...
    def to_poetry_dependencies(self, requirements_style_deps: List[str], latest_dependencies_for_hard_pin: dict) -> dict:
        dependencies = {}
    ...
    async def get_dependencies(self, connector_container, groups=None) -> dict:
        ...
        for line in pip_install_dry_run_output.splitlines():
            if "Requirement already satisfied" in line and "->" not in line:
                non_transitive_deps.append(line.replace("Requirement already satisfied: ", "").split(" ")[0])
    ...
    async def _run(self) -> StepResult:
        ...
        with_poetry_lock = (
            await connector_container.with_new_file("pyproject.toml", contents=toml_string)
            .with_exec(["poetry", "install"])
            .with_exec(["poetry", "version", self.new_version])
        )
        await with_poetry_lock.file("pyproject.toml").export(f"{self.context.connector.code_directory}/pyproject.toml")
        self.logger.info(f"Generated pyproject.toml for {self.context.connector.technical_name}")
        await with_poetry_lock.file("poetry.lock").export(f"{self.context.connector.code_directory}/poetry.lock")
        self.logger.info(f"Generated poetry.lock for {self.context.connector.technical_name}")
        return StepResult(step=self, status=StepStatus.SUCCESS, output_artifact=(dependencies, dev_dependencies))
...
class DeleteSetUpPy(Step):
    ...
    async def _run(self) -> StepResult:
        ...
        return StepResult(step=self, status=StepStatus.SUCCESS, output_artifact=original_setup_py)
...
class RegressionTest(Step):
    ...
    def __init__(self, context: ConnectorContext, *args, **kwargs):
        super().__init__(context, *args, **kwargs)

    async def _run(self, new_connector_container, original_dependencies, original_dev_dependencies) -> StepResult:
        try:
    ...
    async def check_all_original_deps_are_installed(self, new_connector_container, original_main_dependencies, original_dev_dependencies):
        previous_pip_freeze = (
        ...
        main_dependencies_names = [re.match(PACKAGE_NAME_PATTERN, dep.split(",")[0]).group(1) for dep in original_main_dependencies]
        dev_dependencies_names = [re.match(PACKAGE_NAME_PATTERN, dep.split(",")[0]).group(1) for dep in original_dev_dependencies]
        previous_package_name_version_mapping = {
            re.match(PACKAGE_NAME_PATTERN, dep).group(1): dep for dep in previous_pip_freeze if "==" in dep
        }
        current_package_name_version_mapping = {
            re.match(PACKAGE_NAME_PATTERN, dep).group(1): dep for dep in current_pip_freeze if "==" in dep
        }
...
async def run_connector_migration_to_poetry_pipeline(context: ConnectorContext, semaphore: "Semaphore") -> Report:
    ...
    steps_to_run = [
        [StepToRun(id="CHECK_MIGRATION_CANDIDATE", step=CheckIsMigrationCandidate(context))],
        [StepToRun(id="POETRY_INIT", step=PoetryInit(context, new_version), depends_on=["CHECK_MIGRATION_CANDIDATE"])],
        [StepToRun(id="DELETE_SETUP_PY", step=DeleteSetUpPy(context), depends_on=["POETRY_INIT"])],
        [StepToRun(id="BUILD_CONNECTOR_IMAGE", step=BuildConnectorImages(context), depends_on=["DELETE_SETUP_PY"])],
        [
            StepToRun(
                id="REGRESSION_TEST",
                step=RegressionTest(context),
                depends_on=["BUILD_CONNECTOR_IMAGE"],
                args=lambda results: {
                    "new_connector_container": results["BUILD_CONNECTOR_IMAGE"].output_artifact[LOCAL_BUILD_PLATFORM],
                    "original_dependencies": results["POETRY_INIT"].output_artifact[0],
                    "original_dev_dependencies": results["POETRY_INIT"].output_artifact[1],
                },
            )
        ],
        [
            StepToRun(
                id="BUMP_METADATA_VERSION",
        ...
        [StepToRun(id="UPDATE_README", step=UpdateReadMe(context), depends_on=["BUMP_METADATA_VERSION"])],
    ]
