...
@click.command()
@click.option("--modified", default=False, is_flag=True, help="Run on only on modified internal packages.")
@click.option(
    "--poetry-package-path",
    "-p",
            ...
            poetry_package_poe_tasks_results.append(
                (
                    ...
                    poetry_packages_task_group.soonify(pipeline.run_poe_tasks_for_package)(
                        dagger_client, poetry_package_path, pipeline_context.params["is_ci"]
                    ),
                )
            )
