...
from pipelines.utils import check_path_in_workdir, format_duration, slugify, with_exit_code, with_stderr, with_stdout
...
class ConnectorReport(Report):
    ...
    async def to_html(self) -> str:
        env = Environment(
            loader=PackageLoader("pipelines.tests"), autoescape=select_autoescape(), trim_blocks=False, lstrip_blocks=True
        )
