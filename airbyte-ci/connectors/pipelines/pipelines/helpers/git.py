...
async def get_modified_files_in_branch_remote(
    ...
    try:
        async with Connection(DAGGER_CONFIG) as dagger_client:
            ...
            modified_files = await container.with_exec(["diff", f"--diff-filter={DIFF_FILTER}", "--name-only", f"origin/{diffed_branch}...target/{current_git_branch}"], use_entrypoint=True).stdout()
    except SessionError:
        if retries > 0:
...
async def get_modified_files_in_commit_remote(current_git_branch: str, current_git_revision: str, retries: int = 3) -> Set[str]:
    try:
        async with Connection(DAGGER_CONFIG) as dagger_client:
            ...
            modified_files = await container.with_exec(["diff-tree", "--no-commit-id", "--name-only", current_git_revision, "-r"], use_entrypoint=True).stdout()
    except SessionError:
        if retries > 0:
