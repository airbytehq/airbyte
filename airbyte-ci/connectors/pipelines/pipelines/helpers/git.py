    ...
    try:
        async with Connection(DAGGER_CONFIG) as dagger_client:
            container = await checked_out_git_container(
                dagger_client, current_git_branch, current_git_revision, diffed_branch
            )
            modified_files = await container.with_exec(
                [
                    ...
                    "--name-only",
                    f"{diffed_branch}...{current_git_branch}",
                ]
            ).stdout()
    except SessionError:
        if retries > 0:
            return await get_modified_files_in_branch_remote(
                current_git_branch, current_git_revision, diffed_branch, retries - 1
            )
        else:
            raise
...
async def get_modified_files_in_branch(
    ...
    if is_local:
        ...
        return await get_modified_files_in_branch_remote(
            current_git_branch, current_git_revision, diffed_branch
        )
