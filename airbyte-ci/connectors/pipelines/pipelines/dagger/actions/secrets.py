...
async def get_secrets_to_mask(
    ci_credentials_with_downloaded_secrets: Container, connector_technical_name: str
) -> list[str]:
    ...
    """
    secrets_to_mask = []
    if secrets_to_mask_file := await get_file_contents(
        ci_credentials_with_downloaded_secrets, "/tmp/secrets_to_mask.txt"
    ):
        for secret_to_mask in secrets_to_mask_file.splitlines():
            if (
                secret_to_mask in NOT_REALLY_SECRETS
                or secret_to_mask in connector_technical_name
            ):
...
async def download(
    ...
    with_downloaded_secrets = (
        ...
        .with_exec(
            ["ci_credentials", context.connector.technical_name, "write-to-storage"]
        )
    )
    # We don't want to print secrets in the logs when running locally.
    if context.is_ci:
        context.secrets_to_mask = await get_secrets_to_mask(
            with_downloaded_secrets, context.connector.technical_name
        )
    connector_secrets = {}
    for secret_file in await with_downloaded_secrets.directory(secrets_path).entries():
        secret_plaintext = (
            await with_downloaded_secrets.directory(secrets_path)
            .file(secret_file)
            .contents()
        )
        ...
        connector_secrets[secret_file] = context.dagger_client.set_secret(
            unique_secret_name, secret_plaintext
        )
...
async def load_from_local(context: ConnectorContext) -> Dict[str, Secret]:
    ...
    if not await local_secrets_path.is_dir():
        context.logger.warning(
            f"Local secrets directory {local_secrets_path} does not exist, no secrets will be loaded."
        )
        return connector_secrets
    async for secret_file in local_secrets_path.iterdir():
        ...
        connector_secrets[secret_file.name] = context.dagger_client.set_secret(
            unique_secret_name, secret_plaintext
        )
    if not connector_secrets:
        context.logger.warning(
            f"Local secrets directory {local_secrets_path} is empty, no secrets will be loaded."
        )
    return connector_secrets
...
async def mounted_connector_secrets(
    context: ConnectorContext, secret_directory_path: str
    ...
    def with_secrets_mounted_as_dagger_secrets(container: Container) -> Container:
        ...
        for secret_file_name, secret in connector_secrets.items():
            container = container.with_mounted_secret(
                f"{secret_directory_path}/{secret_file_name}", secret
            )
        return container
