...
async def finalize_build(
    ...
    connector_container = connector_container.with_label(
        "io.dagger.engine_version", get_dagger_sdk_version()
    )
    connector_dir_with_finalize_script = await context.get_connector_dir(
        include=["finalize_build.sh", "finalize_build.py"]
    )
    ...
    if has_finalize_python_script and has_finalize_bash_script:
        raise Exception(
            "Connector has both finalize_build.sh and finalize_build.py, please remove one of them"
        )

    if has_finalize_python_script:
        context.logger.info(
            f"{context.connector.technical_name} has a finalize_build.py script, running it to finalize build..."
        )
        ...
        if connector_finalize_module_spec is None:
            raise Exception(
                "Connector has a finalize_build.py script but it can't be loaded."
            )
        connector_finalize_module = importlib.util.module_from_spec(
            connector_finalize_module_spec
        )
        ...
        try:
            connector_container = await connector_finalize_module.finalize_build(
                context, connector_container
            )
        except AttributeError:
            raise Exception(
                "Connector has a finalize_build.py script but it doesn't have a finalize_build function."
            )

    if has_finalize_bash_script:
        context.logger.info(
            f"{context.connector.technical_name} has finalize_build.sh script, running it to finalize build..."
        )
        connector_container = (
            connector_container.with_file(
                "/tmp/finalize_build.sh",
                connector_dir_with_finalize_script.file("finalize_build.sh"),
            )
            .with_entrypoint("sh")
