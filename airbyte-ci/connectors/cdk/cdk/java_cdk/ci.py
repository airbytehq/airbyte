...
async def build(settings: JavaCDKSettings, ctx: PipelineContext, client: Optional[Client] = None, scan: bool = False) -> List[Awaitable[Container]]:
...
async def test(settings: JavaCDKSettings, ctx: PipelineContext, client: Optional[Client] = None, scan: bool = False) -> List[Awaitable[Container]]:
    ...
    results = await test_java_cdk_task.submit(build_result=build_result[0], client=test_client, settings=settings, ctx=quote(ctx), scan=scan)
    return [results]
