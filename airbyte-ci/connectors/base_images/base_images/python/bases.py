...
class AirbytePythonConnectorBaseImage(bases.AirbyteConnectorBaseImage):
    ...
    def install_cdk_system_dependencies(self) -> Callable:
        def get_nltk_data_dir() -> dagger.Directory:
            ...
            for nltk_data_subfolder, nltk_data_urls in self.ntlk_data.items():
                ...
                for nltk_data_url in nltk_data_urls:
                    ...
                    data_container = (
                        data_container.with_file("/tmp/data.zip", zip_file).with_exec(["mkdir", "-p", full_nltk_data_path]).with_exec(["unzip", "-o", "/tmp/data.zip", "-d", full_nltk_data_path]).with_exec(["rm", "/tmp/data.zip"])
                    )
        ...
        def with_tesseract_and_poppler(container: dagger.Container) -> dagger.Container:
            ...
            container = container.with_exec(["sh", "-c", "apt-get update && apt-get install -y tesseract-ocr=5.3.0-2 poppler-utils=22.12.0-2+b1"])
    ...
    def get_container(self, platform: dagger.Platform) -> dagger.Container:
        ...
        return (
            ...
            .with_mounted_cache("/root/.cache/pip", pip_cache_volume).with_exec(["ln", "-snf", "/usr/share/zoneinfo/Etc/UTC", "/etc/localtime"], use_entrypoint=True).with_exec(["pip", "install", "--upgrade", "pip==24.0", "setuptools==70.0.0"], use_entrypoint=True)
            ...
            .with_env_variable("POETRY_NO_INTERACTION", "1").with_exec(["pip", "install", "poetry==1.6.1"]).with_exec(["sh", "-c", "apt-get update && apt-get upgrade -y && apt-get dist-upgrade -y && apt-get clean"], use_entrypoint=True).with_exec(["sh", "-c", "apt-get install -y socat=1.7.4.4-2"], use_entrypoint=True)
