...
def dummy_repo(dummy_repo_path, eligible_connectors, excluded_connectors) -> git.Repo:
    ...
    # set main branch instead of main
...
def test_get_metadata_file_path(
    checkout_master, eligible_connectors, dummy_repo_path: Path
):
    for connector in eligible_connectors:
        path = cloud_availability_updater.get_metadata_file_path(
            dummy_repo_path, connector
        )
        assert path.exists() and path.name == "metadata.yaml"


def test_checkout_new_branch(mocker, checkout_master, dummy_repo):
    new_branch = cloud_availability_updater.checkout_new_branch(
        dummy_repo, "test-branch"
    )
...
def test_enable_in_cloud(mocker, dummy_repo_path, expect_update, eligible_connectors):
    ...
    connector_metadata_path = (
        dummy_repo_path
        / f"airbyte-integrations/connectors/{connector.connector_technical_name}"
        / "metadata.yaml"
    )
    with open(connector_metadata_path, "w") as definitions_mask:
        mask_yaml = yaml.safe_dump(
            {"data": {"registries": {"cloud": {"enabled": not expect_update}}}}
        )
        definitions_mask.write(mask_yaml)
    updated_path = cloud_availability_updater.enable_in_cloud(
        connector, connector_metadata_path
    )
...
def test_add_new_connector_to_cloud_catalog(mocker, updated_files, dummy_repo_path):
    ...
    mocker.patch.object(
        ...
        "enable_in_cloud",
        mocker.Mock(return_value=updated_files),
    )
    ...
    updated_connector = cloud_availability_updater.add_new_connector_to_cloud_catalog(
        dummy_repo_path, repo, connector
    )
    assert updated_connector == updated_files
    cloud_availability_updater.get_metadata_file_path.assert_called_with(
        dummy_repo_path, connector
    )
    ...
    if updated_files:
        cloud_availability_updater.commit_all_files.assert_called_with(
            repo, f"🤖 Add {connector.connector_name} connector to cloud
        ")
...
def test_create_pr(mocker, pr_already_created):
    ...
    pr_post_response = mocker.Mock(
        json=mocker.Mock(return_value={"url": "pr_url", "number": "pr_number"})
    )
    cloud_availability_updater.requests.post.side_effect = [
        pr_post_response,
        mocker.Mock(),
    ]
    mocker.patch.object(
        cloud_availability_updater,
        "pr_already_created_for_branch",
        mocker.Mock(return_value=pr_already_created),
    )
    mocker.patch.object(
        cloud_availability_updater, "GITHUB_API_COMMON_HEADERS", {"common": "headers"}
    )
    ...
    expected_issue_url = (
        "https://api.github.com/repos/airbytehq/airbyte/issues/pr_number/labels"
    )
    expected_issue_data = {"labels": cloud_availability_updater.PR_LABELS}

    response = cloud_availability_updater.create_pr(
        "my pr title",
        "my pr body",
        "my_awesome_branch",
    ...
    if not pr_already_created:
        expected_post_calls = [
            mocker.call(
                ...
                json=expected_pr_data,
            ),
            mocker.call(
                expected_issue_url,
                headers=cloud_availability_updater.GITHUB_API_COMMON_HEADERS,
                json=expected_issue_data,
            ),
        ]
        cloud_availability_updater.requests.post.assert_has_calls(
            expected_post_calls, any_order=False
        )
...
@pytest.mark.parametrize(
    "json_response, expected_result", [([], False), (["foobar"], True)]
)
def test_pr_already_created_for_connector(mocker, json_response, expected_result):
    ...
    cloud_availability_updater.requests.get.return_value.json.return_value = (
        json_response
    )
    mocker.patch.object(
        cloud_availability_updater, "GITHUB_API_COMMON_HEADERS", {"common": "headers"}
    ...
    is_already_created = cloud_availability_updater.pr_already_created_for_branch(
        "my-awesome-branch"
    )
...
def test_set_git_identity(mocker):
    ...
    repo.git.config.assert_has_calls(
        [
            mocker.call(
                "--global", "user.email", cloud_availability_updater.GIT_USER_EMAIL
            ),
            mocker.call(
                "--global", "user.name", cloud_availability_updater.GIT_USERNAME
            ),
        ]
    )
...
def test_get_authenticated_repo_url(mocker):
    mocker.patch.object(
        cloud_availability_updater, "AIRBYTE_GITHUB_REPO_URL", "https://foobar.com"
    )
    repo_url = cloud_availability_updater.get_authenticated_repo_url(
        "username", "token"
    )
...
def test_pr_already_created_for_branch(mocker, response, expected_output):
    ...
    cloud_availability_updater.requests.get.return_value = mocker.Mock(
    ...
    cloud_availability_updater.requests.get.assert_called_with(
        ...
        headers=cloud_availability_updater.GITHUB_API_COMMON_HEADERS,
        params={
            "head": f"{cloud_availability_updater.AIRBYTE_REPO_OWNER}:foo",
            "state": "open",
        },
    )
...
def test_get_pr_body(mocker, eligible_connectors, excluded_connectors):
    pr_body = cloud_availability_updater.get_pr_body(
        eligible_connectors, excluded_connectors
    )
...
def test_batch_deploy_eligible_connectors_to_cloud_repo(
    ...
    all_connectors = eligible_connectors + excluded_connectors
    mocker.patch.object(
        cloud_availability_updater.tempfile,
        "mkdtemp",
        mocker.Mock(return_value=str(dummy_repo_path)),
    )
    ...
    mocker.patch.object(
        cloud_availability_updater, "add_new_connector_to_cloud_catalog"
    )
    mocker.patch.object(
        cloud_availability_updater, "enable_in_cloud", side_effect=False
    )
    ...
    if added_connectors:
        cloud_availability_updater.add_new_connector_to_cloud_catalog.side_effect = (
            lambda _path, _repo, connector: (
                connector not in expected_excluded_connectors
            )
        )
        expected_added_connectors = eligible_connectors
    else:
        cloud_availability_updater.add_new_connector_to_cloud_catalog.return_value = (
            False
        )
    ...
    expected_pr_title = (
        "🤖 Cloud Availability updater: new connectors to deploy [20230214]"
    )

    cloud_availability_updater.batch_deploy_eligible_connectors_to_cloud_repo(
        all_connectors
    )
    cloud_availability_updater.clone_airbyte_repo.assert_called_once_with(
        dummy_repo_path
    )
    cloud_availability_updater.set_git_identity.assert_called_once_with(
        cloud_availability_updater.clone_airbyte_repo.return_value
    )
    mock_repo.git.checkout.assert_called_with(
        cloud_availability_updater.AIRBYTE_MAIN_BRANCH_NAME
    )

    cloud_availability_updater.checkout_new_branch.assert_called_once_with(
        mock_repo, expected_new_branch_name
    )
    cloud_availability_updater.add_new_connector_to_cloud_catalog.assert_has_calls(
        [
            mocker.call(
                dummy_repo_path,
                cloud_availability_updater.set_git_identity.return_value,
                eligible_connectors[0],
            ),
        ]
    )
    if added_connectors:
        cloud_availability_updater.push_branch.assert_called_once_with(
            mock_repo, expected_new_branch_name
        )
        ...
        cloud_availability_updater.get_pr_body.assert_called_with(
            expected_added_connectors, expected_excluded_connectors
        )
    else:
