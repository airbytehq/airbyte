...
def augment_and_normalize_connector_dataframes(
    cloud_df, oss_df, primaryKey, connector_type, github_connector_folders, cached_specs
):
    """
    ...
    total_registry = pd.merge(cloud_df, oss_df, how="outer", on=composite_key).drop_duplicates(subset=composite_key)
    ...
    merged_registry["connector_type"] = connector_type
    merged_registry["is_source_controlled"] = merged_registry["dockerRepository"].apply(
...
def all_sources_dataframe(
    cloud_sources_dataframe, oss_sources_dataframe, github_connector_folders, cached_specs
) -> pd.DataFrame:
    """
...
def all_destinations_dataframe(
    cloud_destinations_dataframe, oss_destinations_dataframe, github_connector_folders, cached_specs
) -> pd.DataFrame:
    """
