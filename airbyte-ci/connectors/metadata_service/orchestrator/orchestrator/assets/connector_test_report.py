...
def compute_connector_nightly_report_history(
    ...
    nightly_report_test_output_df["nightly_path"] = nightly_report_test_output_df["file_path"].apply(
        lambda file_path: next(parent_prefix for parent_prefix in nightly_report_complete_df["parent_prefix"] if parent_prefix in file_path),
    )
    ...
    return matrix_df.reindex(sorted(matrix_df.columns), axis=1)
