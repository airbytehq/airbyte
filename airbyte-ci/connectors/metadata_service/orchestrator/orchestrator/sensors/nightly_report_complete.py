...
def new_nightly_report_complete_sensor(job, resources_def) -> SensorDefinition:
    """
    ...
    @sensor(
        name=f"{job.name}_on_new_nightly_report_complete",
        job=job,
        minimum_interval_seconds=(1 * 60 * 60),
        default_status=DefaultSensorStatus.STOPPED,
    )
    def new_nightly_report_complete_definition(context: SensorEvaluationContext):
        context.log.info("Starting new_nightly_report_complete_sensor")

        with build_resources(resources_def) as resources:
            context.log.info("Got resources for new_nightly_report_complete_sensor")
    ...
    return new_nightly_report_complete_definition
