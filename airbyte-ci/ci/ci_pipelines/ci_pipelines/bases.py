...
from ci_pipelines.actions import remote_storage
from ci_pipelines.consts import GCS_PUBLIC_DOMAIN, LOCAL_REPORTS_PATH_ROOT, PYPROJECT_TOML_FILE_PATH
from ci_pipelines.utils import check_path_in_workdir, format_duration, slugify, with_exit_code, with_stderr, with_stdout
...
if TYPE_CHECKING:
    from ci_pipelines.contexts import PipelineContext
...
class ConnectorReport(Report):
    ...
    def post_comment_on_pr(self) -> None:
        ...
        markdown_comment += "**You can run the same pipeline locally on this branch with the [airbyte-ci](https://github.com/airbytehq/airbyte/blob/master/airbyte-ci/ci_connector_ops/ci_connector_ops/pipelines/README.md) tool with the following command**\n"
    ...
    async def to_html(self) -> str:
        env = Environment(
            loader=PackageLoader("ci_pipelines.tests"), autoescape=select_autoescape(), trim_blocks=False, lstrip_blocks=True
        )
