...
public class ConnectorMarkdownGenerator {
  ...
  public void run(final Path outputRoot, final Path seedRoot, final String outputFileName) {
    ...
    final String body = buildMarkdown(sourceDefinitionsJson, destinationDefinitionsJson);
  ...
  private String buildMarkdown(final List<JsonNode> sourceDefinitionsJson, final List<JsonNode> destinationDefinitionsJson) {
  ...
  private String buildMarkdownTable(final List<JsonNode> definitions, final String type) {
    final List<String> bodyParts = new ArrayList<>();
