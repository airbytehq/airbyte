...
class ContentAnalysisAgent:
    ...
    def __init__(self):
        self.integration_patterns = {
            "connector": [r"airbyte\s+connector", r"source\s+connector", r"destination\s+connector", r"custom\s+connector"],
            "pipeline": [r"data\s+pipeline", r"etl\s+pipeline", r"workflow", r"orchestration"],
            "platform": [r"data\s+platform", r"integration\s+platform", r"infrastructure"],
        }
    ...
    def _determine_integration_type(self, content: str) -> str:
        ...
        type_counts = {
            itype: sum(1 for pattern in patterns if re.search(pattern, content)) for itype, patterns in self.integration_patterns.items()
        }
    ...
    def _extract_key_features(self, content: str) -> List[str]:
        ...
        airbyte_patterns = {
            ...
            "integration": [r"integrat(?:e|ing|ion)\s+with\s+([^.\n]+)", r"connect(?:s|ing)?\s+to\s+([^.\n]+)"],
            "orchestration": [r"orchestrat(?:e|ing|ion)\s+(?:of\s+)?([^.\n]+)", r"pipeline\s+(?:for|to)\s+([^.\n]+)"],
        }
        ...
        if not features:
            feature_sections = re.findall(r"(?:features|highlights|what it does)[:|-]\s*(?:\n\s*[-*]\s*.+)+", content, re.IGNORECASE)
    ...
    def _determine_complexity(self, content: str, features: List[str]) -> str:
        ...
        advanced_indicators = ["custom", "advanced", "complex", "enterprise", "scalable"]

        # Check for basic features
        basic_indicators = ["simple", "basic", "easy", "straightforward", "quick"]

        complexity_score += sum(2 for indicator in advanced_indicators if any(indicator in f.lower() for f in features))
        complexity_score -= sum(1 for indicator in basic_indicators if any(indicator in f.lower() for f in features))
