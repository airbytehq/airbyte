...
/**
 * Convenience wrapper around a thread-safe BlockingQueue. Keeps track of available ports for Kube Pod Processes.
 *
 * Although this data structure can do without the wrapper class, this class allows easier testing via the {@link #getNumAvailablePorts()} function.
 *
 * The singleton pattern clarifies that only one copy of this class is intended to exist per scheduler deployment.
 */
public class KubePortManagerSingleton {
  ...
  /**
   * Sets up the port range; make sure init(ports) is called once prior to repeatedly using getInstance(). Init won't fail (it will perform a no-op)
   * if re-initializd with the same set of ports.
   */
