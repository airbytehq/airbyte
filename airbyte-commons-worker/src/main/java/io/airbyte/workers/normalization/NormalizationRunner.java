...
public interface NormalizationRunner extends AutoCloseable {
  ...
  /**
   * After this method is called, the caller must call close. Previous to this method being called a NormalizationRunner can be instantiated and not
   * worry about close being called.
   *
  ...
  /**
   * Prepare a configured folder to run dbt commands from (similar to what is required by normalization models) However, this does not run the
   * normalization file generation process or dbt at all. This is pulling files from a distant git repository instead of the dbt-project-template.
   *
   * @return true if configuration succeeded. otherwise false.
   * @throws Exception - any exception thrown from configuration will be handled gracefully by the caller.
   */
