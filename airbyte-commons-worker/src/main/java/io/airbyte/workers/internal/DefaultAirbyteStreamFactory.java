...
//        .flatMap(this::toAirbyteMessage)
  ...
  protected Stream<AirbyteMessage> parseJson(final String line) {
//    final Optional<JsonNode> jsonLine = Jsons.tryDeserialize(line);
    final Optional<AirbyteMessage> jsonLine = Jsons.tryDeserialize(line, AirbyteMessage.class);
    if (jsonLine.isEmpty()) {
      ...
      try (final var mdcScope = containerLogMdcBuilder.build()) {
        logger.info("Unable to deserialise into AB message: {}", line);
      }
    }
