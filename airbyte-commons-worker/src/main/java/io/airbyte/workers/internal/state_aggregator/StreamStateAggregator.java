...
class StreamStateAggregator implements StateAggregator {
  ...
  public void ingest(final AirbyteStateMessage stateMessage) {
    /**
     * The destination emit a Legacy state in order to be retro-compatible with old platform. If we are running this code, we know that the platform
     * has been upgraded and we can thus discard the legacy state. Keeping the legacy state is causing issue because of its size
