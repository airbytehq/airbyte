...
# Excludes .gitbook paths and /integrations/ paths
...
find /home/ubuntu/repos/airbyte/docs/integrations -type f -name "*.md" | while read -r file; do
  ...
  # Fix absolute links with pattern /release_notes/file.md
  sed -i 's|\(\[.*\]\s*(\)/release_notes/|\1/platform/release_notes/|g' "$file"
