...
public class DockerImageValidator {
   ...
   */
  public void assertValidIntegrationImage(final String dockerRepository, final String imageTag)
      throws BadObjectSchemaKnownException {
    // Validates that the docker image exists and can generate a compatible spec by running a
    // getSpec
    ...
    try {
      ...
      throw new BadObjectSchemaKnownException(
          String.format(
              "Encountered an issue while validating input docker image (%s): %s",
