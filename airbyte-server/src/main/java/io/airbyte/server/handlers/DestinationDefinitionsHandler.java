...
public class DestinationDefinitionsHandler {
  ...
  public DestinationDefinitionsHandler(
  ...
  @VisibleForTesting
  static DestinationDefinitionRead buildDestinationDefinitionRead(
    ...
    try {
      ...
      throw new InternalServerKnownException(
          "Unable to process retrieved latest destination definitions list", e);
    }
  }
  ...
  private static DestinationDefinitionReadList toDestinationDefinitionReadList(
      final List<StandardDestinationDefinition> defs) {
    final List<DestinationDefinitionRead> reads =
        defs.stream()
  ...
  private List<StandardDestinationDefinition> getLatestDestinations() {
    try {
      ...
      throw new InternalServerKnownException(
          "Request to retrieve latest destination definitions failed", e);
    }
  }
  ...
  public DestinationDefinitionRead updateDestinationDefinition(
      ...
      throws ConfigNotFoundException, IOException, JsonValidationException {
    final StandardDestinationDefinition currentDestination =
        configRepository.getStandardDestinationDefinition(
            destinationDefinitionUpdate.getDestinationDefinitionId());
    imageValidator.assertValidIntegrationImage(
        currentDestination.getDockerRepository(), destinationDefinitionUpdate.getDockerImageTag());

    final StandardDestinationDefinition newDestination =
        new StandardDestinationDefinition()
