...
/**
 * This class implements two {@code filter()} methods that execute as part of the Jersey framework request/response chain.
 * <p>
 * The first {@code filter()} is the Request Filter. It takes an incoming {@link ContainerRequestContext} that contains request information, such as
 * the request body. In this filter, we extract the request body and store it back on the request context as a custom property. We don't write any
 * logs for requests. However, since we want to include the request body in logs for responses, we have to extract the request body in this filter.
 * <p>
 * The second @{code filter()} is the Response Filter. It takes an incoming {@link ContainerResponseContext} that contains response information, such
 * as the status code. This method also has read-only access to the original {@link ContainerRequestContext}, where we set the request body as a
 * custom property in the request filter. This is where we create and persist log lines that contain both the response status code and the original
 * request body.
 */
