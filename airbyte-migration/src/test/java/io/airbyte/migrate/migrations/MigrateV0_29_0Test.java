...
public class MigrateV0_29_0Test {
  ...
  private static final UUID NEW_WORKSPACE_ID =
      UUID.fromString("763a9b47-f5fa-45c0-8caa-00b51f108183");
  ...
  private static final ResourceId SOURCE_DEF_RESOURCE_ID =
      ResourceId.fromConstantCase(ResourceType.CONFIG, "STANDARD_SOURCE_DEFINITION");
  private static final ResourceId DEST_DEF_RESOURCE_ID =
  ...
  void testMigration() throws IOException {
    final MigrationV0_29_0 migration =
        new MigrationV0_29_0(Migrations.MIGRATION_V_0_28_0, () -> NEW_WORKSPACE_ID);

    final Map<ResourceId, Stream<JsonNode>> inputConfigs =
        ImmutableMap.<ResourceId, Stream<JsonNode>>builder()
            .put(
                SOURCE_DEF_RESOURCE_ID,
                getResourceStream(INPUT_CONFIG_PATH + "/SOURCE_DEFINITION.yaml"))
            .put(
                DEST_DEF_RESOURCE_ID,
                getResourceStream(INPUT_CONFIG_PATH + "/DESTINATION_DEFINITION.yaml"))
            .put(
    ...
    final Map<ResourceId, List<JsonNode>> expectedOutputOverrides =
        ImmutableMap.<ResourceId, List<JsonNode>>builder()
            .put(
                SOURCE_DEF_RESOURCE_ID,
                getResourceList(INPUT_CONFIG_PATH + "/SOURCE_DEFINITION.yaml"))
            .put(
                DEST_DEF_RESOURCE_ID,
                getResourceList(INPUT_CONFIG_PATH + "/DESTINATION_DEFINITION.yaml"))
            .put(
                WORKSPACE_RESOURCE_ID,
                getResourceList(OUTPUT_CONFIG_PATH + "/STANDARD_WORKSPACE.yaml"))
            .put(
                SOURCE_RESOURCE_ID, getResourceList(OUTPUT_CONFIG_PATH + "/SOURCE_CONNECTION.yaml"))
            .put(
                DESTINATION_RESOURCE_ID,
                getResourceList(OUTPUT_CONFIG_PATH + "/DESTINATION_CONNECTION.yaml"))
            .put(
                CONNECTION_RESOURCE_ID, getResourceList(OUTPUT_CONFIG_PATH + "/STANDARD_SYNC.yaml"))
            .put(
                OPERATION_RESOURCE_ID,
                getResourceList(OUTPUT_CONFIG_PATH + "/STANDARD_SYNC_OPERATION.yaml"))
            .build();
  ...
  private void assertExpectedOutput(
      final Map<ResourceId, List<JsonNode>> expected,
      final Map<ResourceId, List<JsonNode>> actual) {
    ...
    expected.forEach(
        (key, value) ->
            assertEquals(
                value,
                actual.get(key),
                String.format("Resources output do not match for %s:", key.getName())));
