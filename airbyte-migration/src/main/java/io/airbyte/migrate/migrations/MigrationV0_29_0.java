...
public class MigrationV0_29_0 extends BaseMigration implements Migration {
  ...
  private static final ResourceId WORKSPACE_RESOURCE_ID =
  ...
  private static final ImmutableSet<ResourceId> RESOURCE_WITH_WORKSPACE_ID =
      ImmutableSet.of(
  ...
  private static final Set<String> NAMESPACE_DEFINITIONS =
      ImmutableSet.of("source", "destination", "customformat");
  ...
  public Map<ResourceId, JsonNode> getOutputSchema() {
    ...
    outputSchema.put(
        ...
        MigrationUtils.getSchemaFromResourcePath(RESOURCE_PATH, CONNECTION_RESOURCE_ID));
    return outputSchema;
  }
  ...
  public void migrate(
    ...
    for (final Map.Entry<ResourceId, Stream<JsonNode>> entry : inputData.entrySet()) {
      ...
      entry
          ...
          .forEach(
              r -> {
                // if standard sync make sure namespaceDefinition is set. it is now a required
                // property. default to
                // destination.
                if (entry.getKey().equals(CONNECTION_RESOURCE_ID)) {
                  if (!r.hasNonNull("namespaceDefinition")
                      || !NAMESPACE_DEFINITIONS.contains(r.get("namespaceDefinition").asText())) {
                    LOGGER.info(
                ...
                // if the resource contains a workspace id. figure out if it is the default one and
                // switch it.
