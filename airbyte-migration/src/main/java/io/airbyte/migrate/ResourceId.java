...
public class ResourceId {
  ...
  /**
   * Constructs a {@link ResourceId} from a {@link ResourceType} and a path. Assumes the file name
   * is snake case (case agnostic). The file name (without the yaml extension) is used as the name.
   * If the input file is not a yaml, this will break.
   *
  ...
  public static ResourceId fromRecordFilePath(final ResourceType resourceType, final Path path) {
    // we can change this precondition in the future if our scheme changes, but for now fail loudly
    // when
    // we get unexpected paths.
    Preconditions.checkState(
        path.getFileName().toString().endsWith(".yaml"),
        "ResourceId constructor expected yaml file.");

    return new ResourceId(
        resourceType, path.getFileName().toString().replace(".yaml", "").toUpperCase());
  }
  ...
  private ResourceId(final ResourceType type, final String name) {
    Preconditions.checkArgument(
        name.matches("^[A-Z][A-Z0-9]*(_[A-Z0-9]+)*$"),
        "name must be constant case. e.g MY_CONSTANT.");
    this.type = type;
    this.name = name;
  }
  ...
  public String toString() {
    return "ResourceId{" + "type=" + type + ", name='" + name + '\'' + '}';
  }
}
