...
public class MigrationUtils {
   ...
   */
  public static Map<ResourceId, JsonNode> getNameToSchemasFromResourcePath(
      final Path migrationResourcePath,
      final Path relativePath,
      final ResourceType resourceType,
      final Set<String> schemasToInclude) {
    return getNameToSchemasFromResourcePath(
        migrationResourcePath.resolve(relativePath), resourceType, schemasToInclude);
  }

  public static Map<ResourceId, JsonNode> getNameToSchemasFromResourcePath(
      final Path pathToSchemasResource,
      final ResourceType resourceType,
      final Set<String> schemasToInclude) {
    final Map<ResourceId, JsonNode> schemas = new HashMap<>();
    final Path pathToSchemas =
        JsonSchemas.prepareSchemas(pathToSchemasResource.toString(), MigrationUtils.class);
    FileUtils.listFiles(pathToSchemas.toFile(), null, false).stream()
        .map(JsonSchemaValidator::getSchema)
        .filter(j -> schemasToInclude.contains(getTitleAsConstantCase(j)))
        .forEach(
            j -> {
              final ResourceId resourceId =
                  ResourceId.fromConstantCase(resourceType, getTitleAsConstantCase(j));
  ...
  public static JsonNode getSchemaFromResourcePath(
      final Path pathToSchema, final ResourceId resourceId) {
    final Map<ResourceId, JsonNode> nameToSchemas =
        getNameToSchemasFromResourcePath(
            pathToSchema, resourceId.getType(), Collections.singleton(resourceId.getName()));
  ...
  public static Map<ResourceId, JsonNode> getConfigModels(
      final Path migrationResourcePath, final Set<String> schemasToInclude) {
    return getNameToSchemasFromResourcePath(
        migrationResourcePath,
        ResourceType.CONFIG.getDirectoryName(),
        ResourceType.CONFIG,
        schemasToInclude);
  }

  public static Map<ResourceId, JsonNode> getJobModels(
      final Path migrationResourcePath, final Set<String> schemasToInclude) {
    return getNameToSchemasFromResourcePath(
        migrationResourcePath,
        ResourceType.JOB.getDirectoryName(),
        ResourceType.JOB,
        schemasToInclude);
  }
   ...
   */
  public static void registerMigrationRecord(
    ...
    metadataOutputConsumer.accept(
        Jsons.jsonNode(
            ImmutableMap.of(
                "key", String.format("%s_migrate_to", current_timestamp()), "value", version)));
  }
  ...
  public static Map<ResourceId, Consumer<JsonNode>> mapRecordConsumerToConsumer(
      final Map<ResourceId, ? extends Consumer<JsonNode>> recordConsumers) {
    return recordConsumers.entrySet().stream()
