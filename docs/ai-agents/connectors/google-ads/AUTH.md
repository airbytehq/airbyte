...
| `replication_config.customer_id` | `str` | Yes | Comma-separated list of Google Ads customer IDs (10 digits each, no dashes). |
| `replication_config.start_date` | `str (date)` | No | UTC date in YYYY-MM-DD format from which to start replicating data. Defaults to 2 years ago if not specified. |
| `replication_config.conversion_window_days` | `int` | No | Number of days for the conversion attribution window. Default is 14. (default: `14`) |
