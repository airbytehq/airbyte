...
Use the Confluence connector through the Airbyte Agent CLI, the Python SDK, or the API.
...
agent = Agent(name="Confluence Assistant", tools=openai_tools)
```
...
When you need custom tool bodies — or a framework without native support — use `ConfluenceConnector.agent_tool`. Register execute, inspect, and docs together so the agent can fetch connector guidance progressively. Pass the framework explicitly when it has a supported failure strategy:
...
@agent.tool_plain
@ConfluenceConnector.agent_tool(
    framework="pydantic_ai",
    inspect_tool="confluence_inspect",
    docs_tool="confluence_read_docs",
)
async def confluence_execute(entity: str, action: str, params: dict | None = None):
    return await connector.execute(entity, action, params or {})

@agent.tool_plain
@ConfluenceConnector.agent_tool(framework="pydantic_ai")
async def confluence_inspect():
    return await connector.inspect_connector()

@agent.tool_plain
@ConfluenceConnector.agent_tool(framework="pydantic_ai")
async def confluence_read_docs(section: str | None = None):
...
handlers = {
    fn.__name__: fn
    for fn in (confluence_inspect, confluence_read_docs, confluence_execute)
}
...
These examples are kept for existing integrations. The deprecated `ConfluenceConnector.tool_utils` pattern loads the connector's full generated catalog into one broad `execute` tool description instead of letting the agent read skill docs on demand. For new code, use `build_connector_tools` or `ConfluenceConnector.agent_tool` above.
