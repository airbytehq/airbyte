...
| Jobs | [List](#jobs-list), [Get](#jobs-get), [Context Store Search](#jobs-context-store-search), [Semantic Search](#jobs-semantic-search) |
...
| Job Posts | [List](#job-posts-list), [Get](#job-posts-get), [Context Store Search](#job-posts-context-store-search), [Semantic Search](#job-posts-semantic-search) |
...
|------------|---------------------|-------------|
| `notes` | 2048 | Additional notes or comments about the job |
...
| `internal_content` | `string` | The job post as written for the internal job board, present only when it differs from the external one. Semantically searchable; HTML, same as `content`. |
...
| `data[].internal_content` | `string` | The job post as written for the internal job board, present only when it differs from the external one. Semantically searchable; HTML, same as `content`. |
...
| Field Name | Max Context (chars) | Description |
|------------|---------------------|-------------|
| `content` | 2048 | Content or description of the job post. |
| `internal_content` | 2048 | The job post as written for the internal job board, present only when it differs from the external one. Semantically searchable; HTML, same as `content`. |
...
| `data[].entity.location` | `string` | Source record field |
