...
This section is for developers who want to build agents powered by Airbyte Agents. Pick a path based on how you prefer to work.

```mermaid
flowchart TD
    START(["Build my own agent"])
    START -->|"I write the code myself"| TUT["Tutorials"]
    START -->|"I use a coding agent"| SKILL["Skills"]
...
Step-by-step guides that take you from an empty project to a working agent in about 15 minutes. Each tutorial uses a different Python framework, but all of them follow the same pattern: create a project, install the Airbyte Agent SDK, connect to a data source, and execute operations.
...
Skills are pre-packaged instructions you install into a coding agent so it can generate working code against Airbyte Agents. Instead of writing the integration yourself, you install a skill and prompt the agent to build with Airbyte.
