...
1. **Add credentials**: Provide the third-party service's credentials through the [web app](../../interfaces/ui/add-connector) or [API](../../interfaces/api/add-connector). Airbyte stores the credentials and returns a connector ID.
...
Credentials you add through one interface are available to all of them. A connector configured in the web app works through the API, SDK, or Airbyte Agent MCP without re-entering credentials. This applies to every interface in the platform.
