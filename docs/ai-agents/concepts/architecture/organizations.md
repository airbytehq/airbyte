...
- **Platform API credentials**: A `client_id`, `client_secret`, and `organization_id` that authenticate programmatic access through the [API](../../interfaces/api), [SDK](../../interfaces/sdk), and [Airbyte Agent MCP](../../interfaces/mcp). Find these on the Profile page at [app.airbyte.ai](https://app.airbyte.ai).
