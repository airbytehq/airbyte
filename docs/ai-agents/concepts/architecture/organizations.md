...
- **Platform API credentials**: A `client_id`, `client_secret`, and `organization_id` that authenticate programmatic access through the [API](../../interfaces/api), [SDK](../../interfaces/sdk), and [MCP server](../../interfaces/mcp). Find these at [app.airbyte.ai](https://app.airbyte.ai).
