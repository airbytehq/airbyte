...
Airbyte-managed agents include chats, Automation runs, and the Automation Builder chat. These surfaces can consume AOs for both tool calls and reasoning. If you bring your own agent through the MCP, the API, or the SDK, Airbyte doesn't charge for your agent's reasoning. Those surfaces consume AOs for tool calls only. Connector calls made locally through the open source SDK don't consume AOs.
