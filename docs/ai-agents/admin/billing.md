...
The Usage panel on the Billing page shows how your activity consumes AOs and tool calls over time. Chat, Automation, and Automation Builder Chat usage includes AOs from tool calls and reasoning. MCP, API, and SDK usage includes AOs from hosted tool calls only.
...
Rows from Chat, Automation, and Automation Builder Chat link to the originating session, so you can investigate the tool calls and reasoning that drove a particular spike.
