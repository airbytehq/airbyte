...
These hosted tool calls still consume AOs and appear in your [Usage panel](./billing.md#monitor-usage), but Airbyte doesn't charge for the reasoning your own agent performs and doesn't create a corresponding Sessions row. To review them, open the Usage panel on the Billing page and filter by the **MCP**, **API**, or **SDK** source.
