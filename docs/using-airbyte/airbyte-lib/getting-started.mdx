...
airbyte-lib is a library that provides a set of utilities to use Airbyte connectors in Python. It is meant to be used in situations where setting up an Airbyte server or cloud account is not possible or desirable,
for example in a Jupyter notebook or when iterating on early prototypes in a basic local Python script.
...
For all the details, please refer to the [API reference](./reference).
...
airbyte-lib is a python library that can be run in any context that supports Python >=3.10. It contains the following main components:
