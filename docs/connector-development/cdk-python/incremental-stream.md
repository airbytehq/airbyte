...
The `AirbyteStateMessage` persists state between syncs, and allows a new sync to pick up from where the previous sync last finished. See the [incremental sync guide](/understanding-airbyte/connections/incremental-append) for more information.
