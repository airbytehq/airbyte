import NewStreamModal from './assets/tutorial-new-stream.png';
import SchemaTab from './assets/tutorial-schema.png';
import InputModal from './assets/tutorial-user-input.png';
...
<img src={NewStreamModal} width="400" alt="Stream modal" />
...
![Testing values](./assets/tutorial-testing-values.png)
...
<img src={InputModal} width="400" alt="User input modal" />

The value input is now set to `{{ config['base'] }}`. When making requests, the connector will replace this placeholder by the user supplied value. This syntax can be used in all fields that have a user icon on the right side.
...
![Successful sync](./assets/tutorial-sync.png)
