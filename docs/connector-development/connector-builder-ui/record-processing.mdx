...
For the example above, the CSV response format would result in the following normalized JSON output:
...
Some APIs respond with a compressed ZIP or gzip file. This is more common when you're dealing with large, reporting-based datasets. Amplitude's [Export API](https://amplitude.com/docs/apis/analytics/export) is an excellent example. Ocasionally, compressed zip files are nested within each other, like a zip file that contains gzip files, which themselves contain more files.
