...
To implement `discover`, we'll: 1. Add a method `discover` in `source.py` which outputs the Catalog 2. Extend the arguments parser to use detect the `discover --config <config_path>` command and call the `discover` method.
