...
- **Client-supplied authentication headers are stripped**: This closes a path to cross-tenant authorization. Previously, the server added a derived workspace or organization header only when the client had not sent one, so a client-supplied header could survive into authorization. Internal authentication headers are now removed before the server derives trusted identity headers.

- **Invitation codes no longer exposed**: Administrative invitation reads and cancellations reference invitations by ID and return an admin-safe payload, so invite codes aren't disclosed. The `/v1/user_invitations/create`, `/v1/user_invitations/list_pending`, and `/v1/user_invitations/cancel` responses no longer include `inviteCode`; `cancel` still accepts `inviteCode`.
