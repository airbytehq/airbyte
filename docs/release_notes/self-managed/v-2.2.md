...
- **Google OAuth setup for Gmail and Google Calendar sources**: Gmail now shows a Google-branded sign-in button, and the OSS OAuth factory registers `DeclarativeOAuthFlow` for both connectors. These platform registrations do not make Google Calendar OAuth complete; the connector version's specification determines whether OAuth is available.
