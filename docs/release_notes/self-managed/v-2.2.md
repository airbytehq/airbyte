...
This upgrade is one-way. The 2.2.0 migrations drop the `workload_label` table and unused template and orchestration tables. On Community defaults, Airbyte 2.1.x writes to `workload_label` on every workload create and LEFT JOINs it on every queue poll; both governing flags default to `false`. Restoring 2.1.x images against a database that has run the 2.2.0 migration stops syncs, and the dropped data cannot be recovered. Take a database backup before upgrading; rolling back to 2.1.x is not supported.
...
Community deployments no longer accept HTTP Basic credentials on the API. Scripts or integrations that use `curl -u` break after upgrading even when you make no change to your Helm values. Switch those callers to an access token from `/api/v1/applications/token`.
:::
...
- **Correct CPU parsing when sizing sockets**: For connections that run over the socket transport, the workload launcher now parses whole-core Kubernetes CPU limits expressed in millicores, such as `4000m`, correctly when calculating socket counts. That value now allocates 8 sockets instead of 2.
...
- **Google OAuth setup for Gmail and Google Calendar sources**: Gmail now shows a Google-branded sign-in button, the OSS OAuth factory registers `DeclarativeOAuthFlow` for both connectors, and the public API recognizes the names `gmail`, `google-calendar`, and `linear`. These platform registrations do not make Google Calendar OAuth complete; the connector version's specification determines whether OAuth is available.
...
- **Failed notification attempts are measured**: Notification metrics now distinguish failed attempts from successful ones, so a failed Slack or webhook notification is visible in metrics.
...
- **Invitation codes no longer exposed**: Administrative invitation reads and cancellations reference invitations by ID and return an admin-safe payload, so invite codes aren't disclosed. The `/v1/user_invitations/create` response no longer includes `inviteCode`, so only readers of that create response must be updated; `cancel` still accepts `inviteCode`.
