...
Community deployments no longer accept HTTP Basic credentials on the API. Scripts or integrations that use `curl -u` break after upgrading unless you change your values. Switch those callers to an access token from `/api/v1/applications/token`.
:::
...
- **Google OAuth setup for Gmail and Google Calendar sources**: Gmail now shows a Google-branded sign-in button, the OSS OAuth factory registers `DeclarativeOAuthFlow` for both connectors, and the public API recognizes the names `gmail` and `google-calendar`. These platform registrations do not make Google Calendar OAuth complete; the connector version's specification determines whether OAuth is available.
...
- **No browser credential prompt on community deployments**: Unauthenticated requests to a community-edition deployment no longer advertise or accept HTTP Basic credentials, which caused browsers to pop up a native username and password dialog. Callers must use an access token instead.
