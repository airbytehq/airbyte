...
MinIO's Docker Hub image has vanished from the face of the registry, and with it your storage layer turned to the dark side. Airbyte 2.3 brings balance back to the pod. Airbyte version 2.3 was released on September 15, 2026.
...
- **MinIO image pulled from Airbyte's registry**: The default `minio.image.repository` is now `airbyte/minio`, a copy of the same image mirrored under Airbyte's Docker Hub organization. See [Action required before upgrading](#action-required-before-upgrading) if you use a private registry.
