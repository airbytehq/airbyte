...
- **Secret coordinates rejected on write**: Source and destination configuration writes no longer accept `secret_coordinate::` values as input. Coordinates returned from a read are pointers into Airbyte-managed secret storage, and accepting them on write allowed a caller to reference a secret they didn't own.

- **Keycloak updated to 26.7.3**: The bundled Keycloak image moves from 26.2 to 26.7.3, which includes a number of CVE fixes and resolves an admin API slowdown on instances with many realms. This is a patch release with no schema change.
