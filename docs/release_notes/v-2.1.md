...
- **PostHog integration removed**: All PostHog analytics integration has been removed from the frontend, backend, and Helm chart. Any PostHog-related environment variables in your configuration can be safely removed.
...
- **DB2 enterprise source connector**: Added entitlement definition for source-db2. Customers with the appropriate Enterprise license can now configure the DB2 source connector.

- **Enterprise connector filtering**: Fixed an issue where enabled enterprise connectors were incorrectly filtered out of the connector list after activation.

- **Enterprise connector pro-features modal**: The pro-features modal now correctly appears for available enterprise connectors, rather than only for connectors that have not yet been enabled.

- **Salesforce enterprise connector definition**: Corrected the `actorDefinitionId` for the Salesforce destination enterprise connector entitlement.
...
- **Workspace tombstone check before discover**: Discovery operations now verify that the target workspace has not been deleted before proceeding, preventing unnecessary work and confusing errors.
...
- **`additionalStats` fully implemented in `SyncStats`**: The `additionalStats` field now properly participates in equality checks, hash codes, and string representations, fixing potential inconsistencies in stats reporting.
...
- **User settings navigation in OSS and Enterprise**: Fixed broken navigation to user settings in OSS and Enterprise deployments with authentication enabled. The sidebar link now correctly opens the account settings page.
...
- **Plan naming update**: Updated UI text from "Cloud/Teams" to "Standard/Pro" to reflect current plan naming.
...
- **SSO draft configuration handling**: Improved handling of draft SSO configurations, including proper Keycloak realm cleanup and user preservation when SSO setup is abandoned or restarted.

- **SSO access code lifespan**: Fixed SSO login failures caused by expired access codes by setting a 3-minute access code lifespan.

- **SSO activation for existing users**: SSO activation now permits existing users whose email matches the verified SSO domain, rather than requiring new account creation.

- **SSO config activation validation**: Loosened SSO configuration validation to allow activation when the configuration has been successfully read, fixing cases where valid SSO setups could not be activated.

- **Domain verification uniqueness**: Fixed the organization domain verification uniqueness constraint to properly ignore deleted records, preventing conflicts when re-verifying a domain.

- **Role management simplification**: Simplified user removal conditions in role management, fixing edge cases where users could not be properly removed from organizations.

- **Set-Cookie header serialization**: Fixed cookie serialization to use Netty's `ServerCookieEncoder`, ensuring proper `Set-Cookie` header formatting and preventing potential authentication issues.

- **Reduced redundant auth calls**: Reduced redundant calls to `users/get_or_create_by_auth_id` on webapp load, improving initial page load performance.
