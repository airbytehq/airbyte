...
We go fast so you don't get furious. Airbyte version 2.0 was released on October 14, 2025. This version marks a major achievement in Airbyte's platform architecture. We're excited to share new improvements and changes to the Airbyte platform.
...
## A better Connector Builder interface
...
## A better organizations and workspaces interface
...
Version 2.0 is the final version of Airbyte that supports Helm chart V1. Starting in version 2.1, Airbyte will only support Helm Chart V2.
...
- **Ingress**: Airbyte now ships ingress with the Helm chart. If you already have ingress set up, you can continue using your existing configuration. For new deployments, you might find that it's easier to enable ingress through values.yaml. [**Learn&nbsp;more&nbsp;>**](/platform/deploying-airbyte/integrations/ingress)
...
- **Workspaces**: disabled email validation when adding new users to a workspace. If you're a Self-Managed Enterprise customer using SSO, you can now add users who have user IDs that aren't email addresses.
...
- Fixed alignment issues for timestamps in the Connection Timeline UI.

- Fixed a situation in which the enable/disable toggles didn't work on the Connections home page.

- Fixed a rare situation in which turning a connection off and then on again could cause a reset.
...
Airbyte is an open source product. We win together, and we've come a long way together. Here are some of the main enhancements that landed in 2025. Thank you for being on this journey with us. We're just getting started.
