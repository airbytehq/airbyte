...
In order to protect these sensitive values, the Helm Chart assumes that these values are pre-configured and stored in a Kubernetes Secret *before* the Helm installation begins. Each [integration](#deploying-airbyte/integrations)  will provide the Secret values that are required for the specific integration.
...
Each [Integrations](#deploying-airbyte/integrations) will provide a section of the specific values that you should override and provide examples of what the values should look like. An example `values.yaml` file might look something like:
