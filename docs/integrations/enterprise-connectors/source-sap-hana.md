...
# python cdc_setup.py --host <HANA_HOST> --port <PORT> --user <USER> --password <PASSWORD> --schema <SOURCE_SCHEMA>
# ===============================================
...
def create_cdc_table(conn, cdc_schema, cdc_table, columns_with_types):
    ...
    try:
        ...
        if e.errorcode in [288, 321]:
            print(f"CDC table {cdc_schema}.{cdc_table} already exists. Skipping.")
        else:
...
def create_single_trigger(conn, operation_type, source_schema, source_table, cdc_schema, cdc_table, columns_with_types):
    ...
    if operation_type == 'INSERT':
        referencing = 'REFERENCING NEW ROW AS NEW'
    elif operation_type == 'UPDATE':
        referencing = 'REFERENCING OLD ROW AS OLD, NEW ROW AS NEW'
    elif operation_type == 'DELETE':
        referencing = 'REFERENCING OLD ROW AS OLD'
    else:
    ...
    for col in columns_with_types:
        ...
        if operation_type in ['INSERT', 'UPDATE']:
            ...
            values.append(f':NEW."{safe_col}"' if operation_type != 'UPDATE' else f':NEW."{safe_col}"')
        if operation_type in ['UPDATE', 'DELETE']:
            columns.append(f'"_ab_trigger_{safe_col}_before"')
            values.append(f':OLD."{safe_col}"')
    ...
    ddl = f"""
        ...
        FOR EACH ROW
        BEGIN
            INSERT INTO "{cdc_schema}"."{cdc_table}" ({columns_str})
            VALUES ({values_str});
        END;
    """
...
def get_tables_from_schema(conn, schema):
    ...
    cursor = conn.cursor()
    query = "SELECT TABLE_NAME FROM SYS.TABLES WHERE SCHEMA_NAME = ? AND TABLE_TYPE = 'COLUMN'"
    tables = []
    try:
...
def main():
    ...
    for table in tables:
        ...
        for op in ['INSERT', 'UPDATE', 'DELETE']:
            create_single_trigger(conn, op, source_schema, source_table, args.cdc_schema, cdc_table, columns)
...
- `--cdc-schema`: CDC schema name (default: `_ab_cdc`)
