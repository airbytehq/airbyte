...
- A Redshift cluster ([provisioned](https://docs.aws.amazon.com/redshift/latest/gsg/new-user.html) or [serverless](https://docs.aws.amazon.com/redshift/latest/gsg/new-user-serverless.html))
...
3. [Create an IAM user](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_users_create.html) (or
   use an existing one) with [read and write permissions](https://docs.aws.amazon.com/IAM/latest/UserGuide/reference_policies_examples_s3_rw-bucket.html)
...
For reference, the legacy raw table contained these columns:

- `_airbyte_raw_id`: A UUID assigned by Airbyte to each event. Column type: `VARCHAR`.
- `_airbyte_extracted_at`: A timestamp representing when the event was pulled from the data source. Column type: `TIMESTAMP WITH TIME ZONE`.
- `_airbyte_loaded_at`: A timestamp representing when the row was processed into the final table. Column type: `TIMESTAMP WITH TIME ZONE`.
- `_airbyte_data`: A JSON blob representing the event data. Column type: `SUPER`.
- `_airbyte_meta`: A JSON object containing metadata about the record. Column type: `SUPER`.
- `_airbyte_generation_id`: An identifier for the generation of the sync. Column type: `BIGINT`.
...
Redshift has size limits on certain data types that affect how Airbyte handles records:

- **VARCHAR**: Maximum 65,535 bytes. Values exceeding this limit are nulled out, and the `_airbyte_meta` column records the change.
- **SUPER**: Maximum 16 MB per record. When a record exceeds this limit, the connector nulls the entire record while attempting to preserve the Primary Keys and cursor field declared in your stream configuration.

For more details, see the AWS documentation on [SUPER type](https://docs.aws.amazon.com/redshift/latest/dg/r_SUPER_type.html) and [SUPER limitations](https://docs.aws.amazon.com/redshift/latest/dg/limitations-super.html).
...
Airbyte Redshift destination will create tables and schemas using Unquoted
identifiers when possible, or fall back to Quoted Identifiers if the names contain special
characters.
...
1. [Allow connections](https://aws.amazon.com/premiumsupport/knowledge-center/cannot-connect-redshift-cluster/) from Airbyte to your Redshift cluster (if they exist in separate VPCs).
2. Configure an SSH Bastion Host (see [Step 2](#optional-ssh-bastion-host)) to tunnel through to the private cluster.
3. For Airbyte Cloud, ensure the [Airbyte IP addresses](https://docs.airbyte.com/platform/using-airbyte/configuring-connections/configuring-the-connection#allow-list-ip-addresses) are allowed in your Redshift cluster's security group and network policy.

### 'S3 access denied' during staging

Ensure your IAM credentials have [read and write permissions](https://docs.aws.amazon.com/IAM/latest/UserGuide/reference_policies_examples_s3_rw-bucket.html) to the staging S3 bucket. Verify that:
...
### Record exceeds SUPER size limit

Redshift has a 16 MB limit on SUPER type columns and a 65,535-byte limit on VARCHAR fields. If records are being nulled, check the `_airbyte_meta` column for details about which fields were affected. See [Precision and size limits](#precision-and-size-limits) for more details.
...
| :------ | :--------- | :--------------------------------------------------------- | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 4.0.0 | 2026-04-08 | [76168](https://github.com/airbytehq/airbyte/pull/76168) | V2 rewrite: new CDK architecture with direct final-table writes, aggressive connection checker, improved error messages |
