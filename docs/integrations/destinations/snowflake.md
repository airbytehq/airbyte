...
Setting up the Snowflake destination connector involves setting up a Snowflake entities (warehouse, database, schema, user, and role) in the Snowflake console, then setting up the data loading method (internal stage, AWS S3, GCS bucket, or Azure Blob Storage), and then configuring the Snowflake destination connector using the Airbyte UI.  

This page guides you step-by-step through the process of setting up the Snowflake destination connector.
...
The Snowflake destination supports the following snc modes:
