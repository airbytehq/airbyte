...
|:----------------|:-----------|:-----------------------------------------------------------|:---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 4.1.3           | 2026-09-01 | [84953](https://github.com/airbytehq/airbyte/pull/84953)   | Add missing Airbyte meta columns (`_airbyte_meta`, `_airbyte_generation_id`) to tables created by connector versions prior to 3.10.0 when the `AIRBYTE_DESTINATION_SNOWFLAKE_META_COLUMN_REPAIR` env var is set to `true` (off by default), fixing "invalid identifier" sync failures after upgrading to 4.x. Upgrade CDK to 1.0.25. |
