...
2. Open your SQL warehouse:
   ...
   ![](/platform/.gitbook/assets/destination/databricks/databricks_open_sql_warehouse.png)

3. Open the Connection Details tab:

   ![](/platform/.gitbook/assets/destination/databricks/databricks_sql_warehouse_connection_details.png)
...
2. Click on your icon in the top-right corner, and head to `settings`, then `developer`, then `manage` under `access tokens`
   ...
   ![](/platform/.gitbook/assets/destination/databricks/dtabricks_token_user_new.png)

3. Enter a description for the token and how long it will be valid for (or leave blank for a permanent token):

   ![](/platform/.gitbook/assets/destination/databricks/databricks_generate_token.png)
