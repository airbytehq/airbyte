...
### Creating tables as UNLOGGED

Enable the `unlogged_tables` setting to create the final and temporary tables written by Airbyte as
UNLOGGED tables. This can be useful when the destination database also serves as a logical
replication or CDC source, because WAL generated by large full-refresh loads can otherwise bloat
logical replication slots. Unlogged tables are not crash-safe, are not replicated to physical
standbys, and cannot be queried on read replicas; on Amazon Aurora, also set
`rds.logically_replicate_unlogged_tables=0` to exclude them from logical replication. Existing
tables are not altered: only tables created after enabling the setting use UNLOGGED, so
full-refresh overwrite tables are converted on the next sync while append/dedupe tables remain
logged unless you run `ALTER TABLE ... SET UNLOGGED`.
...
|:--------|:-----------|:-----------------------------------------------------------|:------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 3.0.19 | 2026-09-04 | [TBD](https://github.com/airbytehq/airbyte/pull/TBD) | Add optional `unlogged_tables` setting to create tables as UNLOGGED. |
