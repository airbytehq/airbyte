...
You may want to create objects that depend on the tables generated by Airbyte, such as views or rules. If you
do so, be aware that PostgreSQL enforces referential integrity and will block certain operations on tables that have dependent objects.
...
**Alter column type errors:**
```
ERROR: cannot alter type of a column used by a view or rule
```

### Handling drop table errors

When Airbyte needs to drop a table (e.g., during schema changes or refreshes), PostgreSQL will block the operation if views or rules depend on it. To resolve this:

1. Enable the **"Drop tables with CASCADE"** option in your destination configuration
2. This will automatically drop dependent objects when tables are dropped

:::warning

Enabling CASCADE will delete your views and rules. You MUST have a way to recreate them after the sync completes.

:::

### Handling alter column type errors

When source schema changes cause Airbyte to alter a column's type, PostgreSQL will block the operation if views depend on that column. **Unlike DROP operations, PostgreSQL does not support CASCADE for ALTER COLUMN TYPE.** This is a PostgreSQL limitation, not an Airbyte bug.

To work around this limitation:

1. **Drop dependent views before the sync** and recreate them afterward
2. **Use an orchestrator** (like Airflow or Dagster) to automate the drop/recreate workflow
3. **Use dbt** to manage your views - dbt can recreate views after each sync
4. **Consider using materialized views** with a `REFRESH MATERIALIZED VIEW` pattern instead of regular views
5. **Use Append sync mode** if you expect frequent schema changes - this avoids ALTER operations

### Recommended setup

We strongly recommend:

- Using a tool like `dbt` to automate the creation of dependent objects
- Using an orchestrator to trigger `dbt` after Airbyte syncs complete
- Enabling the "Drop tables with CASCADE" option if you have views on Airbyte tables

This ensures your dependent objects are automatically recreated after any sync that modifies the underlying tables.
