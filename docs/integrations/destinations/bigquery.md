...
10. For **CDC deletion mode**, choose how the connector handles records deleted at a CDC-enabled
    source. **Hard delete** (the default) propagates deletions by removing the matching rows from
    the destination table. **Soft delete** keeps the rows and leaves a tombstone record instead, so
    you retain a history of deleted data in the destination.
