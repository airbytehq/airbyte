...
The Airbyte S3 destination allows you to sync data to AWS & Minio S3. Each stream is written to its own directory under the bucket.
...
| :--- | :---: | :--- |
| S3 Endpoint | string | Endpoint of S3 to connect to |
...
1. Allow connections from Airbyte server to your AWS/Minio cluster.
...
* Fill up S3 info
  * **S3 Endpoint**
    * Use 'aws' if want to connect to AWS S3, use URL to Minio Host if want to connect to Minio S3.
  * **S3 Bucket Name**
