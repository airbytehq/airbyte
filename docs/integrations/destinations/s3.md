...
4. Configure the required fields. See the [Configuration Options](#configuration-options) section below for detailed descriptions of each field.
...
#### Access Key ID
**Access Key ID** is used for authentication to the S3 bucket. See [this AWS guide](https://docs.aws.amazon.com/general/latest/gr/aws-sec-cred-types.html#access-keys-and-secret-access-keys) on how to generate an access key.
...
If the Access Key and Secret Access Key are not provided, the authentication will rely either on the Role ARN using STS Assume Role or on the instance profile.
</FieldAnchor>

<FieldAnchor field="secret_access_key">
#### Secret Access Key
**Secret Access Key** is the corresponding secret to the access key ID.

Make sure your S3 bucket is accessible from the machine running Airbyte. This depends on your networking setup.
...
<FieldAnchor field="role_arn">
#### Role ARN
**Role ARN** (Amazon Resource Name) can be used for authentication via STS Assume Role. See [this AWS guide](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_roles_create.html) on how to create a role.
...
<FieldAnchor field="s3_bucket_name">
#### S3 Bucket Name
**S3 Bucket Name** is the name of the S3 bucket where data will be written. See [this AWS guide](https://docs.aws.amazon.com/AmazonS3/latest/userguide/create-bucket-overview.html) to create an S3 bucket.
</FieldAnchor>

<FieldAnchor field="s3_bucket_path">
#### S3 Bucket Path
**S3 Bucket Path** is the subdirectory under the S3 bucket where data will be written.
</FieldAnchor>

<FieldAnchor field="s3_bucket_region">
#### S3 Bucket Region
**S3 Bucket Region** is the region where your S3 bucket is located. See [this AWS guide](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/using-regions-availability-zones.html#concepts-available-regions) for all region codes.
</FieldAnchor>

<FieldAnchor field="s3_endpoint">
#### S3 Endpoint
**S3 Endpoint** is the URL endpoint for your S3 service. Leave empty if using AWS S3, fill in S3 URL if using Minio S3 or other S3-compatible storage.
</FieldAnchor>

### Path and File Configuration

<FieldAnchor field="s3_path_format">
#### S3 Path Format
**S3 Path Format** is an additional string format that defines how data is stored under the S3 Bucket Path. The default value is `${NAMESPACE}/${STREAM_NAME}/${YEAR}_${MONTH}_${DAY}_${EPOCH}_`.

The full path of the output data with the default S3 Path Format is:
...
- `${YEAR}`: Year in which the sync was writing the output data.
- `${MONTH}`: Month in which the sync was writing the output data.
- `${DAY}`: Day in which the sync was writing the output data.
- `${HOUR}`: Hour in which the sync was writing the output data.
- `${MINUTE}`: Minute in which the sync was writing the output data.
- `${SECOND}`: Second in which the sync was writing the output data.
- `${MILLISECOND}`: Millisecond in which the sync was writing the output data.
- `${EPOCH}`: Milliseconds since Epoch in which the sync was writing the output data.
...
#### S3 Filename Pattern
**S3 Filename Pattern** allows you to set the file-name format for the S3 staging file(s). The following placeholder combinations are supported:
- `{date}`
- `{date:yyyy_MM}`
- `{timestamp}`
- `{timestamp:millis}`
...
Do not use empty spaces or unsupported placeholders, as they won't be recognized.
...
**Output Format** determines the format of the data output. Options include:
- CSV (Comma-Separated Values)
- JSONL (JSON Lines: Newline-delimited JSON)
- Avro (Apache Avro)
- Parquet (Columnar Storage)

Each format has its own configuration options for compression and other settings.
</FieldAnchor>
