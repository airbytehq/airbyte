...
9. Three Airbyte specific fields will be added to each Avro record:
   ...
   | `_ab_additional_properties` | `map` of `string` | See below |

10. A Json object can have additional properties of unknown types, which is not compatible with the Avro schema. To solve this problem during Json to Avro object conversion, we introduce a special field: `_ab_additional_properties` typed as a nullable `map` from `string` to `string`:
