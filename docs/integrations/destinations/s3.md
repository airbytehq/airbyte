...
4. Configure fields:
   ...
   - **Use a Staging Directory**
     - If enabled, the connector will stage new data in a temporary directory before deleting
       ...
       become costly for buckets with a large number of objects. Note that it is safe to switch
       off staging at any time; the connector will fall back gracefully to the old strategy.
       However, doing so after a failed sync might leave data stranded in the staging directory.
       Further, reactivating staging a second time without manually clearing the manifest file
