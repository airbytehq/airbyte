...
| Part Size | int | Size of each file part in MBs. Please note that a larger part size will result in larger memory requirements. A rule of thumb is to multiply the part size by 10 to get the memory requirement. Modify this parameter with care. |
...
```
<bucket-name>/<sorce-namespace-if-exists>/<stream-name>/<upload-date>-<upload-mills>.<format-extension>
```

For example:

```
testing_bucket/data_output_path/public/users/2021-01-01-1609541171643.csv
↑              ↑                ↑      ↑     ↑          ↑             ↑
|              |                |      |     |          |             format extension
...
Each stream will be outputted into an CSV file (possibly with many parts). Each CSV file includes at least two Airbyte metadata columns. Depending on the `flattening` config, the data may reside in one column (`_airbyte_data`) when there is no flattening, or multiple columns with root level flattening.
...
* Fill up S3 info
  ...
  * **Part Size**
    * Each Airbyte stream is uploaded to S3 in parts. This determines the size of each part, in MBs. S3 has a limit of 10,000 parts per file. This is 10MB by default. Please note that a larger part size will result in larger memory requirements. A rule of thumb is to multiply the part size by 10 to get the memory requirement. Modify this parameter with care.
