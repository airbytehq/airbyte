...
The Parquet output filename also includes an extension based on the `compression_codec` config. The extension for each algorithm can be found [here](https://github.com/apache/parquet-mr/blob/master/parquet-common/src/main/java/org/apache/parquet/hadoop/metadata/CompressionCodecName.java#L26). For example, when `GZIP` is chosen, the file name will end wil `.parquet.gz`.
...
5. Two Airbyte specific fields will be added to each Parquet record:
