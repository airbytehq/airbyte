...
Each stream will be outputted into an CSV file. Each CSV file includes at least two Airbyte metadata columns. Depending on the `flattening` config, the data may reside in one column (`_airbyte_data`) when there is no flattening, or multiple columns with root level flattening.
