...
#### Access Key ID
Used for authentication to the S3 bucket. See [this AWS guide](https://docs.aws.amazon.com/general/latest/gr/aws-sec-cred-types.html#access-keys-and-secret-access-keys) on how to generate an access key.
...
#### Role ARN
(Amazon Resource Name) can be used for authentication via STS Assume Role. See [this AWS guide](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_roles_create.html) on how to create a role.
...
#### S3 Bucket Name
The name of the S3 bucket where data will be written. See [this AWS guide](https://docs.aws.amazon.com/AmazonS3/latest/userguide/create-bucket-overview.html) to create an S3 bucket.
</FieldAnchor>

<FieldAnchor field="s3_bucket_path">
#### S3 Bucket Path
The subdirectory under the S3 bucket where data will be written.
</FieldAnchor>

<FieldAnchor field="s3_bucket_region">
#### S3 Bucket Region
The region where your S3 bucket is located. See [this AWS guide](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/using-regions-availability-zones.html#concepts-available-regions) for all region codes.
</FieldAnchor>

<FieldAnchor field="s3_endpoint">
#### S3 Endpoint
The URL endpoint for your S3 service. Leave empty if using AWS S3, fill in S3 URL if using Minio S3 or other S3-compatible storage.
</FieldAnchor>
...
#### S3 Path Format
An additional string format that defines how data is stored under the S3 Bucket Path. The default value is `${NAMESPACE}/${STREAM_NAME}/${YEAR}_${MONTH}_${DAY}_${EPOCH}_`.
...
#### S3 Filename Pattern
Allows you to set the file-name format for the S3 staging file(s). The following placeholder combinations are supported:
- `{date}`
- `{date:yyyy_MM}`
- `{timestamp}`
- `{timestamp:millis}`
...
Determines the format of the data output. Options include:
