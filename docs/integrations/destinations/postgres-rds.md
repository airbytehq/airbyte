...
This page guides you through the process of setting up the Postgres RDS destination connector.

## Prerequisites

To use the Postgres RDS destination, you'll need:
...
You'll need the following information to configure the Postgres RDS destination:
...
## Step 2: Set up the Postgres RDS connector in Airbyte
...
Note, that Airbyte Postgres RDS destination will create tables and schemas using the Unquoted
...
3. On the Set up the destination page, enter the name for the Postgres RDS connector
   and select **Postgres RDS** from the Destination type dropdown.
...
The Postgres RDS destination connector supports the
...
## Tutorials
Now that you have set up the Postgres RDS destination connector, check out the following tutorials:
