...
The Customer.io destination connector allows you to sync data to [Customer.io](https://customer.io), a customer engagement platform. This connector uses the [Customer.io Track API v2](https://docs.customer.io/integrations/api/track/) batch endpoint to identify people and track events. This connector supports [data activation](/platform/move-data/elt-data-activation).

## Prerequisites

- A [Customer.io](https://customer.io) account
- A Track API Key and Site ID from your Customer.io workspace
- Airbyte version 1.8 or later, or Airbyte Cloud

## Setup guide

To configure this connector, generate a Track API Key and obtain your Site ID from Customer.io:

1. In Customer.io, go to **Workspace Settings > API and webhook credentials**.
...
### Object storage for rejected records

This connector can optionally store [rejected records](/platform/move-data/rejected-records) in object storage such as S3. Records that can't be synced to Customer.io due to validation errors or other issues are written to the configured storage location.
...
The connector supports the following destination objects:

- **Person Identify** (`person_identify`): Identifies a [person](https://docs.customer.io/journeys/create-update-person/) in Customer.io and assigns attributes to them. Required field: `person_email`. All additional fields are sent as person attributes.
- **Person Event** (`person_event`): Tracks an [event](https://docs.customer.io/journeys/events/) for a person in Customer.io. Required fields: `person_email`, `event_name`. Optional fields: `event_id` (for event deduplication), `timestamp`.

## Limitations

The Customer.io Track API enforces the following limits:

- The Track API has a rate limit of 3,000 requests per 3 seconds.
- Maximum of 300 attributes per person.
- Attribute names must be 150 bytes or smaller.
- Attribute values must be 1,000 bytes or smaller.
- Event names must be 100 bytes or smaller.

For more details, refer to the [Customer.io Track API limits](https://docs.customer.io/integrations/api/track/#tag/Track-API-limits).
