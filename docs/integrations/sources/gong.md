...
| :------ | :--------- | :------------------------------------------------------- | :------------------------------------------------------------------------------ |
| 2.0.0 | 2026-09-07 | [85373](https://github.com/airbytehq/airbyte/pull/85373) | Fix two schema bugs in `extensiveCalls` stream: the `media` field changed. It lacked data type definition and is now declared as `"null"` or `object`. Additionally, the subfield `parties.items.properties.context` was changed from `object` or `null` to `array` or `null`. This change could cause breakings for users syncing the `extensiveCalls` stream. See the [migration guide](https://docs.airbyte.com/integrations/sources/gong-migrations). |
