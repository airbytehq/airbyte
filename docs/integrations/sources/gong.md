...
The call transcripts stream is a substream of the calls stream. Starting with version 1.3.0, it batches up to 100 call IDs into each `POST /v2/calls/transcript` request instead of sending one request per call. This keeps large backfills within Gong's rate limit and daily quota. Subsequent incremental syncs only fetch transcripts for new calls.
...
| :------ | :--------- | :------------------------------------------------------- | :------------------------------------------------------------------------------ |
| 1.3.0 | 2026-07-22 | [82260](https://github.com/airbytehq/airbyte/pull/82260) | Batch `call transcripts` requests to fetch up to 100 call IDs per API call, avoiding the rate-limit (429) errors and daily-quota exhaustion caused by one request per call |
