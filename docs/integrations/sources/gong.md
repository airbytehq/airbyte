...
| :------ | :--------- | :------------------------------------------------------- | :------------------------------------------------------------------------------ |
| 1.3.6 | 2026-09-07 | [85371](https://github.com/airbytehq/airbyte/pull/85371) | Fix two schema bugs in `extensiveCalls` stream: fix `parties` from object to array type and add missing types `null` and `object` to `media`. This change could cause breakings for users syncing the `extensiveCalls` stream. See the [migration guide](https://docs.airbyte.com/integrations/sources/gong-migrations). |
