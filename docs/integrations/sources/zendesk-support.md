...
The connector's **Number of concurrent threads** setting (default: 4) controls how many streams sync in parallel. If your plan supports higher rate limits, increase this value for faster syncs. The minimum is 2 and the maximum is 40. Single-threaded syncs stall, so configurations saved with one thread are raised to 2 automatically on upgrade to 5.6.0.
