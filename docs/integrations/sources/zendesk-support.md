...
The connector's **Number of concurrent threads** setting (default: 4) controls how many streams sync in parallel. If your plan supports higher rate limits, increase this value for faster syncs. The minimum is 2 and the maximum is 40. Single-threaded syncs stall, so a saved value of 1 is treated as 2 from the first sync after upgrading to 5.6.0, and the stored configuration is updated to match.
...
|:------------|:-----------|:---------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 5.6.0 | 2026-09-09 | [85760](https://github.com/airbytehq/airbyte/pull/85760) | Raise the minimum **Number of concurrent threads** to 2 and migrate existing single-thread configurations, which stalled on the heartbeat timeout during long `tickets` reads |
