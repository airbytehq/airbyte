...
The connector's **Number of concurrent threads** setting (default: 4) controls how many streams sync in parallel. If your plan supports higher rate limits, increase this value for faster syncs. The minimum is 2 and the maximum is 40.

Do not run the connector with a single thread. The `tickets` stream reads one long, unsliced walk of the incremental export endpoint, which is capped at 10 requests per minute. With one thread that walk blocks every other stream, so nothing emits a record or a state message while it runs and Airbyte cancels the sync on the heartbeat timeout. A second thread lets another stream keep emitting during the walk. Configurations that were saved with one thread are raised to 2 automatically on the first sync after upgrading to 5.6.0.
...
|:------------|:-----------|:---------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 5.6.0 | 2026-09-09 | [85760](https://github.com/airbytehq/airbyte/pull/85760) | Raise the minimum **Number of concurrent threads** to 2 and migrate existing single-thread configurations to 2. A single thread serializes every stream behind the long `tickets` incremental-export walk, so no stream can emit a record or state message during it and the sync is cancelled on the heartbeat timeout |
