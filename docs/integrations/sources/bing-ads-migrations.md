...
Version 3.0.0 expands the primary keys of all report streams so that they include every attribute (dimension) column the connector requests from Microsoft's reporting API. Without this fix, destinations performing incremental append+dedup silently collapsed rows that differed only on an attribute column the connector requested but did not include in the previous primary key.
