...
Harvest v3 supports two authentication paths, and the credential type must match the Greenhouse application type. **Authorization Code (`Client`)** requires Airbyte's Greenhouse partner OAuth app: in Airbyte Cloud, select **OAuth**, click **Authenticate**, and complete the Greenhouse consent flow; self-managed Airbyte uses the partner app with its registered redirect URI. **Client Credentials (`ClientCredentials`)** requires a customer-created **Harvest V3 (OAuth)** custom integration credential, which self-managed users can create in Greenhouse and enter directly in the source. These grants are not interchangeable: partner OAuth apps reject `client_credentials`, while custom integrations reject `authorization_code`.

Every Harvest v3 list endpoint requires Site Admin authorization. For Client Credentials, provide the optional Site Admin `sub` user ID unless the custom integration's service user is itself a Site Admin. For Authorization Code, the Greenhouse user who approves consent must be a Site Admin. Users who previously used an API key must obtain one of these OAuth credentials before setting up the new connection. After upgrading to 1.0.0, every existing connection must be authenticated again. If consent or custom integration access does not include the scopes required by an enabled stream, Greenhouse returns `403` and 1.0.0 fails the sync with a configuration error instead of silently yielding an empty stream.
