...
By default, syncs are run with a slice period of 7 days. PayPal caps a transaction search at 10K records, and the `transactions` stream automatically retries a rejected slice as smaller date ranges, so a `RESULTSET_TOO_LARGE` error no longer fails the sync. If a sync still fails with `Result set size is greater than the maximum limit` or `RESULTSET_TOO_LARGE`:
...
| :------ | :--------- | :------------------------------------------------------- | :--------------------------------------------------------------------------------------------------------------------------- |
| 2.6.46 | 2026-08-20 | [PRNUMBER](https://github.com/airbytehq/airbyte/pull/PRNUMBER) | Retry oversized `transactions` date slices as smaller date ranges instead of failing the sync with `RESULTSET_TOO_LARGE` |
