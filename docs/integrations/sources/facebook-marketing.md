...
If your sync fails with an "Invalid OAuth access token" or "Error validating access token" error, your access token is invalid, expired, or has been revoked. To resolve the issue:

1. Re-authenticate your Facebook connection in Airbyte by navigating to the source settings and clicking **Authenticate your account**.
2. If re-authentication does not resolve the issue, go to [facebook.com](https://www.facebook.com) > **Settings** > **Business Integrations**, remove the Airbyte app, and then re-authenticate again.
3. If you use a service account access token instead of OAuth, generate a new token from your app's dashboard under **Marketing API** > **Tools**, and update the connector configuration with the new token.

Access tokens can become invalid for several reasons, including password changes, expired sessions, or revoked app permissions.
...
|:-----------|:-----------|:---------------------------------------------------------|:--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 4.2.1      | 2026-02-09 | [72952](https://github.com/airbytehq/airbyte/pull/72952) | Classify "Invalid OAuth access token" as a configuration error                                                                                                                                                                                                                                    |
