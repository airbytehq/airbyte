...
Slack API [rate limits](https://api.slack.com/changelog/2025-05-terms-rate-limit-update-and-faq#what) for the [conversations.replies](https://api.slack.com/methods/conversations.replies) and [conversations.history](https://api.slack.com/methods/conversations.history) endpoints are one request per minute. The Source Slack adds appropriate requests policy to avoid exceeding rate limits while reading data. Due to that policy, syncs of Channel Messages and Threads streams can be slow. If you want to sync data from Users, Channels and Channel Members streams in the Source Slack and you want to receive data on them faster, it's recommended to create a separate connection for Channel Messages and Threads streams and other streams which rate limits policy is not that strict. Please note that Users, Channels and Channel Members streams are not being read with the one request per minute policy, so their read time is only depends on how much data should be extracted.
...
|:-----------|:-----------|:---------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 2.3.0      | 2025-08-14 | [64553](https://github.com/airbytehq/airbyte/pull/64553) | Add api budget for Threads and Channel Messages streams.                                                                                                               |
