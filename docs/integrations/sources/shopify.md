...
- [Order Customers](https://shopify.dev/api/admin-rest/latest/resources/order#top) — The nested `customer` object of each order, flattened into its own stream (one record per order, linked via `order_id`) so that individual customer attributes can be selected, hashed, or excluded independently of the `orders` stream. Orders without a customer (guest checkouts) are skipped.
...
|:-----------|:-----------|:---------------------------------------------------------|:------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 4.2.0 | 2026-09-25 | [PR_NUMBER](https://github.com/airbytehq/airbyte/pull/PR_NUMBER) | Add `order_customers` stream that flattens the nested `orders.customer` object into top-level columns for field-level selection and hashing |
