...
- If the connector can't read the flag (for example, because Shopify throttled the request), `Market Countries` fails with a transient error rather than writing an empty table, and the next sync attempt retries it.
