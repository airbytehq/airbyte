...
|:------------|:-----------|:---------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 2.0.3 | 2026-05-22 | [78368](https://github.com/airbytehq/airbyte/pull/78368) | Add per-worker random jitter to the `search_analytics_*` rate-limit backoff via an inline Jinja `random` filter (uniform integer in `[45, 74]` seconds, replacing the constant `60s`) to de-synchronize concurrent worker wake-ups and reduce peak memory during rate-limit storms. Manifest-only — no `components.py` changes. See airbytehq/oncall#12246 for context. |
