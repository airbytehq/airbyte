...
8. (Optional) **Exponential Backoff Factor** is hidden in the UI, so it can only be set through the API or Terraform provider. It controls how aggressively the connector backs off when Apple's API returns rate-limit (`429`) or server (`5xx`) errors. Valid values are 1 to 20 and the default is 5; raise it for very large accounts that frequently hit rate limits.
