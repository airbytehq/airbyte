...
- `CampaignDetails` has no date dimension, so it's full refresh only: every sync reads from `start_time` through the current date in a single request. What your destination does with those rows depends on the sync mode you choose for the stream.
- The connector calls MNTN's synchronous reporting endpoint. MNTN [recommends its asynchronous `/batch` endpoint](https://help.mountain.com/en/articles/8017927-generate-api-requests-from-reports) for requests that cover more than four months of data, return high-cardinality data, or time out. `CampaignDetails` requests your entire window in one call, so keep that guidance in mind when you set an early `start_time`.
