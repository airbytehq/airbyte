...
|:-----------|:-----------|:------------------------------------------------------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 2.4.0 | 2026-09-10 | [PLACEHOLDER](https://github.com/airbytehq/airbyte/pull/PLACEHOLDER) | Declarative migration Step 9 (part 1) - move the `releases`, `projects_v2` and `pull_request_stats` streams to the manifest. These streams now reduce the GraphQL page size and retry the same page when GitHub answers 502/504, instead of retrying the identical oversized query and eventually failing the stream; lowering "Page size for large streams" by hand is no longer needed to get past a timeout. `pull_request_stats` now always reads newest-first and stops at the first already-synced pull request, rather than only doing so on syncs that already had state |
