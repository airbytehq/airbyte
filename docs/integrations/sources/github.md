...
| :----------- | :----------- | :------------------------------------------------------------------------------------------------------------------ | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2.7.2 | 2026-09-22 | [86914](https://github.com/airbytehq/airbyte/pull/86914) | Restore the GraphQL page size of the `reviews`, `issue_reactions` and `pull_request_comment_reactions` streams to 100. 2.7.0 migrated them at 10 — the default reserved for the two streams the legacy code marked `large_stream` — which made these three issue ten times as many GraphQL requests as before the migration. Also reduce the GraphQL page size when GitHub abandons a query and reports it as an HTTP 200 with a null `data` and `errors` in the response body, not only when it reports it as a 502 or 504. Matching the status alone left that form retrying the same oversized page until the attempts ran out and the stream failed. This does not cover a partial response, where `data` is populated and an `errors` entry is scoped to a single unresolvable node: a smaller page cannot resolve that node, so such a stream still fails. A repository the token cannot read is still skipped rather than reduced. Also stop reading the deprecated `page_size_for_large_streams` setting. It was removed from the connector's configuration form in 1.0.1 and has not been settable from the UI since, but it was still read from stored configurations, where a single value steered both the REST streams (`pull_requests`, `comments`, `issues`, `review_comments`) and the two GraphQL ones — and it steered them in opposite directions, since a value high enough to speed the REST streams up pushes `releases` and `pull_request_stats` back into the query timeouts the connector reduces the page size to survive. Those six streams now use the page size the connector shipped before the declarative migration, 10. A source that still carries the setting through the API or Terraform keeps syncing unchanged, but the value no longer affects any page size |
