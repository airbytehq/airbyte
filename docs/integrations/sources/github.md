...
| :----------- | :----------- | :------------------------------------------------------------------------------------------------------------------ | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2.7.2 | 2026-09-23 | [TBD](https://github.com/airbytehq/airbyte/pull/TBD) | Replace the custom `WorkflowRunsPaginationStrategy` of `workflow_runs` with a CDK `CursorPagination` `stop_condition` that reads the slice start from `stream_interval`. The stream no longer sends an internal `X-Airbyte-Window-Start` request header. Records and the 32-day look-back for re-run workflows are unchanged |
