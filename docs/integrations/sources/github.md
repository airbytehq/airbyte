...
Starting in version 2.1.28, the `releases` stream uses the smaller large-stream page size by default because repositories with long release notes can return HTTP 504 Gateway Timeout when each GraphQL page asks for too many releases. If a large GraphQL stream still fails with HTTP 502 or 504 errors, reduce the **Page size for large streams** option below its current value if this option is available in your Airbyte deployment. Smaller pages make each request cheaper for GitHub to process, but can increase the total sync time because the connector makes more requests.
