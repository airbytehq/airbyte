...
The connector now paces these calls conservatively at about 200 requests per day per endpoint, so large reporting backfills take longer but are less likely to exhaust the quota in one sync. This quota is account-wide and shared across private-key integrations, so the connector cannot guarantee absolute protection; OAuth applications receive a separate quota.
