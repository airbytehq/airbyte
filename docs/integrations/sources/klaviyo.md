...
The connector now paces these calls at about 225 requests per day per endpoint, matching the documented quota, so it is spread across the day instead of being spent in the first couple of hours, and large reporting backfills take longer. This quota is account-wide and shared across private-key integrations, so the connector cannot guarantee absolute protection; OAuth applications receive a separate quota.
...
|:--------|:-----------|:-----------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 3.0.2 | 2026-08-25 | [85043](https://github.com/airbytehq/airbyte/pull/85043) | Pace `flow_series_reports` and `campaign_values_reports` requests to stay within Klaviyo's per-endpoint 225/day reporting quota. |
