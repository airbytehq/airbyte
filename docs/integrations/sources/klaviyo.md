...
The connector is restricted by Klaviyo [rate limits](https://developers.klaviyo.com/en/docs/rate_limits_and_error_handling). When Klaviyo answers a request with `429`, the connector waits for the number of seconds in the response's `Retry-After` header and then retries. Waits for burst and steady limits are seconds to about a minute, and syncs recover from them without any action from you.

The connector waits at most 10 minutes. The reporting endpoints behind the **Campaign Values Reports** and **Flow Series Reports** streams also enforce a daily quota of 225 requests. Once a sync exhausts that quota, `Retry-After` holds the seconds remaining until the next daily reset, which can be 20 hours or more, so the connector fails the sync with a rate limit error rather than idling until then. To stay inside the quota, narrow the conversion metrics you request with **Report Stream Conversion Metric IDs**, sync these two streams less frequently, or move them to their own connection so a quota failure doesn't interrupt your other streams. Connector versions before 3.0.1 waited for the full `Retry-After` value on every stream, which stalled the sync until Airbyte stopped the attempt and restarted it.
