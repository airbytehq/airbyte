...
| `messages`         | Messages from all text-capable channels. Paginated, starting from oldest to newest.            |
...
- **Server Members Intent**: The `members` stream requires the `GUILD_MEMBERS` privileged intent. Without it, requests to list guild members will return a 403 error (which the connector handles gracefully by skipping).
- **Messages pagination**: The `messages` stream performs a full refresh starting from the oldest message. For channels with very large message histories, the initial sync may take a long time.
...
- **Channel permissions**: The connector gracefully handles channels where the bot lacks access by skipping those channels (403 errors are ignored for the `messages`, `pinned_messages`, `members`, `threads`, `scheduled_events`, and `audit_log` streams).
...
| :------ | :--------- | :----------- | :------------------------------ |
| 0.1.0   | 2026-04-16 | TBD          | Initial release of source-discord connector with 9 streams: guilds, channels, messages, members, roles, threads, pinned_messages, scheduled_events, and audit_log. |
