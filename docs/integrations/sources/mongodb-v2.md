...
| Size of the queue (Advanced)               | How many change events the connector may hold in memory at once during incremental (CDC) syncs. Lower this (for example to 10-100) for collections with large documents - hundreds of KB or more, especially with pre-images enabled - to avoid out-of-memory failures, since memory use scales with this value multiplied by your document size. Higher values can improve throughput for collections with small documents. Valid range: 10 to 10,000. Defaults to 10,000. |
...
|:--------|:-----------|:---------------------------------------------------------|:---------------------------------------------------------------------------------------------------------|
| 2.0.8 | 2026-09-09 | [81456](https://github.com/airbytehq/airbyte/pull/81456) | Allow the "Size of the queue" setting to go as low as 10, and make it also bound Debezium's `max.batch.size`, to prevent OOM on collections with large documents |
