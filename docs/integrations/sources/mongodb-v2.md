...
By default, when schema is enforced, all fields from documents are synced regardless of whether they were present in the discovered schema. This allows for richer downstream schemas without introducing sync fragility from schema changes in MongoDB. If you want stricter behavior where only discovered fields are synced, enable the "Fail Sync on Schema Mismatch" option.
...
| Fail Sync on Schema Mismatch               | When enabled, syncs will fail if a document contains fields not present in the discovered schema. When disabled (default), all fields from documents will be synced regardless of whether they were in the discovered schema. This allows for richer downstream schemas without introducing sync fragility from schema changes in MongoDB. Only applies when Schema Enforced is enabled.                                                    |
