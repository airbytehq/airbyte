...
| Update Capture Mode (Advanced)             | Determines how Airbyte retrieves the full document for update events during CDC syncs. **Lookup** (default): performs a separate read to fetch the current document state; if a document is updated multiple times rapidly, all events may reflect the final state rather than intermediate states (see [Troubleshooting: Lookup vs Post Image](/integrations/sources/mongodb-v2/mongodb-v2-troubleshooting#update-capture-mode-lookup-vs-post-image)). **Post Image**: uses MongoDB's built-in change stream post-images to capture the document state immediately after each change, avoiding this inconsistency. **IMPORTANT**: Post Image requires MongoDB 6.0+ and collections must be configured to [return pre and post images](https://www.mongodb.com/docs/manual/changeStreams/#change-streams-with-document-pre-and-post-images). Failure to do so will lead to data loss. |
