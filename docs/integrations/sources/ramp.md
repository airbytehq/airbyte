...
- A Ramp account with admin access, so you can create a developer app.
...
- **Production only.** The connector always calls `https://api.ramp.com`. Ramp's sandbox environment (`https://demo-api.ramp.com`) isn't supported, so use production credentials.
