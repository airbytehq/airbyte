...
For incremental streams, data is synced up to the previous day using your Google Ads account time zone since Google Ads can filter data only by [date](https://developers.google.com/google-ads/api/fields/v23/ad_group_ad#segments.date) without time. For report streams with daily or weekly segment data, Airbyte only queries data inside Google Ads' 37-month granular retention window; older data is skipped. If both `start_date` and `end_date` are older than the retention window, the connector checks availability using the earliest retained date and returns no historical records outside the retained window. If you use Overwrite sync mode, store historical data older than 37 months before upgrading to connector version 6.0.0 if you need to keep it. Existing older records in Append Dedup sync mode are preserved by that sync mode, but the connector no longer syncs them. Also, some reports cannot load data real-time due to Google Ads [limitations](https://support.google.com/google-ads/answer/2544985?hl=en).
...
Note that `segments.date` is automatically added to the `WHERE` clause if it is included in the `SELECT` clause. Custom reports including `segments.date` in the `SELECT` clause are synced by day for data inside Google Ads' 37-month granular retention window. Older report slices are skipped. If both `start_date` and `end_date` are older than the retention window, the connector checks availability using the earliest retained date and emits no rows for the configured historical date range. If the connection uses Overwrite sync mode, store any historical custom-query data older than 37 months before upgrading to connector version 6.0.0 if you need to keep it. Append Dedup sync mode preserves existing older records because of that sync mode, but the connector no longer emits them.
...
|:------------|:-----------|:---------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 6.0.0 | 2026-05-29 | [78504](https://github.com/airbytehq/airbyte/pull/78504) | Clamp incremental report start dates to Google Ads' 37-month granular data retention window. |
