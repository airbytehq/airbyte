...
- A Google account or JSON credentials for a service account with read access to your GCS bucket. For more details, see the [service account instructions](https://cloud.google.com/iam/docs/creating-managing-service-accounts)
- GCS bucket
...
4. Create a JSON key file for the service account. You provide the contents of this file as the **Service Account Information** when you set up the connector.

### Grant permission to GCS

Using the service account ID from above, grant read access to your target bucket. See [Use IAM permissions](https://cloud.google.com/storage/docs/access-control/using-iam-permissions) for more details.
...
The connector reads files through the authenticated GCS client using `gs://{bucket}/{blob}` URIs for both authentication types. The `_ab_source_file_url` field on each record depends on the authentication type:
...
This option is deprecated and has no effect. Before version 0.10.32, the connector used signed URLs for Service Account authentication, and this option stripped the credential-bearing query parameters (such as `X-Goog-Credential` and `X-Goog-Signature`) from `_ab_source_file_url`. The connector no longer generates signed URLs, so `_ab_source_file_url` never contains these parameters. The option remains in the connector specification only so that existing configurations keep validating.
...
| 0.10.24 | 2026-07-24 | [82238](https://github.com/airbytehq/airbyte/pull/82238) | Bump base image to python-connector-base 4.1.1 (Python 3.13.14) |
