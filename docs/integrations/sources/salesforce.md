...
| :---- | :------: | :---------- |
| `auth_type` | Yes | Authentication method. Use `Client`. |
| `client_id` | Yes | Client ID for the Salesforce external client app or connected app. |
| `client_secret` | Yes | Client secret for the Salesforce external client app or connected app. |
| `refresh_token` | Yes | Salesforce refresh token. The OAuth app must allow the `api`, `web`, `refresh_token`, and `lightning` scopes. |
| `is_sandbox` | No | Set to `true` to authenticate against a Salesforce sandbox with `test.salesforce.com`. Defaults to `false`. |
| `start_date` | No | Date or date-time in `YYYY-MM-DD` or `YYYY-MM-DDTHH:mm:ssZ` format. Records updated before this value aren't replicated. If unset, the connector replicates records from the last two years. |
| `force_use_bulk_api` | No | Set to `true` to force the Bulk API for streams that contain fields the Bulk API doesn't support. Unsupported fields are ignored. Defaults to `false`. |
| `stream_slice_step` | No | ISO 8601 duration for incremental stream time windows. Defaults to `P30D`. |
| `lookback_window` | No | ISO 8601 duration to re-read before the last cursor position during incremental syncs. Defaults to `PT10M`. |
| `streams_criteria` | No | Filters for discovered Salesforce objects. Each filter contains `criteria` and `value`. Use this to reduce the number of streams shown for Salesforce orgs with many objects. |
