...
const allSupportedColumnTypePropertyGenerators = [
  ...
  (i) =>
    dateProperty(`timestamp_w_tz_${i}`, "date-time", "timestamp_with_timezone"),
...
function generateWideSchema(columns) {
  ...
  while (columnCount < columns) {
    const propertyInfo =
      allSupportedColumnTypePropertyGenerators[propertyGeneratorIndex](
        columnCount,
      );
    ...
    if (
      propertyGeneratorIndex === allSupportedColumnTypePropertyGenerators.length
    ) {
...
export const SchemaGenerator = () => {
  ...
  function updateSchema(event) {
    const columns = parseInt(
      document.getElementById("schema-generator-column-count").value,
    );
  ...
  return (
    <div>
      <label for="schema-generator-column-count">
        Desired Number of Columns:
      </label>
      <input
        type="number"
        id="schema-generator-column-count"
        name="schema-generator-column-count"
