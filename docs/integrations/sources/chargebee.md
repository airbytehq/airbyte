...
|:------------|:-----------|:---------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 0.10.35-rc.1 | 2026-04-23 | [74345](https://github.com/airbytehq/airbyte/pull/74345) | Raise `default_concurrency` to 4 (Path B tuning calibrated to the Chargebee Starter tier, 150 req/min); bump `num_workers` minimum to 2 and default to 4; add `subscription_tier` spec field (enum `starter`/`performance`/`enterprise`, default `starter`); record original Starter and tier-aware `HTTPAPIBudget` shapes as YAML comments for activation in a follow-up GA RC; enable progressive rollout |
