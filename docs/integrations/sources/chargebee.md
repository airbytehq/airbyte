...
|:------------|:-----------|:---------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 0.10.35-rc.1 | 2026-04-23 | [74345](https://github.com/airbytehq/airbyte/pull/74345) | Raise `default_concurrency` to 14 (Path B tuning against Chargebee Enterprise tier, 3500 req/min); bump `num_workers` minimum to 2; add `subscription_tier` spec field (enum `starter`/`performance`/`enterprise`, default `starter`); record config-interpolated HTTPAPIBudget shape as YAML comments for follow-up rate-limit activation; enable progressive rollout |
