...
Twilio HTTP requests to the REST API are protected with HTTP Basic authentication. In short, you
...
[dashboard](https://www.twilio.com/user/account).
...
3. On the Set up the source page, enter the name for the Twilio connector and select **Twilio** from
   the Source/Destination type dropdown.
...
The Twilio connector will gracefully handle rate limits. For more information, see
