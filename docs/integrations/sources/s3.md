...
|:------------|:-----------|:----------------------------------------------------------------------------------------------------------------|:---------------------------------------------------------------------------------------------------------------------|
| 4.15.5 | 2026-05-26 | [78325](https://github.com/airbytehq/airbyte/pull/78325) | Migrate to `FileBasedConcurrentCursor`; bump airbyte-cdk to a prerelease that throttles file-based state emission, fixes the file-based source's cursor-dispatch fallback, and exposes a per-connector record-queue-size override; size the boto3 connection pool to the CDK default concurrency; cap glibc malloc arenas at process start; tighten the inter-worker record queue to 1 000; expose `concurrency_level` in the spec for per-connection tuning. Peak RSS on the affected dictionary-style streams drops from ~2.8 GB to ~0.85 GB. |
