...
**If you are syncing from a private bucket**, you will need to provide your `AWS Access Key ID` and `AWS Secret Access Key` to authenticate the connection, and ensure that the IAM user associated with the credentials has `read` and `list` permissions for the bucket. If you are unfamiliar with configuring AWS permissions, please follow these steps to create an IAM user with the necessary permissions and credentials:
...
:::caution
Your `Secret Access Key` will only be visible once upon creation. Be sure to copy it and store securely for future use.
:::
...
5. Enter the **Pattern of files to replicate**. If your bucket contains _only_ files containing data for this table, use `**` as path_pattern. See the [Path Patterns section](s3.md#path-patterns) for more specific pattern matching.
