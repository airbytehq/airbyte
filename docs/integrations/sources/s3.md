...
#### Skip Full Check for Parquet

Parquet streams expose a per-stream **Skip Full Check for Parquet** option (`skip_full_check_for_parquet`) that controls how thoroughly the connector validates files during the connection check. Set this option per stream that uses the Parquet format.

By default (`false`), the connection check lists matching files, opens the first one, and parses a record to confirm the file is valid Parquet and matches the configured schema. On large Parquet files, this parse step can load entire row groups into memory and cause the check to fail with an out-of-memory error.

When set to `true`, the connection check still validates the stream's configuration, lists matching files, and opens the first file to confirm it is reachable with the provided credentials, but skips the record-parse step. Enable this option if a stream's check fails on large Parquet files. Schema mismatches or corrupt files that the full check would have caught only surface at sync time instead of during the check.

This option has no effect on streams that use other file formats.
