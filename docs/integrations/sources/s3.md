...
There are no predefined streams. The streams are based on the content of your bucket.

## Limitations

- **Maximum file size**: Individual files cannot exceed 1.5 GB. Files larger than this limit are skipped during syncing.
- **Partitioned Parquet datasets**: Partitioned Parquet datasets (Hive-style partitioning) are not supported.
...
- **AWS Region**: The AWS region where the S3 bucket is located (for example, `us-east-1`). If not provided, the region is determined automatically.
...
- **Skip Full Check for Parquet**: When enabled, the connection test (CHECK) for Parquet streams verifies file accessibility but skips the full record-parse step. Enable this if you encounter out-of-memory errors during CHECK on large Parquet files.
...
|:------------|:-----------|:----------------------------------------------------------------------------------------------------------------|:---------------------------------------------------------------------------------------------------------------------|
| 4.15.15 | 2026-07-22 | [82237](https://github.com/airbytehq/airbyte/pull/82237) | Bump base image to python-connector-base 4.1.1 (Python 3.13.14) |
