...
**If you are syncing from a private bucket**, you will need to provide your `AWS Access Key ID` and `AWS Secret Access Key` to authenticate the connection, and ensure that the IAM user associated with the credentials has access to the correct bucket permissions. Please follow these steps to ensure proper configuration:

1. Log in to your Amazon AWS account and open the [IAM console](https://console.aws.amazon.com/iam/home#home).
2. Create or select an existing IAM user with the necessary permissions for accessing the S3 bucket. For more information on creating an IAM user, refer to the [official AWS documentation](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_users_create.html).
3. Generate the `AWS Access Key ID` and `AWS Secret Access Key` for the designated IAM user. For more information on generating these credentials, please refer
[to this page](https://docs.aws.amazon.com/IAM/latest/UserGuide/security-creds.html#access-keys-and-secret-access-keys).
4. Grant the IAM user associated with the credentials `read` and `list` access to the bucket using the following JSON:
...
The `list` permission is required to discover files based on the provided pattern(s). More information on bucket and user policies can be found [here](https://docs.aws.amazon.com/AmazonS3/latest/userguide/using-iam-policies.html).
...
5. If your bucket contains _only_ files containing data for this table, use `**` as path_pattern. See the [Path Patterns section](s3.md#path-patterns) for more specific pattern matching.
