...
|:------------|:-----------|:----------------------------------------------------------------------------------------------------------------|:---------------------------------------------------------------------------------------------------------------------|
| 4.15.5 | 2026-05-25 | [78325](https://github.com/airbytehq/airbyte/pull/78325) | Migrate to `FileBasedConcurrentCursor`, bump airbyte-cdk to a prerelease that throttles file-based state emission and bounds the inter-worker queue, size the boto3 connection pool to the CDK default concurrency, cap glibc malloc arenas at process start to keep concurrent-read peak RSS under the connector pod limit, and expose `concurrency_level` in the spec for per-connection tuning. |
