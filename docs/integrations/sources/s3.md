...
Once history is full, we drop the older messages out of the file, and only read files that were last modified between the date of the oldest file in history and `Days to Sync if History is Full` days prior.
