...
**If you are syncing from a private bucket**, you will need to provide your `AWS Access Key ID` and `AWS Secret Access Key` to authenticate the connection, and ensure that the IAM user associated with the credentials has `read` and `list` permissions for the bucket. If you are unfamiliar with configuring AWS permissions, please follow these steps to obtain the necessary permissions and credentials:
...
3. Select the **JSON** tab, then paste the following JSON into the Policy editor (be sure to add in your bucket name):
...
7. Toggle the **Optional fields** under the **Bucket** field to expand additional configuration options. **If you are syncing from a private bucket**, you must fill the **AWS Access Key ID** and **AWS Secret Access Key** fields with the appropriate credentials to authenticate the connection. All other fields are optional and may be left blank. Refer to the [S3 Provider Settings section](#s3-provider-settings) below for more information on each field.
...
- **Path Prefix**: An optional string that limits the files returned by AWS when listing files to only those starting with the specified prefix. This is different than the **Path Pattern**, as the prefix is applied directly to the API call made to S3, rather than being filtered within Airbyte. It does not accept pattern-style symbols like wildcards (`*`). We recommend using this filter if your bucket has many folders and files that are unrelated to this stream, and all the relevant files will always reside under the specified prefix.
  - Together with the **Path Pattern**, there are multiple ways to specify the files to sync. For example, all the following configurations are equivalent:
    ...
    - **Prefix** = `path1/path2/` and **Pattern** = `myFolder/**/*.csv`
    - **Prefix** = `path1/path2/myFolder/`, **Pattern** = `**/*.csv`. 

  :::note
  The last combination is the most efficient one, because the directories are filtered earlier in the S3 API call. However, the difference in efficiency is usually negligible.
  :::

  - The ability to individually configure the prefix and pattern has been included to accommodate as many use cases as possible. If you are unsure of the best approach, you can safely leave the **Path Prefix** field empty and just [set the Path Pattern](#path-patterns) to meet your requirements.
...
- **Delimiter**: Even though CSV is an acronymn for Comma Separated Values, it is used more generally as a term for flat file data that may or may not be comma separated. The delimiter field lets you specify which character acts as the separator. By default, this value is set to `,`.
- **Infer Datatypes**: This option determines whether a schema for the source should be inferred from the current data. When set to false and a custom schema is provided, the manually enforced schema takes precedence. If no custom schema is set and this option is also set to false, all fields will be read as strings. Enabled by default.
...
- **Escape Character**: An escape character can be used to prefix a reserved character and ensure correct parsing. Leaving this field blank (default option) will disallow escaping.
...
- **Double Quote**: This option determines whether two quotes in a quoted CSV value denote a single quote in the data. Enabled by default.
- **Allow newlines in values**: Also known as _multiline_, this option addresses situations where newline characters occur within text data. Typically, newline characters signify the end of a row in a CSV, but when this option is enabled, parsing correctly handles newlines within values. Disabled by default.
...
- **Block Size**: This is the number of bytes to process in memory at a time while reading files. The default value of `10000` is usually suitable, but if your table is particularly wide \(lots of columns / data in fields is large\), increasing this might help avoid schema detection failures. 
...
The JSONL parser uses the [PyArrow library](https://arrow.apache.org/docs/python/index.html). Only the line-delimited JSON format is supported. For more detailed info, please refer to the [docs](https://arrow.apache.org/docs/python/generated/pyarrow.json.read_json.html)

- **Unexpected field behavior**: Set to `infer` by default.
