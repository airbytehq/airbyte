...
5. Enter the **Pattern of files to replicate**. This is a regular expression that allows Airbyte to pattern match the specific files to replicate. If your bucket contains _only_ files containing data for this table, use `**` as path_pattern. See the [Path Patterns section](s3.md#path-patterns) for more specific pattern matching.
