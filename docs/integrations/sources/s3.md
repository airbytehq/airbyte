...
### Markdown/PDF/Word Format (Experimental)

:::warning
The Markdown/PDF/Word format is currently an experimental feature and not subject to SLAs. Use at your own risk.
:::

The Markdown/PDF/Word format is a special format that allows you to extract text from Markdown, PDF, and Word documents. If selected, the connector will extract text from the documents and output it as a single column named `content`. The content of the document will contain markdown formatting converted from the original file format.

One record will be emitted for each document. If the content of the document is longer than 4MB, the document will be split up into multiple records, each with a maximum size of 4MB. The fields "chunk_number" (starting at 0) and "no_of_chunks" will be added to each record to indicate the order of the chunks and the total number of chunks. The field "id" will be added to each record containing the file name and chunk id and is suitable as a primary key.

:::warning
Keep in mind that deduping in the destination by the primary key will not work properly if files in the source can get smaller over time - if this happens it's possible stale chunks will be left in the destination. If this is a concern for your use case, it's recommended to run a full refresh sync from time to time to keep the destination clean.

Airbyte only supports incremental syncs for up to 10,000 files matching a single stream. If there are more files than this, all files that have been changed in the last days (as configured by "Days To Sync If History Is Full") will be synced.
:::
