...
JSON is essentialy an ubiquitous, lightweight, plain-text, key-value data-interchange file format. The following settings are available:

* `orient`: Expected JSON format, more info in the [Pandas documentation](https://pandas.pydata.org/docs/reference/api/pandas.io.json.read_json.html)
* `convert_dates`: Can be a boolean or a list of columns to be parsed as datetime objects. If True (the default), will try to parse default datelike columns (see `keep_default_dates`). If False, no dates will be converted. If a list of columns, they will be parsed as datetime objects along with the the default datelike columns (again, depending on `keep_default_dates`).
* `keep_default_dates`: A boolean. If True and `convert_dates` is not False, parse the default datelike columns as datetime objects, _i.e._ columns that verify one of these conditions:
    * it ends with '_at',
    * it ends with '_time',
    * it begins with 'timestamp',
    * it is 'modified', 
    * it is 'date'
* `lines`: If True (the default), read the JSON file as having one JSON object per line
* `chunk_size`: If `lines` is True, returns a JsonReader iterator to read batches of `chunk_size` lines instead of the whole file at once.
* `compression`: For on-the-fly decompression of on-disk data. If `infer`, then use gzip, bz2, zip or xz if the file name is a string ending in ‘.gz’, ‘.bz2’, ‘.zip’, or ‘xz’, respectively, and no decompression otherwise. If using `zip`, the ZIP file must contain only one data file to be read in. Set to None for no decompression.
* `encoding`: The encoding to use, defaults to `utf8`
* `nrows`: Only applies if `lines` is True. Read only the first `nrows` rows of the file. If `chunk_size` is not None, it takes precedence over `nrows`, _e.g._ :
    * if `chunk_size = 10` and `nrows = 1`, the first 10 lines will be read,
    * if `chunk_size = 10` and `nrows = 11`, the first 20 lines will be read

By default, the expected JSON format is line-delimited JSON (or JSONL), which will be streamed in batches based on the `chunk_size` and `nrows` values. Multi-line JSON is also supported with parameters `lines=False,chunk_size=None` but bear in mind the **whole file will be loaded in memory**.
