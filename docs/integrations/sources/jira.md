...
1. To get access to the Jira API you need to create an API token. Refer to the Jira [documentation](https://support.atlassian.com/atlassian-account/docs/manage-api-tokens-for-your-atlassian-account/) for instructions on how to create an API token.

### Step 2: Set up the Jira connector in Airbyte

1. Enter the **API Token** that you have created in the Jira API Token field. This field is used for Authorization to your account by BasicAuth.
2. Enter the **Domain** for your Jira account in the Domain field, e.g. `airbyteio.atlassian.net`. This field specifies the domain for your Jira account.
3. Enter the **Email** for your Jira account in the Email field. This is the email you used to generate the API token. This field is used for Authorization to your account by BasicAuth.
...
8. Toggle **Enable Experimental Streams** to enable experimental PullRequests stream. Note that this allows the use of experimental streams which rely on undocumented Jira API endpoints. For more information, refer to the [experimental tables documentation](https://docs.airbyte.com/integrations/sources/jira#experimental-tables).## Supported sync modes
