...
|:-----------|:-----------|:---------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 9.2.0 | 2026-08-05 | [83305](https://github.com/airbytehq/airbyte/pull/83305) | Request every documented column that Amazon accepts for each Sponsored Products, Sponsored Display, and `sbPurchasedProduct` report stream's `groupBy` — 370 columns across 26 streams. All changes are additive — refresh the source schema in each connection to pick up the new fields. |
| 9.1.0 | 2026-08-05 | [83744](https://github.com/airbytehq/airbyte/pull/83744) | Add `sponsored_brands_ads` (`POST /sb/v4/ads/list`), `sponsored_brands_ads_report_stream`, and `sponsored_brands_ads_report_stream_daily`; request the full documented column set — including every video metric the removed V2 `sponsored_brands_video_report_stream` returned — for the `sbCampaigns`, `sbAdGroup`, and `sbAds` report types; add `goal`, `isMultiAdGroupsEnabled`, `kpi`, `siteRestrictions`, and `targetedPGDealId` to the `sponsored_brands_campaigns` schema; retry throttling and server errors on the entity streams instead of treating them as an empty page. All changes are additive — refresh the source schema in each connection to pick up the new fields. |
