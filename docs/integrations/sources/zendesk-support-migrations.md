...
In order to improve the speed of syncs for the `TicketMetrics` stream, the `cursor_field` for `TicketsMetric` stream is updated to `_ab_updated_at`. It is necessary to refresh the data and schema for the affected stream. Additionally, the `generated_timestamp` field has been removed from the schema.

### Schema Changes - Added Field

| Stream Name        | Added Fields            | Removed Fields          |
| -------------------|-------------------------|-------------------------|
| `TicketMetrics`    | `_ab_updated_at`        | `generated_timestamp`   |
