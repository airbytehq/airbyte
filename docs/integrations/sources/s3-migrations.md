...
- [CSV] `newlines_in_values` option is deprecated: v3 used PyArrow's permissive parser and exposed a `newlines_in_values` toggle to allow newline characters inside quoted CSV values. v4 uses the file-based CDK's Python `csv` parser, which has no equivalent option; the field is dropped during config migration. Files with newlines embedded inside quoted values may now fail with `ERROR_PARSING_RECORD_MISMATCHED_COLUMNS` / `ERROR_PARSING_RECORD_MISMATCHED_ROWS` errors. As a workaround, set `ignore_errors_on_fields_mismatch: true` on the stream's CSV format to skip offending rows, ensure values with embedded newlines are fully double-quoted per RFC 4180, or pre-process files before landing them in S3. Starting with 4.15.4, a warning is emitted in the sync logs when the migration drops a `newlines_in_values: True` setting so the change is no longer silent.
