...
- [CSV] `newlines_in_values` option is deprecated: v3 exposed an "Allow newlines in values" toggle backed by PyArrow. The File-Based CDK parser used in v4 has no equivalent option, so the setting is dropped during migration (a warning is logged when it was enabled). The v4 parser still reads newlines inside fully double-quoted ([RFC 4180](https://www.rfc-editor.org/rfc/rfc4180)) values. If a file relies on unquoted embedded newlines and a sync fails with `ERROR_PARSING_RECORD_MISMATCHED_COLUMNS` / `ERROR_PARSING_RECORD_MISMATCHED_ROWS`, you can: set `ignore_errors_on_fields_mismatch: true` on the stream's CSV format to skip offending rows; ensure fields with embedded newlines are fully double-quoted and the file uses LF or standard CRLF line endings; or pre-process non-compliant files before ingest.
