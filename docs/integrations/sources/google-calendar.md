...
7. (Optional) For **Start Date**, enter the earliest `updated` timestamp for incremental `events` syncs in the format `YYYY-MM-DDTHH:mm:ss.SSSZ`. Defaults to 25 days ago; Google rejects bounds older than roughly 30 days.
...
5. Under **Authentication**, choose one of:
   ...
   - **Refresh token (manual):** enter a manually obtained refresh token granted the `calendar.readonly` scope, plus your **Client ID** and **Client Secret**.
...
7. (Optional) For **Start Date**, enter the earliest `updated` timestamp for incremental `events` syncs (`YYYY-MM-DDTHH:mm:ss.SSSZ`). Defaults to 25 days ago.
8. (Optional) For **Number of concurrent workers**, set the number of concurrent request workers (1–10, default 3).
9. Click **Set up source** and wait for the tests to complete.
...
The `events`, `acl`, and `freebusy` streams sync once per calendar. When **Calendar Id** is configured, only that calendar is synced; otherwise every calendar in the account's calendar list is synced.
...
|------------------|-------------------|--------------|----------------|
| 0.3.0 | 2026-09-18 | [86470](https://github.com/airbytehq/airbyte/pull/86470) | Add `acl` and `freebusy` streams, partition `events`/`acl`/`freebusy` over all calendars, make `calendarid` optional |
