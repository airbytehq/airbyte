...
| Vendor Inventory Report | 5.0.0 through 5.10.x | No report window was sent at all. Amazon rejected every request with `dataStartTime and dataEndTime must be supplied`, so the stream returned no records. The stream was also full refresh only. | 5.11.0 |
...
| `GET_FLAT_FILE_RETURNS_DATA_BY_RETURN_DATE` | 60 days |
...
|:-----------|:-----------|:----------------------------------------------------------|:------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 5.11.0 | 2026-09-23 | [86943](https://github.com/airbytehq/airbyte/pull/86943) | Give `GET_VENDOR_INVENTORY_REPORT` the daily date window and `endDate` cursor its sibling reports already have, making it incremental; hold the cursor back four days for Amazon's publishing lag |
