...
Incremental syncs track two cursors. The connector lists report files with the `createdAfter` parameter, using the creation time (`createTime`) of the newest report file it has already downloaded, so each sync only requests files YouTube published since the previous sync, including re-issued files with backfilled data. Within each report stream, the `date` field is the cursor, so records are emitted per day rather than from a timestamp within a day. Connections created before version 1.4.0 are migrated automatically; the first sync after upgrading may re-download a day or two of report files but doesn't skip any.
...
| :------ | :--- | :----------- | :------ |
| 1.4.0 | 2026-09-10 | [85335](https://github.com/airbytehq/airbyte/pull/85335) | Restore incremental syncs for report streams: only newly created report files are requested and downloaded instead of re-downloading every retained report on each sync. Reports that YouTube re-issues with corrected data are now picked up, where previously they were silently skipped. Existing connections are migrated automatically; no action is required. |
| 1.3.4 | 2026-09-08 | [85727](https://github.com/airbytehq/airbyte/pull/85727) | Update dependencies |
| 1.3.3 | 2026-08-26 | [84945](https://github.com/airbytehq/airbyte/pull/84945) | Fail fast with a clear transient error when the daily YouTube API project quota is exhausted, and correctly classify rate-limited and server-error responses |
...
| 0.1.5 | 2024-06-13 | [38546](https://github.com/airbytehq/airbyte/pull/38546) | [autopull] base image + poetry + up_to_date |
| 0.1.4 | 2023-05-26 | [26420](https://github.com/airbytehq/airbyte/pull/26420) | Migrate to advancedAuth |
| 0.1.3 | 2022-10-07 | [17454](https://github.com/airbytehq/airbyte/pull/17454) | Added custom backoff logic |
| 0.1.2 | 2022-09-29 | [17399](https://github.com/airbytehq/airbyte/pull/17399) | Fixed `403` error while `check connection` |
| 0.1.1 | 2022-08-22 | [15744](https://github.com/airbytehq/airbyte/pull/15744) | Fix `channel_basic_a2` schema fields data type |
| 0.1.0 | 2021-11-19 | [7407](https://github.com/airbytehq/airbyte/pull/7407) | Initial Release |
