...
| `binary`                                  | string                 |                                                                                                                                                                                                                                                                                                                                            |
| `blob`                                    | string                 |                                                                                                                                                                                                                                                                                                                                            |
| `date`                                    | string                 | ISO-8601 date string. In CDC mode, zero-date values (`0000-00-00`) are converted to Unix epoch (`1970-01-01`) for NOT NULL columns to prevent sync failures. For nullable columns, zero-dates pass through as NULL. In non-CDC syncs, zero-dates are NULL. If using a zero-date column as a cursor, convert to EPOCH.                      |
| `datetime`                                | string                 | ISO-8601 datetime string. In CDC mode, zero-date values (`0000-00-00 00:00:00`) are converted to Unix epoch (`1970-01-01T00:00:00`) for NOT NULL columns to prevent sync failures. For nullable columns, zero-dates pass through as NULL. In non-CDC syncs, zero-dates are NULL. If using a zero-date column as a cursor, convert to EPOCH.|
| `timestamp`                               | string                 | ISO-8601 timestamp string. Zero-date values will be converted to Unix epoch (`1970-01-01T00:00:00`) when using CDC mode. In non-CDC syncs, zero-dates will be NULL. If using a zero-date column as a cursor or if the column is NOT NULL, convert to EPOCH.                                                                                |
