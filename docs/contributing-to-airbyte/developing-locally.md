...
The `dev` version is compatible with any specific version. If you want to test version specific changes, you can run the `dev` mode at a specific version by setting the `DEV_VERSION` environment variable. When `VERSION=dev`, the `DEV_VERSION` variable will override the version the server is running on. Its value must be a valid airbyte version string. Otherwise, it will fall back to `dev`. For example:
