...
5. We prefer `import foo.bar.ClassName` over `import foo.bar.*`. Even in cases where we import multiple classses from the same package. This can be set going to `Preferences > Code Style > Java > Imports` and setting `Class count to use import with '*'` to 9999 and `Names count to use static import with '*' to 9999.
6. You're done!
