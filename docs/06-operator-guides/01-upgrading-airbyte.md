...
If you are running [Airbyte on Kubernetes](../05-deploying-airbyte/05-on-kubernetes.md), you will need to use one of the two processes defined [here](#upgrading-on-k8s-0270-alpha-and-above) that differ based on your Airbyte version.
...
2. Back to the terminal, migrate the local archive to the new version using the Migration App (packaged in a docker container).
   ...
   ```bash
   docker run --rm -v /tmp:/config airbyte/migration:0.39.35-alpha --\
   --input /config/airbyte_archive.tar.gz\
