...
- Airbyte version 1.6.0 and later, if installed and managed with Helm _and_ connected to an external database, storage, and secrets manager
...
- Airbyte versions that aren't connected to an external database, storage, and secrets manager

:::danger
Don't migrate to Helm chart V2 if you haven't set up an external database, storage, and secrets manager. These resources contain the data your Airbyte deployment needs to operate and a fresh deployment won't work without them. Airbyte is evaluating possible migration paths for open source deployments that don't meet this requirement.
:::
