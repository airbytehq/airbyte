...
| Use on-demand capacity (Cloud only)                                                  | When enabled, syncs always run even if committed capacity is exhausted, by consuming credits at a premium rate. This toggle also appears during connection creation. See [Monitor data worker usage](./manage-data-workers.md). |
