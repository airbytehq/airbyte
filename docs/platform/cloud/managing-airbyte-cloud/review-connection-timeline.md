...
Queued syncs may appear as "Cancelled" in the timeline if they were cancelled due to a connection update, the next scheduled run arriving, or the 8-hour timeout for connections with a manual schedule type. For more information about capacity queuing, see [Manage and monitor data workers](./manage-data-workers.md).
