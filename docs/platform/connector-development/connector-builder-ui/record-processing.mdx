...
For most connectors targeting APIs with variable schemas, the `Automatically import detected schema` toggle is the right starting point:
...
#### When the detected schema is not enough
...
- **The API has a metadata endpoint.** Some APIs expose endpoints that list all available fields and their types, such as `GET /custom-fields` or `GET /properties`. If such an endpoint exists, switching to the YAML editor and using a `DynamicSchemaLoader` component lets the connector query that endpoint at discover time and build the schema dynamically. This is the most robust approach for APIs with highly variable schemas.
