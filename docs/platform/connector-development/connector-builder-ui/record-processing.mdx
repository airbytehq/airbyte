...
### APIs with variable schemas

Many APIs return different fields depending on the account, customer configuration, or data model. Common examples include:

- **CRMs with custom fields** -- Platforms like HubSpot or Salesforce let users define custom properties on contacts, deals, or other objects. Each account's API responses include a different set of custom fields.
- **Configurable SaaS platforms** -- Project management, e-commerce, or enterprise resource planning tools where administrators add custom attributes, tags, or metadata fields that appear in API responses.
- **Multi-tenant APIs** -- APIs where the response schema varies by tenant, workspace, or organization because each has configured the underlying data model differently.
- **Versioned or feature-gated APIs** -- APIs where the set of fields depends on the customer's subscription tier or enabled features.

For example, one account's API might return:
...
You can't know all possible fields ahead of time when building the connector. The Connector Builder handles this through schema detection at connection setup time.

#### How schema detection works for variable APIs

When a user sets up a connection using your connector, Airbyte runs a **discover** step that determines the schema for each stream. For Connector Builder connectors, the schema used during discover is the declared schema configured at build time.

Airbyte then detects schema changes over time. If the API starts returning new fields, Airbyte can detect and propagate those changes depending on the connection's [schema change management](/platform/using-airbyte/schema-change-management) settings.

#### Recommended approach: metadata endpoint

Some APIs expose an endpoint that lists all available fields and their types, such as `GET /custom-fields` or `GET /properties`. If the API you're building a connector for has one, this is the preferred approach: switch to the YAML editor and use a `DynamicSchemaLoader` component to query that endpoint at discover time and build the schema dynamically. This produces an accurate, complete schema for each user's account without relying on sampling records.

#### Fallback approach: auto-detected schema

When no metadata endpoint is available, the `Automatically import detected schema` toggle is the right starting point:

1. **During development**, connect the Builder to a test account and run a test read. The Builder infers the schema from the returned records and auto-imports it as the declared schema.
2. **At connection setup**, when a user configures a connection with their own credentials, Airbyte runs discover using the declared schema. The fields present in the declared schema are available for column selection and destination table creation.
3. **Schema change detection** handles field drift over time. If the API starts returning additional fields for a user's account, Airbyte detects the change and either propagates it automatically or flags it for review, depending on the connection's schema change management settings.

If your test account has a representative set of fields, this approach works well. The declared schema serves as a baseline, and Airbyte's schema change management handles variations from there.

#### Limitations of the auto-detected schema

In some cases, the auto-detected schema might not cover all scenarios:

- **The test account has few custom fields.** If the test account doesn't represent typical usage, the declared schema misses fields that most users have. Those fields won't appear in syncs until Airbyte detects a schema change.
