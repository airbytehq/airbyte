...
The Connector Builder UI generates forms directly from Airbyte's component schema, ensuring that all available options and configurations are accessible through the interface. You can switch between the UI and YAML modes at any time to view or edit the underlying configuration.
