...
Now that you configured how to talk to the API, let's set up the stream of records that will be sent to a destination later on. To do so, click the button with the plus icon next to the "Streams" section in the side menu and fill out the form:
* Set the name to "Rates"
* Set the "URL path" to `/exchangerates_data/latest`
* Submit

Now the stream is configured and can be tested. To send test requests, supply testing values by clicking the "Testing Values" button in the top right and pasting your API key in the input.
...
The Exchange Rates API supports configuring a different base currency via query parameter - let's make this part of the user inputs that can be controlled by the user of the connector when configuring a source, similar to the API key.
...
According to the API documentation, we can read the exchange rates for a specific date by querying the `"/exchangerates_data/{date}"` endpoint instead of `"/exchangerates_data/latest"`.
...
* In a lot of cases the start and end date are injected into the request body or query parameters. However in the case of the Exchange Rates API it needs to be added to the path of the request, so disable the "Inject start/end time into outgoing HTTP request" options
* Expand the incremental sync options and enable "Split up interval" so that the connector will partition the dataset into chunks
...
When used in a connection, the connector will ensure exchange rates for the same day are not requested multiple times - the date of the last fetched record will be stored and the next scheduled sync will pick up from where the previous one stopped.
...
* Tested whether the connector works correctly in the Connector Builder
