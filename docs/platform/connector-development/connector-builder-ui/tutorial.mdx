...
In tis tutorial, you'll create an Airbyte connector using the Connector Builder UI to read and extract data from an API called the Exchange Rates API. The steps apply to other APIs you might be interested in integrating with in the future.

[Here's the API documentation](https://apilayer.com/marketplace/exchangerates_data-api). In this tutorial, you'll read data from the following endpoints:

- `Latest Rates Endpoint`
- `Historical Rates Endpoint`

Your goal is to implement a source connector with two streams containing exchange rates from a base currency to many other currencies. The output schema of our streams will look like this:
...
3. Once you're signed in, visit https://apilayer.com/marketplace/exchangerates_data-api and sign up for the Free tier.

4. Copy the API key on the page.

### Set up Airbyte

If you don't already, you need access to Airbyte. **Do one of the following**:
...
This API Key is not part of the connector itself. Instead, it will be set as part of the source configuration when configuring a connection based on your connector in a later step. You can find more information about authentication method on the [authentication concept page](./authentication).
...
This tutorial only glosses over the many features of the Connector Builder. Learn more on the following pages:
