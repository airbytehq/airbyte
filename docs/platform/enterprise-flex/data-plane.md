...
- **Pod's AWS identity**: the AWS identity that data plane pods run as. On EKS this is normally a Kubernetes service account linked to an IAM role through IAM roles for service accounts. The role name is your choice. A name that starts with `oidc-` is still just a role name. The OIDC provider is the trust mechanism, not the role.
