...
- **Pod's AWS identity**: the IAM role that data plane pods use for AWS requests. The pod obtains this identity through the deployment's configured AWS authentication method. The role name is your choice. A name that starts with `oidc-` is still just a role name; it does not establish the authentication method. If your cluster uses IAM roles for service accounts, the Kubernetes service account annotation and the OIDC provider trust are what link the pod to that role.
- **Workload role**: the IAM role behind the pod's AWS identity. It is separate from the secrets role unless you deliberately make them the same role.
- **Data plane credentials**: the `clientId` and `clientSecret` from step 2. They authenticate your deployment to Airbyte. They have nothing to do with AWS IAM. See the [credential map](#credential-map).
...
| A new connector fails after the storage configuration was changed | The workspace now points at a storage the callers cannot reach. | Which storage the workspace uses, and whether both callers can reach the new storage. |
...
Do not delete and recreate a secret storage to fix an access problem. Fix the IAM configuration instead. Connectors keep a reference to the storage that holds their credentials, so removing a storage leaves existing connectors pointing at a storage the callers may no longer be able to read.
