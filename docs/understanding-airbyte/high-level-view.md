...
---
flowchart LR
    W[fa:fa-display Airbyte Web UI]
    S[fa:fa-server Airbyte Server]
...
* `Airbyte Web UI`: An easy-to-use graphical interface for interacting with the Airbyte API.
* `WebApp Server`: Handles connection between UI and API.
* `Database Config & Jobs`: Stores all the connections information \(credentials, frequency...\).
* `Airbyte API`: Airbyte's main control plane. All operations in Airbyte such as creating sources, destinations, connections, managing configurations, etc.. are configured and invoked from the API.
* `Temporal Service`: Manages the task queue and workflows. 
