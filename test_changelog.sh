...
#
# Usage: ./update_changelog.sh <pr_number> <connector_name> "<message>"
#   pr_number: The GitHub Pull Request number (e.g., 12345)
#   connector_name: The name of the connector (e.g., source-postgres)
...
changelog_update() {
    ...
    # Using standard sed syntax to insert after a specific line number.
    sed -i.bak "${separator_line_num}a\\
${new_row}
" "$filepath"

    # Check if sed command was successful
    if [ $? -ne 0 ]; then
        echo "Error: sed command failed to insert the new row. Check ${filepath}.bak for the original."
    ...
    echo "Backup of the original file created at ${filepath}.bak"
...
changelog_update 123 'destination-bigquery' 'test message'

# Explicitly exit with 0 if the function completes successfully
exit 0
