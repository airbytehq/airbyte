...
class SegmentTrackingClientTest {
  ...
  void setup() {
    ...
    segmentTrackingClient =
        new SegmentTrackingClient(MOCK_TRACKING_IDENTITY, DEPLOYMENT, null, analytics);
  }

  @SuppressWarnings("OptionalGetWithoutIsPresent")
  @Test
  void testIdentify() {
    // equals is not defined on MessageBuilder, so we need to use ArgumentCaptor to inspect each
    // field
    // manually.
    final ArgumentCaptor<IdentifyMessage.Builder> mockBuilder =
        ArgumentCaptor.forClass(IdentifyMessage.Builder.class);
    ...
    final Map<String, Object> expectedTraits =
  ...
  void testIdentifyWithRole() {
    segmentTrackingClient =
        new SegmentTrackingClient((workspaceId) -> IDENTITY, DEPLOYMENT, "role", analytics);
    // equals is not defined on MessageBuilder, so we need to use ArgumentCaptor to inspect each
    // field
    // manually.
    final ArgumentCaptor<IdentifyMessage.Builder> mockBuilder =
  ...
  @Test
  void testTrack() {
    final ArgumentCaptor<TrackMessage.Builder> mockBuilder =
        ArgumentCaptor.forClass(TrackMessage.Builder.class);
    final ImmutableMap<String, Object> metadata =
        ImmutableMap.of("airbyte_version", AIRBYTE_VERSION);
