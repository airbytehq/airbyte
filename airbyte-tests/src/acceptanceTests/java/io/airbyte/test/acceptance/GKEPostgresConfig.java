...
/**
 * This class is used to provide information related to the test databases for running the {@link
 * AcceptanceTests} on GKE. We launch 2 postgres databases in GKE as pods which act as source and
 * destination and the tests run against them. In order to allow the test instance to connect to
...
public class GKEPostgresConfig {
  ...
  public static Map<Object, Object> dbConfig(
      final Type connectorType, final boolean hiddenPassword, final boolean withSchema) {
  ...
  public static Database getSourceDatabase() {
    return Databases.createPostgresDatabase(
        USERNAME, PASSWORD, "jdbc:postgresql://localhost:2000/postgresdb");
  }

  public static Database getDestinationDatabase() {
    return Databases.createPostgresDatabase(
        USERNAME, PASSWORD, "jdbc:postgresql://localhost:3000/postgresdb");
  }
}
