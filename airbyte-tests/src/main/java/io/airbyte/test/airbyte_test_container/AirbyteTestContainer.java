...
public class AirbyteTestContainer {
  ...
  public AirbyteTestContainer(
      final File dockerComposeFile,
  ...
  /**
   * Starts Airbyte docker-compose configuration. Will block until the server is reachable or it
   * times outs.
   */
  ...
  private static Map<String, String> prepareDockerComposeEnvVariables(final File envFile)
      throws IOException {
  ...
  private static File prepareDockerComposeFile(final File originalDockerComposeFile)
      throws IOException {
    final File cleanedDockerComposeFile =
        Files.createTempFile(Path.of("/tmp"), "docker_compose", "acceptance_test").toFile();
    ...
    while (true) {
      ...
      if (i == 10) {
        throw new IllegalStateException(
            "Airbyte took too long to start. Including last exception.", lastException);
      }
      Thread.sleep(5000);
      i++;
    }
  ...
  private void serviceLogConsumer(
      final DockerComposeContainer<?> composeContainer, final String service) {
  ...
  /**
   * This method is hacked from {@link org.testcontainers.containers.DockerComposeContainer#stop()}
   * We needed to do this to avoid removing the volumes when the container is stopped so that the
   * data persists and can be tested against in the second run
   */
  ...
  private void stopRetainVolumesInternal()
      throws InvocationTargetException, IllegalAccessException, NoSuchMethodException,
          NoSuchFieldException {
    final Class<? extends DockerComposeContainer> dockerComposeContainerClass =
        dockerComposeContainer.getClass();
    try {
      final Field ambassadorContainerField =
          dockerComposeContainerClass.getDeclaredField("ambassadorContainer");
      ambassadorContainerField.setAccessible(true);
      final SocatContainer ambassadorContainer =
          (SocatContainer) ambassadorContainerField.get(dockerComposeContainer);
      ambassadorContainer.stop();

      final String cmd = "down ";

      final Method runWithComposeMethod =
          dockerComposeContainerClass.getDeclaredMethod("runWithCompose", String.class);
      ...
      final Method randomProjectId =
          dockerComposeContainerClass.getDeclaredMethod("randomProjectId");
