 ...
 *
 * <p>This class mainly exists to help Airbyte instrument/debug application on Airbyte Cloud. Within
 ...
 *
 * <p>Open source users are free to turn this on and consume the same metrics.
 */
public class MetricSingleton {
  ...
  public static synchronized MetricSingleton getInstance() {
    if (instance == null) {
      throw new RuntimeException(
          "You must initialize configuration with the initializeMonitoringServiceDaemon() method before getting an instance.");
    }
    return instance;
  }
  ...
  public void setGauge(final String name, final double val, final String description) {
    validateNameAndCheckDescriptionExists(
        name,
        description,
        () ->
            ifPublish(
                ...
                () -> {
                  if (!nameToHistogram.containsKey(name)) {
                    final Histogram hist =
                        Histogram.build().name(name).help(description).register();
  ...
  private static void validateNameAndCheckDescriptionExists(
      final String name, final String description, final Runnable execute) {
    if (name.contains("-")) {
   ...
   * constructor) must be called ahead of recording time, in order to set up the monitoring daemon
   * and initialize the isPublish() configuration as true/false.
   *
   * @param monitorPort to publish metrics to
   */
  public static synchronized void initializeMonitoringServiceDaemon(
      final String monitorPort, final Map<String, String> mdc, final boolean publish) {
    if (instance != null) {
