...
def connection(ctx: click.Context, connection_name: str, source_path: str, destination_path: str):
    ...
    if not source.was_created:
        msg = f"The source defined at {source_path} does not exists. Please run octavia apply before creating this connection."
    ...
    if not destination.was_created:
        msg = f"The destination defined at {destination_path} does not exists. Please run octavia apply before creating this connection."
        raise resources.NonExistingResourceError(
            msg,
        )
