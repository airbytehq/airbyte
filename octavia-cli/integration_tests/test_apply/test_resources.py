...
def test_connection_lifecycle(source, destination, connection, workspace_id):
    ...
    # TODO(https://github.com/airbytehq/airbyte/issues/11432): assert no diff once we migrate the schedule schema.
    assert "Item root['schedule'] removed from dictionary." == connection.get_diff_with_remote_resource()
...
def test_connection_lifecycle_with_normalization(source, destination, connection_with_normalization, workspace_id):
    ...
    # TODO(https://github.com/airbytehq/airbyte/issues/11432): assert no diff once we migrate the schedule schema.
    assert "Item root['schedule'] removed from dictionary." == connection_with_normalization.get_diff_with_remote_resource()
