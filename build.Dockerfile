##################################
# Setup caching for node_modules #
##################################
FROM node:14.7-alpine AS node-cache

WORKDIR /code

COPY dataline-webapp/package.json dataline-webapp/package-lock.json ./
RUN npm install
RUN echo test > node_modules/source
...
COPY --from=node-cache /code/node_modules dataline-webapp/node_modules
