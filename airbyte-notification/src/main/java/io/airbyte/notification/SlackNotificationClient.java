 ...
 *
 * <p>This class also reads a resource YAML file that defines the template message to send.
 *
 * <p>It is stored as a YAML so that we can easily change the structure of the JSON data expected by
 * the API that we are posting to (and we can write multi-line strings more easily).
 *
 * <p>For example, slack API expects some text message in the { "text" : "Hello World" } field...
 */
public class SlackNotificationClient extends NotificationClient {
  ...
  private boolean notify(final String message) throws IOException, InterruptedException {
    final ImmutableMap<String, String> body =
        new Builder<String, String>().put("text", message).build();
    ...
    if (isSuccessfulHttpResponse(response.statusCode())) {
      ...
      final String errorMessage =
          String.format(
              "Failed to deliver notification (%s): %s", response.statusCode(), response.body());
